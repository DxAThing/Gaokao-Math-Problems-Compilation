...
即
\[
(2+2d)^{2}=2(2+10d)\Longleftrightarrow 4d(d-3)=0
\]
...
取投影平面 \(\alpha\) 为 \(xy\) 平面，令 \(O\) 为原点. 直角 \(AOB\) 的两条射线方向向量内积为零；将方向向量的第三个坐标删去，就得到两条射线在 \(\alpha\) 内的射影. 下面分别构造题目所列的几种投影角.
    ...
    取 \(\overrightarrow{OA}=(1,0,1)\)，\(\overrightarrow{OB}=(1,0,-1)\)，两向量内积为 \(0\)，而两射影均为 \((1,0)\)，故射影角可能为 \(0^{\circ}\). 取 \(\overrightarrow{OA}=(1,0,1)\)，\(\overrightarrow{OB}=(1,1,-1)\)，两向量内积仍为 \(0\)，两射影的内积为 \(1>0\)，故射影角可能为锐角. 取 \(\overrightarrow{OA}=(1,0,0)\)，\(\overrightarrow{OB}=(0,1,0)\)，两向量及其射影均互相垂直，故射影角可能为直角. 再取 \(\overrightarrow{OA}=(1,0,1)\)，\(\overrightarrow{OB}=(-1,1,1)\)，两向量内积为 \(0\)，而两射影的内积为 \(-1<0\)，故射影角可能为钝角. 最后取 \(\overrightarrow{OA}=(1,0,1)\)，\(\overrightarrow{OB}=(-1,0,1)\)，两向量内积为 \(0\)，而两射影方向相反，故射影角可能为 \(180^{\circ}\). 因此五种判断均正确，正确的序号为 \(12345\).
\end{solution}
\end{problem}
...
将原题给出的方程配方，得
\[
x^{2}+y^{2}-2x-2y+8=(x-1)^{2}+(y-1)^{2}+6=0
\]
即 \((x-1)^{2}+(y-1)^{2}=-6\). 左边对任意实数 \(x,y\) 都不小于零，不可能等于 \(-6\)，所以该方程不表示实圆. 因而原题条件下不存在切点 \(A\)、\(B\)，四边形 \(PACB\) 及其面积最小值均无法定义，不能确定填空答案.
\end{solution}
\end{problem}
...
\bitmapinlinefigure[0.48\linewidth][max width=0.48\linewidth]{img/2002/beijing_science_autumn/q18_fig1.png}
...
侧面 \(ABB_{1}A_{1}\) 与底面的二面角，在垂直于 \(AB\) 的截面内对应于水平位移为 \(v\)、竖直位移为 \(h\) 的直角三角形；相对侧面给出的相等二面角使两个水平位移相等，所以 \(v=b-v-d\)，即 \(v=\frac{b-d}{2}\). 同理，由另一对相对侧面的二面角相等，得 \(u=a-u-c\)，即 \(u=\frac{a-c}{2}\). 因此在上述截面内，二面角记为 \(\theta\)，则
\[
...
\]
两式相等时，\(t(b-d)=b\)，所以 \(z_{E}=\frac{bh}{b-d}\). 对另一侧面 \(BCC_{1}B_{1}\) 作完全相同的计算，得到 \(z_{F}=\frac{bh}{b-d}\). 于是 \(E\)、\(F\) 位于同一个与底面平行的平面内，故 \(EF\parallel\) 平面 \(ABCD\).
...
\cline{3-8}
 & & 被读机号 & 结果 & 被读机号 & 结果 & 被读机号 & 结果 \\
\hline
1 & \(v_1\) & 2 & \(v_1+v_2\) & 3 & \(\sum_{i=1}^{4}v_i\) & & \\
\hline
2 & \(v_2\) & 1 & \(v_2+v_1\) & 4 & \(\sum_{i=1}^{4}v_i\) & & \\
\hline
3 & \(v_3\) & 4 & \(v_3+v_4\) & 1 & \(\sum_{i=1}^{4}v_i\) & & \\
\hline
4 & \(v_4\) & 3 & \(v_4+v_3\) & 2 & \(\sum_{i=1}^{4}v_i\) & & \\
\hline
\end{tabular}
\endgroup
\end{center}
...
\begin{solution}
\begin{enumerate}
\item 一开始每台机器只含有一个数. 第一单位时间内，机器至多再读入一个数，所以经过一个单位时间后，每台机器至多含有两个初始数，不能得到四个数的总和. 因此至少需要两个单位时间. 上表给出一种两单位时间完成的方法：第一单位时间先将机器两两配对，得到 \(v_1+v_2\) 和 \(v_3+v_4\)；第二单位时间再让每台机器从另一组读取数据，于是四台机器都得到 \(\sum_{i=1}^{4}v_i\). 故最少需要 \(2\) 个单位时间.
