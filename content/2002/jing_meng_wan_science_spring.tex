...
\begin{problem}
对于任意两个复数 \(z_{1}=x_{1}+y_{1}\i\)，\(z_{2}=x_{2}+y_{2}\i\)（\(x_{1}\)，\(y_{1}\)，\(x_{2}\)，\(y_{2}\in\R\)），定义运算 \(\odot\) 为：\(z_{1}\odot z_{2}=x_{1}x_{2}+y_{1}y_{2}\). 设非零复数 \(w_{1}\)，\(w_{2}\) 在复平面内对应的点分别为 \(P_{1}\)，\(P_{2}\)，点 \(O\) 为坐标原点. 如果 \(w_{1}\odot w_{2}=0\)，那么在 \(\triangle P_{1}OP_{2}\) 中，\(\angle P_{1}OP_{2}\) 的大小为\fillinblank{}.
\end{problem}
