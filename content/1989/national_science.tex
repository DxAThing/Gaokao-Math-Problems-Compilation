...
\bitmapfigure[max width=0.34\linewidth,max totalheight=6.0cm]{img/1989/national_science/q18_fig1.png}
...
\begin{problem}
如图，在平行六面体 \(ABCD - A_1B_1C_1D_1\) 中，已知 \(AB = 5\)，\(AD = 4\)，\(AA_1 = 3\)，\(AB \perp AD\)，\(\angle A_1AB = \angle A_1AD = \frac{\pi}{3}\). （1）求证：顶点 \(A_1\) 在底面 \(ABCD\) 的射影 \(O\) 在 \(\angle BAD\) 的平分线上；（2）求这个平行六面体的体积.

\bitmapfigure[max width=0.44\linewidth,max totalheight=6.0cm]{img/1989/national_science/q20_fig1.png}
...
\begin{problem}
设 \(f(x)\) 是定义在区间 \((-\infty, +\infty)\) 上以 \(2\) 为周期的函数，对 \(k \in \Z\)，用 \(I_k\) 表示区间 \((2k - 1, 2k + 1)\)，已知当 \(x \in I_0\) 时，\(f(x) = x^2\). （1）求 \(f(x)\) 在 \(I_k\) 上的解析表达式；（2）对自然数 \(k\)，求集合 \(M_k = \left\{ a \mid \text{使方程 } f(x) = ax \text{ 在 } I_k \text{ 上有两个不等的实根} \right\}\).
\end{problem}
