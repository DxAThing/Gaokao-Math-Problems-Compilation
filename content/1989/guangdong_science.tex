...
\bitmapfigure[max width=0.98\linewidth,max totalheight=6.4cm]{img/1989/guangdong_science/q25_fig1.png}
...
\begin{problem}
在平面 \(\beta\) 内有 \(\triangle ABC\)，在平面 \(\beta\) 外有点 \(S\)，斜线 \(SA\perp AC\)，\(SB\perp BC\). 且斜线 \(SA\)、\(SB\) 分别与平面 \(\beta\) 所成的角相等. （1）求证：\(AC=BC\)；（2）又设点 \(S\) 与平面 \(\beta\) 的距离为 \(4\) 厘米，\(AC\perp BC\)，且 \(AB=6\) 厘米，求点 \(S\) 与直线 \(AB\) 的距离.
...
\begin{problem}
设 \(\i\) 是虚数单位，复数 \(z\) 和 \(\omega\) 满足 \(z\omega+2\i z-2\i\omega+1=0\). （1）如果 \(z\) 和 \(\omega\) 又满足 \(\overline{\omega}-z=2\i\)，求 \(z\) 和 \(\omega\) 的值；（2）求证：如果 \(\left| z \right|=\sqrt{3}\)，那么 \(\left| \omega-4\i \right|\) 的值是一个常数，并求出这个常数.
...
\begin{problem}
已知实数 \(a>0\)，函数 \(f(x)=x^2\log_4 a\). （1）如果方程 \(f(x-1)+2x=0\) 无实根，求 \(a\) 的取值范围；（2）如果 \(a=4\)，在抛物线 \(y=f\left( \frac{x}{2} \right)\) 上有两点 \(A(x_1,y_1)\) 和 \(B(x_2,y_2)\) 满足 \(\left| AB \right|=y_1+y_2+2\)，求证：点 \(A\)、\(B\) 和这抛物线的焦点三点共线.
...
\begin{problem}
设圆 \(C\) 的方程为 \(x^2+y^2-2x\left( \frac{1-\cos \theta}{1+\cos \theta} \right)-2y\tan \frac{\theta}{2}+\left( \frac{1-\cos \theta}{1+\cos \theta} \right)^2=0\)，式中 \(\theta\) 是实数，且 \(0<\theta<\pi\). （1）当 \(\theta\) 在区间 \((0,\pi)\) 内变动时，求圆 \(C\) 的圆心轨迹的方程（写成普通形式），并画出轨迹曲线的草图；（2）又设 \(\theta_1\)、\(\theta_2\)、\(\theta_3\) 都是区间 \((0,\pi)\) 内的实数，且 \(\theta_1\)、\(\theta_2\)、\(\theta_3\) 成为公差不为零的等差数列，当 \(\theta\) 依次取值 \(\theta_1\)、\(\theta_2\) 或 \(\theta_3\) 时，所对应的圆 \(C\) 的半径依次为 \(r_1\)、\(r_2\) 和 \(r_3\)，试问：\(r_1\)、\(r_2\)、\(r_3\) 是否成等比数列？为什么？
...
（1）化圆 \(C\) 的方程为
\[
\left( x-\frac{1-\cos \theta}{1+\cos \theta} \right)^2+\left( y-\tan \frac{\theta}{2} \right)^2=\tan^2 \frac{\theta}{2}.
\]
得圆 \(C\) 的圆心坐标为
\[
\begin{cases}
...
\[
r_1=\tan \frac{\theta_1}{2},\qquad r_2=\tan \frac{\theta_2}{2},\qquad r_3=\tan \frac{\theta_3}{2}.
\]
