...
\begin{problem}
计算 \(\lim\limits_{n\to\infty}\left( \frac{1}{n^2}+\frac{4}{n^2}+\frac{7}{n^2}+\dots+\frac{3n-2}{n^2} \right)=\)\fillinblank{}.
...
\begin{solution}
解不等式组 \(\left\{\begin{aligned}2+\log_{1/2}x&\geqslant 0,\\ \tan x&\geqslant 0.\end{aligned}\right.\) \(\circled{1}\) \(\circled{2}\). 由 \(\circled{1}\)，得 \(0<x\leqslant 4\)；由 \(\circled{2}\)，得 \(k\pi\leqslant x<k\pi+\frac{\pi}{2}\)，\(k\in\Z\). 所以函数 \(y\) 的定义域为 \(\left( 0,\frac{\pi}{2} \right)\cup [\pi,4]\).
\end{solution}
...
\begin{solution}
过 \(A\)，\(B\) 分别作 \(AC\)，\(BD\) 垂直于平面 \(M\)，\(N\)，垂足分别为 \(C\)，\(D\)，连 \(CE\)，\(DF\)，\(BC\) 和 \(DA\)，得 \(\angle ABC=\alpha\)，\(\angle BAD=\beta\). 在 \(\mathrm{Rt}\triangle ABC\) 和 \(\mathrm{Rt}\triangle ABD\) 中，分别有 \(AC=AB\sin\alpha\)，\(BD=AB\sin\beta\) \(\circled{1}\).

\solutionfigure{\bitmapfigure[max width=0.30\linewidth]{img/1989/shanghai/q5_solution_fig1.png}}

因为 \(AE\perp PQ\)，所以 \(CE\perp PQ\)（三垂线定理逆定理）. 故 \(\angle AEC\) 为二面角 \(M-PQ-N\) 的平面角，同理 \(\angle BFD\) 也为二面角 \(M-PQ-N\) 的平面角，于是 \(\angle AEC=\angle BFD\)，\(\mathrm{Rt}\triangle AEC\sim \mathrm{Rt}\triangle BFD\). 因此 \(\frac{AE}{BF}=\frac{AC}{BD}\). 将 \(\circled{1}\) 式代入，即有 \(\frac{AE}{BF}=\frac{\sin \alpha}{\sin \beta}\).
\end{solution}
...
\solutionfigure{\bitmapfigure[max width=0.24\linewidth]{img/1989/shanghai/q6_solution_fig1.png}}
...
\(1^{\odot}\) 当 \(-1<q<\frac{\sqrt{5}-1}{2}\) 且 \(q\neq 0\) 时，\(A_n>B_n\)；

\(2^{\odot}\) 当 \(q=\frac{\sqrt{5}-1}{2}\) 时，\(A_n=B_n\)；

\(3^{\odot}\) 当 \(q>\frac{\sqrt{5}-1}{2}\) 时，\(A_n<B_n\).

【法二】因为 \(b_n=a_{n+1}+a_{n+2}=a_1(1+q)q^n\)，得 \(\frac{b_{n+1}}{b_n}=q\)，\(q\neq 0\)，\(n\in \N\)，
因此数列 \(\{b_n\}\) 也是等比数列.

\(1^{\odot}\) 当 \(q=1\) 时，\(A_n=na_1\)，\(B_n=2na_1\)，
所以 \(A_n<B_n\).
...
\solutionfigure{\bitmapfigure[max width=0.22\linewidth]{img/1989/shanghai/q7_solution_fig1.png}}
...
\solutionfigure{\bitmapfigure[max width=0.22\linewidth]{img/1989/shanghai/q7_solution_fig2.png}}

有 \(|MF|=p\sqrt{1+k^2}\)，\(|FN|=\sqrt{x^2+y^2}=x\sqrt{1+k^2}\)，\(|MN|=|MF|+|FN|=(x+p)\sqrt{1+k^2}\)，
按题意有 \((p\sqrt{1+k^2})\cdot (x\sqrt{1+k^2})=(x+p)\sqrt{1+k^2}\). 因为 \(k=\frac{y}{x}\)，所以 \(p\sqrt{x^2+y^2}=x+p\) \(\circled{1}\)，
...
【法二】将 \(y=kx\) 代入 \(\circled{1}\)，得 \(x=\frac{p}{p\sqrt{1+k^2}-1}\)（由 \(x>0\)，得 \(k^2>\frac{1-p^2}{p^2}\)），
