\FigureLayoutDeclare{img/1989/shanghai/q28_solution_fig2.png}{3.8cm}{9bp 6bp 63bp 13bp}
...
\bitmapfigure[max width=0.42\linewidth]{img/1989/shanghai/q6_fig1.png}
\end{problem}
...
\begin{solution}
解不等式组 \(\left\{\begin{aligned}2+\log_{1/2}x&\geqslant 0,\\ \tan x&\geqslant 0\end{aligned}\right.\). 由第一式得 \(0<x\leqslant 4\)；由第二式得 \(k\pi\leqslant x<k\pi+\frac{\pi}{2}\)，\(k\in\Z\). 所以函数 \(y\) 的定义域为 \(\left( 0,\frac{\pi}{2} \right)\cup [\pi,4]\).
\end{solution}
...
\begin{solution}
设直线 \(l\) 的斜率为 \(k\). 直线 \(OP\) 的斜率 \(k_{OP}=-\frac{4}{3}\). 按题意，有 \(\left| \frac{k+4/3}{1-4k/3} \right|=1\)，即 \(3k+4=\pm(3-4k)\). 解得 \(k_1=-\frac{1}{7}\)，\(k_2=7\). 故所求直线方程是 \(x+7y-25=0\) 和 \(7x-y+25=0\).
\end{solution}
...
\begin{enumerate}
\item 求 \(r\) 的值；
\item 写出展开式中的第 \(4r\) 项和第 \(r+2\) 项.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 展开式中第 \(4r\) 项的二项式系数为 \(\mathrm C_{20}^{4r-1}\)，第 \(r+2\) 项的二项式系数为 \(\mathrm C_{20}^{r+1}\). 根据二项式系数的性质，当且仅当 \(4r-1=r+1\) 或 \((4r-1)+(r+1)=20\) 时，它们的二项式系数相等. 解得 \(r=\frac{2}{3}\)（不合题意，舍去），或 \(r=4\).
\item 当 \(r=4\) 时，第 \(4r\) 项是 \(T_{16}=\mathrm C_{20}^{15}\left( -x^2 \right)^{15}=-15504x^{30}\)，第 \(r+2\) 项是 \(T_6=\mathrm C_{20}^5\left( -x^2 \right)^5=-15504x^{10}\).
\end{enumerate}
\end{solution}
...
\bitmapfigure[max width=0.34\linewidth]{img/1989/shanghai/q5_fig1.png}
\end{problem}

\begin{solution}
过 \(A\)，\(B\) 分别作 \(AC\)，\(BD\) 垂直于平面 \(M\)，\(N\)，垂足分别为 \(C\)，\(D\)，连 \(CE\)，\(DF\)，\(BC\) 和 \(DA\)，得 \(\angle ABC=\alpha\)，\(\angle BAD=\beta\). 在 \(\mathrm{Rt}\triangle ABC\) 和 \(\mathrm{Rt}\triangle ABD\) 中，分别有 \(AC=AB\sin\alpha\)，\(BD=AB\sin\beta\).

\solutionfigure{\bitmapfigure[max width=0.30\linewidth]{img/1989/shanghai/q25_solution_fig1.png}}

因为 \(AE\perp PQ\)，所以 \(CE\perp PQ\)（三垂线定理逆定理）. 故 \(\angle AEC\) 为二面角 \(M-PQ-N\) 的平面角，同理 \(\angle BFD\) 也为二面角 \(M-PQ-N\) 的平面角，于是 \(\angle AEC=\angle BFD\)，\(\mathrm{Rt}\triangle AEC\sim \mathrm{Rt}\triangle BFD\). 因此 \(\frac{AE}{BF}=\frac{AC}{BD}\). 将上述两式代入，即有 \(\frac{AE}{BF}=\frac{\sin \alpha}{\sin \beta}\).
\end{solution}

\begin{problem}
设复数集合 \(M=\left\{z\mid |z-2+\i|\leqslant 2,\ z\in \mathbf{C}\right\}\cap \left\{z\mid |z-2-\i|=|z-4+\i|,\ z\in \mathbf{C}\right\}\).
\begin{enumerate}
\item 试在复平面内作出表示集合 \(M\) 的图形，并说出图形的名称；
\item 求集合 \(M\) 中元素 \(z\) 的辐角主值的范围；
\item 求集合 \(M\) 中元素 \(z\) 的模的范围.
\end{enumerate}
\end{problem}
...
\solutionfigure{\bitmapfigure[max width=0.24\linewidth]{img/1989/shanghai/q26_solution_fig1.png}}

则集合 \(M\) 的图形是图中直线段 \(AB\).

\item 解方程组 \(\left\{\begin{aligned}(x-2)^2+(y+1)^2&=4,\\ x-y&=3\end{aligned}\right.\) 得 \(A(2+\sqrt{2},\sqrt{2}-1)\)，\(B(2-\sqrt{2},-\sqrt{2}-1)\). 记点 \(A\)，\(B\) 所对应的复数的辐角主值分别为 \(\theta_1\)，\(\theta_2\)，则 \(\theta_1=\arctan\frac{3\sqrt{2}-4}{2}\)，\(\theta_2=2\pi-\arctan\frac{3\sqrt{2}+4}{2}\). 所以集合 \(M\) 中元素 \(z\) 的辐角主值的范围是 \(\left[ 2\pi-\arctan\frac{3\sqrt{2}+4}{2},2\pi \right)\cup \left[ 0,\arctan\frac{3\sqrt{2}-4}{2} \right]\).

\item \(|OA|=\sqrt{9+2\sqrt{2}}\)，\(|OB|=\sqrt{9-2\sqrt{2}}\). 作 \(OP\perp AB\)，垂足为 \(P\). 由 \(\left\{\begin{aligned}x-y&=3,\\ x+y&=0\end{aligned}\right.\) 解得 \(P\left( \frac{3}{2},-\frac{3}{2} \right)\)，\(|OP|=\frac{3\sqrt{2}}{2}\). 因为点 \(P\) 在圆 \((x-2)^2+(y+1)^2=4\) 内，又 \(|OA|>|OB|\)，所以集合 \(M\) 中元素 \(z\) 的模的范围是 \(\left[ \frac{3\sqrt{2}}{2},\sqrt{9+2\sqrt{2}} \right]\).
\end{enumerate}
\end{solution}
...
\begin{solution}
【法一】因为 \(b_n=a_{n+1}+a_{n+2}=a_n(q^2+q)\)，得 \(B_n=(q^2+q)A_n\).
当 \(q>0\) 时，\(a_n=a_1q^{n-1}>0\)，所以 \(A_n>0\)；当 \(-1<q<0\) 时，\(A_n=\frac{a_1(1-q^n)}{1-q}>0\)，
...
\begin{solution}
\begin{enumerate}
\item 【法一】作 \(FK\perp l\)，垂足为 \(K\)，以 \(F\) 为极点，\(FK\) 的反向延长线为极轴，建立极坐标系. 设动点 \(N\) 的极坐标为 \((\rho,\theta)\). 根据题意，不妨取 \(\rho>0\)，\(\cos\theta>0\).

\solutionfigure{\bitmapfigure[max width=0.22\linewidth]{img/1989/shanghai/q28_solution_fig1.png}}

则 \(|MF|=\frac{p}{\cos\theta}\)，\(|FN|=\rho\)，从而 \(|MN|=|MF|+|FN|=\frac{p}{\cos\theta}+\rho\)，
由点 \(N\) 所满足的条件，得 \(\frac{p}{\cos\theta}\cdot \rho=\frac{p}{\cos\theta}+\rho\)，
故所求轨迹的极坐标方程为 \(\rho=\frac{1}{1-\frac{1}{p}\cos\theta}\)（\(0<\cos\theta<p\)）.
...
\solutionfigure{\bitmapfigure[max width=0.22\linewidth]{img/1989/shanghai/q28_solution_fig2.png}}

有 \(|MF|=p\sqrt{1+k^2}\)，\(|FN|=\sqrt{x^2+y^2}=x\sqrt{1+k^2}\)，\(|MN|=|MF|+|FN|=(x+p)\sqrt{1+k^2}\)，
按题意有 \((p\sqrt{1+k^2})\cdot (x\sqrt{1+k^2})=(x+p)\sqrt{1+k^2}\). 因为 \(k=\frac{y}{x}\)，所以 \(p\sqrt{x^2+y^2}=x+p\)，
化简得 \((p^2-1)x^2+p^2y^2-2px-p^2=0\)（\(x>0\)）.
...
【法二】将 \(y=kx\) 代入上式，得 \(x=\frac{p}{p\sqrt{1+k^2}-1}\)（由 \(x>0\)，得 \(k^2>\frac{1-p^2}{p^2}\)），
\(|MN|=(x+p)\sqrt{1+k^2}=\frac{p^2(1+k^2)}{p\sqrt{1+k^2}-1}\)，
令 \(k=\tan\theta\)（\(-\frac{\pi}{2}<\theta<\frac{\pi}{2}\)），由 \(\tan^2\theta>\frac{1-p^2}{p^2}\)，得 \(0<\cos\theta<p\). 则 \(|MN|=\frac{p^2}{\cos\theta (p-\cos\theta)}\).
以下同法一.
\end{enumerate}
\end{solution}
