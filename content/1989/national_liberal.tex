...
\examdisplaymath{\overline{M}\cap\overline{N}=\{b,e\}\cap\{a,c\}=\varnothing.}
...
\examdisplaymath{l=\sqrt{\left(\sqrt{2}\right)^2+2^2}=\sqrt{6}.}

所以圆锥的侧面积为

\examdisplaymath{\pi rl=\pi\cdot\sqrt{2}\cdot\sqrt{6}=2\sqrt{3}\pi.}
...
\examdisplaymath{a_1(1+q)=12,\qquad a_1q(1+q)=-6.}

因为 \(a_1(1+q)=12\neq0\)，两式相除得 \(q=-\dfrac12\). 于是 \(|q|<1\)，且

\examdisplaymath{a_1=\frac{12}{1+q}=\frac{12}{1-\frac12}=24.}

因此无穷等比数列的和为

\examdisplaymath{\lim_{n\to\infty}S_n=\frac{a_1}{1-q}=\frac{24}{1+\frac12}=16.}
...
\examdisplaymath{a_0+a_1+a_2+\cdots+a_7=(1-2)^7=-1.}

令 \(x=0\)，可得 \(a_0=1\). 因而

\examdisplaymath{a_1+a_2+\cdots+a_7=-1-a_0=-1-1=-2.}
...
\examdisplaymath{\sin^2\frac\theta2=\frac{1-\cos\theta}{2}=\frac{1+\frac15}{2}=\frac35.}
...
\examdisplaymath{x=2-X,\qquad y=-2-Y.}

代入原直线方程，得

\examdisplaymath{2(2-X)+3(-2-Y)-6=0,}
...
\examdisplaymath{d_1^2=R^2-5,\qquad d_2^2=R^2-8.}

面积为 \(5\pi\) 的截面半径较小，距离球心较远，因此 \(d_1-d_2=1\). 又

\examdisplaymath{d_1^2-d_2^2=3,}

所以 \((d_1-d_2)(d_1+d_2)=3\)，从而 \(d_1+d_2=3\). 解得 \(d_1=2\)，\(d_2=1\)，进而

\examdisplaymath{R^2=d_1^2+5=4+5=9,\qquad R=3.}
...
\examdisplaymath{2\times4!=2\times24=48.}
...
\examdisplaymath{c=\sqrt{8^2+6^2}=10,}

离心率为 \(e=\dfrac{c}{8}=\dfrac54\). 由双曲线的定义，点到右焦点的距离与点到右准线的距离之比等于离心率. 设点 \(P\) 到右准线的距离为 \(d\)，则

\examdisplaymath{\frac{8}{d}=\frac54,\qquad d=\frac{32}{5}.}
...
\examdisplaymath{-\frac{\sqrt2}{2}\leqslant t\leqslant\frac{\sqrt2}{2}.}

又因为 \(\cos^2x=1-\sin^2x\)，所以

\examdisplaymath{f(x)=1+t-t^2=\frac54-\left(t-\frac12\right)^2.}

这是关于 \(t\) 的开口向下的二次函数，故在上述闭区间上的最小值取在端点. 计算得

\examdisplaymath{f\left(-\frac\pi4\right)=1-\frac12-\frac{\sqrt2}{2}=\frac{1-\sqrt2}{2},}
...
\examdisplaymath{g'(x)=f'(2-x^2)(-2x)=\left(2-2(2-x^2)\right)(-2x)=4x(1-x^2).}
...
\examdisplaymath{k_{BC}=\frac{2-0}{1-(-1)}=1.}

所求直线与 \(BC\) 垂直，所以其斜率为 \(-1\). 该直线经过 \(A(1,0)\)，故

\examdisplaymath{y-0=-(x-1),}
...
\examdisplaymath{x^2-3x>4\quad\text{或}\quad x^2-3x<-4.}

第一种情况给出

\examdisplaymath{x^2-3x-4>0\Longleftrightarrow (x-4)(x+1)>0,}

所以 \(x<-1\) 或 \(x>4\). 第二种情况给出

\examdisplaymath{x^2-3x+4<0,}
...
\examdisplaymath{y+1=\frac{2t}{t+1}>0,\qquad 1-y=\frac{2}{t+1}>0,}

所以 \(-1<y<1\). 反过来，若 \(-1<y<1\)，则由 \(y=\dfrac{t-1}{t+1}\) 解得

\examdisplaymath{t=\frac{1+y}{1-y}>0,}
...
\begin{solution}
“\(A\) 是 \(B\) 的充分条件”表示 \(A\Rightarrow B\)，因此 \(B\) 是 \(A\) 的必要条件. 对这个命题取逆否命题，得到 \(\overline{B}\Rightarrow\overline{A}\)，所以 \(\overline{A}\) 是 \(\overline{B}\) 的充分条件.
\end{solution}
...
\examdisplaymath{a^{\log_b(x-3)}<1\Longleftrightarrow \log_b(x-3)>0.}

又因为 \(0<b<1\)，对数函数 \(\log_b t\) 在 \(t>0\) 上是减函数，且 \(\log_b1=0\)，故

\examdisplaymath{\log_b(x-3)>0\Longleftrightarrow 0<x-3<1.}
...
\examdisplaymath{\bs m=\frac{\sqrt2}{2}\bs e+\frac{\sqrt2}{2}\bs u,\qquad \bs n=\frac{\sqrt2}{2}\bs e+\frac{\sqrt2}{2}\bs v.}

于是

\examdisplaymath{\cos60^\circ=\bs m\cdot\bs n=\frac12+\frac12\bs u\cdot\bs v=\frac12+\frac12\cos\varphi.}
...
\examdisplaymath{1-\sqrt3\i=2\left(\cos\left(-\frac\pi3\right)+\i\sin\left(-\frac\pi3\right)\right).}

由棣莫弗定理，

\examdisplaymath{z=2^5\left(\cos\left(-\frac{5\pi}{3}\right)+\i\sin\left(-\frac{5\pi}{3}\right)\right)=32\left(\cos\frac\pi3+\i\sin\frac\pi3\right).}
...
\examdisplaymath{\tan\frac{3x}{2}-\tan\frac{x}{2}=\frac{\sin\left(\frac{3x}{2}-\frac{x}{2}\right)}{\cos\frac{3x}{2}\cos\frac{x}{2}}=\frac{\sin x}{\cos\frac{3x}{2}\cos\frac{x}{2}}.}

再由积化和差公式，

\examdisplaymath{\cos\frac{3x}{2}\cos\frac{x}{2}=\frac{\cos2x+\cos x}{2}.}

代入上式即得

\examdisplaymath{\tan\frac{3x}{2}-\tan\frac{x}{2}=\frac{2\sin x}{\cos x+\cos2x}.}
...
\examdisplaymath{x=AA_1\cos\angle A_1AB=3\cos\frac\pi3=\frac32,\qquad y=AA_1\cos\angle A_1AD=3\cos\frac\pi3=\frac32.}

所以 \(A_1\) 在底面上的射影为 \(O\left(\dfrac32,\dfrac32,0\right)\). 又因为 \(AB\perp AD\)，底面内 \(\angle BAD\) 的平分线方向是 \((1,1)\)，故 \(O\) 在 \(\angle BAD\) 的平分线上.

由 \(AA_1=3\)，得

\examdisplaymath{h=\sqrt{3^2-\left(\frac32\right)^2-\left(\frac32\right)^2}=\frac{3}{\sqrt2}.}

底面是矩形，面积为 \(AB\cdot AD=5\times4=20\). 因而平行六面体的体积为

\examdisplaymath{V=20\cdot\frac{3}{\sqrt2}=30\sqrt2.}
\end{solution}
...
\examdisplaymath{T_1=1\cdot2^2-2\cdot3^2=4-18=-14=-1\cdot2\cdot7.}

等式成立.

假设当 \(n=k\) 时等式成立，即

\examdisplaymath{T_k=-k(k+1)(4k+3).}

则

\examdisplaymath{\begin{gathered}T_{k+1}=T_k+(2k+1)(2k+2)^2-(2k+2)(2k+3)^2\\=-k(k+1)(4k+3)+(2k+2)\left((2k+1)(2k+2)-(2k+3)^2\right)\\=-k(k+1)(4k+3)-2(k+1)(6k+7)=-(k+1)(4k^2+15k+14)\\=-(k+1)(k+2)(4k+7).\end{gathered}}
...
\examdisplaymath{x-ak>0,\qquad x^2-a^2>0.}

又因为 \(\log_{a^2}u=\dfrac12\log_a u\)，原方程等价于

\examdisplaymath{2\log_a(x-ak)=\log_a(x^2-a^2).}

由于 \(a>0\) 且 \(a\neq1\)，对数函数 \(\log_a x\) 是一一对应的，所以

\examdisplaymath{(x-ak)^2=x^2-a^2.}

整理得

\examdisplaymath{-2akx+a^2k^2=-a^2.}

当 \(k=0\) 时上式变为 \(0=-a^2\)，不可能. 当 \(k\neq0\) 时，

\examdisplaymath{x=\frac{a(k^2+1)}{2k},\qquad x-ak=\frac{a(1-k^2)}{2k}.}

因为 \(a>0\)，定义域条件 \(x-ak>0\) 等价于

\examdisplaymath{\frac{1-k^2}{k}>0.}
...
\examdisplaymath{c=\sqrt{a^2-b^2}.}

与椭圆有公共焦点的双曲线只能取纵轴为实轴的形式，设为

\examdisplaymath{\frac{y^2}{u^2}-\frac{x^2}{v^2}=1,\qquad u>0,\ v>0.}

其焦半距满足 \(u^2+v^2=c^2\). 记两个曲线交点的横、纵坐标平方分别为 \(X=x^2\)、\(Y=y^2\)，则

\examdisplaymath{\frac{X}{b^2}+\frac{Y}{a^2}=1,\qquad \frac{Y}{u^2}-\frac{X}{v^2}=1.}

解这个关于 \(X,Y\) 的方程组，得

\examdisplaymath{X=\frac{b^2v^2(a^2-u^2)}{a^2v^2+b^2u^2},\qquad Y=\frac{a^2u^2(b^2+v^2)}{a^2v^2+b^2u^2}.}

由 \(u^2+v^2=c^2=a^2-b^2\)，有

\examdisplaymath{b^2+v^2=a^2-u^2,\qquad a^2v^2+b^2u^2=c^2(a^2-u^2).}

因此

\examdisplaymath{XY=\frac{a^2b^2u^2v^2}{c^4}.}

四个交点关于两坐标轴对称，组成的四边形为矩形，其面积为

\examdisplaymath{S=4\sqrt{XY}=\frac{4abuv}{c^2}.}

由均值不等式，

\examdisplaymath{uv\leqslant\frac{u^2+v^2}{2}=\frac{c^2}{2},}

当且仅当 \(u=v=\dfrac{c}{\sqrt2}\) 时取等号. 所以面积最大时的双曲线为

\examdisplaymath{\frac{y^2}{c^2/2}-\frac{x^2}{c^2/2}=1,\qquad\text{即}\qquad y^2-x^2=\frac{a^2-b^2}{2}.}

此时

\examdisplaymath{X=\frac{b^2}{2},\qquad Y=\frac{a^2}{2}.}

故四边形的四个顶点为

\examdisplaymath{\left(\frac{b}{\sqrt2},\frac{a}{\sqrt2}\right),\quad\left(-\frac{b}{\sqrt2},\frac{a}{\sqrt2}\right),\quad\left(-\frac{b}{\sqrt2},-\frac{a}{\sqrt2}\right),\quad\left(\frac{b}{\sqrt2},-\frac{a}{\sqrt2}\right),}
