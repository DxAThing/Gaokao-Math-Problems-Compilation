...
\begin{solution}
因为 \(\alpha\)，\(\beta\) 都是锐角，所以 \(\cos\alpha>0\)，\(\cos\beta>0\)，且 \(\cos(\alpha+\beta)=\cos\alpha\cos\beta-\sin\alpha\sin\beta<\cos\alpha\cos\beta<\cos\alpha+\cos\beta\). 因此 D 正确.

例如取 \(\alpha=\beta=30^{\circ}\)，可知前两项均不成立；取 \(\alpha=\beta=1^{\circ}\)，则 \(\cos(\alpha+\beta)>\sin\alpha+\sin\beta\)，故第三项也不是恒成立的，选 D.
\end{solution}
...
设 \(O\) 是正三角形 \(ABC\) 的中心. 因为 \(AE\perp BC\)，且正四面体的高 \(PO\perp\) 平面 \(ABC\)，又 \(O\in AE\)，所以 \(PO\) 是平面 \(PAE\) 内的一条直线. 由 \(DF\parallel BC\) 可得 \(DF\perp AE\)，同时 \(DF\perp PO\)，故 \(DF\perp\) 平面 \(PAE\)，结论 B 成立. 又因为 \(PO\subset\) 平面 \(PAE\) 且 \(PO\perp\) 平面 \(ABC\)，所以平面 \(PAE\perp\) 平面 \(ABC\)，结论 D 成立.
...
\begin{solution}
因为 \(f(x)=\lg x\) 的定义域为 \((0,+\infty)\). 结论 \(\circled{1}\) 一般不成立，例如取 \(x_1=1\)，\(x_2=2\) 时，左边为 \(\lg3\)，右边为 \(\lg2\). 结论 \(\circled{2}\) 成立，因为 \(\lg(x_1x_2)=\lg x_1+\lg x_2\).

函数 \(f(x)=\lg x\) 在 \((0,+\infty)\) 上单调递增，所以当 \(x_1\neq x_2\) 时，\(\dfrac{\lg x_1-\lg x_2}{x_1-x_2}>0\)，结论 \(\circled{3}\) 成立. 又因为 \(\lg x\) 在 \((0,+\infty)\) 上是严格凹函数，所以 \(\lg\left(\dfrac{x_1+x_2}{2}\right)>\dfrac{\lg x_1+\lg x_2}{2}\)，结论 \(\circled{4}\) 的不等号方向相反. 因此正确结论的序号是 \(\circled{2}\)、\(\circled{3}\).
\end{solution}
