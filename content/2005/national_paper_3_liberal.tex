...
\begin{solution}
原式有意义时，必须有 \(\cos\alpha\ne0\) 且 \(\cos2\alpha\ne0\)，因为 \(1+\cos2\alpha=2\cos^2\alpha\). 在此范围内，
\(\dfrac{2\sin2\alpha}{1+\cos2\alpha}\cdot\dfrac{\cos^2\alpha}{\cos2\alpha}=\dfrac{2\sin2\alpha}{2\cos^2\alpha}\cdot\dfrac{\cos^2\alpha}{\cos2\alpha}=\dfrac{\sin2\alpha}{\cos2\alpha}=\tan2\alpha\)，所以应选 B.
\end{solution}
...
\begin{solution}
由于四点不共面，它们不可能全在平面 \(\alpha\) 的同一侧且到 \(\alpha\) 的距离相等；否则四点会同在一个与 \(\alpha\) 平行的平面内. 它们也不可能都在 \(\alpha\) 上. 因此四点必须分成两个非空组，分别位于 \(\alpha\) 两侧的两平行平面上，而 \(\alpha\) 是这两平行平面的中间平面.

按 \(1+3\) 分组时，任选一个点作为单独的一组，共有 \(4\) 种；按 \(2+2\) 分组时，把四点分成两对共有 \(\dfrac{\mathrm C_4^2}{2}=3\) 种. 对于每一种分组，三点组确定一个平面；在 \(2+2\) 情形，两条由点对确定的异面直线唯一确定一对平行平面. 因四点不共面，每种分组都唯一确定相应的中间平面，所以共有 \(4+3=7\) 个，应选 D.
\end{solution}
...
在 \([0,2\pi]\) 上，\(\sin x\) 在 \((0,\pi)\) 内为正，在 \((\pi,2\pi)\) 内为负，并在 \(0\)、\(\pi\)、\(2\pi\) 处为零. 又
\(\sin x+\cos x=\sqrt2\sin\left(x+\dfrac\pi4\right)\)，所以它在 \(\dfrac{3\pi}{4}\)、\(\dfrac{7\pi}{4}\) 处为零，在 \(\left[0,\dfrac{3\pi}{4}\right)\cup\left(\dfrac{7\pi}{4},2\pi\right]\) 内为正，在 \(\left(\dfrac{3\pi}{4},\dfrac{7\pi}{4}\right)\) 内为负. 因而两因子同号且都不为零的区间恰为 \(\left(0,\dfrac{3\pi}{4}\right)\) 和 \(\left(\pi,\dfrac{7\pi}{4}\right)\). 因此使 \(f(x)\) 为正值的 \(x\) 的集合为 \(\left(0,\dfrac{3\pi}{4}\right)\cup\left(\pi,\dfrac{7\pi}{4}\right)\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 设甲、乙、丙三台机器需要照顾的概率分别为 \(p\)、\(q\)、\(r\). 由于三者相互独立，且三个两两同时需要照顾的概率均为正，所以 \(p\)、\(q\)、\(r\) 均为正数，并有 \(pq=0.05\)，\(pr=0.1\)，\(qr=0.125\). 因此
...
\begin{solution}
设截去的小正方形边长为 \(h\mathrm{~cm}\)，则容器的高为 \(h\mathrm{~cm}\). 为使底面宽度为正，必须有 \(0<h<24\). 容器底面的长、宽分别为 \((90-2h)\mathrm{~cm}\)、\((48-2h)\mathrm{cm}\)，所以以 \(\mathrm{cm}^3\) 为单位的容积函数的数值为
...
\(V(10)=10\times70\times28=19600\). 因而容器的高为 \(10\mathrm{~cm}\)，最大容积为 \(19600\mathrm{~cm}^3\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 将抛物线写成 \(x^2=\dfrac12y\). 与标准式 \(x^2=4ay\) 对比，得 \(4a=\dfrac12\)，所以焦点为 \(F\left(0,\dfrac18\right)\). 直线 \(l\) 是线段 \(AB\) 的垂直平分线，因此 \(F\) 在 \(l\) 上的充要条件是 \(FA=FB\). 由 \(A=(x_1,2x_1^2)\)、\(B=(x_2,2x_2^2)\)，有
\(FA^2=x_1^2+\left(2x_1^2-\dfrac18\right)^2\)，\(FB^2=x_2^2+\left(2x_2^2-\dfrac18\right)^2\)，从而
\(FA^2-FB^2=(x_1^2-x_2^2)\left[\dfrac12+4(x_1^2+x_2^2)\right]\).
方括号内的因子恒为正，故 \(FA=FB\) 当且仅当 \(x_1^2=x_2^2\). 因为 \(A\)、\(B\) 是抛物线上的两个不同点，必有 \(x_1\ne x_2\)，所以 \(x_1^2=x_2^2\) 又等价于 \(x_1+x_2=0\). 反过来，若 \(x_1+x_2=0\)，则 \(FA^2=FB^2\)，从而 \(F\in l\). 因此，当且仅当 \(x_1+x_2=0\) 时，直线 \(l\) 经过焦点 \(F\).
