...
\begin{solution}
由 \(x^2-3x=x(x-3)\)，得 \(N=\{0,3\}\). 又由 \(|x|\leqslant2\) 得 \(M=[-2,2]\)，所以 \(3\notin M\)，而 \(0\in M\). 因此 \(M\cap N=\{0\}\)，故选 B.
\end{solution}
...
\begin{solution}
利用 \(\i^2=-1\)，将等式左边化为 \((a-2\i)\i=a\i-2\i^2=2+a\i\). 与右边 \(b-\i=b-1\i\) 比较实部和虚部，得到 \(b=2\)，\(a=-1\). 因此 \(a^2+b^2=(-1)^2+2^2=5\)，故选 D.
\end{solution}
...
\begin{solution}
当 \(x\ne-3\) 时，\(x^2-9=(x+3)(x-3)\)，所以 \(\frac{x+3}{x^2-9}=\frac{1}{x-3}\). 在 \(x\to-3\) 时，分母 \(x-3\to-6\)，从而原极限为 \(-\frac{1}{6}\)，故选 A.
\end{solution}
...
\begin{solution}
正三角形 \(ABC\) 的面积为 \(S_{ABC}=\frac{\sqrt{3}}{4}\times1^2=\frac{\sqrt{3}}{4}\). 直棱柱的侧棱 \(BB'\) 垂直于底面 \(ABC\)，且 \(BB'=3\)，所以点 \(B'\) 到平面 \(ABC\) 的距离为 \(3\). 因此三棱锥 \(B'-ABC\) 的体积为
\[
V=\frac{1}{3}S_{ABC}\cdot BB'=\frac{1}{3}\cdot\frac{\sqrt{3}}{4}\cdot3=\frac{\sqrt{3}}{4}
\]
故选 D.
\end{solution}
...
\begin{solution}
由于焦点在 \(x\) 轴上，椭圆的长半轴平方为 \(a^2=2\)，短半轴平方为 \(b^2=m\)，且焦半距满足 \(c^2=a^2-b^2=2-m\). 由离心率 \(e=\frac{c}{a}=\frac12\)，得 \(\frac{2-m}{2}=e^2=\frac14\). 所以 \(2-m=\frac12\)，即 \(m=\frac32\)，故选 B.
\end{solution}
...
\begin{solution}
函数的导数为 \(f'(x)=3x^2-6x=3x(x-2)\). 当 \(x<0\) 或 \(x>2\) 时，\(f'(x)>0\)；当 \(0<x<2\) 时，\(f'(x)<0\). 因此 \(f(x)\) 在 \((0,2)\) 上是减函数，故选 D.
\end{solution}
...
\begin{solution}
对于第一条命题，若 \(l\) 与 \(m\) 共面，设它们所在的平面为 \(\gamma\). 由于 \(m\subset\alpha\)，且 \(l\cap\alpha=A\)，所以 \(A\in\gamma\cap\alpha\)，又 \(A\notin m\). 平面 \(\alpha\) 中的直线 \(m\) 与不在 \(m\) 上的点 \(A\) 唯一确定平面 \(\alpha\)，从而 \(\gamma=\alpha\)，这将导致 \(l\subset\alpha\)，与 \(l\cap\alpha=A\) 矛盾. 因此第一条命题正确.

对于第二条命题，异面直线 \(l\)，\(m\) 的方向不共线，且它们都平行于平面 \(\alpha\). 直线 \(n\) 同时垂直于 \(l\) 和 \(m\)，所以 \(n\) 的方向垂直于平面 \(\alpha\) 内两个不共线的方向，故 \(n\perp\alpha\)，第二条命题正确.

第三条命题不成立. 例如取两个互相平行的水平面 \(\alpha\)，\(\beta\)，在平行于它们的方向中分别取互相垂直的直线 \(l\)，\(m\)，则 \(l\parallel\alpha\)，\(m\parallel\beta\)，但 \(l\) 与 \(m\) 不平行. 因此第三条命题是假命题.

对于第四条命题，直线 \(l\)，\(m\) 在平面 \(\alpha\) 内相交，且两条直线都平行于平面 \(\beta\). 平面 \(\alpha\) 由这两条相交直线确定，它的两个相交方向都平行于平面 \(\beta\)，所以 \(\alpha\parallel\beta\)，第四条命题正确. 故选 C.
\end{solution}
...
\begin{solution}
两枚骰子的结果 \((X,Y)\) 共有 \(6\times6=36\) 个，且等可能. 因为 \(X\in\{1,2,3,4,5,6\}\)，所以 \(2X>1\)，对数底数均符合要求. 由 \(\log_{2X}Y=1\)，得 \(Y=2X\). 在 \(1\leqslant Y\leqslant6\) 的限制下，只有 \(X=1\)，\(2\)，\(3\) 可行，对应结果为 \((1,2)\)，\((2,4)\)，\((3,6)\)，共有 \(3\) 个. 因此所求概率为 \(\frac{3}{36}=\frac{1}{12}\)，故选 C.
\end{solution}
...
\begin{solution}
记平移后的函数为 \(h\). 由图可读出，平移后的折线经过 \((-2,0)\)，\((0,1)\)，\((1,3)\)，所以
\[
h(x)=
\begin{cases}
\frac{x}{2}+1,&-2\leqslant x\leqslant0,\\
2x+1,&0<x\leqslant1
\end{cases}
\]
平移关系为 \(h(x)=g(x+2)+1\)，故
...
g(x)=h(x-2)-1=
\begin{cases}
\frac{x}{2}-1,&0\leqslant x\leqslant2,\\
2x-4,&2<x\leqslant3
\end{cases}
\]
...
f(x)=
\begin{cases}
2x+2,&-1\leqslant x\leqslant0,\\
\frac{x}{2}+2,&0<x\leqslant2
\end{cases}
\]
故选 A.
\end{solution}
...
\begin{solution}
将递推式写为 \(2x_n-x_{n-1}-x_{n-2}=0\)，其特征方程为 \(2r^2-r-1=0\)，即 \((2r+1)(r-1)=0\). 因此数列的一般项可写为 \(x_n=A+B\left(-\frac12\right)^{n-1}\)，其中 \(A\)，\(B\) 为常数. 由 \(x_2=\frac{x_1}{2}\) 得 \(A-\frac{B}{2}=\frac{A+B}{2}\)，从而 \(A=2B\). 又因为 \(\left(-\frac12\right)^{n-1}\to0\)，所以 \(\lim_{n\to\infty}x_n=A=2\)，进而 \(B=1\). 于是 \(x_1=A+B=3\)，故选 B.
\end{solution}
...
\begin{solution}
由于根式在分母中，必须满足 \(1-\e^x>0\)，不能取等号. 于是 \(\e^x<1=\e^0\)，由指数函数的单调性得 \(x<0\). 因此定义域为 \((-\infty,0)\).
\end{solution}
...
两向量平行时对应坐标的行列式为零，因此
\[
\begin{vmatrix}2&3\\x&6\end{vmatrix}=2\times6-3x=0
\]
解得 \(x=4\).
\end{solution}
...
\begin{solution}
在 \((x\cos\theta+1)^5\) 中，\(x^2\) 项由五个因式中选出两个 \(x\cos\theta\) 相乘得到，其系数为 \(\mathrm C_5^2\cos^2\theta=10\cos^2\theta\). 在 \(\left(x+\frac54\right)^4\) 中，\(x^3\) 项由三个 \(x\) 和一个常数项相乘得到，其系数为 \(\mathrm C_4^3\cdot\frac54=5\). 由题意 \(10\cos^2\theta=5\)，得 \(\cos^2\theta=\frac12\)，所以 \(\cos\theta=\pm\frac{\sqrt2}{2}\).
\end{solution}
...
\begin{solution}
任取两条不平行的直线，它们有且只有一个交点；又因为任意三条直线不过同一点，所以不同直线对产生的交点彼此不同. 总的直线对数为 \(\mathrm C_n^2=\frac{n(n-1)}{2}\)，其中恰有一对平行直线不产生交点，因此 \(f(n)=\mathrm C_n^2-1=\frac{n(n-1)}{2}-1\). 对 \(n=4\) 代入得 \(f(4)=\frac{4\times3}{2}-1=5\).
\end{solution}
...
\begin{enumerate}
\item
建立空间直角坐标系，使 \(B\) 为原点，\(x\) 轴正向为 \(\overrightarrow{BP}\)，且 \(A\) 在 \(xOy\) 平面内，于是 \(P=(2\sqrt{34},0,0)\). 由
\[
PA^2+AB^2=6^2+10^2=136=PB^2
\]
知 \( \triangle PAB\) 在 \(A\) 处为直角三角形. 因此由投影关系，\(x_A=\frac{PB^2+AB^2-PA^2}{2PB}=\frac{50}{\sqrt{34}}\)，\(y_A=\sqrt{AB^2-x_A^2}=\frac{15\sqrt{34}}{17}\).
取 \(A\) 在 \(xOy\) 平面的正侧，则
\[
...
\(E=\left(\lambda\frac{50}{\sqrt{34}},\lambda\frac{15\sqrt{34}}{17},0\right)\)，\(F=\left(\lambda\frac{50}{\sqrt{34}},0,0\right)\).
...
\item
由 \(\lambda=\frac{9}{25}\)，
\(E=\left(\frac{18}{\sqrt{34}},\frac{27\sqrt{34}}{85},0\right)\)，\(F=\left(\frac{18}{\sqrt{34}},0,0\right)\).
...
\item
设
\(p=P(\text{取到黄球})=\frac{s}{s+t}\)，\(q=P(\text{取到白球})=\frac{t}{s+t}\).
...
\(x=10k+1\quad\text{或}\quad x=10k+3\)，\(k\in\Z\)
在 \([-2005,2005]\) 内，
\(-2005\leqslant10k+1\leqslant2005\)，\(\Longrightarrow\)，\(-200\leqslant k\leqslant200\)
以及
\(-2005\leqslant10k+3\leqslant2005\)，\(\Longrightarrow\)，\(-200\leqslant k\leqslant200\).
两类根不可能相等，因为 \(10k+1=10l+3\) 将推出 \(10(k-l)=2\). 每类根各有 \(401\) 个，所以根的个数为 \(401+401=802\).
\end{enumerate}
\end{solution}
...
\(y-\frac12=k\left(x+\frac{k}{2}\right)\)，即 \(y=kx+\frac{1+k^2}{2}\).
...
当 \(t>0\) 时，由 \(y=0\) 得 \(x_0=\frac{1+t^2}{2t}\)，由 \(y=1\) 得 \(x_1=\frac{t^2-1}{2t}\)，并且 \(y(0)=\frac{1+t^2}{2}\). 当 \(t=0\) 时，折痕为 \(y=\frac12\)，直接归入第一种情形.
