...
\begin{problem}
如图，在多面体 \(ABCDEF\) 中，已知 \(ABCD\) 是边长为 1 的正方形，且 \(\triangle ADE\)，\(\triangle BCF\) 均为正三角形，\(EF // AB\)，\(EF = 2\)，则该多面体的体积为（\quad）

\bitmapinclude[max width=0.48\linewidth,max totalheight=4.8cm]{img/2005/hebei_science/q5_fig1.png}
...
\begin{problem}
当 \(0 < x < \frac{\pi}{2}\) 时，函数 \(f(x) = \frac{1 + \cos 2x + \sin^2 x}{\sin 2x}\) 的最小值为（\quad）
...
\begin{center}
\begin{tabular}{@{}cccc@{}}
\shortstack[c]{\choicebitmap[max width=0.22\linewidth,max totalheight=3.2cm]{img/2005/hebei_science/q8_optA.png}\\\textbf{A.}} &
\shortstack[c]{\choicebitmap[max width=0.22\linewidth,max totalheight=3.2cm]{img/2005/hebei_science/q8_optB.png}\\\textbf{B.}} &
\shortstack[c]{\choicebitmap[max width=0.22\linewidth,max totalheight=3.2cm]{img/2005/hebei_science/q8_optC.png}\\\textbf{C.}} &
\shortstack[c]{\choicebitmap[max width=0.22\linewidth,max totalheight=3.2cm]{img/2005/hebei_science/q8_optD.png}\\\textbf{D.}}
\end{tabular}
\end{center}
\end{problem}
...
\begin{problem}
过三棱柱任意两个顶点的直线共15条，其中异面直线有（\quad）
...
\begin{problem}
\(\left(2x - \frac{1}{\sqrt{x}}\right)^8\) 的展开式中，常数项为\fillinblank{}.（用数字作答）
\end{problem}
...
\begin{problem}
在正方体 \(ABCD - A'B'C'D'\) 中，对角线 \(BD'\) 的一个平面交 \(AA'\) 于 \(E\)，交 \(CC'\) 于 \(F\)，则：
...
\begin{problem}
已知四棱锥 \(P - ABCD\) 的底面为直角梯形，\(AB // DC\)，\(\angle DAB = 90^{\circ}\)，\(PA \perp\) 底面 \(ABCD\)，且 \(PA = AD = DE = \frac{1}{2} AB = 1\)，\(M\) 是 \(PB\) 的中点.
...
\bitmapinclude[max width=0.42\linewidth,max totalheight=5.0cm]{img/2005/hebei_science/q18_fig1.png}
\end{problem}
...
\begin{problem}
9 粒种子分种在 3 个坑内，每坑 3 粒. 每粒种子发芽的概率为 0.5，若一个坑内至少有 1 粒种子发芽，则这个坑不需要补种，若一个坑里的种子都没发芽，则这个坑需要补种，假定每个坑至多补种一次. 每补种 1 个坑需 10 元，用 \(\xi\) 表示补种费用. 写出 \(\xi\) 的分布列并求 \(\xi\) 的数学期望.（精确到 0.01）
\end{problem}
