...
由集合加法的定义，\(P+Q\) 中的和为
\[
0+1=1,\quad 0+2=2,\quad 0+6=6,\quad 2+1=3,\quad 2+2=4,
\]
\[
2+6=8,\quad 5+1=6,\quad 5+2=7,\quad 5+6=11.
\]
...
\]
所以它在该区间上为严格凹函数，题中的不等式恒成立. 对于 \(\cos2x\)，有 \(\left(\cos2x\right)''=-4\cos2x\)，当 \(\frac\pi4<x<1\) 时二阶导数为正，因而不可能在整个区间上保持严格凹性，题中的不等式不恒成立. 因此只有 \(\log_2x\) 满足条件，函数个数为 \(1\)，故选 B.
\end{solution}
...
\begin{solution}
前四个命题均不恒成立. 第1个命题可取 \(b\) 为 \(x\) 轴，\(a\) 为过原点的 \(y\) 轴，\(c\) 为过原点的 \(z\) 轴，此时 \(a\perp b\)，\(b\perp c\)，但 \(a\) 与 \(c\) 不平行. 第2个命题可取 \(b\) 为 \(y\) 轴，\(a\) 为与 \(b\) 平行且位于平面 \(z=1\) 内的直线，\(c\) 为 \(x\) 轴，此时 \(b\perp c\)，但 \(a\) 与 \(c\) 异面，不能说 \(a\perp c\). 第3个命题中，取 \(\beta\) 为平面 \(z=0\)，取位于平面 \(z=1\) 内且方向与 \(x\) 轴相同的直线 \(a\)，再取平面 \(\beta\) 内方向与 \(y\) 轴相同的直线 \(b\)，则 \(a\parallel\beta\)，但 \(a\) 不平行于 \(b\). 第4个命题中，取 \(\beta\) 为平面 \(z=0\)，取平面 \(z=1\) 内的直线 \(a\) 与平面 \(z=2\) 内的直线 \(b\)，使二者方向不同，则 \(a\) 与 \(b\) 异面，且 \(b\parallel\beta\)，所以 \(b\) 不与 \(\beta\) 相交. 第5个命题是异面直线的公垂线唯一性的直接结论，故为真命题. 因此真命题只有 \(1\) 个，故选 A.
\end{solution}
...
\begin{solution}
以 \(D\) 为原点，分别以 \(DA\)，\(DC\)，\(DD_1\) 所在直线为 \(x\) 轴，\(y\) 轴，\(z\) 轴建立空间直角坐标系，则
\[
D(0,0,0),\quad A(2,0,0),\quad B(2,4,0),\quad C(0,4,0),\quad C_1(0,4,3).
\]
由于 \(E\) 在 \(BB_1\) 上且 \(BE=1\)，所以 \(E(2,4,1)\). 截面四边形 \(AEC_1F\) 的两组对边分别位于两组平行平面内，故 \(AEC_1F\) 是平行四边形. 设 \(F(0,0,z)\)，由
\[
\overrightarrow{AE}=(0,4,1),\quad \overrightarrow{FC_1}=(0,4,3-z)
\]
及 \(\overrightarrow{AE}=\overrightarrow{FC_1}\)，得 \(z=2\)，即 \(F(0,0,2)\).
\begin{enumerate}
\item 由点的坐标，
\[
...
\begin{solution}
\begin{enumerate}
\item 第一次更换工作时，某只初始灯泡不需要更换的概率为 \(p_1\). 五只灯泡都不需要更换，必须五只都能使用 \(1\) 年以上，因此概率为 \(p_1^5\). 恰好更换 \(2\) 只灯泡时，从五只灯泡中选出这 \(2\) 只，且其余 \(3\) 只均能使用 \(1\) 年以上，所以概率为
\[
