...
\item 由上式得 \(x=a\left(1-\dfrac{y}{b}\right)\). 代入抛物线方程 \(y^2=2px\)，得到
\[
...
\]
因此当 \(0<v<40\) 时，\(y'(v)>0\)；当 \(v>40\) 时，\(y'(v)<0\). 所以车流量在 \(v=40\text{ 千米/小时}\) 时取得最大值，且
\[
...
\begin{solution}
记 \(A_n=a_1+a_2+\cdots+a_n\)（\(n=1,2,\ldots,5\)），则 \(T=A_5\)，并且 \(y_n=\dfrac{A_n}{T}\)，其中 \(y_0=0\).
...
\item 对任意 \(n=1,2,3,4\)，由于 \(a_i<a_j\)（\(1\leqslant i\leqslant n<j\leqslant5\)），前 \(n\) 个正数的平均数小于后 \(5-n\) 个数的平均数，即
\[
