...
\(f'(x)=-\dfrac{1}{2-x}-\dfrac{1}{2+x}=-\dfrac{4}{4-x^2}<0\)，故它在 \([-1,1]\) 上单调递减. 选项 A 在该区间上单调递增，\(B\) 不是奇函数，\(C\) 是偶函数，因此选 D.
\end{solution}
...
\begin{solution}
设双曲线的焦半距为 \(c\)，则 \(c^2=a^2+b^2\)，右焦点为 \(F=(c,0)\)，右准线为 \(x=\dfrac{a^2}{c}\). 令 \(d=\dfrac{a^2}{c}\)，则渐近线与准线的交点为 \(P=\left(d,\dfrac{bd}{a}\right)\)，\(Q=\left(d,-\dfrac{bd}{a}\right)\). 因为 \(PQ\) 是竖直线，且在 \(P\)、\(Q\) 处的内积均不为零，所以直角在 \(F\) 处. 于是
\(\left(c-d\right)^2=\left(\dfrac{bd}{a}\right)^2\).

由于 \(c-d=\dfrac{c^2-a^2}{c}=\dfrac{b^2}{c}\)，而 \(\dfrac{bd}{a}=\dfrac{ab}{c}\)，故 \(b^2=ab\). 由 \(a,b>0\) 得 \(a=b\)，从而 \(c=\sqrt2a\)，所以 \(e=\dfrac ca=\sqrt2\).
\end{solution}
...
\begin{solution}
可行域的顶点依次为 \((0,0)\)，\((3,0)\)，\(\left(3,\dfrac32\right)\)，\((2,3)\)，\((1,4)\)，\((0,4)\). 在这些顶点处，目标函数值分别为 \(0\)，\(18\)，\(\dfrac{51}{2}\)，\(27\)，\(26\)，\(20\). 最大值为 \(27\)，对应的点为 \((2,3)\)，所以填 \((2,3)\).
\end{solution}
...
\begin{problem}
已知数列 \(\{a_{n}\}\) 的首项 \(a_1 = 5\)，而 \(n\) 项和为 \(S_{n}\)，且 \(S_{n+1} = 2S_{n} + n + 5\)（\(n \in \mathbf{N}^{*}\)）.
...
\begin{solution}
\begin{enumerate}
\item 由已知式，\(a_{n+1}=S_{n+1}-S_n=S_n+n+5\). 当 \(n=1\) 时，\(S_2=2S_1+1+5=16\)，所以 \(a_2=11=2a_1+1\). 对 \(n\geqslant2\)，由 \(S_n=2S_{n-1}+n+4\) 得
\(2a_n+1=2(S_n-S_{n-1})+1=S_n+n+5=a_{n+1}\). 因而 \(a_{n+1}+1=2(a_n+1)\)，且 \(a_1+1=6\)，所以 \(\{a_n+1\}\) 是首项为 \(6\)、公比为 \(2\) 的等比数列.
...
\item 设 \(u=\cot\alpha\)，\(v=\cot\beta\). 因为 \(A\)，\(B\) 是抛物线上异于原点的点，分别代入直线 \(OA\)，\(OB\) 的方程，可得
\(A=(2pu^2,2pu)\)，\(B=(2pv^2,2pv)\). 由 \(\tan(\alpha+\beta)=1\)，有
\(\dfrac{u+v}{uv-1}=1\)，即 \(u+v=uv-1\). 过上述两点的直线方程为
\(x-(u+v)y+2puv=0\). 令 \(s=u+v\)，则 \(uv=s+1\)，直线方程化为
