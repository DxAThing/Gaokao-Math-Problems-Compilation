...
\begin{center}
\begin{tabular}{@{}cc@{}}
\choicebitmap[max width=0.38\linewidth,max totalheight=4.4cm]{img/2005/shandong_liberal/q3_optA.png} &
\choicebitmap[max width=0.38\linewidth,max totalheight=4.4cm]{img/2005/shandong_liberal/q3_optB.png} \\
\choicebitmap[max width=0.38\linewidth,max totalheight=4.4cm]{img/2005/shandong_liberal/q3_optC.png} &
\choicebitmap[max width=0.38\linewidth,max totalheight=4.4cm]{img/2005/shandong_liberal/q3_optD.png}
\end{tabular}
\end{center}
\end{problem}
...
\begin{problem}
设 \(x\)，\(y\) 满足约束条件 \(\left\{ \begin{array}{l} x + y \leqslant 5, \\ 3x + 2y \leqslant 12, \\ 0 \leqslant x \leqslant 3, \\ 0 \leqslant y \leqslant 4 \end{array} \right.\) 则使得目标函数 \(z = 6x + 5y\) 的最大的点 \((x,y)\) 是\fillinblank{}.
\end{problem}
...
\begin{enumerate}
\item 求动圆圆心 \(C\) 的轨迹的方程；
\item 设 \(A\)，\(B\) 是轨迹 \(C\) 上异于原点 \(O\) 的两个不同点，直线 \(OA\) 和 \(OB\) 的倾斜角分别为 \(\alpha\) 和 \(\beta\)，当 \(\alpha\)，\(\beta\) 变化且 \(\alpha + \beta = \frac{\pi}{4}\)，证明直线 \(AB\) 恒过定点，并求出该定点的坐标.
\end{enumerate}
\end{problem}
...
\begin{center}
\bitmapinclude[max width=0.46\linewidth,max totalheight=6.0cm]{img/2005/shandong_liberal/q22_fig1.png}
\end{center}
