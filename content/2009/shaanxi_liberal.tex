...
\begin{problem}
设 \(x, y\) 满足约束条件 \(\begin{cases} x + y \geqslant 1, \\ x - y \geqslant -1, \\ 2x - y \leqslant 2, \end{cases}\) 目标函数 \(z = x + 2y\) 的最小值是\fillinblank{}，最大值是\fillinblank{}.
\end{problem}

\begin{problem}
如图球 \(O\) 的半径为 2，圆 \(O_{1}\) 是一小圆，\(O_{1}O = 2\). \(A, B\) 是圆 \(O_{1}\) 上两点. 若 \(A, B\) 两点间的球面距离为 \(\frac{2\pi}{3}\)，则 \(\angle AO_{1}B =\fillinblank{}\).
