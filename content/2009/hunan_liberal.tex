...
此时 \(f'(x)=3x^2-12x+c=3(x-2)^2+c-12\).
若 \(f(x)\) 在 \(x=t\) 处取得极小值，则 \(t\) 是导函数右侧的零点，因而 \(t>2\)，并且由 \(3(t-2)^2+c-12=0\)，得 \(c=12t-3t^2\).
于是
\[
g(t)=f(t)=t^3-6t^2+ct
=t^3-6t^2+(12t-3t^2)t
...
\begin{circlelist}
\item A 组：数列 \(\{x_n\}\) 是 \(B\)-数列；
\item A 组：数列 \(\{x_n\}\) 不是 \(B\)-数列；
\item B 组：数列 \(\{S_n\}\) 是 \(B\)-数列；
\item B 组：数列 \(\{S_n\}\) 不是 \(B\)-数列.
\end{circlelist}
