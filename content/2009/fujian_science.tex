...
\renewcommand{\bitmapinclude}[2][]{%
    \begingroup
    \adjustbox{max width=0.42\linewidth,#1}{\includegraphics{#2}}%
    \endgroup
}
\texfigure[max width=0.42\linewidth]{img_repaint/2009/fujian_science/q6_fig1.tex}
\endgroup
...
赛道的前一部分为曲线段 \(OSM\)，该曲线段为函数 \(y = A \sin \omega x\) （\(A > 0, \omega > 0\)），\(x \in [0,4]\) 的图象，且图象的最高点为 \(S(3, 2\sqrt{3})\)；赛道的后一部分为折线段 \(MNP\). 为保证参赛运动员的安全，限定 \(\angle MNP = 120^{\circ}\).
