...
\begin{solution}
原始试卷文件中第二组分组清晰印为 \((20,20]\). 若严格按该表字面理解，区间 \((10,40)\) 只包含 \((20,30)\) 和 \((30,40)\)，对应的频数之和为 \(24+15=39\)，频率为 \(\frac{39}{100}=0.39\)，对应选项 B.

但现稿答案为 C，且公开同题版本将第二组记为 \((10,20]\). 按这一修正版的连续分组，区间 \((10,40)\) 包含 \((10,20]\)、\((20,30)\) 和 \((30,40)\)，对应的频数之和为 \(13+24+15=52\)，频率为 \(\frac{52}{100}=0.52\)，对应选项 C. 因此本题存在原始试卷文件与答案及公开修正版之间的版本冲突，不能在不作勘误假设的情况下唯一确定选项；现稿答案 C 对应后一种读法.
\end{solution}
...
\begin{solution}
按四个候选俯视图分别还原为高为 \(1\) 的柱体：选项 A 对应立方体，体积为 \(1\)；选项 B 对应底面直径为 \(1\) 的圆柱，体积为 \(\frac{\pi}{4}\)；选项 C 对应底面直角边长均为 \(1\) 的直三棱柱，体积为 \(\frac{1}{2}\)；选项 D 对应底面半径为 \(1\) 的四分之一圆柱，体积为 \(\frac{\pi}{4}\). 这些柱体的正视图和侧视图都可以是边长为 \(1\) 的正方形，只有选项 C 的体积符合题设，故选 C.
\end{solution}

\begin{problem}
阅读如图所示的程序框图，运行相应的程序，输出的结果是（\quad）

\bitmapfigure[max width=0.32\linewidth]{img/2009/fujian_liberal/q6_fig1.png}
...
\begin{solution}
前两条不等式给出 \(y\geqslant1-x\) 和 \(x\leqslant1\)，第三条给出 \(y\leqslant ax+1\). 要使区域有有限的正面积，必须有 \(a>-1\). 此时两条斜边 \(y=1-x\) 与 \(y=ax+1\) 的交点为 \((0,1)\)，它们分别与 \(x=1\) 交于 \((1,0)\) 和 \((1,a+1)\)，所以区域的横坐标范围为 \(0\leqslant x\leqslant1\). 在横坐标为 \(x\) 处，区域的纵向长度为 \((ax+1)-(1-x)=(a+1)x\)，所以面积为 \(\int_0^1(a+1)x\,\mathrm{d}x=\frac{a+1}{2}\). 由题意 \(\frac{a+1}{2}=2\)，得 \(a=3\). 当 \(a=-1\) 时区域退化，当 \(a<-1\) 时区域向左无界，均不符合面积为 \(2\)，故选 D.
\end{solution}
