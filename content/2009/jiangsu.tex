...
\begin{solution}
\(z_1-z_2=(4+29\i)-(6+9\i)=-2+20\i\)，所以
\[
(z_1-z_2)\i=(-2+20\i)\i=-2\i+20\i^2=-20-2\i
\]
因此该复数的实部为 \(-20\).
\end{solution}
...
由向量数量积公式，
\[
\bs{a}\cdot\bs{b}=|\bs{a}|\,|\bs{b}|\cos30^\circ
=2\cdot\sqrt3\cdot\frac{\sqrt3}{2}=3
\]
所以填 \(3\).
\end{solution}
...
函数在 \(f'(x)<0\) 时单调递减. 由于
\[
(x-11)(x+1)<0\quad\Longleftrightarrow\quad -1<x<11
\]
所以函数的单调减区间为 \((-1,11)\).
\end{solution}
...
\begin{solution}
由图象可见，函数在 \(-\dfrac{2\pi}{3}\) 和 \(-\dfrac{\pi}{3}\) 处有相邻的两个零点，二者间距为 \(\dfrac{\pi}{3}\). 对于正弦函数，相邻零点的间距为 \(\dfrac{\pi}{\omega}\)，故
\[
\frac{\pi}{\omega}=\frac{\pi}{3}
\]
又 \(\omega>0\)，所以 \(\omega=3\).
\end{solution}
...
\begin{solution}
从 \(5\) 根竹竿中一次抽取 \(2\) 根，所有等可能结果共有
\[
\mathrm C_5^2=10
\]
个. 长度相差 \(0.3\text{ m}\) 的结果只有 \((2.5,2.8)\) 和 \((2.6,2.9)\)，共有 \(2\) 个. 因此所求概率为
\[
\frac{2}{10}=0.2
\]
\end{solution}
...
甲班数据的平均数为
\[
\bar{x}_{\text{甲}}=\frac{6+7+7+8+7}{5}=7
\]
所以方差为
\[
s_{\text{甲}}^2=\frac{(6-7)^2+(7-7)^2+(7-7)^2+(8-7)^2+(7-7)^2}{5}=\frac25=0.4
\]
乙班数据的平均数也是 \(7\)，方差为
\[
s_{\text{乙}}^2=\frac{(6-7)^2+(7-7)^2+(6-7)^2+(7-7)^2+(9-7)^2}{5}=\frac65=1.2
\]
因此较小的方差为 \(0.4\).
\end{solution}
...
\bitmapfigure[max width=0.55\linewidth]{img/2009/jiangsu/q7_flow.png}
\end{problem}
...
\begin{solution}
初始时 \(S=0\)，\(T=1\). 每次先作 \(S\leftarrow T^2-S\)，若 \(S<10\)，再作 \(T\leftarrow T+2\) 并重复计算. 因此各次计算得到
\((S,T)=(1,1)\)，\((8,3)\)，\((17,5)\).
前两次的 \(S\) 都小于 \(10\)，第三次得到 \(S=17\ge10\)，于是停止并输出
\[
W=S+T=17+5=22
\]
\end{solution}
...
\begin{solution}
两个正四面体相似，且相似比等于棱长比 \(1:2\). 相似立体的体积比等于相似比的立方，因此体积比为
\[
1^3:2^3=1:8
\]
\end{solution}
...
\begin{solution}
曲线方程为 \(y=x^3-10x+3\)，故其在点 \(P\) 处的切线斜率为
\[
y'=3x^2-10
\]
由斜率为 \(2\)，得
\(3x^2-10=2\)，\(\Longrightarrow\)，\(x^2=4\)
所以 \(x=\pm2\). 点 \(P\) 在第二象限，故 \(x=-2\)，再代入曲线方程得
\[
y=(-2)^3-10(-2)+3=15
\]
因此 \(P=(-2,15)\).
...
因为 \(1<\sqrt5<3\)，所以
\[
0<a=\frac{\sqrt5-1}{2}<1
\]
因此指数函数 \(f(x)=a^x\) 在 \(\R\) 上单调递减. 由 \(f(m)>f(n)\) 可知自变量的大小规律相反，所以 \(m<n\).
\end{solution}
...
由对数不等式及定义域，
\[
\log_2x\le2\quad\Longleftrightarrow\quad 0<x\le4
\]
所以 \(A=(0,4]\). 又 \(B=(-\infty,a)\) 不含端点 \(a\)，要使 \(A\subseteq B\)，必须且只需 \(a>4\). 因而题设中的 \(c=4\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 设 \(\alpha\) 内的两条相交直线分别为 \(l_1\)，\(l_2\)，\(\beta\) 内的对应直线分别为 \(m_1\)，\(m_2\). 因为 \(l_1\) 与 \(l_2\) 相交，所以它们的方向不相同；又分别平行于 \(m_1\)，\(m_2\)，故 \(\alpha\) 与 \(\beta\) 含有两组对应的不同平行方向. 两平面若不平行则相交，而相交两平面只能共有一个方向，不可能含有两组不同的平行方向. 因此 \(\alpha\parallel\beta\)，命题为真.

\item 直线 \(l\) 不在平面 \(\alpha\) 内，且平行于平面 \(\alpha\) 内的直线. 若 \(l\) 与 \(\alpha\) 相交于点 \(P\)，则过 \(P\) 可作平面 \(\alpha\) 内且与 \(l\) 平行的直线. 这条直线与 \(l\) 过同一点且平行，因而两直线重合，从而 \(l\subset\alpha\)，与 \(l\) 在 \(\alpha\) 外矛盾. 因此 \(l\) 与 \(\alpha\) 无公共点，即 \(l\parallel\alpha\)，命题为真.

\item 该命题不成立. 例如取 \(\alpha:z=0\)，\(\beta:y=z\)，则两平面的交线为 \(x\) 轴. 平面 \(\alpha\) 内的 \(y\) 轴垂直于交线，但 \(\alpha\) 与 \(\beta\) 的夹角不是直角，所以不能推出 \(\alpha\perp\beta\).

\item 直线垂直于平面的判定需要垂直于平面内两条相交直线，而题中没有相交条件. 取 \(\alpha:z=0\)，取 \(l\) 为 \(x\) 轴，再取平面 \(\alpha\) 内两条互相平行且平行于 \(y\) 轴的直线，则 \(l\) 分别垂直于这两条直线，但 \(l\) 在平面 \(\alpha\) 内，不可能垂直于平面 \(\alpha\). 因此命题为假.
\end{enumerate}
所以真命题的序号为 \(1\)、\(2\).
\end{solution}
...
\begin{solution}
设 \(A_1=(-a,0)\)，\(B_2=(0,b)\)，\(B_1=(0,-b)\)，椭圆的右焦点为 \(F(c,0)\)，其中 \(c=\sqrt{a^2-b^2}\)，并记离心率为 \(e=\dfrac ca\). 由两点式，直线 \(A_1B_2\) 和直线 \(B_1F\) 分别可写为
\(\frac{x}{a}-\frac{y}{b}=-1\)，\(\frac{x}{c}-\frac{y}{b}=1\).
联立得交点
\[
T\left(\frac{2ac}{a-c},\frac{b(a+c)}{a-c}\right)
\]
因为 \(M\) 是线段 \(OT\) 的中点，所以
\[
M\left(\frac{ac}{a-c},\frac{b(a+c)}{2(a-c)}\right)
\]
将 \(M\) 代入椭圆方程，得
\[
\frac{c^2}{(a-c)^2}+\frac{(a+c)^2}{4(a-c)^2}=1
\]
整理为
\[
3a^2-10ac-c^2=0
...
\begin{solution}
令 \(x_n=a_n=b_n-1\). 题设集合中的四项对应的 \(x_n\) 属于
\[
\{-54,-24,18,36,81\}
\]
由于 \(|q|>1\)，且这四项均非零，连续四项的绝对值严格递增. 若首项绝对值为 \(18\)，第二项绝对值分别取 \(24\)，\(36\)，\(54\)，\(81\) 时，按相同公比继续得到的第三项绝对值分别为 \(32\)，\(72\)，\(162\)，\(\dfrac{729}{2}\)，均不在集合中. 若首项绝对值为 \(24\)，第二项绝对值为 \(36\) 时可继续得到 \(54,81\)，而第二项取 \(54\) 或 \(81\) 时第三项绝对值分别为 \(\dfrac{243}{2}\) 和 \(\dfrac{2187}{8}\)，均不在集合中. 若首项绝对值为 \(36\)，第二项取 \(54\) 时第四项绝对值为 \(\dfrac{243}{2}\)，取 \(81\) 时第三项绝对值为 \(\dfrac{729}{4}\)，也不能得到四项. 首项绝对值为 \(54\) 或 \(81\) 时更不可能得到四项. 因此只有
\(24\)，\(36\)，\(54\)，\(81\)
这一组，公比的绝对值为 \(\dfrac32\). 对应的四项必须按符号交替排列为
\(-24\)，\(36\)，\(-54\)，\(81\)
从而 \(q=-\dfrac32\). 这四项加上 \(1\) 后为 \(-23\)，\(37\)，\(-53\)，\(82\)，确实都在题设集合中. 故
\[
6q=6\left(-\frac32\right)=-9
\]
\end{solution}
...
\]
若该数是数列中的项，则它必须是整数，所以 \(t\mid8\). 由于 \(t\) 是奇数，只可能有 \(t=1\) 或 \(t=-1\). 当 \(t=1\) 时，\(m=2\)，上式的值为 \(3=a_5\)；当 \(t=-1\) 时，\(m=1\)，上式的值为 \(-15\). 但对任意正整数 \(k\)，\(a_k=2k-7\ge-5\)，所以 \(-15\) 不可能是数列中的项. 故所有满足条件的正整数为 \(m=2\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 直线 \(l\) 若为竖直直线，则为 \(x=4\)，圆心 \((-3,1)\) 到它的距离为 \(7>2\)，不能截得圆 \(C_1\) 的弦. 因此 \(l\) 非竖直时可设其方程为 \(y=k(x-4)\)，即 \(kx-y-4k=0\). 圆 \(C_1\) 的半径为 \(2\)，弦长为 \(2\sqrt3\)，所以圆心 \((-3,1)\) 到直线 \(l\) 的距离为
\[
...
\item 设 \(P=(m,n)\). 若 \(l_1\) 为竖直直线，则与之垂直的 \(l_2\) 为水平直线，二者分别由 \(P\) 唯一确定；反之亦然，因此这两种方向至多产生一对直线. 对于非水平且非竖直的直线，设过 \(P\) 的直线 \(l_1\) 斜率为 \(k\)，则与之垂直的直线 \(l_2\) 斜率为 \(-\frac1k\)，它们的方程可写为
\[
l_1:\ y-n=k(x-m)
\]
\[
...
对于 \(P=\left(\frac52,-\frac12\right)\) 和 \(P=\left(-\frac32,\frac{13}{2}\right)\)，分别有
\[
\overrightarrow{PC_1}\cdot\overrightarrow{PC_2}=0
\]
取 \(l_1\) 平行于 \(PC_1\)，取 \(l_2\) 平行于 \(PC_2\)，则两条直线互相垂直，且它们分别过圆心 \(C_1\)，\(C_2\)，公共弦长均为直径. 又前面的恒等关系说明，将这组方向作充分小的连续改变时，弦长仍相等且两圆心到直线的距离仍小于半径 \(2\)，故有无穷多对满足条件的直线. 所以所求点为 \(\left(-\frac32,\frac{13}{2}\right)\) 或 \(\left(\frac52,-\frac12\right)\).
\end{enumerate}
\end{solution}
...
f(x)=x^2+2ax-a^2
\]
当 \(a>0\) 时，顶点 \(x=-a\) 在该分支的定义域内，最小值为 \(-2a^2\)；当 \(a=0\) 时，左支为 \(x^2\)，而 \(x=0\) 属于右支，故全局最小值为 \(0\)；当 \(a<0\) 时，该分支在 \(x<a\) 上递减，其下确界为在 \(x=a\) 处的函数值 \(2a^2\)，而右侧分支可在 \(x=a/3\) 处取得更小的值 \(2a^2/3\). 因此
\[
\min_{x\in\R}f(x)=
\begin{cases}
-2a^2,&a\ge0,\\[2pt]
...
\begin{solution}
由全等三角形的对应角相等，\(\triangle ABC\cong\triangle BAD\) 给出 \(\angle ACB=\angle BDA\). 这两个角分别是线段 \(AB\) 在点 \(C\)，\(D\) 所对的圆周角，因此点 \(A\)，\(B\)，\(C\)，\(D\) 共圆.

在这个圆中，同弦 \(CB\) 所对的圆周角相等，所以
\[
\angle CAB=\angle CDB
\]
又由全等三角形的对应角相等，\(\angle CAB=\angle DBA\). 因而 \(\angle DBA=\angle CDB\)，这两角是直线 \(BD\) 被直线 \(AB\)，\(CD\) 所截形成的一对内错角. 根据内错角相等，两直线平行，故 \(AB\parallel CD\).
\end{solution}
