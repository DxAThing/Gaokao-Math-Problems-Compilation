...
\[
\cos\theta=\frac{\bs{a}\cdot\bs{b}}{\left|\bs{a}\right|\left|\bs{b}\right|}=\frac{-3}{3\times2}=-\frac12
\]
...
一只猴子随机敲击只有\(26\)个小写英文字母的练习键盘. 若每敲\(1\)次在屏幕上出现一个字母，它连续敲击\(10\)次，屏幕上的\(10\)个字母依次排成一行，则出现单词“\mbox{monkey}”的概率为\fillinblank{}（结果用数值表示）.
\end{problem}
...
\begin{solution}
连续敲击得到的全部长度为 \(10\) 的字母序列共有 \(26^{10}\) 种且等可能. 单词“\mbox{monkey}”有 \(10-6+1=5\) 个可能的起始位置，每固定一个起始位置后，其余 \(4\) 个字母任意，共有 \(26^4\) 种. 不同起始位置不能同时出现“\mbox{monkey}”，所以有利序列数为 \(5\times26^4\)，所求概率为 \(\frac{5\times26^4}{26^{10}}=\frac{5}{26^6}\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 当 \(n\geqslant2\) 时，原递推式分别给出 \(3a_{n+1}+2S_n=3\) 和 \(3a_n+2S_{n-1}=3\). 两式相减，并利用 \(S_n-S_{n-1}=a_n\)，得
\[
3a_{n+1}-3a_n+2a_n=0
\]
...
\]
其中 \(n\in\mathbb Z_{>0}\).
\item 由于公比满足 \(\left|\frac13\right|<1\)，无穷级数收敛，且
\[
...
x^2=\frac{a^2b}{a+b}
\]
从而 \(x\approx239.653\)，且 \(y=\sqrt{ab-x^2}\approx156.657\).
探测器到火星中心的距离为
\[
...
\begin{solution}
\begin{enumerate}
\item 当 \(a=8\)，\(d=4\) 时，\(a_1=8\)，\(a_2=12\)，\(a_3=16\). 由图形位置，
\(A_1=(8,4)\)，\(A_2=(18,6)\)，\(A_3=(32,8)\).
因此 \(k_{A_1A_2}=\frac{6-4}{18-8}=\frac15\)，\(k_{A_2A_3}=\frac{8-6}{32-18}=\frac17\).
两斜率不相等，故三点不在同一直线上.
\item 一般地，因 \(a_n=a+(n-1)d\)，且 \(B_1=(d,0)\)，有
\[
x_n=d+\sum_{i=1}^{n-1}a_i+\frac12a_n
\]
且 \(y_n=\frac12a_n\).
在本小题条件下，\(a_n=4(n+1)\)，所以
\[
x_n=4+\sum_{i=1}^{n-1}4(i+1)+2(n+1)=2(n+1)^2
\]
且 \(y_n=2(n+1)\).
于是 \(y_n^2=4(n+1)^2=2x_n\)，故所有顶点 \(A_n\) 均在抛物线 \(y^2=2x\) 上.
\item 令 \(t=n-1\)，则
\[
x_n=d+\frac a2+at+\frac d2t^2
\]
且 \(y_n=\frac12(a+dt)\).
由 \(t=\frac{2y_n-a}{d}\) 消去 \(t\)，得
\[
...
\]
且 \(d+\frac{a(d-a)}{2d}=0\).
第二式化为 \(a^2-ad-2d^2=0\)，即 \((a-2d)(a+d)=0\). 因为 \(a=a_1\) 是正方形的对角线长，故 \(a>0\)，而 \(d>0\)，只能取 \(a=2d\). 反之，若 \(a=2d\)，则常数项为零且 \(p=\frac d4>0\)，上式给出所有顶点都满足 \(y^2=2px\). 所以所求关系式为 \(a=2d\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(f_1(\theta)=\sin\theta-\cos\theta\)，\(f_3(\theta)=\sin^3\theta-\cos^3\theta\). 对任意 \(0\leqslant\theta_1<\theta_2\leqslant\frac\pi4\)，有 \(\sin\theta_1<\sin\theta_2\) 且 \(\cos\theta_2<\cos\theta_1\)，因此
\[
...
\item 记 \(s=\sin\theta\)，\(c=\cos\theta\). 由 \(s^2+c^2=1\)，
\[
...
\]
两边相等，等式得证.
\item 当 \(n\) 为奇数时，\(f_n(\theta)=\sin^n\theta-\cos^n\theta\). 对区间内任意 \(\theta_1<\theta_2\)，由于函数 \(t\mapsto t^n\) 在 \([0,+\infty)\) 上严格递增，有 \(\sin^n\theta_1<\sin^n\theta_2\) 及 \(\cos^n\theta_2<\cos^n\theta_1\)，并且
\[
...
f_n(0)=-1
\]
且 \(f_n\left(\frac\pi4\right)=0\).
所以奇数 \(n\) 时最大值为 \(0\)，最小值为 \(-1\).
