...
\begin{solution}
集合 \(A=(-\infty,1]\)，\(B=[a,+\infty)\). 若 \(a\leqslant1\)，两个区间相接或重叠，故 \(A\cup B=\R\). 若 \(a>1\)，则区间 \((1,a)\) 中的数既不属于 \(A\) 也不属于 \(B\)，不可能有 \(A\cup B=\R\). 因此 \(a\leqslant1\)，故填 \(a\leqslant1\).
\end{solution}
...
\bitmapfigure[max width=0.36\linewidth]{img/2009/shanghai_liberal/q4_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.35\linewidth]{img/2009/shanghai_liberal/q5_fig1.png}
\end{problem}
...
y \geqslant -2x,\\
x \leqslant 3,
\end{cases}\) 则目标函数 \(z = x - 2y\) 的最小值是\fillinblank{}.
\end{problem}
...
=-f(t+jh)-f(t-jh)
\]
这里 \(t+jh>-(t-jh)\)，且 \(f\) 严格递增、\(f\) 为奇函数，所以 \(f(t-jh)+f(t+jh)>0\)，上述和确实小于 \(0\).
且 \(f(a_{14})<0\)，所以总和小于 \(0\). 因题设总和为 \(0\)，必有 \(a_{14}=0\)，从而 \(f(a_{14})=f(0)=0\). 故 \(k=14\).
\end{solution}

\begin{problem}
某地街道呈现东—西，南—北向的网格状，相邻街距都为 \(1\)，两街道相交的点称为格点. 若以互相垂直的两条街道为轴建立直角坐标系，现有下述格点 \((-2, 2)\)，\((3, 1)\)，\((3, 4)\)，\((-2, 3)\)，\((4, 5)\) 为报刊零售点. 请确定一个格点为发行站，使 \(5\) 个零售点沿街道到发行站之间路程的和最短，发行站为\fillinblank{}.
\end{problem}
...
\bitmapfigure[max width=0.32\linewidth]{img/2009/shanghai_liberal/q16_fig1.png}

\choices
    {\choicebitmap[width=0.15\paperwidth]{img/2009/shanghai_liberal/q16_optA_clean.png}}
    {\choicebitmap[width=0.15\paperwidth]{img/2009/shanghai_liberal/q16_optB_clean.png}}
    {\choicebitmap[width=0.15\paperwidth]{img/2009/shanghai_liberal/q16_optC_clean.png}}
    {\choicebitmap[width=0.15\paperwidth]{img/2009/shanghai_liberal/q16_optD_clean.png}}
\end{problem}
...
\begin{solution}
设圆上动点为 \(Q(X,Y)\)，线段 \(PQ\) 的中点为 \(M(x,y)\). 则
\(x=\frac{4+X}{2}\)，\(y=\frac{-2+Y}{2}\)
...
\begin{problem}
在发生某公共卫生事件期间，有专业机构认为该事件在一段时间内没有发生大规模群体感染的标志为“连续 \(10\) 天，每天新增疑似病例不超过 \(7\) 人”. 根据过去 \(10\) 天甲，乙，丙，丁四地新增疑似病例数据，一定符合该标志的是（\quad）
...
\begin{problem}
已知复数 \(z = a + b\i\)（\(a\)，\(b \in \R^+\)）（\(\i\) 是虚数单位）是方程 \(x^2 - 4x + 5 = 0\) 的根. 复数 \(w = u + 3\i\)（\(u \in \R\)）满足 \(|w - z| < 2\sqrt{5}\)，求 \(u\) 的取值范围.
\end{problem}
...
\]
得方程的两个根为 \(2+\i\) 和 \(2-\i\). 由 \(a\)，\(b\in\R^+\)，题设中的根满足 \(z=2+\i\). 于是
\[
...
\begin{problem}
已知 \(\triangle ABC\) 的角 \(A\)，\(B\)，\(C\) 所对的边分别是 \(a\)，\(b\)，\(c\)，设向量 \(\bs{m} = (a, b)\)，\(\bs{n} = (\sin B, \sin A)\)，\(\bs{p} = (b - 2, a - 2)\).

\begin{enumerate}
\item 若 \(\bs{m} \parallel \bs{n}\)，求证：\(\triangle ABC\) 为等腰三角形；

\item 若 \(\bs{m} \perp \bs{p}\)，边长 \(c = 2\)，角 \(C = \frac{\pi}{3}\)，求 \(\triangle ABC\) 的面积.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 由正弦定理，存在外接圆半径 \(R\)，使
...
\begin{solution}
\begin{enumerate}
\item 当 \(x\geqslant7\) 时，\(x\)，\(x+1>6\)，故
  \[
...
\item \(f(6)=85\%=0.85\)，故
\(0.1+15\ln\frac{a}{a-6}=0.85\)，\(\ln\frac{a}{a-6}=0.05\).
  于是
\(\frac{a}{a-6}=e^{0.05}\)，\(a=\frac{6e^{0.05}}{e^{0.05}-1}\approx123.0\).
  按原 PDF 的区间，学科乙为 \((121,127]\)，学科丙为 \((121,133]\)，故该 \(a\) 同时属于乙、丙两类，结论为学科乙或丙.
\end{enumerate}
\end{solution}

\begin{problem}
已知双曲线 \(C\) 的中心是原点，右焦点为 \(F(\sqrt{3}, 0)\)，一条渐近线 \(m: x + \sqrt{2}y = 0\)，设过点 \(A(-3\sqrt{2}, 0)\) 的直线 \(l\) 的方向向量 \(\bs{e} = (1, k)\).
...
\(\frac{x^2}{a^2}-\frac{y^2}{b^2}=1\)，\(a\)，\(b>0\).
  其右焦点横坐标满足 \(c^2=a^2+b^2=3\). 渐近线斜率的绝对值为 \(b/a\)，而 \(x+\sqrt2y=0\) 的斜率绝对值为 \(1/\sqrt2\)，所以 \(b^2=a^2/2\). 解得 \(a^2=2\)，\(b^2=1\)，故
  \[
  \frac{x^2}{2}-y^2=1
  \]
\item 直线 \(l\) 的方程为
  ...
  k=\pm\frac{\sqrt2}{2}
  \]
\item 反设右支存在点 \(Q(x,y)\) 满足到 \(l\) 的距离为 \(\sqrt6\). 则
  \[
...
\item 由 \(b_n=aq^n\) 及 \(aq\ne0\)，条件
  ...
  \]
  取 \(m=1\)，可知 \(a=q^c\)，其中 \(c=k-3\in\Z\)，且因为 \(k\in\N^*\)，有 \(c\geqslant-2\). 反之，若 \(a=q^c\)，其中 \(c\in\Z\)，\(\ c\geqslant-2\)，则对任意 \(m\in\N^*\)，取
  \[
  k=2m+1+c\geqslant1
  \]
  就有 \(a^2q^{2m+1}=aq^k\)，故条件成立. 因此充要条件为 \(a=q^c\)，其中 \(c\in\Z\)，\(\ c\geqslant-2\).
\item 连续 \(p\) 项的和（从第 \(n\) 项开始）为
  \[
  \sum_{j=0}^{p-1}b_{n+j}=3^n\frac{3^p-1}{3-1}
  =\frac{3^n(3^p-1)}2
  \]
  若 \(p\) 为偶数，则这是偶数个奇数的和，为偶数；而 \(a_m=2m+1\) 为奇数，不可能相等. 若 \(p\) 为奇数，则上述和为奇数，且 \(n\geqslant1\)、\(p\geqslant1\) 时不小于 \(3\). 令其为 \(S\)，取 \(m=(S-1)/2\in\N^*\)，便有 \(a_m=S\). 所以当且仅当 \(p\) 为奇数.
\end{enumerate}
\end{solution}
