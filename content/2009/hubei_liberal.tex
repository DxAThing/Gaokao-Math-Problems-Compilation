...
\begin{solution}
\(y=225x+\frac{129600}{x}-360\)（\(x>0\)），当 \(x=24\) 时总费用最少，最小总费用为 \(10440\text{ 元}\).
\end{solution}
...
\begin{solution}
\(AC\perp BE\)，\(\lambda=\frac{\sqrt{2}}{2}\).
\end{solution}
...
\begin{solution}
\(FM_1\perp FN_1\)，且 \(S_2^2=4S_1S_3\) 成立.
\end{solution}
...
\begin{solution}
\(b=-1\)，\(c=3\)；对任意的 \(c\) 都有 \(M>2\)；\(k\) 的最大值为 \(\frac{1}{2}\).
\end{solution}
