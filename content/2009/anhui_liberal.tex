...
\begin{solution}
由 \((2x+1)(x-3)<0\) 的两个根为 \(-\frac{1}{2}\) 和 \(3\)，且二次项系数为正，得
\[
-\frac{1}{2}<x<3
\]
所以 \(A=\left(-\frac{1}{2},3\right)\). 由 \(x\in\N_+\) 且 \(x\leqslant5\)，得 \(B=\{1,2,3,4,5\}\). 因此 \(A\cap B=\{1,2\}\)，故选 B.
\end{solution}
...
\begin{solution}
边界直线 \(x+3y=4\) 与 \(3x+y=4\) 的交点为 \((1,1)\). 由 \(x\geqslant0\)，区域还被 \(y\) 轴截得；令 \(x=0\)，两条边界分别给出 \(y=\frac{4}{3}\) 与 \(y=4\). 结合不等式方向，区域是顶点为 \((0,\frac{4}{3})\)、\((0,4)\)、\((1,1)\) 的三角形.

其底边在 \(y\) 轴上，长为 \(4-\frac{4}{3}=\frac{8}{3}\)，顶点 \((1,1)\) 到 \(y\) 轴的高为 \(1\)，所以面积为
\[
\frac{1}{2}\cdot\frac{8}{3}\cdot1=\frac{4}{3}
\]
故选 C.
\end{solution}
...
\begin{solution}
把六个面中心分别记为 \((\pm1,0,0)\)，\((0,\pm1,0)\)，\((0,0,\pm1)\). 任取三个点有两种情形.

若三个点分别来自三组相对面，则任意两点的距离都为 \(\sqrt{2}\)，第一个三角形是边长为 \(\sqrt{2}\) 的等边三角形；剩余三个点也分别来自三组相对面，组成同样的等边三角形.

若三个点中含有一对相对面中心，则第三个点来自另一组相对面，三边为 \(2\)，\(\sqrt{2}\)，\(\sqrt{2}\). 剩余三个点也含有一对相对面中心，三边仍为 \(2\)，\(\sqrt{2}\)，\(\sqrt{2}\). 所以无论怎样选取，两个三角形都全等，事件必然发生，概率为 \(1\)，故选 A.
\end{solution}
...
\begin{solution}
设 \(M=(0,t,0)\). 由空间两点间距离公式，
\(MA^2=(0-1)^2+t^2+(0-2)^2=t^2+5\)，\(MB^2=(0-1)^2+(t+3)^2+(0-1)^2=t^2+6t+11\).
因为 \(MA=MB\)，所以 \(MA^2=MB^2\)，从而 \(t^2+5=t^2+6t+11\)，解得 \(t=-1\). 因此 \(M=(0,-1,0)\).
\end{solution}
...
\begin{solution}
流程图先取 \(a=1\)，然后反复执行 \(a\leftarrow2a+1\)，直到 \(a>100\) 时输出 \(a\). 设第 \(k\) 次更新后的数为 \(a_k\)，则 \(a_0=1\)，且
\(a_{k+1}=2a_k+1\)，\(a_{k+1}+1=2(a_k+1)\).
所以 \(a_k+1=2^k(a_0+1)=2^{k+1}\)，即 \(a_k=2^{k+1}-1\). 逐次计算得 \(a_5=63\leqslant100\)，\(a_6=127>100\)，故第一次满足判断条件时输出 \(127\).
\end{solution}
...
\mathrm C_4^3=4
\]
三条线段构成三角形的充要条件是最长边小于另外两边之和. 四种取法及判断如下：
\(\{2,3,4\}:2+3>4\)，\(\{2,3,5\}:2+3=5\)，\(\{2,4,5\}:2+4>5\)，\(\{3,4,5\}:3+4>5\).
因此只有 \(\{2,3,5\}\) 不能构成三角形，有利事件数为 \(3\)，所求概率为 \(\frac{3}{4}\).
\end{solution}
...
\begin{solution}
以 \(A\) 为原点，令 \(\overrightarrow{AB}=\bs{b}\)，\(\overrightarrow{AD}=\bs{d}\). 则
\(\overrightarrow{AC}=\bs{b}+\bs{d}\)，\(\overrightarrow{AE}=\frac{1}{2}\bs{b}+\bs{d}\)，\(\overrightarrow{AF}=\bs{b}+\frac{1}{2}\bs{d}\).
代入题设得
\[
\bs{b}+\bs{d}
=\lambda\left(\frac{1}{2}\bs{b}+\bs{d}\right)
+\mu\left(\bs{b}+\frac{1}{2}\bs{d}\right)
\]
由于 \(\bs{b}\)，\(\bs{d}\) 不共线，比较系数得
\(\frac{\lambda}{2}+\mu=1\)，\(\lambda+\frac{\mu}{2}=1\).
两式相减得 \(\lambda=\mu\)，代回得 \(\lambda=\mu=\frac{2}{3}\)，所以 \(\lambda+\mu=\frac{4}{3}\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 以数据的百位和十位组成茎，以个位作为叶. 例如 \(357\) 记作茎 \(35\)、叶 \(7\)，\(359\) 记作茎 \(35\)、叶 \(9\). 原 PDF 虽写两种小麦各种植了 \(25\) 亩，但实际列出的两组数据各有 \(24\) 个；以下严格按原 PDF 列出的数据完成茎叶图. 对全部数据按茎从小到大排列，并将同一茎上的叶按从小到大排列，得到完整茎叶图：
...
\(\frac{c}{a}=\frac{\sqrt{3}}{3}\)，\(c^2=a^2-b^2\).
于是
\(\frac{a^2-b^2}{a^2}=\frac{1}{3}\)，\(\frac{b^2}{a^2}=\frac{2}{3}\).
代入 \(b^2=2\)，得 \(a^2=3\)，所以
\(a=\sqrt{3}\)，\(b=\sqrt{2}\).
...
u^2+v^2=u^2+(v-2)^2
\]
从而 \(v=1\). 设 \(F'=(r,s,0)\)，由 \(FB=FC\) 得 \(F'B=F'C\)，于是
\[
...
\(4=1+1+h^2\)，\(h=\sqrt{2}\).
由第一问，\(E'F'\parallel AB\)，而 \(EF\) 是其在平行平面上的对应线段，所以 \(EF\parallel AB\). 结合原图中 \(E\)，\(F\) 的位置，\(F\) 是将 \(E\) 沿 \(AB\) 方向平移 \(2\) 个单位得到的点；因此 \(ABCDEF\) 是以 \(\triangle ADE\) 为底、以 \(EF\) 为侧棱的三棱柱. 由上面得到 \(AE=2\)，所以
\[
...
\begin{enumerate}
\item 讨论 \( f(x) \) 的单调性；
\item 设 \(a = 3\)，求 \(f(x)\) 在区间 \([1, \e^2]\) 上值域，其中 \(\e = 0.71828 \cdots\) 是自然对数的底数.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 题干原 PDF 将自然对数的底数的近似值印作 \(0.71828\cdots\)，这与自然对数底数的定义矛盾；若照此数值，则 \(e^2<1\)，题中的 \([1,e^2]\) 不能作为通常意义下的非空区间. 以下按数学定义 \(\e=2.71828\cdots\) 计算，并保留这一题面冲突. 函数的定义域为 \(x>0\)，且
\[
...
\(f(1)=0\)，\(f(2)=2-3\ln2\)，\(f(\e^2)=\e^2-\frac{2}{\e^2}-5\).
由于 \(\ln2>\frac{1}{2}\)，有 \(f(2)=2-3\ln2<0=f(1)\). 令 \(h(t)=t-\frac{2}{t}-5\)，则 \(h'(t)=1+\frac{2}{t^2}>0\). 又 \(\e>2.7\)，故 \(\e^2>7.29\)，从而
\[
h(\e^2)=f(\e^2)>h\left(\frac{729}{100}\right)=\frac{729}{100}-\frac{200}{729}-5>0=f(1)
\]
结合单调性，最小值为 \(2-3\ln2\)，最大值为 \(\e^2-\frac{2}{\e^2}-5\). 所以值域为
\[
