...
由题意，
\[
\bs{a}+\bs{b}=(3,x+1),\qquad 4\bs{b}-2\bs{a}=(6,4x-2).
\]
两向量平行，所以其坐标行列式为零，即
\[
3(4x-2)-6(x+1)=0.
...
\[
11^{\circ}<12^{\circ}<80^{\circ},
\qquad \cos10^{\circ}=\sin80^{\circ}.
\]
...
\]
\[
A_1(0,0,l),\quad B_1(a,0,l),\quad D_1(0,a,l).
\]
...
e^2+2e-1\geqslant0,
\]
所以 \(e\geqslant\sqrt2-1\). 当 \(e=\sqrt2-1\) 时，\(x/a=-1\)，点 \(P\) 为椭圆左顶点，三点 \(P,F_1,F_2\) 共线，题设中的两个正弦均为零，原等式无意义，故应排除该端点. 于是离心率的取值范围为
\[
...
所以
\[
3x+\frac\pi4\in\left(\pi+2k\pi,2\pi+2k\pi\right),\qquad k\in\Z,
\]
从而单调增区间为
\[
\left(\frac\pi4+\frac{2k\pi}{3},\frac{7\pi}{12}+\frac{2k\pi}{3}\right),\qquad k\in\Z.
\]
\end{enumerate}
\end{solution}
...
\[
A(0,0,0),\quad B(p,0,0),\quad D(0,2,0),\quad C(2,2,0),
\]
其中 \(p=AB>0\). 设 \(FA=q>0\)，由 \(FA\perp\) 平面 \(ABCD\) 及平行四边形 \(ABFE\)，可取
\[
F(0,0,q),\qquad E(-p,0,q).
\]
由 \(FC=3\)，得
\[
FC^2=2^2+2^2+q^2=9,
...
\item 因为 \(FA\perp\) 平面 \(ABCD\)，所以 \(FA\perp AD\)；又 \(AD\perp AB\)，且 \(AB\parallel FE\)，故 \(AD\perp FE\). 从而 \(AF\) 和 \(AE\) 分别是两个半平面内过 \(A\) 且垂直于棱 \(AD\) 的直线，所以 \(\angle FAE\) 是二面角 \(F-AD-E\) 的平面角.
...
\item 记双曲线右焦点为 \(F(\sqrt5,0)\)，圆心为 \(O_1(0,\sqrt5)\). 对双曲线右支上的点 \(M\)，由双曲线的定义，
\[
MA-MF=2a=2.
\]
又因为圆的半径为 \(1\)，所以
...
所以
\[
b_1=\frac{a_2}{a_1}=4,\qquad b_2=\frac{a_3}{a_2}=\frac{17}{4},\qquad b_3=\frac{a_4}{a_3}=\frac{72}{17}.
\]
