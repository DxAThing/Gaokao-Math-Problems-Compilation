...
\begin{solution}
由 \(a>b\) 不能推出 \(a^2>b^2\)，例如 \(a=-1,b=-2\) 时结论相反；当 \(a\) 为负数时，\(\frac{b}{a}<1\) 也不一定成立；又 \(a-b\) 只保证为正数，不保证大于 \(1\)，所以 \(\lg(a-b)>0\) 不一定成立.因为底数 \(\frac{1}{2}\) 在 \((0,+\infty)\) 上是减函数，且 \(a>b\)，所以 \(\left(\frac{1}{2}\right)^a<\left(\frac{1}{2}\right)^b\)，故选 D.
\end{solution}
...
\(\tan\theta-\sin\theta=\frac{\sin\theta(1-\cos\theta)}{\cos\theta}\).因此在 \(\left(\frac{\pi}{2},\pi\right)\) 和 \(\left(\frac{3\pi}{2},2\pi\right)\) 内该式为负，其他区间内不为负，故 \(F=\left(\frac{\pi}{2},\pi\right)\cup\left(\frac{3\pi}{2},2\pi\right)\).于是 \(E\cap F=\left(\frac{\pi}{2},\pi\right)\)，故选 A.
\end{solution}
...
\begin{solution}
抛物线 \(y^2=4x\) 的标准参数为 \(p=1\)，所以焦点为 \(F=(1,0)\).弦 \(AB\) 垂直于 \(x\) 轴，设其所在直线为 \(x=t\)，则弦端点的纵坐标为 \(\pm2\sqrt{t}\)，从而 \(AB=4\sqrt{t}=4\sqrt{3}\)，得 \(t=3\).因此焦点到直线 \(AB\) 的距离为 \(|3-1|=2\).
\end{solution}
...
\item 因为 \(A_1C_1\parallel AC\)，所以 \(l\parallel AC\).作 \(A_1E\perp l\)，则 \(A_1E\) 是所求距离.设 \(A\) 在 \(l\) 上的垂足为 \(E\)，由于 \(A_1A\perp\) 平面 \(ABC\)，由三垂线定理的逆定理可知 \(AE\perp l\)，且
\(A_1E^2=A_1A^2+AE^2\).
...
\begin{solution}
由 \(z=\cos\theta+\i\sin\theta\) 得 \(z=\e^{\i\theta}\)、\(\overline{z}=\e^{-\i\theta}\).因为分母不为零，
\(\displaystyle\omega=\frac{1-\e^{-4\i\theta}}{1+\e^{4\i\theta}}=\frac{2\i\e^{-2\i\theta}\sin2\theta}{2\e^{2\i\theta}\cos2\theta}=\i\e^{-4\i\theta}\tan2\theta\).

由 \(|\omega|=\frac{\sqrt{3}}{3}\) 和 \(\arg\omega=\frac{\pi}{2}\)，应有 \(\omega=\frac{\sqrt{3}}{3}\i\)，因此
\(\displaystyle\e^{-4\i\theta}\tan2\theta=\frac{\sqrt{3}}{3}\).左边的 \(\tan2\theta\) 是非零实数，所以 \(\e^{-4\i\theta}\) 必须为实数，从而 \(\sin4\theta=0\).结合 \(0<\theta<\pi\)，得到 \(\theta=\frac{\pi}{4}\)、\(\frac{\pi}{2}\) 或 \(\frac{3\pi}{4}\).

但当 \(\theta=\frac{\pi}{4}\) 或 \(\frac{3\pi}{4}\) 时，\(\cos2\theta=0\)，原式分母 \(1+z^4\) 为零，\(\omega\) 无定义；当 \(\theta=\frac{\pi}{2}\) 时，\(\tan2\theta=0\)，与 \(|\omega|=\frac{\sqrt{3}}{3}\) 矛盾.因此按原始题面不存在满足全部条件的 \(\theta\)，现题条件彼此不相容.
\end{solution}
