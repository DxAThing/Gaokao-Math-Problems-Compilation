...
\begin{problem}
当圆锥的侧面积和底面积的比例是 \(\sqrt{2}\) 时，圆锥的轴截面顶角是（\quad）
...
\begin{problem}
曲线的参数方程为 \(\left\{ \begin{array}{l} x = 3t^2 + 2, \\ y = t^2 - 1, \end{array} \right.\) （\(0 \leqslant t \leqslant 5\)），则曲线是（\quad）
...
\begin{problem}
解不等式: \(2 + \log_{\frac{1}{3}}(5 - x) + \log_{\frac{1}{2}}\frac{1}{x} > 0\).
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/1993/national_science/q26_fig1.png}
...
\begin{problem}
已知关于 \(x\) 的实系数二次方程 \(x^{2} + ax + b = 0\) 有两个实数根 \(\alpha\)、\(\beta\). 证明：（1）如果 \(|\alpha| < 2\)，\(|\beta| < 2\)，那么 \(2|\alpha| < 4 + b\) 且 \(|b| < 4\)；（2）如果 \(2|\alpha| < 4 + b\) 且 \(|b| < 4\)，那么 \(|\alpha| < 2\)，\(|\beta| < 2\).
\end{problem}
