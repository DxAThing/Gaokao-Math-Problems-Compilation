...
\begin{solution}
由直线方程得 \(y=-\frac{b}{a}x+b\). 由于 \(a<0\)，\(b<0\)，所以
\(\frac{b}{a}>0\)，直线的斜率 \(-\frac{b}{a}<0\). 设直线的倾斜角为
\(\alpha\)，则 \(0\leqslant\alpha<\pi\)，且 \(\tan\alpha=-\frac{b}{a}<0\).
因此 \(\frac{\pi}{2}<\alpha<\pi\). 又因为
\(\arctan\frac{b}{a}\in\left(0,\frac{\pi}{2}\right)\)，所以
...
\(V=\pi r^2h=\pi\left(\frac{l}{2}r^2-2r^3\right)\)，从而
\(V'=\pi r(l-6r)\).
...
\(a=\frac{k}{3}\)，\(b=\frac{k}{4}\)，\(c=\frac{k}{6}\)，从而
\(\frac{1}{a}=\frac{3}{k}\)，\(\frac{1}{b}=\frac{4}{k}\)，\(\frac{1}{c}=\frac{6}{k}\).
...
x(5-x)<4
\]
从而 \(x^2-5x+4>0\)，即 \((x-1)(x-4)>0\). 结合定义域
\(0<x<5\)，得 \(x\in(0,1)\cup(4,5)\).
...
\(A=(4,0,0)\)，\(C=(0,3,0)\)，\(A_1=(4,0,1)\)，\(C_1=(0,3,1)\).
...
\(\tan M=\frac{1}{2}\)，\(\tan N=-2=-\frac{1}{\tan M}\)，
所以 \(N=\frac{\pi}{2}+M\)，从而 \(P=\pi-M-N=\frac{\pi}{2}-2M\).
...
\(\tan 2\theta\sin 4\theta=0\)，即 \(\sin 4\theta=0\).
...
\begin{solution}
按原题面，两个待证命题均不成立，因而不能给出它们的证明. 由韦达定理
\(b=\alpha\beta\)，逐条构造反例即可；这里保留原题印刷的根 \(\alpha\)，不把它改写为二次方程的系数 \(a\).
...
\]
但是 \(2|\alpha|=3>\frac{23}{20}=4+b\)，而 \(|b|=\frac{57}{20}<4\).
所以第（1）条中的第一个不等式不成立.

\item 取 \(\alpha=\frac{5}{2}\)，\(\beta=\frac{3}{2}\)，则对应的实系数二次方程为
\[
x^2-4x+\frac{15}{4}=0=(x-\tfrac{5}{2})(x-\tfrac{3}{2})
\]
此时 \(2|\alpha|=5<\frac{31}{4}=4+b\)，\(|b|=\frac{15}{4}<4\)，
但 \(|\alpha|=\frac{5}{2}>2\). 所以第（2）条的结论也不成立.
\end{enumerate}

因此，第29题的原题面存在客观矛盾；本稿保留原题面，不能将其改写成另一道题. 若把题中的 \(2|\alpha|\) 改成系数条件 \(2|a|\)，则会得到另一条可证明的命题，但不属于原卷的文字.
\end{solution}
