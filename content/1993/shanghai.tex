...
\begin{problem}
已知等比数列 \(\{a_n\}\)（\(a_n \in \R\)），\(a_1 + a_2 = 9\)，\(a_1 \cdot a_2 \cdot a_3 = 27\)，且 \(S_n = a_1 + a_2 + \cdots + a_n\)（\(n = 1\)，\(2\)，\(\cdots\)），则 \(\lim\limits_{n \to \infty} S_n =\)\fillinblank{}.
...
\begin{center}
\begin{tabular}{@{}cc@{}}
\choicebitmap[max width=0.28\linewidth,max totalheight=4.0cm]{img/1993/shanghai/q11_optA.png} &
\choicebitmap[max width=0.28\linewidth,max totalheight=4.0cm]{img/1993/shanghai/q11_optB.png} \\
\choicebitmap[max width=0.28\linewidth,max totalheight=4.0cm]{img/1993/shanghai/q11_optC.png} &
\choicebitmap[max width=0.28\linewidth,max totalheight=4.0cm]{img/1993/shanghai/q11_optD.png}
\end{tabular}
\end{center}
...
【法二】 \circled{1} 以前同解一. 直线方程代入抛物线方程得 \(x^2 + 2kx - 2 = 0\) \circled{2}.
...
\begin{solution}
【法一】 （1）在平面 \(\beta\) 内作 \(BD \perp PQ\)，垂足为 \(D\)，连接 \(AD\). 因 \(\angle BCD = \angle ACD\)，且 \(BC = AC\)，\(DC\) 为公共边，故 \(\triangle BCD \cong \triangle ACD\)，所以 \(AD \perp PQ\). 于是 \(PQ \perp\) 平面 \(ABD\)，故 \(AB \perp PQ\).
...
\begin{solution}
【法一】 （1）双曲线的半焦距 \(c = \sqrt{a^2 + 1}\)，焦点为 \(F_1(-\sqrt{a^2 + 1},0)\) 和 \(F_2(\sqrt{a^2 + 1},0)\). 设 \(P(x_0,y_0)\) 为椭圆上一点，与长轴的端点 \(A_1\) 及 \(A_2\) 不重合，由对称性，不妨设 \(0 < y_0 \leqslant 1\).
...
【法二】 （1）双曲线的半焦距 \(c = \sqrt{a^2 + 1}\)，焦点为 \(F_1(-\sqrt{a^2 + 1},0)\) 和 \(F_2(\sqrt{a^2 + 1},0)\). 设 \(P(a\cos \theta,\sin \theta)\) 为椭圆上一点，与长轴的端点 \(A_1\) 及 \(A_2\) 不重合，由对称性，不妨设 \(0 < \theta \leqslant \frac{\pi}{2}\). 记 \(\alpha = \angle PF_2x\)，\(\beta = \angle PF_1x\)，故 \(\varphi = \alpha - \beta\). 因 \(PF_1\) 的斜率 \(k_1 = \frac{\sin \theta}{a\cos \theta + \sqrt{a^2 + 1}}\)，
