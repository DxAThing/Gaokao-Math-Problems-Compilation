...
\begin{solution}
由 \(\frac{y-3}{x-2}=1\) 可知 \(x\neq2\)，且 \(y=x+1\)，所以 \(M\) 是直线 \(y=x+1\) 去掉点 \((2,3)\) 后的集合. 由于 \(N\) 是不满足 \(y=x+1\) 的点的集合，故 \(\overline{N}=\{(x,y)\mid y=x+1\}\). 在这条直线上，属于 \(\overline{M}\) 的只有被 \(M\) 排除的点 \((2,3)\)，因此 \(\overline{M}\cap\overline{N}=\{(2,3)\}\)，故选 B.
\end{solution}
\end{problem}
...
\begin{solution}
设等差数列首项为 \(a_1\)，公差为 \(d\)，其中 \(d\neq0\). 则 \(a_n=a_1+(n-1)d=dn+a_1-d\)，且 \(S_n=\frac{n}{2}(a_1+a_n)=\frac{n}{2}(dn+2a_1-d)\). 因此在分母非零的项上有 \(\frac{na_n}{S_n}=\frac{2(dn+a_1-d)}{dn+2a_1-d}\). 由于 \(d\neq0\)，分式分子、分母同除以 \(n\) 后取极限，得到 \(\displaystyle\lim_{n\to\infty}\frac{na_n}{S_n}=2\).
\end{solution}
...
\begin{solution}
体积比在仿射变换下不变，取三棱柱的坐标为 \(A=(0,0,0)\)，\(B=(1,0,0)\)，\(C=(0,1,0)\)，\(A_1=(0,0,1)\)，\(B_1=(1,0,1)\)，\(C_1=(0,1,1)\). 此时 \(E=(\frac{1}{2},0,0)\)，\(F=(0,\frac{1}{2},0)\)，平面 \(EB_1C_1F\) 的方程为 \(2(x+y)=1+z\). 在高度 \(z\) 处，三棱柱的底面截面为 \(x\geqslant0\)，\(y\geqslant0\)，\(x+y\leqslant1\) 的三角形，平面将含 \(A\) 的一侧截成 \(x+y\leqslant\frac{1+z}{2}\) 的相似三角形，其面积占原截面面积的 \(\left(\frac{1+z}{2}\right)^2\). 因而该部分体积占总体积的 \(\int_0^1\left(\frac{1+z}{2}\right)^2\,\mathrm dz=\frac{7}{12}\)，另一部分占 \(\frac{5}{12}\). 所以 \(V_1:V_2=7:5\).
\end{solution}
...
\begin{solution}
设 \(z=x+y\i\)，记 \(r=|z|=\sqrt{x^2+y^2}\). 比较方程 \(z^2+2r=a\) 的虚部，得 \(2xy=0\)，所以必须分两种情况讨论.

当 \(y=0\) 时，\(z=x\)，方程化为 \(x^2+2|x|=a\). 令 \(t=|x|\geqslant0\)，则 \(t^2+2t=a\)，解得 \(t=-1+\sqrt{a+1}\)，因此得到 \(z=\pm\left(-1+\sqrt{a+1}\right)\).

当 \(x=0\) 时，\(z=y\i\)，方程化为 \(-y^2+2|y|=a\). 令 \(t=|y|\geqslant0\)，则 \(t^2-2t+a=0\)，所以 \(t=1\pm\sqrt{1-a}\). 这要求 \(0\leqslant a\leqslant1\)，并给出 \(z=\pm\left(1+\sqrt{1-a}\right)\i\) 或 \(z=\pm\left(1-\sqrt{1-a}\right)\i\). 综上，当 \(0\leqslant a\leqslant1\) 时，全部解为 \(z=\pm\left(-1+\sqrt{a+1}\right)\)，以及 \(z=\pm\left(1+\sqrt{1-a}\right)\i\)，\(z=\pm\left(1-\sqrt{1-a}\right)\i\)；当 \(a>1\) 时，全部解为 \(z=\pm\left(-1+\sqrt{a+1}\right)\).
\end{solution}
