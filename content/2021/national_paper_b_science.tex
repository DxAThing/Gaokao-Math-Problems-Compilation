...
\bitmapfigure[max width=5.2cm]{img/2021/national_paper_B_science/q9_fig1.png}
...
故 \(MB=\frac{ED\cdot DF}{GC-EH}=\frac{\text{表高}\times \text{表距}}{\text{表目距的差}}\)，所以高 \(AB=\frac{\text{表高}\times \text{表距}}{\text{表目距的差}}+\text{表高}\).
\end{solution}
...
\solutionfigure{\bitmapfigure[max width=4.8cm]{img/2021/national_paper_B_science/q10_solution_fig1.png}}
...
所以 \(e=\frac{c}{a}\le \frac{\sqrt2}{2}\)，故 \(e\in \left( 0,\frac{\sqrt2}{2} \right]\).
\end{solution}
...
当 \(x\ge 0\) 时，\(1+x=\sqrt{(1+x)^2}\ge \sqrt{1+2x}\)，故 \(f'(x)\le 0\).
所以 \(f(x)\) 在 \([0,+\infty)\) 上单调递减，所以 \(f(0.02)<f(0)=0\)，故 \(b<c\).

再设 \(g(x)=2\ln(1+x)-\sqrt{1+4x}+1\)，
则 \(a-c=g(0.01)\)，易得
\(g'(x)=\frac{2}{1+x}-\frac{4}{2\sqrt{1+4x}}=2\cdot \frac{\sqrt{1+4x}-(1+x)}{(1+x)\sqrt{1+4x}}\).
当 \(0\le x<2\) 时，\(\sqrt{1+4x}\ge \sqrt{1+2x+x^2}=1+x\)，所以 \(g'(x)\) 在 \([0,2)\) 上 \(\ge 0\).
...
\begin{solution}
\(1^{\odot}\)因为 \(PD\perp\) 平面 \(ABCD\)，且矩形 \(ABCD\) 中，\(AD\perp DC\).所以以 \(DA\)，\(DC\)，\(DP\) 分别为 \(x\)，\(y\)，\(z\) 轴正方向，\(D\) 为原点建立空间直角坐标系 \(D-xyz\).设 \(BC=t\)，则
...
\(2^{\odot}\)设平面 \(APM\) 的一个法向量为 \(\bs m=(x,y,z)\)，由于
...
\(1^{\odot}\)当 \(x<0\) 时，易得 \(H'(x)<0\)，则 \(H(x)\) 在 \((-\infty,0)\) 上单调递减.
因为 \(H(0)=0\)，所以 \(H(x)>H(0)=0\)，得证.

\(2^{\odot}\)当 \(0<x<1\) 时，易得 \(H'(x)>0\)，则 \(H(x)\) 在 \((0,1)\) 上单调递增.
因为 \(H(0)=0\)，所以 \(H(x)>H(0)=0\)，得证.
...
\begin{solution}
\(1^{\odot}\)圆 \(C\) 的参数方程为 \(x=2+\cos\theta\)，\(y=1+\sin\theta\)（\(\theta\) 为参数）.
