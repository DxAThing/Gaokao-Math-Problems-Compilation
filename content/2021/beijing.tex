...
\bitmapfigure[max width=3.5cm,max totalheight=4.8cm]{img/2021/beijing/q4_fig1.png}
...
\bitmapfigure[max width=3.5cm,max totalheight=4.8cm]{img/2021/beijing/q8_fig1.png}
...
\begin{enumerate}
    ...
    条件 \circled{1}：\(c=\sqrt2 b\)；条件 \circled{2}：周长为 \(4+2\sqrt3\)；条件 \circled{3}：面积为 \(S_{\triangle ABC}=\frac{3\sqrt3}{4}\). 
\end{enumerate}
...
\bitmapfigure[max width=3.5cm,max totalheight=4.8cm]{img/2021/beijing/q17_fig1.png}
...
\end{solution}
\begin{problem}
定义 \(R_p\) 数列 \(\{a_n\}\)：对实数 \(p\)，满足：\circled{1} \(a_1+p\ge0\)，\(a_2+p=0\)；\circled{2} \(\forall n\in\N^*\)，\(a_{4n-1}<a_{4n}\)；\circled{3} \(a_{m+n}\in\{a_m+a_n+p,\ a_m+a_n+p+1\}\)，\(m,n\in\N^*\). 
