...
\begin{center}
\bitmapfigure[float=inline,width=4.9cm]{img/2021/beijing/q8_fig1.png}
\end{center}
...
\begin{enumerate}
    \item \(B=\dfrac{\pi}{6}\).
    \item 选择条件\(\circled{2}\)时，\(BC\) 边上中线长为 \(\sqrt7\)；选择条件\(\circled{3}\)时，\(BC\) 边上中线长为 \(\dfrac{\sqrt{21}}{2}\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
    \item \(F\) 为 \(B_1C_1\) 中点.
    \item \(\dfrac{A_1M}{A_1B_1}=\dfrac12\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item 检测总次数为 \(20\).
        \item \(P(X=20)=\dfrac{1}{11}\)，\(P(X=30)=\dfrac{10}{11}\)，\(EX=\dfrac{320}{11}\).
    \end{enumerate}
    \item \(EY>EX\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
    \item 切线方程为 \(y=-4x+5\).
    \item \(a=4\)；单调递增区间为 \((-\infty,-1)\) 和 \((4,+\infty)\)，单调递减区间为 \((-1,4)\)，最大值为 \(1\)，最小值为 \(-\dfrac14\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
    \item 椭圆 \(E\) 的方程为 \(\dfrac{x^2}{5}+\dfrac{y^2}{4}=1\).
    \item \(k\in[-3,-1)\cup(1,3]\).
\end{enumerate}
\end{solution}
