...
\begin{solution}因为 \( A\cap B=\{1\} \)，所以 \( \left( A\cap B \right)\cup C=\{0,1,2,4\} \).
故选 C.
\end{solution}
...
\begin{solution}由题意，若 \( a>6 \)，则 \( a^2>36 \)，故充分性成立；
...
\choices
    {\choicebitmap[width=0.15\paperwidth]{img/2021/tianjin/q3_choiceA.png}}
    {\choicebitmap[width=0.15\paperwidth]{img/2021/tianjin/q3_choiceB.png}}
    {\choicebitmap[width=0.15\paperwidth]{img/2021/tianjin/q3_choiceC.png}}
    {\choicebitmap[width=0.15\paperwidth]{img/2021/tianjin/q3_choiceD.png}}
...
\begin{solution}设 \( y=f(x)=\dfrac{\ln |x|}{x^2+2} \)，则函数 \( f(x) \) 的定义域为 \( \{x\mid x\ne 0\} \)，关于原点对称，
...
\texfigure[max width=5.5cm]{img_repaint/2021/tianjin/q4_fig1.tex}
...
\begin{solution}由频率分布直方图可知，评分在区间 \( [82,86) \) 内的影视作品数量为
...
\begin{solution}因为 \( \log_{2}0.3<\log_{2}1=0 \)，所以 \( a<0 \)；
...
\begin{solution}如下图所示，设两个圆锥的底面圆圆心为点 \(D\)，设圆锥 \(AD\) 和圆锥 \(BD\) 的高之比为 \( 3:1 \)，即 \( AD=3BD \).

\solutionfigure{\bitmapfigure[max width=3.6cm]{img/2021/tianjin/q6_fig1.png}}
设球的半径为 \(R\)，则
...
\begin{solution}因为 \( 2^a=5^b=10 \)，所以 \( a=\log_{2}10 \)，\( b=\log_{5}10 \)，所以 \( \frac{1}{a}+\frac{1}{b}=\frac{1}{\log_{2}10}+\frac{1}{\log_{5}10}=\lg 2+\lg 5=\lg 10=1 \).
故选 C.
\end{solution}
...
\begin{solution}设双曲线 \( \dfrac{x^2}{a^2}-\dfrac{y^2}{b^2}=1 \)\( \left( a>0,b>0 \right) \) 与抛物线 \( y^2=2px \)\( \left( p>0 \right) \) 的公共焦点为 \( \left( c,0 \right) \)，则抛物线的准线为 \( x=-c \).
令 \( x=-c \)，则
...
\begin{solution}因为 \( x^2-2(a+1)x+a^2+5=0 \) 最多有 \( 2 \) 个根，所以 \( \cos\left( 2\pi x-2\pi a \right)=0 \) 至少有 \( 4 \) 个根.
...
\(2^{\odot}\) 当 \( x\ge a \) 时，\( f(x)=x^2-2(a+1)x+a^2+5 \)，
...
\begin{solution}\(\frac{9+2\i}{2+\i} =\frac{(9+2\i)(2-\i)}{(2+\i)(2-\i)} =\frac{20-5\i}{5} =4-\i\).
故答案为：\( 4-\i \).
\end{solution}
...
\begin{solution} \( \left( 2x^3+\dfrac{1}{x} \right)^6 \) 的展开式的通项为
...
\begin{solution}设直线 \(AB\) 的方程为 \( y=\sqrt3\,x+b \)，则点 \( A\left( 0,b \right) \).
...
\begin{solution}由题可得一次活动中，甲获胜的概率为
...
\begin{solution}设 \( BE=x \)，\( x\in \left( 0,\dfrac{1}{2} \right) \). 因为 \(ABC\) 为边长为 \( 1 \) 的等边三角形，\( DE\perp AB \)，所以
...
\solutionfigure{\bitmapfigure[max width=5cm]{img/2021/tianjin/q17_solution_fig1.png}}
则
...
\solutionfigure{\bitmapfigure[max width=4.2cm]{img/2021/tianjin/q18_solution_fig1.png}}
...
（2）由题意知，\( \frac{a_na_{n+1}}{c_n^2-c_{2n}}=\frac{(2n-1)(2n+1)}{2\cdot 4^n}=\frac{4n^2-1}{2\cdot 4^n}<\frac{4n^2}{2\cdot 2^{2n}}=\frac{n^2}{2^{2n-1}} \).
所以
...
\solutionfigure{\bitmapfigure[max width=4.6cm]{img/2021/tianjin/q20_solution_fig1.png}}
