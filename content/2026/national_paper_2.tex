...
\bitmapfigure[max width=8.8cm,max totalheight=5.8cm]{img/2026/national_paper_2/q15_fig1.png}
...
（ii）因为 \(X\sim B\left( 100,0.15 \right)\)，所以 \(E(X)=100\times 0.15=15\)，\(D(X)=100\times 0.15\times 0.85=\frac{51}{4}\). 因此 \(E(X)=15\)，\(D(X)=\frac{51}{4}\).
\end{solution}
\begin{problem}
...
（ii）对方程 \(\left( t^2-2 \right)x^2+2t^2y^2+4tx-2t^2=0\) 配方. 当 \(t^2-2=0\)，即 \(t=\pm \sqrt{2}\) 时，方程中 \(x\) 只一次出现，曲线 \(M\) 没有对称中心.
