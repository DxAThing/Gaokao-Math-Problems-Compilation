...
\texfigure[max width=8.8cm]{img_repaint/2026/national_paper_2/q15_fig1.tex}

\end{problem}

\begin{solution}
\begin{enumerate}
\item 由直方图可知，各组组距都为 \(10\). 各组的频率分别为 \(0.05\)，\(0.10\)，\(0.20\)，\(0.25\)，\(0.15\)，\(0.15\)，\(0.05\)，\(0.05\).
...
\item
    \begin{enumerate}
    \item 首次出现故障的时间小于 \(365\) 天的频率为前两组频率之和，所以 \(\hat{p}=0.05+0.10=0.15\).

    \item 因为 \(X\sim B\left( 100,0.15 \right)\)，所以 \(E(X)=100\times 0.15=15\)，\(D(X)=100\times 0.15\times 0.85=\frac{51}{4}\). 因此 \(E(X)=15\)，\(D(X)=\frac{51}{4}\).
    \end{enumerate}
\end{enumerate}
\end{solution}
...
\bitmapfigure[max width=5.6cm]{img/2026/national_paper_2/q16_fig1.png}

\end{problem}

\begin{solution}
\begin{enumerate}
\item 因为 \(AE\perp CE\)，\(AE\perp DE\)，而 \(CE\) 与 \(DE\) 是平面 \(CED\) 内两条相交直线，所以 \(AE\perp \text{平面} CED\). 于是 \(AE\perp CD\).
...
\item 以 E 为坐标原点，所在平面 \(CED\) 为 \(xOy\) 平面，建立空间直角坐标系. 由题意可取 \(E(0,0,0)\)，\(D(2,0,0)\)，\(B(-1,0,0)\)，\(A(0,0,\sqrt{2})\). 设 \(C(x,y,0)\).
...
\begin{solution}
\begin{enumerate}
\item 因为 \(A+C=\pi-B\)，所以 \(\cos^2\left( A+C \right)=\cos^2 B=\left( \frac{3}{4} \right)^2=\frac{9}{16}\). 由题设得 \(\sin A\sin C=1-\frac{9}{16}=\frac{7}{16}\).
...
\item 设边 \(a\)，\(b\)，\(c\) 分别是角 \(A\)，\(B\)，\(C\) 的对边，设 \(\lambda=2R\)，其中 \(R\) 为外接圆半径. 由正弦定理，\(a=\lambda \sin A\)，\(b=\lambda \sin B\)，\(c=\lambda \sin C\).
...
\begin{solution}
\begin{enumerate}
\item 设椭圆 \(E\) 的右焦点为 \(F(c,0)\). 因为椭圆方程为 \(\frac{x^2}{a^2}+y^2=1\)，所以其短半轴长为 \(1\)，并且 \(c^2=a^2-1\).
...
\begin{solution}
\begin{enumerate}
\item 由切线方程 \(y=-2x+1\) 可知 \(f(0)=1\)，\(f'(0)=-2\). 而 \(f(0)=b\)，所以 \(b=1\).
又 \(f'(x)=\left( x+1 \right)\e^x+a\)，因此 \(f'(0)=1+a=-2\)，故 \(a=-3\).

\item 由（1）知 \(f(x)=x\e^x-3x+1\). 设 \(g(x)=f\left( x+m \right)-f(x)-m\). 则 \(g(x)=\left( x+m \right)\e^{x+m}-x\e^x-4m=\e^x\left[ \left( \e^m-1 \right)x+m\e^m \right]-4m\).
...
\item 设 \(h(x)=f\left( k+x \right)+f\left( k-x \right)-2f(k)\). 代入 \(f(x)=x\e^x-3x+1\)，得 \(h(x)=\left( k+x \right)\e^{k+x}+\left( k-x \right)\e^{k-x}-2k\e^k\). 提取公因子 \(\e^k\)，得 \(h(x)=\e^k\left[ \left( k+x \right)\e^x+\left( k-x \right)\e^{-x}-2k \right]\). 再化简为 \(h(x)=2\e^k\left[ k\left( \cosh x-1 \right)+x\sinh x \right]\).
