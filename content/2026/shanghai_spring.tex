...
\choices
    {\bitmapinclude[max width=3.0cm,max totalheight=3.2cm]{img/2026/shanghai_spring/q15_choice_a.png}}
    {\bitmapinclude[max width=3.0cm,max totalheight=3.2cm]{img/2026/shanghai_spring/q15_choice_b.png}}
    {\bitmapinclude[max width=3.0cm,max totalheight=3.2cm]{img/2026/shanghai_spring/q15_choice_c.png}}
    {\bitmapinclude[max width=3.0cm,max totalheight=3.2cm]{img/2026/shanghai_spring/q15_choice_d.png}}
...
而平面 \(BC_1D\) 中
\(\overrightarrow{BC_1}=(-1,3,3)\)，\(\overrightarrow{BD}=(-4,4,0)\)，
可得法向量
\(\overrightarrow{BC_1}\times\overrightarrow{BD}=(-12,-12,8)\parallel (1,1,3)^\perp\)，
故 \(AA_1\parallel\) 平面 \(BC_1D\).

再求三棱锥 \(A_1-BC_1D\) 的体积. 用向量 \(\overrightarrow{A_1B}=(3,-1,-3)\)，\(\overrightarrow{A_1C_1}=(2,2,0)\)，\(\overrightarrow{A_1D}=(-1,3,-3)\)，得
\[
...
\begin{solution}
（1）由 \(f(\pi/12)=0\) 得 \(\sin(\pi/6+\varphi)=0\)，故 \(\varphi=\dfrac{5\pi}{6}\). 此时 \(f'(x)=2\cos(2x+\varphi)\)，所以 \(f(0)=\sin\frac{5\pi}{6}=\frac12\)，\(f'(0)=2\cos\frac{5\pi}{6}=-\sqrt3\).
故切线方程为 \(y=-\sqrt3\,x+\dfrac12\).

（2）最小正周期为 \(3\pi\)，故 \(\omega=\dfrac23\). 于是 \(f(x)=\sin\left(\frac23x+\varphi\right)\).
把区间 \([0,2026\pi)\) 分成若干个长度为 \(3\pi\) 的周期区间后计数，可知恰有 1351 个解当且仅当相位 \(\varphi\) 落在
\(0<\varphi\le \frac{\pi}{12}\) 或 \(\frac{\pi}{4}<\varphi\le \frac{3\pi}{4}\).
\end{solution}
...
（3）设直线 \(l\) 的方程为 \(x=ny+m\)（\(n\ne0\)）. 联立双曲线方程并用 \(\overrightarrow{F_1A'}\cdot\overrightarrow{F_2B}=0\) 化简，可得 \(m^2-2m>0\)，\(m\ne 2\). 结合 \(m>0\) 得 \(m\in\left(\frac43,2\right)\cup(2,+\infty)\).
\end{solution}
