...
\begin{solution}
由正视图和侧视图可知，该几何体的下部是棱长为 \(2\) 的正方体；由俯视图可知，上部四棱锥的底面是边长为 \(2\) 的正方形，且由正视图可知其高为 \(2\). 因此其体积为
...
设 \(A\)，\(B\) 的横坐标分别为 \(x_A\)，\(x_B\). 三点 \(A\)，\(B\)，\(C\) 共线，且 \(C\) 在 \(y\) 轴上，所以同一直线上各线段长度之比等于其横坐标差的绝对值之比；又 \(x_C=0\)，从而
\(\dfrac{BC}{AC}=\dfrac{x_B}{x_A}\). 三角形 \(BCF\) 与 \(ACF\) 以同一直线上的 \(BC\)，\(AC\) 为底边，它们的高相同，因此
...
取沿 \(DC\) 的单位向量为 \(\bs{e}\)，并将 \(DA'\)，\(DB\) 的单位方向向量分别分解为沿 \(DC\) 与垂直于 \(DC\) 的分量. 由 \(\angle A'DC=\theta\) 和 \(\angle BDC=\pi-\theta\)，这两个单位向量可写成
\(\cos\theta\,\bs{e}+\sin\theta\,\bs{u}\) 和 \(-\cos\theta\,\bs{e}+\sin\theta\,\bs{v}\)，其中 \(\bs{u}\)，\(\bs{v}\) 是相应的垂直分量方向. 过 \(D\) 作垂直于 \(CD\) 的截面，根据二面角平面角的定义，\(\bs{u}\cdot\bs{v}=\cos\alpha\). 因而
...
\begin{solution}
令 \(u=2x+y-2\)，\(v=6-x-3y\). 由三角不等式，
\(|u|+|v|\geqslant|v-u|\geqslant v-u=8-3x-4y\).
...
以 \(OB\)，\(OA\)，\(OA_1\) 所在直线分别为 \(x\)，\(y\)，\(z\) 轴建立空间直角坐标系，并取
\(A(0,\sqrt{2},0)\). 则
\(A_1(0,0,\sqrt{14})\)，\(B(\sqrt{2},0,0)\)，\(C(-\sqrt{2},0,0)\). 由三棱柱的平移关系，平移向量为
\(\overrightarrow{AA_1}=(0,-\sqrt{2},\sqrt{14})\)，所以 \(B_1=(\sqrt{2},-\sqrt{2},\sqrt{14})\)，\(C_1=(-\sqrt{2},-\sqrt{2},\sqrt{14})\). 因而 \(D\) 为 \(B_1C_1\) 的中点，即 \(D(0,-\sqrt{2},\sqrt{14})\).
...
\(\bs{p}=\overrightarrow{BA_1}-\frac{\overrightarrow{BA_1}\cdot\bs{u}}{|\bs{u}|^2}\bs{u}=\left(-\frac{\sqrt{2}}{9},\frac{8\sqrt{2}}{9},\frac{\sqrt{14}}{9}\right)\)，\(\bs{q}=\overrightarrow{BB_1}-\frac{\overrightarrow{BB_1}\cdot\bs{u}}{|\bs{u}|^2}\bs{u}=\left(\frac{8\sqrt{2}}{9},-\frac{\sqrt{2}}{9},\frac{\sqrt{14}}{9}\right)\)
...
将上式写成二次型的交叉项计算为
\(Q(P+t(m,-1))-Q(P-t(m,-1))=4t\left(\dfrac{um}{2}-y\right)=0\)，这也直接验证了 \(um=2y\).

记 \(v=(m,-1)\). 由于 \(A\)，\(B\) 在椭圆上且交叉项已经为零，故
\[
...
\(T^2=\frac{(3z-2)(z+2)}{8z^2}\)，\(z>\frac23\).
...
\(a_{n+1}=g(a_n)\geqslant g\left(\frac{1}{2n}\right)=\frac{2n-1}{4n^2}\geqslant\frac{1}{2(n+1)}\)，\(a_{n+1}=g(a_n)\leqslant g\left(\frac{1}{n+1}\right)=\frac{n}{(n+1)^2}\leqslant\frac{1}{n+2}\).
