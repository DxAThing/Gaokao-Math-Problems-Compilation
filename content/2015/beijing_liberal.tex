...
\bitmapinlinefigure[0.84\linewidth][]{img/2015/beijing_liberal/q14_fig1.png}
\end{problem}
...
\begin{solution}
短答案：\(f(x)\) 的最小正周期为 \(2\pi\)；在区间 \(\left[0, \frac{2\pi}{3}\right]\) 上的最小值为 \(-\sqrt{3}\).
\end{solution}
...
\begin{solution}
短答案：\(a_n=2n+2\)；\(b_6\) 与数列 \(\{a_n\}\) 的第 \(63\) 项相等.
\end{solution}
...
\begin{solution}
短答案：第（1）问为 \(0.2\)，第（2）问为 \(0.3\)，第（3）问为丙.
\end{solution}
...
\begin{solution}
短答案：\(VB\parallel\) 平面 \(MOC\)；平面 \(MOC\perp\) 平面 \(VAB\)；三棱锥 \(V-ABC\) 的体积为 \(\dfrac{\sqrt{3}}{3}\).
\end{solution}
...
\begin{solution}
短答案：\(f(x)\) 的单调递减区间为 \((0,\sqrt{k})\)，单调递增区间为 \((\sqrt{k},+\infty)\)，在 \(x=\sqrt{k}\) 处取得极小值 \(\dfrac{k(1-\ln k)}{2}\)；若存在零点，则在区间 \((1,\sqrt{\e}]\) 上仅有一个零点.
\end{solution}
...
\begin{solution}
短答案：椭圆 \(C\) 的离心率为 \(\dfrac{\sqrt{6}}{3}\)；直线 \(BM\) 的斜率为 \(1\)；直线 \(BM\) 与直线 \(DE\) 平行.
\end{solution}
