...
\item 若甲参加活动，求甲得分 X 的分布列和数学期望 EX.
\end{enumerate}
\end{problem}
...
\begin{enumerate}[label={（\arabic*）},itemsep=0.2em]
\item 求椭圆 \(C\) 的方程；
\item 设椭圆 \(E: \frac{x^2}{4a^2} + \frac{y^2}{4b^2} = 1, P\) 为椭圆 \(C\) 上任意一点，过点 \(P\) 的直线 \(y = kx + m\) 交椭圆 \(E\) 于 \(A, B\) 两点，射线 \(PO\) 交椭圆 \(E\) 于点 \(Q\). \circled{1} 求 \( \frac{|OQ|}{|OP|} \) 的值；\circled{2} 求 \(\triangle ABQ\) 面积的最大值.
\end{enumerate}
\end{problem}
