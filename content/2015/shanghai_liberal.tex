...
\begin{solution}
设 \(z=u+v\i\)，其中 \(u\)，\(v\in\R\)，则 \(\overline{z}=u-v\i\). 代入已知等式，得 \(3z+\overline{z}=4u+2v\i=1+\i\). 比较实部和虚部，得 \(4u=1\)，\(2v=1\)，所以 \(u=\dfrac14\)，\(v=\dfrac12\). 因而 \(z=\dfrac14+\dfrac12\i\).
\end{solution}
...
\begin{solution}
展开式的通项为
\(T_{r+1}=\mathrm C_6^r(2x)^{6-r}\left(\frac{1}{x^2}\right)^r=\mathrm C_6^r2^{6-r}x^{6-3r}\)，\(r=0\)，\(1\)，\(\dots\)，\(6\).
要取得常数项，须有 \(6-3r=0\)，所以 \(r=2\). 因而常数项为 \(\mathrm C_6^2 2^4=15\times16=240\).
\end{solution}
...
\begin{solution}
由于 \(\bs{a}\perp\bs{b}\)，有 \(|\bs{a}+\bs{b}|=\sqrt{|\bs{a}|^2+|\bs{b}|^2}\). 对于固定的 \(|\bs{a}|\)、\(|\bs{b}|\)、\(|\bs{c}|\)，由三角不等式，且当 \(\bs{c}\) 与 \(\bs{a}+\bs{b}\) 同向时取等号，得
\[
|\bs{a}+\bs{b}+\bs{c}|\leqslant\sqrt{|\bs{a}|^2+|\bs{b}|^2}+|\bs{c}|
\]
将 \(1\)，\(2\)，\(3\) 分别作为 \(|\bs{c}|\) 及 \(|\bs{a}|\)，\(|\bs{b}|\) 的取值，可能的上界为 \(1+\sqrt{13}\)、\(2+\sqrt{10}\)、\(3+\sqrt5\). 比较可得 \(3+\sqrt5\) 最大，且取 \(|\bs{c}|=3\)、\(|\bs{a}|=1\)、\(|\bs{b}|=2\) 并令 \(\bs{c}\) 与 \(\bs{a}+\bs{b}\) 同向时可以达到该上界. 所以最大值为 \(3+\sqrt5\).
\end{solution}
...
取
\(x_1=0\)，\(x_2=\frac\pi2\)，\(x_3=\frac{3\pi}{2}\)，\(x_4=\frac{5\pi}{2}\)，\(x_5=\frac{7\pi}{2}\)，\(x_6=\frac{9\pi}{2}\)，\(x_7=\frac{11\pi}{2}\)，\(x_8=6\pi\).
对应的正弦值为 \(0\)，\(1\)，\(-1\)，\(1\)，\(-1\)，\(1\)，\(-1\)，\(0\)，相邻差的绝对值之和为 \(1+2+2+2+2+2+1=12\). 因此 \(m=8\) 可以达到，故最小值为 \(8\).
\end{solution}
...
\]
除以 \(x_n-1\)，并令 \(n\to\infty\)，得到
\(x_n+1+(y_n+1)\frac{y_n-1}{x_n-1}=0\)，\(\Longrightarrow\)，\(\lim_{n\to\infty}\frac{y_n-1}{x_n-1}=-\frac{2}{2}=-1\).
因此极限为 \(-1\)，故选 A.
\end{solution}
...
\(f(t)=MN=\sqrt{(3t-3)^{2}+(4t-8t+3)^{2}}=\sqrt{25t^{2}-42t+18}\)，\(\frac{3}{8}\leqslant t\leqslant\frac{7}{8}\).
...
\item 设 \(l_{1}\) 的斜率为 \(k\)，则 \(l_{2}\) 的斜率为 \(\frac{m}{k}\). 先取 \(k\neq0\)，由两条过原点的直线与椭圆的交点坐标，取绝对值后有
\[
