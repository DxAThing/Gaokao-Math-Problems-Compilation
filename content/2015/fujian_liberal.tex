...
\bitmapfigure[max width=0.48\linewidth,max totalheight=6.5cm,scale=1.3]{img/2015/fujian_liberal/q4_fig1.png}
...
\begin{problem}
如图，矩形 \(ABCD\) 中，点 \(A\) 在 \(x\) 轴上，点 \(B\) 的坐标为 \((1,0)\)，且点 \(C\) 与点 \(D\) 在函数 \(f(x) = \begin{cases} x + 1, & x \geqslant 0, \\ -\frac{1}{2}x + 1, & x < 0 \end{cases}\) 的图象上. 若在矩形 \(ABCD\) 内随机取一点，则此点取自阴影部分的概率等于（\quad）
...
\begin{problem}
变量 \(x\)，\(y\) 满足约束条件 \(\begin{cases} x + y \geqslant 0, \\ x - 2y + 2 \geqslant 0, \\ mx - y \leqslant 0 \end{cases}\) 若 \(z = 2x - y\) 的最大值为 2，则实数 \(m\) 等于（\quad）
...
\begin{problem}
已知椭圆 \(E: \frac{x^2}{a^2} + \frac{y^2}{b^2} = 1 (a > b > 0)\) 的右焦点为 \(F\)，短轴的一个端点为 \(M\)，直线 \(l: 3x - 4y = 0\) 交椭圆 \(E\) 于 \(A\)，\(B\) 两点. 若 \(|AF| + |BF| = 4\)，点 \(M\) 到直线 \(l\) 的距离不小于 \(\frac{5}{4}\)，则椭圆 \(E\) 的离心率的取值范围是（\quad）
