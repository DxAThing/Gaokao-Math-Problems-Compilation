...
\bitmapinlinefigure[0.52\linewidth][max width=0.52\linewidth]{img/2015/fujian_science/q6_fig1.png}
...
\begin{problem}
\((x + 2)^{4}\) 的展开式中，\(x^{2}\) 的系数等于\fillinblank{}.
\end{problem}
...
\begin{solution}
短答案：当天银行卡被锁定的概率为 \(\dfrac{1}{2}\)；\(X\) 的分布列为 \(P(X=1)=\dfrac{1}{6}\)、\(P(X=2)=\dfrac{1}{6}\)、\(P(X=3)=\dfrac{2}{3}\)，数学期望为 \(E(X)=\dfrac{5}{2}\).
\end{solution}
...
\begin{solution}
短答案：\(GF\parallel\) 平面 \(ADE\)；平面 \(AEF\) 与平面 \(BEC\) 所成锐二面角的余弦值为 \(\dfrac{2}{3}\).
\end{solution}
...
\end{enumerate}
\bitmapinlinefigure[0.52\linewidth][max width=0.52\linewidth]{img/2015/fujian_science/q18_fig1.png}
\end{problem}

\begin{solution}
短答案：椭圆 \(E\) 的方程为 \(\dfrac{x^2}{4}+\dfrac{y^2}{2}=1\)；点 \(G\) 在以 \(AB\) 为直径的圆外.
\end{solution}
...
\begin{solution}
短答案：第（1）问结论为当 \(x>0\) 时 \(\ln(1+x)<x\)；第（2）问的条件为 \(k<1\)；第（3）问 \(k=1\).
\end{solution}
...
\begin{solution}
短答案：圆 \(C\) 的普通方程为 \((x-1)^2+(y+2)^2=9\)，直线 \(l\) 的直角坐标方程为 \(x+y-m=0\)；\(m=-3+2\sqrt{2}\) 或 \(m=-3-2\sqrt{2}\).
\end{solution}
...
\begin{solution}
短答案：\(f(x)=2\sin x\)，图象的对称轴方程为 \(x=k\pi+\frac{\pi}{2}\)（\(k\in\Z\)）；\(m\) 的取值范围为 \((-\sqrt{5},\sqrt{5})\)，且 \(\cos(\alpha-\beta)=\frac{2m^2}{5}-1\).
\end{solution}
...
\begin{solution}
短答案：\(A^{-1}=\begin{pmatrix}\frac{3}{2}&-\frac{1}{2}\\-2&1\end{pmatrix}\)，\(C=\begin{pmatrix}\frac{3}{2}&2\\-2&-3\end{pmatrix}\).
\end{solution}
...
\begin{solution}
短答案：\(a+b+c=4\)；\(\frac{1}{4}a^2+\frac{1}{9}b^2+c^2\) 的最小值为 \(\frac{8}{7}\).
\end{solution}
