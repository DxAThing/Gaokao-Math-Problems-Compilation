...
\begin{problem}
在直角坐标系 \(xOy\) 中，以 \(O\) 为极点，\(x\) 轴的正半轴为极轴建立极坐标系. 已知直线 \(l\) 的极坐标方程为 \(\rho(\sin\theta-3\cos\theta)=0\)，曲线 \(C\) 的参数方程为 \(x=t-\dfrac1t,\ y=t+\dfrac1t\)（\(t\) 为参数），\(l\) 与 \(C\) 相交于 \(A,B\) 两点，则 \(|AB|=\)\fillinblank{}.
\end{problem}
