...
\begin{problem}
已知直线 \(l\) 的参数方程为 \(\left\{ \begin{array}{l} x = -1 + t, \\ y = 1 + t, \end{array} \right.\) （\(t\) 为参数），以坐标原点为极点，\(x\) 轴的正半轴为极轴建立极坐标系，曲线 \(C\) 的极坐标方程为 \(\rho^2 \cos 2\theta = 4\rho\) （\(\rho > 0, \frac{3\pi}{4} < \theta < \frac{5\pi}{4}\)），则直线 \(l\) 与曲线 \(C\) 的交点的极坐标为\fillinblank{}.
\end{problem}
