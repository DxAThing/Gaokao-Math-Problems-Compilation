...
\begin{optionlist}
\textbf{
\item[\textbf{A.}] }设矩阵 \(M = \begin{pmatrix} a & 0 \\ 0 & b \end{pmatrix}\).（其中 \(a > 0\)，\(b > 0\)）
...
\textbf{
\item[\textbf{B.}] }在直角坐标系 \(xOy\) 中，直线 \(l\) 的方程为 \(x-y+4=0\)，曲线 \(C\) 的参数方程为 \(\begin{cases} x=\sqrt{3}\cos\alpha,\\ y=\sin\alpha \end{cases}\)（\(\alpha\) 为参数）.
...
\textbf{
\item[\textbf{C.}] }设不等式 \(|2x-1|<1\) 的解集为 \(M\).
