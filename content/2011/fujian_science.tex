...
\item 设 \(AB=u\)，\(AD=v\). 由题设，\(u+v=4\)，且 \(AP=AB=u\). 在底面内过点 \(C\) 作 \(CE\parallel AB\)，交 \(AD\) 于点 \(E\). 因为 \(AB\perp AD\)，所以 \(CE\perp AD\)，三角形 \(CDE\) 是直角三角形. 又 \(CD=\sqrt{2}\)，\(\angle CDE=\angle CDA=45^\circ\)，所以 \(DE=CD\cos45^\circ=1\)，\(CE=CD\sin45^\circ=1\). 建立空间直角坐标系，取 \(A=(0,0,0)\)，\(B=(u,0,0)\)，\(D=(0,v,0)\)，\(P=(0,0,u)\)，则 \(E=(0,v-1,0)\)，\(C=(1,v-1,0)\).
