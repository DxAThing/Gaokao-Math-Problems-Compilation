...
\begin{solution}
由正视图和俯视图可知，几何体沿侧视方向的轮廓应为关于中线对称的三角形，且中线为相应的可见棱. 四个选项中只有 D 的侧视图同时满足这一轮廓和中线位置，故选 D.
\end{solution}
...
\begin{solution}
函数 \(f(x)=\e^x+4x-3\) 的导数为 \(f'(x)=\e^x+4>0\)，所以 \(f(x)\) 在 \(\R\) 上单调递增. 又 \(f\left(\frac{1}{4}\right)=\e^{1/4}-2<0\)，\(f\left(\frac{1}{2}\right)=\e^{1/2}-1>0\)，故其唯一零点在 \(\left(\frac{1}{4},\frac{1}{2}\right)\) 内，选 C.
\end{solution}
...
\begin{solution}
由垂直条件，\((\bs{a}+\bs{b})\cdot(k\bs{a}-\bs{b})=0\). 由于 \(\bs{a}\)，\(\bs{b}\) 是单位向量，设 \(\bs{a}\cdot\bs{b}=c\)，则
\[
(\bs{a}+\bs{b})\cdot(k\bs{a}-\bs{b})=k-c+kc-1=(k-1)(1+c).
\]
两向量不共线，所以 \(c>-1\)，从而 \(1+c\neq0\)，故 \(k=1\).
\end{solution}
...
\begin{solution}
设球的半径为 \(R\)，公共底面半径为 \(r\). 由题意，\(\pi r^2=\frac{3}{16}\cdot4\pi R^2=\frac{3}{4}\pi R^2\)，所以 \(r=\frac{\sqrt{3}}{2}R\). 设球心到公共底面所在平面的距离为 \(d\)，则
\[
d=\sqrt{R^2-r^2}=\frac{R}{2}.
\]
两个圆锥的顶点在公共底面两侧的球面上，因此它们的高分别为 \(R-d=\frac{R}{2}\) 和 \(R+d=\frac{3R}{2}\). 圆锥底面相同，体积大小规律与高大小规律相同，故所求比值为
\[
...
\[
h'(x)=\frac{(x-1)^2}{2x^2}\geqslant0,\qquad h(1)=0.
\]
