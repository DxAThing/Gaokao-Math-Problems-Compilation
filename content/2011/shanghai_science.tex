...
\begin{problem}
已知点 \(O(0,0), Q_0(0,1)\) 和点 \(R_0(3,1)\)，记 \(Q_0R_0\) 的中点为 \(P_1\)，取 \(Q_0P_1\) 和 \(P_1R_0\) 中的一条，记其端点为 \(Q_1, R_1\)，使之满足 \((|OQ_1| - 2)(|OR_1| - 2) < 0\)，记 \(Q_1R_1\) 的中点为 \(P_2\)，取 \(Q_1P_2\) 和 \(P_2R_1\) 中的一条，记其端点为 \(Q_2, R_2\)，使之满足 \((|OQ_2| - 2)(|OR_2| - 2) < 0\). 依次下去，得到 \(P_1, P_2, \cdots, P_n, \cdots\)，则 \(\lim_{n \to \infty} \left|Q_0P_n\right| = \fillinblank{} \).
\end{problem}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=4.8cm]{img/2011/shanghai_science/q21_fig1.png}
\end{problem}
...
对于下列三组点只需选做一种，满分分别是 \circled{1} 2 分、\circled{2} 6 分、\circled{3} 8 分；若选择了多于一种情形，则按照序号较小的解答计分.
