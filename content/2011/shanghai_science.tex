...
短答案：
\begin{enumerate}
\item 当 \(a>0,b>0\) 时，\(f(x)\) 在 \(\R\) 上单调递增；当 \(a<0,b<0\) 时，\(f(x)\) 在 \(\R\) 上单调递减.
\item 当 \(a<0<b\) 时，\(x>\log_{\frac{3}{2}}\left(-\frac{a}{2b}\right)\)；当 \(b<0<a\) 时，\(x<\log_{\frac{3}{2}}\left(-\frac{a}{2b}\right)\).
\end{enumerate}
\end{solution}
...
短答案：
\begin{enumerate}
\item \(c_1=9,c_2=11,c_3=12,c_4=13\).
\item 在数列 \(\{a_n\}\) 中但不在数列 \(\{b_n\}\) 中的项恰为 \(a_2,a_4,\cdots,a_{2n},\cdots\).
\item \(\displaystyle c_n=\begin{cases}
6k+3,&n=4k-3,\\
6k+5,&n=4k-2,\\
6k+6,&n=4k-1,\\
6k+7,&n=4k.
...
\item
\begin{enumerate}
\item \(\Omega=\{(x,y)\mid x=0\}\).
\item \(\Omega=\{(x,y)\mid x=0,\ y\geqslant0\}\cup\{(x,y)\mid y^2=4x,\ -2\leqslant y<0\}\cup\{(x,y)\mid x+y+1=0,\ x>1\}\).
\item \(\Omega=\{(x,y)\mid x\leqslant0,\ y\leqslant0\}\cup\{(x,y)\mid y=x,\ 0<x\leqslant1\}\cup\{(x,y)\mid x^2=2y-1,\ 1<x\leqslant2\}\cup\{(x,y)\mid 4x-2y-3=0,\ x>2\}\).
\end{enumerate}
\end{enumerate}
\end{solution}
