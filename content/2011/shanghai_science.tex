...
\begin{solution}
集合 \(A\) 包含所有满足 \(x\leqslant0\) 或 \(x\geqslant1\) 的实数，因此在全集 \(\R\) 中不属于 \(A\) 的数恰好满足 \(0<x<1\). 所以 \(\complement_U A=(0,1)\).
\end{solution}
...
\begin{problem}
设 \(A_1\)，\(A_2\)，\(A_3\)，\(A_4\)，\(A_5\) 是平面上给定的 5 个不同点，则使 \(\overrightarrow{MA_1} + \overrightarrow{MA_2} + \overrightarrow{MA_3} + \overrightarrow{MA_4} + \overrightarrow{MA_5} = \bs{0}\) 成立的点 \(M\) 的个数为（\quad）
...
\begin{solution}
设各点的位置向量分别为 \(\bs{a}_1\)，\(\bs{a}_2\)，\(\bs{a}_3\)，\(\bs{a}_4\)，\(\bs{a}_5\)，点 \(M\) 的位置向量为 \(\bs m\). 由 \(\overrightarrow{MA_i}=\bs{a}_i-\bs m\)，原式等价于
\[
\sum_{i=1}^{5}(\bs{a}_i-\bs m)=\bs0
\]
即 \(5\bs m=\bs{a}_1+\bs{a}_2+\bs{a}_3+\bs{a}_4+\bs{a}_5\). 因而 \(\bs m\) 唯一确定，恰有一个点 \(M\)，所以选 B.
\end{solution}
...
\]
所以 \(z_1=2-\i\). 设 \(z_2=u+2\i\)，其中 \(u\in\R\)，则
\[
...
\begin{solution}
\begin{enumerate}
\item 当 \(a>0\)，\(b>0\) 时，\(2^x\)，\(3^x\) 都在 \(\R\) 上递增，正系数倍仍递增，故 \(f(x)\) 在 \(\R\) 上单调递增. 当 \(a<0\)，\(b<0\) 时，两个正指数函数分别乘以负数后都递减，故 \(f(x)\) 在 \(\R\) 上单调递减.
\item
\[
...
\bitmapfigure[max width=0.34\linewidth]{img/2011/shanghai_science/q21_fig1.png}
\end{problem}
...
\begin{enumerate}
\item 线段 \(AB_1\) 在底面上的射影为 \(A_1B_1\)，故 \(\tan\alpha=\dfrac{AA_1}{A_1B_1}=h\). 因为 \(O_1\) 是正方形对角线的交点，\(A O_1\perp B_1D_1\)，且 \(A_1O_1\perp B_1D_1\)，所以二面角的平面角为 \(\angle AO_1A_1\). 又 \(A_1O_1=\dfrac{\sqrt2}{2}\)，故
\[
\tan\beta=\frac{AA_1}{A_1O_1}=\sqrt2h=\sqrt2\tan\alpha
\]
\item 由三点 \(A\)，\(B_1\)，\(D_1\) 可得平面 \(AB_1D_1\) 的方程为 \(hx+hy+z-h=0\). 因而点 \(C\) 到该平面的距离为
\[
...
\begin{solution}
\begin{enumerate}
\item \(a_1=9\)，\(a_2=12\)，\(b_1=9\)，\(b_2=11\)，\(b_3=13\). 合并集合时重复的 \(9\) 只保留一次，所以 \(c_1=9\)，\(c_2=11\)，\(c_3=12\)，\(c_4=13\).
\item 若 \(a_n\) 也是 \(\{b_n\}\) 中的项，则存在 \(m\in\N^*\) 使
\[
3n+6=2m+7
\]
即 \(m=\dfrac{3n-1}{2}\). 当 \(n=2k+1\) 为奇数时，\(m=3k+1\in\N^*\)，所以 \(a_{2k+1}=b_{3k+1}\). 当 \(n=2k\) 为偶数时，\(\dfrac{3n-1}{2}=3k-\dfrac12\) 不是整数，故 \(a_{2k}\) 不在 \(\{b_n\}\) 中. 因而所求各项恰为 \(a_2\)，\(a_4\)，\(\ldots\)，\(a_{2k}\)，\(\ldots\).
\item 由上一步，奇数项 \(a_{2k-1}=6k+3\) 与 \(b_{3k-2}=6k+3\) 重合，偶数项 \(a_{2k}=6k+6\) 不重合. 另外
\(b_{3k-1}=6k+5\)，\(b_{3k}=6k+7\).
所以每四个相邻的不同元素依次为 \(6k+3<6k+5<6k+6<6k+7\)，从而
\[
c_n=\begin{cases}
6k+3,&n=4k-3,\\
6k+5,&n=4k-2,\\
...
\begin{solution}
\begin{enumerate}
\item 线段上的点可写为 \(Q=(x,x-3)\)，其中 \(3\leqslant x\leqslant5\). 因而
\[
|PQ|^2=(x-1)^2+(x-4)^2
\]
其导数为 \(4x-10>0\)（当 \(x\geqslant3\)），所以最小值在 \(x=3\) 处取得. 此时 \(Q=(3,0)\)，故 \(d(P,l)=\sqrt{(3-1)^2+(0-1)^2}=\sqrt5\).
\item 将线段平移不影响面积. 以线段为中心线，距离不超过 \(1\) 的点集由一个长为 \(2\)、宽为 \(2\) 的矩形和两个半径为 \(1\) 的半圆组成. 因而图形面积为 \(2\times2+\pi\times1^2=4+\pi\).
\item 对每一组点按纵坐标区间分段比较到两条线段的距离.
