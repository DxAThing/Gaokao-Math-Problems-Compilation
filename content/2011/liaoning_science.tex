...
\begin{problem}
\begin{optionlist}
\item[\textbf{A.}] 如图，A, B, C, D 四点在同一圆上，AD 的延长线与 BC 的延长线交于 E 点，且 EC = ED.
\begin{enumerate}[label={（\arabic*）},itemsep=0.2em]
\item 证明: \(CD // AB\)；
...
\item[\textbf{B.}] 在平面直角坐标系 \(xOy\) 中，曲线 \(C_1\) 的参数方程为 \(\left\{ \begin{array}{l} x = \cos \varphi, \\ y = \sin \varphi. \end{array} \right.\) （\(\varphi\) 为参数）. 曲线 \(C_2\) 的参数方程为 \(\left\{ \begin{array}{l} x = a\cos \varphi, \\ y = b\sin \varphi, \end{array} \right.\) （\(a > b > 0, \varphi\) 为参数）. 在以 \(O\) 为极点，\(x\) 轴的正半轴为极轴的极坐标系中，射线 \(l: \theta = \alpha\) 与 \(C_1, C_2\) 各有一个交点. 当 \(\alpha = 0\) 时，这两个交点间的距离为 2，当 \(\alpha = \frac{\pi}{2}\) 时，这两个交点重合.
\begin{enumerate}[label={（\arabic*）},itemsep=0.2em]
