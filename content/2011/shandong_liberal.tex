...
\begin{solution}
函数在 \(x=\dfrac{\pi}{3}\) 处由递增转为递减，因此 \(f'(x)=\omega\cos(\omega x)\) 在该点为零，且在此前后分别保持非负、非正. 于是 \(\cos\dfrac{\omega\pi}{3}=0\)，从而 \(\dfrac{\omega\pi}{3}=\dfrac{(2k+1)\pi}{2}\)，其中 \(k\) 为非负整数. 若 \(k\geqslant1\)，则在区间 \(\left(0,\dfrac{\pi}{3}\right)\) 内已经出现余弦函数的零点，函数不可能在整个该区间上单调递增，所以 \(k=0\). 因而 \(\omega=\dfrac32\)，选 B.
\end{solution}
...
\begin{solution}
函数 \(f(x)=\dfrac{x}{2}-2\sin x\) 是奇函数，且 \(f(0)=0\). 又 \(f'(x)=\dfrac12-2\cos x\)，\(f'(0)=-\dfrac32<0\)，
所以图象经过原点，并在原点附近随 \(x\) 增大而下降. 在 \(x\in[0,2\pi]\) 上，令 \(f'(x)=0\)，得 \(\cos x=\dfrac14\)，故图象先下降、后上升、再下降；同时 \(f(\pi)=\dfrac\pi2>0\)，\(f(2\pi)=\pi>0\). 这些性质与选项 C 的图象一致，故选 C.
\end{solution}
...
\begin{solution}
设图中矩形的长、宽分别为 \(L\)、\(W\). 对于三棱柱，取棱柱长度为 \(L\)，并取其三角形底面的两个正投影在两个相互垂直方向上的长度都为 \(W\)，则正视图和俯视图均为 \(L\times W\) 的矩形. 对于四棱柱，取长、宽、高分别为 \(L\)、\(W\)、\(W\) 的长方体，其正视图和俯视图也均为 \(L\times W\) 的矩形. 对于圆柱，取轴长为 \(L\)、底面直径为 \(W\)，并使轴平行于矩形的长边，则正视图和俯视图均为 \(L\times W\) 的矩形. 因此三个命题都是真命题，真命题的个数为 \(3\)，选 A.
\end{solution}
