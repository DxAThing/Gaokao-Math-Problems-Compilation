...
\begin{solution}
因为 \(f(x)\) 是偶函数，\(g(x)\) 是奇函数，所以 \(f(-x)=f(x)\)，\(g(-x)=-g(x)\)，进而 \(\lvert g(-x)\rvert=\lvert g(x)\rvert\).于是 \(f(x)+\lvert g(x)\rvert\) 在 \(-x\) 处的值仍为 \(f(x)+\lvert g(x)\rvert\)，它恒为偶函数.取 \(f(x)=1\)，\(g(x)=0\)，则 \(f(x)-\lvert g(x)\rvert=1\) 不是奇函数，\(\lvert f(x)\rvert-g(x)=1\) 也不是奇函数；取 \(f(x)=0\)，\(g(x)=x\)，则 \(\lvert f(x)\rvert+g(x)=x\) 不是偶函数.因此只有 A 恒成立，故选 A.
\end{solution}
...
\begin{solution}
整数 \(1\) 必属于 \(T\) 或 \(V\) 中的一个.若 \(1\in T\)，则任取 \(a\)，\(b\in T\)，由题设关于三个元素的乘积的条件，取第三个元素为 \(1\)，得 \(ab=ab\cdot1\in T\)，所以 \(T\) 关于乘法封闭.若 \(1\in V\)，则任取 \(x\)，\(y\in V\)，取三个元素为 \(x\)，\(y\)，\(1\)，得 \(xy=xy\cdot1\in V\)，所以 \(V\) 关于乘法封闭.因此 \(T\)，\(V\) 中至少有一个关于乘法是封闭的，故选 A.
\end{solution}
...
\begin{solution}
建立空间直角坐标系，令 \(A=(0,0,0)\)，\(B=(1,0,0)\)，\(D=\left(\frac12,\frac{\sqrt3}{2},0\right)\)，则 \(C=\left(\frac32,\frac{\sqrt3}{2},0\right)\).设 \(P=(u,v,w)\).由 \(PA=PD\) 得 \(u+\sqrt3v=1\).又由 \(PB^2-PA^2=4-2=2\)，即 \((u-1)^2-u^2=2\)，得 \(u=-\frac12\)，所以 \(v=\frac{\sqrt3}{2}\).再由 \(PA^2=2\) 得 \(w^2=1\).图形关于底面反射不影响结论，取 \(w=1\)，即 \(P=\left(-\frac12,\frac{\sqrt3}{2},1\right)\).于是 \(E=\left(\frac54,\frac{\sqrt3}{4},0\right)\)，\(F=\left(\frac12,\frac{\sqrt3}{2},\frac12\right)\).
...
当 \(p_1>0\) 时，\(M\) 在线段 \(EF\) 内部等价于 \(0<a<p_1\)，即 \(-p_1<p_2<p_1\)，等价于 \(|p_2|<|p_1|\). 当 \(p_1<0\) 时，\(M\) 在线段 \(EF\) 内部等价于 \(p_1<a<0\)，而 \(a=\frac{p_1+p_2}{2}\)，所以 \(p_1<\frac{p_1+p_2}{2}<0\)，即 \(p_1<p_2<-p_1\). 因为 \(p_1<0\)，这又等价于 \(|p_2|<|p_1|\). 若 \(p_1=0\)，则由 \(a\neq0\) 有 \(p_2=2a\neq0\)，三者均不成立. 因此 \(M(a,b)\in X\Longleftrightarrow |p_1|>|p_2|\).

又由 \(p_1+p_2=2a\) 以及 \(p_1p_2=4b\)，方程 \(x^2-ax+b=0\) 的两个根正是 \(\frac{p_1}{2}\) 和 \(\frac{p_2}{2}\).由于 \(a^2-4b>0\)，有 \(p_1\ne p_2\)；又 \(a\ne0\)，故 \(p_1+p_2\ne0\)，从而 \(|p_1|\ne|p_2|\).所以 \(\varphi(a,b)=\frac12\max\{|p_1|,|p_2|\}\)，因此 \(|p_1|>|p_2|\Longleftrightarrow\varphi(a,b)=\frac{|p_1|}{2}\).
