...
\bitmapfigure[max width=0.58\linewidth,max totalheight=4.8cm]{img/2011/beijing_liberal/q5_fig1.png}
...
\bitmapfigure[max width=0.38\linewidth,max totalheight=5.8cm]{img/2011/beijing_liberal/q6_fig1.png}
...
\bitmapfigure[max width=0.56\linewidth,max totalheight=4.8cm]{img/2011/beijing_liberal/q16_fig1.png}
...
\bitmapfigure[max width=0.48\linewidth,max totalheight=5.2cm]{img/2011/beijing_liberal/q17_fig1.png}
...
\begin{problem}
已知椭圆 \(G: \frac{x^2}{a^{2}} + \frac{y^2}{b^{2}} = 1 (a > b > 0)\) 的离心率为 \(\frac{\sqrt{6}}{3}\)，右焦点为 \((2\sqrt{2}, 0)\)，斜率为 1 的直线 \(l\) 与椭圆 \(G\) 交于 \(A, B\) 两点. 以 \(AB\) 为底边作等腰三角形，顶点为 \(P(-3, 2)\). (1) 求椭圆 \(G\) 的方程; (2) 求 \( \triangle PAB \) 的面积.
\end{problem}

\begin{problem}
若数列 \(A_{n}: a_{1}, a_{2}, \cdots, a_{n} (n \geqslant 2)\) 满足 \(\left|a_{k+1} - a_{k}\right| = 1 (k = 1, 2, \cdots, n-1)\)，则称 \(A_{n}\) 为 \(E\) 数列. 记 \(S(A_{n}) = a_{1} + a_{2} + \cdots + a_{n}\). (1) 写出一个 \(E\) 数列 \(A_{n}\) 满足 \(a_{1}=a_{3}=0\)；(2) 若 \(a_1 = 12, n = 2000\)，证明：\(E\) 数列 \(A_n\) 是递增数列的充要条件是 \(a_n = 2011\)；(3) 在 \(a_1 = 4\) 的 \(E\) 数列 \(A_n\) 中，求使得 \(S(A_n) = 0\) 成立的 \(n\) 的最小值.
\end{problem}
