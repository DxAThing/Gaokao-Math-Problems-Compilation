...
\begin{problem}
已知实数 \(a\neq 0\)，函数 \(f(x)=\begin{cases} 2x+a,  x<1, -x-2a,  x\geqslant 1. \end{cases}\)
...
\begin{problem}
设集合 \(A=\left\{(x,y)\mid \frac{m}{2}\leqslant (x-2)^2+y^2\leqslant m^2,\ x,y\in\R\right\},\quad B=\left\{(x,y)\mid 2m\leqslant x+y\leqslant 2m+1,\ x,y\in\R\right\}.\)
...
\item 在平面直角坐标系 \(xOy\) 中，求过椭圆 \(\left\{\begin{array}{l} x=5\cos\varphi, y=3\sin\varphi, \end{array}\right.\)
（\(\varphi\) 为参数）的右焦点，且与直线 \(\left\{\begin{array}{l} x=4-2t, y=3-t, \end{array}\right.\)
（\(t\) 为参数）平行的直线的普通方程.
\item 解不等式：\(x+|2x-1|<3\).
\end{enumerate}
\end{problem}
