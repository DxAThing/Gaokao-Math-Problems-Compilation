...
(x-2)^2+y^2=(x+2)^2
\]
即 \(x^2-4x+4+y^2=x^2+4x+4\)，所以 \(y^2=8x\)，故选 C.
\end{solution}
...
\begin{solution}
由 \(f(-x)=f(x)\)，函数图象关于 \(y\) 轴对称；由 \(f(x+2)=f(x)\)，函数图象沿 \(x\) 轴方向平移 \(2\) 个单位后与原图象重合，即 \(2\) 是它的一个周期. 选项 B 的图象关于 \(y\) 轴对称，且相邻波峰（或相邻波谷）的横坐标相差 \(2\)，例如可由 \(y=-\cos \pi x\) 表示. 选项 A、C 的图象均不关于 \(y\) 轴对称，选项 D 虽关于 \(y\) 轴对称但相邻波峰（或相邻波谷）的横坐标相差 \(4\)，不满足以 \(2\) 为周期的条件，因此选 B.
\end{solution}
...
\(\mathrm C_6^k(t^2)^{6-k}(-t^{-1})^k=(-1)^k\mathrm C_6^k t^{12-3k}\)，\(k=0\)，\(1\)，\(\ldots\)，\(6\)
...
\begin{solution}
主视图和左视图都是边长为 \(2\) 的正方形，图中的两条斜线在正方形底边中点汇聚；俯视图是在边长为 \(2\) 的正方形内的半径为 \(1\) 的圆. 因而该几何体可看作棱长为 \(2\) 的正方体内部去掉一个底面半径为 \(1\)、高为 \(2\) 的圆锥. 正方体体积为 \(2^3=8\)，圆锥体积为
\[
...
\begin{solution}
函数在 \([0,+\infty)\) 上连续，且 \(f(0)=-1<0\). 因为 \(0<1<\pi\)，所以 \(\cos 1<1\)，从而 \(f(1)=1-\cos 1>0\).
...
\begin{solution}
由于每个人独立、等可能地从 \(6\) 个景点中选择游览安排，交换景点编号不会改变规则，所以每个人最后 \(1\) 小时所在的景点在 \(1\)，\(2\)，\(\ldots\)，\(6\) 中等可能，即每个景点的概率都是 \(\frac{1}{6}\). 记 \(A_j\) 为甲最后 \(1\) 小时在景点 \(j\)，\(B_j\) 为乙最后 \(1\) 小时在景点 \(j\)，其中 \(j=1\)，\(2\)，\(\ldots\)，\(6\). 甲、乙的选择相互独立，因此
\[
\displaystyle P(\text{两人最后一小时同在一个景点})=\sum_{j=1}^{6}P(A_j)P(B_j)=6\left(\frac{1}{6}\right)^2=\frac{1}{6}
\]
故选 D.
\end{solution}
...
\begin{solution}
由图，\(E\) 是 \(AD\) 与 \(BC\) 的交点，所以 \(BE\subset BC\)，且 \(\angle ABE=\angle ABC=\angle B\). 在直角三角形 \(ACD\) 中，\(\angle ACD=90^\circ\)，且 \(AD=12\)，\(AC=4\)，所以
\[
...
\]
又因为 \(\angle B=\angle D\)，所以 \(\angle ABE=\angle ADC\). 由于 \(AE\perp BC\)，三角形 \(ABE\) 在 \(E\) 处为直角，\(AB=6\) 为斜边，于是
\[
...
\begin{solution}
\begin{enumerate}
\item 折叠只改变三角形 \(ABD\) 所在平面的方向，折痕 \(AD\) 上的点不动，因此 \(AD\perp DC\) 仍成立. 又因折起后 \(\angle BDC=90^{\circ}\)，有 \(BD\perp DC\). 直线 \(AD\) 与 \(BD\) 在平面 \(ADB\) 内相交，且 \(DC\) 同时垂直于这两条直线，所以 \(DC\perp\) 平面 \(ADB\). 由于 \(DC\subset\) 平面 \(BDC\)，根据二面角的判定，平面 \(ADB\perp\) 平面 \(BDC\).
\item 折叠前 \(AD\perp BD\)，折叠后这一垂直关系保持不变. 在直角三角形 \(ABD\) 中，\(\angle ABD=\angle ABC=60^{\circ}\)，所以 \(\angle BAD=30^{\circ}\)，从而 \(AD=\sqrt{3}BD\). 在直角三角形 \(ABC\) 中，\(AD\) 是斜边上的高，故
\(AD^{2}=BD\cdot DC\)，于是 \(3BD^{2}=BD\cdot DC\)，即 \(DC=3BD\). 不妨设 \(BD=1\)，建立直角坐标系，使 \(D\) 为原点，\(DC\) 在 \(x\) 轴正半轴上，\(DB\) 在 \(y\) 轴正半轴上，\(DA\) 在 \(z\) 轴正半轴上，则
...
\]
因此，余弦定理的叙述是：在 \(\triangle ABC\) 中，三边 \(a\)，\(b\)，\(c\) 分别为角 \(A\)，\(B\)，\(C\) 的对边，则
\(a^{2}=b^{2}+c^{2}-2bc\cos A\)，\(b^{2}=c^{2}+a^{2}-2ca\cos B\)，\(c^{2}=a^{2}+b^{2}-2ab\cos C\). 上述三个等式已经分别证明，故余弦定理得证.
\end{solution}
...
\]
由于除 \(x=1\) 外均有 \(h'(x)<0\)，所以 \(h(x)\) 在 \((0,+\infty)\) 上严格递减，且 \(h(1)=0\). 所以当 \(0<x<1\) 时 \(h(x)>0\)，当 \(x=1\) 时 \(h(x)=0\)，当 \(x>1\) 时 \(h(x)<0\). 即当 \(0<x<1\) 时 \(g(x)>g\left(\frac{1}{x}\right)\)，当 \(x=1\) 时 \(g(x)=g\left(\frac{1}{x}\right)\)，当 \(x>1\) 时 \(g(x)<g\left(\frac{1}{x}\right)\).
