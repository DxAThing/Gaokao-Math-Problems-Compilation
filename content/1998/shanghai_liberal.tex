...
\]
因此函数的最小值为 \(4-\sqrt{4+a}\). 由题意，
\(4-\sqrt{4+a}=1\)，所以 \(\sqrt{4+a}=3\)，解得 \(a=5\).
\end{solution}
...
\begin{solution}
设 \(y=f(x)=(x-1)^{1/3}+2\). 则
\(y-2=(x-1)^{1/3}\)，两边立方得 \(x=(y-2)^3+1\).
交换 \(x\)、\(y\)，得到
\[
f^{-1}(x)=(x-2)^3+1
\]
由于立方根函数的定义域和值域都是 \(\R\)，所以 \(f^{-1}\) 的定义域为 \(\R\).
\end{solution}
...
\begin{solution}
取正四面体的一个面为底面，底面是边长为 \(2\) 的正三角形，面积为
\(S=\frac{\sqrt3}{4}\cdot2^2=\sqrt3\). 设顶点到底面的高为 \(h\)，底面中心到任一顶点的距离为正三角形的外接圆半径 \(\frac{2}{\sqrt3}\). 由勾股定理，
\[
...
\begin{solution}
设各事件最早完成时间分别为 \(T_1\)，\(T_2\)，\(\ldots\)，\(T_8\)，从事件 1 开始计时，则 \(T_1=0\). 按流程图逐个计算：
\(T_2=T_1+2=2\)，\(T_3=T_1+0=0\)，\(T_4=T_3+3=3\).

\(T_5=\max\{T_2+3,T_4+0\}=5\)，\(T_6=\max\{T_4+1,T_5+0\}=5\).

\(T_7=\max\{T_5+3,T_6+4\}=9\)，\(T_8=T_7+2=11\).
因此工程总时数为 \(11\) 天.
\end{solution}
...
\]
其渐近线斜率为 \(\pm B/A\)，由题意 \(B/A=4/3\)，又由焦半距公式 \(A^2+B^2=c^2=25\). 因此
\(B^2=\frac{16}{9}A^2\)，\(A^2+\frac{16}{9}A^2=25\)
解得 \(A^2=9\)、\(B^2=16\). 所求双曲线方程为
\[
...
\begin{solution}
函数 \(y=\sin4x\) 的周期为 \(\frac\pi2\)，但它是奇函数；函数
\(y=\cos^22x-\sin^22x=\cos4x\)，是偶函数且周期为 \(\frac\pi2\). 函数 \(y=\tan2x\) 虽周期为 \(\frac\pi2\)，但不是偶函数；函数 \(y=\cos2x\) 是偶函数，但其最小正周期为 \(\pi\). 故选 B.
\end{solution}
...
\begin{solution}
由 \(x^2-5x-6=(x+1)(x-6)\)，得
\(A=(-\infty,-1)\cup(6,+\infty)\)，\(\overline A=[-1,6]\).
又因为 \(11\in B\)，所以 \(|11-5|<a\)，即 \(a>6\). 从而
\[
B=(5-a,5+a),\qquad \overline B=(-\infty,5-a]\cup[5+a,+\infty)
\]
其中上横线表示相对于全集 \(\R\) 的补集. 且 \(5-a<-1\)、\(5+a>6\)，所以 \([-1,6]\subset B\). 因此
\(A\cup B=\R\)，选项 D 正确.
...
\begin{solution}
取两条直线的方向向量分别为
\(\bs u=(a,-\sin A)\)，\(\bs v=(-\sin B,-b)\).
由正弦定理，存在同一个正数 \(2R\)，使得
\(a=2R\sin A\)、\(b=2R\sin B\). 因而
\[
...
\begin{solution}
\begin{enumerate}
\item 因为 \(DD_1\perp\) 底面 \(ABCD\)，且 \(O_1O\parallel D_1D\)，所以 \(O_1O\perp\) 底面 \(ABCD\). 过 \(O\) 作 \(OF\perp BC\)，交 \(BC\) 于 \(F\)，连接 \(O_1F\). 由三垂线定理，\(O_1F\perp BC\)，故 \(\angle O_1FO\) 是二面角 \(O_1-BC-D\) 的平面角.
...
\item 由条件，\(O(0,0,0)\)，\(B_1(0,2,6)\)，\(D_1(0,-2,6)\)，\(E(\sqrt{3},0,3)\).
...
\begin{solution}
\begin{enumerate}
\item 设动点 \(M\) 的坐标为 \((x,y)\). 由于 \(M\) 到 \(y\) 轴的距离为 \(\lvert x\rvert\)，由题意
\[
...
若直线 \(l\) 垂直于 \(x\) 轴，则其方程为 \(x=2\)，与抛物线交于 \(P(2,2)\)、\(Q(2,-2)\). 此时 \(\lvert PQ\rvert=4\)，点 \(E(1,0)\) 到直线 \(l\) 的距离为 \(1\)，所以 \(S_{\triangle EPQ}=\frac12\cdot4\cdot1=2\ne4\)，故竖直直线不是所求直线.
...
\begin{solution}
\begin{enumerate}
\item 由 \(t=0\) 表示 \(12{:}00\)，可知 \(8{:}00\)、\(13{:}00\)、\(16{:}00\) 分别对应 \(t=-4\)、\(t=1\)、\(t=4\). 因此
\[
\begin{cases}
d=60,\\
-64a+16b-4c+d=8,\\
...
\end{cases}
\]
第一式给出 \(d=60\)，第四式给出 \(b=0\). 代入第二、三式，得到
\(16a+c=13\)，\(a+c=-2\).
两式相减得 \(15a=15\)，所以 \(a=1\)，进而 \(c=-3\). 因此温度函数为
\[
T(t)=t^3-3t+60
\]
...
因此，函数 \(T(t)\) 在 \([-2,-1]\) 上先增，在 \([-1,1]\) 上减，在 \([1,2]\) 上增. 计算各个端点和驻点的函数值：
