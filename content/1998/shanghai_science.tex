...
\item 由条件得 \(O\)，\(B_{1}\)，\(D_{1}\)，\(E\) 的坐标为 \(O(0,0,0)\)，\(B_{1}(0,2,6)\)，\(D_{1}(0,-2,6)\)，\(E(\sqrt{3},0,3)\).
...
\item
【法一】设直线 \(l\) 的方程为 \(y=k(x-2)\)，因 \(l\) 与抛物线有两个交点，故 \(k\neq0\).

\bitmapinlinefigure[0.42\linewidth][max width=0.26\linewidth]{img/1998/shanghai_science/q20_solution_fig1.png}
...
【法二】设直线 \(l\) 的方程为 \(y=k(x-2)\)，代入 \(y^{2}=4(x-1)\)，得
\[
...
\begin{solution}
\begin{enumerate}
\item 因为 \(T(0)=60\)，\(T(-4)=8\)，\(T(1)=58\)，\(T'(-4)=T'(4)\)，所以 \(d=60\)，\(b=0\)，\(a=1\)，\(c=-3\).
...
而 \(\{4b_n\}\) 是一个以 \(-12\) 为公差的等差数列，所以 \(d=-12m\)（\(m\in\N\)），故 \(d=-24\). 因此 \(c_n=7-24n\)（\(n\in\N\)）.
\end{enumerate}
\end{solution}
