...
\begin{solution}
%
由 \(\frac{1}{2}\log_{\frac{1}{3}}(x+1)\geqslant \log_{\frac{1}{3}}(1-x)\) 得
\[
\begin{cases}
x+1>0,\\
1-x>0,\\
...
\]
由上述三个条件，得 \(x>-1\)，\(x<1\)，且
\(x+1\leqslant(1-x)^2\)，即
\(x\leqslant 0\) 或 \(x\geqslant 3\).
...
\begin{solution}
%
由 \(y=\frac{1-\sin\theta\cos\theta}{1+\sin\theta\cos\theta}=\frac{2-\sin 2\theta}{2+\sin 2\theta}\). 因此当 \(\theta=\frac{3}{4}\pi\) 时，\(\sin 2\theta=-1\)，\(y\) 取得最大值；当 \(\theta=\frac{1}{4}\pi\) 时，\(\sin 2\theta=1\)，\(y\) 取得最小值.
\end{solution}
...
\end{aligned}
\]
即定理在 \(n=k+1\) 时也成立. 根据上述证明，对于任何正整数 \(n\)，定理都成立.
\end{solution}
