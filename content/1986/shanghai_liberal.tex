...
函数 \(y = 2\tan (3x + \frac{1}{2}\pi) \) 的最小正周期是（\quad）
...
\choices
    ...
    {如果平面 \(p\) 与两平面 \(\alpha,\beta\) 所成的二面角都是直二面角，那么 \(\alpha\parallel\beta\)}
...
函数 \(y = \lg (x + \frac{1}{2}\pi) \) 的定义域是（\quad）

\choices
    {\(\{x \mid x \in \R \text{ 且 } x \neq k\pi + \frac{1}{2}\pi,\ k \in \Z\}\)}
    {\((x \mid x \in \R \text{ 且 } x \neq k\pi - \frac{1}{2}\pi,\ k \in \Z)\)}
    {\(\{x \mid x \in \R \text{ 且 } x \neq 2k\pi + \frac{1}{2}\pi,\ k \in \Z\}\)}
    {\(\{x \mid x \in \R \text{ 且 } x \neq 2k\pi - \frac{1}{2}\pi,\ k \in \Z\}\)}
...
等式 \(\log_3 x^2 = 2\) 成立是等式 \(\log_2 x = 1\) 成立的（\quad）
...
在等比数列中，已知首项为 \(\frac{9}{8}\)，末项为 \(1\)，公比为 \(1\)，则项数是（\quad）
...
若 \(\sin \frac{1}{2}\theta = \frac{3}{5}\)，\(\cos \frac{1}{2}\theta = -\frac{1}{5}\)，则 \(\theta\) 角的终边在（\quad）
...
\choices
    {\(\sin\varphi_{1}>\sin\varphi_{2} \)}
    {\(\operatorname{tg}(15\pi/8) > \operatorname{tg}(-\pi) \)}
    {\(\sin(-\frac{1}{2}\pi) > \sin(-\frac{1}{2}\pi) \)}
    {\(\cos (-1 - \theta \pi) > \cos (-3\pi /4)\)}
...
\(\left( \sqrt{2}x-\frac{1}{\sqrt{x}} \right)^8\) 的展开式中，\(x\) 的一次项的系数是 \fillinblank{}.
...
已知 \(\cos \theta = -\frac{3}{5}\) 且 \(\pi < \theta < 3\pi / 2\)，则 \(\operatorname{ctg}(\theta - \frac{1}{2}\pi) =\)\fillinblank{}.
...
设 \(f(x)=\frac{1}{x}\log_{\frac{1}{2}}(x+1)\)，\(g(x)=\log_{\frac{1}{2}}(1-x)\)，试解不等式 \(f(x)\geqslant g(x)\).
