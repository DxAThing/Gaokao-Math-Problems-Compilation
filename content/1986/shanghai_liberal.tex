...
函数 \(y = 2\tan\left( 3x + \frac{1}{6}\pi \right) \) 的最小正周期是（\quad）
...
\begin{solution}
\(\tan u\) 在 \(u=\frac{\pi}{2}+k\pi\) 时无定义，其中 \(k\in\Z\). 因而 \(x+\frac{\pi}{3}=\frac{\pi}{2}+k\pi\)，即 \(x=k\pi+\frac{\pi}{6}\). 所以定义域为 \(\{x\mid x\in\R\text{ 且 }x\neq k\pi+\frac{\pi}{6},\ k\in\Z\}\)，故选 A.
\end{solution}

\begin{problem}

若空间有 \(10\) 个点，则可以确定的平面总数最多有（\quad）
...
\begin{solution}
函数 \(y=x+3\) 的定义域为 \(\R\)，但 \(f(-x)=-x+3\neq-(x+3)\)，不是奇函数. 函数 \(y=-2x^2\) 和 \(y=-|x|\) 都满足 \(f(-x)=f(x)\)，是偶函数而不是奇函数. 对 \(y=\lg 3^x\)，有 \(3^x>0\)，定义域为 \(\R\)，且 \(\lg 3^x=x\lg3\)，所以 \(f(-x)=-x\lg3=-f(x)\)，它是奇函数. 故选 C.
\end{solution}
...
已知 \(\cos \theta = -\frac{3}{5}\) 且 \(\pi < \theta < 3\pi / 2\)，则 \(\cot\left( \theta - \frac{1}{4}\pi \right) =\)\fillinblank{}.
\end{problem}
...
证明：当 \(n=1\) 时，等式就是 \(z=r(\cos\theta+\i\sin\theta)\)，成立. 假设 \(n=k\) 时成立，即
\(z^k=r^k(\cos k\theta+\i\sin k\theta)\). 则
\[
\begin{aligned}
z^{k+1}
&=z^{k}\cdot z
...
\bitmapfigure[max width=0.42\linewidth]{img/1986/shanghai_liberal/q6_fig1.png}
\end{problem}

\begin{solution}
%
连 \(AC\)，与 \(BD\) 交于 \(O\)，则 \(O\) 为 \(AC\)，\(BD\) 的中点. 连 \(A_{1}O\)，\(EO\)，\(A_{1}E\) 和 \(A_{1}C_{1}\).
因为 \(A_{1}B=A_{1}D\)，\(EB=ED\)，所以 \(A_{1}O\perp BD\)，且 \(EO\perp BD\).

故 \(\angle A_{1}OE\) 为两截面所成二面角的平面角. 设 \(\angle A_{1}OE=\alpha\)，正方体的棱长为 \(a\).
因为 \(A_{1}O=\sqrt{AA_{1}^{2}+AO^{2}}=\frac{\sqrt{6}a}{2}\)，\(EO=\sqrt{EC^{2}+CO^{2}}=\frac{\sqrt{3}a}{2}\)，\(A_{1}E=\sqrt{A_{1}C_{1}^{2}+C_{1}E^{2}}=\frac{3a}{2}\)，在 \(\triangle A_{1}OE\) 中，
...
\bitmapfigure[max width=0.42\linewidth]{img/1986/shanghai_liberal/q7_fig1.png}
\end{problem}
...
\end{cases}
\]
消去 \(y\)，得 \(x^{2}+(p-4)x+1=0\).
所以 \(x_{1}+x_{2}=4-p\)，\(x_{1}x_{2}=1\).

设 \(M\) 点坐标为 \((x_{M},y_{M})\)，则 \(x_{M}=\frac{x_{1}+x_{2}}{2}=\frac{4-p}{2}\).
因为点 \(A\) 和点 \(B\) 在 \(x\) 轴上方，
\[
\begin{gathered}
...
\end{gathered}
\]
于是 \(y_{M}=\frac{y_{1}+y_{2}}{2}=\frac{\sqrt{6p-p^{2}}}{2}\).
因为点 \(M\) 在直线 \(y=x\) 上，
\(\sqrt{6p-p^{2}}=4-p\).
即 \(6p-p^{2}=\left( 4-p \right)^{2}\)，
所以 \(p^{2}-7p+8=0\).
解得 \(p=\frac{7\pm \sqrt{17}}{2}\).
又因为 \(4-p\geqslant 0\)，故舍去 \(p=\frac{7+\sqrt{17}}{2}\). 因此 \(p=\frac{7-\sqrt{17}}{2}\).
对保留的值 \(p=\frac{7-\sqrt{17}}{2}\)，有 \(0<p<4\)，且 \((4-p)^2-4=p^2-8p+12=4-p>0\)，所以关于 \(x\) 的二次方程有两个正根；由 \(y_i^2=px_i\) 可取两个正的 \(y_i\)，确实得到题设的两个 \(x\) 轴上方交点，故该值满足原题条件.
\end{solution}
