...
\begin{problem}
已知 \(\sin A + \sin 3A + \sin 5A = a\)，\(\cos A + \cos 3A + \cos 5A = b\). 求证：当 \(b \neq 0\) 时，\(\tan 3A = \frac{a}{b}\)；\(\left(1 + 2\cos 2A\right)^2 = a^2 + b^2\).

\end{problem}

\begin{problem}
已知数列 \(\{a_n\}\)，其中 \(a_1 = \frac{4}{3}\)，\(a_2 = \frac{13}{9}\)，且当 \(n \geqslant 3\) 时，\(a_n - a_{n-1} = \frac{1}{3}\left(a_{n-1} - a_{n-2}\right)\). 求数列 \(\{a_n\}\) 的通项公式，并求 \(\displaystyle\lim_{n\to\infty}a_n\).
\end{problem}
