...
\begin{solution}
复数的三角形式规定为 \(r\left(\cos\theta+\i\sin\theta\right)\)，其中 \(r>0\). 按题目给出的现成写法比较，只有 B 的括号内是 \(\cos\theta+\i\sin\theta\)，且外面的模长因子为 \(2>0\). A 虽可再利用 \(-\sin\frac{\pi}{4}=\sin\left(-\frac{\pi}{4}\right)\) 改写，但原式并未按规定形式写出；C 将正弦、余弦的位置交换，D 的外因子为负数且括号内也不是规定形式. 因此选 B.
\end{solution}
...
\begin{solution}
由 \(y=5^x+1\) 得 \(y-1=5^x\). 因为 \(5>0\) 且 \(5\neq1\)，对两边取以 \(5\) 为底的对数，得到 \(x=\log_5(y-1)\)，并且原函数的值域为 \(y>1\). 交换自变量与因变量的字母，反函数为 \(y=\log_5(x-1)\)，其定义域为 \(x>1\). 因此选 C.
\end{solution}
...
\begin{solution}
由补集定义，\(\overline{A}=I\mathbin{\backslash}A=\{1,2,6,7,8\}\)，\(\overline{B}=I\mathbin{\backslash}B=\{2,4,5,7,8\}\). 因而 \(\overline{A}\cap\overline{B}=\{2,7,8\}\)，所以选 D.
\end{solution}
...
\begin{solution}
利用二倍角公式 \(2\sin 2x\cos 2x=\sin 4x\)，有 \(y=\frac{\sqrt{2}}{2}\sin 4x\). 因为 \(\sin 4x\) 的最小正周期为 \(\frac{2\pi}{4}=\frac{\pi}{2}\)，所以该函数的最小正周期为 \(\frac{\pi}{2}\). 又有 \(y(-x)=\frac{\sqrt{2}}{2}\sin(-4x)=-y(x)\)，故它是奇函数. 因此选 A.
\end{solution}
...
\begin{solution}
由 \(c>0\) 且 \(4>1\)，可知 \(4^c>1\). 而 \(\displaystyle\frac{2^c}{\left(\frac{1}{2}\right)^c}=4^c\)，所以 \(2^c>\left(\frac{1}{2}\right)^c\) 对一切 \(c>0\) 成立. 这正是选项 D.
\end{solution}
...
\begin{solution}
以 \(90\) 为基准，20 个数的偏差依次为 \(-3\)，\(1\)，\(4\)，\(-2\)，\(3\)，\(1\)，\(-1\)，\(-3\)，\(2\)，\(-4\)，\(0\)，\(2\)，\(-2\)，\(0\)，\(1\)，\(-4\)，\(-1\)，\(2\)，\(5\)，\(-2\)，其和为 \(-1\). 因此这 20 个数的和为 \(20\times90-1=1799\)，所以选 B.
\end{solution}
...
\begin{solution}
设正方体的棱长为 \(s\). 空间对角线长满足 \(a=s\sqrt{3}\)，所以 \(s=\frac{a}{\sqrt{3}}\). 正方体的全面积为 \(6s^2=6\cdot\frac{a^2}{3}=2a^2\)，因此选 B.
\end{solution}
...
\begin{solution}
配方得 \(\left(x+\frac{D}{2}\right)^2+\left(y+\frac{E}{2}\right)^2=\frac{D^2+E^2-4F}{4}\). 由题设右端为正，所表示的曲线是圆，其圆心为 \(\left(-\frac{D}{2},-\frac{E}{2}\right)\). 圆关于直线 \(y=x\) 对称，当且仅当圆心在该直线上，因此必须有 \(-\frac{D}{2}=-\frac{E}{2}\)，即 \(D=E\). 所以选 A.
\end{solution}
...
\begin{solution}
把“甲是乙的充分条件”写成 \(\text{甲}\Rightarrow\text{乙}\)，把“乙是丙的充要条件”写成 \(\text{乙}\Leftrightarrow\text{丙}\)，把“丙是丁的必要条件”写成 \(\text{丁}\Rightarrow\text{丙}\). 已知条件只能推出 \(\text{甲}\Rightarrow\text{丙}\) 和 \(\text{丁}\Rightarrow\text{丙}\)，没有推出甲与丁之间的任一方向的关系.

例如取甲、乙、丙为真而丁为假，已知三个条件仍成立，说明甲不一定推出丁，故丁不是甲的必要条件；取丁、丙、乙为真而甲为假，已知三个条件仍成立，说明丁不一定推出甲，故丁不是甲的充分条件. 因此丁对甲既不充分也不必要，选 D.
\end{solution}
...
\begin{solution}
抛物线 \(y=ax^2+bx=x(ax+b)\) 必经过原点，另一个与横轴的交点为 \(\left(-\frac{b}{a},0\right)\). 直线 \(y=ax+b\) 的斜率为 \(a\)，纵截距为 \(b\)，横截距同样为 \(-\frac{b}{a}\). 因此两条图象除原点外还必须在同一个非零横坐标处与横轴相交.

图 C 中，抛物线开口向下，故 \(a<0\)；直线斜率为负且纵截距为正，故 \(b>0\). 于是 \(-\frac{b}{a}>0\)，两图象都经过原点并在同一个正横坐标处再次与横轴相交，完全符合上述条件. 图 A 的抛物线不经过原点，图 B 中抛物线开口方向与直线斜率不相容，图 D 中抛物线开口方向与直线斜率、纵截距不相容. 因此只可能是 C.
\end{solution}
...
\bitmapfigure[width=9.1cm]{img/1986/national_liberal/q17_fig1.png}
\end{problem}
