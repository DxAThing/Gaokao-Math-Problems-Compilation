...
\begin{solution}
\begin{enumerate}
\item 当 \(a=2\) 时，有 \(\lg(2x)=2\lg(x-1)\)，可得 \(2x=(x-1)^{2}\)，即 \(x^{2}-4x+1=0\)，解得 \(x=2\pm\sqrt{3}\). 经检验可知，对数方程的解是 \(x=2+\sqrt{3}\).
\item 由原方程可知 \(ax>0\) 且 \(x>1\)，从而必须 \(x>1\) 和 \(a>0\). 对于 \(a>0\)，\(x>1\)，原方程可化为 \(\lg(ax)=\lg(x-1)^2\)，于是可得 \(ax=(x-1)^2\)，即 \(x^2-(2+a)x+1=0\)，解得 \(x=\frac{2+a\pm\sqrt{a^2+4a}}{2}\). 由于 \(a>0\)，故 \(\frac{2+a+\sqrt{a^2+4a}}{2}>1\)，\(\frac{2+a-\sqrt{a^2+4a}}{2}<1\). 经检验可知，当 \(a>0\) 时，原对数方程有解. 对数方程的解为 \(x=\frac{2+a+\sqrt{a^2+4a}}{2}\).
\end{enumerate}
\end{solution}
...
\item 在平面 \(\gamma\) 内过 \(B\) 作直线 \(n \parallel m\)，又作 \(AE \perp n\)，垂足为 \(E\)，则 \(\angle ABE\) 为异面直线 \(AB\) 与 \(m\) 所成的角. 作 \(AD \perp m\)，垂足为 \(D\). 因 \(\beta \perp \gamma\)，\(\beta \cap \gamma = m\)，故 \(AD \perp \gamma\)，由 \(l\) 与 \(\gamma\) 的距离为 \(b\) 得 \(AD = b\). 作 \(BC \perp m\)，垂足为 \(C\)，连 \(AC\). 因 \(\gamma \perp \beta\)，故 \(BC \perp \beta\)，则 \(\angle BAC = 30^{\circ}\)，又因 \(AB = a\)，故 \(BC = \frac{a}{2}\). 连 \(DE\)，\(DE\) 是 \(AE\) 在平面 \(\gamma\) 内的射影，故 \(DE \perp BE\). 于是 \(DE \parallel BC\). 在 \(\mathrm{Rt}\triangle ADE\) 中，\(AE=\sqrt{AD^{2}+DE^{2}}=\sqrt{b^{2}+\frac{a^{2}}{4}}\). 故 \(\sin\angle ABE=\frac{AE}{AB}=\frac{\sqrt{4b^{2}+a^{2}}}{2a}\). 从而可得 \(\angle ABE = \arcsin \frac{\sqrt{4b^{2} + a^{2}}}{2a}\).
\end{enumerate}
\end{solution}
...
\item 当 \(n=1\) 时，左边是 \(\cos\alpha\)，右边是 \(\frac{\sin\frac{\alpha}{2}\cos\alpha}{\sin\frac{\alpha}{2}}=\cos\alpha\)，等式成立. 假设当 \(n=k\) 时等式成立，即 \(a_{1}+a_{2}+\cdots+a_{k}=\frac{\sin\frac{k\alpha}{2}\cos\frac{k+1}{2}\alpha}{\sin\frac{\alpha}{2}}\)，那么
\[ \begin{aligned}
