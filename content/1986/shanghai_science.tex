...
（2）当 \(n\geqslant 2\) 时，\(a_n=S_n-S_{n-1}=n^2+2n+4-\left[ \left( n-1 \right)^2+2\left( n-1 \right)+4 \right]=2n+1\).
...
\begin{solution}
根据棣莫弗定理，\(z^3+k\overline{z}^3=\cos 3\theta+\i\sin 3\theta+k\left( \cos 3\theta-\i\sin 3\theta \right)=\left( 1+k \right)\cos 3\theta+\i\left( 1-k \right)\sin 3\theta\).
...
（2）由上式得 \(\left| z^3+k\overline{z}^3 \right|^2=\left( 1+k \right)^2\cos^2 3\theta+\left( 1-k \right)^2\sin^2 3\theta=1+k^2+2k\cos 6\theta\).
因此：
...
又因为 \(PH=CH\)，\(RH=CH\tan 30^\circ\)，所以 \(\cos \angle PHR=\dfrac{RH}{PH}=\tan 30^\circ=\dfrac{\sqrt{3}}{3}\).
