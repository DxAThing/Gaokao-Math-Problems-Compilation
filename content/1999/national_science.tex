...
\begin{solution}
由图可知，阴影部分同时属于集合 \(M\) 和 \(P\)，但不属于集合 \(S\)，所以所表示的集合为 \((M\cap P)\cap\overline{S}\)，故选 C.
\end{solution}
...
\begin{solution}
因为 \(f(a)=b\)，而 \(g\) 是 \(f\) 的反函数，所以 \(g(f(a))=a\)，从而 \(g(b)=a\)，故选 A.
\end{solution}
...
\begin{solution}
由 \(f(a)=-M<f(b)=M\) 且 \(f\) 在 \([a,b]\) 上为增函数可知 \(M>0\). 令 \(u=\omega x+\varphi\)，则 \(u\) 随 \(x\) 增大而增大，并且
\(\sin(\omega a+\varphi)=-1\)，\(\sin(\omega b+\varphi)=1\).
在这段递增的正弦曲线上，相位从 \(-\frac{\pi}{2}+2k\pi\) 变化到 \(\frac{\pi}{2}+2k\pi\)，其中必经过 \(u=2k\pi\). 因此 \(g(x)=M\cos u\) 可以取得最大值 \(M\)，故选 C.
\end{solution}
...
\begin{solution}
逐项检验：若 \(f(x)=\sin x\)，则 \(f(x)\sin x=\sin^2x\) 为偶函数；若 \(f(x)=\cos x\)，则
\(f(x)\sin x=\sin x\cos x=\frac{1}{2}\sin 2x\)，它是奇函数且以 \(\pi\) 为周期；若 \(f(x)=\sin 2x\)，乘积为偶函数；若 \(f(x)=\cos 2x\)，乘积虽为奇函数，但以 \(\pi\) 平移后变号，不以 \(\pi\) 为周期. 因此选 B.
\end{solution}
...
\begin{solution}
将极坐标方程化为直角坐标方程：
\(\rho=4\sin\left(\theta-\frac{\pi}{3}\right)=2\sin\theta-2\sqrt{3}\cos\theta\)，所以
...
\begin{solution}
沿 \(AB\) 方向作剪切不改变各平行截面的面积和体积，所以可将图形特殊化为 \(ED\perp\) 平面 \(ABCD\)，且 \(ED=2\) 的情形. 此时 \(EF\parallel AB\)，所以 \(EF\perp AD\)；又 \(EF\perp ED\)，从而 \(EF\perp\) 平面 \(ADE\).

沿三角形 \(ADF\) 分割多面体，可得到四棱锥 \(F\text{-}ABCD\) 和三棱锥 \(F\text{-}ADE\). 四棱锥 \(F\text{-}ABCD\) 的底面积为 \(9\)，高为 \(2\)，所以其体积为
\[
V_1=\frac{1}{3}\cdot9\cdot2=6
\]

在直角三角形 \(ADE\) 中，\(AD=3\)，\(DE=2\)，故其面积为 \(S_{\triangle ADE}=\frac{1}{2}\cdot3\cdot2=3\). 由于 \(EF\perp\) 平面 \(ADE\)，三棱锥 \(F\text{-}ADE\) 的高为 \(EF=\frac{3}{2}\)，从而
\[
V_2=\frac{1}{3}\cdot3\cdot\frac{3}{2}=\frac{3}{2}
\]

因此该多面体的体积为
\[
V=V_1+V_2=6+\frac{3}{2}=\frac{15}{2}
\]
故选 D.
\end{solution}
...
由 \(\sin\alpha>\tan\alpha\) 得 \(t<0\). 在 \(t<0\) 时，\(\tan\alpha>\cot\alpha\) 等价于 \(t>1/t\)，即 \(t^2<1\). 因而
...
\begin{solution}
设圆台母线长为 \(l\)，中截面的半径为 \(r=\frac{R+5}{2}\). 上、下两个圆台的母线长相等，分别为原圆台母线长的一半，因此侧面积之比等于相应上下底面周长和之比：
\[
...
\begin{solution}
取直线 \(m\)，\(n\) 的方向向量分别为 \(\bs{u}\)，\(\bs{v}\). 由 \(m\perp\alpha\)，\(\bs{u}\) 平行于平面 \(\alpha\) 的法向量；由 \(n\perp\beta\)，\(\bs{v}\) 平行于平面 \(\beta\) 的法向量. 因为 \(\alpha\perp\beta\)，两平面的法向量互相垂直，所以 \(\bs{u}\cdot\bs{v}=0\)，从而 \(m\perp n\). 因此所写命题正确.
\end{solution}
...
\begin{solution}
因为 \(0<\theta<\frac{\pi}{2}\)，复数 \(z\) 的实部和虚部均为正，所以 \(\arg z=\arctan\frac{2\sin\theta}{3\cos\theta}=\arctan\left(\frac{2}{3}\tan\theta\right)\). 令 \(t=\tan\theta>0\)，则
\[
y'=1-\frac{\frac{2}{3}\sec^2\theta}{1+\frac{4}{9}\tan^2\theta}=\frac{3-2t^2}{9+4t^2}
\]
分母恒为正，因此当 \(0<t<\sqrt{\frac{3}{2}}\) 时 \(y'>0\)，当 \(t>\sqrt{\frac{3}{2}}\) 时 \(y'<0\). 所以最大值在 \(t=\sqrt{\frac{3}{2}}=\frac{\sqrt{6}}{2}\)，即 \(\theta=\arctan\frac{\sqrt{6}}{2}\) 时取得.

在此时，利用正切差角公式得 \(\tan y=\frac{t-\frac{2}{3}t}{1+\frac{2}{3}t^2}=\frac{t}{6}=\frac{\sqrt{6}}{12}\). 又 \(0<y<\frac{\pi}{2}\)，故函数的最大值为 \(\arctan\frac{\sqrt{6}}{12}\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 设安装 \(n\) 对轧辊，若每对轧辊都按允许的最大减薄率工作，则出口厚度为 \(\alpha(1-r_0)^n\). 要使带钢能够减薄到 \(\beta\)，必须有 \(\alpha(1-r_0)^n\leqslant\beta\). 由于 \(0<1-r_0<1\)，两边取对数时不等号方向反转，得 \(n\geqslant\frac{\lg\beta-\lg\alpha}{\lg(1-r_0)}\). 因为 \(n\) 为整数，所需轧辊对数为 \(\left\lceil\frac{\lg\beta-\lg\alpha}{\lg(1-r_0)}\right\rceil\)，也就是至少安装不小于该分数的整数对轧辊.
\item 设带钢宽度为 \(w\)，第 \(j\) 对轧辊出口处的厚度为 \(t_j\). 由于每对轧辊的减薄率为 \(20\%\)，有 \(t_j=0.8^j t_0\). 第 \(k\) 对轧辊出口处相邻疵点的间距是轧辊周长 \(1600\mathrm{~mm}\)，所以两疵点之间带钢的体积为 \(1600t_kw\). 这段带钢经过第 \(k+1\) 至第 \(4\) 对轧辊后，输出间距为 \(L_k\)，体积变为 \(L_kt_4w\). 由带钢宽度不变且轧制过程中不考虑损耗，得 \(1600t_kw=L_kt_4w\)，即 \(L_k=1600\frac{t_k}{t_4}=\frac{1600}{0.8^{4-k}}\). 因而 \(L_1=3125\mathrm{~mm}\)，\(L_2=2500\mathrm{~mm}\)，\(L_3=2000\mathrm{~mm}\)，且 \(L_4=1600\mathrm{~mm}\)，与表中已给数值一致.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 当 \(0\leqslant y\leqslant1\) 时，图象从原点出发且斜率为 \(b^0=1\)，所以 \(x_1=1\). 当 \(1\leqslant y\leqslant2\) 时斜率为 \(b\)，故 \(x_2-x_1=1/b\)，从而 \(x_2=1+1/b\). 一般地，第 \(n\) 段线段的斜率为 \(b^n\)，所以 \(x_{n+1}-x_n=1/b^n\). 因此 \(x_n=\sum_{k=0}^{n-1}\frac{1}{b^k}=\frac{1-b^{-n}}{1-b}=\frac{b-(1/b)^{n-1}}{b-1}\)（\(n=1,2,\ldots\)）.
...
由上式可得 \(a-x=\frac{a(a+1)}{a+r}\)，从而 \(r=\frac{a(x+1)}{a-x}\)，同时 \(t=\frac{(a+1)y}{a-x}\). 代入 \(r^2=1+t^2\)，得 \(a^2(x+1)^2=(a-x)^2+(a+1)^2y^2\). 整理得点 \(C\) 的轨迹方程
\[
(a-1)x^2+2ax-(a+1)y^2=0
\]
由参数表示还可知实际轨迹取该二次曲线中 \(0\leqslant x<a\) 的部分；若排除使三点退化的 \(B=(-1,0)\)，则相应取 \(0<x<a\).
