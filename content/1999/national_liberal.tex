...
\begin{solution}
因为 \(f\) 在 \([a,b]\) 上递增且 \(f(a)=-M<f(b)=M\)，所以 \(M>0\). 记 \(u=\omega x+\varphi\). 由于 \(f\) 在 \([a,b]\) 上递增，且在端点分别取得最小值 \(-M\) 和最大值 \(M\)，相位从正弦函数的谷值沿递增支变化到峰值. 因而可写成
...
设倒圆锥中水面的半径为 \(r\)，水面高度为 \(h\). 水面以下的小圆锥与原倒圆锥相似，其轴截面是正三角形，且该截面的底边为 \(2r\)，所以 \(h=\frac{\sqrt3}{2}\cdot2r=\sqrt3r\)，即 \(r=h/\sqrt3\). 因而水的体积为
...
\begin{solution}
由题图可知，\(EF\) 到底面 \(ABCD\) 的距离为 \(2\). 设从底面起高为 \(2t\)（\(0\leqslant t\leqslant1\)）的截面面积为 \(A_t\). 该截面在 \(EF\) 方向的边长为 \(3(1-t)+\frac32t\)，在垂直于 \(EF\) 的方向的边长为 \(3(1-t)\)，所以
\[
A_t=\left(3(1-t)+\frac32t\right)3(1-t)
\]
这个面积只由 \(t\) 决定，与 \(EF\) 在底面上的水平位置无关. 因此由卡瓦列里原理，可将 \(EF\) 平行于底面平移到 \(E\) 在 \(D\) 的正上方的位置而不改变多面体体积；此时 \(ED\perp\) 平面 \(ABCD\)，且 \(ED=2\).
...
\]
因为正切函数在 \(\left(0,\frac\pi2\right)\) 上递增，且 \(\frac23\tan\theta<\tan\theta\)，所以 \(0<\alpha<\theta\)，从而 \(0<y=\theta-\alpha<\frac\pi2\). 令 \(t=\tan\theta>0\)，利用正切差角公式，得
\[
...
\bitmapfigure[float=inline,width=0.90\linewidth]{img/1999/national_liberal/q23_fig1.png}
\end{problem}
...
反过来，取满足该方程及 \(0<x<a\) 的点 \(C(x,y)\)，令 \(r=\dfrac{a(1+x)}{a-x}\)，并令 \(t=\dfrac{y(a+r)}a\). 由
\(r-1=\frac{(a+1)x}{a-x}\)，\(a+r=\frac{a(a+1)}{a-x}\)
可得 \(t=\dfrac{(a+1)y}{a-x}\). 又由轨迹方程 \((a+1)y^2=(a-1)x^2+2ax\)，有
\[
r^2-1=\frac{a^2(1+x)^2-(a-x)^2}{(a-x)^2}=\frac{(a+1)\bigl((a-1)x^2+2ax\bigr)}{(a-x)^2}=t^2
\]
故 \(r^2=1+t^2\)，点 \(B=(-1,t)\) 确实满足 \(OB=r\). 另外
\(\frac{a(r-1)}{a+r}=x\)，\(\frac{at}{a+r}=y\)
所以由角平分线定理得到的交点正是 \(C(x,y)\). 当 \(x=0\) 时，方程给出 \(y=0\)，正对应于上面的 \(B=(-1,0)\). 所以轨迹为上述方程在 \(0\leqslant x<a\) 内的部分.
