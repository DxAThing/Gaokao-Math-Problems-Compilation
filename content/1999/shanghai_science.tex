...
\begin{problem}
设正数数列 \(\{a_n\}\) 为一等比数列，且 \(a_2 = 4\)，\(a_4 = 16\)，求 \(\lim\limits_{n \to \infty} \frac{\lg a_{n + 1} + \lg a_{n + 2} + \cdots + \lg a_{2n}}{n^2}\).

\end{problem}

\begin{solution}
设公比为 \(q\). 因为 \(q^2=\dfrac{a_4}{a_2}=4\)，又 \(a_n>0\)，所以 \(q=2\). 于是 \(a_1=\dfrac{a_2}{q}=2\)，\(a_n=a_1q^{n-1}=2^n\).
因此
\[
...
\]
且 \(0<c\le1\) 时有 \(0\le\frac{2}{1+c}-1<1\)，所以 \(0\le \arg[\bar z(1-c\i)]<\dfrac{\pi}{4}\).
...
\]
且 \(c>1\) 时有 \(-1<\frac{2}{1+c}-1<0\)，所以 \(-\dfrac{\pi}{4}<\arctan\left(\dfrac{2}{1+c}-1\right)<0\)，从而 \(\dfrac{7\pi}{4}<\arg[\bar z(1-c\i)]<2\pi\).
...
\bitmapfigure[max width=0.78\linewidth,max totalheight=4.8cm]{img/1999/shanghai_science/q20_fig1.png}
...
（1）【法二】由题设 \(AB\perp AD\)，\(AB\perp AP\)，所以 \(AB\perp\) 平面 \(PAD\). 因此 \(AB\perp PD\). 又 \(AE\perp PD\)，所以 \(PD\perp\) 平面 \(ABE\). 从而 \(BE\perp PD\).

（2）【法一】设 \(A\) 为原点，\(AB\)，\(AD\)，\(AP\) 所在直线分别为 \(x\) 轴，\(y\) 轴，\(z\) 轴，建立空间直角坐标系，则 \(C(a,a,0)\)，\(D(0,2a,0)\).
...
（2）【法二】如图，设 \(G\)，\(H\) 分别为 \(ED\)，\(AD\) 的中点，连结 \(BH\)，\(HG\)，\(GB\). 易知 \(DH \cong CB\)，所以 \(BH\parallel CD\). 又 \(G\)，\(H\) 分别为 \(ED\)，\(AD\) 的中点，所以 \(HG\parallel AE\).
...
\bitmapfigure[max width=0.90\linewidth,max totalheight=4.8cm]{img/1999/shanghai_science/q21_fig1.png}
...
当 \(t=\dfrac{\sqrt2}{2}\) 时，\(S\) 取得最小值，此时所挖的土最少，所以改挖后的沟底宽为 \(\dfrac{\sqrt2}{2}\text{ 米}\).
\end{solution}
...
\begin{solution}
（1）将 \(y=\dfrac{1}{x}\) 代入椭圆方程，得 \(\dfrac{x^2}{a^2}+\dfrac{1}{b^2x^2}=1\)，化简得 \(b^2x^4-a^2b^2x^2+a^2=0\). 由题意，有 \(\Delta=a^4b^4-4a^2b^2=0\)，故 \(ab=2\). 于是由方程解得 \(x=\dfrac{a}{\sqrt2}\)（舍去 \(x=-\dfrac{a}{\sqrt2}\)），所以 \(P\left(\dfrac{a}{\sqrt2},\dfrac{\sqrt2}{a}\right)\).

（2）在 \(\triangle ABP\) 中，\(|AB|=2\sqrt{a^2-b^2}\)，高为 \(\dfrac{\sqrt2}{a}\)，所以 \(S(a)=\dfrac12\cdot2\sqrt{a^2-b^2}\cdot\dfrac{\sqrt2}{a}=\sqrt{2\left(1-\dfrac{4}{a^4}\right)}\).
又 \(a>b>0\)，\(b=\frac{2}{a}\)，所以 \(a>\sqrt2\). 因而 \(0<S(a)<\sqrt2\)，
故 \(\triangle ABP\) 的面积函数 \(S(a)\) 的值域为 \((0,\sqrt2)\).

（3）
\(g(a)=c^2=a^2-b^2=a^2-\dfrac{4}{a^2}\)，
若不等式 \(g(a)\ge S(a)\)，则 \(a^2-\dfrac{4}{a^2}\ge\sqrt{2\left(1-\dfrac{4}{a^4}\right)}\)，整理得 \(a^8-10a^4+24\ge0\)，即 \((a^4-4)(a^4-6)\ge0\). 因 \(a>\sqrt2\)，故 \(a^4-4>0\). 于是 \(a\ge\sqrt[4]{6}\).
所以
\[
f(a)=\min\{g(a),S(a)\}=
\begin{cases}
