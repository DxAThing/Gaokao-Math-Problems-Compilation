...
\]
故 \(\cos 2\alpha=\frac{1}{3}\). 又 \(2\alpha\in(\pi,2\pi)\)，得 \(\sin 2\alpha=-\frac{2\sqrt{2}}{3}\). 故
...
\begin{solution}
\begin{enumerate}
\item 由点 \(A\) 的坐标为 \((0,9)\)，得 \(c=9\)，即轨迹方程为 \(y=ax^2+9\). 令 \(y=0\)，得 \(ax^2+9=0\)，\(x^2=-\frac{9}{a}\). 由题意 \(6<\sqrt{-\frac{9}{a}}<7\)，解得 \(-\frac{1}{4}<a<-\frac{9}{49}\).
...
\begin{solution}
\begin{enumerate}
\item 连接 \(PD\). 因为 \(PA\perp\alpha\)，\(AD\perp l\)，所以 \(PD\perp l\). 因此 \(\angle PDA\) 是二面角 \(\alpha-l-\beta\) 的平面角. 在直角三角形 \(PAD\) 中，因为 \(PA=AD\)，所以 \(\angle PDA=45^\circ\). 即二面角 \(\alpha-l-\beta\) 的大小为 \(45^\circ\).

\item 设 \(E\) 是 \(DC\) 的中点，连结 \(ME\)，\(NE\). 因为 \(M\)，\(N\)，\(E\) 依次是 \(AB\)，\(PC\)，\(DC\) 的中点，所以 \(ME\parallel AD\)，\(NE\parallel PD\). 因此 \(ME\perp l\)，\(NE\perp l\). 因而 \(l\perp\) 平面 \(MNE\). 因为 \(AB\parallel l\)，所以 \(AB\perp\) 平面 \(MNE\). 因为 \(MN\subset\) 平面 \(MNE\)，所以 \(MN\perp AB\).

\item 设 \(Q\) 是 \(DP\) 的中点，连结 \(NQ\)，\(AQ\). 因为 \(NQ\parallel DC\)，且 \(NQ=\frac{1}{2}DC\)，又 \(AM\parallel DC\)，且 \(AM=\frac{1}{2}AB=\frac{1}{2}DC\)，所以 \(QN\parallel AM\)，\(QN=AM\). 因此 \(QNMA\) 为平行四边形，从而 \(AQ\parallel MN\). 于是 \(\angle PAQ\) 是异面直线 \(PA\) 与 \(MN\) 所成的角. 又 \(\triangle PAD\) 为等腰直角三角形，\(AQ\) 为斜边上的中线，所以 \(\angle PAQ=45^\circ\). 即 \(PA\) 与 \(MN\) 所成角的大小为 \(45^\circ\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 由 \(y=2x^2\)，得 \(y^\prime=4x\). 当 \(x=-1\) 时，\(y^\prime=-4\). 因此 \(l_1\) 的方程为 \(y-2=-4(x+1)\)，即 \(4x+y+2=0\).

\item 由 \(y=2x^2\) 及 \(x=a\)，解得点 \(B\) 的坐标为 \((a,2a^2)\). 由 \(4x+y+2=0\) 及 \(x=a\)，解得点 \(D\) 的坐标为 \((a,-4a-2)\). 又可求得点 \(A\) 到直线 \(BD\) 的距离为 \(\left|a+1\right|\)，且 \(\left|BD\right|=2a^2+4a+2=2\left(a+1\right)^2\). 因此 \(S_1=\left|a+1\right|^3\).

\item 当 \(a>-1\) 时，\(S_2=\int_{-1}^{a}\left(2x^2+4x+2\right)\,\mathrm{d}x=\frac{2}{3}\left(a+1\right)^3\). 当 \(a<-1\) 时，\(S_2=\int_{a}^{-1}\left(2x^2+4x+2\right)\,\mathrm{d}x=-\frac{2}{3}\left(a+1\right)^3\). 所以 \(S_1:S_2=\frac{3}{2}\). 即 \(S_1:S_2\) 的值为与 \(a\) 无关的常数.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 由已知得双曲线的渐近线为 \(y=\pm x\)，因而 \(S\) 为等轴双曲线. 因为其一个顶点为 \(A^\prime(0,\sqrt{2})\)，所以双曲线 \(S\) 的方程为 \(\frac{y^2}{2}-\frac{x^2}{2}=1\).

\item 若 \(B\left(x,\sqrt{x^2+2}\right)\) 是双曲线 \(S\) 的上支上到直线 \(l:y=x-\sqrt{2}\) 的距离为 \(\sqrt{2}\) 的点，则 \(\frac{\left|x-\sqrt{x^2+2}-\sqrt{2}\right|}{\sqrt{2}}=\sqrt{2}\). 解得 \(x=\sqrt{2}\)，\(y=2\). 因此点 \(B\) 的坐标是 \((\sqrt{2},2)\).

\item 若直线 \(l^\prime:y=\frac{2\sqrt{5}}{5}x+m\) 与直线 \(l\) 平行，且两线间的距离是 \(\sqrt{2}\)，则点 \(A(\sqrt{2},0)\) 到直线 \(l^\prime\) 的距离为 \(\sqrt{2}\). 解得 \(m_1=\frac{\sqrt{10}}{5}\)，\(m_2=-\sqrt{10}\). 设 \(l_1:y=\frac{2\sqrt{5}}{5}x+\frac{\sqrt{10}}{5}\)，\(l_2:y=\frac{2\sqrt{5}}{5}x-\sqrt{10}\). 由 \(y^2-x^2=2\) 及 \(y=\frac{2\sqrt{5}}{5}x+\frac{\sqrt{10}}{5}\)，得 \(x^2-4\sqrt{2}x+8=0\). 因此 \(\Delta_1=0\)，所以直线 \(l_1\) 与双曲线 \(S\) 只有一个公共点.
...
\begin{solution}
\begin{enumerate}
\item 由已知 \(A_n=\frac{3}{2}(a_n-1)\)（\(n\in\N\)）. 当 \(n=1\) 时，\(a_1=\frac{3}{2}(a_1-1)\)，解得 \(a_1=3\).
...
\item
\begin{enumerate}
\item 先证明形如 \(3^{2n+1}\)（\(n\in\N\)）的数都是数列 \(\{a_n\}\) 与 \(\{b_n\}\) 的公共项. 因为 \(a_{2n+1}=3^{2n+1}\)，所以 \(3^{2n+1}\in\{a_1,a_2,\cdots\}\).
又
\[
...
\item 再证明数列 \(\{a_n\}\) 中除了形如 \(3^{2n+1}\)（\(n\in\N\)）的项外，其它项都不是 \(\{b_n\}\) 中的项. 显然 \(a_1=3\) 不是 \(\{b_n\}\) 中的项. 对任意偶数项，
\[
