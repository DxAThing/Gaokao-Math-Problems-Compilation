...
\begin{problem}
复数 \(\frac{(2 + 2\i)^4}{(1 - \sqrt{3\i})^2}\) 等于（\quad）
...
\begin{problem}
如果直线 \(l\)，\(m\) 与平面 \(\alpha\)，\(\beta\)，\(\gamma\) 满足: \(l = \beta \cap \gamma\)，\(l \neq \alpha\)，\(m \subset \alpha\)，\(m \perp \gamma\)，那么必有（\quad）

\choices
    {\(\alpha \perp \gamma\) 且 \(l \perp m\)}
    {\(\alpha \perp \gamma\) 且 \(m \notin \beta\)}
    {\(m \notin \beta\) 且 \(l \perp m\)}
    {\(\alpha \notin \beta\) 且 \(\alpha \perp \gamma\)}

\end{problem}

\begin{problem}
当 \(-\frac{\pi}{2} \leqslant x \leqslant \frac{\pi}{2}, f(x) = \sin x + \sqrt{3} \cos x\) 的 20. 解不等式: \(\log_a(x + 1 - a) > 1\)（\quad）
...
\begin{problem}
已知点 \((-2,3)\) 与抛物线 \(y^{2} = 2px\) (\(p > 0\)) 的焦点的距离是 5，则 \(p = \) .
...
\begin{problem}
设等比数列 \(\{a_{n}\}\) 的前 \(n\) 项和为 \(S_{n}\). 若 \(S_{2} + S_{6} = 2S_{5}\)，求数列的公比 \(q\).

\end{problem}

\begin{problem}
已知 \(\triangle ABC\) 的三个内角 \(A, B, C\) 满足: \(A + C = 2B, \frac{1}{c \cos A} + \frac{1}{c \cos B} = -\frac{\sqrt{2}}{c \cos B}\)，求 \(\cos \frac{A - C}{2}\) 的值.

\end{problem}

\begin{problem}
如图，在三柱棱柱 \(ABC - A_{1}B_{1}C_{1}\) 中，\(AB = \frac{AA_{1}}{3} = a\)，\(E\)，\(F\) 分别是 \(BB_{1}\)，\(CC_{1}\) 上的点，\(BE = a\)，\(CF = 2a\). （1）求证: 面 \(AEF \perp\) 面 \(AFC\)；（2）求三棱锥 \(A_{1} - AEF\) 的体积.
...
\begin{problem}
已知 \(I_{1}\)，\(I_{2}\) 是过点 \(P(-\sqrt{2},0)\) 的两条互相垂直的直线，且 \(I_{1}\)，\(I_{2}\) 与双曲线 \(y^{2}-x^{2}=1\) 各有两个交点，分别为 \(A_{1}\)，\(B_{1}\) 和 \(A_{2}\)，\(B_{2}\). （1）求 \(I_{1}\) 的斜率 \(h_{1}\) 的取值范围；（2）若 \(A_{1}\) 恰是双曲线的一个顶点，求 \(|A_{2}B_{2}|\) 的值.
\end{problem}
