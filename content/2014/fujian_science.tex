...
\begin{solution}
由题图，函数 \(y=\log_a x\) 经过点 \((3,1)\)，所以 \(\log_a3=1\)，从而 \(a=3\). 逐一核对选项：A 中 \(y=a^{-x}=3^{-x}\)，有 \(y(1)=\frac{1}{3}\)，且函数递减，而图象在 \(x=1\) 处对应 \(y=3\) 且递增，不符；B 中 \(y=x^a=x^3\)，经过 \((0,0)\) 和 \((1,1)\)，并且在 \(\R\) 上递增，与图象相符；C 中 \(y=(-x)^a=(-x)^3=-x^3\)，应为递减函数，图象却递增，不符；D 中 \(y=\log_a(-x)=\log_3(-x)\) 的定义域为 \(x<0\)，且 \(x=-3\) 时 \(y=1\)，而图象标出的点为 \((-3,-1)\)，不符. 故选 B.
\end{solution}

\begin{problem}
阅读如图所示的程序框图，运行相应的程序，输出的 \(S\) 的值等于（\quad）

\bitmapfigure[width=0.42\linewidth]{img/2014/fujian_science/q5_fig1.png}
...
\begin{solution}
当 \(x>0\) 时，\(f(x)=x^2+1>1\)，其值域为 \((1,+\infty)\)；当 \(x\leqslant0\) 时，\(f(x)=\cos x\)，由于 \((-\infty,0]\) 中包含任意整数倍周期所需的区间，其值域为 \([-1,1]\). 因此 \(f(x)\) 的值域为 \([-1,+\infty)\).

另外，\(f(1)=2\)，而 \(f(-1)=\cos1\neq2\)，所以不是偶函数；在 \(x\leqslant0\) 上余弦函数并非单调递增，故 \(f\) 不是增函数；当 \(x\to+\infty\) 时 \(f(x)=x^2+1\to+\infty\)，不可能是周期函数. 故选 D.
\end{solution}
...
这是因为对圆上任意点 \(P\)，由三角不等式有 \(PQ\leqslant QO_1+O_1P=QO_1+\sqrt{2}\)，取 \(P\) 为圆心 \(O_1\) 与 \(Q\) 反向的半径端点时等号成立.
...
\begin{solution}
由约束条件，得 \(y\geqslant x+1\)，\(y\leqslant4-\frac{x}{2}\)，且 \(x\geqslant0\). 要使上下界有交集，必须有 \(x+1\leqslant4-\frac{x}{2}\)，即 \(0\leqslant x\leqslant2\). 因而可行域是由 \(x=0\)、\(y=x+1\) 和 \(y=4-\frac{x}{2}\) 围成的三角形，顶点为 \((0,1)\)、\((0,4)\)、\((2,3)\). 由于目标函数中 \(y\) 的系数为正，所以固定 \(x\) 时应取最小的 \(y=x+1\)，从而 \(z\geqslant3x+(x+1)=4x+1\geqslant1\). 当 \((x,y)=(0,1)\) 时取等号，所以 \(z\) 的最小值为 \(1\)，故填 \(1\).
\end{solution}
...
\(\frac{t^2+4}{8t}x+\frac{4-t^2}{16t}y=1\)，即为直线 \(l\). 为核对公共点数，取直线 \(l\) 的一个方向向量为 \(\bs{v}=(4-t^2,-2(t^2+4))\)，则直线 \(l\) 上的点可写为 \(P_t+\lambda\bs{v}\). 代入 \(E_0\) 的方程，利用 \(P_t\in E_0\) 及
\(\nabla\left(\frac{x^2}{4}-\frac{y^2}{16}\right)\big|_{P_t}\cdot\bs{v}=0\)，得
\(\frac{x^2}{4}-\frac{y^2}{16}-1=-4t^2\lambda^2=0\). 因为 \(t>0\)，所以 \(\lambda=0\)，直线 \(l\) 与 \(E_0\) 只有公共点 \(P_t\). 因而每条满足条件的直线 \(l\) 都与 \(E_0\) 有且只有一个公共点.
...
此时 \(f(x)=\e^x-2x\)，且 \(f'(x)=\e^x-2\). 当 \(x<\ln2\) 时 \(f'(x)<0\)，当 \(x>\ln2\) 时 \(f'(x)>0\)，所以 \(f(x)\) 在 \(x=\ln2\) 处取得极小值，极小值为 \(f(\ln2)=2-2\ln2\). 又当 \(x\to\pm\infty\) 时 \(f(x)\to+\infty\)，所以函数没有极大值.
