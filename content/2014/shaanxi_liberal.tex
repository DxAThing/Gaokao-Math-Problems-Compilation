...
\begin{problem}
已知 \( f(x) = \frac{x}{1 + x} \)，\( x \geqslant 0 \)，若 \( f_1(x) = f(x) \)，\( f_{n+1}(x) = f(f_n(x)) \)，\( n \in \mathbf{N}_+ \)，则 \( f_{2014}(x) \) 的表达式为 \fillinblank{} \fillinblank{}.
\end{problem}

\begin{problem}
三选一. 【A】设 \(a, b, m, n \in \R\)，且 \(a^2 + b^2 = 5\)，\(ma + nb = 5\)，则 \(\sqrt{m^{2} + n^{2}}\) 的最小值为 \fillinblank{}.

【B】如图，\(\triangle ABC\) 中，\(BC = 6\)，以 \(BC\) 为直径的半圆分别交 \(AB, AC\) 于点 \(E, F\)，若 \(AC = 2AE\)，则 \(EF = \fillinblank{}.\)
