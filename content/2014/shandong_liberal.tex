...
\bitmapinlinefigure[0.30\linewidth][max width=0.30\linewidth]{img/2014/shandong_liberal/q6_fig1.png}
...
\[
\log_a(x+c)=0\quad\Longrightarrow\quad x=1-c.
\]
...
两边平方并化简，得
\[
9+6\sqrt3m+3m^2=27+3m^2,\qquad m=\sqrt3.
\]
代入原等式可知等式成立，故选 B.
\end{solution}
...
\begin{solution}
由频率分布直方图可知，第一组和第二组的频率分别为 \(0.24\) 和 \(0.16\)，第三组的频率为 \(0.36\). 设志愿者总人数为 \(N\)，则
\[
(0.24+0.16)N=20,
\]
...
约束条件等价于
\[
y\geqslant x-1,\qquad y\leqslant2x-3.
\]
...
\[
a=\frac{4\sqrt5}{5},\qquad b=\frac{2\sqrt5}{5},
\]
...
\texfigure[max width=0.46\linewidth]{img_repaint/2014/shandong_liberal/q11_fig1.tex}
\end{problem}
...
\[
OE=2\cos\frac\pi6=\sqrt3,qquad PE=\sqrt{PO^2+OE^2}=\sqrt{1+3}=2.
\]
...
\[
\frac{a^2+b^2}{a^2}-\frac{p^2}{4b^2}=1,
\qquad
\frac{a^2p^2}{4b^2}=b^2.
\]
...
\[
50\times\frac1{50}=1,\qquad 150\times\frac1{50}=3,\qquad 100\times\frac1{50}=2.
\]
...
\bitmapfigure[max width=0.34\linewidth]{img/2014/shandong_liberal/q18_fig1.png}
\end{problem}

\begin{solution}
设 \(\overrightarrow{BA}=\bs{a}\)，\(\overrightarrow{BC}=\bs{c}\)，\(\overrightarrow{BP}=\bs{p}\). 由四边形的顶点顺序、\(AD\parallel BC\) 以及 \(AD=2BC\)，有
\[
\overrightarrow{AD}=2\overrightarrow{BC},\qquad \overrightarrow{BD}=\bs{a}+2\bs{c}.
\]
由于 \(E\)，\(F\) 分别是 \(AD\)，\(PC\) 的中点，故
\[
\overrightarrow{BE}=\bs{a}+\bs{c},\qquad
\overrightarrow{BF}=\frac{\bs{p}+\bs{c}}{2}.
\]
...
\]
因此，直线 \(AP\) 的方向向量是平面 \(BEF\) 内两条相交直线 \(BE\)，\(BF\) 的方向向量的线性组合，从而 \(AP\parallel\) 平面 \(BEF\).
...
\[
\overrightarrow{AC}=\bs{c}-\bs{a},\qquad
\overrightarrow{BE}\cdot\overrightarrow{AC}=(\bs{a}+\bs{c})\cdot(\bs{c}-\bs{a})=\lvert\bs{c}\rvert^{2}-\lvert\bs{a}\rvert^{2}=0,
\]
...
若 \(-\frac12<a<0\)，令
\[
x_{1}=\frac{-(a+1)+\sqrt{2a+1}}{a},\qquad
x_{2}=\frac{-(a+1)-\sqrt{2a+1}}{a}.
\]
由于 \(a<0\) 且 \(0<\sqrt{2a+1}<a+1\)，有 \(0<x_{1}<x_{2}\). 此时 \(g(x)=0\) 的两个根为 \(x_{1}\)，\(x_{2}\)，又因二次项系数 \(a<0\)，故
\[
f'(x)<0\quad(0<x<x_{1}\text{ 或 }x>x_{2}),\qquad
f'(x)>0\quad(x_{1}<x<x_{2}).
\]
...
\[
\frac{c}{a}=\frac{\sqrt3}{2},\qquad c^{2}=a^{2}-b^{2},
\]
...
\[
\frac{u^{2}}4+t^{2}u^{2}=1,\qquad u^{2}=\frac4{1+4t^{2}}.
\]
...
\]
当 \(r=\frac12\)，即 \(\lvert t\rvert=\frac12\) 时取等号. 因此三角形 \(OMN\) 面积的最大值为 \(\frac98\).
\end{enumerate}
\end{enumerate}
\end{solution}
