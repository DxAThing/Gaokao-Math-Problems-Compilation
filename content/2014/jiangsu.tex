...
\]
三角形的三个不等式具体化为 \(t+1>\frac{t+\sqrt2}{2}\)、\(t+\frac{t+\sqrt2}{2}>1\)、\(1+\frac{t+\sqrt2}{2}>t\). 第一式等价于 \(t>\sqrt2-2\)，由 \(t>0\) 自动满足；第二、第三式分别等价于 \(t>\frac{2-\sqrt2}{3}\) 和 \(t<2+\sqrt2\). 因而
\[
...
\bitmapfigure[width=0.5\linewidth]{img/2014/jiangsu/q21_fig1.png}
\end{problem}
...
\angle OCB=\angle OBC
\]
又因为 \(A\)，\(O\)，\(B\) 共线，所以 \(\angle OBC=\angle ABC\). 由 \(C\)，\(D\) 位于直径 \(AB\) 的两侧，点 \(B\)，\(D\) 位于弦 \(AC\) 的同侧，因此圆周角 \(\angle ABC\) 与 \(\angle ADC\) 处于同一弓形内，故
\[
...
\begin{solution}
因为 \(x>0\)，\(y>0\)，所以 \(1\)，\(x\)，\(y^2\) 以及 \(1\)，\(x^2\)，\(y\) 都是正数. 分别对这两组三个正数应用基本算术平均不等式，得
\[
...
\]
两式左、右两端均为正数，故可以相乘，得到
\[
(1+x+y^2)(1+x^2+y)\geqslant9\sqrt[3]{x^3y^3}=9xy
\]
当且仅当 \(1=x=y^2\) 且 \(1=x^2=y\) 时取等号；这两个条件同时成立时 \(x=y=1\)，故不等式得证.
\end{solution}
...
对等式 \(xf_0(x)=\sin x\) 求 \(n\) 阶导数. 由乘积求导公式，因 \(x\) 的二阶及以上导数为 \(0\)，左边为
\[
