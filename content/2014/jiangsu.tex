...
\begin{problem}
\begin{optionlist}
\item[\textbf{A.}] 如图，AB 是圆 O 的直径，C、D 是圆 O 上位于 AB 异侧的两点，证明： \( \angle OCB = \angle D \) .

\bitmapfigure{img/2014/jiangsu/q21_fig1.png}

\item[\textbf{B.}] 已知矩阵 \( A=\begin{bmatrix}-1&2\\1&x\end{bmatrix},B=\begin{bmatrix}1&1\\2&-1\end{bmatrix} \) ，向量 \( \alpha=\begin{bmatrix}2\\y\end{bmatrix},x,y \) 为实数，若 \( A\alpha=B\alpha \) ，求 \( x+y \) 的值.

\item[\textbf{C.}] 在平面直角坐标系 \(xOy\) 中，已知直线 \(l\) 的参数方程为 \(\begin{cases}x = 1 - \frac{\sqrt{2}}{2} t,\\ y = 2 + \frac{\sqrt{2}}{2} t,\end{cases}\)（\(t\) 为参数），直线 \(l\) 与抛物线 \(y^2 = 4x\) 交于 \(A, B\) 两点，求线段 \(AB\) 的长.

\item[\textbf{D.}] 已知 \(x > 0, y > 0\)，证明: \((1 + x + y^2)(1 + x^2 + y) \geqslant 9xy\).
\end{optionlist}
\end{problem}
