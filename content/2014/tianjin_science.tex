...
\bitmapfigure[max width=0.50\linewidth]{img/2014/tianjin_science/q3_fig1.png}
...
s=\frac56
\]
进一步由 \(p=s-\frac23=\frac16\)，知 \(\lambda\)，\(\mu\) 是方程
\[
t^2-\frac56t+\frac16=0
\]
的两个根，即 \(\lambda\)，\(\mu\) 为 \(\frac12\)，\(\frac13\)（次序不定），均确实属于区间 \([0,1]\). 因而 \(\lambda+\mu=\frac56\)，答案为 C.
\end{solution}
...
\((1-x_i)\frac{\mathrm d x_i}{\mathrm d a}=\e^{x_i}=\frac{x_i}{a}\)，\(\frac{\mathrm d x_i}{\mathrm d a}=\frac{x_i}{a(1-x_i)}\).
于是
\[
...
\(x_1=\frac{\ln u}{u-1}\)，\(x_2=\frac{u\ln u}{u-1}\).
又令 \(\varphi(u)=\frac{u-u^{-1}}{2}-\ln u\)，则 \(\varphi(1)=0\)，且
\(\varphi'(u)=\frac{(u-1)^2}{2u^2}>0\)，\((u>1)\).
故 \(u-u^{-1}>2\ln u\)，从而
\[
