...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2014/tianjin_science/q3_fig1.png}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2014/tianjin_science/q6_fig1.png}
...
\begin{problem}
已知菱形 \(ABCD\) 的边长为 \(2\)，\(\angle BAD = 120^{\circ}\)，点 \(E, F\) 分别在边 \(BC, DC\) 上，\(BE = \lambda BC\)，\(DF = \mu DC\)，若 \(\overrightarrow{AE} \cdot \overrightarrow{AF} = 1\)，\(\overrightarrow{CE} \cdot \overrightarrow{CF} = -\frac{2}{3}\)，则 \(\lambda + \mu =\)（\quad）
...
\begin{problem}
已知一个几何体的三视图如图所示（单位：m），则该几何体的体积为\fillinblank{} \(m^{3}\).

\bitmapfigure[max width=0.58\linewidth,max totalheight=5.2cm]{img/2014/tianjin_science/q10_fig1.png}
\end{problem}
...
\end{enumerate}
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2014/tianjin_science/q17_fig1.png}
\end{problem}

\begin{problem}
已知 \(q\) 和 \(n\) 均为给定的大于1的自然数. 设集合 \(M = \{0, 1, 2, \dots, q - 1\}\)，集合 \(A = \{x \mid x = x_1 + x_2q + \dots + x_nq^{n-1}, x_i \in M, i = 1, 2, \dots, n\}\).
...
\begin{problem}
已知函数 \( f(x) = x - ae^{x} (a \in \R) \)，\( x \in \R \)，已知函数 \( y = f(x) \) 有两个零点 \( x_{1}, x_{2} \)，且 \( x_{1} < x_{2} \).
