...
因为
\[
b=\log_2\frac13=-\log_2 3<0,
\qquad
c=\log_{\frac12}\frac13=\log_2 3>1,
\]
...
\[
j_1=i_1,\qquad j_2=i_2-1,\qquad j_3=i_3-2,
\]
...
\begin{solution}
对任意 \(0\leqslant x<y\leqslant1\)，由题设有 \(|f(x)-f(y)|<\frac12(y-x)\). 又因为 \(f(0)=f(1)=0\)，由三角不等式得
\[
|f(x)-f(y)|=|f(x)-f(0)+f(1)-f(y)|<\frac12x+\frac12(1-y)=\frac12(1-y+x).
\]
因此
\[
...
第三次计算得到
\[
y=\frac{11/3}{3}+2=\frac{29}{9},
\qquad
|y-x|=\left|\frac{29}{9}-\frac{11}{3}\right|=\frac89<1.
\]
...
\[
A=2F_1-M,\qquad B=2F_2-M,\qquad N=2N_0-M.
\]
从而
\[
|AN|=|2F_1-2N_0|=2|F_1N_0|,\qquad
|BN|=2|F_2N_0|.
\]
又椭圆的定义给出
\[
...
\[
a=3t,\qquad b=2t,\qquad 40t^2=c,\qquad t\neq0.
\]
此时
\[
...
所以
\[
4=9+9-18\cos C,\qquad \cos C=\frac79.
\]
因为 \(B\)，\(C\) 是三角形内角，
\[
\sin B=\frac{2\sqrt2}{3},\qquad \sin C=\sqrt{1-\frac{49}{81}}=\frac{4\sqrt2}{9}.
\]
于是
\[
...
并且
\[
E(X)=3\times\frac35=\frac95,
\qquad
D(X)=3\times\frac35\times\frac25=\frac{18}{25}.
\]
\end{solution}
...
\[
B=(0,0,0),\quad C=(2,0,0),\quad A=(-1,\sqrt3,0),\quad D=(-1,0,\sqrt3).
\]
于是
\[
E=\left(\frac12,\frac{\sqrt3}{2},0\right),\qquad
F=\left(\frac12,0,\frac{\sqrt3}{2}\right).
\]
所以
\[
\overrightarrow{EF}=\left(0,-\frac{\sqrt3}{2},\frac{\sqrt3}{2}\right),
\qquad
...
于是
\[
\lvert\bs{n}_1\rvert=\frac{\sqrt{15}}4,
\qquad
\lvert\bs{n}_2\rvert=\sqrt3,
\qquad
\bs{n}_1\cdot\bs{n}_2=\frac34.
\]
因此
\[
...
\[
C_2:\quad\frac{x^2}{6}+\frac{y^2}{3}=1.
\]
...
的两个根，因此
\[
t_1+t_2=-\frac{2\sqrt3}{1+2k^2},\qquad
t_1t_2=-\frac3{1+2k^2}.
\]
以 \(AB\) 为直径的圆过 \(P\)，等价于
\[
...
\[
\lvert Q\rvert^2=7-2\sqrt6,\qquad
Q\cdot(1,k)=\sqrt2(1+k)-\sqrt3.
\]
利用上面的根和与根积，整理得
\[
...
所以竖直直线不是所求直线. 因此
\[
l:\quad y=\frac{2+3\sqrt6}{25}(x-\sqrt3)
\quad\text{或}\quad
y=-(2+\sqrt6)(x-\sqrt3).
\]
\end{solution}
...
f(0)=\pi-\frac83>0,
\qquad
f\left(\frac\pi2\right)=-\pi^2-\frac{16}{3}<0.
\]
...
\]
下面证明 \(H\) 在 \(\left(0,\frac\pi2\right)\) 内恰有一个零点.
...
令
\[
K(t)=\frac{(\pi+2t)(\cos t-t)}{1+\sin t}.
\]
当 \(0<t<\frac\pi4\) 时，
\[
K'(t)=\frac{\cos t[2(1+\sin t)+\pi t+2t^2]-(1+\sin t)(2\pi+6t)}{(1+\sin t)^2}.
\]
利用 \(\cos t\leqslant1\)，\(1+\sin t\geqslant1\)，可得其分子不大于
\[
-(2\pi-2)+(\pi-6)t+2t^2<0,
\]
其中最后一个不等号由 \(0<t<\frac\pi4<1\) 和 \(\pi<4\) 得到，因为
\[
(\pi-6)t+2t^2=t(\pi-6+2t)<0
\]
所以 \(K\) 在 \(\left(0,\frac\pi4\right)\) 上严格递减. 又
\[
K(0)=\pi>\frac83,\qquad K\left(\frac\pi4\right)<0,
\]
故存在唯一的 \(t_0\in\left(0,\frac\pi4\right)\) 使 \(K(t_0)=\frac83\). 另一方面，当 \(t\geqslant\frac\pi4\) 时，\(\cos t-t<0\)，从而 \(H'(t)<0\). 由于
\[
H'(0)=\frac34-\frac2\pi>0,
\]
且在 \(\left(0,\frac\pi4\right)\) 上 \(H'\) 的符号由 \(3K(t)-8\) 决定，故 \(H\) 先严格递增，后严格递减. 又
\[
H(0)=0,\qquad H\left(\frac\pi2\right)=-\ln2<0,
\]
所以 \(H\) 在 \(\left(0,\frac\pi2\right)\) 内存在唯一零点，记为 \(t_1\). 于是
\[
x_1=\pi-t_1
\]
是 \(\left(\frac\pi2,\pi\right)\) 内 \(g(x)=0\) 的唯一解.

最后证明 \(x_0<t_1\). 由 \(f\) 的严格递减性，且
\[
f\left(\frac\pi6\right)
<\left(1-\frac\pi6\right)\frac{4\pi}{3}-4
=2-\frac29(\pi-3)^2-4<0,
\]
结合 \(f(0)>0\)，得 \(0<x_0<\frac\pi6\). 由 \(f(x_0)=0\)，有
\[
\frac{3(\cos x_0-x_0)}{4(1+\sin x_0)}=\frac2{\pi+2x_0},
\]
从而
\[
\frac{3x_0\cos x_0}{4(1+\sin x_0)}
=\frac{2x_0}{\pi+2x_0}+\frac{3x_0^2}{4(1+\sin x_0)}.
\]
又 \(x_0<\frac\pi6\) 给出 \(1+\sin x_0<\frac32\)，因此
\[
3\pi(\pi+2x_0)>3\pi^2>27>24>16(1+\sin x_0).
\]
于是
\[
\frac{3x_0^2}{4(1+\sin x_0)}
>\frac{4x_0^2}{\pi(\pi+2x_0)}
=\frac{2x_0}{\pi}-\frac{2x_0}{\pi+2x_0}.
\]
利用 \(\ln(1+u)<u\)（\(u>0\)），得到
\[
H(x_0)
>\frac{2x_0}{\pi+2x_0}+\frac{2x_0}{\pi}-\frac{2x_0}{\pi+2x_0}-\ln\left(1+\frac{2x_0}{\pi}\right)>0.
\]
由于 \(H\) 的唯一零点为 \(t_1\)，且 \(H\) 在零点前为正，所以 \(x_0<t_1\). 因而
\[
...
\[
E=(-u,h),\qquad C=(u,h),\qquad P=(p,h),
\]
其中 \(u^2+h^2=r^2\)，\(p>u>0\). 设 \(G=(g,h)\). 由 \(PG=PD\) 以及点 \(P\) 的切线长公式，记
\[
t=PG=PD=\sqrt{p^2-u^2},\qquad g=p-t.
\]
...
\[
\lambda_0=\frac{2r(h+r)}{g^2+(h+r)^2},\qquad D_0=X(\lambda_0).
\]
...
=-rh+r(h+r)=r^2.
\]
由于 \(\lvert D_0\rvert=r\)，上式说明 \(OD_0\perp PD_0\)，即 \(PD_0\) 是圆在 \(D_0\) 处的切线. 取图示位置的切点，故 \(D_0=D\)，从而 \(A_0\)，\(D\)，\(G\) 共线. 因而题设中的点 \(A=A_0=(0,-r)\).

直线 \(AB\perp EP\)，所以 \(AB\) 是过 \(A\) 的竖直弦，另一交点为 \(B=(0,r)\). 因此 \(O\) 是 \(AB\) 的中点，\(AB\) 为圆的直径.

在此基础上，令
\[
E=(-r\cos\theta,-r\sin\theta),\quad
C=(r\cos\theta,-r\sin\theta),
\]
\[
D=(r\cos\varphi,r\sin\varphi),\qquad
0<\theta,\varphi<\frac\pi2.
\]
此时 \(A=(0,-r)\)，\(B=(0,r)\)，所以
\[
AC^2=2r^2(1-\sin\theta),\qquad
BD^2=2r^2(1-\sin\varphi).
\]
...
原圆的参数方程可写为
\[
x=\cos\theta,\qquad y=\sin\theta,\qquad 0\leqslant\theta<2\pi.
\]
...
直线 \(l\) 与 \(C\) 联立，得
\[
y=2-2x,\qquad x^2+(1-x)^2=1.
\]
所以 \(x=0\) 或 \(x=1\)，交点为
\[
P_1=(0,2),\qquad P_2=(1,0).
\]
线段 \(P_1P_2\) 的中点为
\[
...
\[
y-1=\frac12\left(x-\frac12\right),\qquad
y=\frac12x+\frac34.
\]
...
所以
\[
N=\left[-\frac14,\frac34\right],\qquad M\cap N=\left[0,\frac34\right].
\]
