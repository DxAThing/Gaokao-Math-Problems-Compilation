...
\begin{solution}
\begin{enumerate}
\item \(\left|\overrightarrow{OP}\right| = 2\sqrt{2}\).
\item \(m - n = y - x\)，其最大值为 \(1\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(g_n(x) = \dfrac{x}{1 + nx}\).
\item \(\alpha \leqslant 1\).
\item \(g(1) + g(2) + \cdots + g(n) > n - f(n)\).
\end{enumerate}
\end{solution}
