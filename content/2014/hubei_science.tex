...
\begin{solution}
正视图是把各顶点沿 \(y\) 轴方向投影到 \(xOz\) 平面，四个顶点的投影为 \((0,2)\)、\((2,0)\)、\((1,1)\)、\((2,2)\)，按原四面体的棱连接并判断可见性，所得图形与图\(\circled{4}\)一致. 俯视图是沿 \(z\) 轴方向投影到 \(xOy\) 平面，四个顶点的投影为 \((0,0)\)、\((2,2)\)、\((1,2)\)、\((2,2)\)，所得图形与图\(\circled{2}\)一致. 故选 D.
\end{solution}
...
\begin{solution}
两条直线的斜率都为 \(1\). 要使它们与单位圆的四个交点把圆周分成四段等弧，这四个交点必须是相邻的四个四等分点 \((1,0)\)、\((0,1)\)、\((-1,0)\)、\((0,-1)\). 经过 \((0,1)\)、\((-1,0)\) 的直线为 \(y=x+1\)，经过 \((1,0)\)、\((0,-1)\) 的直线为 \(y=x-1\). 所以 \(\{a,b\}=\{1,-1\}\)，从而 \(a^2+b^2=2\).
\end{solution}
...
\[
C:\quad y^2=4x\ (x\geqslant0)\quad\text{或}\quad y=0\ (x\leqslant0).
\]
