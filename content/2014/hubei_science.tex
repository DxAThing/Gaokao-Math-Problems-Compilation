...
\begin{problem}
已知曲线 \(C_{1}\) 的参数方程是 \(\left\{ \begin{array}{l} x = \sqrt{t}, \\ y = \frac{\sqrt{3} t}{3}, \end{array} \right.\)（\(t\) 为参数），以坐标原点为极点，\(x\) 轴的正半轴为极轴建立极坐标系，曲线 \(C_{2}\) 的极坐标方程是 \(\rho = 2\)，则 \(C_{1}\) 与 \(C_{2}\) 交点的直角坐标为\fillinblank{}.
\end{problem}
