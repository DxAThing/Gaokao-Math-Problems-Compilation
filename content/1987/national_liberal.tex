\chapter{1987年全国卷（文）}

\section{单选题}

\begin{problem}
设 \(S\)、\(T\) 是两个非空集合，且 \(S \not\subseteq T\)，\(T \not\subseteq S\)，今 \(X = S \cap T\)，那么 \(S \cup X\) 等于（\quad）

\choices
    {\(X\)}
    {\(T\)}
    {\(\varnothing\)}
    {\(S\)}

\end{problem}

\begin{problem}
设椭圆方程为 \(\frac{x^2}{a^2} + \frac{y^2}{b^2} = 1\)（\(a > b > 0\)），今 \(c = \sqrt{a^2 - b^2}\)，那么它的准线方程为（\quad）

\choices
    {\(y = \pm \frac{a^2}{c}\)}
    {\(y = \pm \frac{b^2}{c}\)}
    {\(x = \pm \frac{a^2}{c}\)}
    {\(x = \pm \frac{b^2}{c}\)}

\end{problem}

\begin{problem}
设 \(\log_3 4 \cdot \log_4 8 \cdot \log_8 m = \log_4 16\)，那么 \(m\) 等于（\quad）

\choices
    {\(\frac{9}{2}\)}
    {\(9\)}
    {\(18\)}
    {\(27\)}

\end{problem}

\begin{problem}
复数 \(\sin 40^\circ - \i\cos 40^\circ\) 的辐角为（\quad）

\choices
    {\(40^\circ\)}
    {\(140^\circ\)}
    {\(220^\circ\)}
    {\(310^\circ\)}

\end{problem}

\begin{problem}
二次函数 \(y = f(x)\) 的图象如图所示，那么此函数为（\quad）

\bitmapfigure[max width=0.44\linewidth,max totalheight=4.0cm]{img/1987/national_liberal/q5_fig1.png}

\choices
    {\(y = x^2 - 4\)}
    {\(y = 4 - x^2\)}
    {\(y = \frac{3}{4}\left(4 - x^2\right)\)}
    {\(y = \frac{3}{4}\left(2 - x^2\right)\)}

\end{problem}

\begin{problem}
在区间 \((-\infty,0)\) 上为增函数的是（\quad）

\choices
    {\(y = -\log_{\frac{1}{2}}(-x)\)}
    {\(y = \frac{x}{1 - x}\)}
    {\(y = -(x + 1)^2\)}
    {\(y = 1 + x^2\)}

\end{problem}

\begin{problem}
已知平面上一点 \(P\) 在原坐标系中的坐标为 \((0,m)\)（\(m \neq 0\)），而在平移后的新坐标系中的坐标为 \((m,0)\)，那么新坐标系的原点 \(O'\) 在原坐标系中的坐标为（\quad）

\choices
    {\((-m,m)\)}
    {\((m,-m)\)}
    {\((m,m)\)}
    {\((-m,-m)\)}

\end{problem}

\begin{problem}
要得到函数 \(y = \sin \left(2x - \frac{\pi}{3}\right)\) 的图象，只需将函数 \(y = \sin 2x\) 的图象（如图）（\quad）

\bitmapfigure[max width=0.5\linewidth,max totalheight=3.6cm]{img/1987/national_liberal/q8_fig1.png}

\choices
    {向左平行移动 \(\frac{\pi}{3}\)}
    {向右平行移动 \(\frac{\pi}{3}\)}
    {向左平行移动 \(\frac{\pi}{6}\)}
    {向右平行移动 \(\frac{\pi}{6}\)}

\end{problem}

\section{解答题}

\begin{problem}
求函数 \(y = \sin^2 2x\) 的周期.

\end{problem}

\begin{problem}
已知方程 \(\frac{x^2}{2 + \lambda} - \frac{y^2}{1 + \lambda} = 1\) 表示双曲线，求 \(\lambda\) 的范围.

\end{problem}

\begin{problem}
若 \((1 + x)^n\) 的展开式中，\(x^3\) 的系数等于 \(x\) 的系数的 \(7\) 倍，求 \(n\).

\end{problem}

\begin{problem}
求极限：\(\displaystyle\lim_{n\to\infty}\left(\frac{1}{n^2} + \frac{2}{n^2} + \frac{3}{n^2} + \cdots + \frac{2n}{n^2}\right)\).

\end{problem}

\begin{problem}
由数字 \(1\)，\(2\)，\(3\)，\(4\)，\(5\) 组成没有重复数字且数字 \(1\) 与 \(2\) 不相邻的五位数，求这种五位数的个数.

\end{problem}

\begin{problem}
求函数 \(y = \log_2(1 + 2x - 3x^2)\) 的定义域.

\end{problem}

\begin{problem}
圆锥底面积为 \(3\pi\)，母线与底面所成的角为 \(60^\circ\)，求它的体积.

\end{problem}

\begin{problem}
发电厂发出的电是三相交流电，它的三根导线上的电流强度分别是时间 \(t\) 的函数：\(I_A = I\sin \omega t\)，\(I_B = I\sin(\omega t + 120^\circ)\)，\(I_C = I\sin(\omega t + 240^\circ)\)，求 \(I_A + I_B + I_C\) 的值.

\end{problem}

\begin{problem}
在复平面内，已知等边三角形的两个顶点表示的复数分别为 \(2\)，\(\frac{1}{2} + \frac{\sqrt{3}}{2}\i\)，求第三个顶点表示的复数.

\end{problem}

\begin{problem}
如图，三棱锥 \(P-ABC\) 中，已知 \(PA \perp BC\)，\(PA = BC = L\)，\(PA\)、\(BC\) 的公垂线 \(ED = h\). 求证：三棱锥 \(P-ABC\) 的体积 \(V = \frac{1}{6}L^2h\).

\bitmapfigure[max width=0.34\linewidth,max totalheight=4.8cm]{img/1987/national_liberal/q18_fig1.png}

\end{problem}

\begin{problem}
设对所有实数 \(x\)，不等式 \(x^2\log_2 \frac{4(a + 1)}{a} + 2x\log_2 \frac{2a}{a + 1} + \log_2 \frac{(a + 1)^2}{4a^2} > 0\) 恒成立，求 \(a\) 的取值范围.

\end{problem}

\begin{problem}
设复数 \(z_1\) 和 \(z_2\) 满足关系式 \(z_1\overline{z_2} + \overline{A}z_1 + A\overline{z_2} = 0\)，其中 \(A\) 为不等于 \(0\) 的复数. 证明：

（1）\(|z_1 + A||z_2 + A| = |A|^2\)；

（2）\(\frac{z_1 + A}{z_2 + A} = \left|\frac{z_1 + A}{z_2 + A}\right|\).

\end{problem}

\begin{problem}
设数列 \(a_1,a_2,\cdots,a_n,\cdots\) 的前 \(n\) 项的和 \(S_n\) 与 \(a_n\) 满足 \(S_n = ka_n + 1\)（其中 \(k\) 是与 \(n\) 无关的常数，且 \(k \neq 1\)）.

（1）试写出用 \(n\)、\(k\) 表示的 \(a_n\) 的表达式；

（2）若 \(\displaystyle\lim_{n\to\infty}S_n = 1\)，求 \(k\) 的取值范围.

\end{problem}

\begin{problem}
正方形 \(ABCD\) 在直角坐标平面内，已知其一条边 \(AB\) 在直线 \(y = x + 4\) 上，\(C\)、\(D\) 在抛物线 \(x = y^2\) 上，求正方形 \(ABCD\) 的面积.
\end{problem}

