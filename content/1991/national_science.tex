...
\begin{solution}
最小值为 \(2-\sqrt{2}\)，此时 \(x=k\pi-\frac{3\pi}{8}\)，其中 \(k\in\Z\).
\end{solution}
\end{problem}
...
\begin{solution}
点 \(B\) 到平面 \(EFG\) 的距离为 \(\frac{2\sqrt{11}}{11}\).
\end{solution}
\end{problem}
...
\begin{solution}
函数 \(f(x)=-x^3+1\) 在 \(\left(-\infty,+\infty\right)\) 上是减函数.
\end{solution}
\end{problem}
...
\begin{solution}
当 \(n\) 为奇数时，解集为 \(\left\{x\mathrel{\mid}\sqrt{a}<x<\frac{1+\sqrt{1+4a}}{2}\right\}\)；当 \(n\) 为偶数时，解集为 \(\left\{x\mathrel{\mid}x>\frac{1+\sqrt{1+4a}}{2}\right\}\).
\end{solution}
\end{problem}
