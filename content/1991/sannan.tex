...
\begin{problem}
在直角坐标系 \(xOy\) 中，参数方程 \(\left\{ \begin{array}{l} x=2t+1, \\ y=2t^2-1, \end{array} \right.\)（其中 \(t\) 是参数）表示的曲线是（\quad）
...
\begin{problem}
已知函数 \(f(x)=\frac{2^x-1}{2^x+1}\). （1）证明：\(f(x)\) 在 \((-\infty,+\infty)\) 上是增函数；（2）证明：对于任意不小于 \(3\) 的自然数 \(n\)，都有 \(f(n)>\frac{n}{n+1}\).
\end{problem}
