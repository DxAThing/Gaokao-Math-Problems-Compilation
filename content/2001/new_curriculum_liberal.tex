...
\bitmapfigure[max width=0.74\linewidth,max totalheight=4.6cm]{img/2001/new_curriculum_liberal/q11_fig1.png}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2001/new_curriculum_liberal/q12_fig1.png}
...
\begin{enumerate}
\item 求 \(a\) 及 \(k\) 的值；
\item 求 \(\lim\limits_{n\to\infty}\left(\frac{1}{S_1}+\frac{1}{S_2}+\cdots+\frac{1}{S_n}\right)\).
\end{enumerate}
\end{problem}

\begin{solution}
（1）由等差数列性质，\(a+3a=2\times4\)，得 \(a=2\)，公差 \(d=2\). 因而 \(S_k=2k+\frac{k(k-1)}{2}\times2=k^2+k=2550\).
解得 \(k=50\)（舍去 \(k=-51\)）.
...
\bitmapfigure[max width=0.55\linewidth,max totalheight=4.8cm]{img/2001/new_curriculum_liberal/q19_fig1.png}
\end{problem}

\begin{solution}
由相互独立性，\(P_1=0.80\times0.90\times0.90=0.648\). 又 \(P_2=0.80\left[1-(1-0.90)(1-0.90)\right]=0.792\).
...
\bitmapfigure[max width=0.62\linewidth,max totalheight=4.8cm]{img/2001/new_curriculum_liberal/q20a_fig1.png}

\end{problem}

\begin{solution}
取 \(B(a,a,0)\)，\(C(-a,a,0)\)，\(D(-a,-a,0)\)，\(V(0,0,h_0)\)，则 \(E=(-a/2,a/2,h_0/2)\). 因此 \(\overrightarrow{BE}=\left(-\frac{3a}{2},-\frac a2,\frac{h_0}{2}\right)\)，\(\overrightarrow{DE}=\left(\frac a2,\frac{3a}{2},\frac{h_0}{2}\right)\).
从而
\[
\cos\left\langle\overrightarrow{BE},\overrightarrow{DE}\right\rangle=\frac{h_0^2-6a^2}{10a^2+h_0^2}
\]

又 \(\overrightarrow{CV}=(a,-a,h_0)\). 由题设 \(BE\perp CV\)，得 \(h_0^2=2a^2\)，故 \(\cos\angle BED=-\frac13\).
\end{solution}
...
\bitmapfigure[max width=0.48\linewidth,max totalheight=4.8cm]{img/2001/new_curriculum_liberal/q20b_fig1.png}

\end{problem}

\begin{solution}
底面直角梯形 \(ABCD\) 的面积为 \(\frac12(BC+AD)AB=\frac12\left(1+\frac12\right)=\frac34\).
所以四棱锥体积为 \(\frac13\times1\times\frac34=\frac14\).

延长 \(BA\)，\(CD\) 交于 \(E\)，连接 \(SE\). 由 \(AD\parallel BC\) 且 \(BC=2AD\)，得 \(EA=AB=SA=1\)，故 \(SB=\sqrt2\). 又 \(BC\perp SB\)，所以所求二面角的正切值为 \(\tan\angle BSC=\frac{BC}{SB}=\frac{\sqrt2}{2}\).
\end{solution}
...
两式相加得 \(x^2+y^2=2\cos\theta\)，故四个交点共圆，圆心为原点，半径 \(r=\sqrt{2\cos\theta}\). 当 \(0<\theta<\frac\pi4\) 时，\(\sqrt[4]{2}<r<\sqrt2\).
\end{solution}
