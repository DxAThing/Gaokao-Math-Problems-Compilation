...
\]
所以
\(x^2+y^2=\sqrt2(x+y)\)，\(\left(x-\frac1{\sqrt2}\right)^2+\left(y-\frac1{\sqrt2}\right)^2=1\).
...
\]
交换自变量与函数值，反函数为
\(y=-\arccos(x-1)\)，\(0\leqslant x\leqslant2\)
选 A.
\end{solution}
...
a=h(\alpha)<h(\beta)=b
\]
并且由 \(h(0)=1\)、\(h\left(\frac\pi4\right)=\sqrt2\) 及严格递增性，得
\[
1<a<b<\sqrt2
\]
所以 \(ab>1\) 且 \(ab<2\). 因此只有选项 B 正确.
\end{solution}
...
\(A=(s,0,0)\)，\(C=\left(\frac s2,\frac{\sqrt3s}{2},0\right)\)，\(B_1=(0,0,h)\)
则 \(C_1=\left(\frac s2,\frac{\sqrt3s}{2},h\right)\). 取两直线方向向量
\(\overrightarrow{AB_1}=(-s,0,h)\)，\(\overrightarrow{BC_1}=\left(\frac s2,\frac{\sqrt3s}{2},h\right)\).
它们的数量积为
\[
...
\[
h(x_2)-h(x_1)=\bigl(f(x_2)-f(x_1)\bigr)+\bigl(g(x_1)-g(x_2)\bigr)\geqslant0
\]
所以 \(h\) 单调递增，命题\circled{2}正确. 若 \(f\) 单调递减、\(g\) 单调递增，则
\[
h(x_2)-h(x_1)=\bigl(f(x_2)-f(x_1)\bigr)-\bigl(g(x_2)-g(x_1)\bigr)\leqslant0
\]
所以 \(h\) 单调递减，命题\circled{3}正确.

命题\circled{1}和命题\circled{4}均不一定成立. 例如，取 \(f(x)=x\)，\(g(x)=2x\)，则二者都递增而 \(f(x)-g(x)=-x\) 递减；取 \(f(x)=-x\)，\(g(x)=-2x\)，则二者都递减而 \(f(x)-g(x)=x\) 递增. 因此正确的命题为\circled{2}\circled{3}，选 C.
\end{solution}
...
\bitmapfigure[max width=6.4cm]{img/2001/syllabus_science/q12_fig1.png}
...
\(\overrightarrow{PF_1}=(-x-5,-y)\)，\(\overrightarrow{PF_2}=(5-x,-y)\).
由 \(PF_1\perp PF_2\)，得
\((x+5)(x-5)+y^2=0\)，\(x^2+y^2=25\).
另一方面，双曲线方程可写为 \(16x^2-9y^2=144\). 代入 \(x^2=25-y^2\)，得
\(16(25-y^2)-9y^2=144\)，\(25y^2=256\).
...
\]
\item 由二项式定理，
\((1+m)^n=\sum_{i=0}^{n}\mathrm C_n^i m^i\)，\((1+n)^m=\sum_{i=0}^{m}\mathrm C_m^i n^i\).
由第（1）问及 \(\mathrm C_k^i=\dfrac{\mathrm A_k^i}{i!}\)，对于 \(2\leqslant i\leqslant m\)，有
\[
n^i\mathrm A_m^i<m^i\mathrm A_n^i
\Longrightarrow
n^i\mathrm C_m^i<m^i\mathrm C_n^i
\]
当 \(i=0\) 时两边相应的项都为 \(1\)，当 \(i=1\) 时两边相应的项都为 \(mn\)；而左边还含有 \(i=m+1\)，\(\ldots\)，\(n\) 的正项. 因此逐项比较可得
\[
(1+n)^m<(1+m)^n
\]
即 \((1+m)^n>(1+n)^m\).
