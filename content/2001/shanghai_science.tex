...
\begin{problem}
设数列 \(\{a_{n}\}\) 是公比 \(q > 0\) 的等比数列，\(S_{n}\) 是它的前 \(n\) 项和. \(\displaystyle\lim_{n \to \infty} S_{n} = 7\)，则此数列的首项 \(a_{1}\) 的取值范围是\fillinblank{}.
\end{problem}
...
\begin{problem}
直线 \( y = 2x - \frac{1}{2} \) 与曲线 \( \left\{ \begin{array}{l} x = \sin \varphi, \\ y = \cos 2\varphi. \end{array} \right. \)（\(\varphi\) 为参数）的交点坐标为\fillinblank{}.
\end{problem}
...
\bitmapfigure[page=1,pagebox=mediabox,trim=32pt 4pt 659pt 666pt,clip,max width=0.64\linewidth,max totalheight=4.2cm]{source/普通高考/2001/2001上海理.pdf}
\end{problem}
...
\bitmapinclude[page=2,pagebox=mediabox,trim=812pt 194pt 7pt 224pt,clip,max totalheight=6.0cm]{source/普通高考/2001/2001上海理.pdf}
\end{problem}
