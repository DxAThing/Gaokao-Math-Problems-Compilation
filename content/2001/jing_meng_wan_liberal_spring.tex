...
\bitmapfigure[max width=0.28\linewidth]{img/2001/jing_meng_wan_liberal_spring/q11_fig1.png}
...
\]
\[
E=(0,0,1),\quad F=(1,0,1),\quad M=(1,1,1),\quad N=(0,1,1).
\]
于是
\[
\overrightarrow{BM}=(0,1,1),\qquad \overrightarrow{ED}=(0,1,-1),
\]
...
\[
T_n=S_n-S_{n-1}\qquad(n=1,2,\ldots,12),
\]
其中 \(S_0=0\). 由题设
\[
...
\[
\bs{u}=r(\cos\varphi,\sin\varphi),\qquad
\bs{v}=r(-\sin\varphi,\cos\varphi),
\]
其中 \(r>0\)，这样两向量等长且互相垂直. 因为 \(A+\bs{u}\)，\(A+\bs{v}\) 都在椭圆上，分别代入 \(x^2+4y^2=4\)，得到
\[
r=-\frac{4\cos\varphi}{\cos^2\varphi+4\sin^2\varphi},\qquad
r=\frac{4\sin\varphi}{\sin^2\varphi+4\cos^2\varphi}.
\]
消去 \(r\)，并整理得
\[
...
\]
设 \(z\) 的辐角为 \(\alpha\)，则 \(z^7=1\) 给出 \(7\alpha=2k\pi\)，其中 \(k\) 为整数. 因此
\[
\cos(7\alpha)=1,\qquad \cos(7\alpha-j\alpha)=\cos(j\alpha).
\]
特别地，\(\cos3\alpha=\cos4\alpha\)，\(\cos5\alpha=\cos2\alpha\)，\(\cos6\alpha=\cos\alpha\). 对第一问的等式取实部，得到
\[
...
为证明第二问，建立直角坐标系：取 \(D\) 为原点，\(AB\) 为 \(x\) 轴，平面 \(\Pi\) 为 \(yz\) 平面，并令
\[
A=(-u,0,0),\quad B=(v,0,0),\quad C=(0,c,0),\quad N=(0,n,0),\quad V=(0,n,w),
\]
...
\[
\overrightarrow{AM}=(u,r,s),\qquad \overrightarrow{BM}=(-v,r,s),
\]
从而
\[
...
再求第三问. 在平面 \(\Pi\) 内，设 \(K\) 是 \(M\) 在 \(DC\) 上的正投影. 则 \(MK\) 是点 \(M\) 到平面 \(ABC\) 的距离. 由 \(\angle MDC=\theta\)，有
\[
\frac{MK}{DK}=\tan\theta.
\]
...
\[
DK=c\cos^2\theta,\qquad MK=c\sin\theta\cos\theta.
\]
由于平面 \(\Pi\perp AB\)，直线 \(AB\) 与 \(VC\) 的距离等于点 \(D\) 到直线 \(VC\) 的距离. 直线 \(VC\) 与 \(DC\) 的夹角为 \(\frac{\pi}{2}-\theta\)，所以
\[
h=CD\sin\left(\frac{\pi}{2}-\theta\right)=c\cos\theta,
\qquad c=\frac{h}{\cos\theta}.
\]
