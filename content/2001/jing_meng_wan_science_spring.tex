...
\begin{solution}
由指数运算律，\(f(x+y)=a^{x+y}=a^xa^y=f(x)f(y)\). 其他三个等式对任意实数 \(x\)，\(y\) 并不恒成立. 因此选 C.
\end{solution}
...
\begin{solution}
由 \(y=-\sqrt{1-x}\) 可知 \(y\leqslant0\)，且 \(y^2=1-x\)，从而 \(x=1-y^2\). 交换 \(x\)，\(y\) 得反函数 \(y=1-x^2\)，其定义域是原函数的值域 \(x\leqslant0\). 因此选 C.
\end{solution}
...
\]
所以圆心的直角坐标为 \((2,\frac32)\)，其极径为 \(\frac52\). 又圆心在第一象限，故极角满足 \(\sin\theta=\frac{3/2}{5/2}=\frac35\)，圆心的极坐标为 \(\left(\frac52,\arcsin\frac35\right)\). 因此选 A.
\end{solution}
...
\begin{solution}
设 \(P=(1,t)\)，则 \(\overrightarrow{OP}=(1,t)\). 以 \(OP\) 为直角边作等腰直角三角形时，\(\overrightarrow{OQ}\) 是 \(\overrightarrow{OP}\) 旋转 \(90^\circ\) 所得的向量，因此
\[
Q=(-t,1)\quad\text{或}\quad Q=(t,-1).
\]
所以 \(Q\) 的轨迹是两条直线 \(y=1\) 与 \(y=-1\)，选 B.
\end{solution}
...
\begin{solution}
因 \(x>0\)，令 \(u=x^6\)，则 \(u>0\)，且 \(x=u^{1/6}\). 因而
\[
f(u)=\log_2u^{1/6}=\frac16\log_2u.
\]
取 \(u=8\)，得 \(f(8)=\frac16\log_2 8=\frac12\)，所以选 D.
\end{solution}
...
\begin{solution}
因三角形 \(ABC\) 为锐角三角形，\(C<\frac\pi2\)，所以 \(A+B=\pi-C>\frac\pi2\). 因而 \(A>\frac\pi2-B\)，从而 \(\sin A>\cos B\)，点 \(P\) 的横坐标为负.

同理 \(B>\frac\pi2-A\)，所以 \(\sin B>\cos A\)，点 \(P\) 的纵坐标为正. 因此 \(P\) 在第二象限，选 B.
\end{solution}
...
\begin{solution}
设圆锥底面半径为 \(r\)，母线长为 \(l\). 侧面展开图为半径 \(l\) 的半圆，所以其弧长为 \(\pi l\)，这等于圆锥底面周长 \(2\pi r\). 因此 \(l=2r\).

设圆锥轴截面顶角为 \(\varphi\). 在轴截面的一半中，\(\sin\frac\varphi2=\frac rl=\frac12\)，且 \(0<\frac\varphi2<\frac\pi2\)，所以 \(\frac\varphi2=30^\circ\)，即 \(\varphi=60^\circ\). 因此选 C.
\end{solution}
...
\]
当且仅当 \(3^a=3^b\)，即 \(a=b=1\) 时取等号. 所以最小值为 \(6\)，选 B.
\end{solution}
...
\[
\overrightarrow{BM}=(0,1,1),\quad \overrightarrow{ED}=(0,1,-1),
\]
...
此外，
\[
\overrightarrow{CN}\cdot\overrightarrow{BM}=1,\qquad |CN|=|BM|=\sqrt2,
\]
故 \(CN\) 与 \(BM\) 所成的角为 \(60^\circ\)，命题\(\circled{3}\)正确. 最后
\[
\overrightarrow{DM}=(1,0,1),\qquad \overrightarrow{BN}=(-1,1,1),
\]
...
由 \(B\)，\(C\) 在椭圆上，分别得到
\[
u^2+4v^2+4u=0,\qquad 4u^2+v^2-4v=0.
\]
...
命题\(\circled{4}\)成立. 直线 \(n\) 平行于平面 \(\alpha\cap\beta\) 的直线 \(m\)，且 \(n\) 不在 \(\alpha\)，所以 \(n\parallel\alpha\)；同理 \(n\parallel\beta\). 因此正确命题为 \(\circled{2}\)，\(\circled{4}\).
\end{solution}
...
\]
又 \(VC\perp AB\)，所以 \(VC\) 垂直于平面 \(AMB\) 内相交的两条直线 \(AB\)，\(DM\)，即 \(VC\perp\) 平面 \(AMB\).

令 \(L=VC=\sqrt{r^2+v^2}\). 由 \(\angle CVN=\theta\)，有 \(r=L\sin\theta\)，\(v=L\cos\theta\). 在垂直于 \(AB\) 的截面内，直线 \(VC\) 到 \(AB\) 的距离为
\[
...
\[
a_k=q^k=2^{\frac{k}{n+1}}\quad(k=1,2,\ldots,n).
\]
因此
\[
...
\[
b_k=1+\frac{k}{n+1}\quad(k=1,2,\ldots,n),
\]
从而
\[
...
\[
y^2=2p(y+a),\qquad y^2-2py-2pa=0.
\]
...
\[
\overrightarrow{QM}=(-p,-p),\qquad \overrightarrow{QN}=(p,-p),
\]
