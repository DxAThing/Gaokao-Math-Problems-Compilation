...
\begin{center}
\bitmapinclude[max width=4.8cm,max totalheight=4.8cm]{img/2019/tianjin_liberal/q2_fig1.png}
\end{center}
...
\begin{solution}
\(|x-1|<1\) 等价于 \(0<x<2\). 故 \(0<x<5\) 推不出 \(|x-1|<1\)；

由 \(|x-1|<1\) 能推出 \(0<x<5\).

故“\(0<x<5\)”是“\(|x-1|<1\)”的必要而不充分条件. 故选 B.
\end{solution}
\begin{problem}
阅读右边的程序框图，运行相应的程序，输出 \(S\) 的值为（\quad）

\begin{center}
\bitmapinclude[max width=3.8cm,max totalheight=4.8cm]{img/2019/tianjin_liberal/q4_fig1.png}
\end{center}
...
\begin{solution}
按程序框图逐步计算：\(S=0\)，\(i=1\) 时 \(S=1\)；\(i=2\) 时 \(j=1\)，\(S=1+2\times 2=5\)；\(i=3\) 时 \(S=8\)；\(i=4\) 后结束循环，故输出 \(8\). 故选 B.
\end{solution}
\begin{problem}
...
\begin{solution}
因为 \(f(x)\) 是奇函数，所以 \(\varphi=0\)，从而 \(f(x)=A\sin(\omega x)\). 将横坐标伸长到原来的 2 倍后，\(g(x)=A\sin\!\left(\dfrac{\omega x}{2}\right)\). 由 \(g(x)\) 的最小正周期为 \(2\pi\)，得 \(\omega=2\). 又 \(g\!\left(\dfrac{\pi}{4}\right)=A\sin\!\left(\dfrac{\pi}{4}\right)=\sqrt2\)，所以 \(A=2\). 因此 \(f\!\left(\dfrac{3\pi}{8}\right)=2\sin\!\left(\dfrac{3\pi}{4}\right)=2\). 故选 C.
\end{solution}
\begin{problem}
...
\end{solution}
\begin{problem}
设 \(x>0\)，\(y>0\)，\(x+2y=4\)，则 \(\dfrac{(x+1)(2y+1)}{xy}\) 的最小值为 \fillinblank{}.
...
\begin{solution}
\(\dfrac{(x+1)(2y+1)}{xy}=\dfrac{2xy+x+2y+1}{xy}=\dfrac{2xy+5}{xy}=2+\dfrac5{xy}\). 由 \(x+2y=4\) 可得 \(xy=x(4-x)/2\le 2\)，当 \(x=2\)，\(y=1\) 时取等号. 故所求最小值为 \(2+\dfrac52=\dfrac92\).
\end{solution}
\begin{problem}
...
\begin{solution}
（1）由已知，老、中、青员工人数之比为 \(72:108:120=6:9:10\). 又样本容量为 \(25\)，所以应从老、中、青员工中分别抽取 \(6\) 人、\(9\) 人、\(10\) 人.

（2）（i）从 \(A,B,C,D,E,F\) 中随机抽取 \(2\) 人的所有结果为
\examdisplayaligned{
...
\begin{solution}
（1）由正弦定理，\(\dfrac{b}{\sin B}=\dfrac{c}{\sin C}\)，故 \(b\sin C=c\sin B\). 又由 \(3c\sin B=4a\sin C\)，得 \(3b=4a\). 再由 \(b+c=2a\)，可得 \(b=\dfrac{4a}{3}\)、\(c=\dfrac{2a}{3}\). 由余弦定理可得 \(\cos B=\dfrac{a^2+c^2-b^2}{2ac}=-\dfrac14\).

（2）由（1）知 \(\sin B=\sqrt{1-\cos^2 B}=\dfrac{\sqrt{15}}{4}\)，从而 \(\sin 2B=2\sin B\cos B=-\dfrac{\sqrt{15}}{8}\)、\(\cos 2B=\cos^2 B-\sin^2 B=-\dfrac78\). 于是 \(\sin\left(2B+\dfrac{\pi}{6}\right)=\sin2B\cos\dfrac{\pi}{6}+\cos2B\sin\dfrac{\pi}{6}=-\dfrac{\sqrt{15}+7}{16}\).
\end{solution}
\begin{problem}
如图，在四棱锥 \(P-ABCD\) 中，底面 \(ABCD\) 为平行四边形，\(\triangle PCD\) 为等边三角形，平面 \(PAC\perp\) 平面 \(PCD\)，\(PA\perp CD\)，\(CD=2\)，\(AD=3\).

\begin{center}
\bitmapinclude[max width=8.5cm,max totalheight=4.8cm]{img/2019/tianjin_liberal/q17_fig1.png}
\end{center}
...
\begin{solution}
（1）连接 \(BD\). 因为 \(ABCD\) 是平行四边形，所以 \(AC\) 与 \(BD\) 互相平分，设交点为 \(H\). 又 \(G\) 为 \(PB\) 的中点，所以在三角形 \(PBD\) 中，\(GH\) 是中位线，故 \(GH//PD\). 而 \(PD\subset\) 平面 \(PAD\)，且 \(GH\) 不在平面 \(PAD\) 内，所以 \(GH//\) 平面 \(PAD\).

\bitmapfigure[max width=8.5cm,max totalheight=4.8cm]{img/2019/tianjin_liberal/q17_solution_fig1.png}
...
（3）连接 \(AN\). 由（2）知 \(DN\perp\) 平面 \(PAC\)，所以 \(\angle DAN\) 是直线 \(AD\) 与平面 \(PAC\) 所成的角. 因为 \(\triangle PCD\) 为等边三角形，\(CD=2\)，且 \(N\) 为 \(PC\) 的中点，所以 \(DN=\sqrt3\). 又 \(AD=3\)，故
...
\begin{solution}
（1）设等差数列 \(\{a_n\}\) 的公差为 \(d\)，等比数列 \(\{b_n\}\) 的公比为 \(q\). 由 \(a_1=b_1=3\)、\(b_2=a_3\)、\(b_3=4a_2+3\)，得 \(3q=3+2d\)、\(3q^2=15+4d\). 解得 \(d=3\)，\(q=3\). 所以 \(a_n=3+3(n-1)=3n\)、\(b_n=3\cdot 3^{n-1}=3^n\).
...
\end{solution}
\begin{problem}
设椭圆 \(\dfrac{x^2}{a^2}+\dfrac{y^2}{b^2}=1\)（\(a>b>0\)） 的左焦点为 \(F\)，左顶点为 \(A\)，顶点为 \(B\). 已知 \(\sqrt3|OA|=2|OB|\)（\(O\) 为原点）.
...
\begin{solution}
（1）\(f(x)\) 的定义域为 \((0,+\infty)\)，且 \(f'(x)=\dfrac1x-a x\e^x\). 当 \(a\le0\) 时，\(f'(x)>0\)，故 \(f(x)\) 在 \((0,+\infty)\) 内单调递增.

（2）（i）令 \(g(x)=1-a x^2\e^x\). 则 \(f'(x)=\dfrac{g(x)}{x}\). 由于 \(0<a<\dfrac1\e\)，故 \(g(x)\) 在 \((0,+\infty)\) 内单调递减，且 \(g(1)=1-a\e>0\)、\(g\!\left(\ln\dfrac1a\right)=1-a\left(\ln\dfrac1a\right)^2\cdot\dfrac1a<0\). 所以 \(g(x)=0\) 在 \((0,+\infty)\) 内有唯一解，不妨记为 \(x_0\)，且 \(1<x_0<\ln\dfrac1a\). 于是 \(f(x)\) 在 \((0,x_0)\) 内单调递增，在 \((x_0,+\infty)\) 内单调递减，因此 \(x_0\) 是唯一极值点.

又令 \(h(x)=\ln x-x+1\)，则当 \(x>1\) 时，\(h'(x)=\dfrac1x-1<0\)，故 \(h(x)\) 在 \((1,+\infty)\) 内单调递减，并且 \(h(1)=0\). 从而当 \(x>1\) 时，\(\ln x<x-1\).

又 \(f\!\left(\ln\dfrac1a\right)=\ln\ln\dfrac1a-a\left(\ln\dfrac1a-1\right)\cdot\dfrac1a=\ln\ln\dfrac1a-\ln\dfrac1a+1=h\!\left(\ln\dfrac1a\right)<0\).
因为 \(f(x_0)>f(1)=0\)，所以 \(f(x)\) 在 \((x_0,+\infty)\) 内有唯一零点.

又 \(1<x_0\)，且 \(f(1)=0\)，所以 \(f(x)\) 在 \((0,x_0)\) 内有唯一零点 \(1\). 因此 \(f(x)\) 恰有两个零点.

（2）（ii）由题意，\(f'(x_0)=0\)、\(f(x_1)=0\). 即 \(\dfrac1{x_0}=a x_0\e^{x_0}\)、\(\ln x_1=a(x_1-1)\e^{x_1}\)，从而 \(\ln x_1=\dfrac{x_1-1}{x_0^2}\e^{x_1-x_0}\). 由 \(x_1>x_0>1\) 可得 \(\ln x_1<x_1-1\)，从而 \(\e^{x_1-x_0}<x_0^2\). 两边取对数得 \(x_1-x_0<2\ln x_0<2(x_0-1)\)，于是 \(3x_0-x_1>2\).
\end{solution}
