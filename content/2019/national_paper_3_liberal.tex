...
\begin{center}
\bitmapinclude[max width=0.42\linewidth,max totalheight=4.8cm]{img/2019/national_paper_3_liberal/q8_fig1.png}
\end{center}
...
\begin{center}
\bitmapinclude[max width=0.33\linewidth,max totalheight=4.8cm]{img/2019/national_paper_3_liberal/q9_fig1.png}
\end{center}
...
\begin{center}
\bitmapinclude[max width=0.42\linewidth,max totalheight=4.8cm]{img/2019/national_paper_3_liberal/q10_fig1.png}
\end{center}
...
\end{solution}
\begin{problem}
记不等式组 \(x+y\ge6\)，\(2x-y>0\) 表示的平面区域为 \(D\). 命题 \(p\)：\(\exists (x,y)\in D\)，\(2x+y\ge9\)；命题 \(q\)：\(\forall (x,y)\in D\)，\(2x+y\le12\). 下面给出了四个命题 \(\circled{1}p\vee q\)；\(\circled{2}\neg p\vee q\)；\(\circled{3}p\wedge\neg q\)；\(\circled{4}\neg p\wedge\neg q\). 这四个命题中，所有真命题的编号是（\quad）
...
\begin{solution}
由区域 \(D\) 可判断命题 \(p\) 为真，命题 \(q\) 为假，所以 \(\neg p\) 为假，\(\neg q\) 为真. 于是 \(\circled{1}\) 与 \(\circled{3}\) 为真，其余为假. 故选 \(A\).
\end{solution}
\begin{problem}
...
\begin{problem}
已知向量 \(\bs a=(2,2)\)，\(\bs b=(-8,6)\)，则 \(\cos\langle\bs a,\bs b\rangle=\)
...
\end{solution}
\begin{problem}
记 \(S_n\) 为等差数列 \(\{a_n\}\) 的前 \(n\) 项和. 若 \(a_3=5\)，\(a_7=13\)，则 \(S_{10}=\)
...
\end{solution}
\begin{problem}
学生到工厂劳动实践，利用 \(3D\) 打印技术制作模型. 如图，该模型为长方体 \(ABCD-A_1B_1C_1D_1\) 挖去四棱锥 \(O-EFGH\) 后所得的几何体，其中 \(O\) 为长方体的中心，\(E,F,G,H\) 分别为所在棱的中点，\(AB=BC=6\text{ cm}\)，\(AA_1=4\text{ cm}\). \(3D\) 打印所用原料密度为 \(0.9\text{ g/cm}^3\). 不考虑打印损耗，制作该模型所需原料的质量为

\begin{center}
\bitmapinclude[max width=0.42\linewidth,max totalheight=4.8cm]{img/2019/national_paper_3_liberal/q16_fig1.png}
\end{center}
...
\begin{solution}
该模型为长方体 \(ABCD-A_1B_1C_1D_1\) 挖去四棱锥 \(O-EFGH\) 后所得的几何体，其中 \(O\) 为长方体的中心，\(E,F,G,H\) 分别为所在棱的中点，\(AB=BC=6\text{ cm}\)，\(AA_1=4\text{ cm}\).

因此该模型的体积为 \(V_{ABCD-A_1B_1C_1D_1}-V_{O-EFGH}=6\times6\times4-\dfrac13\left(4\times6-4\times\dfrac12\times3\times2\right)\times3=144-12=132\ (\text{cm}^3)\). 因为 \(3D\) 打印所用原料密度为 \(0.9\text{ g/cm}^3\)，不考虑打印损耗，所以制作该模型所需原料的质量为 \(132\times0.9=118.8\text{ g}\).
\end{solution}
\section{解答题}
...
\begin{center}
\bitmapinclude[max width=0.75\linewidth,max totalheight=4.8cm]{img/2019/national_paper_3_liberal/q17_fig1.png}
\end{center}
\end{problem}

\begin{solution}
（1）\(C\) 为事件：“乙离子残留在体内的百分比不低于 \(5.5\)”，根据直方图得到 \(P(C)\) 的估计值为 \(0.70\).
...
（2）估计甲离子残留百分比的平均值为 \(2\times0.15+3\times0.20+4\times0.30+5\times0.20+6\times0.10+7\times0.05=4.05\). 乙离子残留百分比的平均值为 \(3\times0.05+4\times0.10+5\times0.15+6\times0.35+7\times0.20+8\times0.15=6.00\).
\end{solution}
\begin{problem}
...
\begin{solution}
（1）由 \(A+B+C=\pi\)，可得 \(\dfrac{A+C}{2}=\dfrac{\pi-B}{2}\). 因此 \(a\sin\dfrac{A+C}{2}=b\sin A\)，即 \(a\sin\dfrac{\pi-B}{2}=a\cos\dfrac B2=b\sin A\). 由正弦定理，\(a=2R\sin A\)，\(b=2R\sin B\)，所以 \(\sin A\cos\dfrac B2=\sin B\sin A=2\sin\dfrac B2\cos\dfrac B2\sin A\). 因为 \(\sin A>0\)，所以 \(\cos\dfrac B2=2\sin\dfrac B2\cos\dfrac B2\). 若 \(\cos\dfrac B2=0\)，则 \(B=(2k+1)\pi\)（\(k\in\Z\)）不成立；因此 \(\sin\dfrac B2=\dfrac12\). 又 \(0<B<\pi\)，故 \(B=\dfrac{\pi}{3}\).

（2）若 \(\triangle ABC\) 为锐角三角形，且 \(c=1\)，由余弦定理可得 \(b=\sqrt{a^2+1-2a\cos\dfrac{\pi}{3}}=\sqrt{a^2-a+1}\). 由 \(\triangle ABC\) 为锐角三角形，可得 \(a^2+a^2-a+1>1\)，\(1+a^2-a+1>a^2\)，解得 \(\dfrac12<a<2\). 于是 \(S_{\triangle ABC}=\dfrac12 a\cdot1\cdot\sin\dfrac{\pi}{3}=\dfrac{\sqrt3}{4}a\)，故 \(S_{\triangle ABC}\in\left(\dfrac{\sqrt3}{8},\dfrac{\sqrt3}{2}\right)\).
\end{solution}
\begin{problem}
...
\begin{center}
\bitmapinclude[max width=0.85\linewidth,max totalheight=4.8cm]{img/2019/national_paper_3_liberal/q19_fig1.png}
\end{center}
\end{problem}
...
又 \(BC\cap BE=E\)，所以 \(AB\perp\) 平面 \(BCGE\).
...
（2）当 \(0<a<3\) 时，由（1）知，\(f(x)\) 在 \(\left(0,\dfrac a3\right)\) 上单调递减，在 \(\left(\dfrac a3,1\right)\) 上单调递增.

因此 \(f(x)\) 在区间 \([0,1]\) 的最小值为 \(f\!\left(\frac a3\right)=2-\frac{a^3}{27}\)，最大值为 \(f(0)=2\) 或 \(f(1)=4-a\).

于是 \(m=2-\dfrac{a^3}{27},\qquad M=\begin{cases}
4-a, & 0<a<2,\\
2, & 2\le a<3.
\end{cases}\).
...
\begin{solution}
（1）证明：设 \(D(t,-\dfrac12)\)，\(A(x_1,y_1)\)，则 \(x_1^2=2y_1\). 由于 \(y'=x\)，所以切线 \(DA\) 的斜率为 \(x_1\)，故 \(\dfrac{y_1+\frac12}{x_1-t}=x_1\)，整理得 \(2tx_1-2y_1+1=0\). 设 \(B(x_2,y_2)\)，则 \(x_2^2=2y_2\). 由于切线 \(DB\) 的斜率为 \(x_2\)，所以 \(\dfrac{y_2+\frac12}{x_2-t}=x_2\)，整理得 \(2tx_2-2y_2+1=0\). 因此直线 \(AB\) 的方程为 \(2tx-2y+1=0\). 所以直线 \(AB\) 过定点 \(\left(0,\frac12\right)\).

（2）由（1）得直线 \(AB\) 的方程为 \(y=tx+\frac12\). 由 \(\begin{cases}
y=tx+\dfrac12,\\
y=\dfrac{x^2}{2},
...
\begin{center}
\bitmapinclude[max width=0.55\linewidth,max totalheight=4.8cm]{img/2019/national_paper_3_liberal/q22_fig1.png}
\end{center}
\end{problem}

\begin{solution}
（1）由题设可得，弧 \(\widehat{AB}\)，\(\widehat{BC}\)，\(\widehat{CD}\) 所在圆的极坐标方程分别为 \(\rho=2\cos\theta\)，\(\rho=2\sin\theta\)，\(\rho=-2\cos\theta\). 因此 \(M_1:\rho=2\cos\theta\)（\(0\le\theta\le\dfrac{\pi}{4}\)），\(M_2:\rho=2\sin\theta\)（\(\dfrac{\pi}{4}\le\theta\le\dfrac{3\pi}{4}\)），\(M_3:\rho=-2\cos\theta\)（\(\dfrac{3\pi}{4}\le\theta\le\pi\)）.
...
\begin{solution}
（1）由 \(x,y,z\in\R\)，且 \(x+y+z=1\)，应用柯西不等式可得 \((1^2+1^2+1^2)\bigl[(x-1)^2+(y+1)^2+(z+1)^2\bigr]\ge(x-1+y+1+z+1)^2=4\)，因此 \((x-1)^2+(y+1)^2+(z+1)^2\ge\dfrac43\). 所以最小值为 \(\dfrac43\).

（2）由 \(x+y+z=1\)，应用柯西不等式可得 \((1^2+1^2+1^2)\bigl[(x-2)^2+(y-1)^2+(z-a)^2\bigr]\ge(x-2+y-1+z-a)^2=(a+2)^2\)，因此 \((x-2)^2+(y-1)^2+(z-a)^2\ge\dfrac{(a+2)^2}{3}\).
