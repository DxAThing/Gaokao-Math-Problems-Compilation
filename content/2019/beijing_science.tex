...
\begin{problem}
设等差数列 \(\{a_n\}\) 的前 \(n\) 项和为 \(S_n\). 若 \(a_2=-3\)，\(S_5=-10\)，则 \(a_5=\fillinblank{}\)，\(S_n\) 的最小值为\fillinblank{}.
\end{problem}
...
\begin{enumerate}
\item \(CD\perp\) 平面 \(PAD\).
\item 二面角 \(F\)-\(AE\)-\(P\) 的余弦值为 \(\dfrac{\sqrt3}{3}\).
\item 直线 \(AG\) 在平面 \(AEF\) 内.
\end{enumerate}
\end{solution}
...
\begin{enumerate}
\item 估计该概率为 \(0.4\).
\item \(P(X=0)=0.24\)，\(P(X=1)=0.52\)，\(P(X=2)=0.24\)，\(E(X)=1\).
...
\begin{solution}
\begin{enumerate}
\item 抛物线 \(C\) 的方程为 \(x^2=-4y\)，准线方程为 \(y=1\).
\item 以 \(AB\) 为直径的圆经过 \(y\) 轴上的两个定点 \((0,1)\) 和 \((0,-3)\).
\end{enumerate}
\end{solution}
...
\item 当 \(x\in[-2,4]\) 时，\(x-6\le f(x)\le x\).
\item 当 \(M(a)\) 最小时，\(a=-3\).
\end{enumerate}
\end{solution}
