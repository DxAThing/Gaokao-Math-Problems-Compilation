...
\(E=(2,0,0)\)，\(F=\left(\frac72,\frac{\sqrt3}{2},2\sqrt3\right)\).
于是
\(\overrightarrow{EF}=\left(\frac32,\frac{\sqrt3}{2},2\sqrt3\right)\)，\(\overrightarrow{BC}=(1,-\sqrt3,0)\)
且 \(\overrightarrow{EF}\cdot\overrightarrow{BC}=\dfrac32-\dfrac32=0\). 因此 \(EF\perp BC\).

\item 取平面 \(A_1BC\) 内的两个相交向量
\(u=\overrightarrow{BC}=(1,-\sqrt3,0)\)，\(v=\overrightarrow{BA_1}=(-1,-\sqrt3,2\sqrt3)\).
其法向量可取
\[
...
\]
所以 \(a_1=0\)，进而 \(d=2\). 因此
\(a_n=2n-2\)，\(S_n=\frac n2\left(a_1+a_n\right)=n(n-1)\).

令 \(X_n=S_n+b_n\). 由于
\(S_{n+1}-S_n=a_{n+1}=2n\)，\(S_{n+2}-S_n=a_{n+1}+a_{n+2}=4n+2\)
题设三个数成等比数列等价于
\[
...
\]
记其横坐标为 \(g\). 直接计算得
\(g-1=\frac{(2s-1)(s-2)}{3s}>0\)，\(q-g=\frac{(s-2)(s+1)}{3s}>0\).
又因为 \(w<0\)，所以
\(S_1=\frac12(g-1)\cdot2u=u(g-1)\)，\(S_2=\frac12(q-g)\cdot(-2w)=(-w)(q-g)\).
利用 \(-w=\dfrac{s-1}{u}\)，得
\(\frac{S_1}{S_2}=\frac{u(g-1)}{\dfrac{s-1}{u}(q-g)}=\frac{s(2s-1)}{(s-1)(s+1)}=:R(s)\)，\(s>2\).
于是
\[
...
\(D_a(x)=\frac{\sqrt{x}}{2a}-a\ln x-\sqrt{x+1}\)，\(a_0=\frac{1}{2\sqrt2}\).
