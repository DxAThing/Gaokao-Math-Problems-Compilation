...
\begin{solution}
第二天的新订单不超过 \(1600\) 份的概率为 \(0.95\). 要使第二天完成积压订单及当日订单的配货的概率不小于 \(0.95\)，只需保证在新订单为 \(1600\) 份时仍能完成配货.

此时共需完成 \(500+1600=2100\) 份订单，超市本身能完成 \(1200\) 份，还需完成 \(2100-1200=900\) 份. 因为每名志愿者每天能完成 \(50\) 份订单，所以至少需要 \(\frac{900}{50}=18\) 名志愿者. 故选 \(B\).
\end{solution}
\begin{problem}
...
\begin{solution}
设每层有 \(n\) 环. 由题意可知，从内到外每环石板数构成公差为 \(9\) 的等差数列，且首项为 \(9\).

记前三层对应等差数列前 \(n\) 项和分别为 \(S_n\)、\(S_{2n}-S_n\)、\(S_{3n}-S_{2n}\). 因为下层比中层多 \(729\) 块，所以 \(\left( S_{3n}-S_{2n} \right)-\left( S_{2n}-S_n \right)=729\). 由等差数列性质可得 \(\left( S_{3n}-S_{2n} \right)-\left( S_{2n}-S_n \right)=n^2\cdot 9\). 于是 \(9n^2=729\)，解得 \(n=9\).

所以三层共有扇面形石板 \(S_{27}=27\times 9+\frac{27\times 26}{2}\times 9=3402\). 故选 \(C\).
\end{solution}
\begin{problem}
...
\begin{solution}
由 \(a_1=2\) 且 \(a_{m+n}=a_m a_n\)，取 \(m=1\)，得 \(a_{n+1}=a_1a_n=2a_n\). 所以数列 \(\{a_n\}\) 是首项为 \(2\)、公比为 \(2\) 的等比数列，即 \(a_n=2^n\).

于是 \(a_{k+1}+a_{k+2}+\cdots+a_{k+10}=2^{k+1}+2^{k+2}+\cdots+2^{k+10}=2^{k+1}\left( 2^{10}-1 \right)\). 而 \(2^{15}-2^5=2^5\left( 2^{10}-1 \right)\). 所以 \(k+1=5\)，得 \(k=4\). 故选 \(C\).
\end{solution}
\begin{problem}
如图是一个多面体的三视图，这个多面体某条棱的一个端点在正视图中对应的点为 \(M\)，在俯视图中对应的点为 \(N\)，则该端点在侧视图中对应的点为（\quad）

\begin{center}
\bitmapinclude[max width=3.5cm,max totalheight=2.4cm]{img/2020/national_paper_2_science/q7_fig1.png}
\end{center}
...
\begin{solution}
双曲线 \(C\) 的渐近线方程为 \(y=\pm \frac{b}{a}x\). 直线 \(x=a\) 与两条渐近线交于 \(D(a,b)\)、\(E(a,-b)\). 所以 \(S_{\triangle ODE}=\frac{1}{2}\cdot 2b\cdot a=ab=8\).

双曲线的焦距为 \(2\sqrt{a^2+b^2}\). 由 \(a^2+b^2\ge 2ab=16\) 可得 \(2\sqrt{a^2+b^2}\ge 2\sqrt{16}=8\). 当 \(a=b\) 时取等号，所以焦距的最小值为 \(8\). 故选 \(B\).
\end{solution}
\begin{problem}
...
\begin{solution}
因为 \(f(-x)=\ln |1-2x|-\ln |-(2x+1)|=\ln |2x-1|-\ln |2x+1|=-f(x)\)，所以 \(f(x)\) 是奇函数.

又 \(f'(x)=\frac{2}{2x+1}-\frac{2}{2x-1}=-\frac{4}{4x^2-1}\). 当 \(x\in\left( -\infty,-\dfrac{1}{2} \right)\) 时，\(4x^2-1>0\)，故 \(f'(x)<0\)，所以 \(f(x)\) 在 \(\left( -\infty,-\dfrac{1}{2} \right)\) 上单调递减. 故选 \(D\).
\end{solution}
\begin{problem}
...
\begin{solution}
设等边三角形边长为 \(s\)，由面积公式得 \(\frac{\sqrt3}{4}s^2=\frac{9\sqrt3}{4}\)，解得 \(s=3\).

所以 \(\triangle ABC\) 的外接圆半径为 \(R_1=\frac{s}{\sqrt3}=\sqrt3\).

又球 \(O\) 的表面积为 \(16\pi\)，故球半径 \(R\) 满足 \(4\pi R^2=16\pi\)，解得 \(R=2\).

因此球心 \(O\) 到平面 \(ABC\) 的距离为 \(\sqrt{R^2-R_1^2}=\sqrt{4-3}=1\). 故选 \(C\).
\end{solution}
\begin{problem}
...
\begin{solution}
由 \(2^x-2^y<3^{-x}-3^{-y}\) 可得 \(2^x-3^{-x}<2^y-3^{-y}\). 令 \(f(x)=2^x-3^{-x}\)，则 \(f'(x)=2^x\ln 2+3^{-x}\ln 3>0\). 所以 \(f(x)\) 在 \(\R\) 上单调递增.

由 \(f(x)<f(y)\) 得 \(x<y\)，即 \(y-x>0\)，于是 \(y-x+1>1\). 故 \(\ln \left( y-x+1 \right)>\ln 1=0\). 故选 \(A\).
\end{solution}
\begin{problem}
...
\begin{solution}
因为 \(\bs{a},\bs{b}\) 为单位向量，且 \(\bs{a},\bs{b}\) 的夹角为 \(45^\circ\)，所以 \(\bs{a}\cdot\bs{b}=|\bs{a}|\cdot|\bs{b}|\cos 45^\circ=1\times 1\times \frac{\sqrt2}{2}=\frac{\sqrt2}{2}\).

又因为 \(k\bs{a}-\bs{b}\) 与 \(\bs{a}\) 垂直，所以 \(\left( k\bs{a}-\bs{b} \right)\cdot \bs{a}=k|\bs{a}|^2-\bs{a}\cdot\bs{b}=0\). 即 \(k-\frac{\sqrt2}{2}=0\)，所以 \(k=\frac{\sqrt2}{2}\). 故答案为 \(\dfrac{\sqrt2}{2}\).
\end{solution}
\begin{problem}
...
\begin{solution}
因为 \(|z_1+z_2|=|\sqrt3+\i|=2\)，所以 \(|z_1+z_2|^2=\left( z_1+z_2 \right)\overline{\left( z_1+z_2 \right)}=4\). 即 \(|z_1|^2+|z_2|^2+z_1\overline{z_2}+\overline{z_1}z_2=4\). 由 \(|z_1|=|z_2|=2\) 得 \(8+z_1\overline{z_2}+\overline{z_1}z_2=4\)，所以 \(z_1\overline{z_2}+\overline{z_1}z_2=-4\).

于是 \(|z_1-z_2|^2=\left( z_1-z_2 \right)\overline{\left( z_1-z_2 \right)}=|z_1|^2+|z_2|^2-\left( z_1\overline{z_2}+\overline{z_1}z_2 \right)=12\). 又 \(|z_1-z_2|>0\)，故 \(|z_1-z_2|=2\sqrt3\). 故答案为 \(2\sqrt3\).
\end{solution}
\begin{problem}
设有下列四个命题：
...
由正弦定理可得 \(a^2-b^2-c^2=bc\)，即 \(b^2+c^2-a^2=-bc\).
由余弦定理可得
...
又 \(B+C=\frac{\pi}{3}\)，可设
\(B=\frac{\pi}{6}-d\)，\(C=\frac{\pi}{6}+d\)，\(-\frac{\pi}{6}<d<\frac{\pi}{6}\).

由正弦定理可得 \(\frac{3}{\sin \frac{2\pi}{3}}=\frac{b}{\sin B}=\frac{c}{\sin C}=2\sqrt3\)，所以
\(b=2\sqrt3\sin \left( \frac{\pi}{6}-d \right)\)，\(c=2\sqrt3\sin \left( \frac{\pi}{6}+d \right)\).
...
（2）由已知，
\(\sum_{i=1}^{20}\left( x_i-\bar{x} \right)^2=80\)，\(\sum_{i=1}^{20}\left( y_i-\bar{y} \right)^2=9000\)，\(\sum_{i=1}^{20}\left( x_i-\bar{x} \right)\left( y_i-\bar{y} \right)=800\).
所以
\(r=\frac{800}{\sqrt{80\times 9000}}=\frac{800}{600\sqrt2}=\frac{2\sqrt2}{3}\approx 0.94\).
...
\begin{solution}
（1）因为 \(F\) 为 \(C_1\) 的焦点，且 \(AB\perp x\) 轴，所以
\(F(c,0)\)，\(|AB|=\frac{2b^2}{a}\).

设 \(C_2\) 的标准方程为
\(y^2=2px\)（\(p>0\)）.
因为 \(F\) 为 \(C_2\) 的焦点，且 \(CD\perp x\) 轴，所以
\(F\left( \frac{p}{2},0 \right)\)，\(|CD|=2p\).

由 \(|CD|=\dfrac{4}{3}|AB|\)，且 \(C_1,C_2\) 的焦点重合，得
\(\begin{cases}
c=\dfrac{p}{2},\\[4pt]
...
又因为
\(b^2=a^2-c^2\)，所以 \(3ac=2a^2-2c^2\).

设 \(C_1\) 的离心率为 \(e\)，则 \(e=\dfrac{c}{a}\). 上式化为
\(2e^2+3e-2=0\).
解得 \(e=\frac{1}{2}\)（\(-2\text{ 舍去}\)）.
故 \(C_1\) 的离心率为 \(\dfrac{1}{2}\).

（2）由（1）可得
\(a=2c\)，\(b=\sqrt3\,c\)，\(p=2c\).
所以
\(C_1:\frac{x^2}{4c^2}+\frac{y^2}{3c^2}=1\)，\(C_2:y^2=4cx\).

联立两曲线方程，消去 \(y\)，可得
\(3x^2+16cx-12c^2=0\).
所以
\(\left( 3x-2c \right)\left( x+6c \right)=0\).
解得
\(x=\frac{2}{3}c\) 或 \(x=-6c\). 后者舍去，所以
\(x=\frac{2}{3}c\).

由抛物线的定义，
 \(|MF|=x+\frac{p}{2}=\frac{2}{3}c+c=\frac{5}{3}c=5\).
解得
\(c=3\).

所以
\(C_1:\frac{x^2}{36}+\frac{y^2}{27}=1\)，\(C_2:y^2=12x\).
\end{solution}
\begin{problem}
...
\begin{center}
\bitmapinclude[max width=4.2cm,max totalheight=3.4cm]{img/2020/national_paper_2_science/q20_fig1.png}
\end{center}
\end{problem}

\begin{solution}
（1）因为 \(M,N\) 分别为 \(BC,B_1C_1\) 的中点，且底面为正三角形，所以
\(B_1N=BM\)，
四边形 \(BB_1NM\) 为矩形，且
\(A_1N\perp B_1C_1\).
于是
...
又因为
\(MN\perp B_1C_1\)，\(A_1N\perp B_1C_1\)，\(MN\cap A_1N=N\)，
所以
\(B_1C_1\perp \text{平面 }A_1AMN\).
而
\(B_1C_1\subset \text{平面 }EB_1C_1F\)，
故
\(\text{平面 }A_1AMN\perp \text{平面 }EB_1C_1F\).

（2）因为三棱柱上下底面平行，平面 \(EB_1C_1F\) 与上下底面分别交于 \(B_1C_1,EF\)，所以
\(EF//B_1C_1//BC\).

又因为
\(AO//\text{平面 }EB_1C_1F\)，\(AO\subset \text{平面 }AMNA_1\)，\(\text{平面 }AMNA_1\cap \text{平面 }EB_1C_1F=PN\)，
所以
\(AO//PN\).
从而四边形 \(APNO\) 为平行四边形.

因为 \(O\) 是正三角形的中心，且 \(AO=AB\)，所以
\(A_1N=3ON\)，\(AM=3AP\)，\(PN=BC=B_1C_1=3AP=3EF\).

由（1）知直线 \(B_1E\) 在平面 \(A_1AMN\) 内的投影为 \(PN\)，因此直线 \(B_1E\) 与平面 \(A_1AMN\) 所成角即为等腰梯形 \(EFC_1B_1\) 中 \(B_1E\) 与 \(PN\) 所成角.
...
\(EH\perp B_1C_1\)
于 \(H\)，则
\(PN=B_1C_1=EH=3\)，\(B_1H=1\).
所以
\(B_1E=\sqrt{3^2+1^2}=\sqrt{10}\).

于是
\(\sin \angle B_1EH=\frac{B_1H}{B_1E}=\frac{1}{\sqrt{10}}=\frac{\sqrt{10}}{10}\).
故直线 \(B_1E\) 与平面 \(A_1AMN\) 所成角的正弦值为
\(\frac{\sqrt{10}}{10}\).
\end{solution}
\begin{problem}
...
当 \(x\in\left( 0,\frac{\pi}{3} \right)\) 或 \(x\in\left( \frac{2\pi}{3},\pi \right)\) 时，\(f'(x)>0\)；当
\(x\in\left( \frac{\pi}{3},\frac{2\pi}{3} \right)\)
时，\(f'(x)<0\).

因此 \(f(x)\) 在
\(\left( 0,\frac{\pi}{3} \right)\)，\(\left( \frac{2\pi}{3},\pi \right)\)
上单调递增，在
...
（2）因为
\(f(0)=f(\pi)=0\)，由（1）可知
\(f\left( \frac{\pi}{3} \right)=\frac{3\sqrt3}{8}\)，\(f\left( \frac{2\pi}{3} \right)=-\frac{3\sqrt3}{8}\).
所以 \(f(x)_{\max}=\frac{3\sqrt3}{8}\)，\(f(x)_{\min}=-\frac{3\sqrt3}{8}\).
又因为 \(f(x)\) 为周期函数，所以 \(|f(x)|\le \frac{3\sqrt3}{8}\).

（3）由（2）可知，对任意实数 \(t\)，都有
\(\left| \sin^2 t\sin 2t \right|\le \frac{3\sqrt3}{8}=\left( \frac{3}{4} \right)^{\frac{3}{2}}\).
令
\(a_k=\left| \sin 2^k x \right|\ (k=0,1,\cdots,n)\)，
则对 \(k=0,1,\cdots,n-1\)，都有
\(a_k^2a_{k+1}\le \left( \frac{3}{4} \right)^{\frac{3}{2}}\).
将上式两边同时取 \(\dfrac{2}{3}\) 次方，得 \(a_k^{\frac{4}{3}}a_{k+1}^{\frac{2}{3}}\le \frac{3}{4}\).

将上述 \(n\) 个不等式相乘，得
\(a_0^{\frac{4}{3}}a_1^2a_2^2\cdots a_{n-1}^2a_n^{\frac{2}{3}}\le \frac{3^n}{4^n}\).
又因为 \(0\le a_0\le 1\)，\(0\le a_n\le 1\)，所以
\(a_0^2a_1^2a_2^2\cdots a_{n-1}^2a_n^2\le a_0^{\frac{4}{3}}a_1^2a_2^2\cdots a_{n-1}^2a_n^{\frac{2}{3}}\).
因此
\(\sin^2 x\sin^2 2x\sin^2 4x\cdots \sin^2 2^n x=a_0^2a_1^2a_2^2\cdots a_{n-1}^2a_n^2\le \frac{3^n}{4^n}\).
\end{solution}
\section{选作题}
...
\begin{solution}
（1）对于曲线 \(C_1\)，因为
\(x=4\cos^2\theta\)，\(y=4\sin^2\theta\)，所以 \(x+y=4\)，\(0\le x\le 4\).
故 \(C_1\) 的普通方程为
\(x+y=4\)（且 \(0\le x\le 4\)）.

对于曲线 \(C_2\)，因为 \(x=t+\frac{1}{t}\)，\(y=t-\frac{1}{t}\)，所以
\(x^2-y^2=\left( t+\frac{1}{t} \right)^2-\left( t-\frac{1}{t} \right)^2=4\).
故 \(C_2\) 的普通方程为
\(x^2-y^2=4\).

（2）法一：由 \(x+y=4\) 和 \(\dfrac{x^2}{4}-\dfrac{y^2}{4}=1\) 解得 \(x=\frac{5}{2}\)，\(y=\frac{3}{2}\)，因此 \(P\left( \frac{5}{2},\frac{3}{2} \right)\).

设所求圆的圆心为 \((x_0,0)\)，则由圆经过极点和点 \(P\)，得
\(x_0^2=\left( x_0-\frac{5}{2} \right)^2+\frac{9}{4}\).
解得 \(x_0=\frac{17}{10}\).
故所求圆的极坐标方程为 \(\rho=\frac{17}{5}\cos\theta\).

法二：由 \(x+y=4\) 和 \(x^2-y^2=4\) 得 \(x-y=1\)，所以 \(x=\frac{5}{2}\)，\(y=\frac{3}{2}\)，即 \(P\left( \frac{5}{2},\frac{3}{2} \right)\).

设圆的方程为 \(\left( x-a \right)^2+y^2=r^2\).
由于圆经过点 \(P\) 和原点，所以 \(a^2=r^2\)，且 \(\left( \dfrac{5}{2}-a \right)^2+\left( \dfrac{3}{2} \right)^2=r^2\). 解得 \(a=\frac{17}{10}\)，\(r^2=\frac{289}{100}\). 因此圆的方程为 \(\left( x-\frac{17}{10} \right)^2+y^2=\frac{289}{100}\)，即 \(x^2+y^2-\frac{17}{5}x=0\).
化为极坐标方程为
...
当 \(x\le 3\) 时，不等式 \(f(x)\ge 4\) 化为
\(-2x+7\ge 4\).
解得
\(x\le \frac{3}{2}\).

当 \(3<x<4\) 时，不等式 \(f(x)\ge 4\) 化为
\(1\ge 4\)，
无解.

当 \(x\ge 4\) 时，不等式 \(f(x)\ge 4\) 化为
\(2x-7\ge 4\).
解得
\(x\ge \frac{11}{2}\).

所以当 \(a=2\) 时，不等式 \(f(x)\ge 4\) 的解集为
\(\left\{ x\mid x\le \frac{3}{2}\ \text{或}\ x\ge \frac{11}{2} \right\}\).

（2）由绝对值不等式的性质，
\(f(x)=|x-a^2|+|x-2a+1|\ge \left| x-a^2-\left( x-2a+1 \right) \right|=|(a-1)^2|=(a-1)^2\).
又因为 \(f(x)\ge 4\)，所以
\((a-1)^2\ge 4\).
解得
\(a-1\le -2\ \text{或}\ a-1\ge 2\)，
即
\(a\le -1\ \text{或}\ a\ge 3\).
