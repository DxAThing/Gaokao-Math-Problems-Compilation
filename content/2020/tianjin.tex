...
\begin{center}
\texfigure[max width=0.90\linewidth]{img_repaint/2020/tianjin/q4_fig1.tex}
\end{center}
...
\solutionfigure{\bitmapfigure[max width=0.50\linewidth]{img/2020/tianjin/q9_solution_fig1.png}}

当 \( k<0 \) 时，如图 2，此时 \( y=\left| kx-2 \right| \) 与 \( y=h(x) \) 恒有 3 个不同交点，满足题意；

\solutionfigure{\bitmapfigure[max width=0.50\linewidth]{img/2020/tianjin/q9_solution_fig3.png}}

当 \( k>0 \) 时，如图 3，当 \( y=kx-2 \) 与 \( y=x^2 \) 相切时，联立方程得
\(x^2-kx+2=0\)，
令 \( \Delta=0 \) 得 \( k^2-8=0 \)，解得 \( k=2\sqrt2 \)（负值舍去），所以 \( k>2\sqrt2 \).

\solutionfigure{\bitmapfigure[max width=0.50\linewidth]{img/2020/tianjin/q9_solution_fig2.png}}
...
\FigureLayoutDeclare{img/2020/tianjin/q15_solution_fig1.png}{7.3cm}{6bp 5bp 5bp 8bp}
\solutionfigure{\bitmapfigure[max width=0.66\linewidth]{img/2020/tianjin/q15_solution_fig1.png}}
...
\begin{solution}
\begin{enumerate}
\item 依题意，以 \(C\) 为原点，分别以 \( \overrightarrow{CA} \)，\( \overrightarrow{CB} \)，\( \overrightarrow{CC_1} \) 的方向为 \( x \) 轴，\( y \) 轴，\( z \) 轴的正方向建立空间直角坐标系（如图）：

\solutionfigure{\bitmapfigure[max width=0.60\linewidth]{img/2020/tianjin/q17_solution_fig1.png}}
...
\item 依题意， \( \overrightarrow{CA}=(2,0,0) \) 是平面 \(BB_1E\) 的一个法向量，
...
\begin{solution}
\begin{enumerate}
\item 因为椭圆 \( \dfrac{x^2}{a^2}+\dfrac{y^2}{b^2}=1 \)（\( a>b>0 \)）的一个顶点为 \( A(0,-3) \)，
所以 \( b=3 \).
由 \( OA=OF \)，得 \( c=b=3 \)，
又由 \( a^2=b^2+c^2 \)，得
\(a^2=3^2+3^2=18\)，
...
\item 因为直线 \(AB\) 与以 \(C\) 为圆心的圆相切于点 \(P\)，所以 \( CP\perp AB \).
...
\begin{solution}
\begin{enumerate}
\item 设等差数列 \( \{a_n\} \) 的公差为 \( d \)，等比数列 \( \{b_n\} \) 的公比为 \( q \).
