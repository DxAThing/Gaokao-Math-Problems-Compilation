...
\item 依题意，对任意 \(t\in(0,1)\)，\(f(x)\geq f(x+t)\) 恒成立，所以 \(f(x)=x+\dfrac{1}{x}\)（\(x\geq a\)）为增函数（不可能为常值函数）. 由双勾函数的图象及性质可得 \(a\geq1\)（因为 \(f(x)\) 在 \((1,+\infty)\) 上是增函数）. 当 \(a\geq1\) 时，函数单调递增，满足对任意 \(t\in(0,1)\)，\(f(x)\geq f(x+t)\) 恒成立，综上，实数 \(a\) 的取值范围为 \([1,+\infty)\).
\item 因为 \(D\) 为整数集，具有 \(A\) 性质的函数均为常值函数，所以当 \(t=-2\) 时，\(f(x)=f(x-2)\) 恒成立，周期为 \(2\). 设 \(f(2k)=p\) （\(k\in\Z\)），\(f(2n-1)=q\) （\(n\in\Z\)），由题意 \(p=q\)，则 \(f(2k)=f(2n-1)\). 当 \(x=2k\) 时，\(f(x)=f(x+2n-2k-1)\)，所以 \(m=2n-2k-1\) （\(n\)，\(k\in\Z\)）；当 \(x=2n-1\) 时，\(f(x)=f(x+2k-2n+1)\)，所以 \(m=2k-2n+1\) （\(n\)，\(k\in\Z\)）. 综上，\(m\) 为奇数.
\end{enumerate}
\end{solution}
