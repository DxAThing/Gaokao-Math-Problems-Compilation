...
集合 \(A=\{(x,y)\mid x,y\in \mathbf N^*,y\ge x\}\)，\(B=\{(x,y)\mid x+y=8\}\)，
\(A\cap B=\{(x,y)\mid \begin{cases}
y\ge x\\
x+y=8
\end{cases},\ x,y\in \mathbf N^*\}=\{(7,1),(6,2),(5,3),(4,4)\}\).
...
选项 A：\(E(x)=1\times0.1+2\times0.4+3\times0.4+4\times0.1=2.5\)，所以
...
向量 \(\bs{a}\)，\(\bs{b}\) 满足 \(|\bs{a}|=5\)，\(|\bs{b}|=6\)，\(\bs{a}\cdot\bs{b}=-6\)，可得
...
在 \(\triangle ABC\) 中，\(\cos C=\dfrac{2}{3}\)，\(AC=4\)，\(BC=3\)，由余弦定理可得
...
\solutionfigure{\bitmapfigure[max width=3.2cm]{img/2020/national_paper_3_science/q8_solution_fig1.png}}
...
直线 \(l\) 与圆 \(x^2+y^2=\dfrac{1}{5}\) 相切，那么直线到圆心 \((0,0)\) 的距离等于半径 \(\dfrac{\sqrt{5}}{5}\).
四个选项中，只有 A，D 满足题意.
...
\(\frac{a}{b}=\frac{\log_5 3}{\log_8 5}=\log_5 3\cdot\log_5 8<\frac{(\log_5 3+\log_5 8)^2}{4}=\left( \frac{\log_5 24}{2} \right)^2<1\)，所以 \(a<b\).
因为 \(5^5<8^4\)，所以
\(5<4\log_5 8\)，
即
\(\log_5 8>1.25\)，
...
先根据约束条件画出可行域，由 \(\begin{cases}
x=1\\
2x-y=0
\end{cases}\)
解得 \(A(1,2)\)，

\solutionfigure{\bitmapfigure[max width=4.2cm]{img/2020/national_paper_3_science/q13_solution_fig1.png}}
...
由于 \(\left( x^2+\dfrac{2}{x} \right)^6\) 的展开式的通项公式为
...
\solutionfigure{\bitmapfigure[max width=4.2cm]{img/2020/national_paper_3_science/q15_solution_fig1.png}}
...
\begin{solution}
\begin{enumerate}
\item 数列 \(\{a_n\}\) 满足 \(a_1=3\)，\(a_{n+1}=3a_n-4n\)，
则 \(a_2=3a_1-4=5\)，\(a_3=3a_2-4\times2=7\)，...，
猜想 \(\{a_n\}\) 的通项公式为 \(a_n=2n+1\).
证明如下：\(1^{\odot}\) 当 \(n=1\) 时，\(a_1=3=2\times1+1\)；当 \(n=2\) 时，\(a_2=5=2\times2+1\)；当 \(n=3\) 时，\(a_3=7=2\times3+1\)，所以猜想在 \(n=1\)，\(2\)，\(3\) 时成立，

\(2^{\odot}\) 假设 \(n=k\) 时，\(a_k=2k+1\)（\(k\in \N^+\)）成立，
当 \(n=k+1\) 时，
\(a_{k+1}=3a_k-4k=3(2k+1)-4k=2k+3=2(k+1)+1\)，
故 \(n=k+1\) 时成立，
由\(1^{\odot}\)，\(2^{\odot}\)知，\(a_n=2n+1\)，猜想成立，
...
\begin{solution}
\begin{enumerate}
\item 该市一天的空气质量等级为 \(1\) 的概率为 \(\frac{43}{100}\)，等级为 \(2\) 的概率为 \(\frac{27}{100}\)，等级为 \(3\) 的概率为 \(\frac{21}{100}\)，等级为 \(4\) 的概率为 \(\frac{9}{100}\).

\item 由题意可得：一天中到该公园锻炼的平均人次的估计值为：
\(\bar{x}=100\times0.20+300\times0.35+500\times0.45=350\).

\item 根据所给数据，可得下面的 \(2\times2\) 列联表，
...
\begin{solution}
\begin{enumerate}
\item 证明：在 \(AA_1\) 上取点 \(M\)，使得 \(A_1M=2AM\)，连接 \(EM\)，\(MB_1\)，\(EC_1\)，\(FC_1\)，
...
\item 在长方体 \(ABCD-A_1B_1C_1D_1\) 中，以 \(C_1\) 为坐标原点，分别以 \(C_1D_1\)，\(C_1B_1\)，\(C_1C\) 所在直线为 \(x\)，\(y\)，\(z\) 轴建立空间直角坐标系.
...
\item 由（1）知 \(A(-5,0)\)，设 \(P(s,t)\)，点 \(Q(6,n)\).
...
\item 证明：设 \(x_0\) 为 \(f(x)\) 的一个零点，根据题意，
...
\item 不妨设 \(a\le b<0<c<\sqrt[3]{4}\)，则
