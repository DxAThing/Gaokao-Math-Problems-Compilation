...
\(\pi rl=\sqrt{3}\pi\)，\(h=DO=\sqrt{2}\)，\(l^2=h^2+r^2\).
所以 \(rl=\sqrt{3}\)，\(l^2=2+r^2\). 两式平方并代入，得
\(r^2(2+r^2)=3\)，\(r^4+2r^2-3=0\)，\((r^2-1)(r^2+3)=0\).
...
\item 因为 \(f(-2)=\e^{-2}\ne0\)，所以零点不可能是 \(-2\). 对 \(x\ne-2\)，方程 \(f(x)=0\) 等价于
\(a=g(x)\)，\(g(x)=\dfrac{\e^x}{x+2}\).
又
\[
g'(x)=\dfrac{\e^x(x+1)}{(x+2)^2}
\]
在 \((-\infty,-2)\) 上，\(x+1<0\)，所以 \(g(x)\) 单调递减，且
\(\lim_{x\to-\infty}g(x)=0^{-}\)，\(\lim_{x\to-2^-}g(x)=-\infty\).
因此对每个 \(a<0\)，方程 \(a=g(x)\) 只有一个根.

在 \((-2,-1)\) 上，\(g'(x)<0\)；在 \((-1,+\infty)\) 上，\(g'(x)>0\)，且
\(\lim_{x\to-2^+}g(x)=+\infty\)，\(g(-1)=\dfrac1{\e}\)，\(\lim_{x\to+\infty}g(x)=+\infty\).
...
\]
当 \(k=4\) 时，\(C_1\) 上的点满足
\(x=\cos^4t\ge0\)，\(y=\sin^4t\ge0\)，\(\sqrt{x}+\sqrt{y}=\cos^2t+\sin^2t=1\).
