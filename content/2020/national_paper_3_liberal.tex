...
\choices
    ...
    {\(f(x)\) 的图象关于 \(y\) 轴对称}
    {\(f(x)\) 的图象关于直线 \(x=\pi\) 对称}
    {\shortstack{\(f(x)\) 的图象\\关于直线\\\(x=\dfrac{\pi}{2}\) 对称}}
...
\end{solution}
\begin{problem}
设双曲线 \(C:\dfrac{x^2}{a^2}-\dfrac{y^2}{b^2}=1\)（\(a>0\)，\(b>0\)）的一条渐近线为 \(y=\sqrt2x\)，则 \(C\) 的离心率为
...
\end{solution}
\begin{problem}
设函数 \(f(x)=\dfrac{\e^x}{x+a}\)，若 \(f'(1)=\dfrac{\e}{4}\)，则 \(a=\)
...
\end{solution}
\begin{problem}
已知圆锥的底面半径为 \(1\)，母线长为 \(3\)，则该圆锥内半径最大的球的表面积为
...
所以该球的表面积为
\(4\pi r^2=4\pi\times \left( \frac{\sqrt2}{2} \right)^2=2\pi\).
...
\begin{center}
\begin{tabular}{c|ccc}
\shortstack{锻炼人次\\空气质量等级} & \([0,200]\) & \((200,400]\) & \((400,600]\) \\
\hline
\(1\)（优） & \(2\) & \(16\) & \(25\) \\
\(2\)（良） & \(5\) & \(10\) & \(12\) \\
\(3\)（轻度污染） & \(6\) & \(7\) & \(8\) \\
...
因为点 \(E\) 在 \(DD_1\) 上，且 \(2DE=ED_1\)，所以 \(ED//AM\)，且 \(ED=AM\)，所以四边形 \(AED_1M\) 为平行四边形，所以 \(D_1M//AE\)，且 \(D_1M=AE\).

又因为 \(F\) 在 \(BB_1\) 上，且 \(BF=2FB_1\)，所以 \(A_1M//FB_1\)，且 \(A_1M=FB_1\)，所以 \(A_1B_1FM\) 为平行四边形.

所以 \(FM//A_1B_1\)，且 \(FM=A_1B_1\)，即 \(FM//C_1D_1\)，且 \(FM=C_1D_1\)，所以 \(C_1D_1MF\) 为平行四边形.

所以 \(D_1M//C_1F\).

因为 \(D_1M//AE\)，所以 \(AE//C_1F\)，于是 \(A,E,F,C_1\) 四点共面，所以点 \(C_1\) 在平面 \(AEF\) 内.
\end{solution}
\begin{problem}
已知函数 \(f(x)=x^3-kx+k^2\).
