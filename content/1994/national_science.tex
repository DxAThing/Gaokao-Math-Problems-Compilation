...
\begin{solution}
\begin{enumerate}
\item \(\omega=\sqrt{2}\left(\cos\frac{5\pi}{4}+\i\sin\frac{5\pi}{4}\right)\).
\item \(a=-1\)，\(b=2\).
\end{enumerate}
\end{solution}
...
\bitmapinlinefigure[0.42\linewidth][trim=0 5.29cm 0 0,clip,max width=0.42\linewidth]{img/1994/national_science/q23_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item \(AB_1\parallel\text{平面 }DBC_1\).
\item \(45^{\circ}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
直线 \(l\) 的方程为 \(y=\frac{1+\sqrt{5}}{2}x\)，抛物线 \(C\) 的方程为 \(y^2=\frac{4\sqrt{5}}{5}x\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(a_1=2\)，\(a_2=6\)，\(a_3=10\).
\item \(a_n=4n-2\).
\item \(\displaystyle\lim_{n\to\infty}(b_1+b_2+\cdots+b_n-n)=1\).
\end{enumerate}
\end{solution}
