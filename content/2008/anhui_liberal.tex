...
\begin{problem}
若 \(A\) 为不等式组 \(\left\{ \begin{array}{l} x \leqslant 0, \\ y \geqslant 0, \\ y - x \leqslant 2 \end{array} \right.\) 表示的平面区域，则当 \(a\) 从 \(-2\) 连续变化到 \(1\) 时，动直线 \(x + y = a\) 扫过 \(A\) 中的那部分区域的面积为（\quad）
...
\begin{problem}
在数列 \(\{a_{n}\}\) 中，\(a_{n}=4n-\frac{5}{2}\)，\(a_{1}+a_{2}+\cdots+a_{n}=an^{2}+bn, n\in\mathbf{N}^{*}\)，其中 \(a, b\) 为常数，则 \(ab=\)\fillinblank{}\fillinblank{}.
\end{problem}
...
\begin{center}
\bitmapinclude[max width=0.34\linewidth,max totalheight=5.2cm]{img/2008/anhui_liberal/q19_fig1.png}
\end{center}
\end{problem}
...
\begin{problem}
设数列 \(\{a_{n}\}\) 满足 \(a_1 = a, a_{n+1} = c a_n + 1 - c, n \in \mathbf{N}^*\)，其中 \(a, c\) 为实数. 且 \(c \neq 0\).

\begin{enumerate}
\item 求数列 \(\{a_{n}\}\) 的通项公式；
\item 设 \(a = \frac{1}{2}, c = \frac{1}{2}, b_n = n(1 - a_n), n \in \mathbf{N}^*\)，求数列 \(\{b_n\}\) 的前 \(n\) 项和 \(S_n\)；
\item 若 \(0 < a_{n} < 1\) 对任意 \(n \in \mathbf{N}^{*}\) 成立，证明 \(0 < c \leqslant 1\).
\end{enumerate}
\end{problem}
