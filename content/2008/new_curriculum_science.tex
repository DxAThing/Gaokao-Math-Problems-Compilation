...
\bitmapinlinefigure[0.42\linewidth][max width=0.42\linewidth]{img/2008/new_curriculum_science/q5_fig1.png}
...
\begin{enumerate}
\item \(a_n=-2n+5\)；
\item \(S_n=-n^2+4n=4-(n-2)^2\)，当 \(n=2\) 时，\(S_n\) 取得最大值 \(4\).
\end{enumerate}
\end{solution}
\end{problem}
...
\begin{enumerate}
\item \(D(Y_1)=4\)，\(D(Y_2)=12\)；
\item \(f(x)\) 的最小值为 \(3\)，此时 \(x=75\).
\end{enumerate}
\end{solution}
\end{problem}
...
\begin{solution}
\begin{enumerate}
\item \(C_1\) 的方程为 \(\frac{x^2}{4}+\frac{y^2}{3}=1\)；
\item 直线 \(l\) 的方程为 \(y=\sqrt{6}x-2\sqrt{3}\) 或 \(y=\sqrt{6}x+2\sqrt{3}\).
\end{enumerate}
\end{solution}
\end{problem}
...
\begin{enumerate}
\item \(OM \cdot OP = OA^2\)；
\item \(\angle OKM = 90^{\circ}\).
\end{enumerate}
\end{solution}
\end{problem}
...
\begin{enumerate}
\item \(C_1\) 是圆 \(x^2+y^2=1\)，\(C_2\) 是直线 \(x-y+\sqrt{2}=0\)，两曲线有一个公共点；
\item 压缩后的参数方程为 \(C_1^{\prime}: x=\cos\theta,\ y=\frac{1}{2}\sin\theta\)，\(C_2^{\prime}: x=\frac{\sqrt{2}}{2}t-\sqrt{2},\ y=\frac{\sqrt{2}}{4}t\)，两曲线仍有一个公共点.
\end{enumerate}
\end{solution}
\end{problem}
...
\bitmapinlinefigure[6.0cm][]{img/2008/new_curriculum_science/q24_fig1.png}
...
4,&x\leqslant 4,\\
-2x+12,&4<x\leqslant 8,\\
-4,&x>8,
\end{cases}\)；
\item 不等式的解集为 \((-\infty,5)\).
\end{enumerate}
\end{solution}
\end{problem}
