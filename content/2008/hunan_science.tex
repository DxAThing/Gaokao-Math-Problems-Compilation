...
\begin{solution}
短答案：至少有 \(1\) 人面试合格的概率为 \(\frac{7}{8}\)；\(P(\xi=0)=\frac{3}{8}\)，\(P(\xi=1)=\frac{3}{8}\)，\(P(\xi=2)=\frac{1}{8}\)，\(P(\xi=3)=\frac{1}{8}\)，\(E(\xi)=1\).
\end{solution}
...
\begin{solution}
短答案：平面 \(PBE\) 与平面 \(PAB\) 垂直；所成二面角（锐角）的大小为 \(\arcsin\frac{\sqrt{10}}{5}\).
\end{solution}
...
2^{n/2},&n=2k\ (k\in\N^{*}).
\end{cases}\) 又 \(b_{n}=\dfrac{n}{2^{n}}\)，\(S_{n}=2-\dfrac{n+2}{2^{n}}\)，从而当 \(n\geqslant6\) 时 \(\lvert S_{n}-2\rvert<\dfrac{1}{n}\).
\end{solution}
...
\begin{solution}
短答案：该船的行驶速度为 \(15\sqrt{5}\) 海里/小时；会进入警戒水域，航线到点 \(E\) 的距离为 \(3\sqrt{5}<7\) 海里.
\end{solution}
...
\begin{solution}
短答案：所有“相关弦”的中点横坐标均为 \(x_0-2\)；当 \(x_0>3\) 时存在最大弦长，最大值为 \(2(x_0-1)\)，当 \(2<x_0\leqslant3\) 时不存在最大值.
\end{solution}
...
\begin{solution}
短答案：\(f(x)\) 在 \((-1,0)\) 上单调递增，在 \((0,+\infty)\) 上单调递减；\(\alpha\) 的最大值为 \(\dfrac{1}{\ln 2}-1\).
\end{solution}
