...
\begin{solution}
函数的导数为 \(y'=\e^x+a\). 若函数有极值点，则存在 \(x_0\) 使 \(\e^{x_0}+a=0\)，从而 \(a<0\)，且 \(x_0=\ln(-a)\). 当 \(a<0\) 时，\(y'\) 随 \(x\) 严格递增，并在 \(x_0\) 左侧为负、右侧为正，所以 \(x_0\) 确为极小值点. 题意中的“大于零的极值点”指极值点的横坐标大于零，因此 \(\ln(-a)>0\)，即 \(-a>1\)，所以 \(a<-1\)，故选 A.
\end{solution}
...
\(y=560+48x+\frac{2160\times10^4}{2000x}=560+48x+\frac{10800}{x}\)，其中楼层数 \(x\) 为不小于 \(10\) 的正整数. 因为 \(x>0\)，由基本不等式，
\(48x+\frac{10800}{x}\geqslant2\sqrt{48x\cdot\frac{10800}{x}}=1440\)，等号成立当且仅当 \(48x=\frac{10800}{x}\)，即 \(x^2=225\). 结合 \(x\geqslant10\)，得到唯一可取的等号情形 \(x=15\)，且此时 \(y=560+1440=2000\). 因此该楼房应建为 \(15\) 层.
\end{solution}
...
\item 因为 \(y+z=500\)，且 \(y\)，\(z\) 为学生人数，所以二者为正整数. 再由 \(y\geqslant245\)，\(z\geqslant245\)，得 \(245\leqslant y\leqslant255\)，并且 \(z=500-y\). 因而满足条件的整数配置为
\((245,255)\)，\((246,254)\)，\(\ldots\)，\((255,245)\)，共 \(11\) 个；按题目所采用的等可能配置模型，每个配置的概率相同. 初三年级女生比男生多等价于 \(y>z\)，结合 \(z=500-y\) 得 \(y>250\)，故满足条件的配置为 \(y=251\)，\(252\)，\(253\)，\(254\)，\(255\)，共 \(5\) 个. 所以所求概率为 \(\frac5{11}\).
\end{enumerate}
\end{solution}
...
若 \(\angle PAB=90^\circ\)，则 \(AP\perp AB\). 由于 \(AB\) 在 \(x\) 轴上，\(AP\) 必为竖直线，所以点 \(P\) 的横坐标必须为 \(-\sqrt2\). 代入抛物线可得唯一点 \(P\left(-\sqrt2,\frac54\right)\). 若 \(\angle PBA=90^\circ\)，同理由 \(BP\perp AB\) 得点 \(P\) 的横坐标必须为 \(\sqrt2\)，代入抛物线可得唯一点 \(P\left(\sqrt2,\frac54\right)\). 因而这两种情形各有一个点.

若 \(\angle APB=90^\circ\)，由 \(\overrightarrow{PA}\cdot\overrightarrow{PB}=0\)，得
\(x^2-2+\left(1+\frac{x^2}{8}\right)^2=0\). 令 \(t=x^2\)，则 \(t^2+80t-64=0\)，解得 \(t=-40\pm8\sqrt{26}\). 其中 \(t=-40-8\sqrt{26}<0\) 不合题意，而 \(t=-40+8\sqrt{26}>0\) 符合，因此 \(x=\pm\sqrt{-40+8\sqrt{26}}\)，对应两个点. 这两个点的横坐标不等于 \(\pm\sqrt2\)，所以不与前两种情形重复. 综上，抛物线上共有 \(1+1+2=4\) 个点，使 \(\triangle ABP\) 为直角三角形.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 将递推式改写为 \(3a_n-a_{n-1}-2a_{n-2}=0\). 其特征方程为 \(3r^2-r-2=0\)，即 \((r-1)(3r+2)=0\)，所以两个特征根为 \(1\) 和 \(-\frac23\). 因此设
\(a_n=A+B\left(-\frac23\right)^{n-1}\). 代入前两项得
\(A+B=1\)，\(A-\frac23B=2\). 两式相减得 \(-\frac53B=1\)，所以 \(B=-\frac35\)，再由 \(A+B=1\) 得 \(A=\frac85\). 从而
\(a_n=\frac85-\frac35\left(-\frac23\right)^{n-1}\).

在题设中取 \(k=0\)，得到 \(-1\leqslant b_m\leqslant1\). 由于 \(b_m\) 是非零整数，所以 \(b_m\) 只能取 \(-1\) 或 \(1\). 再取 \(k=1\)，有 \(-1\leqslant b_m+b_{m+1}\leqslant1\). 两项都属于 \(\{-1,1\}\)，故它们不能同号，只能满足 \(b_{m+1}=-b_m\). 结合 \(b_1=1\)，逐项得到 \(b_n=(-1)^{n+1}\). 反过来，交错数列从任意一项开始的连续和在区间 \([-1,1]\) 内：当连续项数为偶数时和为 \(0\)，当连续项数为奇数时和为首项 \(1\) 或 \(-1\)，所以它确实满足题设条件.

\item 由 \(a_n\) 与 \(b_n\) 的通项公式，
...
先计算交错和. 将相邻两项配对可得
\(\displaystyle\sum_{k=1}^n k(-1)^{k+1}=\begin{cases}
-\frac n2,&n\text{ 为偶数},\\[2pt]
\frac{n+1}{2},&n\text{ 为奇数},
\end{cases}\)
这也可统一写成 \(\displaystyle\frac{1+(-1)^{n+1}(2n+1)}4\). 对于任意 \(r\neq1\)，由有限等比数列和
\(\displaystyle\sum_{k=1}^n r^k=\frac{r-r^{n+1}}{1-r}\) 对 \(r\) 求导，得
\(\displaystyle\sum_{k=1}^n kr^{k-1}=\frac{1-(n+1)r^n+nr^{n+1}}{(1-r)^2}\). 令 \(r=\frac23\)，则
\(\displaystyle\sum_{k=1}^n k\left(\frac23\right)^{k-1}=9\left[1-(n+1)\left(\frac23\right)^n+n\left(\frac23\right)^{n+1}\right]=9-3(n+3)\left(\frac23\right)^n\).

代回 \(S_n\)，分别化简两部分，得到
\(S_n=\frac85\cdot\frac{1+(-1)^{n+1}(2n+1)}4-\frac35\left[9-3(n+3)\left(\frac23\right)^n\right]\)
\(=-5+\frac25(-1)^{n+1}(2n+1)+\frac95(n+3)\left(\frac23\right)^n\).
\end{enumerate}
\end{solution}
