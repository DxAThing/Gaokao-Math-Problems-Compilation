...
\begin{solution}
还原正方体，设 \(AD=1\)，则 \(AB=\sqrt5\)，\(AF=1\)，\(BE=EF=2\sqrt2\)，\(AE=3\). \(CD\) 与 \(AB\) 所成角等于 \(BE\) 与 \(AB\) 所成角，因此
...
\begin{solution}
\begin{enumerate}
\item 由正弦定理及条件，得 \(\sin^2A+\sin^2C=2\sin^2B=1\). 故 \(\sin^2C=\cos^2A\). 又 \(A\) 为钝角，所以 \(\sin C=-\cos A\). 由 \(A+C=\pi-\dfrac{\pi}{4}=\dfrac{3\pi}{4}\) 得 \(\sin C=\sin\!\left(\dfrac{\pi}{4}-C\right)\). 因为 \(0<C<\dfrac{\pi}{4}\)，所以 \(C=\dfrac{\pi}{8}\)，从而 \(A=\dfrac{5\pi}{8}\).
...
\begin{solution}
\begin{enumerate}
\item 设一次抽检后不需要调整设备的事件为 \(C\). 由题意，\(C=A_1A_2+A_1B_2+B_1A_2\). 因此 \(P(C)=0.9^2+2\cdot0.9\cdot0.05=0.9\).

\item 设需要调整设备的概率为 \(p=0.1\). 由题意，\(\xi\sim B(3,0.1)\)，故
...
\item 【法一】如图，延长 \(C_0B\) 到 \(E\)，使 \(BE=C_0B\)，连结 \(AE\)，\(CE\).

\bitmapinlinefigure[0.38\linewidth][max width=0.38\linewidth]{img/2008/sichuan_postponed_science/q19_solution_fig1.png}
...
由前一小问可知，\(\angle CBE\) 为二面角 \(A-BD-C\) 的平面角，即二面角 \(A-BD-C\) 的大小为 \(60^\circ\).

【法二】以 \(D\) 为坐标原点，射线 \(DA\)，\(DB\) 分别为 \(x\) 轴正半轴和 \(y\) 轴正半轴，建立如图的空间直角坐标系 \(D-xyz\).

\bitmapinlinefigure[0.35\linewidth][max width=0.35\linewidth]{img/2008/sichuan_postponed_science/q19_solution_fig2.png}

则 \(A(1,0,0)\)，\(B(0,1,0)\)，\(D(0,0,0)\).
由前一小问可设点 \(C\) 的坐标为 \((x,1,z)\)，其中 \(z>0\)，则有 \(x^2+z^2=1\). \(\circled{1}\)
...
\item \examdisplaymath{b_n=a_{n+1}-\dfrac12a_n=\dfrac{(n+1)^2}{2^n}-\dfrac{n^2}{2^n}=\dfrac{2n+1}{2^n}.}
...
\begin{solution}
\begin{enumerate}
\item 设 \(C_1:\dfrac{x^2}{a^2}+\dfrac{y^2}{b^2}=1\)（\(a>b>0\)），其半焦距为 \(c\)（\(c>0\)）. 则 \(C_2:y^2=4cx\). 由条件知 \((2b)^2=2a\left(\dfrac{a^2}{c}-c\right)\)，得 \(a=2c\). \(C_1\) 的右准线方程为 \(x=\dfrac{a^2}{c}\)，即 \(x=4c\). \(C_2\) 的准线方程为 \(x=-c\). 由条件知 \(5c=15\)，所以 \(c=3\)，故 \(a=6\)，\(b=3\sqrt3\). 从而 \(C_1:\dfrac{x^2}{36}+\dfrac{y^2}{27}=1\)，\(C_2:y^2=12x\).

\item 由题设知 \(l:y=x-c\)，设 \(M(x_1,y_1)\)，\(N(x_2,y_2)\)，\(P(x_3,y_3)\)，\(Q(x_4,y_4)\). 由前一小问知 \(C_1:\dfrac{x^2}{4c^2}+\dfrac{y^2}{3c^2}=1\)，即 \(3x^2+4y^2=12c^2\). 由
\examdisplaycases{}{
3x^2+4y^2=12c^2,\\
y=x-c
}
