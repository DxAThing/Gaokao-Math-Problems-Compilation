...
\begin{problem}
已知等差数列 \(\{a_{n}\}\) 满足 \(a_{2} + a_{4} = 4\)，\(a_{3} + a_{5} = 10\)，则它的前10项的和 \(S_{10} =\)（\quad）
...
\begin{problem}
已知 5 只动物中有 1 只患有某种疾病，需要通过化验血液来确定患病的动物. 血液化验结果呈阳性的即为患病动物，呈阴性即没患病. 下面是两种化验方案：方案甲：逐个化验，直到能确定患病动物为止. 方案乙：先任取3只，将它们的血液混在一起化验. 若结果呈阳性则表明患病动物为这3只中的1只，然后再逐个化验，直到能确定患病动物为止；若结果呈阴性则在另外2只中任取1只化验.
