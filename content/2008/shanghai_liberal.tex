...
\begin{solution}
函数 \(y=\log_{2}x\) 的定义域为 \(x>0\)，值域为 \(\R\)，其反函数由交换变量得到 \(y=2^x\). 题设给出的反函数为 \(f^{-1}(x)=\log_{2}x\)，因此原函数为 \(f(x)=2^x\).
\end{solution}
...
由向量和的模的平方公式，
\[
|\bs{a}+\bs{b}|^2=|\bs{a}|^2+|\bs{b}|^2+2|\bs{a}||\bs{b}|\cos\frac{\pi}{3}=1^2+2^2+2\cdot1\cdot2\cdot\frac12=7
\]
由于模长非负，得 \(|\bs{a}+\bs{b}|=\sqrt7\).
\end{solution}
...
\begin{solution}
抛物线 \(y^2=4x\) 的标准形式为 \(y^2=4px\)，故 \(p=1\)，焦点为 \((1,0)\). 将焦点坐标代入直线方程，得
\(a\cdot1-0+1=0\)，
所以 \(a=-1\).
\end{solution}
...
\begin{solution}
设虚根为 \(z\). 因为方程是实系数方程，且 \(z\) 为非实根，所以其另一根为 \(\overline{z}\). 由韦达定理，
\(p=z\overline{z}=|z|^2=2^2=4\).
因此 \(p=4\).
\end{solution}
...
\begin{solution}
从五个点中任取三个点的取法总数为 \(\mathrm C_5^3=10\). 对平面向量 \((u_1,v_1)\)、\((u_2,v_2)\)，记 \(\det((u_1,v_1),(u_2,v_2))=u_1v_2-v_1u_2\). 三点共线当且仅当以其中一点为起点的两条向量的行列式为零. 十组点对应的行列式（取绝对值）依次为
\(|\det(\overrightarrow{AB},\overrightarrow{AC})|=2\)，\(|\det(\overrightarrow{AB},\overrightarrow{AD})|=4\)，\(|\det(\overrightarrow{AB},\overrightarrow{AE})|=4\).
\(|\det(\overrightarrow{AC},\overrightarrow{AD})|=2\)，\(|\det(\overrightarrow{AC},\overrightarrow{AE})|=0\)，\(|\det(\overrightarrow{AD},\overrightarrow{AE})|=4\).
\(|\det(\overrightarrow{BC},\overrightarrow{BD})|=0\)，\(|\det(\overrightarrow{BC},\overrightarrow{BE})|=2\)，\(|\det(\overrightarrow{BD},\overrightarrow{BE})|=4\)，\(|\det(\overrightarrow{CD},\overrightarrow{CE})|=2\).
因此只有 \(A\)，\(C\)，\(E\) 和 \(B\)，\(C\)，\(D\) 三点共线，不能构成三角形的取法有 \(2\) 种.
...
\begin{problem}
若函数 \(f(x) = (x + a)(bx + 2a)\) （常数 \(a\)，\(b \in \R\)）是偶函数，且它的值域为 \(\left(-\infty, 4\right]\)，则该函数的解析式 \(f(x)=\)\fillinblank{}.
\end{problem}
...
\begin{solution}
总体共有 \(10\) 个数据，中位数为中间两个数据的平均数，因此
\(\frac{a+b}{2}=10.5\)，\(a+b=21\).
由于其余数据固定，且 \(a+b\) 固定，总体平均数也固定. 于是方差最小时，等价于 \(a^2+b^2\) 最小. 由
\[
...
\begin{problem}
在平面直角坐标系中，点 \(A\)，\(B\)，\(C\) 的坐标分别为 \((0,1)\)，\((4,2)\)，\((2,6)\). 如果 \(P(x,y)\) 是 \(\triangle ABC\) 围成的区域（含边界）上的点，那么当 \(w = xy\) 取到最大值时，点 \(P\) 的坐标是\fillinblank{}.
\end{problem}
...
\begin{solution}
三角形区域的两条上边界所在直线分别为
\(AC:y=1+\frac52x\)，\(BC:y=-2x+10\).
区域内 \(x\geqslant0\)，所以固定 \(x\) 时，\(w=xy\) 随 \(y\) 增大而增大，只需考察上边界 \(AC\) 与 \(BC\).
...
\begin{problem}
设 \(P\) 是椭圆 \(\frac{x^2}{25} + \frac{y^2}{16} = 1\) 上的点. 若 \(F_1\)，\(F_2\) 是椭圆的两个焦点，则 \(\left|PF_1\right| + \left|PF_2\right|\) 等于（\quad）
...
\begin{solution}
椭圆 \(\frac{x^2}{25}+\frac{y^2}{16}=1\) 的长半轴为 \(5\). 椭圆上任一点到两个焦点的距离之和等于长轴长，即
\(|PF_1|+|PF_2|=2\times5=10\).
所以正确选项为 D.
\end{solution}
...
反之，取 \(\alpha\) 为 \(xOy\) 平面，取 \(l\) 为 \(x\) 轴. 对任意 \(c\in\R\)，平面 \(\alpha\) 内的直线 \(m_c\colon x=c\) 都与 \(l\) 相交且垂直，因此 \(l\) 与平面 \(\alpha\) 内无数条直线垂直；但 \(l\subset\alpha\)，所以 \(l\) 不垂直于 \(\alpha\). 因此题述条件不是充分条件，故为必要非充分条件，正确选项为 B.
\end{solution}
...
\bitmapfigure[float=inline,max width=0.62\linewidth]{img/2008/shanghai_liberal/q15_fig1.png}
\FloatBarrier
...
\begin{solution}
设圆的半径为 \(r>0\)，则其圆心为 \((r,r)\)，四等分点可表示为
\(A=(r,2r)\)，\(B=(2r,r)\)，\(C=(r,0)\)，\(D=(0,r)\).
先看圆盘内部的点. 若点在内部，向左上方作足够短的移动仍在圆盘内，所得点的横坐标更小、纵坐标更大，因此该点不可能是题述集合中的点.

在劣弧 \(AB\)（不含端点 \(A\)）和劣弧 \(BC\) 上，点 \(A\) 的横坐标不大于该点、纵坐标不小于该点，所以这些点均被 \(A\) 优于. 在劣弧 \(CD\)（不含端点 \(D\)）上，沿圆弧向 \(D\) 移动可得到横坐标更小且纵坐标更大的圆盘内点，所以这些点也都不满足条件.

对劣弧 \(DA\) 上任一点 \(Q\)，若圆盘内另有点 \(P\) 满足 \(x_P\leqslant x_Q\)、\(y_P\geqslant y_Q\)，则相对于圆心有
\(r-x_P\geqslant r-x_Q\)，\(y_P-r\geqslant y_Q-r\)
且至少一个不等号严格成立，从而 \(P\) 到圆心的距离大于 \(Q\) 到圆心的距离 \(r\)，这与 \(P\) 在圆盘内矛盾. 因此劣弧 \(DA\) 上的点恰好满足题设条件，正确选项为 D.
\end{solution}
...
\bitmapfigure[float=inline,max width=0.52\linewidth]{img/2008/shanghai_liberal/q16_fig1.png}
\FloatBarrier
\end{problem}
...
\bitmapfigure[float=inline,max width=0.5\linewidth]{img/2008/shanghai_liberal/q17_fig1.png}
\FloatBarrier
\end{problem}
...
\begin{enumerate}
\item 若 \(f(x) = 2\)，求 \(x\) 的值；
\item 若 \(2^{t}f(2t) + mf(t) \geqslant 0\) 对于 \(t \in \left[1,2\right]\) 恒成立，求实数 \(m\) 的取值范围.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 当 \(x<0\) 时，\(|x|=-x\)，所以 \(f(x)=2^x-\frac{1}{2^{-x}}=2^x-2^x=0\)，不可能满足 \(f(x)=2\).

当 \(x\geqslant0\) 时，令 \(u=2^x>0\)，则 \(f(x)=u-\frac{1}{u}=2\). 因而
\(u^2-2u-1=0\)，
解得 \(u=1+\sqrt{2}\) 或 \(u=1-\sqrt{2}\). 因为 \(u>0\)，所以 \(u=1+\sqrt{2}\)，从而
\(x=\log_{2}(1+\sqrt{2})\).

\item 当 \(t\in\left[1,2\right]\) 时，\(t>0\)，故 \(f(t)=2^t-2^{-t}>0\). 令 \(u=2^t\)，则 \(u\in\left[2,4\right]\)，且 \(f(t)=u-\frac{1}{u}=\frac{u^2-1}{u}\)，\(f(2t)=u^2-\frac{1}{u^2}=\frac{(u^2-1)(u^2+1)}{u^2}\).
因此
\[
\frac{2^tf(2t)}{f(t)}
=\frac{u\cdot\frac{(u^2-1)(u^2+1)}{u^2}}{\frac{u^2-1}{u}}=u^2+1=2^{2t}+1
\]
原不等式等价于 \(m\geqslant-(2^{2t}+1)\).
这里 \(t\in\left[1,2\right]\)，右端的最大值为 \(-5\)，所以不等式对所有 \(t\in\left[1,2\right]\) 恒成立的充要条件是 \(m\geqslant-5\). 因此 \(m\) 的取值范围为 \(\left[-5,+\infty\right)\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
\item 求双曲线 \(C\) 的渐近线方程；
\item 已知点 \(M\) 的坐标为 \((0,1)\). 设 \(P\) 是双曲线 \(C\) 上的点，\(Q\) 是点 \(P\) 关于原点的对称点. 记 \(\lambda = \overrightarrow{MP} \cdot \overrightarrow{MQ}\)，求 \(\lambda\) 的取值范围；
\item 已知点 \(D\)，\(E\)，\(M\) 的坐标分别为 \((-2, -1)\)，\((2, -1)\)，\((0, 1)\)，\(P\) 为双曲线 \(C\) 上在第一象限内的点. 记 \(l\) 为经过原点与点 \(P\) 的直线，\(s\) 为 \(\triangle DEM\) 截直线 \(l\) 所得线段的长. 试将 \(s\) 表示为直线 \(l\) 的斜率 \(k\) 的函数.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 将双曲线方程中的常数项 \(1\) 换成 \(0\)，得到渐近线方程
\(\frac{x^2}{2}-y^2=0\)，
即 \(y=\pm\frac{\sqrt{2}}{2}x\).

\item 设 \(P=(x,y)\)，则 \(Q=(-x,-y)\). 因而
\[
...
\item 设直线 \(l\) 的方程为 \(y=kx\). 因为 \(P\) 在双曲线 \(C\) 的第一象限内，所以 \(k>0\)，且
\[
\frac{x^2}{2}-k^2x^2=1
\]
...
\begin{enumerate}
\item 若 \(a_1 + a_2 + a_3 + \cdots + a_{12} = 64\)，求 \(r\) 的值；
\item 求证：当 \(n\) 是正整数时，\(T_{12n} = -4n\)；
\item 已知 \(r > 0\)，且存在正整数 \(m\)，使得在 \(T_{12m+1}\)，\(T_{12m+2}\)，\(\cdots\)，\(T_{12m+12}\) 中有 4 项为 100，求 \(r\) 的值，并指出哪 4 项为 100.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 由递推关系，前 \(12\) 项为 \(1\)，\(2\)，\(r\)，\(3\)，\(4\)，\(r+2\)，\(5\)，\(6\)，\(r+4\)，\(7\)，\(8\)，\(r+6\). 因而
\(a_1+a_2+\cdots+a_{12}=48+4r\).
由题设 \(48+4r=64\)，解得 \(r=4\).

\item 由 \(a_{n+3}=a_n+2\) 可知，对 \(1\leqslant j\leqslant12\)，有 \(a_{12k+j}=a_j+8k\). 又因为 \(b_n\) 的一个周期为 \(1\)，\(0\)，\(-1\)，\(0\)，所以每连续 \(12\) 项中只有下标 \(12k+1\)，\(12k+3\)，\(12k+5\)，\(12k+7\)，\(12k+9\)，\(12k+11\) 对 \(T_n\) 有贡献. 因此
\[
...
\item 由上面的结论 \(T_{12m}=-4m\)，并利用 \(a_{12m+1}=8m+1\)、\(a_{12m+3}=8m+r\)、\(a_{12m+5}=8m+4\)、\(a_{12m+7}=8m+5\)、\(a_{12m+9}=8m+r+4\)、\(a_{12m+11}=8m+8\)，以及相应的 \(b_n\) 周期，逐项累加得到
\(T_{12m+1}=T_{12m+2}=1+4m\)，
\(T_{12m+3}=T_{12m+4}=1-r-4m\)，
\(T_{12m+5}=T_{12m+6}=5-r+4m\)，
\(T_{12m+7}=T_{12m+8}=-r-4m\)，
\(T_{12m+9}=T_{12m+10}=4+4m\)，
\(T_{12m+11}=T_{12m+12}=-4-4m\).
