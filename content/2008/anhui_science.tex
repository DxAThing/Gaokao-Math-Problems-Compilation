...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.6cm]{img/2008/anhui_science/q10_fig1.png}
...
\begin{problem}
在数列 \(\{a_{n}\}\) 中，\(a_{n} = 4n - \frac{5}{2}\)，\(a_{1} + a_{2} + \cdots + a_{n} = an^{2} + bn\)，\(n \in \N^*\)，其中 \(a\)，\(b\) 为常数，则 \(\lim\limits_{n \to \infty} \frac{a^{n} - b^{n}}{a^{n} + b^{n}}\) 的值是\fillinblank{}.
\end{problem}

\begin{problem}
若 \(A\) 为不等式组 \(\left\{ \begin{array}{l} x \leqslant 0, \\ y \geqslant 0, \\ y - x \leqslant 2 \end{array} \right.\) 表示的平面区域，则当 \(a\) 从 \(-2\) 连续变化到 1 时，动直线 \(x + y = a\) 扫过 \(A\) 中的那部分区域的面积为\fillinblank{}.
\end{problem}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=5.2cm]{img/2008/anhui_science/q18_fig1.png}
...
\begin{problem}
设函数 \( f(x) = \frac{1}{x \ln x} (x > 0 \text{且} x \neq 1) \).
