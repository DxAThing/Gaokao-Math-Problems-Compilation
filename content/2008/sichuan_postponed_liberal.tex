...
\begin{solution}
\begin{enumerate}
\item 由题设及正弦定理，有 \(\sin^2 A+\sin^2 B=2\sin^2 B=1\)，故 \(\sin^2 C=\cos^2 A\). 因为 \(A\) 为钝角，所以 \(\sin C=-\cos A\). 由 \(\cos A=\cos\left(\pi-\frac{\pi}{4}-C\right)\)，可得 \(\sin C=\sin\left(\frac{\pi}{4}-C\right)\)，得 \(C=\frac{\pi}{8}\)，\(A=\frac{5\pi}{8}\).

\item 由余弦定理及条件 \(b^2=\frac{1}{2}(a^2+c^2)\)，有 \(\cos B=\frac{a^2+c^2}{4ac}\). 因为 \(a^2+c^2\ge 2ac\)，所以 \(\cos B\ge \frac{1}{2}\)，故 \(\sin B\le \frac{\sqrt{3}}{2}\). 当 \(a=c\) 时，等号成立. 从而，\(\sin B\) 的最大值为 \(\frac{\sqrt{3}}{2}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 设 \(A_i\) 表示事件“在一次抽检中抽到的第 \(i\) 件产品为 \(A\) 类品”，\(i=1,2\). \(B_i\) 表示事件“在一次抽检中抽到的第 \(i\) 件产品为 \(B\) 类品”，\(i=1,2\). 设 \(C\) 表示事件“一次抽检后，设备不需要调整”. 则 \(C=A_1A_2+A_1B_2+B_1A_2\). 由已知 \(P(A_i)=0.9\)，\(P(B_i)=0.05\)，\(i=1,2\). 所以所求概率为
\[
P(C)=P(A_1A_2)+P(A_1B_2)+P(B_1A_2)=0.9^2+2\times 0.9\times 0.05=0.9
\]

\item 由（1）知，一次抽检后设备不需要调整的概率为 \(P(C)=0.9\). 故所求概率为 \(1-0.9^3=0.271\).
\end{enumerate}
\end{solution}
...
\item 解法一：如图，由（1）知 \(BC \perp DB\)，\(BC_0 \perp DB\)，所以 \(\angle CBC_0\) 是二面角 \(C-BD-C_0\) 的平面角. 由已知得 \(\angle CBC_0=60^\circ\). 作 \(CF \perp C_0B\)，垂足为 \(F\). 由 \(BC=BC_0=1\)，可得 \(CF=\frac{\sqrt{3}}{2}\)，\(BF=\frac{1}{2}\). 连结 \(AF\)，在 \(\triangle ABF\) 中，
\[
...
解法二：由已知得 \(\angle ADB=\angle DBC_0=90^\circ\). 以 \(D\) 为坐标原点，射线 \(DA\)，\(DB\) 分别为 \(x\) 轴，\(y\) 轴的正半轴，建立如图的空间直角坐标系 \(D-xyz\). 则 \(A(1,0,0)\)，\(B(0,1,0)\)，\(C_0(-1,1,0)\)，\(D(0,0,0)\). 由（1）知 \(BC \perp DB\)，\(BC_0 \perp DB\)，所以 \(\angle CBC_0\) 是二面角 \(C-BD-C_0\) 的平面角. 由已知得 \(\angle CBC_0=60^\circ\)，故 \(C\left(\frac{1}{2},1,\frac{\sqrt{3}}{2}\right)\). 所以
...
\begin{solution}
\begin{enumerate}
\item 由条件得 \(\frac{a_{n+1}}{(n+1)^2}=\frac{1}{2}\cdot \frac{a_n}{n^2}\). 又 \(n=1\) 时，\(\frac{a_n}{n^2}=1\)，故数列 \(\left\{\frac{a_n}{n^2}\right\}\) 构成首项为 \(1\)，公比为 \(\frac{1}{2}\) 的等比数列. 从而 \(\frac{a_n}{n^2}=\frac{1}{2^{n-1}}\)，即 \(a_n=\frac{n^2}{2^{n-1}}\).
...
\item 由 \(S_n=(a_2+a_3+\cdots+a_{n+1})-\frac{1}{2}(a_1+a_2+\cdots+a_n)\) 得
...
\begin{solution}
\begin{enumerate}
\item 设 \(C_1:\frac{x^2}{a^2}+\frac{y^2}{b^2}=1\)（\(a>0,b>0\)），其半焦距为 \(c\)（\(c>0\)）. 则 \(C_2:y^2=4cx\). 由条件知 \((2b)^2=2a\left(\frac{a^2}{c}-c\right)\)，得 \(a=2c\). \(C_1\) 的右准线方程为 \(x=\frac{a^2}{c}\)，即 \(x=4c\). \(C_2\) 的准线方程为 \(x=-c\). 由条件知 \(5c=15\)，所以 \(c=3\)，故 \(a=6\)，\(b=3\sqrt{3}\). 从而 \(C_1:\frac{x^2}{36}+\frac{y^2}{27}=1\)，\(C_2:y^2=12x\).

\item 由（1）知 \(C_1:\frac{x^2}{4c^2}+\frac{y^2}{3c^2}=1\)，即 \(3x^2+4y^2=12c^2\).
由题设知 \(l:y=x-c\). 设 \(M(x_1,y_1)\)，\(N(x_2,y_2)\)，\(P(x_3,y_3)\)，\(Q(x_4,y_4)\).
...
\end{cases}
\]
\(x_3,x_4\) 满足 \(7x^2-8cx-8c^2=0\).

从而
\[
|PQ|=\sqrt{(x_3-x_4)^2+(y_3-y_4)^2}=\sqrt{2}|x_3-x_4|=\frac{24}{7}c.
\]

由条件 \(|PQ|=\frac{36}{7}\)，得 \(c=\frac{3}{2}\).
...
\end{cases}
\]
得 \(x^2-9x+\frac{9}{4}=0\)，所以 \(x_1+x_2=9\).

于是 \(|MN|=|MF|+|FN|=x_1+x_2+2c=12\).
\end{enumerate}
\end{solution}
