...
又 \(c_1=\lambda+18\)，所以
\[
c_n=(\lambda+18)\left(\frac23\right)^{n-1},
\qquad
a_n=3n-21+(\lambda+18)\left(\frac23\right)^{n-1}.
\]

由此 \(\displaystyle\lim_{n\to+\infty}\frac{a_n}{n}=3\)，所以 \(a_n\) 从某一项起恒不为零，且 \(\displaystyle\lim_{n\to+\infty}\frac{a_{n+1}}{a_n}=1\). 若 \(\{a_n\}\) 是等比数列，设其公比为 \(q\)，则由上式必须有 \(q=1\). 但此时数列应为常数列，而
\[
