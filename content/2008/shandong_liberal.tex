...
\bitmapinlinefigure[0.5\linewidth][max width=0.5\linewidth]{img/2008/shandong_liberal/q6_fig1.png}
...
\bitmapinlinefigure[6.0cm][max width=6.0cm]{img/2008/shandong_liberal/q12_fig1.png}
...
\bitmapinlinefigure[0.72\linewidth][max width=0.72\linewidth]{img/2008/shandong_liberal/q14_fig1.png}
\end{problem}
...
\begin{solution}
\begin{enumerate}
\item \(f\left(\frac{\pi}{8}\right)=\sqrt{2}\).
\item \(g(x)\) 的单调递减区间为 \(\left[k\pi+\frac{\pi}{6},k\pi+\frac{2\pi}{3}\right]\)（\(k\in\Z\)）.
\end{enumerate}
\end{solution}
...
\bitmapinlinefigure[0.40\linewidth][max width=0.40\linewidth]{img/2008/shandong_liberal/q19_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 平面 \(MBD \perp\) 平面 \(PAD\).
\item 四棱锥 \(P-ABCD\) 的体积为 \(16\sqrt{3}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 数列 \(\left\{\frac{1}{S_n}\right\}\) 是首项为 \(1\)、公差为 \(\frac{1}{2}\) 的等差数列，且
\[
b_n=\begin{cases}
1, & n=1,\\
...
\end{cases}
\]
\item 第 \(k\) 行所有项和为 \(\dfrac{2(1-2^k)}{k(k+1)}\)（\(k\geqslant 3\)）.
\end{enumerate}
\end{solution}
...
\begin{enumerate}
\item \(a=-\frac{1}{3}\)，\(b=-1\).
\item \(f(x)\) 在 \((-2,0)\) 和 \((1,+\infty)\) 上单调递增，在 \((-\infty,-2)\) 和 \((0,1)\) 上单调递减.
\item 对任意 \(x\in(-\infty,+\infty)\)，都有 \(f(x)\geqslant g(x)\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 椭圆 \(C_2\) 的标准方程为 \(\frac{x^2}{5}+\frac{y^2}{4}=1\).
\item
\begin{enumerate}
\item 点 \(M\) 的轨迹方程为 \(\frac{x^2}{4}+\frac{y^2}{5}=\lambda^2\)（\(\lambda\neq 0\)）.
\item \(\triangle AMB\) 的面积最小值为 \(\frac{40}{9}\).
\end{enumerate}
\end{enumerate}
\end{solution}
