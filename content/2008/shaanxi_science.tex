...
\bitmapfigure[max width=0.34\linewidth,max totalheight=4.8cm]{img/2008/shaanxi_science/q9_fig1.png}
...
\begin{problem}
已知实数 \(x\)，\(y\) 满足 \(\begin{cases} y \geqslant 1, \\ y \leqslant 2x - 1, \\ x + y \leqslant m, \end{cases}\) 如果目标函数 \(z = x - y\) 的最小值为 \(-1\)，则实数 \(m\) 等于（\quad）
...
\begin{problem}
\(\lim\limits_{n\to \infty}\frac{(1 + a)n + 1}{n + a} = 2\)，则 \(a =\)\fillinblank{}.
\end{problem}
...
\begin{problem}
某地奥运火炬接力传递路线共分6段. 传递活动分别由6名火炬手完成. 如果第一棒火炬手只能从甲，乙，丙三人中产生，最后一棒火炬手只能从甲，乙两人中产生，则不同的传递方案共有\fillinblank{}种. （用数字作答）
\end{problem}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=5.2cm]{img/2008/shaanxi_science/q19_fig1.png}
