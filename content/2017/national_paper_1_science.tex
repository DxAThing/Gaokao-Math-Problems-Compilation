...
\bitmapinlinefigure[6.0cm]{img/2017/national_paper_1_science/q8_fig1.png}
...
\begin{solution}
短答案：第（1）问 \(\sin B\sin C=\dfrac23\)；第（2）问 \(\triangle ABC\) 的周长为 \(3+\sqrt{33}\).
\end{solution}
...
\begin{solution}
短答案：第（1）问证明平面 \(PAB\perp\) 平面 \(PAD\)；第（2）问二面角 \(A-PB-C\) 的余弦值为 \(-\dfrac{\sqrt3}{3}\).
\end{solution}
...
\begin{solution}
短答案：第（1）问 \(P(X\ge1)=0.0408\)，\(E(X)=0.0416\)；第（2）问（i）需要对当天的生产过程进行检查；（ii）剔除 \(9.22\) 后，\(\hat\mu=10.02\)，\(\hat\sigma=0.09\).
\end{solution}
...
\begin{solution}
短答案：第（1）问 \(C:\dfrac{x^2}{4}+y^2=1\)；第（2）问直线 \(l\) 过定点 \((2,-1)\).
\end{solution}
...
\begin{solution}
短答案：第（1）问，当 \(a\le0\) 时，\(f(x)\) 在 \(\R\) 上单调递减；当 \(a>0\) 时，\(f(x)\) 在 \(\left(-\infty,\ln\dfrac1a\right)\) 上单调递减，在 \(\left(\ln\dfrac1a,+\infty\right)\) 上单调递增；第（2）问 \(0<a<1\).
\end{solution}
...
\begin{solution}
短答案：第（1）问交点坐标为 \(\left(-\dfrac{21}{25},\dfrac{24}{25}\right)\)，\((3,0)\)；第（2）问 \(a=-16\) 或 \(a=8\).
\end{solution}
...
\begin{solution}
短答案：第（1）问解集为 \(\left[-1,\dfrac{-1+\sqrt{17}}2\right]\)；第（2）问 \(a\in[-1,1]\).
\end{solution}
