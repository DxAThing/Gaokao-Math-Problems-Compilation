...
\begin{solution}
短答案：\(\cos A=-\dfrac{\sqrt{5}}{5}\)，\(\sin(2B-A)=-\dfrac{2\sqrt{5}}{5}\).
\end{solution}
...
总收视人次最多时，\(x=6\)，\(y=3\)，即每周播出甲连续剧 \(6\) 次、乙连续剧 \(3\) 次.
\end{solution}
...
\begin{solution}
短答案：异面直线 \(AP\) 与 \(BC\) 所成角的余弦值为 \(\dfrac{\sqrt{5}}{5}\)；\(PD\perp\) 平面 \(PBC\)；直线 \(AB\) 与平面 \(PBC\) 所成角的正弦值为 \(\dfrac{\sqrt{5}}{5}\).
\end{solution}
...
\begin{solution}
短答案：\(a_n=3n-2\)，\(b_n=2^n\)，数列 \(\{a_{2n}b_n\}\) 的前 \(n\) 项和为 \((3n-4)2^{n+2}+16\).
\end{solution}
...
\begin{solution}
短答案：\(f(x)\) 的单调递增区间为 \((-\infty,a)\)，\((4-a,+\infty)\)，单调递减区间为 \((a,4-a)\)；\(f'(x_0)=0\)；\(b\in[-7,1]\).
\end{solution}
...
\begin{solution}
短答案：椭圆的离心率为 \(\dfrac12\)；直线 \(FP\) 的斜率为 \(\dfrac34\)；椭圆方程为
