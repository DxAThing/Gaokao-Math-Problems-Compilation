...
\begin{solution}
短答案：\(\displaystyle a_n=(-2)^n\)，\(\displaystyle S_n=-\frac{(-2)^{n+1}+2}{3}\)，且 \(S_{n+1}\)，\(S_n\)，\(S_{n+2}\) 成等差数列.
\end{solution}
...
\begin{solution}
短答案：相关系数 \(r\approx0.178\)，可以认为零件的尺寸不随生产过程的进行而系统地变大或变小；需要对当天的生产过程进行检查；剔除离群值 \(x_{13}=9.22\) 后，均值约为 \(10.02\)，标准差约为 \(0.09\).
\end{solution}
...
\begin{solution}
短答案：当 \(a<0\) 时，\(f(x)\) 在 \(\left(-\infty,\ln\left(-\dfrac{a}{2}\right)\right)\) 上单调递减，在 \(\left(\ln\left(-\dfrac{a}{2}\right),+\infty\right)\) 上单调递增；当 \(a=0\) 时，\(f(x)\) 在 \(\R\) 上单调递增；当 \(a>0\) 时，\(f(x)\) 在 \((-\infty,\ln a)\) 上单调递减，在 \((\ln a,+\infty)\) 上单调递增. 且 \(a\in\left[-2\e^{\frac{3}{4}},1\right]\).
\end{solution}
...
\begin{solution}
短答案：当 \(a=-1\) 时，交点为 \((3,0)\) 和 \(\left(-\dfrac{21}{25},\dfrac{24}{25}\right)\)；\(a=-16\) 或 \(a=8\).
\end{solution}
...
\begin{solution}
短答案：当 \(a=1\) 时，不等式的解集为 \(\left[-1,\dfrac{\sqrt{17}-1}{2}\right]\)；\(a\) 的取值范围为 \([-1,1]\).
\end{solution}
