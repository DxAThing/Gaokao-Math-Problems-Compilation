...
\begin{center}
\bitmapinclude[max width=3.15cm,max totalheight=4.6cm]{img/2017/beijing_liberal/q3_fig1.png}
\end{center}
...
\(k=2\)，\(s=\frac{2+1}{2}=\frac32\)

第三次循环，

\(k=3\)，\(s=\frac{\frac32+1}{\frac32}=\frac53\)

此时 \(k=3\)，不满足 \(k<3\)，输出 \(s=\dfrac53\).
...
\begin{solution}
由约束条件，目标函数 \(x+2y\) 在可行域的顶点处取到最大值. 可行域的右上顶点为 \((3,3)\)，代入得
...
\(f(-x)=3^{-x}-\left(\dfrac13\right)^{-x}=-\left(3^x-\left(\dfrac13\right)^x\right)=-f(x)\)

所以 \(f(x)\) 是奇函数.

又因为 \(3^x\) 在 \(\R\) 上单调递增，而 \(\left(\dfrac13\right)^x=3^{-x}\) 在 \(\R\) 上单调递减，所以 \(f(x)=3^x-3^{-x}\) 在 \(\R\) 上单调递增.
...
\begin{center}
\bitmapinclude[max width=3.7cm,max totalheight=4.8cm]{img/2017/beijing_liberal/q6_fig1.png}
\end{center}
...
\begin{solution}
由三视图可得，该三棱锥的底面是直角三角形，底面两直角边长分别为 \(5\) 和 \(3\)，高为 \(4\).

所以体积为

\(V=\frac13\times\frac12\times5\times3\times4=10\)
...
\begin{solution}
若存在负数 \(\lambda\)，使得 \(m=\lambda n\)，则

\(m\cdot n=\lambda |n|^2<0\)

所以“存在负数 \(\lambda\)，使得 \(m=\lambda n\)”是“\(m\cdot n<0\)”的充分条件.

但当 \(m\cdot n<0\) 时，\(m\) 与 \(n\) 未必共线，因此不是必要条件.
...
（1）若教师人数为 \(4\)，则女学生人数的最大值为 \fillinblank{}. 

（2）该小组人数的最小值为 \fillinblank{}. 
...
\begin{solution}
设男学生人数为 \(m\)，女学生人数为 \(f\)，教师人数为 \(t\).

（1）当 \(t=4\) 时，由条件得 \(m>f>4\)，\(m<8\). 因为 \(m,f\) 都是整数，所以 \(m\le7\)，从而 \(f\le6\). 故女学生人数最大为 \(6\).

（2）要使总人数最小，只需使 \(t\) 尽可能小. 由 \(m<2t\) 且 \(m>f>t\) 可得，\(t=1,2\) 时无整数解；当 \(t=3\) 时，可取 \(m=5\)，\(f=4\). 此时总人数最小，为 \(5+4+3=12\).
...
\begin{center}
\bitmapinclude[max width=0.62\linewidth,max totalheight=4.8cm]{img/2017/beijing_liberal/q17_fig1.png}
\end{center}
\end{problem}
...
（2）设 \(M(m,n)\)，则 \(D(m,0)\)，\(N(m,-n)\)，且 \(m\ne\pm2\)，\(n\ne0\). 直线 \(AM\) 的斜率 \(k_{AM}=\dfrac{n}{m+2}\)，故直线 \(DE\) 的方程为 \(y=-\frac{m+2}{n}(x-m)\). 直线 \(BN\) 的方程为 \(y=\frac{n}{2-m}(x-2)\).
...
解得点 \(E\) 的纵坐标 \(y_E=-\frac{n(4-m^2)}{4-m^2+n^2}\). 

由点 \(M\) 在椭圆 \(C\) 上，得 \(\frac{m^2}{4}+n^2=1\)，即 \(4-m^2=4n^2\). 所以 \(y_E=-\frac45n\). 

又 \(S_{\triangle BDE}=\frac12|BD|\cdot|y_E|=\frac25|BD|\cdot|n|\)，\(S_{\triangle BDN}=\frac12|BD|\cdot|n|\). 所以 \(\triangle BDE\) 与 \(\triangle BDN\) 的面积之比为 \(4:5\).
\end{solution}
\begin{problem}
