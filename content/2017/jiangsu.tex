...
\begin{solution}
因为 \(A\cap B=\{1\}\)，所以 \(1\) 必须是集合 \(B\) 的元素. 由 \(B=\{a,a^2+3\}\)，可分两种可能：
\[
a=1\qquad\text{或}\qquad a^2+3=1
\]
第二种可能给出 \(a^2=-2\)，在实数范围内无解，因此只能有 \(a=1\). 此时
\(B=\{1,4\}\)，\(A\cap B=\{1,2\}\cap\{1,4\}=\{1\}\)
满足题设条件，故填 \(1\).
\end{solution}
...
由复数乘法及 \(\i^2=-1\)，
\[
z=(1+\i)(1+2\i)=1+2\i+\i+2\i^2=-1+3\i
\]
因此 \(z\) 的实部为 \(-1\)，虚部为 \(3\). 对于复数 \(u+v\i\)，其模为 \(\sqrt{u^2+v^2}\)，故
\[
...
\[
\frac{60}{1000}=\frac3{50}
\]
丙种型号有 \(300\) 件，所以应抽取
\[
300\times\frac3{50}=18
\]
件，故填 \(18\).
\end{solution}
...
\begin{solution}
输入 \(x=\dfrac1{16}<1\)，所以判断框中的条件 \(x\ge1\) 不成立，流程沿否定分支进入 \(y=2+\log_2x\). 又因为
\(\frac1{16}=2^{-4}\)，\(\log_2\frac1{16}=-4\)
所以
\[
...
\begin{solution}
设 \(t=\tan\alpha\). 由于题设中的 \(\tan(\alpha-\frac\pi4)\) 有意义，且题目要求的 \(\tan\alpha\) 也有意义，差角公式的分母不为零. 于是
\[
\tan\left(\alpha-\frac\pi4\right)
=\frac{\tan\alpha-\tan\frac\pi4}{1+\tan\alpha}
=\frac{t-1}{1+t}=\frac16
\]
...
\begin{solution}
设球的半径为 \(r>0\). 球与圆柱的上、下底面分别相切，故圆柱的高为 \(2r\). 在过圆柱轴线的截面内，球与圆柱的母线相切，故圆柱底面圆的半径也为 \(r\). 因而
\(V_1=\pi r^2\cdot2r=2\pi r^3\)，\(V_2=\frac43\pi r^3\).
消去 \(r\) 后
\[
...
\begin{solution}
将双曲线与标准方程 \(\dfrac{x^2}{a^2}-\dfrac{y^2}{b^2}=1\) 对照，得 \(a^2=3\)，\(b^2=1\). 因而
\[
c=\sqrt{a^2+b^2}=2
\]
右准线为 \(x=\dfrac{a^2}{c}=\dfrac32\)，两条渐近线为 \(y=\pm\dfrac ba x=\pm\dfrac{x}{\sqrt3}\). 所以
\(P=\left(\frac32,\frac{\sqrt3}{2}\right)\)，\(Q=\left(\frac32,-\frac{\sqrt3}{2}\right)\).
双曲线的两个焦点为 \(F_1=(-2,0)\)，\(F_2=(2,0)\). 四边形 \(F_1PF_2Q\) 的两条对角线 \(F_1F_2\) 与 \(PQ\) 分别水平、竖直，互相垂直，且
\(\lvert F_1F_2\rvert=4\)，\(\lvert PQ\rvert=\sqrt3\).
所以其面积为
\[
\frac12\lvert F_1F_2\rvert\,\lvert PQ\rvert=\frac12\cdot4\cdot\sqrt3=2\sqrt3
\]
\end{solution}
...
\begin{solution}
设等比数列的首项为 \(a_1\)，公比为 \(q\). 前六项可分成前后三项，后三项分别是前三项的 \(q^3\) 倍，因此
\[
S_6=S_3+q^3S_3=S_3(1+q^3)
\]
由 \(S_3=\dfrac74\ne0\) 及 \(S_6=\dfrac{63}{4}\)，得
\(1+q^3=\frac{S_6}{S_3}=9\)，\(q^3=8\).
因为 \(q\) 为实数，所以 \(q=2\). 再由
\[
...
\begin{solution}
每次购买 \(x\) 吨，全年购买 \(600\) 吨，故购买次数为 \(\dfrac{600}{x}\)，并且 \(x>0\). 一年的总运费与总存储费用之和为
\[
T(x)=6\cdot\frac{600}{x}+4x=\frac{3600}{x}+4x
\]
由基本不等式，两个加数均为正，故
\[
T(x)\ge2\sqrt{\frac{3600}{x}\cdot4x}=240
\]
等号成立当且仅当
\[
\frac{3600}{x}=4x,\qquad\text{即}\qquad x^2=900
\]
...
\begin{solution}
函数 \(x^3-2x\) 与 \(\e^x-\e^{-x}\) 都是奇函数，因此 \(f\) 是奇函数. 求导得
\[
f'(x)=3x^2-2+\e^x+\e^{-x}=3x^2+\left(\e^{x/2}-\e^{-x/2}\right)^2\ge0
\]
等号只在 \(x=0\) 时成立. 任意非退化区间上 \(f'\) 不恒为零，所以 \(f\) 在 \(\R\) 上严格递增.
...
再由 \(f\) 严格递增，得到
\[
2a^2\le1-a,\qquad\text{即}\qquad
2a^2+a-1\le0
\]
因式分解为 \((2a-1)(a+1)\le0\)，故
\[
a\in\left[-1,\frac12\right]
\]
\end{solution}
...
\begin{solution}
由 \(\tan\alpha=7\) 且 \(\alpha\) 为夹角，得
\(\cos\alpha=\frac1{\sqrt{1+\tan^2\alpha}}=\frac1{\sqrt{50}}=\frac1{5\sqrt2}\)，\(\sin\alpha=\frac7{\sqrt{50}}=\frac7{5\sqrt2}\).
...
=\frac{\cos\alpha-\sin\alpha}{\sqrt2}=-\frac35
\]
又因为 \(|\overrightarrow{OC}|=\sqrt2\)，故
\(\overrightarrow{OC}\cdot\overrightarrow{OA}=\sqrt2\cos\alpha=\frac15\)，\(qquad \overrightarrow{OC}\cdot\overrightarrow{OB}=\sqrt2\cos45^\circ=1\).
...
这里若 \(r=p/q\) 为既约分数，则 \(q\ge2\)；若 \(10^{p/q}\) 为有理数，其 \(q\) 次方的质因数分解中 \(2\) 和 \(5\) 的指数都应能被 \(q\) 整除，但这两个指数均为 \(p\)，矛盾. 因此上述无理性结论成立.

对每个 \(k\ge1\)，令 \(h_k(t)=\lg(k+t)-t\). 因为
\[
h_k'(t)=\frac1{(k+t)\ln10}-1<0
\]
所以 \(h_k\) 严格递减. 当 \(k=1\) 时，\(h_1(0)=0\)，且 \(h_1\) 严格递减，所以除 \(t=0\) 外没有根；当 \(k=2\)，\(3\)，\(\ldots\)，\(8\) 时，\(h_k(0)=\lg k>0\)，且 \(h_k(1)=\lg(k+1)-1<0\)，由连续性和严格递减性，每个 \(k\) 恰有一个 \(t\in(0,1)\) 使 \(h_k(t)=0\). 这个根不属于 \(D\)，否则与上面的无理性矛盾，因此它确实是原方程的根，共得到 \(7\) 个解. 当 \(k=9\) 时，\(h_9(0)>0\)，\(h_9(1)=\lg10-1=0\)，严格递减性说明 \(0\le t<1\) 内没有根.
...
\bitmapfigure[width=8.1cm]{img/2017/jiangsu/q15_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 直线 \(EF\) 在平面 \(ABD\) 内，且 \(EF\perp AD\). 又 \(AB\perp AD\)，所以平面 \(ABD\) 内过点 \(E\) 的 \(AD\) 垂线 \(EF\) 与 \(AB\) 平行，即 \(EF\parallel AB\). 两平面 \(ABD\) 与 \(ABC\) 的交线为 \(AB\)，而 \(E\) 在 \(AD\) 上且 \(E\ne A\)，故 \(EF\) 不在平面 \(ABC\) 内. 因此由线面平行的判定定理，\(EF\parallel\) 平面 \(ABC\).
\item 平面 \(ABD\perp\) 平面 \(BCD\)，两平面的交线为 \(BD\). 直线 \(BC\) 在平面 \(BCD\) 内且 \(BC\perp BD\)，所以由面面垂直的性质，\(BC\perp\) 平面 \(ABD\). 由于 \(AD\) 在平面 \(ABD\) 内，得到 \(BC\perp AD\). 又已知 \(AB\perp AD\)，而 \(AB\)、\(BC\) 是平面 \(ABC\) 内相交的两条直线，故 \(AD\perp\) 平面 \(ABC\). 最后 \(AC\subset\) 平面 \(ABC\)，所以 \(AD\perp AC\).
\end{enumerate}
\end{solution}
...
\bitmapfigure[width=11.2cm]{img/2017/jiangsu/q18_fig1.png}
\end{problem}
...
\(a_{3q+1}=a_1+qu\)，\(a_{3q+2}=a_2+qv\)，\(a_{3q+3}=a_3+qw\).
在 \(P(2)\) 等式中取 \(n=3q+3\)，有
\[
...
\(a_{3q+1}=a_1+3qd\)，\(a_{3q+2}=a_1+(3q+1)d\)，\(a_{3q+3}=a_1+(3q+2)d\).
...
\(r_1+r_2=-\dfrac{2a}{3}\)，\(r_1r_2=\dfrac b3\).
于是
\[
\begin{aligned}
...
由柯西不等式，
\[
(ac+bd)^2\le(a^2+b^2)(c^2+d^2)=4\cdot16=64
\]
于是 \(\lvert ac+bd\rvert\le8\)，从而必有
\[
ac+bd\le\lvert ac+bd\rvert\le8
\]
故 \(ac+bd\le8\).
\end{solution}
...
\(A=(0,0,0)\)，\(B=(2,0,0)\)，\(D=(-1,\sqrt3,0)\)，\(C=(1,\sqrt3,0)\)
\(A_1=(0,0,\sqrt3)\)，\(C_1=(1,\sqrt3,\sqrt3)\).
\begin{enumerate}
...
\begin{solution}
\begin{enumerate}
\item 令 \(N=m+n\). 随机逐个取球等价于在 \(N\) 个抽屉中等可能地选择 \(n\) 个位置放置黑球，共有 \(\mathrm C_N^n\) 种位置组合. 若编号为 \(2\) 的抽屉放黑球，其余 \(n-1\) 个黑球可从剩下的 \(N-1\) 个位置中任选，因此
\[
p=\frac{\mathrm C_{N-1}^{n-1}}{\mathrm C_N^n}
=\frac{n}{N}=\frac{n}{m+n}
\]
