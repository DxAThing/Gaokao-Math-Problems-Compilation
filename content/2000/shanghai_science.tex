...
\begin{solution}
线段 \(AB\) 的中点坐标为 \(\left(-\dfrac{9}{5},\dfrac{1}{5}\right)\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 函数 \(f(x)\) 的最小值为 \(\dfrac{7}{2}\).
\item 实数 \(a\) 的取值范围为 \(a>-3\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 可取指令 \(\left(4\sqrt{2},45^\circ\right)\).
\item 机器人可在点 \(P(7,0)\) 处截住小球，所需指令为 \(\left(5,-98.13^\circ\right)\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(b_n=2000\left(\dfrac{a}{10}\right)^{n+\frac{1}{2}}\).
\item \(5\left(\sqrt{5}-1\right)<a<10\).
\item \(n=20\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(m=\sqrt{3}\)，\(x'=x+\sqrt{3}y\)，\(y'=\sqrt{3}x-y\).
\item 点 \(Q\) 的轨迹方程为 \(y=(2-\sqrt{3})x-2\sqrt{3}+2\).
\item 存在这样的直线，方程为 \(y=\dfrac{\sqrt{3}}{3}x\) 或 \(y=-\sqrt{3}x\).
\end{enumerate}
\end{solution}
