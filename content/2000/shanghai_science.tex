...
\begin{problem}
在 \(xOy\) 平面上有一点列 \(P_{1}(a_{1},b_{1})\)，\(P_{2}(a_{2},b_{2})\)，\(\cdots\)，\(P_{n}(a_{n},b_{n})\)，\(\cdots\)，对每个自然数 \(n\)，点 \(P_{n}\) 位于函数 \(y = 2000 \cdot \left(\frac{a}{10}\right)^{2}\)（\(0 < a < 10\)）的图象上，且点 \(P_{n}\)，点 \((n,0)\) 与点 \((n + 1,0)\) 构成一个以 \(P_{n}\) 为顶点的等腰三角形.
\begin{enumerate}
...
\((a_n-n)^2+b_n^2=(a_n-n-1)^2+b_n^2\)，化简得 \(2a_n-2n-1=0\)，所以 \(a_n=n+\dfrac{1}{2}\). 又 \(P_n\) 在函数 \(y=2000\left(\dfrac{a}{10}\right)^2\) 的图象上，故
\(b_n=2000\left(\dfrac{a}{10}\right)^2\)，其中 \(b_n>0\).
\item 记 \(c=2000\left(\dfrac{a}{10}\right)^2\). 由 \(0<a<10\) 得 \(c>0\)，且 \(b_n=b_{n+1}=b_{n+2}=c\). 因此这三个数作为边长时三边均为正且相等，始终能构成等边三角形. 所以 \(a\) 的取值范围为 \((0,10)\).
\item 区间 \((0,10)\) 内的最小整数为 \(a=1\)，此时 \(b_n=2000\left(\dfrac{1}{10}\right)^2=20\). 因而 \(B_n=20^n\)，并且 \(B_{n+1}=20B_n>B_n\) 对每个正整数 \(n\) 都成立. 所以数列 \(\{B_n\}\) 没有最大项. 这说明原 PDF 中第（3）问的印刷指数 \(2\) 与官方解析采用的指数 \(n\) 不一致；若按题面指数 \(2\) 直译，第（3）问不存在有限的“最大项的项数”.
\end{enumerate}
\end{solution}
