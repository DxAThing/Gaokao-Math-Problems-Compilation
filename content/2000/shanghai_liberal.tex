...
对数的真数必须为正，所以定义域由
\(\frac{2x-1}{3-x}>0\)
确定. 临界点为 \(\dfrac12\) 和 \(3\)，作符号判断可知分式只在两临界点之间为正，故定义域为 \(\left(\dfrac12,3\right)\).
\end{solution}
...
这是中心为 \((1,0)\) 的双曲线，其中 \(a^2=16\)，\(b^2=9\). 双曲线的半焦距满足 \(c^2=a^2+b^2=25\)，所以 \(c=5\). 因而两个焦点为
\((1-5,0)=(-4,0)\) 和 \((1+5,0)=(6,0)\).
\end{solution}
...
\begin{solution}
点 \(Q(5,2)\) 在反函数 \(y=f^{-1}(x)\) 的图象上，当且仅当交换横、纵坐标后点 \((2,5)\) 在函数 \(y=f(x)\) 的图象上. 因此
\(5=f(2)=2^2+b=4+b\)，
解得 \(b=1\).
\end{solution}

\begin{problem}
根据上海市人大十一届三次会议上的市政府工作报告，\(1999\) 年上海市完成 \(\mathrm{GDP}\)（\(\mathrm{GDP}\) 是指国内生产总值）\(4035\text{ 亿元}\)，\(2000\) 年上海市 \(\mathrm{GDP}\) 预期增长 \(9\%\)，市委、市府提出本市常住人口每年的自然增长率将控制在 \(0.08\%\)，若 \(\mathrm{GDP}\) 与人口均按这样的速度增长，则要使本市年人均 \(\mathrm{GDP}\) 达到或超过 \(1999\) 年的 \(2\) 倍，至少需\fillinblank{}\(\text{年}\).按：\(1999\) 年本市常住人口总数约 \(1300\text{ 万}\).
\end{problem}
...
\begin{solution}
设从 \(1999\) 年起经过 \(t\) 年. 则该年的人均 \(\mathrm{GDP}\) 与 \(1999\) 年人均 \(\mathrm{GDP}\) 的比值为
\(\frac{4035\cdot1.09^t/(1300\cdot1.0008^t)}{4035/1300}=\left(\frac{1.09}{1.0008}\right)^t\).
令 \(r=\dfrac{1.09}{1.0008}=\dfrac{2725}{2502}>1\). 要达到两倍，需有 \(r^t\geqslant2\). 直接计算得
\(r^8\approx1.97986<2\)，\(r^9\approx2.15632>2\).
由于 \(r>1\)，最小整数 \(t\) 为 \(9\)，故至少需 \(9\) 年.
\end{solution}
...
反过来，若三条侧棱相等，即 \(DA=DB=DC\)，由直角三角形的勾股定理
\(DA^2=DH^2+HA^2\)，\(DB^2=DH^2+HB^2\)，\(DC^2=DH^2+HC^2\)，
可得 \(HA=HB=HC\). 正三角形中到三个顶点距离相等的点唯一，即为底面中心，所以 \(H\) 是底面中心. 因此与命题 A 等价的条件可以是“侧棱相等”.
\end{solution}
...
\begin{solution}
由图中 \(A(0,2)\)、\(B(1,1)\) 可知，在 \([0,1]\) 上
\(f(x)=2-x\).
对任意 \(x\in[1,2]\)，由周期为 \(2\) 及偶性分别有
\(f(x)=f(x-2)=f(2-x)\).
因为 \(2-x\in[0,1]\)，所以
\(f(x)=2-(2-x)=x\).
\end{solution}
...
\begin{solution}
展开式的通项为 \(T_{k+1}=\mathrm C_{11}^k x^{11-k}(-1)^k\)，其中 \(k=0\)，\(1\)，\(\ldots\)，\(11\). 所以第 \(k+1\) 项的系数为 \((-1)^k\mathrm C_{11}^k\). 负系数对应奇数 \(k\)，其绝对值依次为 \(\mathrm C_{11}^1=11\)，\(\mathrm C_{11}^3=165\)，\(\mathrm C_{11}^5=462\)，\(\mathrm C_{11}^7=330\)，\(\mathrm C_{11}^9=55\)，\(\mathrm C_{11}^{11}=1\).
其中最大值为 \(462\)，故系数最小的项的系数为 \(-462\).
\end{solution}
...
\begin{solution}
可行域的边界交点为 \((0,0)\)，\((3,0)\)，\((0,5)\)，\((1,4)\)，
其中 \((1,4)\) 由 \(x+y=5\) 与 \(2x+y=6\) 联立得到. 目标函数为线性函数，只需比较各顶点处的值：
\(k(0,0)=0\)，\(k(3,0)=18\)，\(k(0,5)=40\)，\(k(1,4)=38\).
最大值在 \((0,5)\) 处取得，所以所求点的坐标为 \((0,5)\).
\end{solution}
...
\begin{answer}
\(b_1b_2\cdots b_n=b_1b_2\cdots b_{17-n}\)，\((n<17,\ n\in\N^{*})\)
\end{answer}

\begin{solution}
设等比数列的首项为 \(b_1\)，公比为 \(q\). 由 \(b_9=1\) 得
\(b_1q^8=1\)，
故 \(b_1=q^{-8}\)，且 \(q\neq0\). 记前 \(n\) 项积为 \(P_n\)，则
\[
...
=q^{\frac{n(n-17)}2}
\]
另一方面，\(P_{17-n}=q^{\frac{(17-n)(17-n-17)}2}=q^{\frac{n(n-17)}2}=P_n\).
所以
\(b_1b_2\cdots b_n=b_1b_2\cdots b_{17-n}\)，其中 \(n<17\) 且 \(n\in\N^{*}\).
\end{solution}
...
\begin{solution}
由三角恒等式
\(y=\sin\left(x+\frac\pi2\right)=\cos x\).
定义域 \(\left[-\dfrac\pi2,\dfrac\pi2\right]\) 关于原点对称，且 \(\cos(-x)=\cos x\)，所以该函数是偶函数，选择 C.
\end{solution}
...
对于命题 2，取 \(\alpha:x=0\)、\(\beta:y=0\)，令直线 \(a\) 为 \(\{(1,1,t)\mid t\in\R\}\). 直线 \(a\) 同时平行于两个平面，而 \(\alpha\) 与 \(\beta\) 相交，故命题 2 错误.
...
\begin{solution}
因为指数函数 \(3^x\) 的值域为 \((0,+\infty)\)，所以 \(S=(0,+\infty)\). 又因为 \(x^2\geqslant0\)，集合 \(T\) 的值域为 \([-1,+\infty)\). 因而
\(S\cap T=(0,+\infty)=S\)，
选择 A.
\end{solution}
...
A. 点 \(P(a,2a)\) 到原点的距离为 \(\sqrt5|a|\)，故
\(\sin\alpha=\frac{2a}{\sqrt5|a|}\).
当 \(a<0\) 时该值为负，不恒等于 \(\dfrac{2\sqrt5}{5}\)，所以 A 错误.

B. 同时满足给定正弦、余弦值的角为
\(\alpha=\frac\pi6+2k\pi\)，其中 \(k\in\Z\)，
不止一个，所以 B 错误.

C. 当 \(a<0\) 且 \(|a|<1\) 时，\(\arcsin a\in\left(-\dfrac\pi2,0\right)\)，从而 \(\tan(\arcsin a)<0\)，所以 C 错误.

D. 由正切函数的周期性，
\(\tan\left(x+\frac\pi3\right)=\sqrt3\Longleftrightarrow x+\frac\pi3=\frac\pi3+k\pi\)，其中 \(k\in\Z\)，
即 \(x=k\pi\). 所以 D 正确，选择 D.
\end{solution}
...
\(\frac{\sqrt{(x-4)^2+16}}{v}=\frac{17-x}{2v}\)，从而 \(4\bigl((x-4)^2+16\bigr)=(17-x)^2\).
...
\(x+\sqrt{3}y=\sqrt{3}\)，\(\sqrt{3}x-y=2\).
