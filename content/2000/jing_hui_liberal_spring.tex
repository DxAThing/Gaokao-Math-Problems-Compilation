...
\begin{problem}
在空间，下列命题正确的是\fillinblank{}.（注：把你认为正确的命题的序号都填上）

\begin{center}
\begin{tabular}{@{}p{0.68\linewidth}p{0.25\linewidth}@{}}
\(\circled{1}\) 如果两直线 \(a\)，\(b\) 分别与直线 \(l\) 平行， & 那么 \(a \parallel b\)； \\
\(\circled{2}\) 如果直线 \(a\) 与平面 \(\beta\) 内的一条直线 \(b\) 平行， & 那么 \(a \parallel \beta\)； \\
\(\circled{3}\) 如果直线 \(a\) 与平面 \(\beta\) 内的两条直线 \(b\)，\(c\) 都有垂直， & 那么 \(a \perp \beta\)； \\
\(\circled{4}\) 如果平面 \(\beta\) 内的一条直线 \(a\) 垂直平面 \(\gamma\)， & 那么 \(\beta \perp \gamma\).
\end{tabular}
\end{center}
\end{problem}
