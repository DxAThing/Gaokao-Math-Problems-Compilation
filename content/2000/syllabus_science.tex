...
=\left(\frac32-\frac32\right)-\left(\frac{3\sqrt{3}}2+\frac{\sqrt{3}}2\right)\i
=-2\sqrt{3}\i
\]
故选 B.
\end{solution}
...
=\frac12\lg(ab)=\frac{\lg a+\lg b}{2}=Q
\]
因此 \(P<Q<R\)，故选 B.
\end{solution}
...
\begin{solution}
极坐标点 \((1,1)\) 表示极径为 \(1\)、极角为 \(1\) 的点，故其直角坐标为 \((\cos1,\sin1)\). 设圆上任意一点的极坐标为 \((\rho,\theta)\)，由圆心距等于半径，有
\[
(\rho\cos\theta-\cos1)^2+(\rho\sin\theta-\sin1)^2=1
\]
展开并利用 \(\sin^2 1+\cos^2 1=1\)，得
\[
\rho^2-2\rho(\cos\theta\cos1+\sin\theta\sin1)=0
\]
即 \(\rho^2-2\rho\cos(\theta-1)=0\). 除去表示极点的因子 \(\rho=0\)，圆的极坐标方程为
\[
\rho=2\cos(\theta-1)
\]
故选 C.
...
\]
从而 \(k^2=\frac13\)，即 \(k=\pm\frac{\sqrt{3}}3\). 对直线 \(kx-y=0\)，圆心 \((-2,0)\) 到直线的垂足为切点，故切点坐标为
\[
(-2,0)-\frac{-2k}{k^2+1}(k,-1)
=\left(-\frac{2}{k^2+1},-\frac{2k}{k^2+1}\right)
\]
当 \(k=\frac{\sqrt{3}}3\) 时，切点为 \(\left(-\frac32,-\frac{\sqrt{3}}2\right)\)，在第三象限；当 \(k=-\frac{\sqrt{3}}3\) 时，切点为 \(\left(-\frac32,\frac{\sqrt{3}}2\right)\)，在第二象限. 因此所求直线为 \(y=\frac{\sqrt{3}}3x\)，故选 C.
\end{solution}
...
旋转线段 \(OA\) 所得的曲面把原圆锥分成两部分. 含有轴截面三角形 \(VOA\) 的那一部分，在旋转后由以 \(O\)、\(V\) 为顶点、以过 \(A\) 的圆为底面的两个圆锥组成；它们的底面半径均为 \(x_A\)，高分别为 \(y_A\) 和 \(h-y_A\)，其中 \(y_A\) 为点 \(A\) 的纵坐标. 因此该部分体积为
\[
\frac13\pi x_A^2y_A+\frac13\pi x_A^2(h-y_A)=\frac13\pi h x_A^2
\]
原圆锥体积为 \(\frac13\pi h r^2\). 由题意上述部分占原圆锥体积的一半，因此
\[
...
\begin{solution}
取正方体棱长为 \(1\)，设
\(A=(0,0,0)\)，\(B=(1,0,0)\)，\(D_1=(0,1,1)\)，\(E=\left(0,\frac12,\frac12\right)\)，\(F=\left(1,\frac12,\frac12\right)\).
在两个与面 \(ADD_1A_1\)、\(BCC_1B_1\) 平行的面上作射影时，\(E\) 与 \(F\) 的射影重合，\(B\) 与 \(D_1\) 的射影分别为该面的两个对角顶点，所以四边形的射影退化为一条对角线，对应图 \(\circled{3}\).

在其余四个面上作射影时，以相应面的两条棱为坐标轴，四个投影点按顶点顺序均可写成
\(\left(1,0\right)\)，\(\left(1,\frac12\right)\)，\(\left(0,1\right)\)，\(\left(0,\frac12\right)\).
相邻两点依次连接后，得到两条平行的短边和两条斜边，即图 \(\circled{2}\) 所示的四边形. 因此可能的图序号为 \(\circled{2}\)、\(\circled{3}\).
\end{solution}
...
\(CM\perp BD\)，\(CM=\sqrt{3}\).
由 \(M\) 为 \(BD\) 的中点，
\(\overrightarrow{CM}=\frac{\bs{b}+\bs{d}}{2}\)，\(\bs{c}\cdot\overrightarrow{CM}=\frac{\bs{c}\cdot\bs{b}+\bs{c}\cdot\bs{d}}{2}=\frac{3}{2}\).
于是
\[
...
\(CM=\frac{\sqrt{3}}{2}s\)，\(CM^2=\frac34s^2\)
并且
\[
...
\item 按等比数列的定义，设 \(\{a_n\}\) 和 \(\{b_n\}\) 的公比分别为 \(r\) 和 \(s\)，其中 \(r\ne s\)，且各项均非零. 则
\(c_{n+1}=ra_n+sb_n\)，\(c_{n+2}=r^2a_n+s^2b_n\).
于是
\[
c_{n+1}^2-c_nc_{n+2}
...
\bitmapfigure[max width=0.75\linewidth]{img/2000/syllabus_science/q21_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 由图一，市场售价折线经过 \((0,300)\)、\((200,100)\)、\((300,300)\). 前一段斜率为
\(\frac{100-300}{200-0}=-1\)，后一段斜率为
\(\frac{300-100}{300-200}=2\)，所以
\[
p=f(t)=\begin{cases}
...
\end{cases}
\]
由图二，成本曲线的顶点为 \((150,100)\)，且经过 \((50,150)\)，可设
\[
Q=g(t)=k(t-150)^2+100
\]
代入 \((50,150)\)，得
...
\]
所以 \(k=\frac{1}{200}\)，从而
\(g(t)=\frac{1}{200}(t-150)^2+100\)，\((0\leqslant t\leqslant300)\).
...
\(\frac{x^2}{a^2}-\frac{y^2}{b^2}=1\)，\(c^2=a^2+b^2\)，\(e=\frac{c}{a}\).
由图中 \(CD\parallel AB\)，且 \(C\)，\(D\) 在双曲线的两支上，设它们的纵坐标均为 \(y_0\). 双曲线方程关于 \(y\) 轴对称，所以 \(C\)，\(D\) 的横坐标互为相反数. 又
\[
CD=\frac{AB}{2}=c
\]
故可设
\(C\left(\frac{c}{2},y_0\right)\)，\(D\left(-\frac{c}{2},y_0\right)\).
由点 \(C\) 在双曲线上，
\[
...
\]
化简可得
\(e^2(1-\lambda)+\lambda^2=1+2\lambda+\lambda^2\)，\(e^2=\frac{1+2\lambda}{1-\lambda}\).
