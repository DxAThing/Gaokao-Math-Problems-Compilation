...
\begin{problem}
一个长方体其一顶点的三个面的面积分别是 \(\sqrt{2}, \sqrt{3}, \sqrt{6}\)，这个长方体对角线的长是（\quad）
...
\begin{problem}
《中华人民共和国个人所得税法》规定，公民全月工资、薪金所得不超过 \(800\mathrm{元}\) 的部分不必纳税. 超过 \(800\mathrm{元}\) 的部分为全月应纳税所得额，此项税款按下列标准分段累进计算：

\begin{circlelist}
\item 不超过 \(500\mathrm{元}\) 的部分，税率为 \(5\%\)；
\item 超过 \(500\mathrm{元}\) 至 \(2000\mathrm{元}\) 的部分，税率为 \(10\%\)；
\item 超过 \(2000\mathrm{元}\) 至 \(5000\mathrm{元}\) 的部分，税率为 \(15\%\).
\end{circlelist}

某人一月份交纳此项税款 \(26.78\mathrm{元}\)，则他的当月工资、薪金所得介于（\quad）

\choices
    {\(800\mathrm{元}\) 至 \(900\mathrm{元}\)}
    {\(900\mathrm{元}\) 至 \(1200\mathrm{元}\)}
    {\(1200\mathrm{元}\) 至 \(1500\mathrm{元}\)}
    {\(1500\mathrm{元}\) 至 \(2800\mathrm{元}\)}
\end{problem}

\begin{problem}
若 \(a > b > 1, P = \sqrt{\lg a - \lg b}, Q = \frac{1}{2} (\lg a + \lg b), R = \lg \left(\frac{a + b}{2}\right)\)，则（\quad）
...
\begin{problem}
椭圆 \(\frac{x^{2}}{a^{2}} + \frac{y^{2}}{b^{2}} = 1\) 的焦点 \(F_{1}\)、\(F_{2}\)，点 \(P\) 为其上的动点，当 \(\angle F_{1}PF_{2}\) 为钝角时，点 \(P\) 横坐标的取值范围是 \fillinblank{} \fillinblank{}.
\end{problem}
...
\item 当 \(\frac{C_{1}C}{CC_{1}}\) 的值为多少时，能使 \(A_{1}C \perp\) 平面 \(C_{1}BD\)？请给出证明.
\end{enumerate}
\end{problem}
