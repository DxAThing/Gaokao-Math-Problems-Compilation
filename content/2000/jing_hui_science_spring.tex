...
\begin{solution}
第一种参数式中，为使参数式有意义需取 \(t>0\)，所以 \(x=\sqrt{t}>0\)，只能表示曲线的第一象限部分. 第二种参数式中 \(x=\sin\alpha\)，因此 \(-1\leq x\leq1\)，且 \(x\neq0\)，不能表示所有非零的 \(x\)；第三种参数式同理，因为 \(x=\cos\alpha\) 也满足 \(-1\leq x\leq1\)，所以前面三种参数式都不能表示整条曲线. 第四种参数式中，取 \(\tan\alpha\neq0\)，则 \(\tan\alpha\) 能取遍所有非零实数，并且 \(x=\tan\alpha\)，\(y=\cot\alpha=1/x\)，从而 \(xy=1\) 且覆盖曲线上的每一点，故选 D.
\end{solution}
...
\bitmapfigure[float=inline,max width=0.45\linewidth]{img/2000/jing_hui_science_spring/q14_fig1.png}
\FloatBarrier
...
\begin{solution}
命题 \(\circled{1}\) 中，两条直线都与同一直线平行，因而方向相同且共面，所以 \(a\parallel b\)，正确.

命题 \(\circled{2}\) 不正确. 例如取 \(\beta\) 为平面 \(z=0\)，取平面内两条不同的平行直线 \(b:y=0\)，\(z=0\) 和 \(a:y=1\)，\(z=0\)，则 \(a\parallel b\)，但 \(a\subset\beta\)，不能说 \(a\parallel\beta\).

命题 \(\circled{3}\) 不正确. 在平面 \(\beta:z=0\) 内取 \(b:y=0\)，\(z=0\)、\(c:y=1\)，\(z=0\) 和 \(a:x=0\)，\(z=0\)，则 \(a\) 分别与 \(b\)、\(c\) 垂直，但 \(a\subset\beta\)，所以 \(a\) 不垂直于平面 \(\beta\). 这说明仅有“垂直于两条直线”还不足以推出垂直于平面.

命题 \(\circled{4}\) 正确. 因为平面 \(\beta\) 内含有直线 \(a\)，且 \(a\perp\gamma\)，根据“一个平面经过另一个平面的一条垂线，则两平面互相垂直”的判定定理，得 \(\beta\perp\gamma\). 因此填 \(\circled{1}\)，\(\circled{4}\).
\end{solution}
...
于是 \(x^{2}+y^{2}=\frac{64p^{2}}{L^{2}+4}=4px\)，所以轨迹满足 \(x^{2}+y^{2}-4px=0\). 当 \(L=0\) 时，\(AB\) 为竖直线，公式给出 \(M=(4p,0)\)，因此没有遗漏斜率不存在的情形. 反之，任意实数 \(L\) 都能取为方程 \(v^{2}-Lv-4=0\) 的两根之和，且两根乘积为 \(-4\)，故上述参数能得到圆上除原点外的所有点. 因而轨迹是方程 \(x^{2}+y^{2}-4px=0\) 所表示的圆去掉原点，即圆心为 \((2p,0)\)、半径为 \(2p\) 的圆去掉原点.
\end{solution}
...
\item 上年度收益为 \(a(0.8-0.3)=0.5a\)，本年度收益至少增长 \(20\%\)，所以必须有 \(y\geq1.2\times0.5a=0.6a\). 这里年用电量 \(a>0\). 令 \(k=0.2a\)，并设 \(u=x-0.4\)，则 \(u\in[0.15,0.35]\)，且 \(\frac{y}{a}=\left(1+\frac{0.2}{u}\right)(u+0.1)=u+0.3+\frac{0.02}{u}\).

由 \(y\geq0.6a\) 且 \(u>0\)，得 \(u+0.3+\frac{0.02}{u}\geq0.6\)，即 \(u^{2}-0.3u+0.02\geq0\)，也就是 \((u-0.1)(u-0.2)\geq0\). 在 \([0.15,0.35]\) 内只能有 \(u\geq0.2\)，所以允许的实际电价必须满足 \(x\in[0.6,0.75]\). 因而把最低电价定为 \(0.6\text{ 元}/(\mathrm{kW}\cdot\mathrm{h})\) 时，区间内每个实际电价都能保证收益至少增长 \(20\%\)，且该下限不能再降低.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 当 \(0\leq x<\frac{1}{2}\) 时，\(f(x)=f_{1}(x)\)，且 \(f_{1}'(x)=2-4x>0\)，同时 \(f_{1}(0)=\frac{1}{2}\)，\(\lim_{x\to(1/2)^{-}}f_{1}(x)=1\)，所以图象是抛物线 \(y=-2\left(x-\frac{1}{2}\right)^{2}+1\) 从实心点 \(\left(0,\frac{1}{2}\right)\) 出发的相应部分，单调上升并在 \(\left(\frac{1}{2},1\right)\) 处留下开端点. 当 \(\frac{1}{2}\leq x\leq1\) 时，\(f(x)=f_{2}(x)=-2x+2\)，图象是从实心点 \(\left(\frac{1}{2},1\right)\) 到实心点 \((1,0)\) 的线段. 因此图象由上述抛物线段和线段组成，在 \(\left(\frac{1}{2},1\right)\) 处取线段的实心点.

\item 由 \(y=-2x+2\)，得反函数 \(g(x)=1-\frac{x}{2}\)，其定义域为 \([0,1]\)，值域为 \([\frac{1}{2},1]\). 因为 \(a_{1}=1\in[0,1]\)，且 \(g\) 把 \([0,1]\) 映到 \([\frac{1}{2},1]\subset[0,1]\)，所以递推中的每一项都有定义. 数列满足 \(a_n=1-\frac{a_{n-1}}{2}\)，\((n\geq2)\)，且 \(a_1=1\). 设该递推的平衡值为 \(L\)，则 \(L=1-\frac{L}{2}\)，所以 \(L=\frac{2}{3}\). 因而 \(a_n-L=-\frac{1}{2}(a_{n-1}-L)\)，从而 \(a_n-\frac{2}{3}=\left(-\frac{1}{2}\right)^{n-1}\left(1-\frac{2}{3}\right)\). 所以 \(a_n=\frac{2}{3}+\frac{1}{3}\left(-\frac{1}{2}\right)^{n-1}\)，并且 \(\displaystyle\lim_{n\to\infty}a_n=\frac{2}{3}\).
