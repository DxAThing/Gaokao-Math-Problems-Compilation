...
\end{solution}
\begin{problem}
不等式 \((x - 1)(x - 3) < 0\) 的解集为 \fillinblank{} \fillinblank{}.
\end{problem}
...
\end{solution}
\begin{problem}
已知等差数列 \(\{a_n\}\) 的首项 \(a_1 = -3\)，公差 \(d = 2\)，则该数列的前 \(6\) 项和为 \fillinblank{} \fillinblank{}.
\end{problem}
...
\end{solution}
\begin{problem}
在二项式 \((2x-1)^5\) 的展开式中，\(x^3\) 的系数为 \fillinblank{} \fillinblank{}.
\end{problem}
...
\begin{solution}
二项式 \((2x-1)^5\) 中 \(x^3\) 的系数对应 \(r=2\)，\(\mathrm{T}_{3}=C_5^2(2)^3(-1)^2=80\).
\end{solution}
\begin{problem}
函数 \(y=\cos x\) 在 \([-\pi/2,\pi/4]\) 上的值域为 \fillinblank{} \fillinblank{}.
\end{problem}
...
\end{solution}
\begin{problem}
已知随机变量 \(X\) 的分布为 \((5,6,7)\to (0.2,0.3,0.5)\)，则期望 \(E[X]=\)  \fillinblank{}.
\end{problem}
...
\end{solution}
\begin{problem}
如图，在正四棱柱 \(ABCD- A_1B_1C_1D_1\) 中，\(BD=4\sqrt2\)，\(D B_1=9\)，则该正四棱柱的体积为 \fillinblank{}. 

\bitmapfigure[max width=4.2cm,max totalheight=4.8cm]{img/2025/shanghai/q7_fig1.png}
...
\begin{solution}
因为 \(BD=4\sqrt2\)，且 \(ABCD\) 为正方形，所以 \(BA=4\). 又 \(DB_1=9\)，所以 \(BB_1^2+BD^2=9^2\)，得 \(BB_1=7\). 底面积为 \(BA\cdot BC=16\)，体积 \(=16\times7=112\).
\end{solution}
\begin{problem}
已知 \(a,b>0,\ a+b=1\)，则 \(\frac1a+\frac1b\) 的最小值为 \fillinblank{} \fillinblank{}.
\end{problem}
...
\begin{solution}
由题设，
\(\frac1a+\frac1b=\frac{a+b}{ab}=\frac{1}{ab}\).
因为 \(a,b>0,a+b=1\)，由 \(ab\le \left(\frac{a+b}{2}\right)^2=\frac14\)，故 \(\frac{1}{ab}\ge4\). 当 \(a=b=\frac12\) 时取等号，故最小值为4.
\end{solution}
\begin{problem}
\(4\) 个家长和 \(2\) 个儿童去爬山，\(6\) 个人需要排成一条队列，要求队列的头和尾均是家长，则不同的排列个数有  \fillinblank{}.
\end{problem}
...
\begin{solution}
先安排首尾两位家长，有 \(P_4^2=12\) 种；再安排中间 \(4\) 人，有 \(P_4^4=24\) 种，故共 \(12\times24=288\) 种.
\end{solution}
\begin{problem}
已知复数 \(z\) 满足 \(z^2=(\bar z)^2\)，\(|z|\le1\)，则 \(|z-2-3\i|\) 的最小值是 \fillinblank{} \fillinblank{}.
\end{problem}
...
\end{solution}
\begin{problem}
小申同学观察发现，生活中有些时候影子可以完全投射在斜面上. 某斜面上有两根长为1米的垂直于水平面放置的杆子，与斜面的接触点分别为 \(A,B\)，它们在阳光的照射下呈现出影子，阳光可视为平行光：其中一根杆子的影子在水平面上，长度为 \(0.4\) 米；另一根杆子的影子完全在斜面上，长度为 \(0.45\) 米. 则斜面的底角 \(\theta=\) . 

\bitmapfigure[max width=3.8cm,max totalheight=4.8cm]{img/2025/shanghai/q11_fig1.png}
...
\begin{solution}
\bitmapfigure[max width=4.8cm,max totalheight=4.8cm]{img/2025/shanghai/q11_solution_fig1.png}

先由 \(A\) 点杆子算出 \(\tan x=\frac{1}{0.4}=2.5\).

再由 \(B\) 点杆子算出 \(\tan\angle CED=2.5\)，
其中 \(ED\) 平行水平面，\(CE\) 为杆顶处光线，在斜面上与 \(E\) 处相交.

设 \(BD=y\)，则 \(ED=\frac{1+y}{2.5}\).
...
\end{solution}
\begin{problem}
已知函数 \(f(x)\) 满足：当 \(x>0\) 时 \(f(x)=1\)，当 \(x=0\) 时 \(f(x)=0\)，当 \(x<0\) 时 \(f(x)=-1\). \(\bs{a},\bs{b},\bs{c}\) 分别为平面内三个不同的单位向量. 若 \(f(\bs{a}\cdot\bs{b})+f(\bs{b}\cdot\bs{c})+f(\bs{c}\cdot\bs{a})=0\)，则 \(|\bs{a}+\bs{b}+\bs{c}|\) 的可取值范围是 \fillinblank{} \fillinblank{}.
\end{problem}
...
\end{solution}
\begin{problem}
设 \(a>0\)，\(s\in\R\)，下列各项中，能推出 \(a^s>a\) 的是（\quad）

\noindent
\choiceentry{A}{\(a>1\) 且 \(s>0\)}

\choiceentry{B}{\(a>1\) 且 \(s<0\)}

\choiceentry{C}{\(0<a<1\) 且 \(s>0\)}

\choiceentry{D}{\(0<a<1\) 且 \(s<0\)}
...
\noindent
\choiceentry{A}{有最大值，但没有最小值}

\choiceentry{B}{没有最大值，但有最小值}

\choiceentry{C}{既有最大值，也有最小值}

\choiceentry{D}{既没有最大值，也没有最小值}
...
\end{solution}
\begin{problem}
已知数列 \(\{a_n\},\{b_n\},\{c_n\}\) 的通项分别为 \(a_n=10n-9\)，\(b_n=2^n\)，\(c_n=\lambda a_n+(1-\lambda)b_n\). 若对任意 \(\lambda\in[0,1]\)，\(a_n,b_n,c_n\) 可构成三角形，则满足条件的正整数 \(n\) 有（\quad）
...
\begin{problem}
2024 年东京奥运会，中国获得了男子 \(4\times100\) 米混合泳接力冠军成绩（单位：秒）如下（按升序）：\(206.78,\ 207.46,\ 207.95,\ 209.34,\ 209.35,\ 210.68,\ 213.73,\ 214.84,\ 216.93,\ 216.93\).

\begin{enumerate}
\item 求这组数据的极差与中位数；
\item 从这 10 个数据中任选 3 个，求恰有 2 个数据在 \(211\) 以上的概率；
\item 若比赛成绩 \(y\) 关于年份 \(x\) 的回归方程为 \(y=-0.311x+\hat b\)，\(x\) 的平均数为 \(2006\)，预测 \(2028\) 年冠军队成绩（精确到 \(0.01\) 秒）.
\end{enumerate}
...
（2）设恰有 2 个数据在 \(211\) 以上，事件 \(A\). 则从 \(4\) 个不小于 \(211\) 的数中选两个，且从剩下 \(6\) 个中选一个：
\[
P(A)=\frac{C_4^2C_6^1}{C_{10}^3}=\frac{36}{120}=\frac3{10}
\]
（3）先求样本均值
\[
\begin{aligned}
...
&\quad+\frac{210.68+213.73+214.84+216.93+216.93}{10}\\
&=211.399.
\end{aligned}
\]
...
\bitmapfigure[max width=4.3cm,max totalheight=4.8cm]{img/2025/shanghai/q18_fig1.png}

\begin{enumerate}
\item 若直线 \(PA\) 与圆锥底面所成角为 \(\pi/3\)，求圆锥侧面积；
\item 已知 \(Q\) 是母线 \(PA\) 中点，点 \(C,D\) 在底面圆周上，且弧 \(AC\) 长为 \(\pi/3\)，\(CD\parallel AB\). 设 \(M\in OC\)，证明 \(QM\parallel PBD\) 所在平面.
\end{enumerate}
...
（2）在三角形 \(PAB\) 中，\(Q\) 与 \(O\) 分别是 \(PA\) 与 \(AB\) 的中点，所以
\(QO\parallel PB\).
...
又 \(M\in OC\)，故 \(QM\subset\Pi_{QOC}\)，从而
\(QM\parallel\Pi_{PBD}\).
\end{solution}
\begin{problem}
...
\item 已知 \(AM\) 的中垂线 \(l\) 的斜率为2， \(l\) 与 \(\Gamma\) 交于 \(C,D\)，且 \(\angle CMD\) 为钝角，求 \(a\) 的取值范围.
\end{enumerate}
...
（2）此时 \(A(4,0)\)，设 \(P(x_P,y_P)\). 由 \(\overrightarrow{PA}=2\overrightarrow{MP}\) 得
\(4-x_P=2x_P\)，\(-y_P=2(y_P-m)\).
所以 \(x_P=\frac43\)，\(y_P=\frac{2m}{3}\). 代入椭圆方程 \(\frac{x^2}{16}+\frac{y^2}{5}=1\) 得 \(\frac{1}{9}+\frac{4m^2}{45}=1\)，故 \(m=\sqrt{10}\).

（3）由 \(l\) 为 \(AM\) 的中垂线且斜率为 \(2\)，知 \(AM\) 的斜率为 \(-\frac12\). 又 \(k_{AM}=\frac{m-0}{0-a}=-\frac{m}{a}\)，故 \(m=\frac a2\). 于是 \(AM\) 的中点为 \(\left(\frac a2,\frac a4\right)\)，直线 \(l\) 的方程为 \(y-\frac a4=2\left(x-\frac a2\right)\)，即 \(y=2x-\frac{3a}{4}\).
...
设 \(f(-3)=n\). 若 \(n\in(0,1)\)，则 \(-3+n\in(-3,-2)\)，从而
\[
f(-3+n)=(-3+n)+3=n=f(-3)
\]
故 \(-3\in M_n\subset M_2\)，于是 \(f(-1)=f(-3)=n\).
