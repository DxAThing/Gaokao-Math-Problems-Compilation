...
\item 根据题意可知：生产线智能化升级改造后，该工厂产品的优级品的频率为
\(\frac{96}{150}=0.64\).
...
\begin{solution}
\begin{enumerate}
\item 因为 \( BC\parallel AD \)，\( BC=2 \)，\( AD=4 \)，\(M\) 为 \(AD\) 的中点，所以 \( BC\parallel MD \)，\( BC=MD \).
四边形 \(BCDM\) 为平行四边形，所以 \( BM\parallel CD \)，又因为 \( BM\nsubseteq \) 平面 \(CDE\)，\( CD\subset \) 平面 \(CDE\)，所以 \( BM\parallel \)平面 \(CDE\).

\item 作 \( BO\perp AD \) 交 \(AD\) 于 \(O\)，连接 \(OF\).
...
\begin{solution}
\begin{enumerate}
\item 设 \( F(c,0) \)，由题设有 \( c=1 \) 且 \( \frac{b^2}{a^2}=\frac{3}{4} \)，故
\(
...
\begin{solution}
\begin{enumerate}
\item 当 \( a=-2 \) 时，\( f(x)=(1+2x)\ln(1+x)-x \).
故
\(
...
\begin{solution}
\begin{enumerate}
\item 由 \( \rho=\rho\cos\theta+1 \)，将 \( \rho=\sqrt{x^2+y^2} \) 和 \( \rho\cos\theta=x \) 代入，可得
\(\sqrt{x^2+y^2}=x+1\).
两边平方后可得曲线的直角坐标方程为
\(y^2=2x+1\).

\item 对于直线 \( l \) 的参数方程消去参数 \( t \)，得直线的普通方程为
\(y=x+a\).
