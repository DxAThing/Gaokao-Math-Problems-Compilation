...
\[
\lim_{x\to0^-}f(x)=-a\le f(0)=\e+\ln1=1
\]
...
\bitmapfigure[max width=6.8cm,max totalheight=4.8cm]{img/2024/new_gaokao_paper_1/q7_fig1.png}
...
\bitmapfigure[max width=6.0cm,max totalheight=4.8cm]{img/2024/new_gaokao_paper_1/q11_fig1.png}
...
\bitmapfigure[max width=6.0cm,max totalheight=4.8cm]{img/2024/new_gaokao_paper_1/q12_fig1.png}
...
\bitmapfigure[max width=6.0cm,max totalheight=4.8cm]{img/2024/new_gaokao_paper_1/q17_fig1.png}

\end{problem}

\begin{solution}
（1）因为 \(PA\perp\) 底面 \(ABCD\)，且 \(AD\subset\) 底面 \(ABCD\)，故 \(PA\perp AD\).
...
（2）由 \(AD\perp DC\) 且 \(AC=2\)，设 \(AD=x\)，则
\(CD=\sqrt{4-x^2}\).
过点 \(D\) 作 \(DE\perp AC\) 于 \(E\)，再过点 \(E\) 作 \(EF\perp CP\) 于 \(F\)，连接 \(DF\).

\bitmapfigure[max width=3.6cm,max totalheight=4.8cm]{img/2024/new_gaokao_paper_1/q17_solution_fig1.png}
...
\begin{solution}
（1）可将原等差数列按比例平移缩放为 \(1,2,\dots,4m+2\) 的数列讨论. 对 \(m=1\) 即 \(1,\dots,6\)，剩下 4 个数必须是 \(1,2,3,4\) 或 \(2,3,4,5\) 或 \(3,4,5,6\)，故 \((i,j)\) 仅有
\((1,2)\)，\((1,6)\)，\((5,6)\).

（2）对 \(m\ge3\)，去掉 \(2,13\) 后，\(\{1,\dots,4m+2\}\) 中余下 \(4m\) 个数可分成
