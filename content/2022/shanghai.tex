...
\bitmapinlinefigure[0.42\linewidth][max width=0.42\linewidth]{img/2022/shanghai/q15_fig1.png}
...
\begin{solution}
\begin{enumerate}
\item 由 \(O\) 为 \(AC\) 的中点，得 \(AO=1\). 又因为 \(PO\perp\) 底面 \(ABC\)，所以 \(PO\perp AO\). 在直角三角形 \(AOP\) 中，
\[
...
\item 以 \(O\) 为原点，\(AC\) 所在直线为 \(x\) 轴，\(PO\) 所在直线为 \(z\) 轴建立空间直角坐标系，并取 \(B\) 在 \(y\) 轴正半轴上，则
\[
A(-1,0,0),\quad C(1,0,0),\quad B(0,\sqrt3,0),\quad P(0,0,\sqrt3).
\]
由于 \(M\) 为 \(BC\) 的中点，得 \(M\left(\frac12,\frac{\sqrt3}{2},0\right)\). 平面 \(PAC\) 的方程为 \(y=0\)，且
\[
\overrightarrow{PM}=\left(\frac12,\frac{\sqrt3}{2},-\sqrt3\right),\qquad PM=\sqrt{\frac14+\frac34+3}=2.
\]
设 \(PM\) 与平面 \(PAC\) 所成的角为 \(\alpha\)，则
\[
...
\begin{solution}
\begin{enumerate}
\item 函数图像向下平移 \(m\) 个单位后为 \(y=f(x)-m\). 该图像经过 \((3,0)\)，\((5,0)\)，所以 \(f(3)=f(5)=m\). 由定义域可知 \(a>-3\). 又
\[
f(3)=\log_3\bigl(3(a+3)\bigr),\qquad f(5)=\log_3(a+5).
\]
由 \(f(3)=f(5)\) 和对数函数的单调性，得
\[
3(a+3)=a+5,
\]
解得 \(a=-2\). 于是
...
\item 要使 \(f(x)\) 与 \(f(6-x)\) 同时有意义，需要
\[
a+x>0,\quad 6-x>0,\quad a+6-x>0,\quad x>0.
\]
因此
\[
...
\begin{solution}
\begin{enumerate}
\item 以 \(O\) 为原点，\(AB\) 所在直线为 \(x\) 轴建立平面直角坐标系，取 \(y\) 轴正方向指向曲线 \(CD\) 一侧，则
\[
A(-10,0),\quad B(10,0).
\]
由 \(AD=6\) 及 \(\angle DAB=120^\circ\)，得
\[
...
\item 由 \(MO\perp AB\) 及 \(OM=14\)，得 \(M=(0,14)\). 设
\[
h=3\sqrt3,\qquad P=(x,y),\qquad Q=(-x,y),\qquad y=\sqrt{196-x^2},
\]
其中 \(0\le x\le13\). 按顶点 \(C,P,M,Q,D\) 的顺序计算五边形面积，得
\[
...
所以取最大值时
\[
x^2=\frac{14^2k^2}{13^2+k^2}=7k=98-21\sqrt3,
\qquad y^2=196-x^2=98+21\sqrt3.
\]
即点 \(P\) 在所建坐标系中的位置为
\[
...
\begin{solution}
\begin{enumerate}
\item 由焦点坐标可知椭圆的半焦距为 \(c=\sqrt2\)，所以 \(a^2-b^2=c^2=2\). 当 \(a=2\) 时，\(b=\sqrt2\)，从而 \(A=(0,-\sqrt2)\). 设 \(M=(x_M,y_M)\)，由 \(AM\) 的中点在 \(x\) 轴上，得
\[
\frac{-\sqrt2+y_M}{2}=0,\qquad y_M=\sqrt2.
\]
又 \(M\in l\)，所以
\[
x_M+\sqrt2=4\sqrt2,\qquad x_M=3\sqrt2.
\]
故 \(M=(3\sqrt2,\sqrt2)\).

\item 直线 \(l\) 与 \(y\) 轴的交点为 \(B=(0,4\sqrt2)\). 令
\[
t=\frac{b}{\sqrt2}>0.
\]
...
\]
直接由向量夹角公式，三角形 \(ABM\) 的三个内角满足
\[
\cos\angle BAM=\frac{t}{\sqrt{1+t^2}},\qquad
\cos\angle ABM=\frac1{\sqrt2},\qquad
\cos\angle AMB=\frac{1-t}{\sqrt{2(1+t^2)}}.
\]
其中 \(\cos\angle ABM\ne\frac35\). 若 \(\cos\angle BAM=\frac35\)，则
\[
\frac{t}{\sqrt{1+t^2}}=\frac35,\qquad t=\frac34,\qquad b=\frac{3\sqrt2}{4}.
\]
若 \(\cos\angle AMB=\frac35\)，则因右端为正数，先得 \(t<1\). 再平方得
\[
25(1-t)^2=18(1+t^2),\qquad 7t^2-50t+7=0.
\]
所以 \(t=7\) 或 \(t=\frac17\)，结合 \(t<1\) 得 \(t=\frac17\)，从而 \(b=\frac{\sqrt2}{7}\). 综上，
\[
b=\frac{3\sqrt2}{4}\quad\text{或}\quad b=\frac{\sqrt2}{7}.
\]

\item 椭圆的焦半距为 \(\sqrt2\)，故椭圆上任意点 \(P\) 都满足
\[
|PF_1|+|PF_2|=2a.
\]
...
\]
由于 \(a>\sqrt2\)，两边非负，平方并整理得
\[
4(a-1)^2\le a^2-1,\qquad (3a-5)(a-1)\le0,
\]
从而 \(a\le\frac53\). 于是
\[
...
\item 若 \(a_1,a_2,\ldots,a_8\) 成等差数列，则其公差为
\[
a_2-a_1=3-1=2,
\]
...
该逆命题为假命题. 例如取
\[
a_1=1,\quad a_2=3,\quad a_3=5,\quad a_4=7,\quad a_5=11,
\quad a_6=15,\quad a_7=19,\quad a_8=23,\quad a_9=27.
\]
这些数满足
\[
a_3=2a_2-a_1,\quad a_4=2a_3-a_2,\quad a_5=2a_4-a_2,
\]
\[
a_6=2a_5-a_4,\quad a_7=2a_6-a_5,\quad a_8=2a_7-a_6,\quad a_9=2a_8-a_7,
\]
其中每一步所用下标均满足规定范围. 但 \(a_4-a_3=2\)，\(a_5-a_4=4\)，故前八项不成等差数列，而 \(a_9=27<30\). 所以逆命题为假命题.
将 \(n\ge9\) 时的后续项定义为 \(a_{n+1}=2a_n-a_{n-1}\)，则后续每一步取 \(i=n-1\) 即满足题设递推条件，故上述前九项确实可以延拓为满足题设的数列.

\item 先证明数列严格递增. 已知 \(a_1<a_2\). 若 \(a_1<a_2<\cdots<a_n\)，则对任意满足条件的 \(i<n\)，有 \(a_i<a_n\)，从而
\[
a_{n+1}=2a_n-a_i>a_n.
\]
由数学归纳法，数列严格递增.
