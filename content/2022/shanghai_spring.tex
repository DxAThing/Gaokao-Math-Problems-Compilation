...
\item 记 \(u(x)=f(x)-f(x-t)\)，则 \(h_1(x)=\lvert u(x+t)-u(x)\rvert=\lvert f(x+t)-2f(x)+f(x-t)\rvert\). 记 \(v(x)=\lvert f(x+t)-f(x)\rvert\)，则 \(h_2(x)=v(x)-v(x-t)=\lvert f(x+t)-f(x)\rvert-\lvert f(x)-f(x-t)\rvert\). 由题设，对任意 \(t>0\)，总存在 \(x\) 使 \(h_1(x)=h_2(x)\)，即
\(\left\lvert[f(x+t)-f(x)]-[f(x)-f(x-t)]\right\rvert=\lvert f(x+t)-f(x)\rvert-\lvert f(x)-f(x-t)\rvert\). 因而存在 \(x\) 使 \(f(x+t)>f(x)>f(x-t)\)，且 \(f(x+t)-f(x)>f(x)-f(x-t)\). 于是对任意 \(t>0\)，存在正增量，结合 \(f(x)\) 在 \((-infty,0)\) 上单调递增，可得 \(f(x+t)>f(x)\) 对任意 \(x\in\R\) 成立，故 \(f(x)\) 在 \(\R\) 上单调递增.
\end{enumerate}
\end{solution}
