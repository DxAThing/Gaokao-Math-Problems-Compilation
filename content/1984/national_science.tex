...
数集 \(X = \{(2n + 1)\pi \mid n\text{ 是整数}\}\) 与数集 \(Y = \{(4k \pm 1)\pi \mid k\text{ 是整数}\}\) 之间的关系是（\quad）
...
设 \(H(x) = \begin{cases} 0, & x \leqslant 0 \\ 1, & x > 0 \end{cases}\)，画出函数 \(y = H(x - 1)\) 的图象.
...
（2）如果 \(a \leqslant 3\)，那么 \(x_n \leqslant 2 + \frac{1}{2^{n - 1}}\)（\(n = 1, 2, \cdots\)）；

（3）如果 \(a > 3\)，那么当 \(n \geqslant \frac{\lg \frac{a}{3}}{\lg \frac{4}{3}}\) 时，必有 \(x_{n + 1} < 3\).
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/1984/national_science/q20_fig1.png}
\end{problem}
