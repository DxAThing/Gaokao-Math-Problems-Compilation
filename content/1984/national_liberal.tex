...
\begin{solution}
任取 \(n\in\Z\). 当 \(n=2q\) 时，\(2n+1=4q+1\)；当 \(n=2q+1\) 时，\(2n+1=4(q+1)-1\)，所以 \(X\subseteq Y\). 反过来，\(4k+1=2(2k)+1\)，\(4k-1=2(2k-1)+1\)，所以 \(Y\subseteq X\). 因此 \(X=Y\)，选 C.
\end{solution}
...
\begin{solution}
复数的模为 \(\sqrt{\left(\frac{1}{2}\right)^2+\left(-\frac{\sqrt{3}}{2}\right)^2}=1\). 其对应点的横坐标为 \(\frac{1}{2}\)，纵坐标为 \(-\frac{\sqrt{3}}{2}\)，故幅角可取 \(-\frac{\pi}{3}\). 因而 \(\frac{1}{2}-\frac{\sqrt{3}}{2}\i=\cos\left(-\frac{\pi}{3}\right)+\i\sin\left(-\frac{\pi}{3}\right)\)，选 A.
\end{solution}
...
\begin{solution}
若直线与平面平行，则它与该平面没有公共点，从而不可能与平面内的任何一条直线相交. 反之，若一条直线与平面内任意一条直线都不相交，那么它既不在该平面内，也不可能与该平面相交；因此它与该平面平行. 故选 C.
\end{solution}
...
\begin{solution}
设方程的两根为 \(r_{1}\)，\(r_{2}\). 由韦达定理，\(r_{1}r_{2}=1\)，\(r_{1}+r_{2}=79\)，所以两根均为正数. 又两根不可能同时为 \(1\)，故一根大于 \(1\)，另一根小于 \(1\). 椭圆的离心率小于 \(1\)，双曲线的离心率大于 \(1\)，而抛物线的离心率等于 \(1\)，所以两根可分别作为一椭圆和一双曲线的离心率，选 A.
\end{solution}
...
已知实数 \(m\) 及 \(x\) 满足 \(2x^{2} - (2\i - 1)x + m - \i = 0\)，求 \(m\) 及 \(x\) 的值\fillinblank{}.
\end{problem}
...
\begin{solution}
将方程左边整理为 \(2x^{2}+x+m-(2x+1)\i=0\). 由于 \(m\) 与 \(x\) 都是实数，实部和虚部必须分别为零，于是 \(2x+1=0\)，且 \(2x^{2}+x+m=0\). 由前式得 \(x=-\frac{1}{2}\)，代入后式得 \(m=0\).
\end{solution}
...
数列 \(\{a_n\}\) 递增，且 \(n\) 为正整数，所以从 \(n=11\) 起产量超过 \(12\) 万件. 第 \(11\) 年对应 \(1983+10=1993\) 年，故从 \(1993\) 年开始.
\end{solution}
