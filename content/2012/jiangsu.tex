...
\]
所以顶点的纵坐标为 \(b-a^{2}/4=0\). 对于 \(c>0\)，不等式 \(f(x)<c\) 的解集为
\[
...
若 \(\cos A=0\)，则上式推出 \(\cos B=0\)，这与三角形内角和为 \(\pi\) 矛盾；若 \(\cos B=0\)，同理可推出 \(\cos A=0\)，也矛盾. 所以 \(\cos A\neq0\)，\(\cos B\neq0\). 将上式除以 \(ac\)，并由正弦定理 \(b/a=\sin B/\sin A\)，得
...
因为 \(CC_{1}\perp\) 平面 \(ABC\)，而 \(E\) 在 \(CC_{1}\) 上，所以 \(DE\) 在底面上的射影是 \(DC\). 直线 \(AD\) 在底面内，且已知 \(AD\perp DE\)，故
\[
\overrightarrow{AD}\cdot\overrightarrow{DE}=\overrightarrow{AD}\cdot\overrightarrow{DC}=0.
\]
由于 \(D\neq C\)，直线 \(DC\) 就是直线 \(BC\)，所以 \(AD\perp BC\). 在等腰三角形 \(ABC\) 中，顶点 \(A\) 到底边 \(BC\) 的垂线也是中线，故 \(D\) 是 \(BC\) 的中点.

\begin{enumerate}
\item 直线 \(AD\) 在底面 \(ABC\) 内，且 \(CC_{1}\perp\) 平面 \(ABC\)，所以 \(AD\perp CC_{1}\). 又已知 \(AD\perp BC\)，而直线 \(BC\) 与 \(CC_{1}\) 相交且都在平面 \(BCC_{1}B_{1}\) 内，故 \(AD\perp\) 平面 \(BCC_{1}B_{1}\). 由于 \(AD\subset\) 平面 \(ADE\)，可得平面 \(ADE\perp\) 平面 \(BCC_{1}B_{1}\).

\item \(D\) 是 \(BC\) 的中点，平移到上底面后，\(F\) 是 \(B_{1}C_{1}\) 的中点. 因而从 \(A_{1}\) 指向 \(F\) 的向量与从 \(A\) 指向 \(D\) 的向量平行，即 \(A_{1}F\parallel AD\). 又 \(AD\subset\) 平面 \(ADE\)，所以直线 \(A_{1}F\parallel\) 平面 \(ADE\).
\end{enumerate}
\end{solution}
...
\]
因此在 \(k>0\) 中，最大高度为 \(5-\frac{a^2}{20}\)，且取得最大值时 \(k=\frac{10}{a}>0\). 由于当 \(k\) 趋近于 \(0\) 时高度为负值，炮弹可以达到高度 \(3.2\) 千米的充要条件是
\[
...
\item 令 \(u=f(x)\). 函数 \(f(u)=u^3-3u\) 在 \(u=-1\) 处取得极大值 \(2\)，在 \(u=1\) 处取得极小值 \(-2\). 当 \(-2<c<2\) 时，方程
\[
f(u)=c
\]
有三个互不相同的根，且三个根都在区间 \((-2,2)\) 内. 对其中每一个根 \(u\)，方程 \(f(x)=u\) 都有三个互不相同的实根，因此 \(f(f(x))=c\) 有 \(3\times3=9\) 个零点.
...
\end{cases}
\]
记 \(d=1+2k^2\)，\(r=\sqrt{2(1+k^2)}\). 当 \(k>0\) 时，取两方程的正根；当 \(k<0\) 时，取两方程的负根. 记 \(T=|t|\)，\(S=|s|\)，由根的表达式可知，无论 \(k\) 正负，都有
\[
TS=\frac{1}{d}.
\]
且 \(S+T=\frac{2r}{d}\)，
...
\]
若 \(q>1\)，则等比数列 \(\{a_n\}\) 无界，与上式矛盾；若 \(0<q<1\)，则 \(a_n\) 趋于 \(0\)，也与 \(a_n>1\) 矛盾. 因此 \(q=1\)，数列 \(\{a_n\}\) 为常数，记其值为 \(a\).

此时 \(b_{n+1}=\frac{\sqrt{2}}{a}b_n\)，所以 \(x_n=\frac{b_n}{a}\) 是以 \(\frac{\sqrt{2}}{a}\) 为公比的正数列. 又 \(a=\frac{1+x_n}{\sqrt{1+x_n^2}}\) 对所有 \(n\) 成立. 若 \(\frac{\sqrt{2}}{a}>1\)，则 \(x_n\) 趋于 \(+\infty\)；若 \(\frac{\sqrt{2}}{a}<1\)，则 \(x_n\) 趋于 \(0\). 这两种情况下右端都趋于 \(1\)，故必须有 \(a=1\)，但对正数 \(x_n\)，始终有 \(\frac{1+x_n}{\sqrt{1+x_n^2}}>1\)，矛盾. 因而 \(\frac{\sqrt{2}}{a}=1\)，即 \(a=\sqrt{2}\).

此时 \(a=\frac{1+x_n}{\sqrt{1+x_n^2}}=\sqrt{2}\)，由 \((1+x_n)^2=2(1+x_n^2)\) 得 \((x_n-1)^2=0\)，所以 \(x_n=1\). 特别地，\(a_1=\sqrt{2}\)，\(b_1=a_1x_1=\sqrt{2}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
因为 \(AB\) 是圆的直径，所以 \(\angle ADB=90^\circ\). 又 \(B,D,C\) 三点共线，故 \(AD\perp BC\). 由 \(BD=DC\)，点 \(D\) 是 \(BC\) 的中点，于是 \(AD\) 是等腰三角形 \(ABC\) 的中线和高，从而 \(AB=AC\) 且
\[
\angle ABD=\angle ABC=\angle BCA=\angle ACD.
\]

点 \(A\)，\(B\)，\(D\)，\(E\) 在同一圆上，圆周角 \(\angle AED\) 与 \(\angle ABD\) 同对弦 \(AD\)，所以 \(\angle AED=\angle ABD\). 因而
\[
...
在同一条链上，条件 2 表明若某一项属于 \(A\)，则下一项不属于 \(A\)；条件 3 表明若某一项不属于 \(A\)，则下一项属于 \(A\). 因此同一条链上的成员资格必须交替出现，而只要确定首项 \(u\) 是否属于 \(A\)，整条链的取法就唯一确定. 不同链之间互不影响.
