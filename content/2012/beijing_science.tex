...
\begin{solution}
\begin{enumerate}
\item 定义域为 \(\{x\in\R\mid x\ne k\pi,\ k\in\Z\}\)，最小正周期为 \(\pi\).
\item 单调递增区间为 \(\left[k\pi-\frac{\pi}{8},k\pi\right)\) 和 \(\left(k\pi,k\pi+\frac{3\pi}{8}\right]\)，其中 \(k\in\Z\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(A_1C\perp\) 平面 \(BCDE\).
\item \(CM\) 与平面 \(A_1BE\) 所成的角为 \(45^\circ\).
\item 不存在这样的点 \(P\).
\end{enumerate}
\end{solution}
...
\item \(\frac{2}{3}\).
\item \(\frac{3}{10}\).
\item \(a=600\)，\(b=0\)，\(c=0\)，此时 \(s^2=80000\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
\item \(a=3\)，\(b=3\).
\item 函数 \(f(x)+g(x)\) 的单调递增区间为 \(\left(-\infty,-\frac{a}{2}\right)\) 和 \(\left(-\frac{a}{6},+\infty\right)\)，单调递减区间为 \(\left(-\frac{a}{2},-\frac{a}{6}\right)\). 当 \(0<a\leqslant 2\) 时，在区间 \((-\infty,-1]\) 上的最大值为 \(a-\frac{a^2}{4}\)；当 \(a>2\) 时，最大值为 \(1\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(m\in\left(\frac{7}{2},5\right)\).
\item \(A\)，\(G\)，\(N\) 三点共线.
\end{enumerate}
\end{solution}
