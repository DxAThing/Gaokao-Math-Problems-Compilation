...
\begin{optionlist}
\item[\textbf{A.}] 设曲线 \(2x^{2} + 2xy + y^{2} = 1\) 在矩阵 \(\mathbf{A} = \begin{pmatrix} a & 0 \\ b & 1 \end{pmatrix}\,(a > 0)\) 对应的变换作用下得到的曲线为 \(x^{2} + y^{2} = 1\).
...
\item[\textbf{B.}] 在平面直角坐标系中，以坐标原点 \(O\) 为极点，\(x\) 轴的正半轴为极轴建立极坐标系. 已知直线 \(l\) 上两点 \(M,N\) 的极坐标分别为 \((2,0)\)，\(\left(\frac{2\sqrt{3}}{3},\frac{\pi}{2}\right)\)，圆 \(C\) 的参数方程为 \(\left\{\begin{aligned}x &= 2 + 2\cos\theta,\\ y &= -\sqrt{3} + 2\sin\theta,\end{aligned}\right.\)（\(\theta\) 为参数）.
...
\item[\textbf{C.}] 已知函数 \(f(x) = m - |x - 2|,\,m \in \R\). 且 \(f(x + 2) \geqslant 0\) 的解集为 \([-1,1]\).
