...
\frac{1}{2}\sin 2x,&-\pi\leqslant x< -\frac{\pi}{2},\\
-\frac{1}{2}\sin 2x,&-\frac{\pi}{2}\leqslant x\leqslant 0.
\end{cases}\)
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(P(X=n+2)=\dfrac{n}{m+n}\cdot\dfrac{n+1}{m+n+2}\).
\item \(P(X=n)=\frac{1}{4}\)，\(P(X=n+1)=\frac{1}{2}\)，\(P(X=n+2)=\frac{1}{4}\)，且 \(E(X)=n+1\).
\end{enumerate}
\end{solution}
...
\item \(AA_{1}\perp BC\).
\item \(AA_{1}=5\).
\item 二面角 \(A-BC-A_{1}\) 的余弦值为 \(-\frac{\sqrt{5}}{5}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(f(x)\) 在 \([0,+\infty)\) 内的最小值为
\(\begin{cases}
a+\dfrac{1}{a}+b,&a\geqslant 1,\\
2+b,&0<a<1.
\end{cases}\)
\item \(a=\dfrac{2}{\e^{2}}\)，\(b=\frac{1}{2}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 椭圆 \(C\) 的方程为 \(\frac{x^{2}}{4}+\frac{y^{2}}{3}=1\).
\item 直线 \(PQ\) 与椭圆 \(C\) 只有一个交点.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(\{x_{n}\}\) 是递减数列的充分必要条件是 \(c<0\).
\item \(\{x_{n}\}\) 是递增数列时，\(0<c\leqslant\frac{1}{4}\).
\end{enumerate}
\end{solution}
