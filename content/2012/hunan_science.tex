...
\bitmapfigure[max width=0.88\linewidth,max totalheight=6.8cm]{img/2012/hunan_science/q3_options.png}
\end{problem}
...
\begin{problem}
在直角坐标系 \(xOy\) 中，已知曲线 \(C_1:\left\{\begin{array}{l}x=t+1,\\ y=1-2t,\end{array}\right.\)（\(t\) 为参数）与曲线 \(C_2:\left\{\begin{array}{l}x=a\sin\theta,\\ y=3\cos\theta,\end{array}\right.\)（\(\theta\) 为参数，\(a>0\)）有一个公共点在 \(x\) 轴上，则 \(a=\)\fillinblank{}.
\end{problem}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2012/hunan_science/q11_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.56\linewidth,max totalheight=6.8cm]{img/2012/hunan_science/q14_fig1.png}
\end{problem}

\begin{problem}
函数 \(f(x)=\sin(\omega x+\varphi)\) 的导函数 \(y=f'(x)\) 的部分图象如图所示，其中，\(P\) 为图象与 \(y\) 轴的交点，\(A\)、\(C\) 为图象与 \(x\) 轴的两个交点，\(B\) 为图象的最低点.

\bitmapfigure[max width=0.36\linewidth,max totalheight=4.4cm]{img/2012/hunan_science/q15_fig1.png}
...
\bitmapfigure[max width=0.72\linewidth,max totalheight=7.0cm]{img/2012/hunan_science/q18_fig1.png}
