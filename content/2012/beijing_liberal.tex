...
\begin{solution}
\begin{enumerate}
\item 定义域为 \(\{x\mid x\ne k\pi,\ k\in\Z\}\)，最小正周期为 \(\pi\)；
\item 单调递减区间为 \(\left[k\pi+\frac{3\pi}{8}, k\pi+\frac{7\pi}{8}\right]\)（\(k\in\Z\)）.
\end{enumerate}
\end{solution}
\end{problem}
...
\begin{solution}
\begin{enumerate}
\item \(DE\parallel\text{平面 }A_1CB\)；
\item \(A_1F\perp BE\)；
\item 存在点 \(Q\)，可取 \(Q\) 为 \(A_1B\) 的中点.
\end{enumerate}
\end{solution}
\end{problem}
...
\item \(\frac{2}{3}\)；
\item \(\frac{3}{10}\)；
\item 当 \(a=600\)，\(b=0\)，\(c=0\) 时，\(s^2=80000\).
\end{enumerate}
\end{solution}
\end{problem}
