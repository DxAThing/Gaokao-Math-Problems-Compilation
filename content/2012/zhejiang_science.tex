...
若 \(\{S_n\}\) 递增，则只需有 \(a_n>0\) 对所有 \(n\geqslant2\) 成立，并不能推出 \(S_1=a_1>0\). 例如取 \(a_1=-1,d=2\)，则 \(S_1=-1,S_2=0,S_3=3,\ldots\)，数列 \(\{S_n\}\) 递增但 \(S_1<0\)，所以第三项错误.

若对任意正整数 \(n\) 都有 \(S_n>0\)，则 \(S_1=a_1>0\). 此时不能有 \(d<0\)，否则 \(S_n\to-\infty\)；故 \(d>0\)，从而所有 \(a_n>0\)，即 \(S_n\) 递增，第四项正确. 故选第三项.
\end{solution}
...
F_1+t(c,b).
\]
与两条渐近线 \(y=\frac ba x\) 和 \(y=-\frac ba x\) 相交时，对应的参数分别为
\[
t_1=\frac{c}{c-a},\qquad t_2=\frac{c}{c+a}.
\]
设 \(H\) 为线段 \(PQ\) 的中点，则
\[
H=F_1+\frac{t_1+t_2}{2}(c,b),\qquad
\frac{t_1+t_2}{2}=\frac{c^2}{b^2}.
\]
...
取初始位置的坐标为
\[
A(0,0,0),\quad B(1,0,0),\quad C(1,\sqrt2,0),\quad D(0,\sqrt2,0).
\]
令两向量
\[
\bs e=\frac1{\sqrt3}(-1,\sqrt2,0),\qquad
\bs n=\frac1{\sqrt3}(\sqrt2,1,0),
\]
...
此外
\[
\overrightarrow{BC}=\frac2{\sqrt3}\bs e+\frac{\sqrt2}{\sqrt3}\bs n,
\quad
\overrightarrow{CD}=\frac1{\sqrt3}\bs e-\frac{\sqrt2}{\sqrt3}\bs n,
\quad
\overrightarrow{BD}=\sqrt3\bs e.
\]
于是
\[
...
由三视图可取四个顶点的坐标为
\[
O(0,0,0),\quad U(3,0,0),\quad V(3,1,0),\quad W(3,0,2).
\]
其中底面 \(OUV\) 在水平面内，且
\[
...
\]
同理，由 \(a_4=a_1q^3\) 及 \(S_4=a_1(1+q+q^2+q^3)\)，得
\[
...
\]
且题设距离不为零，所以 \(a>\frac14\)，从而 \(C_1\) 到直线 \(l\) 的距离为
\[
\frac{a-\frac14}{\sqrt2}.
\]
...
令函数
\[
p(x)=(a-1)x-1,\qquad q(x)=x^2-ax-1.
\]
...
要使 \(p(x)q(x)\geqslant0\) 对所有正数 \(x\) 成立，\(p(x)\) 必须在 \((0,r)\) 上非正、在 \((r,+\infty)\) 上非负. 因而必须有 \(a>1\)，且 \(p(x)\) 的正根与 \(r\) 重合，即
\[
r=\frac1{a-1}.
\]
将其代入 \(q(r)=0\)，得
...
反之，当 \(a=\frac32\) 时，
\[
p(x)=\frac{x-2}{2},\qquad q(x)=x^2-\frac32x-1=(x-2)\left(x+\frac12\right)
\]
所以
\[
...
\[
\cos C=\frac{1}{\sqrt{6}},\qquad \sin C=\frac{\sqrt{5}}{\sqrt{6}}.
\]
由正弦定理，
\[
...
\(MN\parallel BD\). 又 \(BD\) 在平面 \(ABCD\) 内，且 \(MN\) 不在平面 \(ABCD\) 内，故
\[
...
\item 建立如下坐标系：取
\[
A(0,0,0),\quad B(2\sqrt{3},0,0),\quad D(-\sqrt{3},3,0),
\quad C(\sqrt{3},3,0),\quad P(0,0,2\sqrt{6}).
\]
则 \(AB=AD=2\sqrt{3}\)，且 \(\angle BAD=120^\circ\)，符合题设. 由中点坐标公式，
\[
M(\sqrt{3},0,\sqrt{6}),\qquad
N\left(-\frac{\sqrt{3}}{2},\frac{3}{2},\sqrt{6}\right).
\]
取 \(MN\) 的一个方向向量 \(\bs{u}=(-\sqrt{3},1,0)\). 又
\[
\overrightarrow{PC}=(\sqrt{3},3,-2\sqrt{6}),\qquad
|\overrightarrow{PC}|^2=36.
\]
...
\[
A=M+\lambda\bs{d},\qquad B=M-\lambda\bs{d},
\]
...
\[
A=M+\lambda(2,-3),\qquad B=M-\lambda(2,-3).
\]
代入椭圆方程，得
\[
...
令
\[
g(k)=(8-k)^2\left(1-\frac{k^2}{48}\right),\qquad -4\sqrt{3}<k<4\sqrt{3}.
\]
则
\[
...
\]
可知，\(f'(x)\) 在 \([0,1]\) 内至多改变一次符号；若有内点零点，该点是极小值点. 因此 \(f(x)\) 在 \([0,1]\) 上的最大值取在端点.
又有
\[
f(0)=b-a,\qquad f(1)=3a-b,
\]
所以
\[
...
由于 \(a>0\)，这等价于
\[
0<a\leqslant1,\qquad 3a-1\leqslant b\leqslant a+1.
\]
于是
\[
...
当 \(-1<s\leqslant1\) 时，取
\[
a=\frac{s+1}{4},\qquad b=s-a;
\]
当 \(1<s\leqslant3\) 时，取
\[
a=\frac{s-1}{2},\qquad b=s-a.
\]
