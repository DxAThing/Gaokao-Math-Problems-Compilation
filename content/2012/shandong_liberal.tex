...
\bitmapfigure[width=0.75\linewidth]{img/2012/shandong_liberal/q14_fig1.png}
\end{problem}
...
\begin{solution}
短答案：\(b^{2}=ac\)；当 \(a=1\)，\(c=2\) 时，\(S=\frac{\sqrt{7}}{4}\).
\end{solution}
...
\begin{solution}
短答案：第（1）问所求概率为 \(\frac{3}{10}\)；第（2）问所求概率为 \(\frac{8}{15}\).
\end{solution}
...
\begin{solution}
短答案：第（1）问证明 \(BE=DE\)；第（2）问证明 \(DM\parallel\) 平面 \(BEC\).
\end{solution}
...
\begin{solution}
短答案：\(a_n=7n\)；\(S_m=\frac{7}{48}\left(49^m-1\right)\).
\end{solution}
...
\begin{solution}
短答案：椭圆的标准方程为 \(\frac{x^2}{4}+y^2=1\)；\(\frac{|PQ|}{|ST|}\) 的最大值为 \(\frac{2\sqrt{5}}{5}\)，此时 \(m=\pm\frac{5}{3}\) 或 \(m=0\).
\end{solution}
...
\begin{solution}
短答案：\(k=1\)；\(f(x)\) 在 \((0,1)\) 上单调递增，在 \((1,+\infty)\) 上单调递减；对任意 \(x>0\)，\(g(x)<1+\e^{-2}\).
\end{solution}
