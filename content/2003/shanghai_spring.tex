...
\begin{solution}
联立直线和抛物线方程. 由 \(y=x-1\) 得 \(x=y+1\)，代入 \(y^2=4x\)，得到
\(y^2=4(y+1)\)，\(y^2-4y-4=0\).
解得 \(y=2\pm2\sqrt{2}\)，于是两个交点分别为 \(\left(3+2\sqrt{2},2+2\sqrt{2}\right)\) 和 \(\left(3-2\sqrt{2},2-2\sqrt{2}\right)\). 线段中点的横、纵坐标分别为两交点对应坐标的平均值，因此中点为
\[
...
\begin{solution}
任取复数 \(z\)，可唯一写成 \(z=u+\i v\)，其中 \(u\)，\(v\in\R\). 于是 \(\overline{z}=u-\i v\)，从而
\(z+\overline{z}=2u\)，\(z+\overline{z}>2\Longleftrightarrow2u>2\Longleftrightarrow u>1\).
因此，\(z=u+\i v\)，其中 \(u\)，\(v\in\R\) 且 \(u>1\)，是所求的一个充要条件；反过来满足该条件的复数也都满足原不等式.
\end{solution}
...
\begin{solution}
把 \(f(k)\) 与 \(f(1-k)\) 配成一对. 对任意实数 \(k\)，令 \(t=2^{k-1/2}>0\)，则 \(2^k=\sqrt{2}t\)，且 \(2^{1-k}=\sqrt{2}t^{-1}\). 因此
\(f(k)=\frac{1}{\sqrt{2}(t+1)}\)，\(f(1-k)=\frac{1}{\sqrt{2}(1+t^{-1})}=\frac{t}{\sqrt{2}(t+1)}\)
从而 \(f(k)+f(1-k)=\dfrac{1}{\sqrt{2}}\). 题中各项正好可配成
\(f(-5)+f(6)\)，\(f(-4)+f(5)\)，\(f(-3)+f(4)\)，\(f(-2)+f(3)\)，\(f(-1)+f(2)\)，\(f(0)+f(1)\)，共 \(6\) 对. 每对和均为 \(\dfrac{1}{\sqrt{2}}\)，所以原式等于 \(\dfrac{6}{\sqrt{2}}=3\sqrt{2}\).
\end{solution}
...
\begin{solution}
由图象可读出最大值为 \(2\)，最小值为 \(-2\)，所以振幅 \(A=2\). 图象在 \(x=\dfrac{\pi}{2}\) 处取得最大值，在 \(x=\dfrac{5\pi}{2}\) 处取得最小值，这两点相隔半个周期，因此
\(\frac{T}{2}=\frac{5\pi}{2}-\frac{\pi}{2}=2\pi\)，\(T=4\pi\).
又 \(T=\dfrac{2\pi}{\omega}\)，故 \(\omega=\dfrac{1}{2}\). 图象在 \(x=-\dfrac{\pi}{2}\) 处过原点且向上穿过 \(x\) 轴，所以可取相位为 \(\dfrac{\pi}{4}\)，从而
\[
f(x)=2\sin\left(\frac{x}{2}+\frac{\pi}{4}\right)
\]
令函数值等于直线的纵坐标 \(\sqrt{3}\)，得到
\(2\sin\left(\frac{x}{2}+\frac{\pi}{4}\right)=\sqrt{3}\)，\(\sin\left(\frac{x}{2}+\frac{\pi}{4}\right)=\frac{\sqrt{3}}{2}\).
因此
\(\frac{x}{2}+\frac{\pi}{4}=\frac{\pi}{3}+2k\pi \quad\text{或}\quad \frac{2\pi}{3}+2k\pi\)，\(k\in\Z\).
分别解得 \(x=\dfrac{\pi}{6}+4k\pi\) 或 \(x=\dfrac{5\pi}{6}+4k\pi\). 所有交点为
\(\left(\frac{\pi}{6}+4k\pi,\sqrt{3}\right) \quad\text{或}\quad \left(\frac{5\pi}{6}+4k\pi,\sqrt{3}\right)\)，\(k\in\Z\).
\end{solution}
...
\bitmapfigure[max width=0.33\linewidth]{img/2003/shanghai_spring/q19_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 由题给向量的长度，
\(|AB|=\sqrt{\left(\frac{m}{2}\right)^2+\left(-\frac{\sqrt{3}m}{2}\right)^2}=m\)，\(|AC|=m\).
又
\[
...
\item 平面 \(A_1ABB_1\) 内有相交直线 \(AB\) 和 \(AA_1\). 取该平面的法向量
\[
\bs{n}=\overrightarrow{AB}\times\overrightarrow{AA_1}
=\left(-\frac{\sqrt{3}mn}{2},-\frac{mn}{2},0\right)
\]
则 \(|\bs{n}|=mn\). 直线 \(CA_1\) 的方向向量可取
\(\bs{d}=\overrightarrow{CA_1}=(-m,0,n)\)，\(|\bs{d}|=\sqrt{m^2+n^2}\).
计算得
\[
\left|\bs{d}\cdot\bs{n}\right|=\frac{\sqrt{3}m^2n}{2}
\]
...
\begin{solution}
\begin{enumerate}
\item 函数中的负一次方根要求 \(x\ne0\)，故定义域为 \(\R\mathbin{\backslash}\{0\}\). 对任意定义域内的 \(x\)，利用实数立方根的奇性，
...
\item 令 \(a=2^{1/3}\). 则 \(4^{1/3}=a^2\)，\(4^{-1/3}=a^{-2}\)，从而
\[
f(4)=\frac{a^2-a^{-2}}{5}
\]
另一方面，
...
=\frac{x^{2/3}-x^{-2/3}}{5}
\]
两式相等，故对所有 \(x\in\R\mathbin{\backslash}\{0\}\)，恒有 \(f(x^2)=5f(x)g(x)\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 椭圆上任一点到左、右两焦点的距离之和为 \(2a\). 题设点 \(A\) 的距离和为 \(4\)，所以 \(2a=4\)，得 \(a=2\). 又因 \(A\left(1,\dfrac{3}{2}\right)\) 在椭圆上，有
\[
...
\item 对所得椭圆，焦点参数满足 \(c^2=a^2-b^2=4-3=1\)，所以 \(c=1\)，左焦点为 \(F_1=(-1,0)\). 设动点 \(K(x,y)\)，线段 \(F_1K\) 的中点为 \(Q(u,v)\). 则
\(u=\frac{x-1}{2}\)，\(v=\frac{y}{2}\)
...
\item 对双曲线，设 \(M(u,v)\)、\(N(-u,-v)\) 是关于原点对称的两个点，\(P(x,y)\) 是双曲线上任意一点，并假设直线 \(PM\)，\(PN\) 的斜率存在. 由于斜率存在，\(x-u\ne0\) 且 \(x+u\ne0\). 于是
\(k_{PM}=\frac{y-v}{x-u}\)，\(k_{PN}=\frac{y+v}{x+u}\)
从而
\[
k_{PM}k_{PN}
...
\]
点 \(P\)，\(M\) 都在双曲线 \(\dfrac{x^2}{a^2}-\dfrac{y^2}{b^2}=1\) 上，所以
\(y^2=\frac{b^2}{a^2}x^2-b^2\)，\(v^2=\frac{b^2}{a^2}u^2-b^2\).
两式相减，得
\[
...
\begin{solution}
\begin{enumerate}
\item 记第 \(n\) 年的月工资分别为 \(A_n\)，\(B_n\). A 公司满足
\(A_1=1500\)，\(A_{n+1}=A_n+230\)
所以 \(\{A_n\}\) 是首项为 \(1500\)、公差为 \(230\) 的等差数列，故
\[
A_n=1500+230(n-1)\ \text{元}
\]
B 公司满足
\(B_1=2000\)，\(B_{n+1}=1.05B_n\)
所以 \(\{B_n\}\) 是首项为 \(2000\)、公比为 \(1.05\) 的等比数列，故
\[
B_n=2000(1.05)^{n-1}\ \text{元}
\]

\item 连续工作 \(10\) 年时，每年的月工资总和最后都要乘以 \(12\)，所以先比较各年的月工资数之和. A 公司的和为
\[
