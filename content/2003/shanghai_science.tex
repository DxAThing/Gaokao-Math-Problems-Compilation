...
\begin{solution}
设 \(\frac{a_1}{a_2}=\frac{b_1}{b_2}=\frac{c_1}{c_2}=k\)，则第一个二次式等于第二个二次式的 \(k\) 倍. 若 \(k>0\)，两不等式同解；但题设允许 \(k<0\)，这时不等式方向会改变，不能保证解集相同. 例如取 \(a_2=b_2=c_2=1\)，\(a_1=b_1=c_1=-1\)，则比例条件成立，而 \(x^2+x+1>0\) 的解集为 \(\R\)，\(-x^2-x-1>0\) 的解集为空集，故比例条件不是充分条件.

反过来，解集相同也不要求三个系数分别成同一比例. 例如 \(x^2+x+1>0\) 与 \(2x^2+x+1>0\) 的判别式都小于零且二次项系数为正，所以二者解集都为 \(\R\)，但对应系数比不全相等. 因此比例条件也不是必要条件，选 D.
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 由等比数列的通项关系，\(a_2=a_1q\)，\(a_3=a_1q^2\)，\(a_4=a_1q^3\). 因而第一个和为
\[
...
\begin{solution}
\begin{enumerate}
\item 取隧道横截面的对称轴为坐标轴，半个椭圆所在椭圆的方程为
\[
...
\item 由点 \((11,4.5)\) 在椭圆上，任意满足条件的 \(h\) 和 \(l\) 都满足
\[
\frac{484}{l^2}+\frac{81}{4h^2}=1
\]
从而
\(l=\frac{44h}{\sqrt{4h^2-81}}\)，\(h\geqslant6\).
隧道全长固定，所以土方工程量与横截面半椭圆面积 \(S=\frac{\pi}{4}lh\) 成正比，只需使 \(lh\) 最小. 令 \(t=h^2\)，则 \(t\geqslant36\)，且
\[
...
\]
当 \(36\leqslant t<\frac{81}{2}\) 时，\(F'(t)<0\)；当 \(t>\frac{81}{2}\) 时，\(F'(t)>0\). 所以 \(F(t)\) 在 \(t=\frac{81}{2}\) 处取得最小值. 因而
\(h=\sqrt{\frac{81}{2}}=\frac{9}{\sqrt2}\approx6.4\text{ 米}\)，\(l=\frac{44}{\sqrt2}=22\sqrt2\approx31.1\text{ 米}\).
此时横截面面积为 \(\frac{\pi}{4}lh=\frac{99\pi}{2}\text{ 平方米}\)，隧道全长为 \(2500\text{ 米}\)，所以最小土方工程量为 \(\frac{99\pi}{2}\times2500=123750\pi\text{ 立方米}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 由 \(A(4,-3)\)，得 \(\overrightarrow{AO}=(-4,3)\)，所以 \(|OA|=5\)，\(|AB|=10\). 设 \(\overrightarrow{AB}=(u,v)\). 因为 \(A\) 是直角顶点，所以 \(\overrightarrow{AO}\cdot\overrightarrow{AB}=0\)，即 \(-4u+3v=0\)，又 \(u^2+v^2=100\). 由 \(v=\frac{4u}{3}\) 得 \(\frac{25u^2}{9}=100\)，所以 \((u,v)=(6,8)\) 或 \((-6,-8)\). 相应的点 \(B\) 的纵坐标分别为 \(5\) 和 \(-11\)，由题意取 \(\overrightarrow{AB}=(6,8)\)，并得 \(B=(10,5)\).
\item 原圆方程化为 \((x-3)^2+(y+1)^2=10\)，所以圆心为 \(C_0=(3,-1)\)，半径平方为 \(10\). 直线 \(OB\) 的方向向量可取 \(\bs d=(2,1)\)，其方程为 \(y=\frac{x}{2}\). 圆心 \(C_0\) 在该直线上的射影为
\[
...
\item 直线 \(OB\) 的方向向量为 \((2,1)\). 设关于该直线对称的两个不同点为 \(P\)，\(Q\)，它们的中点为 \(M=(2t,t)\). 取垂直于 \(OB\) 的方向向量 \((-1,2)\)，可设
\(P=(2t-r,t+2r)\)，\(Q=(2t+r,t-2r)\)，\(r\neq0\).
若两点都在抛物线 \(y=ax^2-1\) 上，则
\[
\begin{cases}
t+2r=a(2t-r)^2-1,\\
t-2r=a(2t+r)^2-1.
...
\begin{solution}
\begin{enumerate}
\item 若 \(f(x)=x\) 属于 \(M\)，则存在非零常数 \(T\)，使对任意 \(x\in\R\) 有 \(x+T=Tx\). 取 \(x=0\) 立即得到 \(T=0\)，与 \(T\neq0\) 矛盾，所以 \(f(x)=x\) 不属于 \(M\).
\item 设图象的公共点为 \((x_0,x_0)\)，则 \(a^{x_0}=x_0\). 因为 \(a^{x_0}>0\)，所以 \(x_0>0\)，从而 \(T=x_0\) 是非零常数，并且 \(a^T=T\). 对任意 \(x\in\R\)，有
\[
f(x+T)=a^{x+T}=a^x a^T=T a^x=Tf(x)
\]
所以 \(f(x)=a^x\in M\).
\item 当 \(k=0\) 时，\(f(x)=0\)，任取非零常数 \(T\) 都有 \(f(x+T)=Tf(x)=0\)，所以 \(k=0\) 满足条件. 下面设 \(k\neq0\). 由 \(f\in M\)，存在非零常数 \(T\)，使对任意 \(x\) 有
\[
\sin(kx+kT)=T\sin(kx)
\]
由于 \(k\neq0\)，令 \(u=kx\)，则 \(u\) 可取任意实数，故恒有 \(\sin(u+kT)=T\sin u\). 取 \(u=0\)，得 \(\sin(kT)=0\)；取 \(u=\frac\pi2\)，得 \(\cos(kT)=T\). 因而 \(T=1\) 或 \(T=-1\).

当 \(T=1\) 时，\(\cos k=1\)，所以 \(k=2m\pi\)，其中 \(m\in\Z\). 当 \(T=-1\) 时，\(\cos(-k)=-1\)，所以 \(\cos k=-1\)，从而 \(k=(2m+1)\pi\)，其中 \(m\in\Z\). 两种情况合并为 \(k=m\pi\)，\(m\in\Z\).

反之，若 \(k=m\pi\)，当 \(m\) 为偶数时取 \(T=1\)，当 \(m\) 为奇数时取 \(T=-1\)，均有 \(\sin(kx+kT)=T\sin(kx)\). 所以实数 \(k\) 的取值范围为 \(\pi\Z\).
\end{enumerate}
\end{solution}
