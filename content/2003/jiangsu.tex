...
\begin{solution}
函数图象与 \(x\) 轴有两个交点，等价于方程 \(ax^{2}+bx+a=0\) 是二次方程且有两个不相等的实根. 因此必须有 \(a\ne0\)，并且判别式满足 \(\Delta=b^{2}-4a^{2}>0\)，即 \(b^{2}>4a^{2}\). 于是严格的参数条件是 \(a\ne0\) 且 \(b^{2}>4a^{2}\). 选项 C 所表示的两个开区域对应 \(b^{2}>4a^{2}\)，所以按原卷标准答案选 C；但图示区域还包含 \(a=0\) 且 \(b\ne0\) 的点，这些点对应一次函数 \(y=bx\)，与“二次函数图象有两个交点”不符. 因此，严格解集还必须排除这条 \(b\) 轴，原题的图示与严格条件存在争议.
\end{solution}
...
\begin{solution}
当 \(x_{0}\leqslant0\) 时，\(f(x_{0})>1\) 等价于 \(2^{-x_{0}}-1>1\)，即 \(2^{-x_{0}}>2=2^{1}\). 因为底数 \(2>1\)，所以 \(-x_{0}>1\)，从而 \(x_{0}<-1\). 当 \(x_{0}>0\) 时，\(f(x_{0})=\sqrt{x_{0}}>1\)，而两边均为非负数，平方得 \(x_{0}>1\). 两种情形合并为 \(x_{0}\in(-\infty,-1)\cup(1,+\infty)\)，故选 D.
\end{solution}
...
可知 \(P\) 在从 \(A\) 出发的内角平分线射线上. 设三角形内心为 \(I\). 因为 \(I\) 在 \(A\) 的内角平分线上且位于从 \(A\) 向三角形内部的方向，所以存在 \(\lambda_{0}>0\) 使得 \(\overrightarrow{AI}=\lambda_{0}(\bs u+\bs v)\). 取 \(\lambda=\lambda_{0}\) 即得 \(P=I\)，故轨迹一定经过内心，选 B.
\end{solution}
...
\begin{solution}
令原函数为 \(y=\ln\frac{x+1}{x-1}\)，其中 \(x>1\). 因为
\[
f'(x)=\frac{1}{x+1}-\frac{1}{x-1}=-\frac{2}{x^{2}-1}<0
\]
所以原函数在 \((1,+\infty)\) 上严格递减，反函数存在. 又因 \(\frac{x+1}{x-1}>1\)，所以函数值 \(y>0\)，且当 \(x\to1^{+}\) 时 \(y\to+\infty\)，当 \(x\to+\infty\) 时 \(y\to0^{+}\)，故值域为 \((0,+\infty)\). 由
\[
\e^{y}=\frac{x+1}{x-1}
\]
得 \(\e^{y}x-\e^{y}=x+1\)，即 \(x(\e^{y}-1)=\e^{y}+1\)，所以
\(x=\frac{\e^{y}+1}{\e^{y}-1}\)，\(y>0\).
...
\]
由于 \(a>0\)，函数 \(2ax_{0}+b\) 随 \(x_{0}\) 连续变化，且题设给出的倾斜角范围使其恰好取遍 \([0,1]\). 因此上述距离恰好取遍 \(\left[0,\frac{1}{2a}\right]\)，故选 B.
\end{solution}
...
\]
因 \(MN\) 中点横坐标为 \(-\frac23\)，有 \(x_{1}+x_{2}=-\frac43\)，所以 \(b^{2}=\frac52a^{2}\). 联立 \(a^{2}+b^{2}=7\)，得 \(a^{2}=2\)，\(b^{2}=5\). 因此双曲线方程为 \(\frac{x^{2}}2-\frac{y^{2}}5=1\)，故选 D.
\end{solution}
...
\begin{solution}
设 \(t=\tan\theta\). 将每次反射后的长方形关于反射边展开，反射光线即可视为一条直线. 在展开图中，\(P_{0}=(1,0)\)，故光线方程为 \(y=t(x-1)\). 它先在 \(x=2\) 处到达 \(BC\)，再在 \(y=1\) 处到达 \(CD\)，随后在 \(x=4\) 处到达 \(DA\)，最后到达对应于 \(AB\) 的直线 \(y=2\). 设最后交点的横坐标为 \(X_{4}\)，则
\[
X_{4}=1+\frac2t
\]
经过前三次反射，最后一个展开矩形的横坐标范围为 \(4\leqslant x\leqslant6\)，其边 \(y=2\) 折回原矩形的 \(AB\)，所以 \(x_{4}=X_{4}-4\). 题设 \(1<x_{4}<2\) 等价于
\[
1<1+\frac2t-4<2
\]
即 \(4<\frac2t<5\). 这里 \(t>0\)，不等号方向保持不变，故 \(\frac25<t<\frac12\). 该范围同时保证光线依次击中题设的四条边，因而 \(\tan\theta\) 的取值范围为 \(\left(\frac25,\frac12\right)\)，选 C.
\end{solution}
...
\begin{solution}
取正四面体的四个顶点为
\(\left(r,r,r\right)\)，\(\left(r,-r,-r\right)\)，\(\left(-r,r,-r\right)\)，\(\left(-r,-r,r\right)\).
...
\]
因此三种型号依次抽取 \(1200\times\frac1{200}=6\) 辆，\(6000\times\frac1{200}=30\) 辆，\(2000\times\frac1{200}=10\) 辆，三数之和为 \(46\)，所以填 \(6\)，\(30\)，\(10\).
\end{solution}

\begin{problem}
某城市在中心广场建造一个花圃，花圃分为 \(6\) 个部分（如图）. 现要栽种 \(4\) 种不同颜色的花，每部分栽种一种且相邻部分不能栽种同样颜色的花，不同的栽种方法有\fillinblank{}种.
\bitmapfigure[max width=6.0cm]{img/2003/jiangsu/q15_fig1.png}
\end{problem}
...
又 \(f'(x)=-\omega\sin\omega x\). 若 \(0<\omega\leqslant2\)，则当 \(0\leqslant x\leqslant\frac{\pi}{2}\) 时，\(0\leqslant\omega x\leqslant\pi\)，所以 \(f'(x)\leqslant0\)，函数在该区间上单调. 若 \(\omega>2\)，则 \(\frac{\pi}{\omega}<\frac{\pi}{2}\)，在 \(\left(0,\frac{\pi}{\omega}\right)\) 内有 \(f'(x)<0\)，而区间 \(\left(\frac{\pi}{\omega},\min\left\{\frac{2\pi}{\omega},\frac{\pi}{2}\right\}\right)\) 非空且其中 \(f'(x)>0\)，所以函数不单调. 因此 \(\omega\leqslant2\). 在 \(\omega=\frac{2(2k+1)}{3}\) 中只可能有 \(k=0\)，\(1\)，故
\(\omega=\frac{2}{3}\text{ 或 }\omega=2\)，\(\varphi=\frac{\pi}{2}\).
\end{solution}
...
\bitmapfigure[max width=6.0cm]{img/2003/jiangsu/q19_fig1.png}
\end{problem}
...
当 \(a=\frac{\sqrt2}{2}\) 时，轨迹是以 \(\left(0,\frac a2\right)\) 为圆心的圆. 若 \(E\)、\(F\) 是两个不同定点，则具有定和性质的轨迹应是长半轴大于焦距的椭圆，而圆对应焦距为 \(0\)，两个焦点必须重合于圆心，故不存在符合题意的两个不同定点. 若题意允许两定点重合，则可取 \(E=F=\left(0,\frac a2\right)\)，此时距离和恒为圆的直径；按通常将两个焦点视为不同定点的题意，临界情形的结论为不存在.
\end{solution}
...
对 \(0\leqslant k\leqslant n-2\)，有 \(n+1>n\) 且 \(n+1-a\geqslant n-a\geqslant0\)，因此
        ...
        =n\cdot n^{n-2-k}(n-a)^k
        \]
        对 \(0\leqslant k\leqslant n-2\) 的每一项都已有严格不等式，且左边的求和还多出非负项 \(k=n-1\)，所以
        \[
        \frac{f_{n+1}'(n+1)}{n+1}
        >na\sum_{k=0}^{n-2}n^{n-2-k}(n-a)^k
        =f_n'(n)
        \]
        即
\[
...
\bitmapfigure[max width=8.8cm]{img/2003/jiangsu/q22_fig1.png}
\end{problem}
...
\[
\sum_{k=1}^{n}(a_k-a_{k+1})a_{k+2}<\frac13\sum_{k=1}^{n}(a_k^3-a_{k+1}^3)=\frac13(a_1^3-a_{n+1}^3)<\frac13
\]
\end{enumerate}
\end{solution}
