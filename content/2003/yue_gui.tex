...
\begin{solution}
直线 \(y=ax\) 必过原点，斜率为 \(a\)；直线 \(y=x+a\) 的斜率为 \(1\)，在 \(y\) 轴上的截距为 \(a\). 因而两条直线中，斜率为 \(a\) 的一条必须过原点，而另一条的截距必须与前一条的斜率同号. 逐项对照图形，只有第三个图形同时满足这些条件：其中一条负斜率直线过原点，另一条斜率为 \(1\) 且在 \(y\) 轴上的截距为负数，即对应同一个 \(a<0\). 故选第三项.
\end{solution}
...
\[
\sin x=-\sqrt{1-\frac{16}{25}}=-\frac35,
\qquad \tan x=\frac{\sin x}{\cos x}=-\frac34.
\]
于是
\[
...
\[
\frac rR=\frac{3R-h}{3R},
\qquad h=3(R-r),
\qquad 0\leqslant r\leqslant R.
\]
圆柱的全面积为
\[
...
\begin{solution}
在区间 \(\left[\frac\pi2,\frac{3\pi}2\right]\) 上，\(\sin x\) 严格递减，值域为 \([-1,1]\)，所以反函数的定义域为 \([-1,1]\)，值域为 \(\left[\frac\pi2,\frac{3\pi}2\right]\). 令 \(y=f^{-1}(x)\)，则 \(y\in\left[\frac\pi2,\frac{3\pi}2\right]\) 且 \(\sin y=x\). 由 \(\arcsin x\in\left[-\frac\pi2,\frac\pi2\right]\)，满足这个值域条件的角为
\[
y=\pi-\arcsin x.
\]
因此按函数定义，反函数应为 \(f^{-1}(x)=\pi-\arcsin x\)，其中 \(x\in[-1,1]\). 原题中两个重复选项均印为 \(-\pi-\arcsin x\)，没有印出上述正确表达式；现稿的选择题答案仅按原卷参考答案保留，题面该处存在印刷错误.
\end{solution}
...
y=m(x-1).
\]
依次经过右边、上边、左边后，第四次到达的底边对应于展开图中的直线 \(y=2\)，因此交点的横坐标为
\[
x'=1+\frac2m.
\]
左边界在展开图中为 \(x=4\)，反射回原矩形后，\(P_4\) 的横坐标为
\[
x_4=x'-4=\frac2m-3.
\]
由 \(1<x_4<2\)，得
...
\begin{solution}
这是棱长为 \(a=\sqrt2\) 的正四面体. 其高为 \(a\sqrt{\frac23}\)，球心在高线上，且从顶点到对面底面的高被球心按 \(3:1\) 分割，所以外接球半径为
\[
...
\[
\mathrm C_{9}^{k}\left(x^{2}\right)^{9-k}\left(-\frac{1}{2x}\right)^{k}=\mathrm C_{9}^{k}\frac{(-1)^{k}}{2^{k}}x^{18-3k},\qquad k=0,1,\ldots,9.
\]
...
\[
B(1,0,0),\quad D(0,1,0),\quad C(1,1,0),\quad D_{1}(0,1,2),\quad C_{1}(1,1,2).
\]
由中点坐标公式，得
\[
E(1,1,1),\qquad F\left(\frac12,\frac12,1\right).
\]
于是
\[
\overrightarrow{EF}=\left(-\frac12,-\frac12,0\right),\qquad
\overrightarrow{BD_{1}}=(-1,1,2),\qquad
\overrightarrow{CC_{1}}=(0,0,2).
\]
从而
\[
\overrightarrow{EF}\cdot\overrightarrow{BD_{1}}=0,\qquad
\overrightarrow{EF}\cdot\overrightarrow{CC_{1}}=0.
\]
又 \(F\in BD_{1}\)，\(E\in CC_{1}\)，所以 \(EF\) 同时垂直于 \(BD_{1}\) 和 \(CC_{1}\)，即 \(EF\) 是这两条异面直线的公垂线.
\item 由
\[
\overrightarrow{BD}=(-1,1,0),\qquad \overrightarrow{BE}=(0,1,1),
\]
得平面 \(BDE\) 的一个法向量为
\[
...
\[
\cos\theta=\frac{\sqrt2}{10},\qquad \sin\theta=\frac{7\sqrt2}{10},
\]
所以初始位置为
\[
...
\[
A(-2,0),\quad B(2,0),\quad C(2,4a),\quad D(-2,4a).
\]
设公共比值为 \(\lambda\)，则 \(0\leqslant\lambda\leqslant1\)，从而
\[
E=(2,4a\lambda),\quad F=(2-4\lambda,4a),\quad G=(-2,4a(1-\lambda)).
\]
...
\[
F_{1}\left(0,a-\sqrt{a^{2}-\frac12}\right),\qquad
F_{2}\left(0,a+\sqrt{a^{2}-\frac12}\right),
\]
到这两个焦点的距离和为 \(2a\). 当 \(0<a<\frac{1}{\sqrt2}\) 时，其长轴在 \(x\) 轴上，两个焦点为
\[
F_{1}\left(-\sqrt{\frac12-a^{2}},a\right),\qquad
F_{2}\left(\sqrt{\frac12-a^{2}},a\right),
\]
...
&=\frac15\left(3^{k+1}+(-1)^{k}\cdot2^{k+1}\right)+(-1)^{k+1}\cdot2^{k+1}a_{0}.
\end{aligned}
\]
