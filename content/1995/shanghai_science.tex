...
\begin{solution}
因为 \(\frac{\sqrt{3}}{3}=\tan\frac{\pi}{6}\)，所以
\(2x+\frac{\pi}{3}=\frac{\pi}{6}+k\pi\)，\((k\in\Z)\)
从而
\[
...
\]
即 \(\frac{1}{6}\leqslant k<\frac{25}{6}\)，故 \(k=1\)，\(2\)，\(3\)，\(4\). 解为
\(x=\frac{5\pi}{12}\)，\(\ \frac{11\pi}{12}\)，\(\ \frac{17\pi}{12}\)，\(\ \frac{23\pi}{12}\)
共有 \(4\) 个，故选 B.
\end{solution}
...
令 \(h(x)=x\ln(1+x)\). 当 \(x>0\) 时，\(\ln(1+x)>0\)，故
\(h'(x)=\ln(1+x)+\frac{x}{1+x}>0\)，\((x>0)\)
所以 \(h\) 在正半轴上递增，因而 \(h(a)<h(b)\)，即 \((1+a)^a<(1+b)^b\)，选项 B 错误.

令 \(k(x)=x\ln(1-x)\). 当 \(0<x<1\) 时，\(\ln(1-x)<0\) 且 \(\frac{x}{1-x}>0\)，故
\(k'(x)=\ln(1-x)-\frac{x}{1-x}<0\)，\((0<x<1)\)
所以 \(k\) 在 \((0,1)\) 上递减，因而 \(k(a)>k(b)\)，从而 \((1-a)^a>(1-b)^b\). 故正确选项为 D.
\end{solution}
...
\begin{solution}
二项式展开中，\(x^3\) 和 \(x^2\) 的系数分别为
\(a=\mathrm C_n^3\)，\(b=\mathrm C_n^2\).
因此
\[
...
\]
令 \(u=\frac{x}{2}\)，则 \(y'=0\) 的点满足 \(\cos u=\sin u\)，即
\(x=\frac{\pi}{2}+2k\pi\)，\((k\in\Z)\).
由于 \(y'=\frac12\sqrt2\cos\left(\frac{x}{2}+\frac{\pi}{4}\right)\)，其在两个相邻零点 \(-\frac{3\pi}{2}\) 与 \(\frac{\pi}{2}\) 之间为正，在其余相邻区间为负. 与定义域 \((-2\pi,2\pi)\) 取交，递增区间为
\[
\left[-\frac{3\pi}{2},\frac{\pi}{2}\right]
\]
\end{solution}
...
利用
\(\sin2\theta=\frac{2u}{1+u^2}\)，\(\cos2\theta=\frac{1-u^2}{1+u^2}\)，\(2\cos^2\theta=1+\cos2\theta\)
原式为
\[
...
\begin{solution}
\begin{enumerate}
\item 因为 \(O\) 是 \(BC\) 的中点且 \(BC=2\)，所以
\(B=(0,-1,0)\)，\(C=(0,1,0)\).
在直角三角形 \(BDC\) 中，\(\angle BDC=90^\circ\)，所以 \(BC\) 是斜边. 由 \(\angle DCB=30^\circ\)，故
\(DC=BC\cos30^\circ=\sqrt3\)，\(BD=BC\sin30^\circ=1\).
点 \(D\) 在 \(yoz\) 平面上，所以 \(x_D=0\). 按题图，\(\overrightarrow{CD}\) 的 \(y\) 分量为负、\(z\) 分量为正. 因而
\[
\overrightarrow{CD}
...
\item 由题设
\(\overrightarrow{OA}=\left(\frac{\sqrt3}{2},\frac12,0\right)\)，\(\overrightarrow{OB}=(0,-1,0)\)，\(\overrightarrow{OC}=(0,1,0)\).
所以
\(\overrightarrow{AD}=\overrightarrow{OD}-\overrightarrow{OA} =\left(-\frac{\sqrt3}{2},-1,\frac{\sqrt3}{2}\right)\)，\(\overrightarrow{BC}=\overrightarrow{OC}-\overrightarrow{OB}=(0,2,0)\).
于是
\[
...
\item 因为 \(f(x)=(x+1)^2\geqslant0\)，图形与两坐标轴围成的区域对应于 \(-1\leqslant x\leqslant0\). 直线 \(x=-t\) 将该区域二等分，故
\[
...
2t^3-6t^2+6t-1=0
\]
即 \(2(t-1)^3=-1\). 由于 \(0<t<1\)，取该方程的实根得
\(t-1=-\frac1{\sqrt[3]{2}}\)，\(t=1-\frac1{\sqrt[3]{2}}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 因为 \(AB\perp AD\)，\(AB\perp AP\)，且 \(AD\) 与 \(AP\) 是平面 \(PAD\) 内的相交直线，所以 \(AB\perp\) 平面 \(PAD\). 又 \(AB\subset\) 平面 \(ABCD\)，所以平面 \(ABCD\perp\) 平面 \(PAD\).
...
\item 在平面 \(ABCD\) 中，过 \(E\) 作 \(EF\parallel DC\)，交 \(BC\) 的延长线于点 \(F\). 因为 \(ABCD\) 是矩形，\(EF\parallel DC\parallel AB\)，且 \(BF\parallel BC\)，所以 \(EF\perp BF\)，且 \(EF=AB=3\). 连接 \(PF\).
...
\item 按通常定义，椭圆的两个半轴不相等，故需分 \(\frac{m}{n}>1\) 与 \(\frac{m}{n}<1\) 两种情况讨论. 当 \(\frac{m}{n}>1\) 时，\(u=2mn>mn\) 恒成立.
...
\end{cases}
\]
两式相减得 \(ta_n=(t+1)^{n+1}-1\)，所以
\(a_n=\frac{1}{t}\left[(t+1)^{n+1}-1\right]\)，\(b_n=\frac{t+1}{t}\left[1-(t+1)^n\right]\)

\item 由上式可知 \(a_n+b_n=1\)，故圆 \(C_n\) 的圆心 \(O_n=(a_n,b_n)\) 在直线 \(x+y=1\) 上. 另外
\[
