...
\begin{solution}
由 \(\tan\alpha=\dfrac{\sqrt3}{3}\) 得 \(\alpha=\dfrac{\pi}{6}+k\pi\)，其中 \(k\in\Z\). 令 \(\alpha=2x+\dfrac{\pi}{3}\)，则
\[
...
\begin{problem}
\(1992\) 年底世界人口达到 \(54.8\text{ 亿}\)，若人口的年平均增长率为 \(x\%\)，\(2000\) 年底世界人口数为 \(y\)（\(\text{亿}\)），那么 \(y\) 与 \(x\) 的函数关系式是\fillinblank{}.
\end{problem}
...
\begin{solution}
原函数在 \(x\leqslant0\) 上递减，值域为 \([1,+\infty)\). 由 \(y=3x^2+1\) 且 \(x\leqslant0\)，得 \(x=-\sqrt{\dfrac{y-1}{3}}\). 交换自变量和因变量，反函数为
\(y=-\sqrt{\frac{x-1}{3}}\)，\(x\geqslant1\).
\end{solution}
...
\begin{solution}
设 \(z_1=a+b\i\)，\(z_2=c+d\i\)（\(a\)，\(b\)，\(c\)，\(d\in\R\)）. 由
\(z_1+z_2=\i\) 比较实部和虚部，得
\(a+c=0\)，\(b+d=1\).
又由 \(|z_1|=|z_2|=1\)，有
\(a^2+b^2=1\)，\(c^2+d^2=1\).
利用 \(c=-a\) 将两式相减，得 \(b^2=d^2\)，即
\[
(b-d)(b+d)=0
\]
因为 \(b+d=1\ne0\)，所以 \(b=d=\frac{1}{2}\)，且 \(c=-a\). 再由
\(a^2+b^2=1\)，得
\(a^2=1-\frac{1}{4}=\frac{3}{4}\)，\(a=\pm\frac{\sqrt{3}}{2}\).
于是
...
\begin{solution}
\begin{enumerate}
\item 由 \(O\) 是 \(BC\) 的中点且 \(BC=2\)，得 \(B=(0,-1,0)\)，\(C=(0,1,0)\). 三角形 \(BDC\) 在 \(D\) 处直角，故 \(BC\) 是斜边；又 \(\angle DCB=30^\circ\)，所以 \(CD=BC\cos30^\circ=\sqrt{3}\). 由图中 \(D\) 位于 \(z\) 轴正向一侧，且 \(D\) 在平面 \(yoz\) 上，得
\(z_D=CD\sin30^\circ=\frac{\sqrt{3}}{2}\)，\(y_D=1-CD\cos30^\circ=-\frac{1}{2}\).
因此
\[
...
\(\overrightarrow{AD}=\left(-\frac{\sqrt{3}}{2},-1,\frac{\sqrt{3}}{2}\right)\)，\(\overrightarrow{BC}=(0,2,0)\).
于是
\[
...
\item 因为 \(f(x)=(x+1)^2\)，抛物线与 \(x\) 轴相切于 \((-1,0)\)，与 \(y\) 轴交于 \((0,1)\)，且在区间 \([-1,0]\) 上 \(f(x)\geqslant0\)，所以两坐标轴与图象围成的面积为
\[
...
\]
所以
\(x^2=\frac{m^2n^2}{n^2+m^2\tan^2\theta}\)，\(y^2=x^2\tan^2\theta =\frac{m^2n^2\tan^2\theta}{n^2+m^2\tan^2\theta}\).
...
\begin{enumerate}
\item 当 \(m\geqslant n\)，即 \(\frac{n}{m}\leqslant1\) 时，
\[
...
由于 \(0<\theta\leqslant\frac{\pi}{4}\)，\(0<\tan\theta\leqslant1\)，且 \(\frac{n}{m}\leqslant1\)，故取 \(\tan\theta=\frac{n}{m}\)，得 \(u=2mn\). 当 \(m=n\) 时取 \(\theta=\frac{\pi}{4}\)，该结论仍成立.
        ...
        \item 当 \(m<n\)，即 \(\frac{n}{m}>1\) 时，令 \(t_i=\tan\theta_i\)（\(i=1\)，\(2\)）. 对任意 \(0<\theta_1<\theta_2\leqslant\frac{\pi}{4}\)，有 \(0<t_1<t_2\leqslant1\)，且
\[
...
\]
令 \(s=\sqrt{13}\)，则
\(q=\frac{s-2}{s+2}\)，\(q^2=\frac{(s-2)^2}{(s+2)^2}\)，\(1-q^2=\frac{(s+2)^2-(s-2)^2}{(s+2)^2} =\frac{8s}{(s+2)^2}\).
因此
\[
S=\frac{4\pi}{1-q^2}
=\frac{\pi(s+2)^2}{2s}
