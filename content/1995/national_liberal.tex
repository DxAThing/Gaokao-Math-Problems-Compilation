...
\begin{problem}
已知 \(\theta\) 是第三象限角，且 \(\sin^4\theta + \cos^4\theta = \frac{5}{9}\)，那么 \(\sin 2\theta\) 等于 21. 解方程 \(3^{x + 2} - 3^{x - 2} = 80\)（\quad）
...
\begin{problem}
在 \((1 - x^{2})(1 + x^{2})^{10}\) 的展开式中，\(x^{6}\) 的系数是（\quad）
...
\begin{problem}
已知直线 \(l \perp\) 平面 \(\alpha\)，直线 \(m \subset\) 平面 \(\beta\)，有下面四个命题: \circled{1} \(\alpha \parallel \beta \Rightarrow l \perp m;\) \circled{2} \(\alpha \perp \beta \Rightarrow l \parallel m;\) \circled{3} \(l \notin m \Rightarrow \alpha \perp \beta\); \circled{4} \(l \perp m \Rightarrow \alpha \parallel \beta\). 其中正确的两个命题是（\quad）
...
\begin{problem}
已知圆台上、下底面圆周都在球面上，且下底面过球心，母线与底面所成的角为 \(\frac{2}{3}\)，则圆台的体积与球体积之比为 \fillinblank{}.
...
\begin{problem}
设 \(\{a_{n}\} \) 是由正数组成的等比数列， \(S_{n} \) 是其前 \(n\) 项和，证明： \(\frac{\log_{0.9}S_{n}+\log_{0.9}S_{n+2}}{2}>\log_{0.5}S_{n+1}. \)

\end{problem}

\begin{problem}
如图，圆柱的轴截面 \(ABCD\) 是正方形，点 \(E\) 在底面的圆周上，\(AF \perp DE \) ，\(F\) 是垂足. （1）求证: \(AF \perp DB\); （2）如果圆柱与三棱锥 \(D\text{-}ABE\) 的体积的比等于 \(3\pi \) ，求点 \(E\) 到截面 \(ABCD\) 的距离.

\end{problem}

\begin{problem}
某地为促进淡水鱼养殖业的发展，将价格控制在适当范围内，决定对淡水鱼养殖提供政府补贴. 设淡水鱼的市场价格为 \(x\) 元/千克，政府补贴为 \(1\) 元/千克. 根据市场调查，当 \(8 \leqslant x \leqslant 14\) 时，淡水鱼的市场日供应量 \(P\) 千克与市场日需求量 \(Q\) 千克近似满足关系：\(P = 1000(x + t - 8) (x \geqslant 8, t \geqslant 0)\)，\(Q = 500\sqrt{40 - (x - 8)^2} (8 \leqslant x \leqslant 14)\). 当 \(P = Q\) 时市场价格称为市场平衡价格. （1）将市场平衡价格表示为政府补贴的函数，并求出函数的定义域; （2）为使市场平衡价格不高于每千克 10 元，政府补贴至少为每千克多少元\(\perp\)

\end{problem}

\begin{problem}
已知椭圆 \(\frac{x^2}{24} + \frac{y^2}{16} = 1\)，直线 \(t: x = 12\). \(P\) 是 \(l\) 上一点，射线 \(OP\) 交椭圆于点 \(R\)，又点 \(Q\) 在 \(OP\) 上且满足 \(|OQ| \cdot |OP| = |OR|^2\)，当点 \(P\) 在 \(l\) 上移动时，求点 \(Q\) 的轨迹方程，并说明轨迹是什么曲线.
\end{problem}
