...
设 \(a\)，\(b\) 的方向向量分别为 \(\bs{u}\)，\(\bs{v}\). 异面直线不平行，故 \(\bs{u}\times\bs{v}\neq\bs{0}\). 取法向量为 \(\bs{u}\times\bs{v}\) 的平面 \(\beta\)，并将其平移至既不与 \(a\) 也不与 \(b\) 相交的位置. 由于 \(\bs{u}\)，\(\bs{v}\) 都垂直于 \(\bs{u}\times\bs{v}\)，所以 \(a\parallel\beta\)，\(b\parallel\beta\)，两直线与 \(\beta\) 所成的角都为 \(0\)，命题 C 正确. 取 \(A\in a\)，\(B\in b\)，作过直线 \(a\) 和点 \(B\) 的平面 \(\gamma\). 直线 \(a\subset\gamma\)，且 \(b\) 在点 \(B\) 处与 \(\gamma\) 相交，所以 \(a\)，\(b\) 到 \(\gamma\) 的距离都为 \(0\)，命题 D 正确.

若存在过 \(a\) 且垂直于 \(b\) 的平面 \(\alpha\)，则 \(b\perp\alpha\). 设 \(a\)，\(b\) 的方向向量分别为 \(\bs{u}\)，\(\bs{v}\)，因为 \(a\subset\alpha\)，所以 \(\bs{u}\) 平行于 \(\alpha\)；又因为 \(b\perp\alpha\)，所以 \(\bs{v}\) 是 \(\alpha\) 的法向量，从而 \(\bs{u}\cdot\bs{v}=0\)，即两直线的方向垂直. 但异面直线的方向不一定垂直，例如 \(a:(x,y,z)=(t,0,0)\) 与 \(b:(x,y,z)=(s,s,1)\) 是异面直线，而其方向向量的内积为 \(1\neq0\). 因此该命题不能对所有异面直线成立，B 项错误，故选 B.
\end{solution}
...
\begin{solution}
由 \(\sin 2x=\frac12\)，得 \(2x=\frac\pi6+2k\pi\) 或 \(2x=\frac{5\pi}{6}+2k\pi\)，其中 \(k\in\Z\)，即
\[
x=\frac\pi{12}+k\pi\quad\text{或}\quad x=\frac{5\pi}{12}+k\pi
\]
对第一类解，代入 \(-2\pi\leqslant x\leqslant2\pi\)，得到 \(-\frac{25}{12}\leqslant k\leqslant\frac{23}{12}\)，所以 \(k=-2\)，\(-1\)，\(0\)，\(1\)，有 \(4\) 个解. 对第二类解，得到 \(-\frac{29}{12}\leqslant k\leqslant\frac{19}{12}\)，同样只有 \(k=-2\)，\(-1\)，\(0\)，\(1\)，有 \(4\) 个解. 两类解不可能相等，因为这将导致 \(\frac\pi{12}+k\pi=\frac{5\pi}{12}+l\pi\)，即 \(l-k=-\frac13\)，与 \(k\)，\(l\in\Z\) 矛盾. 因此共有 \(4+4=8\) 个解.
\end{solution}
...
\begin{solution}
先求两向量的坐标：\(\bs{a}+\bs{b}=(m+2)\bs{i}+(m-4)\bs{j}\)，\(\bs{a}-\bs{b}=m\bs{i}-(m+2)\bs{j}\). 垂直条件给出它们的数量积为零，即
\[
...
\begin{solution}
\begin{enumerate}
\item 在三棱柱中，\(C'B'\parallel CB\)，由已知 \(C'B'\perp AB\)，得 \(CB\perp AB\). 又因为矩形 \(BCC'B'\) 的邻边互相垂直，所以 \(CB\perp BB'\). 菱形 \(A'ABB'\) 所在的平面就是平面 \(A'AB\)，其中直线 \(AB\) 与 \(BB'\) 相交于 \(B\). 因此，直线 \(CB\) 垂直于平面 \(A'AB\). 又 \(CB\subset\) 平面 \(CA'B\)，所以平面 \(CA'B\perp\) 平面 \(A'AB\).
\item 由 \(CB\perp\) 平面 \(A'AB\) 及 \(C'B'\parallel CB\)，得 \(C'B'\perp\) 平面 \(A'AB\). 因为 \(C'B'\) 在平面 \(BCC'\) 内，所以平面 \(A'AB\perp\) 平面 \(BCC'\)，两平面的交线为 \(BB'\). 在平面 \(A'AB\) 内过 \(A\) 作 \(AH\perp BB'\)，则由两平面垂直的性质，\(AH\perp\) 平面 \(BCC'\)，且垂足 \(H\) 在 \(BB'\) 上. 连接 \(C'H\)，则 \(\angle AC'H\) 是直线 \(AC'\) 与平面 \(BCC'\) 所成的角.
...
\begin{solution}
\begin{enumerate}
\item 因为 \(z_1\)，\(z_2\) 是实系数一元二次方程的两个非实根，所以它们互为共轭复数. 设 \(z_1=a+b\i\)，\(z_2=a-b\i\)，其中 \(a\)，\(b\in\R\)，且 \(b\neq0\). 由 \(z_1^2=z_2\)，得
\[
a^2-b^2+2ab\i=a-b\i
\]
比较实部和虚部，得到
...
(z_1,z_2)=\left(-\frac12-\frac{\sqrt3}{2}\i,-\frac12+\frac{\sqrt3}{2}\i\right)
\]
\item 由 \(z_1=1+m\i\) 且 \(|z_1|\leqslant\sqrt2\)，得 \(1+m^2\leqslant2\). 由于 \(z_1\) 是虚数，\(m\neq0\)，所以 \(0<m^2\leqslant1\). 又 \(z_2=z_1^2=(1-m^2)+2m\i\)，从而 \(\omega=(4-m^2)+2m\i\). 于是
\[
...
\begin{solution}
\begin{enumerate}
\item 联立两条抛物线方程，得 \(-x^2=x^2-2ax\)，即 \(2x(x-a)=0\). 所以交点为 \(O(0,0)\)，\(A(a,-a^2)\). 当 \(0\leqslant x\leqslant a\) 时，\(C_2\) 在 \(C_1\) 下方，旋转后的外半径为 \(2ax-x^2\)，内半径为 \(x^2\). 因此
\[
...
\]
\item 若 \(l\) 为 \(y\) 轴，则它与 \(C_2\) 只有交点 \(O\)，不能围成面积为 \(\frac92a^3>0\) 的图形. 因此可设直线 \(l\) 的方程为 \(y=kx\). 与 \(C_2\) 联立，得 \(kx=x^2-2ax\)，即 \(x(x-k-2a)=0\)，所以两个交点的横坐标为 \(0\) 和 \(k+2a\). 令 \(r=k+2a\).
...
\begin{solution}
\begin{enumerate}
\item 抛物线 \(y^2=p(x+1)\) 可写为 \(y^2=4a(x+1)\)，其中 \(a=\frac p4\)，所以其准线为 \(x=-1-\frac p4\). 直线与 \(x\) 轴的交点为 \((m,0)\)，由题意得 \(m>-1-\frac p4\)，即 \(4m+p+4>0\).
...
\item 设 \(Q=(x_1,y_1)\)，\(R=(x_2,y_2)\). 由上式和韦达定理，\(x_1+x_2=2m+p\)，\(x_1x_2=m^2-p\). 又 \(Q\)，\(R\) 在直线 \(x+y=m\) 上，所以 \(y_1=m-x_1\)，\(y_2=m-x_2\).
...
\item 抛物线的焦点为 \(F\left(-1+\frac p4,0\right)\). 由点到直线距离公式，得
\[
...
\item 设数列 \(\{a_{n}\}\) 的公比为 \(f(t)\)，作数列 \(\{b_{n}\}\)，使 \(b_{1}=1\)，\(b_{n}=f\left(\frac{1}{b_{n-1}}\right)\)（\(n=2\)，\(3\)，\(4\)，\(\dots\)），求 \(b_{n}\)；
...
\begin{solution}
\begin{enumerate}
\item 由 \(S_1=a_1=1\)，\(S_2=a_1+a_2=1+a_2\)，将 \(n=2\) 代入已知关系式，得 \(3t(1+a_2)-(2t+3)=3t\)，所以 \(a_2=\frac{2t+3}{3t}\).
...
\item 将公比写成 \(f(t)=\frac23+\frac1t\)，则
\[
