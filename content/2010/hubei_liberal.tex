...
\begin{solution}
命题\(\circled{1}\)成立，因为平行于同一直线的两条直线互相平行. 命题\(\circled{2}\)不成立，两条分别垂直于同一直线的直线不一定互相垂直. 命题\(\circled{3}\)不成立，平行于同一平面的两条直线不一定互相平行. 命题\(\circled{4}\)成立，因为垂直于同一平面的两条直线互相平行. 所以正确的命题是\(\circled{1}\)\(\circled{4}\)，选 C.
\end{solution}
...
\]
令 \(\varphi'(x)=0\)，得 \(x>2\) 且 \(\sqrt{4x-x^2}=x-2\)，从而 \(x=2+\sqrt{2}\). 结合端点值 \(\varphi(0)=3\)，\(\varphi(4)=-1\)，可得
\[
\varphi(2+\sqrt{2})=1-2\sqrt{2},\qquad
\min\varphi=1-2\sqrt{2},\quad \max\varphi=3.
\]
...
\begin{solution}
\begin{enumerate}
\item 建立直角坐标系，取 \(O=(0,0,0)\)，\(A=(1,0,0)\)，\(B=\left(-\frac12,\frac{\sqrt{3}}2,0\right)\)，\(C=(0,0,1)\). 这些坐标满足题设的长度、垂直和夹角条件. 因为 \(P\) 是 \(AC\) 的中点，所以 \(P=\left(\frac12,0,\frac12\right)\). 又 \(AQ=\frac13AB\)，故
\[
...
\[
\overrightarrow{AB}=\left(-\frac32,\frac{\sqrt{3}}2,0\right),\qquad
...
由 \(F=(1,0)\)，有
\[
\overrightarrow{FA}=(u^2-1,2u),\qquad
\overrightarrow{FB}=(v^2-1,2v).
\]
记 \(s=u+v\)，则 \(uv=-m\)，从而
\[
\overrightarrow{FA}\cdot\overrightarrow{FB}=(u^2-1)(v^2-1)+4uv=u^2v^2-u^2-v^2+1+4uv=m^2-(s^2+2m)+1-4m=m^2-6m+1-s^2.
\]
对过 \(M\) 的任意割线，\(s\) 可以取任意实数，特别地，竖直直线给出 \(s=0\). 因而要使上述内积对任意割线都严格小于 \(0\)，必须且只需 \(m^2-6m+1<0\). 解得
\[
...
\[
\phi(0)=6,\qquad \phi\left(\frac a2\right)=6-\frac{a^3}{4}.
\]
方程 \(\phi(x)=0\) 有三个不同实根，当且仅当局部极小值小于 \(0\)，即 \(6-\frac{a^3}{4}<0\). 因此 \(a^3>24\)，即 \(a>\sqrt[3]{24}\). 由第（2）问，三个不同根对应三条不同切线，所以 \(a\) 的取值范围为 \((\sqrt[3]{24},+\infty)\).
\end{enumerate}
\end{solution}
