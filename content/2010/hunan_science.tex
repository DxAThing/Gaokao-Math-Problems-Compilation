...
\begin{solution}
因为 \(1\in M\) 且 \(1\notin N\)，所以 \(M\not\subseteq N\)；因为 \(4\in N\) 且 \(4\notin M\)，所以 \(N\not\subseteq M\).

由集合交集的定义，既属于 \(M\) 又属于 \(N\) 的元素是 \(2,3\)，所以
\(M\cap N=\{2,3\}\). 又 \(M\cup N=\{1,2,3,4\}\ne\{1,4\}\). 因此选 C.
\end{solution}
...
\begin{solution}
选项 A 中，指数函数的值恒为正，所以对任意 \(x\in\R\)，都有
\(2^{x-1}>0\)，是真命题.

选项 B 中，取正整数 \(x=1\)，则 \((x-1)^2=0\)，不满足严格不等式，所以该命题是假命题.

选项 C 中，取 \(x=1\)，有 \(\lg1=0<1\)，所以该存在性命题为真. 选项 D 中，取 \(x=\arctan2\)，有 \(\tan x=2\)，所以该命题也为真. 因此选 B.
\end{solution}
...
\begin{solution}
由 \(x=\rho\cos\theta\)，\(y=\rho\sin\theta\)，将 \(\rho=\cos\theta\) 两边乘以 \(\rho\)，得
\(\rho^2=\rho\cos\theta=x\)，即
\(x^2+y^2=x\)，整理为
\(\left(x-\frac12\right)^2+y^2=\frac14\)，这是圆的方程.

由参数方程 \(t=-1-x\)，代入 \(y=2+3t\)，得
\(y=2+3(-1-x)=-1-3x\)，即 \(3x+y+1=0\)，这是直线的方程. 因此选 A.
\end{solution}
...
\begin{solution}
因为 \(\angle C=90^{\circ}\)，所以 \(\overrightarrow{AC}\perp\overrightarrow{CB}\). 又
\(\overrightarrow{AB}=\overrightarrow{AC}+\overrightarrow{CB}\)，从而
\[
\overrightarrow{AB}\cdot\overrightarrow{AC}
=(\overrightarrow{AC}+\overrightarrow{CB})\cdot\overrightarrow{AC}
=|\overrightarrow{AC}|^2+\overrightarrow{CB}\cdot\overrightarrow{AC}
=4^2+0=16
\]
因此选 D.
\end{solution}
...
\begin{solution}
在积分区间 \([2,4]\) 上，\(x>0\)，且 \(\displaystyle\int\frac{1}{x}\,\mathrm{d}x=\ln x\). 因此
\[
\int_2^4\frac{1}{x}\,\mathrm{d}x
...
\begin{solution}
由余弦定理以及 \(\cos120^{\circ}=-\frac12\)，有
\[
c^2=a^2+b^2-2ab\cos120^{\circ}=a^2+b^2+ab
\]
代入 \(c=\sqrt2a\)，得 \(2a^2=a^2+b^2+ab\)，即
\(b^2+ab-a^2=0\). 由于 \(a\)，\(b>0\)，令 \(r=\frac ba>0\)，则
\(r^2+r-1=0\)，所以正根为 \(r=\frac{\sqrt5-1}{2}\).

因为 \(0<\frac{\sqrt5-1}{2}<1\)，所以 \(b<a\)，即 \(a>b\). 因此选 A.
\end{solution}
...
\begin{solution}
每一位只能取 \(0\) 或 \(1\)，共有 \(2^4=16\) 个四位排列. 与 \(0110\) 恰有 \(k\) 个对应位置上的数字相同的排列，先选出这 \(k\) 个位置，其余位置的数字被唯一确定为不同数字，故有 \(\mathrm C_4^k\) 个.

“至多有两个对应位置上的数字相同”包含 \(k=0\)，\(1\)，\(2\)，所以信息个数为
\[
\mathrm C_4^0+\mathrm C_4^1+\mathrm C_4^2=1+4+6=11
\]
因此选 B.
\end{solution}
...
\begin{solution}
函数值非负，且 \(f(x)=0\) 当且仅当 \(x=0\) 或 \(x=-t\). 若 \(t=0\)，零点集合只有 \(\{0\}\)，关于直线 \(x=-\frac12\) 的对称点 \(-1\) 不是零点，故不可能.

若 \(t\ne0\)，两个不同零点在该对称变换下必须互相交换，因此 \(0\) 的对称点 \(-1\) 必须等于另一个零点 \(-t\)，从而 \(t=1\). 反之，当 \(t=1\) 时，令 \(x=-\frac12+u\)，则
\[
f\left(-\frac12+u\right)=\min\left\{\left|u-\frac12\right|,\left|u+\frac12\right|\right\}
\]
将 \(u\) 换成 \(-u\) 后函数值不变，所以图象确实关于 \(x=-\frac12\) 对称. 因此选 D.
\end{solution}
...
\begin{solution}
试验区间长度为 \(210-110=100\mathrm{~g}\). 按 \(0.618\) 法，第一次试点取距某一端点为区间长度的 \(0.618\) 倍，因而两个对称的可选试点分别为
\(110+0.618\times100=171.8\mathrm{~g}\)，\(210-0.618\times100=148.2\mathrm{~g}\).
所以第一次试点的加入量可以是 \(171.8\mathrm{~g}\) 或 \(148.2\mathrm{~g}\).
\end{solution}
...
\begin{solution}
由点幂定理，切线长的平方等于割线两段的乘积，即
\(PT^2=PA\cdot PB\). 代入 \(PT=4\)，\(PA=2\)，得
\(16=2PB\)，所以 \(PB=8\). 由于点的顺序为 \(P\)，\(A\)，\(B\)，有
\(AB=PB-PA=8-2=6\).
\end{solution}
...
\begin{solution}
在区间 \([-1,2]\) 上随机取数，样本区间长度为 \(3\). 满足 \(|x|\leqslant1\) 的数构成区间 \([-1,1]\)，其长度为 \(2\). 端点对概率没有影响，因此所求概率为
\[
P=\frac{1-(-1)}{2-(-1)}=\frac23
\]
\end{solution}
...
\begin{solution}
程序开始时 \(i=1\)，每次先判断 \(i\leqslant n\)，若成立则执行
\(s\leftarrow s+i^2\)，再执行 \(i\leftarrow i+1\)，然后重新判断. 因此程序输出的数为
\[
s=1^2+2^2+\cdots+n^2
\]
题目要求输出 \(1^2+2^2+\cdots+100^2\)，故循环应恰好取到 \(i=100\)，从而 \(n=100\).
\end{solution}
...
\begin{solution}
由三个视图可知，该几何体可看作由同一顶点引出的三条互相垂直的棱组成的四面体，其三条棱长分别为 \(5\mathrm{~cm}\)、\(6\mathrm{~cm}\) 和 \(h \mathrm{~cm}\). 因此体积为
\[
V=\frac16\times5\times6\times h=5h
\]
由题设 \(V=20 \mathrm{~cm}^{3}\)，得 \(5h=20\)，所以 \(h=4 \mathrm{~cm}\).
\end{solution}
...
\begin{solution}
抛物线 \(x^2=2py\) 的焦点为 \(\left(0,\frac p2\right)\)，故过焦点且斜率为 \(1\) 的直线为
\(y=x+\frac p2\). 与抛物线联立，得
\[
x^2=2p\left(x+\frac p2\right)=2px+p^2
\]
即 \(x^2-2px-p^2=0\). 所以交点的横坐标为
\(x_A=p(1-\sqrt2)\)，\(x_B=p(1+\sqrt2)\)，相应纵坐标为
\(y_A=p\left(\frac32-\sqrt2\right)\)，\(y_B=p\left(\frac32+\sqrt2\right)\).
点 \(D\)，\(C\) 分别是 \(A\)，\(B\) 在 \(x\) 轴上的正射影，故 \(AD\parallel BC\)，且
\(AD=y_A\)，\(BC=y_B\)，\(DC=x_B-x_A=2\sqrt2p\).
梯形 \(ABCD\) 的面积为
\[
S=\frac{AD+BC}{2}\cdot DC
=\frac{3p}{2}\cdot2\sqrt2p=3\sqrt2p^2
\]
由 \(S=12\sqrt2\) 且 \(p>0\)，得 \(p^2=4\)，所以 \(p=2\).
\end{solution}
...
\begin{solution}
由定义，\((a_n)^*\) 是满足 \(a_m<n\) 的正整数 \(m\) 的个数. 已知 \(a_m=m^2\)，所以
\[
(a_n)^*=\#\{m\in\N^*:m^2<n\}=\left\lfloor\sqrt{n-1}\right\rfloor
\]
当 \(n=5\) 时，满足 \(m^2<5\) 的正整数 \(m\) 为 \(1,2\)，故 \((a_5)^*=2\).

再记 \(b_m=(a_m)^*=\lfloor\sqrt{m-1}\rfloor\)，则
\[
((a_n)^*)^*=\#\{m\in\N^*:b_m<n\}
\]
由 \(\lfloor\sqrt{m-1}\rfloor<n\) 等价于 \(m-1<n^2\)，且 \(m\) 为正整数，得 \(1\leqslant m\leqslant n^2\). 所以共有 \(n^2\) 个这样的 \(m\)，即
\( ((a_n)^*)^*=n^2\).
\end{solution}
...
\]
由于 \(B\) 是平面 \(A_{1}BE\) 上的点，且
\(\overrightarrow{BB_{1}}\cdot\bs{n}=-a^3\ne0\)，所以直线 \(B_{1}F\) 不在该平面内. 因而要使 \(B_{1}F\parallel\) 平面 \(A_{1}BE\)，必须且只需其方向向量满足 \(\overrightarrow{B_{1}F}\cdot\bs{n}=0\)，即
\[
-\frac{a^3}{2}+a^2t=0
\]
...
\(C_{1}:\left(x-4\right)^2+y^2=\frac{36}{5}\)，\(x\geqslant2\).
...
\(C_{2}:\frac{x^2}{20}+\frac{y^2}{4}=1\)，\(x<2\).
...
\]
该椭圆边界点到原直线的垂足为 \((-4\sqrt{3},2)\)，其纵坐标在 \([-1,6]\) 内，故垂足确在线段 \(P_{1}P_{2}\) 上，以上距离确实是该线段的最小移动距离.
直线段 \(P_{2}P_{3}\) 所在直线为 \(y=6\). 考察区域的最高点纵坐标为圆 \(C_{1}\) 的最高点纵坐标 \(\frac{6\sqrt{5}}{5}\)，所以该线段向下移动至考察区域至少需要
\[
6-\frac{6\sqrt{5}}{5}>3\text{ km}
\]
圆的最高点横坐标为 \(4\)，位于线段 \(P_{2}P_{3}\) 的横坐标范围内，所以这个距离也确实可以在该线段上取得.
...
\(\frac{f(c)-f(b)}{c^2-b^2}=\frac{c+4}{c+2}=\frac{6+\varepsilon}{4+\varepsilon}\longrightarrow\frac{3}{2}\)，\(\left(\varepsilon\to0^+\right)\).
...
反之，若 \(a>\frac{4}{9}\)，则 \(a_2=3a\)，且 \(3a>1\). 又 \(9a>4\). 因而
\(3^1a>1^2\)，\(3^2a>2^2\). 对 \(n\geqslant2\)，若 \(3^na>n^2\)，则由 \(3n^2-(n+1)^2=2n^2-2n-1>0\)，得 \(3n^2>(n+1)^2\). 又因 \(3^{n+1}a>3n^2\)，所以 \(3^{n+1}a>(n+1)^2\). 由数学归纳法，\(3^na>n^2\) 对所有 \(n\geqslant1\) 成立，递推式始终取 \(a_{n+1}=3a_n\). 所以 \(a_n=3^{n-1}a\) 是等比数列.
