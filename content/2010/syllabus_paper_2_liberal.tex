...
\begin{enumerate}
\item \(a_n=2^{n-1}\).
\item \(T_n=\displaystyle\frac{4^n-1}{3}+2n+\frac{4}{3}\left(1-4^{-n}\right)\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(DE\) 为异面直线 \(AB_{1}\) 与 \(CD\) 的公垂线.
\item 二面角 \(A_{1}-AC_{1}-B_{1}\) 的大小为 \(\arctan\sqrt{14}\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
\item \(p=0.9\).
\item 电流能在 \(M\) 与 \(N\) 之间通过的概率为 \(0.9891\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 增区间为 \(\left(-\infty,2-\sqrt{3}\right)\)、\(\left(2+\sqrt{3},+\infty\right)\)，减区间为 \(\left(2-\sqrt{3},2+\sqrt{3}\right)\).
\item \(a\) 的取值范围为 \(\left(\frac{5}{4},\frac{5}{3}\right)\).
\end{enumerate}
\end{solution}
