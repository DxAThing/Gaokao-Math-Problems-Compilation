...
\(l_1:\ y=z=0\)，\(l_2:\ x=0\)，\(\ z=h\).
对任意点 \(P(x,y,z)\)，其到两直线的距离分别满足
\(d(P,l_1)^2=y^2+z^2\)，\(d(P,l_2)^2=x^2+(z-h)^2\).
...
\begin{solution}
每天的两人不计先后，因此不受限制的安排总数为 \(\frac{6!}{(2!)^3}=90\). 设事件 \(A\) 为甲值 14 日，事件 \(B\) 为乙值 16 日. 若甲值 14 日，先从其余 5 人中选 1 人与甲同日，再从剩余 4 人中选 2 人值 15 日，故 \(|A|=\mathrm C_5^1\mathrm C_4^2=30\). 若乙值 16 日，先从其余 5 人中选 1 人与乙同日，再从剩余 4 人中选 2 人值 15 日，故 \(|B|=\mathrm C_5^1\mathrm C_4^2=30\).
...
\begin{solution}
设三圆的圆心分别为 \(O_1\)，\(O_2\)，\(O_3\)，并记 \(\beta_1=\angle O_2PO_3\)，\(\beta_2=\angle O_3PO_1\)，\(\beta_3=\angle O_1PO_2\). 由图中三条射线 \(PO_1\)，\(PO_2\)，\(PO_3\) 按顺序把点 \(P\) 周围的周角分成三个角，故
\[
\beta_1+\beta_2+\beta_3=2\pi
\]
设第 \(i\) 个圆与另外两个圆的另一个交点分别为 \(Q_{ij}\) 和 \(Q_{ik}\)，其中 \(\{i,j,k\}=\{1,2,3\}\). 由于两圆半径相等且都经过 \(P\)，四边形 \(P O_i Q_{ij} O_j\) 是菱形. 因此，在以 \(O_i\) 为圆心的圆上，经过 \(P\) 的虚线弧从 \(Q_{ij}\) 到 \(P\) 的圆心角为 \(\pi-\beta_k\)，从 \(P\) 到 \(Q_{ik}\) 的圆心角为 \(\pi-\beta_j\). 于是该虚线弧所对的圆心角为
\[
...
\item 此时 \(g(x)=-\frac{1}{3}x^3+2x\)，所以 \(g'(x)=2-x^2\). 当 \(x<-\sqrt{2}\) 或 \(x>\sqrt{2}\) 时，\(g'(x)<0\)；当 \(-\sqrt{2}<x<\sqrt{2}\) 时，\(g'(x)>0\)，且 \(g'(\pm\sqrt{2})=0\). 因此 \(g(x)\) 在 \((-\infty,-\sqrt{2}]\) 和 \([\sqrt{2},+\infty)\) 上单调递减，在 \([-\sqrt{2},\sqrt{2}]\) 上单调递增.
...
\begin{solution}
\begin{enumerate}
\item 由 \(PA\perp\) 底面 \(ABCD\)，得 \(PA\perp AB\). 又 \(PA=AB\)，所以直角三角形 \(PAB\) 是等腰直角三角形. 在等腰直角三角形 \(PAB\) 中，斜边中线 \(AE\) 同时是高，故 \(AE\perp PB\). 由矩形性质，\(BC\perp AB\)；又 \(PA\perp\) 底面 \(ABCD\)，所以 \(BC\perp PA\). 因此 \(BC\perp\) 平面 \(PAB\)，从而 \(BC\perp AE\). 直线 \(PB\) 与 \(BC\) 相交且都在平面 \(PBC\) 内，且 \(AE\perp PB\)，\(AE\perp BC\)，所以 \(AE\perp\) 平面 \(PBC\).
\item 由 \(PA\perp\) 底面 \(ABCD\) 及矩形性质，\(BC\perp AB\) 且 \(BC\perp PA\)，从而 \(BC\perp\) 平面 \(PAB\). 又 \(AD\parallel BC\)，所以 \(AD\perp\) 平面 \(PAB\). 因此 \(BC\perp BE\)，且 \(AD\perp AE\). 又 \(PB=\sqrt{PA^2+AB^2}=2\)，所以 \(BE=1\). 在直角三角形 \(PAB\) 中，斜边中线 \(AE=\frac{PB}{2}=1\)，进而 \(CE=\sqrt{BE^2+BC^2}=\sqrt{2}\)，\(DE=\sqrt{AD^2+AE^2}=\sqrt{2}\)，而 \(CD=AB=\sqrt{2}\). 所以 \(\triangle CED\) 为等边三角形.
...
\item 如图，已知过点 \(M(x_1, y_1)\) 的直线 \(l_1: x_1x + 4y_1y = 4\) 与过点 \(N(x_2, y_2)\)（其中 \(x_2\ne x_1\)）的直线 \(l_2: x_2x + 4y_2y = 4\) 的交点 \(E\) 在双曲线 \(C\) 上，直线 \(MN\) 与两条渐近线分别交于 \(G\)，\(H\) 两点，求 \(\overrightarrow{OG} \cdot \overrightarrow{OH}\) 的值.
\end{enumerate}
