...
\begin{problem}
若变量 \(x\)，\(y\) 满足约束条件 \(\left\{ \begin{array}{l} x \geqslant -1, \\ y \geqslant x, \\ 3x + 2y \leqslant 5, \end{array} \right.\) 则 \(z = 2x + y\) 的最大值为（\quad）
...
\choices
    {向左平移 \(\frac{\pi}{4}\) 个长度单位}
    {向右平移 \(\frac{\pi}{4}\) 个长度单位}
    {向左平移 \(\frac{\pi}{2}\) 个长度单位}
    {向右平移 \(\frac{\pi}{2}\) 个长度单位}
\end{problem}
...
\begin{problem}
已知 \(\alpha\) 是第二象限的角，\(\tan (\pi + 2\alpha) = -\frac{4}{3}\)，则 \(\tan \alpha =\)\fillinblank{}.
\end{problem}
...
\begin{enumerate}
\item 求 \(\lim\limits_{n\to\infty}\frac{a_{n}}{S_{n}}\)；
...
\end{enumerate}
\bitmapfigure[max width=0.44\linewidth,max totalheight=4.8cm]{img/2010/syllabus_paper_2_science/q19_fig1.png}
\end{problem}
...
\end{enumerate}
\bitmapfigure[max width=0.60\linewidth,max totalheight=4.8cm]{img/2010/syllabus_paper_2_science/q20_fig1.png}
\end{problem}
