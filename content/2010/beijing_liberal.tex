...
\begin{solution}
短答案：\(f\left(\frac{\pi}{3}\right)=-\frac{1}{4}\)；\(f(x)\) 的最大值为 \(2\)，最小值为 \(-1\).
\end{solution}
...
\begin{solution}
短答案：\(a_n=2n-12\)；\(\{b_n\}\) 的前 \(n\) 项和为 \(S_n=2(3^n-1)\).
\end{solution}
...
\begin{solution}
短答案：\(AF \parallel\) 平面 \(BDE\)；\(CF \perp\) 平面 \(BDE\).
\end{solution}
...
\begin{solution}
短答案：椭圆方程为 \(\dfrac{x^2}{3}+y^2=1\)；圆心 \(P\) 的坐标为 \(\left(0,\pm\dfrac{\sqrt{3}}{2}\right)\)；\(y\) 的最大值为 \(2\).
\end{solution}
...
\begin{solution}
短答案：\(A-B=(1,0,1,0,1)\)，\(d(A,B)=3\)；且 \(d(A-C,B-C)=d(A,B)\)，\(d(A,B)\)，\(d(A,C)\)，\(d(B,C)\) 中至少有一个是偶数.
\end{solution}
