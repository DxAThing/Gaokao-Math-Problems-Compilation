...
\bitmapfigure[max width=0.35\linewidth,max totalheight=5.0cm]{img/2010/beijing_liberal/q7_fig1.png}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=5.0cm]{img/2010/beijing_liberal/q8_fig1.png}
...
\begin{problem}
已知函数 \( y = \begin{cases} \log_{2} x, & x \geqslant 2 \\ 2 - x, & x < 2 \end{cases} \)，如图表示的是给定 \(x\) 的值，求其对应的函数值 \(y\) 的程序框图. \circled{1} 应填写\fillinblank{}；\circled{2} 应填写\fillinblank{}.

\bitmapfigure[max width=0.35\linewidth,max totalheight=6.0cm,scale=1.5]{img/2010/beijing_liberal/q9_fig1.png}
\end{problem}
...
\texfigure[max width=0.55\linewidth,max totalheight=5.8cm]{img_repaint/2010/beijing_liberal/q12_fig1.tex}
...
\begin{problem}
如图放置的边长为 1 的正方形 \(PABC\) 沿 \(x\) 轴滚动. 设顶点 \(P(x, y)\) 的纵坐标与横坐标的函数关系是 \(y = f(x)\)，则 \(f(x)\) 的最小正周期为\fillinblank{}；\(y = f(x)\) 在其两个相邻零点间的图象与 \(x\) 轴所围区域的面积为\fillinblank{}. 说明：“正方形 \(PABC\) 沿 \(x\) 轴滚动”包括沿 \(x\) 轴正方向和沿 \(x\) 轴负方向滚动. 沿 \(x\) 轴正方向滚动指的是先以顶点 \(A\) 为中心顺时针旋转，当顶点 \(B\) 落在 \(x\) 轴上时，再以顶点 \(B\) 为中心顺时针旋转，如此继续. 类似地，正方形 \(PABC\) 可以沿 \(x\) 轴负方向滚动.

\bitmapfigure[max width=0.32\linewidth,max totalheight=4.8cm]{img/2010/beijing_liberal/q14_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.44\linewidth,max totalheight=5.6cm]{img/2010/beijing_liberal/q17_fig1.png}
