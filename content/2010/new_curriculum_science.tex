...
\begin{solution}
由 \(\sqrt{x}\) 有意义且 \(\sqrt{x}\leqslant4\)，得 \(0\leqslant x\leqslant16\). 结合 \(x\in\Z\)，有 \(B=\{0,1,2,\ldots,16\}\). 又 \(A=[-2,2]\)，所以
\[
A\cap B=\{0,1,2\}
\]
故选 D.
...
\begin{solution}
由 \(z\overline{z}=|z|^{2}\)，且
\(|\sqrt{3}+\i|^{2}=3+1=4\)，\(|1-\sqrt{3}\i|^{2}=1+3=4\).
因此
\[
z\overline{z}=\left|\frac{\sqrt{3}+\i}{(1-\sqrt{3}\i)^2}\right|^{2}
=\frac{4}{|1-\sqrt{3}\i|^{4}}
=\frac{4}{4^{2}}=\frac14
\]
故选 A.
...
\begin{solution}
函数在 \(x=-1\) 处的导数为
\(y'=\frac{(x+2)-x}{(x+2)^2}=\frac{2}{(x+2)^2}\)，\(y'\big|_{x=-1}=2\).
所以切线过点 \((-1,-1)\)，斜率为 \(2\)，其方程为
\[
y+1=2(x+1)
\]
即 \(y=2x+1\). 故选 A.
...
\begin{solution}
质点初始时的极角为 \(-\dfrac{\pi}{4}\)，角速度为 \(1\)，故时刻 \(t\) 的纵坐标为
\[
...
\]
因此 \(d(0)=\sqrt{2}\)，在 \(0\leqslant t\leqslant\dfrac{\pi}{4}\) 上从 \(\sqrt{2}\) 递减到 \(0\)，随后递增到最大值 \(2\)，并以 \(\pi\) 为周期重复. 与题图相符的是选项 C.
\end{solution}
...
对 \(p_{1}\) 对应的函数求导，得
\[
\left(2^x-2^{-x}\right)'=\ln2\left(2^x+2^{-x}\right)>0
\]
所以 \(p_{1}\) 为真命题. 对 \(p_{2}\) 对应的函数求导，得
\[
\left(2^x+2^{-x}\right)'=\ln2\left(2^x-2^{-x}\right)
\]
该导数在 \(x<0\) 时为负、在 \(x>0\) 时为正，故该函数在 \(\R\) 上不是减函数，\(p_{2}\) 为假命题.

于是 \(q_{1}=p_{1}\vee p_{2}\) 为真，\(q_{2}=p_{1}\wedge p_{2}\) 为假，\(q_{3}=(\neg p_{1})\vee p_{2}\) 为假，\(q_{4}=p_{1}\wedge(\neg p_{2})\) 为真. 真命题为 \(q_{1}\)，\(q_{4}\)，故选 C.
\end{solution}
...
\begin{solution}
设未发芽的种子数为 \(Y\)，则 \(Y\) 服从二项分布 \(B(1000,0.1)\)，且补种数 \(X=2Y\). 因此
\(E(Y)=1000\times0.1=100\)，\(E(X)=2E(Y)=200\).
故选 B.
\end{solution}
...
\begin{solution}
框图从 \(k=1\)、\(S=0\) 开始，每次先累加 \(\dfrac{1}{k(k+1)}\)，再判断是否有 \(k<N\)；若是则令 \(k\) 增加 \(1\)，回到累加步骤. 因此 \(N=5\) 时实际取 \(k=1\)，\(2\)，\(3\)，\(4\)，\(5\)，输出
\[
...
\begin{solution}
由 \(f\) 为偶函数，对任意 \(u\) 都有 \(f(u)=f(|u|)\). 当 \(u\geqslant0\) 时，
\[
f(u)=u^3-8
\]
所以
\[
f(u)>0\Longleftrightarrow u^3>8\Longleftrightarrow u>2
\]
令 \(u=x-2\)，则
\[
f(x-2)>0\Longleftrightarrow |x-2|>2
\Longleftrightarrow x<0\text{或}x>4
\]
故选 B.
\end{solution}
...
由 \(\alpha\) 是第三象限角，得
\[
\sin\alpha=-\sqrt{1-\cos^2\alpha}=-\frac35
\]
利用半角正切恒等式，
\[
\frac{1+\tan\dfrac\alpha2}{1-\tan\dfrac\alpha2}
=\frac{1+\sin\alpha}{\cos\alpha}
=\frac{1-\frac35}{-\frac45}=-\frac12
\]
故选 A.
\end{solution}
...
\begin{solution}
底面是边长为 \(a\) 的正三角形，侧棱垂直于底面且长为 \(a\). 设球心为棱柱的中心，球半径 \(R\) 满足
\[
R^2=\left(\frac{a}{\sqrt3}\right)^2+\left(\frac a2\right)^2
=\frac{7a^2}{12}
\]
因此球的表面积为
\[
4\pi R^2=4\pi\cdot\frac{7a^2}{12}=\frac73\pi a^2
\]
故选 B.
\end{solution}
...
\begin{solution}
设公共函数值为 \(k\). 由于第一段函数为 \(|\lg x|\)，第二段函数在 \(x>10\) 时为 \(6-\dfrac x2<1\)，要有三个互不相等的原像，必须有 \(0<k<1\). 此时第一段给出两个原像
\(x=10^{-k}\)，\(x=10^k\)
第二段给出一个原像
\[
x=12-2k>10
...
所以
\[
abc=10^{-k}\cdot10^k\cdot(12-2k)=12-2k
\]
当 \(0<k<1\) 时，\(10<12-2k<12\)，且该区间内每个值均可取得. 因此 \(abc\) 的取值范围为 \((10,12)\)，故选 C.
\end{solution}
...
其焦距参数满足 \(c^2=a^2+b^2\). 由焦点为 \(F(3,0)\)，得
\[
a^2+b^2=9
\]
直线 \(l\) 经过 \(F(3,0)\) 和中点 \(N(-12,-15)\)，斜率为 \(1\)，故
\[
l:y=x-3
\]
将其代入双曲线方程并乘以 \(a^2b^2\)，得
\[
(b^2-a^2)x^2+6a^2x-a^2(9+b^2)=0
\]
该方程的两根是点 \(A\)，\(B\) 的横坐标. 因其中点横坐标为 \(-12\)，由韦达定理
\[
\frac{-6a^2}{b^2-a^2}=x_A+x_B=-24
\]
从而 \(5a^2=4b^2\). 结合 \(a^2+b^2=9\)，解得
\(a^2=4\)，\(qquad b^2=5\).
所以双曲线方程为
\[
\frac{x^2}{4}-\frac{y^2}{5}=1
...
\begin{solution}
点 \((x_i,y_i)\) 在单位正方形 \([0,1]\times[0,1]\) 内均匀分布，而满足 \(y_i\leqslant f(x_i)\) 的区域正是曲线 \(y=f(x)\) 下方的区域，其面积为
\[
\int_0^1f(x)\,\mathrm{d}x
\]
单位正方形面积为 \(1\)，故该区域所占比例可用样本比例 \(\dfrac{N_1}{N}\) 估计. 因此积分的近似值为 \(\dfrac{N_1}{N}\).
\end{solution}
...
\begin{solution}
取三棱锥，使其一个侧面正对正投影方向，正视图可为三角形；取三棱柱，使三角形底面正对正投影方向，正视图为三角形；取圆锥，使其轴线位于正投影平面内，正视图为等腰三角形. 因此三棱锥、三棱柱、圆锥均符合题意.
\end{solution}
...
\begin{solution}
切点为 \(B(2,1)\)，切线斜率为 \(1\)，所以圆心在过 \(B\) 且斜率为 \(-1\) 的直线上. 设圆心为
\[
O=(2+s,1-s)
\]
由 \(A(4,1)\)、\(B(2,1)\) 同在圆上，得
\[
(2-s)^2+s^2=2s^2
\]
从而 \(s=1\). 故圆心为 \(O(3,0)\)，半径平方为
\[
OB^2=(2-3)^2+(1-0)^2=2
\]
所以圆 \(C\) 的方程为 \((x-3)^2+y^2=2\).
\end{solution}
...
\begin{solution}
设 \(BD=t\)，则 \(DC=2t\)，且 \(BC=3t\). 因 \(B\)，\(D\)，\(C\) 共线，
\[
\angle ADC=180^\circ-\angle ADB=60^\circ
\]
由三角形 \(ADC\) 的面积条件，
\[
\frac12\cdot AD\cdot DC\cdot\sin60^\circ
=\frac12\cdot2\cdot2t\cdot\frac{\sqrt3}{2}
=t\sqrt3=3-\sqrt3
\]
故 \(t=\sqrt3-1\). 在三角形 \(ADB\) 和 \(ADC\) 中分别使用余弦定理，得
\[
AB^2=2^2+t^2-2\cdot2\cdot t\cos120^\circ=4+t^2+2t=6
\]
\(AC^2=2^2+(2t)^2-2\cdot2\cdot2t\cos60^\circ=4+4t^2-4t\)，\(BC^2=9t^2\).
由 \(t^2=4-2\sqrt3\)，得
\(AC^2=4+4(4-2\sqrt3)-4(\sqrt3-1)=24-12\sqrt3\)，\(BC^2=9(4-2\sqrt3)\).
于是
\[
AB^2+AC^2-BC^2
=6+(24-12\sqrt3)-9(4-2\sqrt3)
=6(\sqrt3-1)=6t
\]
又因为
\[
(AB\cdot AC)^2=AB^2AC^2=6(24-12\sqrt3)=36(4-2\sqrt3)=36t^2
\]
且边长为正，所以 \(AB\cdot AC=6t\)，从而 \(2AB\cdot AC=12t\).
由三角形 \(ABC\) 的余弦定理，
\[
\cos\angle BAC
=\frac{AB^2+AC^2-BC^2}{2AB\cdot AC}
=\frac{6t}{12t}=\frac12
\]
因为 \(0^\circ<\angle BAC<180^\circ\)，所以 \(\angle BAC=60^\circ\).
\end{solution}
...
\]
从而
\(T_n=\frac{4+(3n-1)4^{n+1}}{9}\)，\(S_n=\frac{2+2(3n-1)4^n}{9}\).
所以 \(\{b_n\}\) 的前 \(n\) 项和为
\[
...
\item
在底面所在平面内建立直角坐标系，设
\(A(-u,0,0)\)，\(B(u,0,0)\)，\(C(v,h,0)\)，\(D(-v,h,0)\)
其中 \(u>0\)，\(v>0\)，且 \(ABCD\) 为等腰梯形. 由
\(\overrightarrow{AC}=(u+v,h,0)\)，\(\overrightarrow{BD}=(-(u+v),h,0)\)
...
\]
于是
\(\overrightarrow{PE}=\left(-\frac{u+v}{2},\frac{v-u}{2},-p\right)\)，\(\overrightarrow{BC}=(v-u,u+v,0)\)
从而
\[
...
\(\overrightarrow{DA}=(v-u,-u-v,0)\)，\(\overrightarrow{DB}=(u+v,-u-v,0)\)
故
\[
\cos\angle ADB
...
\(\overrightarrow{PA}=(-u,-u,-p)\)，\(\overrightarrow{PB}=(u,-u,-p)\)
从而
\[
...
\(\frac{4ab^2}{a^2+b^2}=\frac{4a}{3}\)，\(a^2=2b^2\)
所以
\(c^2=a^2-b^2=b^2\)，\(e=\frac ca=\frac{\sqrt{2}}{2}\)

再设弦中点为 \(N\). 由二次方程根和，
\(x_1+x_2=-\frac{2a^2c}{a^2+b^2}\)，\(x_N=\frac{x_1+x_2}{2}=-\frac{a^2c}{a^2+b^2}=-\frac{2c}{3}\)
条件 \(|PA|=|PB|\) 表明 \(P(0,-1)\) 在弦 \(AB\) 的垂直平分线上. 由于 \(AB\) 的斜率为 \(1\)，该垂直平分线为 \(y=-x-1\)；它与 \(l:y=x+c\) 的交点就是 \(N\)，故
\[
x_N=-\frac{c+1}{2}
\]
于是
\(-\frac{2c}{3}=-\frac{c+1}{2}\)，\(c=3\)
...
\(h'(x)=\e^x-1\geqslant0\)，\(h(0)=0\)
所以 \(h(x)\geqslant0\)；从而
\(g'(x)=\e^x-1-x=h(x)\geqslant0\)，\(g(0)=0\)
...
\]
利用二倍角公式，
\(x-\frac14=-\frac14\cos2\alpha\)，\(y=-\frac14\sin2\alpha\)
所以
\[
