...
\bitmapfigure[max width=0.34\linewidth,max totalheight=4.8cm]{img/2010/jiangsu/q4_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=6.0cm]{img/2010/jiangsu/q7_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2010/jiangsu/q16_fig1.png}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2010/jiangsu/q17_fig1.png}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2010/jiangsu/q18_fig1.png}
...
\item 如图，\(AB\) 是 \(\odot O\) 的直径，\(D\) 为 \(\odot O\) 上一点，过点 \(D\) 作 \(\odot O\) 的切线交 \(AB\) 延长线于点 \(C\). 若 \(DA=DC\)，求证：\(AB=2BC\).
\item 在平面直角坐标系 \(xOy\) 中，\(A(0,0)\)，\(B(-2,0)\)，\(C(-2,1)\)，设 \(k\) 为非零实数. 矩阵 \(M=\begin{pmatrix}k&0\\0&1\end{pmatrix}\)，\(N=\begin{pmatrix}0&1\\1&0\end{pmatrix}\)，点 \(A,B,C\) 在矩阵 \(MN\) 对应的变换下得到的点分别为 \(A_1,B_1,C_1\)，\(\triangle A_1B_1C_1\) 的面积是 \(\triangle ABC\) 面积的 \(2\) 倍，求实数 \(k\) 的值.
\item 在极坐标系中，圆 \(\rho=2\cos\theta\) 与直线 \(3\rho\cos\theta+4\rho\sin\theta+a=0\) 相切，求实数 \(a\) 的值.
\item 设 \(a,b\) 是非负实数，求证：\(a^3+b^3\geqslant \sqrt{ab}\left(a^2+b^2\right)\).
\end{enumerate}
\end{problem}
