...
\begin{center}
\bitmapinclude[max width=0.50\linewidth,max totalheight=4.5cm]{img/2010/hubei_science/q1_fig1.png}
\end{center}
...
\begin{center}
\bitmapinclude[max width=0.38\linewidth,max totalheight=4.5cm]{img/2010/hubei_science/q7_fig1.png}
\end{center}
...
\begin{problem}
已知 \(z = 2x - y\)，式中变量 \(x, y\) 满足约束条件 \(\left\{ \begin{array}{l} y \leqslant x, \\ x + y \geqslant 1, \\ x \leqslant 2, \end{array} \right.\) 则 \(z\) 的最大值为\fillinblank{}.
\end{problem}

\begin{problem}
圆柱形容器内部盛有高度为 \(8 \mathrm{~cm}\) 的水，若放入三个相同的球 (球半径与圆柱的底面半径相同) 后，水恰好淹没最上面的球 (如图所示). 则球的半径是\fillinblank{} cm.
\begin{center}
\bitmapinclude[max width=0.24\linewidth,max totalheight=4.0cm]{img/2010/hubei_science/q13_fig1.png}
\end{center}
\end{problem}

\begin{problem}
\(\begin{array}{c|cccc} \xi & 7 & 8 & 9 & 10 \\ \hline P & x & 0.1 & 0.3 & y \end{array}\)
...
\begin{center}
\bitmapinclude[max width=0.42\linewidth,max totalheight=4.2cm]{img/2010/hubei_science/q15_fig1.png}
\end{center}
\end{problem}
...
\begin{problem}
为了在夏季降温和冬季供暖时减少能源损耗，房屋的屋顶和外墙需要建造隔热层. 某幢建筑物要建造可使用20年的隔热层，每厘米厚的隔热层建造成本为6万元. 该建筑物每年的能源消耗费用 C (单位：万元) 与隔热层厚度 x (单位：cm) 满足关系: \( C(x) = \frac{k}{3x + 5} \ (0 \leqslant x \leqslant 10) \)，若不建隔热层，每年能源消耗费用为8万元. 设 \( f(x) \) 为隔热层建造费用与20年的能源消耗费用之和.
...
\begin{center}
\bitmapinclude[max width=0.38\linewidth,max totalheight=4.8cm]{img/2010/hubei_science/q18_fig1.png}
\end{center}
\end{problem}
