...
\bitmapfigure[max width=0.9\linewidth,max totalheight=6.0cm]{img/2010/shandong_science/q11_options.png}
\end{problem}
...
\bitmapfigure[max width=0.36\linewidth,max totalheight=6.0cm]{img/2010/shandong_science/q13_fig1.png}
\end{problem}
...
\begin{problem}
已知函数 \( f(x) = \frac{1}{2} \sin 2x \sin \varphi + \cos^2 x \cos \varphi - \frac{1}{2} \sin \left( \frac{\pi}{2} + \varphi \right) (0 < \varphi < \pi) \)，其图象过点 \( \left( \frac{\pi}{6}, \frac{1}{2} \right) \). (1) 求 \(\varphi\) 的值; (2) 将函数 \( y = f(x) \) 的图象上各点的横坐标缩短到原来的 \( \frac{1}{2} \)，纵坐标不变，得到函数 \( y = g(x) \) 的图象. 求函数 \( g(x) \) 在 \( \left[0, \frac{\pi}{4}\right] \) 上的最大值和最小值 \fillinblank{}.
\end{problem}

\begin{problem}
已知等差数列 \(\{a_{n}\}\) 满足：\(a_{3} = 7, a_{5} + a_{7} = 26\). 数列 \(\{a_{n}\}\) 的前 \(n\) 项和为 \(S_{n}\). (1) 求 \(a_{n}\) 及 \(S_{n}\); (2) 令 \( b_{n} = \frac{1}{a_{n}^{2} - 1} (n \in \N^*) \)，求数列 \(\{b_{n}\}\) 的前 \(n\) 项和 \(T_{n}\) \fillinblank{}.
\end{problem}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=4.8cm]{img/2010/shandong_science/q19_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2010/shandong_science/q21_fig1.png}
\end{problem}
