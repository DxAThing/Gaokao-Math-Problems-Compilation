...
\begin{center}\bitmapfigure[max width=6.0cm,max totalheight=4.8cm]{img/2023/national_paper_B_liberal/q3_fig1.png}\end{center}
\choices{\(24\)}{\(26\)}{\(28\)}{\(30\)}
\end{problem}
...
\begin{problem}
若 \(x,y\) 满足约束条件 \(\begin{cases}x-3y\le-1,\\x+2y\le9,\\3x+y\ge7,\end{cases}\) 则 \(z=2x-y\) 的最大值为\fillinblank{}.
\end{problem}
...
（2）对 \(x>0\)，\(f'(x)=-\dfrac{\ln(1+x)}{x^2}+\dfrac{1/x+a}{1+x}\). 因此 \(f'(x)\ge0\) 等价于 \(a\ge h(x)=\dfrac{(1+x)\ln(1+x)-x}{x^2}\). 由求导得 \(h'(x)<0\)，且 \(\lim_{x\to0^+}h(x)=\frac12\)，\(\lim_{x\to+\infty}h(x)=0\). 故 \(h(x)<\frac12\)，所求范围为 \(a\ge\frac12\).
\end{solution}
...
\begin{problem}
在直角坐标系 \(xOy\) 中，以坐标原点 \(O\) 为极点，\(x\) 轴正半轴为极轴建立极坐标系，曲线 \(C_1\) 的极坐标方程为 \(\rho=2\sin\theta\left(\frac\pi4\le\theta\le\frac\pi2\right)\)，曲线 \(C_2:\begin{cases}x=2\cos\alpha,\\y=2\sin\alpha\end{cases}\)（\(\alpha\) 为参数，\(\frac\pi2<\alpha<\pi\)）.
\begin{enumerate}
\item 写出 \(C_1\) 的直角坐标方程；
...
\begin{solution}（1）\(f(x)=\begin{cases}3x-2,&x>2,\\x+2,&0\le x\le2,\\-3x+2,&x<0.\end{cases}\) 分段解不等式得 \(-2\le x\le2\)，故解集为 \([-2,2]\).
