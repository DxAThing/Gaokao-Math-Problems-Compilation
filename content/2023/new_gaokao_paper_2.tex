...
\solutionfigure{\bitmapfigure[max width=6.2cm]{img/2023/new_gaokao_paper_2/q5_solution_fig1.png}}
...
\solutionfigure{\bitmapfigure[max width=6.2cm]{img/2023/new_gaokao_paper_2/q9_solution_fig1.png}}
\end{solution}
...
\solutionfigure{\bitmapfigure[max width=5.4cm]{img/2023/new_gaokao_paper_2/q10_solution_fig1.png}}
\end{solution}
...
\solutionfigure{\bitmapfigure[max width=5.0cm]{img/2023/new_gaokao_paper_2/q14_solution_fig1.png}}
\end{solution}
...
\solutionfigure{\bitmapfigure[max width=5.0cm]{img/2023/new_gaokao_paper_2/q15_solution_fig1.png}}
\end{solution}
...
\solutionfigure{\bitmapfigure[max width=7.0cm]{img/2023/new_gaokao_paper_2/q16_solution_fig1.png}}
\end{solution}
\section{解答题}
...
\begin{solution}
\begin{enumerate}
\item 因为 \(\angle ADC=\frac{\pi}{3}\)，所以 \(\angle ADB=\frac{2\pi}{3}\).
...
\item 由面积公式，\(\frac12bc\sin A=\sqrt3\)，即 \(bc\sin A=2\sqrt3\)（1）.
...
\solutionfigure{\bitmapfigure[max width=4.4cm]{img/2023/new_gaokao_paper_2/q17_solution_fig1.png}}
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 设等差数列首项为 \(a_1\)，公差为 \(d\). 由 \(S_4=4a_1+6d=32\)（1）且
			\[
			T_3=(a_1-6)+2(a_1+d)+(a_1+2d-6)=4a_1+4d-12=16
			\]
			联立以上两式得 \(a_1=5\)，\(d=2\)，故 \(a_n=a_1+(n-1)d=2n+3\).

\item 由（1）得 \(S_n=\frac{n(a_1+a_n)}{2}=\frac{n(5+2n+3)}{2}=n^2+4n\).
...
\begin{center}
\texfigure[max width=8.0cm]{img_repaint/2023/new_gaokao_paper_2/q19_fig1.tex}
\end{center}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 由患病者的频率分布图可知，\([95,100)\) 这一组的频率为 \(5\times 0.002=0.01>0.005\). 所以 \(c\in[95,100)\)，且 \((c-95)\times 0.002=0.005\).
解得 \(c=97.5\).
此时未患病者中指标大于 \(97.5\) 的概率为 \((100-97.5)\times 0.01+5\times 0.002=0.035\)，故 \(q(c)=3.5\%\).

\item 当 \(95\le c<100\) 时，\(p(c)=(c-95)\times 0.002\)，\(q(c)=(100-c)\times 0.01+5\times 0.002\)，所以 \(f(c)=p(c)+q(c)=-0.008c+0.82\).
...
\begin{solution}
\begin{enumerate}
\item 因为 \(DA=DB=DC\) 且 \(\angle ADB=\angle ADC=60^\circ\)，所以 \(\triangle ADB\) 与 \(\triangle ADC\) 都是正三角形，故 \(AB=AC\).
...
\item 不妨设 \(DA=DB=DC=2\). 则 \(AB=AC=2\)，\(BC=\sqrt{DB^2+DC^2}=2\sqrt2\).
...
\solutionfigure{\bitmapfigure[max width=5.0cm]{img/2023/new_gaokao_paper_2/q20_solution_fig1.png}}
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 设双曲线方程为 \(\frac{x^2}{a^2}-\frac{y^2}{b^2}=1\).
由左焦点知 \(c=2\sqrt5\). 又离心率 \(e=\frac{c}{a}=\sqrt5\)，故 \(a=2\). 于是 \(b=\sqrt{c^2-a^2}=\sqrt{20-4}=4\). 所以 \(C:\frac{x^2}{4}-\frac{y^2}{16}=1\).

\item 设 \(A_1(-2,0)\)，\(A_2(2,0)\). 设直线 \(MN\) 的方程为 \(x=my-4\)，由它与双曲线左支有两个交点知 \(-\frac12<m<\frac12\). 与双曲线方程联立得 \((4m^2-1)y^2-32my+48=0\). 设 \(M(x_1,y_1)\)，\(N(x_2,y_2)\)，则 \(y_1+y_2=\frac{32m}{4m^2-1}\)，\(y_1y_2=\frac{48}{4m^2-1}\).
...
\solutionfigure{\bitmapfigure[max width=5.0cm]{img/2023/new_gaokao_paper_2/q21_solution_fig1.png}}
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 令 \(h(x)=x-x^2-\sin x\). 则 \(h(0)=0\)，\(h'(x)=1-2x-\cos x\)，\(h'(0)=0\)，\(h''(x)=-2+\sin x<0\)（\(0<x<1\)）.
...
\item 函数 \(f(x)=\cos ax-\ln(1-x^2)\) 为偶函数，因此 \(f'(0)=0\). 又 \(f''(x)=-a^2\cos ax+\frac{2+2x^2}{(1-x^2)^2}\)，故 \(f''(0)=2-a^2\).
