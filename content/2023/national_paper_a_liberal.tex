...
\texfigure[max width=0.56\linewidth]{img_repaint/2023/national_paper_a_liberal/q6_fig1.tex}
...
\begin{enumerate}
\item \(bc=1\).
\item \(\triangle ABC\) 的面积为 \(\dfrac{\sqrt{3}}{4}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 平面 \(ACC_1A_1\perp\) 平面 \(BCC_1B_1\).
\item 四棱锥 \(A_1-BB_1C_1C\) 的高为 \(1\).
\end{enumerate}
\end{solution}
...
\end{tabular}
\end{center}
\item 能有 \(95\%\) 的把握认为小白鼠在高浓度臭氧环境中与在正常环境中体重的增加量有差异.
\end{enumerate}
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 当 \(a=1\) 时，\(f(x)\) 在 \(\left(0,\dfrac{\pi}{2}\right)\) 上单调递减.
\item \(a\le0\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
\item \(p=2\).
\item \(\triangle MFN\) 面积的最小值为 \(12-8\sqrt{2}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(\alpha=\dfrac{3\pi}{4}\).
\item \(l\) 的极坐标方程为 \(\rho\cos\theta+\rho\sin\theta-3=0\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 不等式 \(f(x)<x\) 的解集为 \(\left(\dfrac{a}{3},3a\right)\).
\item \(a=\dfrac{2\sqrt{6}}{3}\).
\end{enumerate}
\end{solution}
