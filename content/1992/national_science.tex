...
\bitmapfigure[max width=0.28\linewidth,max totalheight=3.8cm]{img/1992/national_science/q6_fig1.png}
...
\begin{problem}
直线 \(\left\{ \begin{array}{l} x=t\cdot \sin 20^{\circ}+3, \\ y=-t\cdot \cos 20^{\circ} \end{array} \right.\)（\(t\) 为参数）的倾斜角是（\quad）
...
\bitmapfigure[max width=0.30\linewidth,max totalheight=4.4cm]{img/1992/national_science/q14_fig1.png}
...
\begin{problem}
设等差数列 \(\{a_n\}\) 的前 \(n\) 项和为 \(S_n\). 已知 \(a_3=12\)，\(S_{12}>0\)，\(S_{13}<0\). （1）求公差 \(d\) 的取值范围；（2）指出 \(S_1\)、\(S_2\)、\(\cdots\)、\(S_{12}\) 中哪一个值最大，并说明理由.
