\chapter{1992年全国卷（文）}
\section{单选题}

\begin{problem}
\(\frac{\log_8 9}{\log_2 3}\) 的值是（\quad）

\choices
    {\(\frac{2}{3} \)}
    {1}
    {\(\frac{3}{2} \)}
    {2}

\end{problem}

\begin{problem}
已知椭圆 \(\frac{x^2}{25} + \frac{y^2}{16} = 1\) 上一点 \(P\) 到椭圆一个焦点的距离为 3，则 \(P\) 到另一焦点的距离为（\quad）

\choices
    {2}
    {3}
    {5}
    {7}

\end{problem}

\begin{problem}
如果函数 \(y=\sin(\omega x)\cos(\omega x)\) 的最小正周期是 \(4\pi\)，那么常数 \(\omega\) 为（\quad）

\choices
    {4}
    {2}
    {\(\frac{1}{2} \)}
    {\(\frac{1}{4} \)}

\end{problem}

\begin{problem}
在 \(\left(\frac{x}{2}-\frac{1}{\sqrt[3]{x}}\right)^8\) 的展开式中常数项是（\quad）

\choices
    {\(-28\)}
    {\(-7\)}
    {7}
    {28}

\end{problem}

\begin{problem}
已知轴截面是正方形的圆柱的高与球的直径相等，则圆柱的全面积与球的表面积的比是（\quad）

\choices
    {6:5}
    {5:4}
    {4:3}
    {3:2}

\end{problem}

\begin{problem}
如图，图中曲线是幂函数 \(y=x^n\) 在第一象限的图象. 已知 \(n\) 取 \(\pm 2,\pm \frac{1}{2}\) 四个值，则相应于曲线 \(C_{1}\)、\(C_{2}\)、\(C_{3}\)、\(C_{4}\) 的 \(n\) 依次为（\quad）

\bitmapfigure[max width=0.28\linewidth,max totalheight=3.8cm]{img/1992/national_liberal/q6_fig1.png}

\choices
    {\(-2,-\frac{1}{2},\frac{1}{2},2\)}
    {\(2,\frac{1}{2},-\frac{1}{2},-2\)}
    {\(-\frac{1}{2},-2,2,\frac{1}{2}\)}
    {\(2,\frac{1}{2},-2,-\frac{1}{2}\)}

\end{problem}

\begin{problem}
若 \(\log_a 2 < \log_b 2 < 0\)，则（\quad）

\choices
    {\(0 < a < b < 1\)}
    {\(0 < b < a < 1\)}
    {\(a > b > 1\)}
    {\(b > a > 1\)}

\end{problem}

\begin{problem}
原点关于直线 \(8x + 6y = 25\) 的对称点坐标为（\quad）

\choices
    {\(\left(2,\frac{3}{2}\right)\)}
    {\(\left(\frac{25}{8}, \frac{25}{6}\right)\)}
    {\(\left(3,4\right)\)}
    {\(\left(4,3\right)\)}

\end{problem}

\begin{problem}
在四棱锥的四个侧面中，直角三角形最多可有（\quad）

\choices
    {1 个}
    {2 个}
    {3 个}
    {4 个}

\end{problem}

\begin{problem}
圆心在抛物线 \(y^{2} = 2x \) 上，且与 \(x \) 轴和该抛物线的准线都相切的一个圆的方程是（\quad）

\choices
    {\(x^{2} + y^{2} - x - 2y - \frac{1}{4} = 0\)}
    {\(x^{2} + y^{2} + x - 2y + 1 = 0\)}
    {\(x^{2} + y^{2} - x - 2y + 1 = 0\)}
    {\(x^{2} + y^{2} - x - 2y + \frac{1}{4} = 0\)}

\end{problem}

\begin{problem}
在 \([0,2\pi]\) 上满足 \(\sin x \geqslant \frac{1}{2}\) 的 \(x\) 的取值范围是（\quad）

\choices
    {\(\left[0, \frac{\pi}{6}\right]\)}
    {\(\left[\frac{\pi}{6}, \frac{5\pi}{6}\right]\)}
    {\(\left[\frac{\pi}{6}, \frac{2\pi}{3}\right]\)}
    {\(\left[\frac{5\pi}{6}, \pi\right]\)}

\end{problem}

\begin{problem}
已知直线 \(l_{1}\) 和 \(l_{2}\) 夹角的平分线为 \(y = x\)，如果 \(l_{1}\) 的方程是 \(ax + by + c = 0\)（\(ab > 0\)），那么 \(l_{2}\) 的方程是（\quad）

\choices
    {\(bx + ay + c = 0\)}
    {\(ax - by + c = 0\)}
    {\(bx + ay - c = 0\)}
    {\(bx - ay + c = 0\)}

\end{problem}

\begin{problem}
如果 \(\alpha,\beta \in \left(\frac{\pi}{2}, \pi\right)\) 且 \(\tan \alpha < \cot \beta\)，那么必有（\quad）

\choices
    {\(\alpha < \beta\)}
    {\(\beta < \alpha\)}
    {\(\alpha + \beta < \frac{3}{2}\pi\)}
    {\(\alpha + \beta > \frac{3}{2}\pi\)}

\end{problem}

\begin{problem}
如图，在棱长为 \(1\) 的正方体 \(ABCD-A_{1}B_{1}C_{1}D_{1}\) 中，\(M\) 和 \(N\) 分别为 \(A_{1}B_{1}\) 和 \(BB_{1}\) 的中点，那么直线 \(AM\) 与 \(CN\) 所成角的余弦值是（\quad）

\bitmapfigure[max width=0.30\linewidth,max totalheight=4.4cm]{img/1992/national_liberal/q14_fig1.png}

\choices
    {\(\frac{\sqrt{3}}{2}\)}
    {\(\frac{\sqrt{10}}{10}\)}
    {\(\frac{3}{5}\)}
    {\(\frac{2}{5}\)}

\end{problem}

\begin{problem}
已知复数 \(z\) 的模为 2，则 \(|z - \i|\) 的最大值为（\quad）

\choices
    {1}
    {2}
    {\(\sqrt{5}\)}
    {3}

\end{problem}

\begin{problem}
函数 \(y=\frac{\e^x-\e^{-x}}{2}\) 的反函数（\quad）

\choices
    {是奇函数，它在 \((0, +\infty)\) 上是减函数}
    {是偶函数，它在 \((0, +\infty)\) 上是减函数}
    {是奇函数，它在 \((0, +\infty)\) 上是增函数}
    {是偶函数，它在 \((0, +\infty)\) 上是增函数}

\end{problem}

\begin{problem}
如果函数 \(f(x)=x^2+bx+c\) 对任意实数 \(t\) 都有 \(f(2+t)=f(2-t)\)，那么（\quad）

\choices
    {\(f(2) < f(1) < f(4)\)}
    {\(f(1) < f(2) < f(4)\)}
    {\(f(2) < f(4) < f(1)\)}
    {\(f(4) < f(2) < f(1)\)}

\end{problem}

\begin{problem}
已知长方体的全面积为 11, 12 条棱长度之和为 24，则这个长方体的一条对角线长为（\quad）

\choices
    {\(2\sqrt{3}\)}
    {\(\sqrt{14}\)}
    {5}
    {6}

\end{problem}

\section{填空题}

\begin{problem}
\(\displaystyle\lim_{n\to \infty}\left[\frac{1}{3}-\frac{1}{9}+\frac{1}{27}+\cdots+(-1)^{n-1}\frac{1}{3^n}\right]\) 的值为 \fillinblank{}.

\end{problem}

\begin{problem}
已知 \(\alpha\) 在第三象限且 \(\tan \alpha = 2\)，则 \(\cos \alpha\) 的值是 \fillinblank{}.

\end{problem}

\begin{problem}
方程 \(\frac{1 + 3^{-x}}{1 + 3^x} = 3\) 的解是 \fillinblank{}.

\end{problem}

\begin{problem}
设含有 10 个元素的集合的全部子集数为 \(S\)，其中由 3 个元素组成的子集数为 \(T\)，则 \(\frac{T}{S}\) 的值为 \fillinblank{}.

\end{problem}

\begin{problem}
焦点为 \(F_{1}(-2,0)\) 和 \(F_{2}(6,0)\)，离心率为 2 的双曲线的方程是 \fillinblank{}.

\end{problem}

\section{解答题}

\begin{problem}
求 \(\sin^2 20^\circ + \cos^2 80^\circ + \sqrt{3} \sin 20^\circ \cos 80^\circ\) 的值.

\end{problem}

\begin{problem}
已知 \(z \in \mathbf{C}\)，解方程: \(z - 2|z| = -7 + 4\i\).

\end{problem}

\begin{problem}
如图，已知 \(ABCD-A_{1}B_{1}C_{1}D_{1}\) 是棱长为 \(a\) 的正方体，\(E,F\) 分别为棱 \(AA_{1}\) 与 \(CC_{1}\) 的中点，求四棱锥 \(A_{1}-EBFD_{1}\) 的体积.

\bitmapfigure[max width=0.28\linewidth,max totalheight=4.4cm]{img/1992/national_liberal/q26_fig1.png}

\end{problem}

\begin{problem}
在 \(\triangle ABC\) 中，\(BC\) 边上的高所在直线的方程为 \(2-2y+1=0\)，\(\angle A\) 的平分线所在直线的方程为 \(y=0\)，若点 \(B\) 的坐标为 \(\left(1,2\right)\)，求点 \(A\) 和点 \(C\) 的坐标.

\end{problem}

\begin{problem}
设等差数列 \(\{a_{n}\}\) 的前 \(n\) 项和为 \(S_{n}\). 已知 \(a_{3}=12\)，\(S_{12}>0\)，\(S_{13}<0\). （1）求公差 \(d\) 的取值范围；（2）指出 \(S_{1},S_{2},\cdots,S_{12}\) 中哪一个值最大，并说明理由.
\end{problem}

