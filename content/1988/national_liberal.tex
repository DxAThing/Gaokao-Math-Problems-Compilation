...
\begin{solution}
由和角公式和二倍角公式，\(\cos 3\alpha=\cos(2\alpha+\alpha)=\cos 2\alpha\cos\alpha-\sin 2\alpha\sin\alpha\)，又因为 \(\cos 2\alpha=2\cos^2\alpha-1\)，\(\sin 2\alpha=2\sin\alpha\cos\alpha\)，所以 \(\cos 3\alpha=(2\cos^2\alpha-1)\cos\alpha-2\sin^2\alpha\cos\alpha\)，再由 \(\sin^2\alpha=1-\cos^2\alpha\)，得
\[
\cos 3\alpha=(2\cos^2\alpha-1)\cos\alpha-2(1-\cos^2\alpha)\cos\alpha=4\cos^3\alpha-3\cos\alpha.
\]
故等式成立.
\end{solution}
...
\bitmapinlinefigure[5.0cm][max width=5.0cm]{img/1988/national_liberal/q22_fig1.png}
\end{problem}

\begin{solution}
设正方形底面所在平面为 \(z=0\)，取 \(B\) 为原点，令 \(BA\)、\(BC\)、\(BS\) 分别为 \(x\)、\(y\)、\(z\) 轴的正向，则
\(B=(0,0,0)\)，\(A=(1,0,0)\)，\(C=(0,1,0)\)，\(D=(1,1,0)\)，\(S=(0,0,\sqrt{3})\).
因此 \(\overrightarrow{SA}=(1,0,-\sqrt{3})\)，\(\overrightarrow{SD}=(1,1,-\sqrt{3})\)，从而
\(\overrightarrow{SA}\cdot\overrightarrow{SD}=1+3=4\)，\(|\overrightarrow{SA}|=2\)，\(|\overrightarrow{SD}|=\sqrt{5}\).
由向量夹角公式，
\[
\cos\alpha=\frac{\overrightarrow{SA}\cdot\overrightarrow{SD}}{|\overrightarrow{SA}|\,|\overrightarrow{SD}|}=\frac{4}{2\sqrt{5}}=\frac{2\sqrt{5}}{5}.
\]
于是
\[
\sin\alpha=\sqrt{1-\cos^2\alpha}=\sqrt{1-\frac45}=\frac{\sqrt{5}}5.
\]
\end{solution}
...
\begin{solution}
由点到直线的距离公式，\(\frac{|a-b|}{\sqrt{2}}=\sqrt{2}\)，
所以 \(|a-b|=2\)，即分两种情况讨论.

当 \(a-b=2\) 时，由 \(a^2-b^2=(a-b)(a+b)=1\) 得 \(a+b=\frac12\)，于是
\(a=\frac{(a-b)+(a+b)}2=\frac{2+\frac12}2=\frac54\)，
\(b=\frac{(a+b)-(a-b)}2=\frac{\frac12-2}2=-\frac34\).
此时 \(a=\frac54>0\)，点在双曲线的右支上.

当 \(a-b=-2\) 时，由 \((a-b)(a+b)=1\) 得 \(a+b=-\frac12\)，于是 \(a=-\frac54<0\)，点在左支上，不合题意.

所以所求点为 \(P\left(\frac54,-\frac34\right)\).
\end{solution}
...
\begin{solution}
因为对数函数以 \(10\) 为底且单调递增，所以 \(\lg\left(x-\frac1x\right)<0\Longleftrightarrow 0<x-\frac1x<1\).
由 \(x-\frac1x>0\) 得 \(\frac{(x-1)(x+1)}x>0\)，
所以定义域为 \((-1,0)\cup(1,+\infty)\).

在此定义域内，继续解 \(x-\frac1x<1\)：当 \(-1<x<0\) 时，乘以负数 \(x\) 后不等号改变方向，得 \(x^2-x-1>0\)；当 \(x>1\) 时，乘以正数 \(x\) 后不等号方向不变，得 \(x^2-x-1<0\). 方程 \(x^2-x-1=0\) 的两个根为 \(\frac{1-\sqrt{5}}2\) 和 \(\frac{1+\sqrt{5}}2\)，故
\(x\in\left(-1,\frac{1-\sqrt{5}}2\right)\cup\left(1,\frac{1+\sqrt{5}}2\right)\).
\end{solution}
...
\begin{solution}
前 \(2m\) 项中奇数项为 \(a_{2k-1}\)，偶数项为 \(a_{2k}\)，其中 \(k=1,2,\ldots,m\). 因而
\(a_{2k-1}=5(2k-1)+1=10k-4\)，\(a_{2k}=2^{\frac{2k}{2}}=2^k\).
所以前 \(2m\) 项的和为
\(\sum_{n=1}^{2m}a_n=\sum_{k=1}^m(10k-4)+\sum_{k=1}^m2^k\).
再由等差数列和与等比数列和公式，
\(\sum_{n=1}^{2m}a_n=10\cdot\frac{m(m+1)}2-4m+(2^{m+1}-2)\)，化简得 \(\sum_{n=1}^{2m}a_n=5m^2+m+2^{m+1}-2\).
\end{solution}
