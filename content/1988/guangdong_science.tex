...
\begin{problem}
如图，在四棱锥 \(P\text{-}ABCD\) 中，侧面 \(PCD\) 是边长为 \(2\mathrm{cm}\) 的等边三角形，且与底面垂直，而底面 \(ABCD\) 是面积为 \(2\sqrt{3}\mathrm{cm}^2\) 的菱形，\(\angle ADC\) 是锐角.
...
\begin{solution}
（1）过 \(P\) 作 \(PE \perp CD\)，垂足是 \(E\). 因为 \(CD = \text{侧面 }PCD \cap \text{底面 }ACD\)，侧面 \(PCD \perp\) 底面 \(ACD\)，且 \(PE \subset\) 侧面 \(PCD\)，所以 \(PE \perp\) 底面 \(ACD\). 又因为 \(\triangle PCD\) 是边长为 \(2\mathrm{cm}\) 的等边三角形，且 \(PE \perp CD\)，所以 \(PE = \frac{\sqrt{3}}{2} \times 2 = \sqrt{3}\mathrm{cm}\). 依设 \(S_{\text{底面}} = 2\sqrt{3}\mathrm{cm}^2\). 因此四棱锥的体积 \(V = \frac{1}{3}PE \cdot S_{\text{底面}} = \frac{1}{3} \times \sqrt{3} \times 2\sqrt{3} = 2\mathrm{cm}^3\).
