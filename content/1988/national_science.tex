...
\begin{solution}
当 \(t=1\) 时两者相等；当 \(t\neq 1\) 时，若 \(0<a<1\)，则 \(\frac{1}{2}\log_a t>\log_a\frac{t+1}{2}\)；若 \(a>1\)，则 \(\frac{1}{2}\log_a t<\log_a\frac{t+1}{2}\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 经过图象上任意两个不同的点的直线不平行于 \(x\) 轴.
\item 图象关于直线 \(y=x\) 成轴对称图形.
\end{enumerate}
\end{solution}
