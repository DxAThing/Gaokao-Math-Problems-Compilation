...
\choices
    {\bitmapinclude[max width=2.7cm,max totalheight=2.4cm]{img/2018/national_paper_2_science/q3_fig1.png}}
    {\bitmapinclude[max width=2.7cm,max totalheight=2.4cm]{img/2018/national_paper_2_science/q3_fig2.png}}
    {\bitmapinclude[max width=2.7cm,max totalheight=2.4cm]{img/2018/national_paper_2_science/q3_fig3.png}}
    {\bitmapinclude[max width=2.7cm,max totalheight=2.4cm]{img/2018/national_paper_2_science/q3_fig4.png}}
...
\begin{center}
\bitmapinclude[max width=0.38\linewidth,max totalheight=4.8cm]{img/2018/national_paper_2_science/q7_fig1.png}
\end{center}
...
\end{solution}
\begin{problem}
在长方体 \(ABCD-A_1B_1C_1D_1\) 中，\(AB=BC=1\)，\(AA_1=3\)，则异面直线 \(AD_1\) 与 \(DB_1\) 所成角的余弦值为（\quad）
...
\begin{solution}
以 \(D\) 为原点，\(DA,DC,DD_1\) 分别为 \(x,y,z\) 轴建立空间直角坐标系，则
\(A(1,0,0)\)，\(D_1(0,0,3)\)，\(B_1(1,1,3)\).
于是
\(\overrightarrow{AD_1}=(-1,0,3)\)，\(\overrightarrow{DB_1}=(1,1,3)\).
所以
\(\overrightarrow{AD_1}\cdot\overrightarrow{DB_1}=8\)，
\(|\overrightarrow{AD_1}|=\sqrt{10}\)，\(|\overrightarrow{DB_1}|=\sqrt{11}\).
因此
\(\cos\theta=\frac8{\sqrt{110}}=\frac{\sqrt5}{5}\).
故选 \(C\).
\end{solution}
\begin{problem}
...
\begin{center}
\bitmapinclude[max width=0.42\linewidth,max totalheight=4.8cm]{img/2018/national_paper_2_science/q14_fig1.png}
\end{center}
...
\begin{center}
\bitmapinclude[max width=0.84\linewidth,max totalheight=4.8cm]{img/2018/national_paper_2_science/q18_fig1.png}
\end{center}
...
方法二：抛物线 \(C:y^2=4x\) 的焦点为 \(F(1,0)\)，设直线 \(AB\) 的倾斜角为 \(\theta\). 由抛物线的焦点弦公式，
...
\bitmapfigure[max width=0.80\linewidth,max totalheight=4.8cm]{img/2018/national_paper_2_science/q19_solution_fig1.png}
\end{solution}
\begin{problem}
...
\begin{center}
\bitmapinclude[max width=0.38\linewidth,max totalheight=4.8cm]{img/2018/national_paper_2_science/q20_fig1.png}
\end{center}
...
\bitmapfigure[max width=0.38\linewidth,max totalheight=4.8cm]{img/2018/national_paper_2_science/q20_solution_fig1.png}
\end{solution}
\begin{problem}
已知函数 \(f(x)=e^x-ax^2\).
...
（2）方法一：\(f(x)\) 在 \((0,+\infty)\) 只有一个零点，等价于方程
\(e^x-ax^2=0\)
...
方法二：\circled{1} 当 \(a\le 0\) 时，
\(f(x)=e^x-ax^2>0\)，
所以 \(f(x)\) 在 \((0,+\infty)\) 没有零点.

\circled{2} 当 \(a>0\) 时，设函数
\(h(x)=1-ax^2e^{-x}\).
...
\circled{1} 当直线的斜率不存在时，\(x=1\). 此时无解，故舍去.

\circled{2} 当直线的斜率存在时，因为 \(t_1\) 和 \(t_2\) 为 \(A,B\) 对应的参数，所以利用中点坐标公式可知
\(\frac{t_1+t_2}{2}=0\).
因此
