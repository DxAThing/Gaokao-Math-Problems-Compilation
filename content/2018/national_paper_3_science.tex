...
\begin{solution}
从俯视方向看，榫头在大木构件右侧中部的投影是一个矩形；要使带卯眼的木构件与它咬合后成为长方体，卯眼必须是从该木构件左侧边缘向内凹入的同样大小的矩形槽，并且咬合后的外轮廓仍应为矩形. 第二、第四项的凹槽轮廓与榫头投影不相同，第三项的凹槽位置或深度不满足完全咬合，只有第一项同时满足凹槽与榫头投影吻合、外轮廓为矩形两个条件，故选 A.
\end{solution}
...
\[
f(0)=2,\qquad f\left(\pm\dfrac1{\sqrt2}\right)=\dfrac94.
\]
...
\[
\dfrac{\sqrt3}{4}s^2=9\sqrt3,\qquad s=6.
\]
...
\[
t=-\ln0.3>0,\qquad u=\ln5,\qquad v=\ln2.
\]
则 \(a=\dfrac tu\)，\(b=-\dfrac tv\). 由于 \(u>v\)，有
\[
a+b=t\left(\dfrac1u-\dfrac1v\right)<0,\qquad ab<0.
\]
此外，\(ab<a+b\) 等价于
\[
...
\[
3x+\dfrac\pi6=\dfrac\pi2+k\pi,\qquad k\in\Z,
\]
从而
\[
...
\[
\dfrac\pi9,\qquad \dfrac{4\pi}{9},\qquad \dfrac{7\pi}{9}.
\]
所以零点个数为 \(3\).
\end{solution}
...
\[
A=(t_1^2,2t_1),\qquad B=(t_2^2,2t_2).
\]
...
因 \(M=(-1,1)\)，所以
\[
\overrightarrow{MA}=(t_1^2+1,2t_1-1),\qquad
\overrightarrow{MB}=(t_2^2+1,2t_2-1).
\]
...
q^4=4q^2.
\]
由 \(q^2(q^2-4)=0\)，得 \(q=0\)，\(q=2\) 或 \(q=-2\). 当 \(q=0\) 时，
\[
a_1=1,\qquad a_n=0\quad(n\ge2).
\]
当 \(q=2\) 或 \(q=-2\) 时，通项公式分别为
\(a_n=2^{n-1}\) 或 \(a_n=(-2)^{n-1}\).
\item 当 \(q=0\) 时，\(S_m=1\)，不满足 \(S_m=63\). 当 \(q=2\) 时，
\[
...
\[
A(0,0,0),\quad B(a,0,0),\quad C(a,b,0),\quad D(0,b,0).
\]
...
\[
\bs n_1=\overrightarrow{AD}\times\overrightarrow{DM}=(bv,0,-bu),
\qquad
\bs n_2=\overrightarrow{BC}\times\overrightarrow{CM}=(bv,0,b(a-u)).
\]
于是
\[
...
\[
\overrightarrow{AB}=(a,0,0),\qquad \overrightarrow{AM}=\left(\dfrac a2,b,\dfrac a2\right),
\]
故其法向量可取
\[
...
\]
该值随矩形的边长比改变. 例如 \(a=b\) 时为 \(\dfrac2{\sqrt5}\)，而 \(a=2b\) 时为 \(\dfrac1{\sqrt2}\). 题目只给出“矩形”而未给出 \(a\)、\(b\) 或其比值，因此第（2）问不能确定唯一数值.
\end{enumerate}
\end{solution}
...
\[
-8k(m-k)=2(3+4k^2),\qquad km=-\dfrac34.
\]
又因为 \(M\) 是椭圆内弦的中点，故
\[
...
\[
|FA|=\dfrac32-\dfrac u2,\qquad |FP|=\dfrac32,\qquad |FB|=\dfrac32+\dfrac u2,
\]
...
\[
x=\sqrt2\sin\alpha\cos\alpha,\qquad y=-\sqrt2\cos^2\alpha.
\]
这已经给出了轨迹的参数方程，参数为 \(\alpha\in\left(\dfrac\pi4,\dfrac{3\pi}4\right)\). 若令 \(m=\cot\alpha\)，则 \(-1<m<1\)，并且
\[
x=\dfrac{\sqrt2m}{1+m^2},\qquad y=-\dfrac{\sqrt2m^2}{1+m^2},\qquad -1<m<1.
\]
\end{enumerate}
\end{solution}
