...
\begin{solution}
由三视图可知，\(M\)，\(N\) 在圆柱侧面上的最短弧长为底面圆周的四分之一，即 \(16/4=4\). 圆柱的高为 \(2\). 将圆柱侧面沿经过 \(M\)，\(N\) 的最短弧展开，得到一个矩形，矩形中 \(M\)，\(N\) 的两点间距离就是侧面上的最短路径长. 因此 \(MN=\sqrt{2^2+4^2}=2\sqrt5\)，故选 B.
\end{solution}
...
\[
h^2=AC_1^2-AC^2=16-8=8,\qquad h=2\sqrt2
\]
所以长方体的体积为
\[
...
\item 在直角三角形 \(ACM\) 中，\(AC=3\)，\(CM=AB=3\)，所以
\(AM=\sqrt{AC^2+CM^2}=3\sqrt2\).
折叠不改变长度和 \(AC\) 两侧的角度，故 \(AD=AM=3\sqrt2\)，且 \(\angle DAC=\angle MAC=45^\circ\). 由两平面垂直，\(AD\) 在平面 \(ABC\) 上的射影为 \(AC\)，所以点 \(D\) 到平面 \(ABC\) 的距离为
\(AD\sin45^\circ=3\).
又 \(DQ=\dfrac23DA\)，所以 \(AQ=\dfrac13AD\)，点 \(Q\) 到平面 \(ABC\) 的距离为 \(1\).

由平行四边形知 \(BC=AM=3\sqrt2\)，且
\(BP=\frac23DA=2\sqrt2=\frac23BC\).
所以 \([ABP]=\dfrac23[ABC]=\dfrac23\cdot\frac12\cdot3\cdot3=3\). 因此三棱锥 \(Q-ABP\) 的体积为
\(V=\frac13[ABP]\cdot1=1\).
\end{enumerate}
\end{solution}
...
直线 \(BM\)，\(BN\) 的斜率分别为 \(k_{BM}=\frac{y_1}{x_1+2}=\frac{ky_1}{y_1+4k}\)，\(k_{BN}=\frac{y_2}{x_2+2}=\frac{ky_2}{y_2+4k}\).
于是
\[
k_{BM}+k_{BN}
...
\]
故 \(BM\)，\(BN\) 的斜率互为相反数. 由于 \(BA\) 是 \(x\) 轴正向，直线 \(BM\)，\(BN\) 分别位于 \(BA\) 的两侧且与 \(BA\) 的倾角相等，所以 \(\angle ABM=\angle ABN\). 当 \(l\) 与 \(x\) 轴垂直时，结论也由 \(M\)，\(N\) 关于 \(BA\) 对称立即成立.
\end{enumerate}
\end{solution}
...
\[
a\e^2-\frac12=0,\qquad a=\frac{1}{2\e^2}
\]
此时
\[
...
第二式有两个正根的条件是 \(4k-2<0\) 且判别式 \(\Delta_-=4k(3k-4)>0\) 成立，即 \(k<0\)；当 \(k=0\) 时，第二式有一个正的二重根；当 \(k>0\) 时没有正根. 故公共点总数为 \(3\) 的唯一情形是第一式有一个正的二重根、第二式有两个正根，即 \(k=-\dfrac43\). 因此 \(C_1\) 的方程为 \(y=-\frac43|x|+2\).
\end{enumerate}
\end{solution}
