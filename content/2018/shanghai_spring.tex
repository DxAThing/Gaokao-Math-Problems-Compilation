...
\]
因 \(\mathrm C_9^6=\mathrm C_9^3\)，得 \(a^3=64\). 又 \(a\in\R\)，所以 \(a=4\).
\end{solution}
...
令 \(t=ax\). 因为 \(a>0\) 且 \(x\in(0,1)\)，所以 \(t\in(0,a)\). 正弦值为 \(-\frac12\) 的正数解依次为
\(\frac{7\pi}{6}\)，\(\frac{11\pi}{6}\)，\(\frac{19\pi}{6}\)，\(\ldots\)
区间 \((0,a)\) 中恰有两个这样的解，当且仅当第二个解被包含而第三个解不被包含，即 \(\frac{11\pi}{6}<a\le\frac{19\pi}{6}\).
\end{solution}
...
\begin{solution}
取正方形中心 \(O\) 为原点，令 \(A=(-10,-10)\)、\(B=(10,-10)\)、\(C=(10,10)\)、\(D=(-10,10)\). 设运动时间为 \(t\) 秒，则在点 \(P\) 到达 \(D\) 前
\(P=(-10,-10+1.5t)\)，\(Q=(10,10-t)\)，\(0\le t\le\frac{40}{3}\)
直线 \(PQ\) 的斜率为 \(1-\frac t8\)，且在 \(y\) 轴上的截距为 \(\frac t4\)，故其方程可写为
\[
...
\begin{solution}
函数 \(y=x^{-2}=\frac1{x^2}\) 的定义域为 \(\R\setminus\{0\}\)，关于原点对称，且 \(f(-x)=(-x)^{-2}=x^{-2}=f(x)\)，所以它是偶函数. 函数 \(x^{1/3}\) 与 \(x^3\) 为奇函数，\(x^{-1/2}\) 的定义域不是关于原点对称，故选 A.
\end{solution}
...
\begin{solution}
建立平面直角坐标系，取 \(A=(0,0)\)、\(B=(2,0)\)，设 \(P=(x,y)\). 由 \(\lvert AP\rvert\le5\) 和 \(\overrightarrow{AP}\cdot\overrightarrow{AB}=6\)，有
\(x^2+y^2\le25\)，\(2x=6\)
所以 \(x=3\)，且 \(-4\le y\le4\). 又由 \(\overrightarrow{AQ}=-2\overrightarrow{AP}\)，得 \(Q=(-6,-2y)\).
...
\begin{enumerate}
\item 因为 \(f(\alpha)=\cos\alpha=\frac13\)，且 \(\alpha\in[0,\pi]\)，所以 \(\sin\alpha=\frac{2\sqrt2}{3}\). 于是
\[
...
\begin{solution}
\begin{enumerate}
\item 将点 \((2,1)\) 代入双曲线方程，得 \(\frac4{a^2}-1=1\)，所以 \(a^2=2\). 双曲线的标准形式中 \(b^2=1\)，故焦半距满足 \(c^2=a^2+b^2=3\)，焦点坐标为 \((\sqrt3,0)\)、\((-\sqrt3,0)\).
\item 当 \(a=1\) 时，双曲线为 \(x^2-y^2=1\). 将 \(y=kx+1\) 代入，得
\[
(1-k^2)x^2-2kx-2=0
\]
...
\begin{solution}
\begin{enumerate}
\item 取 \(C\) 为原点，令抛物线的对称轴为 \(y\) 轴. 由于 \(O\) 在对称轴上且在抛物线上，\(O\) 是抛物线的顶点. 由 \(AB=3\) 及 \(OC\perp AB\)，可取 \(A=(-\frac32,0)\)、\(B=(\frac32,0)\)、\(O=(0,\frac92)\). 设抛物线方程为 \(y=\frac92-bx^2\)，代入 \(A\) 得 \(b=2\)，所以抛物线为 \(y=\frac92-2x^2\). 写成标准形式为 \(x^2=-\frac12(y-\frac92)\)，故 \(4p=-\frac12\)，即 \(p=-\frac18\). 焦点到准线的距离为 \(2\lvert p\rvert=\frac14\).
\item 设圆锥底面半径为 \(r\)，高为 \(h\). 由 \(AB\) 是底面直径，得 \(r=AB/2=3/2\). 取底面圆心 \(C\) 为原点，令圆锥轴为 \(z\) 轴，并令截面内的另一水平坐标为 \(y\). 圆锥方程可写为
\[
...
\begin{solution}
\begin{enumerate}
\item 当 \(a=1\) 时，令 \(y=f(x)=\frac1{1+2^x}\). 因为 \(0<y<1\)，且 \(2^x=\frac{1-y}{y}\)，所以
\[
x=\log_2\frac{1-y}{y}
\]
...
\item 令 \(t=2^x\)，则 \(x\le0\) 等价于 \(0<t\le1\)，并且
\(g(x)=-\frac{at}{(1+at)(2+at)}\)，\(g(0)=-\frac{a}{(1+a)(2+a)}\)
由于各分母均为正，条件 \(g(x)\ge g(0)\) 等价于
\[
...
\begin{solution}
\begin{enumerate}
\item 对任意 \(n\in\N^*\)，取 \(m=2n-1\)，则 \(a_m=m+1=2n=c_n\)，且 \(a_m-c_{n+1}=-2\). 因此
\[
\frac{a_m-c_n}{a_m-c_{n+1}}=\frac0{-2}=0\le0
\]
所以 \(\{a_n\}\) 是 \(\{c_n\}\) 的分隔数列.
\item 有
\(S_m=\sum_{j=1}^{m}(j-4)=\frac{m(m-7)}2\)，\(d_n=c_{3n-2}=3n-6\)
取 \(n=4\)，则 \(d_4=6\)、\(d_5=9\). 若存在相应的 \(m\)，由分式不大于零且分母不能为零，必须有 \(6\le S_m<9\). 但当 \(m\le8\) 时 \(S_m\le4\)，当 \(m\ge9\) 时 \(S_m\ge S_9=9\)，不存在这样的 \(m\). 因此 \(\{S_n\}\) 不是 \(\{d_n\}\) 的分隔数列.
\item 因 \(\{c_n\}\) 递增，参数只能落在 \(a>0\)，\(q>1\) 或 \(a<0\)，\(0<q<1\) 两种情形中. 若 \(a<0\)，\(0<q<1\)，则 \(c_1=a\)，而 \(T_1=a\)，对所有 \(m\ge2\) 有 \(T_m<a<c_2\)，所以区间 \([c_2,c_3)\) 中没有任何 \(T_m\)，不满足条件.
