...
\item 侧面积为 \(6\sqrt{7} \mathrm{~cm}^2\).

\item 侧棱和底面的夹角为 \(45^\circ\).
\end{enumerate}
\end{solution}
...
\begin{solution}
解集为 \(\left[-4,-2\right)\cup\left(3,4\right]\).
\end{solution}
...
\begin{solution}
轨迹方程为 \(\dfrac{x^2}{16}+\dfrac{y^2}{4}=1\)，是椭圆.
\end{solution}
...
\begin{solution}
两腰均为 \(15\)，上底为 \(7.5\)，下底为 \(22.5\) 时，面积最大，最大面积为 \(\dfrac{225\sqrt{3}}{2}\).
\end{solution}
...
\begin{solution}
当 \(k=12\) 时，方程组有两组相同的解，且解为 \((x,y)=(6,38)\).
\end{solution}
...
\bitmapinlinefigure[6.0cm]{img/1977/shanghai_science/q9_fig1.png}
\end{problem}

\begin{solution}
当 \(\angle BOA=150^\circ\) 时，面积最大，最大面积为 \(2+\dfrac{5\sqrt{3}}{4}\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 曲线在 \(P\)，\(Q\) 两点的切线斜率分别为 \(-2\)，\(4\).

\item 曲线与直线所围成的图形面积为 \(\dfrac{9}{2}\).
\end{enumerate}
\end{solution}
