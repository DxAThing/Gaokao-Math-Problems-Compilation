...
\bitmapfigure[max width=0.48\linewidth]{img/1977/jiangsu_suzhou_city/q1_fig1.png}
\end{problem}
...
\item 原方程化为
\(2x^2+5x-12=-5\)，\(2x^2+5x-7=0\)，\((2x+7)(x-1)=0\).
所以 \(x=1\) 或 \(x=-\frac{7}{2}\).
...
\(\left(ab^{-2}\right)^{-\frac{1}{2}}=a^{-\frac{1}{2}}b\)，\(a^{-\frac{3}{2}}b\left(ab^{-2}\right)^{-\frac{1}{2}}=a^{-2}b^2\).
所以原式为
\[
...
\begin{problem}
已知 A、B 两地相距 \(600\text{ 公里}\)，从 A 地开往 B 地的客车每小时比货车快 \(15\text{ 公里}\)，客车比货车少用 \(2\) 小时，求客车和货车的速度.
\end{problem}
...
\]
因为 \(v(v+15)>0\)，所以
\(600\times15=2v(v+15)\)，\(v^2+15v-4500=0\)，\((v-60)(v+75)=0\).
...
\((20-s)^2=s^2-120\)，\(\Longrightarrow 400-40s+s^2=s^2-120 \Longrightarrow 40s=520 \Longrightarrow s=13\).
因此 \(b=7\)，且 \(a\)，\(c\) 是方程
\(t^2-13t+40=0\)，\((t-5)(t-8)=0\)
