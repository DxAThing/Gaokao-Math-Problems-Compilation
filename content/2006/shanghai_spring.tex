...
\begin{problem}
计算：\(\displaystyle\lim_{n\to\infty}\dfrac{3n-2}{4n+3}=\)\fillinblank{}.
\end{problem}
...
\begin{solution}
分式的分子、分母分别在 \(x=\frac12\)、\(x=-1\) 处为零或无意义. 当 \(x<-1\) 时，分子为正、分母为负，分式为负；当 \(-1<x<\frac12\) 时，分子、分母均为正，分式为正；当 \(x>\frac12\) 时，分子为负、分母为正，分式为负.
由于不等式要求分式大于零，且 \(x=-1\) 不在定义域内、\(x=\frac12\) 使分式等于零，故解集为 \(\left(-1,\frac12\right)\).
\end{solution}
...
\]
因此 \(u\le\frac{m}{n}u+\frac{n-m}{n}v\le v\)，即
\(\frac{a_1+\cdots+a_m}{m}\le\frac{a_1+\cdots+a_n}{n}\)，\(\frac{a_{m+1}+\cdots+a_n}{n-m}\ge\frac{a_1+\cdots+a_n}{n}\).
\end{solution}
...
\bitmapfigure[max width=0.42\linewidth]{img/2006/shanghai_spring/q17_fig1.png}
\end{problem}
...
\solutionfigure{\bitmapfigure[max width=0.42\linewidth]{img/2006/shanghai_spring/q17_solution_fig1.png}}
...
\solutionfigure{\bitmapfigure[max width=0.42\linewidth]{img/2006/shanghai_spring/q17_solution_fig2.png}}
...
\begin{solution}
\begin{enumerate}
\item 因为 \(\sin x=\dfrac45\)，且 \(x\in\left[\dfrac{\pi}{2},\pi\right]\)，所以 \(\cos x=-\dfrac35\). 又 \(f(x)=2\left(\dfrac{\sqrt3}{2}\sin x+\dfrac12\cos x\right)-2\cos x=\sqrt3\sin x-\cos x\)，故 \(f(x)=\dfrac{4\sqrt3}{5}+\dfrac35\).
\item 由 \(f(x)=2\sin\left(x-\dfrac{\pi}{6}\right)\)，且 \(x\in\left[\dfrac{\pi}{2},\pi\right]\)，得 \(\dfrac{\pi}{3}\le x-\dfrac{\pi}{6}\le\dfrac{5\pi}{6}\). 因此 \(\dfrac12\le \sin\left(x-\dfrac{\pi}{6}\right)\le1\)，从而函数 \(f(x)\) 的值域为 \([1,2]\).
\end{enumerate}
\end{solution}
...
\bitmapfigure[max width=0.42\linewidth]{img/2006/shanghai_spring/q20_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 设曲线方程为 \(y=ax^2+\dfrac{64}{7}\). 由题意知抛物线过点 \(D(8,0)\)，故 \(0=64a+\dfrac{64}{7}\)，所以 \(a=-\dfrac17\). 因此，航天器变轨后的运行轨迹所在的曲线方程为 \(y=-\dfrac17x^2+\dfrac{64}{7}\).
\end{enumerate}

\solutionfigure{\bitmapfigure[max width=0.42\linewidth]{img/2006/shanghai_spring/q20_fig1.png}}
\FloatBarrier

\begin{enumerate}
\setcounter{enumi}{1}
\item 设变轨点为 \(C(x,y)\). 由题意可知
\[
\begin{cases}
...
\bitmapfigure[max width=0.42\linewidth]{img/2006/shanghai_spring/q21_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 函数 \(y=x^2-4x-5\) 的零点为 \(-1,5\)，顶点为 \((2,-9)\). 因此，在区间 \([-2,6]\) 上，函数 \(f(x)=|x^2-4x-5|\) 的图象由抛物线 \(y=x^2-4x-5\) 在 \(x\le-1\) 与 \(x\ge5\) 部分，以及其在 \([-1,5]\) 上关于 \(x\) 轴对称所得的曲线组成.
\end{enumerate}

\solutionfigure{\bitmapfigure[max width=0.42\linewidth]{img/2006/shanghai_spring/q21_solution_fig1.png}}
\FloatBarrier

\begin{enumerate}
\setcounter{enumi}{1}
\item 方程 \(f(x)=5\) 的解分别是 \(2-\sqrt{14}\)，\(0\)，\(4\)，\(2+\sqrt{14}\). 由于 \(f(x)\) 在 \((-\infty,-1]\) 和 \([2,5]\) 上单调递减，在 \([-1,2]\) 和 \([5,+\infty)\) 上单调递增，所以 \(A=(-\infty,2-\sqrt{14}]\cup[0,4]\cup[2+\sqrt{14},+\infty)\).
又因为 \(2+\sqrt{14}<6\)，\(2-\sqrt{14}>-2\)，所以 \(B\subset A\).

\item 【法一】当 \(x\in[-1,5]\) 时，\(f(x)=-(x^2-4x-5)=-x^2+4x+5\). 令
\[
...
\begin{circlelist}
\item 当 \(-1\le\dfrac{4-k}{2}<1\)，即 \(2<k\le6\) 时，取 \(x=\dfrac{4-k}{2}\)，则 \(g(x)_{\min}=-\dfrac{k^2-20k+36}{4}=-\dfrac14\bigl[(k-10)^2-64\bigr]\). 因为 \(16\le(k-10)^2<64\)，所以 \(g(x)_{\min}>0\).
\item 当 \(\dfrac{4-k}{2}<-1\)，即 \(k>6\) 时，取 \(x=-1\)，则 \(g(x)_{\min}=2k>0\).
\end{circlelist}
...
\begin{solution}
\begin{enumerate}
\item \(a_{10}=10\). 又 \(a_{20}=10+10d=40\)，所以 \(d=3\).

\item 由题意，\(a_{30}=a_{20}+10d^2=10(1+d+d^2)\)（\(d\ne0\)）. 于是 \(a_{30}=10\left[\left(d+\dfrac12\right)^2+\dfrac34\right]\). 当 \(d\in(-\infty,0)\cup(0,+\infty)\) 时，\(a_{30}\in\left[\dfrac{15}{2},+\infty\right)\).

\item 所给数列可推广为无穷数列 \(\{a_n\}\). 其中 \(a_1\)，\(a_2\)，\(\cdots\)，\(a_{10}\) 是首项为 \(1\)，公差为 \(1\) 的等差数列；当 \(n\ge1\) 时，数列 \(a_{10n}\)，\(a_{10n+1}\)，\(\cdots\)，\(a_{10(n+1)}\) 是公差为 \(d^n\) 的等差数列.

研究的问题可以是：试写出 \(a_{10(n+1)}\) 关于 \(d\) 的关系式，并在 \(d\ne0\) 的条件下求 \(a_{10(n+1)}\) 的取值范围，其中 \(n\) 为正整数.

每个长度为 10 的等差数列区段包含 10 个公差，因此对 \(n\ge1\) 有递推关系
\(a_{10(n+1)}=a_{10n}+10d^n\)，\(a_{10}=10\).
逐项展开可得
\[
...
\[
m_n=10\min_{d\in\R}(1+d+\cdots+d^n)
\]
...
\qquad d\ne0
\]
其中 \(m_n=10\min_{d\in\R}(1+d+\cdots+d^n)\). 当 \(n=2\) 时，\(m_2=\dfrac{15}{2}\)，故确实包含第 \(2\) 问作为特例.
\end{enumerate}
\end{solution}
