...
\item 在 \(\triangle ABC\)
\end{circlelist}
中，若 \(\angle C = 90^{\circ}\)，则 \(\|AC\|^{2} + \|CB\|^{2} = \|AB\|^{2}\)；
中，\(\|AC\| + \|CB\| > \|AB\|\).
...
\begin{problem}
如图，连结 \(\triangle ABC\) 的各边中点得到一个新的 \(\triangle A_{1}B_{1}C_{1}\)，又连结 \(\triangle A_{1}B_{1}C_{1}\) 的各边中点得到一个新的 \(\triangle A_{2}B_{2}C_{2}\)，如此无限继续下去，得到一系列三角形：\(\triangle ABC, \triangle A_{1}B_{1}C_{1}, \triangle A_{2}B_{2}C_{2}, \cdots\)，这一系列三角形趋向于一个点 \(M\). 已知 \(A(0,0), B(3,0), C(2,2)\)，则点 \(M\) 的坐标是 \fillinblank{} \fillinblank{}.
\bitmapfigure[max width=0.55\linewidth,max totalheight=5.2cm]{img/2006/fujian_science/q16_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.45\linewidth,max totalheight=4.8cm]{img/2006/fujian_science/q18_fig1.png}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=5.0cm]{img/2006/fujian_science/q20_fig1.png}
