...
\begin{solution}
由 \(x^2+x-6=(x+3)(x-2)\)，得 \(Q=[-3,2]\). 又 \(P=\{1,2,\dots,10\}\)，所以 \(P\cap Q=\{1,2\}\)，故选 B.
\end{solution}
...
\begin{solution}
先计算分子，\((1+\i)^2=1+2\i+\i^2=2\i\). 分母有理化得 \(\frac{2\i}{1-\i}=\frac{2\i(1+\i)}{(1-\i)(1+\i)}=\i(1+\i)=-1+\i\)，故选 C.
\end{solution}
...
\begin{solution}
有理化根式差，得 \(\sqrt{n^2+1}-\sqrt{n^2-1}=\frac{2}{\sqrt{n^2+1}+\sqrt{n^2-1}}\). 因此原式等于 \(\frac{\sqrt{n^2+1}+\sqrt{n^2-1}}{4n}=\frac{1}{4}\left(\sqrt{1+\frac{1}{n^2}}+\sqrt{1-\frac{1}{n^2}}\right)\). 令 \(n\to\infty\)，所得极限为 \(\frac14(1+1)=\frac12\)，故选 B.
\end{solution}
...
\begin{solution}
由函数图象过 \((2,1)\)，得 \(\log_a(2+b)=1\)，所以 \(a=2+b\). 反函数图象过 \((2,8)\)，等价于原函数图象过对称点 \((8,2)\)，故 \(\log_a(8+b)=2\)，即 \(a^2=8+b\). 代入 \(b=a-2\)，得 \(a^2=a+6\)，即 \((a-3)(a+2)=0\). 由 \(a>0\)，得 \(a=3\)，进而 \(b=1\)，所以 \(a+b=4\)，故选 C.
\end{solution}
...
\begin{solution}
直线过 \((0,a)\) 且斜率为 1，方程为 \(y=x+a\)，即 \(x-y+a=0\). 圆心为原点，半径为 \(\sqrt2\)，相切条件是圆心到直线的距离等于半径，即 \(\frac{|a|}{\sqrt2}=\sqrt2\). 所以 \(|a|=2\)，得 \(a=\pm2\)，故选 B.
\end{solution}
...
\begin{solution}
若 \(\alpha\)，\(\beta\)，\(\gamma\) 成等差数列，则 \(\alpha+\gamma=2\beta\)，从而必有 \(\sin(\alpha+\gamma)=\sin2\beta\)，所以该等式是必要条件. 反之不成立，例如取 \(\alpha=0\)，\(\beta=0\)，\(\gamma=\pi\)，则 \(\sin(\alpha+\gamma)=\sin\pi=0=\sin0=\sin2\beta\)，但 \(\alpha+\gamma=\pi\ne0=2\beta\)，三数不成等差数列. 因此该条件必要而不充分，故选 A.
\end{solution}
...
\begin{solution}
双曲线的渐近线为 \(y=\pm\frac{\sqrt2}{a}x\). 设 \(m=\frac{\sqrt2}{a}\)，由 \(a>\sqrt2\) 知 \(0<m<1\). 两直线的锐角夹角为 \(\frac\pi3\)，故 \(\frac{2m}{1-m^2}=\tan\frac\pi3=\sqrt3\). 整理得 \(\sqrt3m^2+2m-\sqrt3=0\)，所以 \(m=\frac1{\sqrt3}\)（舍去负根）. 于是 \(a=\sqrt6\). 双曲线的 \(b^2=2\)，故 \(c^2=a^2+b^2=8\)，\(c=2\sqrt2\)，离心率 \(e=\frac ca=\frac{2\sqrt2}{\sqrt6}=\frac{2\sqrt3}{3}\)，故选 D.
\end{solution}
...
\begin{solution}
令 \(t=\frac{x}{y}>0\)，则 \((x+y)\left(\frac1x+\frac ay\right)=1+a+t+\frac at\). 由基本不等式，\(t+\frac at\geqslant2\sqrt a\)，且在 \(t=\sqrt a\) 时取等号. 因此对任意正数 \(x\)，\(y\) 恒成立的充要条件为 \(1+a+2\sqrt a\geqslant9\). 令 \(u=\sqrt a>0\)，则 \(u^2+2u-8=(u-2)(u+4)\geqslant0\)，所以 \(u\geqslant2\)，即 \(a\geqslant4\). 当 \(a=4\) 且 \(x/y=2\) 时等号成立，故最小值为 4，选 B.
\end{solution}
...
\begin{solution}
设 \(p=|\overrightarrow{AB}|>0\)，\(q=|\overrightarrow{AC}|>0\)，并令 \(\bs u=\frac{\overrightarrow{AB}}{|\overrightarrow{AB}|}\)，\(\bs v=\frac{\overrightarrow{AC}}{|\overrightarrow{AC}|}\). 已知 \(\bs u\cdot\bs v=\frac12\)，且 \(\overrightarrow{BC}=q\bs v-p\bs u\)，所以 \(0=(\bs u+\bs v)\cdot\overrightarrow{BC}=q(\bs u\cdot\bs v+\bs v\cdot\bs v)-p(\bs u\cdot\bs u+\bs v\cdot\bs u)=\frac32(q-p)\). 从而 \(p=q\)，即 \(AB=AC\). 又 \(\bs u\cdot\bs v=\frac12\) 表明 \(\angle BAC=\frac\pi3\). 等腰三角形的顶角为 \(60^{\circ}\)，两个底角也各为 \(60^{\circ}\)，所以 \(\triangle ABC\) 为等边三角形，故选 D.
\end{solution}
...
\begin{solution}
计算函数值之差，得 \(f(x_2)-f(x_1)=a(x_2-x_1)(x_1+x_2+2)=a(x_2-x_1)(3-a)\). 由 \(0<a<3\) 且 \(x_2-x_1>0\)，知右端大于 0，所以 \(f(x_1)<f(x_2)\)，故选 A.
\end{solution}
...
\begin{solution}
设三点到平面 \(\alpha\) 的距离为 \(h>0\). 距离相等的点位于与 \(\alpha\) 平行的两个平面上：一个在 \(\alpha\) 的一侧，另一个在另一侧. 三点只有两侧可分，因此至少有两点，例如 \(A\)，\(B\)，位于同一个与 \(\alpha\) 平行的平面内，于是 \(AB\parallel\alpha\). 连接 \(AC\) 与 \(BC\) 的中点，所得三角形中位线平行于 \(AB\)，因而平行于 \(\alpha\). 所以必存在 \(\triangle ABC\) 的一条中位线平行于 \(\alpha\)，故选 D.
\end{solution}
...
\begin{solution}
由第四个密文 \(4d=28\)，得 \(d=7\). 代入第三个密文方程 \(2c+3d=23\)，得 \(2c+21=23\)，所以 \(c=1\). 再由第二个方程 \(2b+c=9\)，得 \(2b+1=9\)，所以 \(b=4\). 最后由第一个方程 \(a+2b=14\)，得 \(a+8=14\)，所以 \(a=6\). 因此明文为 \(6\)，\(4\)，\(1\)，\(7\)，故选 C.
\end{solution}
...
\begin{solution}
因为 \(\cos77^{\circ}=\sin13^{\circ}\)，且 \(\cos167^{\circ}=\cos(180^{\circ}-13^{\circ})=-\cos13^{\circ}\)，所以原式等于 \(\cos43^{\circ}\sin13^{\circ}-\sin43^{\circ}\cos13^{\circ}=\sin(13^{\circ}-43^{\circ})=\sin(-30^{\circ})=-\frac12\).
\end{solution}
...
\begin{solution}
通项中取第二项 \(-x^{-1/2}\) 共 \(k\) 次时，该项为 \(\mathrm C_{12}^{k}(3x)^{12-k}(-x^{-1/2})^k\)，其中 \(x\) 的指数为 \(12-k-\frac{k}{2}=12-\frac{3k}{2}\). 令其等于 \(-3\)，得 \(k=10\). 因此所求系数为 \(\mathrm C_{12}^{10}3^2(-1)^{10}=66\times9=594\).
\end{solution}
...
\begin{solution}
设正方形四个顶点所对应的大球球心为 \(O_1\)，\(O_2\)，\(O_3\)，\(O_4\)，其中心为 \(O\)，小球球心为 \(P\)，且 \(P\) 到桌面 \(\alpha\) 的距离为 \(h\). 大球半径为 \(2R\)，所以四个大球球心都在距 \(\alpha\) 为 \(2R\) 的平面内. 相邻大球相切，故正方形边长为 \(4R\)，从而正方形中心到任一顶点的距离为半对角线 \(2\sqrt{2}R\).

小球与四个大球都相切，因此 \(P\) 到四个大球球心的距离相等，\(P\) 在正方形中心 \(O\) 的竖直线上. 又因小球放在四个大球上方，\(h>2R\)，且
\[
PO_i=2R+R=3R
\]
于是
\[
(2\sqrt{2}R)^2+(h-2R)^2=(3R)^2
\]
即 \((h-2R)^2=R^2\). 由 \(h>2R\)，得 \(h-2R=R\)，所以 \(h=3R\). 因此小球的球心到桌面 \(\alpha\) 的距离为 \(3R\).
\end{solution}
...
\begin{solution}
把另外 5 名教师记为其余教师. 因为甲和乙不能同时去，且甲、丙必须同去或同不去，分三类计数.

第一类，甲、丙同去. 此时乙不能去，还需从其余 5 名教师中选 2 名，选出的 4 名教师再分配到 4 个不同地区，方案数为
\[
\mathrm C_5^2\cdot4!=10\cdot24=240
\]

第二类，甲、丙同不去且乙去. 此时还需从其余 5 名教师中选 3 名，方案数为
\[
\mathrm C_5^3\cdot4!=10\cdot24=240
\]

第三类，甲、乙、丙都不去. 此时从其余 5 名教师中选 4 名，方案数为
\[
\mathrm C_5^4\cdot4!=5\cdot24=120
\]

三类互不相交，故不同的选派方案共有 \(240+240+120=600\) 种.
\end{solution}
...
\bitmapfigure[width=9.6cm]{img/2006/shaanxi_science/q21_fig1.png}
\end{problem}
