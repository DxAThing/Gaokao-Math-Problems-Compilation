...
\begin{problem}
计算：\(\displaystyle\lim_{n\to\infty}\frac{n(n^2 + 1)}{6n^3 + 1} =\)\fillinblank{}.
\end{problem}
...
\begin{problem}
在一个小组中有 \(8\text{ 名}\) 女同学和 \(4\text{ 名}\) 男同学，从中任意地挑选 \(2\text{ 名}\) 同学担任交通安全宣传志愿者，那么选到的两名都是女同学的概率是\fillinblank{}（结果用分数表示）.
\end{problem}
...
\bitmapfigure[max width=0.40\linewidth]{img/2006/shanghai_liberal/q13_fig1.png}
...
\bitmapfigure[max width=0.44\linewidth]{img/2006/shanghai_liberal/q18_fig1.png}
\end{problem}

\begin{solution}
建立直角坐标系：取 \(A\) 为原点，正东方向为 \(x\) 轴正向，正北方向为 \(y\) 轴正向. 则
\(A=(0,0)\)，\(B=(20,0)\).
“南偏西 \(30^\circ\)”表示从正南方向向西偏 \(30^\circ\)，所以
\[
...
\bitmapfigure[max width=0.38\linewidth]{img/2006/shanghai_liberal/q19_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 取 \(B=(0,0,0)\)，令 \(A=(1,0,0)\)、\(C=(0,1,0)\)，并设直三棱柱高为 \(h\). 则
\(B_1=(0,0,h)\)，\(C_1=(0,1,h)\).
异面直线 \(B_1C_1\) 与 \(AC\) 所成的角，等于它们方向向量的夹角. 取
\(\overrightarrow{B_1C_1}=(0,1,0)\)，\(\overrightarrow{AC}=(-1,1,0)\).
于是
\[
\cos\theta
...
\]
故 \(\theta=45^\circ\).
\item 直线 \(A_1C\) 与平面 \(ABC\) 所成的角为 \(45^\circ\). 其在平面 \(ABC\) 上的射影为 \(AC\)，且
\[
AC=\sqrt{AB^2+BC^2}=\sqrt2
\]
在直角三角形 \(A_1AC\) 中，\(AA_1\perp\text{平面 }ABC\)，所以
\(\tan45^\circ=\frac{AA_1}{AC}\)，\(AA_1=\sqrt2\).
底面 \(ABC\) 的面积为
\[
...
\begin{solution}
\begin{enumerate}
\item 当 \(n=1\) 时，\(S_1=a_1\)，由题设
\(a_1+S_1=2a_1=4096\)，\(a_1=2048\).
对任意 \(n\geqslant2\)，由 \(S_n=S_{n-1}+a_n\) 以及题设分别有
\(a_n+S_n=4096\)，\(a_{n-1}+S_{n-1}=4096\).
两式相减得
\[
a_n+S_{n-1}+a_n-\left(a_{n-1}+S_{n-1}\right)=0
\]
即
\(2a_n=a_{n-1}\)，\(a_n=\frac12a_{n-1}\).
所以 \(\{a_n\}\) 是首项为 \(2048\)、公比为 \(\dfrac12\) 的等比数列，从而
\[
...
\]
要求 \(T_n<-509\)，即
\(T_{45}=\frac{-45^2+23\cdot45}{2}=-495>-509\)，\(T_{46}=\frac{-46^2+23\cdot46}{2}=-529<-509\).
又
\[
T_n-T_{n-1}=12-n
\]
所以当 \(n\geqslant12\) 时，\(\{T_n\}\) 严格递减；由 \(T_{45}>-509\) 和
\(T_{46}<-509\) 可知，从第 \(46\) 项起 \(T_n<-509\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 椭圆中心在原点，右顶点为 \(D(2,0)\)，所以长半轴 \(a=2\). 左焦点为 \(F(-\sqrt3,0)\)，故焦距的一半 \(c=\sqrt3\). 由
\[
b^2=a^2-c^2=4-3=1
\]
得 \(b=1\)，所以椭圆的标准方程为
...
\frac{x^2}{4}+y^2=1
\]
\item 设线段 \(PA\) 的中点为 \(M(x,y)\)，动点 \(P\) 的坐标为 \((u,v)\). 由中点坐标公式，
\(x=\frac{u+1}{2}\)，\(y=\frac{v+\frac12}{2}\)
即
\(u=2x-1\)，\(v=2y-\frac12\).
因为 \(P\) 在椭圆上，
\[
...
\]
\item 过原点的直线与椭圆交于 \(B\)，\(C\). 设该直线的单位方向向量为
\[
(\cos\theta,\sin\theta)
\]
则由于椭圆关于原点中心对称，可设
\(B=t(\cos\theta,\sin\theta)\)，\(C=-B\)
其中
\(\frac{t^2\cos^2\theta}{4}+t^2\sin^2\theta=1\)，\(t=\frac{1}{\sqrt{\frac{\cos^2\theta}{4}+\sin^2\theta}}\).
三角形 \(ABC\) 的面积为
\[
...
\begin{enumerate}
\item 如果函数 \(y = x + \frac{2^b}{x}\)（\(x > 0\)）在 \((0,4]\) 上是减函数，在 \([4, +\infty)\) 上是增函数，求 \(b\) 的值；
...
\begin{solution}
\begin{enumerate}
\item 题设给出的性质中，函数 \(x+\dfrac{a}{x}\) 的转折点是 \(x=\sqrt a\). 这里 \(a=2^b\)，而转折点为 \(4\)，所以
\[
\sqrt{2^b}=4
\iff 2^b=16
\iff b=4
\]
\item 在 \(x>0\) 时，题设性质表明 \(f(x)=x+\dfrac{c}{x}\) 在
\[
...
f(\sqrt c)=2\sqrt c
\]
最大值只能在端点 \(x=1\)，\(2\) 处取得. 两个端点函数值为
\(f(1)=1+c\)，\(f(2)=2+\frac c2\).
比较二者：
\[
f(1)-f(2)=\frac c2-1
\]
...
\min f(x)=2\sqrt c
\]
\item 令 \(r=\sqrt[2n]{c}>0\)，并令 \(t=x^n\). 此时
\[
g(x)=t+\frac ct
\]
当 \(x>0\) 时，\(t=x^n\) 随 \(x\) 增大而增大；而 \(t+\dfrac ct\) 在
\((0,\sqrt c]\) 上递减、在 \([\sqrt c,+\infty)\) 上递增. 转折点
\(t=\sqrt c\) 对应 \(x=r\)，所以 \(g\) 在 \((0,r]\) 上递减，在
\([r,+\infty)\) 上递增.

当 \(x<0\) 时，考察
\(h(t)=t+\frac ct\)，\(h'(t)=1-\frac{c}{t^2}\).
因此 \(h\) 在 \((-\infty,-\sqrt c]\) 上递增，在
\([-\sqrt c,0)\) 上递减.
