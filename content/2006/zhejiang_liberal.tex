...
\begin{solution}
由二项式定理，\((x+1)^6=\sum_{k=0}^{6}\mathrm C_6^k x^k\). 因此含 \(x^3\) 的项对应 \(k=3\)，其系数为 \(\mathrm C_6^3=\frac{6\times5\times4}{3\times2\times1}=20\)，故选 B.
\end{solution}
...
\begin{solution}
由 \(\bs{a}+\bs{b}+\bs{c}=0\)，得 \(\bs{c}=-(\bs{a}+\bs{b})\). 因为 \(\bs{a}\perp\bs{b}\)，所以 \(\bs{a}\cdot\bs{b}=0\)，从而
\(\lvert\bs{c}\rvert^2=\lvert\bs{a}+\bs{b}\rvert^2=\lvert\bs{a}\rvert^2+\lvert\bs{b}\rvert^2+2\bs{a}\cdot\bs{b}=1^2+2^2=5\)，故选 D.
\end{solution}
...
\begin{solution}
对任意实数 \(x\)，由三角不等式
\(\lvert x+1\rvert+\lvert x-2\rvert\geqslant\lvert(x+1)-(x-2)\rvert=3\). 又因为两个非负数中较大的数不小于它们平均数，所以
\(\max\{\lvert x+1\rvert,\lvert x-2\rvert\}\geqslant\frac{\lvert x+1\rvert+\lvert x-2\rvert}{2}\geqslant\frac32\). 当 \(x=\frac12\) 时，\(\lvert x+1\rvert=\lvert x-2\rvert=\frac32\)，上述下界可以取得. 因此最小值为 \(\frac32\)，选 C.
\end{solution}
...
\begin{solution}
利用二倍角公式 \(2\sin x\cos x=\sin2x\)，得 \(y=\sin2x-1\). 因为 \(-1\leqslant\sin2x\leqslant1\)，所以 \(-2\leqslant y\leqslant0\). 当 \(x=\frac{\pi}{4}\) 时 \(\sin2x=1\)，当 \(x=\frac{3\pi}{4}\) 时 \(\sin2x=-1\)，两个端点都能取得，故值域为 \([-2,0]\).
\end{solution}
...
\begin{solution}
由双曲线的标准形式可知必须有 \(m>0\)，并可写成 \(\frac{x^2}{a^2}-\frac{y^2}{b^2}=1\)，其中 \(a^2=m\)，\(b^2=1\). 设焦半距为 \(c\)，则 \(c^2=a^2+b^2=m+1\).

设双曲线上任意一点为 \(Q(x,y)\)，左焦点为 \(F_1(-c,0)\)，左准线为 \(x=-\frac{a^2}{c}\). 由双曲线方程，\(y^2=\frac{b^2}{a^2}(x^2-a^2)\)，于是
\(\lvert QF_1\rvert^2=(x+c)^2+y^2=\frac{c^2}{a^2}\left(x+\frac{a^2}{c}\right)^2\). 因而点到左焦点的距离与到左准线的距离之比为 \(\frac ca=e\). 题设给出 \(e=3\)，所以
\(\frac{c^2}{a^2}=\frac{m+1}{m}=9\)，解得 \(m=\frac18\).
\end{solution}
...
\begin{solution}
建立空间直角坐标系，取 \(A=(0,0,0)\)，\(B=(1,0,0)\)，\(C=\left(\frac12,\frac{\sqrt3}{2},0\right)\)，\(D=\left(\frac12,\frac{\sqrt3}{6},\sqrt{\frac23}\right)\). 令 \(\bs{u}=\overrightarrow{AB}=(1,0,0)\)，\(\bs{v}=\overrightarrow{CD}=\left(0,-\frac{\sqrt3}{3},\sqrt{\frac23}\right)\). 有 \(\lvert\bs{u}\rvert=\lvert\bs{v}\rvert=1\) 且 \(\bs{u}\perp\bs{v}\)，所以过 \(AB\) 且平行于 \(CD\) 的平面 \(\alpha\) 可用 \(\bs{u}\)，\(\bs{v}\) 作为一组正交单位基底.

在这组基底下，点的正投影坐标由分别与 \(\bs{u}\)，\(\bs{v}\) 的内积给出，因此四个顶点的投影分别为 \(A'=(0,0)\)，\(B'=(1,0)\)，\(C'=\left(\frac12,-\frac12\right)\)，\(D'=\left(\frac12,\frac12\right)\). 正四面体及其投影都是相应顶点的凸包，故投影图形为这四个点的凸包. 该图形的两条对角线 \(A'B'\)，\(C'D'\) 互相垂直，长度都为 \(1\)，所以面积为 \(\frac12\times1\times1=\frac12\).
\end{solution}
...
\item 函数取得最大值时，\(\pi x+\frac{\pi}{6}=\frac{\pi}{2}+2k\pi\). 按图取最高点 \(P\) 对应的 \(k=0\)，得 \(P\left(\frac13,2\right)\). 图中与 \(P\) 相邻的两个 \(x\) 轴交点满足 \(\pi x+\frac{\pi}{6}=k\pi\)，取相邻的 \(k=0\)，\(1\)，故 \(M\left(-\frac16,0\right)\)，\(N\left(\frac56,0\right)\).
