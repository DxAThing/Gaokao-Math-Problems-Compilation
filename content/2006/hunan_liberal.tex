...
\begin{solution}
\begin{enumerate}
\item \(PQ \perp \text{平面 }ABCD\).
\item 异面直线所成的角为 \(\arccos\frac{1}{3}\).
\item 点 \(P\) 到平面 \(QAD\) 的距离为 \(2\sqrt{2}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 当 \(a>0\) 时，函数在 \((-\infty,0)\)，\(\left(\frac{2}{a},+\infty\right)\) 上是增函数，在 \(\left(0,\frac{2}{a}\right)\) 上是减函数；当 \(a<0\) 时，函数在 \(\left(\frac{2}{a},0\right)\) 上是增函数，在 \((-\infty,\frac{2}{a})\)，\((0,+\infty)\) 上是减函数.
\item \(a\in\left[-1,0\right)\cup[3,4]\).
\end{enumerate}
\end{solution}

\begin{problem}
在 \(m\)（\(m \geqslant 2\)）个不同数的排列 \(P_{1}P_{2}\cdots P_{m}\) 中，若 \(1 \leqslant i < j \leqslant m\) 时 \(P_{i} > P_{j}\)（即前面某数大于后面某数），则称 \(P_{i}\) 与 \(P_{j}\) 构成一个逆序. 一个排列的全部逆序的总数称为该排列的逆序数. 记排列 \((n + 1)n(n - 1)\cdots 321\) 的逆序数为 \(a_{n}\)，如排列 \(21\) 的逆序数 \(a_{1} = 1\)，排列 \(321\) 的逆序数 \(a_{2} = 3\)，排列 \(4321\) 的逆序数 \(a_{3} = 6\).
...
\begin{solution}
\begin{enumerate}
\item \(a_{4}=10\)，\(a_{5}=15\)，\(a_{n}=\frac{n(n+1)}{2}\).
\item \(b_{n}=\frac{n}{n+2}+\frac{n+2}{n}\)，且 \(b_{1}+b_{2}+\cdots+b_{n}=2n+3-\frac{2}{n+1}-\frac{2}{n+2}\)，故 \(2n<b_{1}+b_{2}+\cdots+b_{n}<2n+3\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(m=0\)，\(p=\frac{9}{8}\)，直线 \(AB\) 为 \(x=1\)，抛物线 \(C_{2}\) 的焦点为 \(\left(\frac{9}{16},0\right)\)，不在直线 \(AB\) 上.
\item \(m=\frac{\sqrt{6}}{3}\) 时，\(AB:y=-\sqrt{6}(x-1)\)；\(m=-\frac{\sqrt{6}}{3}\) 时，\(AB:y=\sqrt{6}(x-1)\).
\end{enumerate}
\end{solution}
