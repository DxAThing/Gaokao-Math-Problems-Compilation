...
\begin{solution}
四条有向线段首尾相接构成封闭四边形时，对应向量的和必须为零向量，因此
\[
...
该结果不在题面给出的四个选项中，所以按原题面不能确定一个合法选项.
\end{solution}
...
作符号判断可得\(q\)的解集为\((-\infty,-2)\cup(-1,1)\cup(2,+\infty)\). 显然\(p\)的解集包含于\(q\)的解集，但例如\(x=0\)满足\(q\)而不满足\(p\). 因此\(p\)是\(q\)的充分不必要条件，故选 A.
\end{solution}
...
\]
由于第三项和第五项存在，\(n\geqslant4\)，故\(n=10\).
此时常数项应满足
\[
3n-\frac72r=0
\Longrightarrow r=\frac{6n}{7}=\frac{60}{7},
\]
这不是整数，所以按题面二项式展开不存在常数项，题面给出的四个选项均不符合该计算结果.
\end{solution}
...
又\(AE=BE=1\)，且由直角三角形可得\(DE=CE=1\)，所以折叠前\(\triangle ADE\)和\(\triangle BEC\)都是边长为\(1\)的等边三角形，且\(DC=DE=CE=1\).
...
\begin{solution}
设过\(P\)的非水平直线写成\(x=4+ky\). 代入\(y^2=4x\)，得
\(y^2-4ky-16=0\).
其两个根为\(y_1\)，\(y_2\)，所以
...
\item 最大数字为\(k\)时，取法数为\(\mathrm C_{2k}^{3}-\mathrm C_{2k-2}^{3}\). 因此\(P(\xi=1)=0\)，\(P(\xi=2)=\dfrac1{30}\)，\(P(\xi=3)=\dfrac2{15}\)，\(P(\xi=4)=\dfrac3{10}\)，\(P(\xi=5)=\dfrac8{15}\).
并且
\[
E(\xi)
...
\]
解得\(q^2=4\)，故\(q=\pm2\). 所以\(Q=(2,0)\)或\(Q=(-2,0)\).
\end{enumerate}
\end{solution}
...
由\(a_1=2\)以及递推式\(a_{n+1}=a_n(a_n+2)\)，归纳可知\(a_n>0\)，所以所有\(1+a_n>0\)，取常用对数得\(\lg(1+a_{n+1})=2\lg(1+a_n)\).
