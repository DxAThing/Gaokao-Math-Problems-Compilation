...
令 \(t=x-\dfrac{\pi}{4}\)，则
\[
f\left(\frac{\pi}{4}+t\right)=\frac{a-b}{\sqrt2}\cos t+\frac{a+b}{\sqrt2}\sin t
\]
由于 \(t=0\) 是上述函数的最小值点，其导数在 \(t=0\) 处为零，所以 \(\dfrac{a+b}{\sqrt2}=0\)，即 \(b=-a\). 又因 \(a\neq0\)，且 \(t=0\) 处取得的是最小值，故 \(\dfrac{a-b}{\sqrt2}=\sqrt2a<0\). 设 \(R=\sqrt{a^{2}+b^{2}}>0\)，于是
\[
...
\(T(x)=4\cdot\frac{400}{x}+4x=\frac{1600}{x}+4x\)，\(x>0\).
由基本不等式，
\[
...
\]
由余弦定理，\(\cos B=\frac{AB^{2}+BC^{2}-AC^{2}}{2AB\cdot BC}=\frac{2+1-4}{2\sqrt2}=-\frac{\sqrt2}{4}\).
由于 \(A\)，\(B\) 是三角形内角，故
\[
\sin A=\sqrt{1-\cos^{2}A}=\frac{\sqrt{14}}{8}
\]
同样由 \(B\) 是三角形内角，\(\sin B>0\)，所以 \(\sin B=\sqrt{1-\cos^{2}B}=\sqrt{1-\frac18}=\frac{\sqrt{14}}{4}\).
因此
\[
...
\]
因为 \(\bs{u}\) 和 \(\bs{w}\) 都是平面 \(CDE\) 内的方向向量，且五面体非退化给出 \(F\notin\) 平面 \(CDE\)，所以 \(FO\parallel\) 平面 \(CDE\).
...
\]
\item 对上述范围内的 \(\theta\)，有 \(0<c<\dfrac{\sqrt3}{2}\)，所以 \(f'(x)>0\) 的区间为 \(\left(-\infty,0\right)\) 和 \(\left(\dfrac{c}{2},+\infty\right)\). 先注意区间 \((2a-1,a)\) 非空，必须有 \(a<1\). 若 \(a\leqslant0\)，该区间完全包含在 \(\left(-\infty,0\right)\) 内，条件成立；若 \(a>0\)，该区间含有正数，不可能对所有参数都落在 \(\left(-\infty,0\right)\) 内，因此必须对所有 \(0<c<\dfrac{\sqrt3}{2}\) 有 \(2a-1\geqslant\dfrac{c}{2}\).
...
当 \(\lambda>0\) 时，\(s_{2}=1\)，且 \(s_{3}\geqslant\lambda=\lambda^{2-1}\). 若对某个 \(n\geqslant2\) 有 \(s_{n}\geqslant\lambda^{n-2}\)，则 \(s_{n+1}\geqslant\lambda s_{n}\geqslant\lambda^{n-1}\). 由数学归纳法，\(s_{n+1}\geqslant\lambda^{n-1}\) 对所有 \(n\geqslant2\) 成立. 因为各项均为正数，所以
\[
\frac{x_{n+1}}{y_{n+1}}=\frac{x_{n}}{y_{n}}\cdot\frac{x_{n+1}/x_{n}}{y_{n+1}/y_{n}}\leqslant\frac{x_{n}}{y_{n}}
\]
\item 当 \(\lambda>1\) 时，\(x_n>0\)，\(y_n>0\). 又 \(x_{1}/y_{1}=x_{2}/y_{2}=1/2\)，由（2）有 \(x_{n}/y_{n}\leqslant1/2\)，所以 \(y_{n}\geqslant2x_{n}>x_{n}\). 令 \(d_{n}=y_{n}-x_{n}>0\)，则 \(d_{1}=d_{2}=1\)，且对 \(n\geqslant2\)，
...
\]
因此
\(\frac{x_{n}-y_{n}}{x_{n+1}-y_{n+1}}=\frac{d_{n}}{d_{n+1}}\leqslant\frac{1}{\lambda^{n-1}}\)，\((n\geqslant2)\).
...
\]
由上面两点的纵坐标分别为 \(\frac{\beta-\alpha}{2}\) 和 \(\frac{\beta+\alpha}{2}\)，有 \(v_{1}v_{2}=\frac{(\beta-\alpha)(\beta+\alpha)}{4}=\frac{\beta^{2}-\alpha^{2}}{4}\).
于是
\[
