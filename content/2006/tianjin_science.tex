...
\begin{problem}
设函数 \(f(x)=\frac{1}{x+1}\)，点 \(A_{0}\) 表示坐标原点，点 \(A_{n}(n,f(n))\)（\(n\in\mathbf{N}^{*}\)）. 若向量 \(\bs{a}_{n}=\overrightarrow{A_{0}A_{1}}+\overrightarrow{A_{1}A_{2}}+\cdots+\overrightarrow{A_{n-1}A_{n}}\)，\(\theta_{n}\) 是 \(\bs{a}_{n}\) 与 \(\bs{i}\) 的夹角（其中 \(\bs{i}=(1,0)\)），设 \(S_{n}=\tan\theta_{1}+\tan\theta_{2}+\cdots+\tan\theta_{n}\)，则 \(\lim\limits_{n\to\infty}S_{n}=\)\fillinblank{}.
\end{problem}
