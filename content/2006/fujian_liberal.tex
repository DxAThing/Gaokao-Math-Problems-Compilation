...
\begin{solution}
\begin{enumerate}
\item 函数 \(f(x)\) 的最小正周期为 \(\pi\)，单调增区间为 \(\left(k\pi-\frac{\pi}{6},k\pi+\frac{\pi}{3}\right)\)，其中 \(k\in\Z\).
\item 将 \(y=\sin 2x\) 的图象向右平移 \(\frac{\pi}{12}\) 个单位长度，再向上平移 \(\frac{1}{2}\) 个单位长度.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(AO\perp\text{平面 }BCD\).
\item \(\arccos\frac{\sqrt{2}}{4}\).
\item \(\frac{\sqrt{21}}{7}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(\left(x+\frac{1}{2}\right)^2+\left(y\pm\sqrt{2}\right)^2=\frac{9}{4}\).
\item \(x-2y+1=0\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(\{a_{n+1}-a_n\}\) 是以 \(2\) 为首项、\(2\) 为公比的等比数列.
\item \(a_n=2^n-1\).
\item \(\{b_n\}\) 是等差数列.
\end{enumerate}
\end{solution}
