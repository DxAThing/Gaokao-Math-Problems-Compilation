...
\bitmapfigure[max width=0.84\linewidth,max totalheight=6.8cm]{img/2007/shaanxi_science/q8_options.png}
\end{problem}

\begin{problem}
\begin{circlelist}
\item 四个非零实数 \(a, b, c, d\) 依次成等比数列的充要条件是 \(ad = bc\)；
\item 设 \(a, b \in \R\)，且 \(ab \neq 0\). 若 \(\frac{a}{b} < 1\)，则 \(\frac{a}{b} > 1\)；
...
\begin{problem}
已知实数 \(x, y\) 满足条件 \(\left\{ \begin{array}{l} x - 2y + 4 \geqslant 0, \\ 2x + y - 2 \geqslant 0, \\ 3x - y - 3 \leqslant 0, \end{array} \right.\) 则 \(z = x + 2y\) 的最大值为 \fillinblank{}.
\end{problem}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=4.8cm]{img/2007/shaanxi_science/q15_fig1.png}
\end{problem}
...
\begin{problem}
设函数 \( f(x) = a \cdot b \)，其中向量 \( a = (m, \cos 2x) \)，\( b = (1 + \sin 2x, 1) \)，\( x \in \R \)，且函数 \( y = f(x) \) 的图象经过点 \( \left(\frac{\pi}{4}, 2\right) \).
\begin{enumerate}
\item 求实数 \(m\) 的值；
...
\bitmapfigure[max width=0.32\linewidth,max totalheight=4.8cm]{img/2007/shaanxi_science/q19_fig1.png}
...
\begin{problem}
已知各项全不为零的数列 \(\{\alpha_{n}\}\) 的前 \(k\) 项和为 \(S_{k}\)，且 \(S_{k} = \frac{1}{2} a_{k} a_{k + 1} (k \in \mathbf{N}^{*})\)，其中 \(a_{1} = 1\).
\begin{enumerate}
\item 求数列 \( \{\alpha_{k}\} \) 的通项公式；
