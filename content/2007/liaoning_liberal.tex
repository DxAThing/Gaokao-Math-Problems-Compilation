...
\(A\cap N=\{1,3\}\cap\{2,3,4\}=\{3\}\). 因此若按题面数值的通常命题意图，将交集处的 \(B\) 视为前面定义的第二个集合，选择 C. 严格按字面看，\(B\) 未定义，原题存在符号歧义.
\end{solution}
...
\(\frac{y}{x}\geqslant1+\frac{2}{5/2}=\frac{9}{5}\)，且 \(\frac{y}{x}\leqslant\frac{7}{1}-1=6\).
...
\begin{solution}
函数定义域由 \(x^2-5x+6=(x-2)(x-3)>0\) 给出，即 \(x<2\) 或 \(x>3\). 设 \(F(x)=\log_{\frac{1}{2}}(x^2-5x+6)\)，则
\[
...
\begin{problem}
\(\left(\sqrt{x}+\frac{1}{\sqrt[4]{x}}\right)^8\) 展开式中含有 \(x\) 的整数次幂的项的系数之和为\fillinblank{}.
\end{problem}
...
\(\mathrm C_8^k\left(\sqrt{x}\right)^{8-k}\left(\frac{1}{\sqrt[4]{x}}\right)^k =\mathrm C_8^k x^{4-\frac{3k}{4}}\)，\(k=0\)，\(1\)，\(\cdots\)，\(8\)
...
\(x+\frac{25}{3}=10\)，所以 \(x=\frac{5}{3}\).
代入椭圆方程，得
\[
\frac{(5/3)^2}{25}+\frac{y^2}{16}=1
\]
即
\(\frac{1}{9}+\frac{y^2}{16}=1\)，\(\Longrightarrow\)，\(y^2=\frac{128}{9}\)
由向量关系，
\[
...
\begin{problem}
某公司在过去几年内使用某种型号的灯管 \(1000\text{ 支}\)，该公司对这些灯管的使用寿命（单位：小时）进行了统计，统计结果如下表所示：
...
\begin{enumerate}
\item 将各组的频率填入表中；
\item 根据上述统计结果，计算灯管使用寿命不足 \(1500\text{ 小时}\) 的频率；
\item 该公司某办公室新安装了这种型号的灯管 \(3\text{ 支}\)，若将上述频率作为概率，试求至少有 \(2\text{ 支}\) 灯管的寿命不足 \(1500\text{ 小时}\) 的概率.
\end{enumerate}
\end{problem}
...
\item 使用寿命不足 \(1500\text{ 小时}\) 包括前四组，因此其频率为
\[
\frac{48+121+208+223}{1000}=\frac{600}{1000}=0.6
\]
\item 设 \(X\) 表示 \(3\text{ 支}\) 灯管中寿命不足 \(1500\text{ 小时}\) 的支数，则 \(X\) 服从参数为 \(3\)，\(0.6\) 的二项分布. 于是
\[
...
\(D=\left(\frac{a}{2},\frac{a}{2},0\right)\)，\(E=\left(0,\frac{a}{2},0\right)\)，
\(C_1=(0,0,h)\)，\(A_1=(a,0,h)\)，\(B_1=(0,a,h)\)，\(M=(a,0,m)\).
...
\(\omega x-\frac{\pi}{6}=k\pi\)，\(x=\frac{k\pi+\pi/6}{\omega}\)，\(k\in\Z\)
...
\(\frac{u^4}{4}+u^2=\frac{v^4}{4}+v^2\)，\(\Longrightarrow\)，\((u^2-v^2)(u^2+v^2+4)=0\)
第二个因式不可能为零，故 \(v^2=u^2\). 由于 \(A\neq B\)，有 \(v=-u\). 又因 \(O\)，\(A\)，\(B\) 是正三角形的三个不同顶点，所以 \(u\neq0\). 再由 \(OA=AB\)，得
\(\frac{u^4}{4}+u^2=(2u)^2\)，\(\Longrightarrow\)，\(u^2\left(\frac{u^2}{4}-3\right)=0\)
舍去退化情形 \(u=0\)，得 \(u^2=12\)，所以可以取
\[
A=(6,2\sqrt{3})
\]
且
...
\cos\beta=\frac{9\cos\alpha-1}{12}
\]
反过来，若这些关系成立，则 \(r\leqslant0\)，从而 \(g(x)=2r(x-2)\) 在 \([0,2]\) 上非负、在 \([2,4]\) 上非正，故它们也充分. 因此这些条件不能唯一确定 \(\alpha\)，\(\beta\)，也不能唯一确定 \(f(x)\). 例如：
取 \(\cos\alpha=\frac{1}{9}\)，\(\cos\beta=0\) 时，\(g(x)=0\)，且
\[
f(x)=18\left(1-\frac{1}{81}\right)=\frac{160}{9}
\]
而
另取 \(\cos\alpha=\frac{1}{3}\)，\(\cos\beta=\frac{1}{6}\) 时，\(g(x)=-4x+8\)，同样满足题设两个符号条件，但
\[
f(x)=-2x^2+8x+16
\]
...
取第一组参数 \(u=\frac{1}{9}\)，即 \(f(x)=\frac{160}{9}\). 当 \(x\leqslant0\) 时，要求
\[
\frac{160}{9}\geqslant x^2-6x-11
\]
给出 \(3-\frac{2\sqrt{85}}{3}\leqslant x\leqslant0\)；当 \(x\geqslant0\) 时，要求
\[
\frac{160}{9}\geqslant x^2+26x-11
\]
给出 \(0\leqslant x\leqslant\frac{2\sqrt{445}-39}{3}\). 所以此时
\[
x\in\left[3-\frac{2\sqrt{85}}{3},\frac{2\sqrt{445}-39}{3}\right]
\]

取第二组参数 \(u=\frac{1}{3}\)，即 \(f(x)=-2x^2+8x+16\). 当 \(x\leqslant0\) 时，要求
\[
-2x^2+8x+16\geqslant x^2-6x-11
\]
给出 \(\frac{7-\sqrt{130}}{3}\leqslant x\leqslant0\)；当 \(x\geqslant0\) 时，要求
\[
-2x^2+8x+16\geqslant x^2+26x-11
\]
给出 \(0\leqslant x\leqslant3(\sqrt{2}-1)\). 所以此时
\[
