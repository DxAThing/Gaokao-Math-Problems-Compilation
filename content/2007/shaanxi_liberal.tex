\chapter{2007年陕西卷（文）}

\section{单选题}

\begin{problem}
已知全集 \(U = \{1, 2, 3, 4, 5, 6\}\)，集合 \(A = \{2, 3, 6\}\)，则集合 \(\complement_U A\) 等于（\quad）

\choices
    {\(\{1, 4\}\)}
    {\(\{4, 5\}\)}
    {\(\{1, 4, 5\}\)}
    {\(\{2, 3, 6\}\)}

\end{problem}

\begin{problem}
函数 \(f(x) = \lg \sqrt{1 - x^2}\) 的定义域为（\quad）

\choices
    {\(\{0, 1\}\)}
    {\((-1, 1)\)}
    {\([-1, 1]\)}
    {\((-\infty, -1) \cup (1, +\infty)\)}

\end{problem}

\begin{problem}
抛物线 \(x^2 = y\) 的准线方程是（\quad）

\choices
    {\(4x + 1 = 0\)}
    {\(4y + 1 = 0\)}
    {\(2x + 1 = 0\)}
    {\(2y + 1 = 0\)}

\end{problem}

\begin{problem}
已知 \(\sin \alpha = \frac{\sqrt{5}}{5}\)，则 \(\sin^4 \alpha - \cos^4 \alpha\) 的值为（\quad）

\choices
    {\(-\frac{3}{5}\)}
    {\(-\frac{1}{5}\)}
    {\(\frac{1}{5}\)}
    {\(\frac{3}{5}\)}

\end{problem}

\begin{problem}
等差数列 \(\{a_n\}\) 的前 \(n\) 项和为 \(S_n\)，若 \(S_2 = 2\)，\(S_4 = 10\)，则 \(S_6\) 等于（\quad）

\choices
    {12}
    {18}
    {24}
    {42}

\end{problem}

\begin{problem}
某商场有四类食品，其中粮食类、植物油类、动物性食品类以及果蔬类分别有 40 种、10 种、30 种、20 种，现从中抽取一个容量为 20 的样本进行食品安全检测. 若采用分层抽样的方法抽取样本，则抽取的植物油类与果蔬类食品种数之和是（\quad）

\choices
    {4}
    {5}
    {6}
    {7}

\end{problem}

\begin{problem}
Rt\(\triangle ABC\) 的三个顶点在半径为 13 的球面上，直角边的长分别为 6 和 8，则球心到平面 \(ABC\) 的距离是（\quad）

\choices
    {5}
    {6}
    {10}
    {12}

\end{problem}

\begin{problem}
设函数 \(f(x) = 2^x + 1\)（\(x \in \R\)）的反函数为 \(f^{-1}(x)\)，则函数 \(y = f^{-1}(x)\) 的图象是（\quad）

\choices
    {}
    {}
    {}
    {}

\end{problem}

\begin{problem}
已知双曲线 \(C: \frac{x^2}{a^2} - \frac{y^2}{b^2} = 1\)（\(a > 0\)，\(b > 0\)），以 \(C\) 的右焦点为圆心且与 \(C\) 的渐近线相切的圆的半径是（\quad）

\choices
    {\(a\)}
    {b}
    {\(\sqrt{ab}\)}
    {\(\sqrt{a^2 + b^2}\)}

\end{problem}

\begin{problem}
已知 \(P\) 为平面 \(\alpha\) 外一点，直线 \(l \subset \alpha\)，点 \(Q \in l\)，记点 \(P\) 到平面 \(\alpha\) 的距离为 \(a\)，点 \(P\) 到直线 \(l\) 的距离为 \(b\)，点 \(P\)、\(Q\) 之间的距离为 \(c\)，则（\quad）

\choices
    {\(a \leqslant b < c\)}
    {\(c \leqslant a \leqslant b\)}
    {\(a \leqslant c \leqslant b\)}
    {\(b \leqslant c \leqslant a\)}

\end{problem}

\begin{problem}
给出如下三个命题: \circled{1} 设 \(a, b \in \R\)，且 \(ab \neq 0\). 若 \(\frac{b}{a} > 1\)，则 \(\frac{a}{b} < 1\); \circled{2} 四个非零实数 \(a, b, c, d\) 依次成等比数列的充要条件是 \(ad = bc\); \circled{3} 若 \(f(x) = \log_2 x\)，则 \(f(|x|)\) 是偶函数. 其中正确命题的序号是（\quad）

\choices
    {\circled{1}\circled{2}}
    {\circled{2}\circled{3}}
    {\circled{1}\circled{3}}
    {\circled{1}\circled{2}\circled{3}}

\end{problem}

\begin{problem}
某生物生长过程中，在三个连续时段的增长量都相等，在各时段内平均增长速度分别为 \(v_1, v_2, v_3\)，该生物在所讨论的整个时段内的平均增长速度为（\quad）

\choices
    {\(\frac{v_1 + v_2 + v_3}{3}\)}
    {\(\frac{3}{\frac{1}{v_1} + \frac{1}{v_2} + \frac{1}{v_3}}\)}
    {\(\sqrt[3]{v_1v_2v_3}\)}
    {\(\frac{3}{\frac{1}{v_1} + \frac{1}{v_2} + \frac{1}{v_3}}\)}

\end{problem}

\section{填空题}

\begin{problem}
\((1 + 2x)^9\) 的展开式中 \(x^2\) 项的系数是 \fillinblank{}. （用数字作答）

\end{problem}

\begin{problem}
已知实数 \(x, y\) 满足条件 \(\left\{ \begin{array}{l} x - 2y + 4 \geqslant 0, \\ 3x - y - 3 \leqslant 0, \\ x \geqslant 0, y \geqslant 0, \end{array} \right.\)，则 \(z = x + 2y\) 的最大值为 \fillinblank{}.

\end{problem}

\begin{problem}
安排 3 名支教教师去 4 所学校任教，每校至多 2 人，则不同的分配方案共有 \fillinblank{} 种. （用数字作答）

\end{problem}

\begin{problem}
如图，平面内有三个向量 \(\overrightarrow{OA}, \overrightarrow{OB}, \overrightarrow{OC}\)，其中 \(\overrightarrow{OA}\) 与 \(\overrightarrow{OB}\) 的夹角为 \(120^\circ\)，\(\overrightarrow{OA}\) 与 \(\overrightarrow{OC}\) 的夹角为 \(30^\circ\)，且 \(\left| \overrightarrow{OA} \right| = \left| \overrightarrow{OB} \right| = 1\)，\(\left| \overrightarrow{OC} \right| = 2\sqrt{3}\). 若 \(\overrightarrow{OC} = \lambda \overrightarrow{OA} + \mu \overrightarrow{OB}\)（\(\lambda, \mu \in \R\)），则 \(\lambda + \mu\) 的值为 \fillinblank{}.

\end{problem}

\section{解答题}

\begin{problem}
设函数 \(f(x) = a \cdot b\)，其中向量 \(a = (m, \cos x)\)，\(b = (1, 1 + \sin x)\)，\(x \in \R\)，且 \(f\left( \frac{\pi}{2} \right) = 2\).

（1）求实数 \(m\) 的值；

（2）求函数 \(f(x)\) 的最小值。

\end{problem}

\begin{problem}
某项选拔共有四轮考核，每轮设有一个问题，能正确回答问题者进入下一轮考核，否则即被淘汰. 已知某选手能正确回答第一、二、三、四轮的问题的概率分别为 \(\frac{1}{3}\)，\(\frac{1}{3}\)，\(\frac{2}{3}\)，\(\frac{1}{3}\)，且各轮问题能否正确回答互不影响.

（1）求该选手进入第四轮才被淘汰的概率；

（2）求该选手至多进入第三轮考核的概率。

\end{problem}

\begin{problem}
如图，在底面为直角梯形的四棱锥 \(P - ABCD\) 中，\(AD // BC\)，\(\angle ABC = 90^\circ\)，\(PA \perp\) 平面 \(ABCD\)，\(PA = 3\)，\(AD = 2\)，\(AB = 2\sqrt{3}\)，\(BC = 6\).

（1）求证: \(BD \perp\) 平面 \(PAC\)；

（2）求二面角 \(P - BD - A\) 的大小。

\end{problem}

\begin{problem}
已知实数列 \(\{a_n\}\) 是等比数列，其中 \(a_7 = 1\)，且 \(a_4, a_5 + 1, a_6\) 成等差数列.

（1）求数列 \(\{a_n\}\) 的通项公式；

（2）数列 \(\{a_n\}\) 的前 \(n\) 项和记为 \(S_n\)，证明: \(S_n < 128\)（\(n = 1, 2, 3, \cdots\)）。

\end{problem}

\begin{problem}
已知 \(f(x) = ax^3 + bx^2 + cx\) 在区间 \([0, 1]\) 上是增函数，在区间 \((-\infty, 0)\)、\((1, +\infty)\) 上是减函数，又 \(f\left( \frac{1}{2} \right) = \frac{3}{2}\).

（1）求 \(f(x)\) 的解析式；

（2）若在区间 \([0, m]\)（\(m > 0\)）上恒有 \(f(x) \leqslant x\) 成立，求 \(m\) 的取值范围。

\end{problem}

\begin{problem}
已知椭圆 \(C: \frac{x^2}{a^2} + \frac{y^2}{b^2} = 1\)（\(a > b > 0\)）的离心率为 \(\frac{\sqrt{6}}{3}\)，短轴一个端点到右焦点的距离为 \(\sqrt{3}\).

（1）求椭圆 \(C\) 的方程；

（2）设直线 \(l\) 与椭圆 \(C\) 交于 \(A, B\) 两点. 坐标原点 \(O\) 到直线 \(l\) 的距离为 \(\frac{\sqrt{3}}{2}\)，求 \(\triangle AOB\) 面积的最大值。
\end{problem}

