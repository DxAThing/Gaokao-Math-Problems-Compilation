...
从而 \(\overrightarrow{BD}=\left(2,2\sqrt{3},0\right)\)，\(\overrightarrow{AP}=\left(0,0,3\right)\)，\(\overrightarrow{AC}=\left(6,-2\sqrt{3},0\right)\). 于是 \(\overrightarrow{BD}\cdot\overrightarrow{AP}=0\)，且 \(\overrightarrow{BD}\cdot\overrightarrow{AC}=12-12=0\). 直线 \(AP\) 与 \(AC\) 是平面 \(PAC\) 内相交的两条直线，所以 \(BD\perp\) 平面 \(PAC\).
...
\begin{solution}
\begin{enumerate}
\item 函数在 \(0\) 的左侧递减、右侧递增，且 \(f'\) 连续，因此 \(f'(0)=0\)；同理，函数在 \(1\) 的左侧递增、右侧递减，因此 \(f'(1)=0\). 又 \(f'(x)=3ax^2+2bx+c\)，所以 \(f'(x)=k x\left(x-1\right)\). 由 \(f'\left(\frac12\right)=\frac32\)，得 \(-\frac{k}{4}=\frac32\)，从而 \(k=-6\). 比较系数可得 \(3a=-6\)，\(2b=6\)，\(c=0\)，故 \(a=-2\)，\(b=3\)，\(c=0\)，即 \(f(x)=-2x^3+3x^2\).
...
\]
其中用到了 \(K\left(\frac{\cos^2\theta}{3}+\sin^2\theta\right)-\frac49\sin^2\theta\cos^2\theta=\frac13\). 因为 \(\bs{v}\) 是单位向量，两个根之差就是弦 \(AB\) 的长度，所以 \(|AB|=\frac{\sqrt{\Delta}}{K}=\frac{\sqrt{4K-1}}{K}\). 又 \(K=\frac13+\frac23\cos^2\theta\)，故 \(\frac13\leqslant K\leqslant1\).
