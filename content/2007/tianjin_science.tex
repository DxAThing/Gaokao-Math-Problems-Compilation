...
\begin{problem}
设变量 \(x, y\) 满足约束条件 \(\left\{\begin{aligned} x - y &\geqslant -1,\\ x + y &\geqslant 1,\\ 3x - y &\leqslant 3 \end{aligned}\right.\)，则目标函数 \(z = 4x + y\) 的最大值为（\quad）
...
\begin{problem}
设等差数列 \(\{a_{n}\}\) 的公差 \(d\) 是 2，前 \(n\) 项的和为 \(S_{n}\)，则 \(\lim_{n\to \infty}\dfrac{a_n^2 - n^2}{S_n} =\)\fillinblank{}.
\end{problem}
...
\begin{center}
\bitmapinclude[max width=4.6cm,max totalheight=2.4cm]{img/2007/tianjin_science/q15_fig1.png}
\end{center}
\end{problem}
...
\begin{center}
\bitmapinclude[max width=6.0cm,max totalheight=1.6cm]{img/2007/tianjin_science/q16_fig1.png}
\end{center}
\end{problem}
...
\begin{center}
\bitmapinclude[max width=5.2cm,max totalheight=5.0cm]{img/2007/tianjin_science/q19_fig1.png}
\end{center}
\end{problem}
