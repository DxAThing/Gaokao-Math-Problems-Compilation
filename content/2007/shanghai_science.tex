\FigureLayoutDeclare{img/2007/shanghai_science/q11_fig1.png}{15.0cm}{18bp 49bp 31bp 5bp}

\chapter{2007年上海卷（秋理）}
\section{填空题}
\begin{problem}
函数 \( f(x)=\frac{\lg(4-x)}{x-3} \) 的定义域是\fillinblank{}.
\end{problem}
...
\begin{problem}
方程 \( 9^x - 6 \cdot 3^x - 7 = 0 \) 的解是\fillinblank{}.
\end{problem}
...
\begin{problem}
若 \(x\)，\(y \in \R^+\)，且 \(x + 4y = 1\)，则 \(x \cdot y\) 的最大值是\fillinblank{}.
\end{problem}
...
\begin{problem}
函数 \( y = \sin\left(x + \frac{\pi}{3}\right)\sin\left(x + \frac{\pi}{2}\right) \) 的最小正周期 \(T =\)\fillinblank{}.
\end{problem}
...
\begin{problem}
以双曲线 \(\frac{x^2}{4} - \frac{y^2}{5} = 1\) 的中心为焦点，且以该双曲线的左焦点为顶点的抛物线方程是\fillinblank{}.
\end{problem}
...
那么，对于非零复数 \(a\)，\(b\)，仍然成立命题的所有序号是\fillinblank{}.
\end{problem}
...
\begin{solution}
命题 \(\circled{1}\) 在复数中不一定成立，例如取 \(a=\i\)，则 \(a+\frac1a=\i-\i=0\). 命题 \(\circled{2}\) 只是复数的乘法分配律展开式，对任意复数均成立.

命题 \(\circled{3}\) 在复数中不一定成立，例如 \(a=1\)，\(b=\i\) 时，\(|a|=|b|=1\)，但 \(a\ne b\) 且 \(a\ne-b\). 命题 \(\circled{4}\) 中由 \(a^2=ab\) 得 \(a(a-b)=0\)，因 \(a\ne0\)，所以 \(a=b\). 因此仍成立的命题序号为 \(\circled{2}\circled{4}\).
\end{solution}
...
\begin{problem}
已知 \(P\) 为圆 \(x^{2} + (y - 1)^{2} = 1\) 上任意一点（原点 \(O\) 除外），直线 \(OP\) 的倾斜角为 \(\theta\) 弧度，记 \(d = |OP|\). 在右侧的坐标系中，画出以 \((\theta, d)\) 为坐标的点的轨迹的大致图形为\fillinblank{}.

\bitmapfigure[max width=0.75\linewidth]{img/2007/shanghai_science/q11_fig1.png}
\end{problem}
...
\section{单选题}
\begin{problem}
已知 \(a\)，\(b \in \R\)，且 \(2 + a\i\)，\(b + \i\)（\(\i\) 是虚数单位）是实系数一元二次方程 \(x^{2} + px + q = 0\) 的两个根，那么 \(p\)，\(q\) 的值分别是（\quad）
...
\begin{solution}
记命题 \(P(k)\) 为 \(f(k)\geqslant k^2\). 题设给出对每个正整数 \(k\)，\(P(k)\Rightarrow P(k+1)\).
...
\bitmapfigure[max width=0.38\linewidth]{img/2007/shanghai_science/q16_fig1.png}
\end{problem}

\begin{solution}
设 \(C\) 为原点，取 \(CA\)，\(CB\)，\(CC_1\) 分别为三个坐标轴正方向. 由底面为直角等腰三角形，底面积为 \(\frac12\)；又棱柱体积为 \(1\)，所以 \(CC_1=2\). 因而可设 \(A=(1,0,0)\)，\(B=(0,1,0)\)，\(A_1=(1,0,2)\).

平面 \(BB_1C_1C\) 的方程为 \(x=0\). 直线 \(A_1B\) 的方向向量可取 \(\bs{v}=(-1,1,-2)\)，其长度为 \(\sqrt6\)，在该平面上的投影方向向量为 \((0,1,-2)\)，长度为 \(\sqrt5\). 设直线与平面所成角为 \(\alpha\)，则 \(\tan\alpha\) 等于方向向量的法向分量与平面内投影分量之比，即 \(\tan\alpha=\frac{1}{\sqrt5}=\frac{\sqrt5}{5}\). 所以 \(\alpha=\arctan\frac{\sqrt5}{5}\).
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 2003，2004，2005，2006 年的年生产量增长率依次为 \(36\%\)，\(38\%\)，\(40\%\)，\(42\%\). 所以 \(2006\) 年的年生产量为 \(670\times1.36\times1.38\times1.40\times1.42=2499.822528\text{ 兆瓦}\)，精确到 \(0.1\text{ 兆瓦}\) 为 \(2499.8\text{ 兆瓦}\).
\item 设这四年中年安装量的平均增长率为 \(x\)，则 \(2010\) 年的安装量为 \(1420(1+x)^4\)，生产量为 \(2499.822528(1.42)^4\). 由安装量不少于生产量的 \(95\%\)，得 \(1420(1+x)^4\geqslant0.95\times2499.822528(1.42)^4\). 因 \(1+x>0\)，所以 \(1+x\geqslant\sqrt[4]{\frac{0.95\times2499.822528\times1.42^4}{1420}}\)，即 \(x\geqslant0.614821\ldots\). 因此至少应达到 \(61.5\%\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 函数定义域为全体非零实数，关于原点对称. 对任意 \(x\ne0\)，\(f(-x)=x^2-\frac{a}{x}\). 当 \(a=0\) 时，\(f(-x)=f(x)=x^2\)，所以函数为偶函数；当 \(a\ne0\) 时，\(f(-x)\ne f(x)\)，且 \(f(-x)=-f(x)\) 会导致 \(x^2-\frac{a}{x}=-x^2-\frac{a}{x}\)，即 \(2x^2=0\)，不可能对所有 \(x\ne0\) 成立. 所以此时函数既不是奇函数，也不是偶函数.
\item 在 \(x\geqslant2\) 上，\(f'(x)=2x-\frac{a}{x^2}=\frac{2x^3-a}{x^2}\). 因为 \(x^2>0\)，要使函数在该区间上为增函数，必须且只需 \(2x^3-a\geqslant0\) 对所有 \(x\geqslant2\) 成立. 左端随 \(x\) 增大而增大，最小值在 \(x=2\) 处，为 \(16-a\)，故 \(a\leqslant16\). 当 \(a>16\) 时，\(f'(2)<0\)，函数不可能在该区间上为增函数. 所以 \(a\in(-\infty,16]\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 设 \(b_1\)，\(b_2\)，\(b_3\)，\(b_4\) 的公差为 \(d\). 则 \(2+3d=11\)，所以 \(d=3\). 由对称性 \(b_5=b_3\)，\(b_6=b_2\)，\(b_7=b_1\)，故 \(\{b_n\}=\{2,5,8,11,8,5,2\}\).
\item 对 \(j=0\)，\(1\)，\(\ldots\)，\(k-1\)，有 \(c_{k+j}=50-4j\). 由对称性，中心项只计算一次，其余各项成对出现，因此 \(S_{2k-1}=50+2\sum_{j=1}^{k-1}(50-4j)=-4k^2+104k-50=626-4(k-13)^2\). 因 \(k>1\) 为整数，故在 \(k=13\) 时取得最大值 \(626\).
\item 记连续出现的 \(m\) 项为 \(1\)，\(2\)，\(\ldots\)，\(2^{m-1}\). 对称性要求这段连续项的逆序列也出现在原数列中. 由于 \(1\)，\(2\)，\(\ldots\)，\(2^{m-1}\) 两两不同，这两段长度为 \(m\) 的连续项至多重合一个位置；否则重合位置会要求两个不同的幂相等. 因此原数列的项数至少为 \(2m-1\).
...
\bitmapfigure[float=inline,max width=0.42\linewidth]{img/2007/shanghai_science/q21_fig1.png}
\end{problem}
\FloatBarrier

\begin{solution}
\begin{enumerate}
\item 右侧椭圆的焦半距为 \(\sqrt{a^2-b^2}=c\)，所以 \(F_0=(c,0)\). 左侧椭圆的焦半距为 \(d=\sqrt{b^2-c^2}\)，所以 \(F_1=(0,-d)\)，\(F_2=(0,d)\). 若三点构成边长为 \(1\) 的等边三角形，则 \(2d=1\)，且 \(c^2+d^2=1\). 因而 \(d=\frac12\)，\(c^2=\frac34\)，\(b^2=c^2+d^2=1\)，再由 \(a^2=b^2+c^2\) 得 \(a^2=\frac74\). 所以“果圆”方程为 \(\frac47x^2+y^2=1\ (x\geqslant0)\)，\(y^2+\frac43x^2=1\ (x\leqslant0)\).
\item \(A_1=(-c,0)\)，\(A_2=(a,0)\)，\(B_1=(0,-b)\)，\(B_2=(0,b)\)，所以 \(\lvert A_1A_2\rvert=a+c\)，\(\lvert B_1B_2\rvert=2b\). 令 \(t=\frac ba\)，则 \(t>\frac{\sqrt2}{2}\)，且 \(\frac ca=\sqrt{1-t^2}\). 条件化为 \(1+\sqrt{1-t^2}>2t\). 在 \(t>\frac{\sqrt2}{2}>\frac12\) 下可平方，得 \(1-t^2>(2t-1)^2\)，即 \(t(5t-4)<0\). 因 \(t>0\)，所以 \(\frac{\sqrt2}{2}<\frac ba<\frac45\).
\item 当 \(k=0\) 时，水平弦的两个端点分别为 \(\left(a\sqrt{1-\frac{y^2}{b^2}},y\right)\) 与 \(\left(-c\sqrt{1-\frac{y^2}{b^2}},y\right)\). 其中点 \(M=(x,y)\) 满足 \(x=\frac{a-c}{2}\sqrt{1-\frac{y^2}{b^2}}\)，即 \(\frac{4x^2}{(a-c)^2}+\frac{y^2}{b^2}=1\)，故这些中点均落在一个椭圆上.
