...
\begin{problem}
方程 \(3^{x-1} = \frac{1}{9}\) 的解是\fillinblank{}.
\end{problem}
...
\begin{problem}
函数 \(f(x)=\frac{1}{x-1}\) 的反函数 \(f^{-1}(x)=\)\fillinblank{}.
\end{problem}
...
\begin{problem}
函数 \(y = \sec x \cdot \cos \left( x + \frac{\pi}{2} \right)\) 的最小正周期 \(T =\)\fillinblank{}.
\end{problem}
...
\begin{problem}
以双曲线 \(\frac{x^2}{4} - \frac{y^2}{5} = 1\) 的中心为顶点，且以该双曲线的右焦点为焦点的抛物线方程是\fillinblank{}.
\end{problem}
...
由数量积公式，
\[
\bs{a}\cdot(\bs{a}-\bs{b})=\bs{a}\cdot\bs{a}-\bs{a}\cdot\bs{b}=|\bs{a}|^2-|\bs{a}|\,|\bs{b}|\cos60^{\circ}=1-\frac{1}{2}=\frac{1}{2}.
\]
\end{solution}

\begin{problem}
如图，在直三棱柱 \(ABC - A_{1}B_{1}C_{1}\) 中，\(\angle ACB = 90^{\circ}\)，\(AA_{1} = 2\)，\(AC = BC = 1\)，则异面直线 \(A_{1}B\) 与 \(AC\) 所成角的大小是\fillinblank{}.（结果用反三角函数值表示）

\bitmapfigure[max width=0.30\linewidth]{img/2007/shanghai_liberal/q7_fig1.png}
\end{problem}
...
\begin{solution}
取 \(C\) 为原点，令 \(A=(1,0,0)\)，\(B=(0,1,0)\)，则可取 \(A_{1}=(1,0,2)\). 因而
\[
\overrightarrow{A_{1}B}=(-1,1,-2),\qquad \overrightarrow{AC}=(-1,0,0).
\]
设异面直线 \(A_{1}B\) 与 \(AC\) 所成的锐角为 \(\theta\)，则
\[
...
\begin{problem}
某工程由 \(A\)，\(B\)，\(C\)，\(D\) 四道工序组成，完成它们需用时间依次为 \(2\text{ 天}\)，\(5\text{ 天}\)，\(x\text{ 天}\)，\(4\text{ 天}\). 四道工序的先后顺序及相互关系是： \(A\)，\(B\) 可以同时开工； \(A\) 完成后，\(C\) 可以开工； \(BC\) 完成后，\(D\) 可以开工. 若该工程总时数为 \(9\) 天，则完成工序 \(C\) 需要的天数 \(x\) 最大是\fillinblank{}.
\end{problem}
...
那么，对于非零复数 \(a\)，\(b\)，仍然成立的命题的所有序号是\fillinblank{}.
\end{problem}
...
命题 \(2\) 是复数的乘法分配律和交换律的直接推论，对任意复数 \(a,b\) 都成立.
...
\begin{problem}
如图，\(AB\) 是直线 \(l\) 上的两点，且 \(AB = 2\). 两个半径相等的动圆分别与 \(l\) 相切于 \(A\)，\(B\) 点，\(C\) 是这两个圆的公共点，则圆弧 \(AC\)，\(CB\) 与线段 \(AB\) 围成图形面积 \(S\) 的取值范围是\fillinblank{}.

\bitmapfigure[max width=0.52\linewidth]{img/2007/shanghai_liberal/q11_fig1.png}
\end{problem}
...
\begin{solution}
设两个圆的半径为 \(r\). 取 \(A=(0,0)\)，\(B=(2,0)\)，则两个圆心可取为 \(O_{1}=(0,r)\)、\(O_{2}=(2,r)\). 两圆有公共点的必要条件为 \(r\geqslant1\). 记 \(h=\sqrt{r^2-1}\)，则图中位于两圆之间的公共点为 \(C=(1,r-h)\). 令
\[
\alpha=\arcsin\frac{1}{r},\qquad 0<\alpha\leqslant\frac{\pi}{2}.
\]
在等腰三角形 \(O_{1}AC\) 中，圆心角 \(\angle AO_{1}C=\alpha\). 由 \(O_{1}A=(0,-r)\)、\(O_{1}C=(1,-h)\) 可得 \(\cos\alpha=\frac{h}{r}\)，从而 \(\sin\alpha=\frac{1}{r}\). 由对称性，所围图形的面积等于三角形 \(ABC\) 的面积减去两个相同的弓形面积. 其中
\[
...
\]
其中不等号由 \(\arcsin u>u\)（\(0<u<1\)）得到. 所以 \(S\) 在 \([1,+\infty)\) 上递减. 当 \(r\to+\infty\) 时，公共点 \(C\) 的纵坐标 \(r-\sqrt{r^2-1}\) 趋于 \(0\)，从而 \(S\to0\). 因此
\[
...
\begin{problem}
已知 \(a\in\R\)，\(b\in\R\)，且 \(2 + a\i\)，\(b + 3\i\)（\(\i\) 是虚数单位）是一个实系数一元二次方程的两个根，那么 \(a\)，\(b\) 的值分别是（\quad）
...
(x-1)^2+y^2=2,
\]
所以圆心为 \(O=(1,0)\)，半径为 \(\sqrt{2}\). 设 \(O\) 关于直线 \(2x-y+3=0\) 的对称点为 \(O'=(x',y')\). 由点关于直线的对称公式，
\[
x'=1-\frac{2\cdot2(2\cdot1-0+3)}{2^2+(-1)^2}=-3,
\qquad
y'=0-\frac{2\cdot(-1)(2\cdot1-0+3)}{2^2+(-1)^2}=2.
\]
对称后的圆半径不变，故方程为
\[
(x+3)^2+(y-2)^2=2.
...
\begin{solution}
记命题 \(P_k\) 为 \(f(k)\geqslant k^2\). 题设条件是对每个正整数 \(k\)，均有 \(P_k\Rightarrow P_{k+1}\).
...
其余选项均不能由题设条件推出：题设只说明不等式成立时可以向后递推，不能由某一项不成立反向推出前一项，也不能由某一项成立反向推出更前面的各项成立.
\end{solution}
...
\bitmapfigure[max width=0.36\linewidth]{img/2007/shanghai_liberal/q16_fig1.png}
\end{problem}

\begin{solution}
设正四棱锥的高为 \(PO\)，其中 \(O\) 是正方形 \(ABCD\) 的中心. 连接 \(AO\)，则 \(PO\perp\) 平面 \(ABCD\)，且 \(AO\) 是 \(PA\) 在底面上的射影，所以 \(\angle PAO=60^{\circ}\). 在直角三角形 \(PAO\) 中，
\[
PO=PA\sin60^{\circ}=2\cdot\frac{\sqrt{3}}{2}=\sqrt{3},
\qquad
AO=PA\cos60^{\circ}=2\cdot\frac{1}{2}=1.
\]
设正方形边长为 \(s\)，则 \(AO\) 是其对角线的一半，故 \(AO=\frac{\sqrt{2}}{2}s\)，从而 \(s=\sqrt{2}\). 底面积为 \(s^2=2\)，所以
\[
...
\begin{enumerate}
\item 求 \(2006\) 年全球太阳电池的年生产量（结果精确到 \(0.1\text{ 兆瓦}\)）；
\item 目前太阳电池产业存在的主要问题是市场安装量远小于生产量，\(2006\) 年的实际安装量为 \(1420\text{ 兆瓦}\). 假设以后若干年内太阳电池的年生产量的增长率保持在 \(42\%\)，到 \(2010\) 年，要使年安装量与年生产量基本持平（即年安装量不少于年生产量的 \(95\%\)），这四年中太阳电池的年安装量的平均增长率至少应达到多少（结果精确到 \(0.1\%\)）？
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 由题意，\(2003\)，\(2004\)，\(2005\)，\(2006\) 年的年生产量增长率依次为 \(36\%\)，\(38\%\)，\(40\%\)，\(42\%\). 设第 \(n\) 年的年生产量为 \(P_n\)，则
\[
P_{2006}=670(1+0.36)(1+0.38)(1+0.40)(1+0.42)=2499.822528\text{ 兆瓦}.
\]
所以 \(2006\) 年全球太阳电池的年生产量约为 \(2499.8\text{ 兆瓦}\).

\item 设这四年中太阳电池年安装量的平均增长率为 \(r\)，则 \(2006\) 年至 \(2010\) 年的年生产量为
\[
...
\begin{enumerate}
\item 当 \(a = 2\) 时，解不等式 \(f(x) - f(x - 1) > 2x - 1\)；
...
\item 设 \(\{c_n\}\) 是 \(49\) 项的“对称数列”，其中 \(c_{25}\)，\(c_{26}\)，\(\cdots\)，\(c_{49}\) 是首项为 \(1\)，公比为 \(2\) 的等比数列，求 \(\{c_n\}\) 各项的和 \(S\)；
\item 设 \(\{d_n\}\) 是 \(100\) 项的“对称数列”，其中 \(d_{51}\)，\(d_{52}\)，\(\cdots\)，\(d_{100}\) 是首项为 \(2\)，公差为 \(3\) 的等差数列. 求 \(\{d_n\}\) 前 \(n\) 项的和 \(S_n\)（\(n = 1\)，\(2\)，\(\cdots\)，\(100\)）.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 设 \(b_1,b_2,b_3,b_4\) 所成等差数列的公差为 \(d\). 由 \(b_1=2\)，\(b_4=11\)，得
\[
2+3d=11,\qquad d=3.
\]
所以 \(b_1,b_2,b_3,b_4=2,5,8,11\). 由对称性，\(b_5=b_3=8\)，\(b_6=b_2=5\)，\(b_7=b_1=2\)，故数列各项依次为
\[
2,5,8,11,8,5,2.
\]

\item 由题意，\(c_{25},c_{26},\ldots,c_{49}\) 为首项 \(1\)、公比 \(2\) 的等比数列，所以
\[
c_{25+k}=2^k\qquad(0\leqslant k\leqslant24).
\]
利用对称性，前 \(24\) 项分别与后 \(24\) 项相等，而中间项 \(c_{25}=1\) 只计算一次. 因此
\[
S=2\sum_{k=0}^{24}2^k-1=2\left(2^{25}-1\right)-1=2^{26}-3.
\]

\item 由题意，后 \(50\) 项 \(d_{51},d_{52},\ldots,d_{100}\) 是首项为 \(2\)、公差为 \(3\) 的等差数列，即
\[
d_k=2+3(k-51)\qquad(51\leqslant k\leqslant100).
\]
由对称性，对 \(1\leqslant i\leqslant50\)，
\[
...
\begin{enumerate}
\item 若 \(\triangle F_0F_1F_2\) 是边长为 \(1\) 的等边三角形，求该“果圆”的方程；
\item 设 \(P\) 是“果圆”的半椭圆 \(\frac{y^2}{b^2} + \frac{x^2}{c^2} = 1\)（\(x \leqslant 0\)）上任意一点. 求证：当 \(|PM|\) 取得最小值时，\(P\) 在点 \(B_1\)，\(B_2\) 或 \(A_1\) 处；
\item 若 \(P\) 是“果圆”上任意一点，求 \(|PM|\) 取得最小值时点 \(P\) 的横坐标.
\end{enumerate}

\bitmapfigure[max width=0.42\linewidth]{img/2007/shanghai_liberal/q21_fig1.png}
\end{problem}

\begin{solution}
记 \(d=\sqrt{b^2-c^2}\). 右侧椭圆的焦点为 \((\pm c,0)\)，左侧椭圆的焦点为 \((0,\pm d)\)，故可取
\[
F_0=(c,0),\qquad F_1=(0,-d),\qquad F_2=(0,d).
\]
又有
\[
A_1=(-c,0),\qquad A_2=(a,0),\qquad M=\left(\frac{a-c}{2},0\right).
\]

\begin{enumerate}
\item 若 \(\triangle F_0F_1F_2\) 为边长 \(1\) 的等边三角形，则
\[
2d=F_1F_2=1,\qquad c^2+d^2=F_0F_1^2=1.
\]
所以 \(d=\frac{1}{2}\)，\(c=\frac{\sqrt{3}}{2}\). 从 \(b^2=c^2+d^2\) 和 \(a^2=b^2+c^2\) 得
\[
b=1,\qquad a=\frac{\sqrt{7}}{2}.
\]
因此该“果圆”的方程为
\[
\begin{cases}
...
\item 设 \(P=(x,y)\) 在左侧半椭圆上，则 \(-c\leqslant x\leqslant0\)，且
\[
...
\item 先研究右侧半椭圆. 设 \(P=(x,y)\)，则 \(0\leqslant x\leqslant a\)，且
\[
...
当 \(a\geqslant2c\) 时，右侧半椭圆上的最小值在 \(x=a\) 处取得，对应点为 \(A_2\). 此时
\[
|MA_2|^2=\frac{(a+c)^2}{4},
\qquad
|MB_1|^2=|MB_2|^2=\frac{(a-c)^2}{4}+b^2,
\]
且
\[
