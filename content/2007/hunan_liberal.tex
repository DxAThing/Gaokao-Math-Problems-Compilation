...
\begin{enumerate}
\item \(T=\pi\).
\item 函数的单调递增区间为 \(\left[k\pi-\frac{\pi}{2},k\pi\right]\)（\(k\in\Z\)）.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(\overrightarrow{CA}\cdot\overrightarrow{CB}=-1\).
\item 点 \(M\) 的轨迹方程为 \(x^{2}-y^{2}=4\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(a_{n+2}-a_n=6\)（\(n\geqslant2\)）.
\item 取 \(a=3\)，则 \(b_n=a_{6\cdot7^{n-1}}\)（\(n\in\N^{*}\)）.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(a^2-4b\) 的最大值为 \(16\).
\item \(f(x)=\frac{1}{3}x^3-x^2-x\).
\end{enumerate}
\end{solution}
