...
\begin{problem}
如图，\( l_{1} \) 、 \( l_{2} \) 、 \( l_{3} \) 是同一平面内的三条平行直线，\( l_{1} \) 与 \( l_{2} \) 间的距离是 1，\( l_{2} \) 与 \( l_{3} \) 间的距离是 2，正三角形 \(\triangle ABC\) 的三顶点分别在 \( l_{1} \) 、 \( l_{2} \) 、 \( l_{3} \) 上，则 \( \triangle ABC \) 的边长是（\quad）
...
\begin{problem}
如图，在正三棱柱 \(ABC - A_{1}B_{1}C_{1}\) 中，侧棱长为 \(\sqrt{2}\)，底面三角形的边长为 1，

\bitmapfigure[max width=0.30\linewidth,max totalheight=4.0cm]{img/2007/sichuan_science/q14_fig1.png}

则 \(BC_{1}\) 与侧面 \(ACC_{1}A_{1}\) 所成的角是 \fillinblank{}.
\end{problem}
...
\begin{problem}
下面有五个命题：
\circled{1} 函数 \( y = \sin^4 x - \cos^4 x \) 的最小正周期是 \( \pi \)；
\circled{2} 终边在 \(y\) 轴上的角的集合是 \(\left\{\alpha \mid \alpha = \frac{k\pi}{2}, k \in \Z\right\}\)；
\circled{3} 在同一坐标系中，函数 \( y = \sin x \) 的图象和函数 \( y = x \) 的图象有三个公共点；
\circled{4} 把函数 \( y = 3\sin \left(2x + \frac{\pi}{3}\right) \) 的图象向右平移 \(\frac{\pi}{6}\) 得到 \( y = 3\sin 2x \) 的图象；
\circled{5} 函数 \( y = \sin \left(x - \frac{\pi}{2}\right) \) 在 \([0, \pi]\) 上是减函数.
...
\begin{enumerate}
\item 若 \(P\) 是该椭圆上的一个动点，求 \(PF_{1} \cdot PF_{2}\) 的最大值和最小值；
