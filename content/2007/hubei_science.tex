...
\begin{problem}
平面 \(\alpha\) 外有两条直线 \(m\) 和 \(n\)，如果 \(m\) 和 \(n\) 在平面 \(\alpha\) 内的射影分别是 \(m'\) 和 \(n'\)，给出下列四个命题：\circled{1} \(m' \perp n' \Rightarrow m \perp n\)；\circled{2} \(m \perp n \Rightarrow m' \perp n'\)；\circled{3} \(m'\) 与 \(n'\) 相交 \(\Rightarrow m\) 与 \(n\) 相交或重合；\circled{4} \(m'\) 与 \(n'\) 平行 \(\Rightarrow m\) 与 \(n\) 平行或重合. 其中不正确的命题个数是（\quad）
...
\begin{problem}
已知 \(p\) 和 \(q\) 是两个不相等的正整数，且 \(q \geqslant 2\)，则 \(\lim_{n\to\infty} \frac{\left(1 + \frac{1}{n}\right)^p - 1}{\left(1 + \frac{1}{n}\right)^q - 1}\) 等于（\quad）
...
\begin{problem}
设变量 \(x, y\) 满足约束条件 \(\left\{ \begin{array}{l} x - y + 3 \geqslant 0, \\ x + y \geqslant 0, \\ -2 \leqslant x \leqslant 3, \end{array} \right.\) 则目标函数 \(2x + y\) 的最小值为 \fillinblank{}.
\end{problem}
...
\bitmapfigure[max width=0.70\linewidth,max totalheight=4.8cm]{img/2007/hubei_science/q15_fig1.png}
...
\bitmapfigure[max width=0.55\linewidth,max totalheight=4.8cm]{img/2007/hubei_science/q17_fig1.png}
...
\bitmapfigure[max width=0.40\linewidth,max totalheight=6.0cm]{img/2007/hubei_science/q18_fig1.png}
...
\bitmapfigure[max width=0.60\linewidth,max totalheight=5.0cm]{img/2007/hubei_science/q19_fig1.png}
