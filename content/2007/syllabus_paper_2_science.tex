...
\begin{problem}
设复数 \(z\) 满足 \(\frac{1 + 2\i}{1 - \i} = \i\)，则 \(z =\)（\quad）
...
\begin{problem}
把函数 \( y = \e^{x} \) 的图象按向量 \( \alpha = (2,3) \) 平移，得到 \( y = f(x) \) 的图象，则 \( f(x) = \)（\quad）
...
\begin{problem}
设 \(F_{1}\)，\(F_{2}\) 分别是双曲线 \(\frac{x^{2}}{9} - \frac{y^{2}}{8} = 1\) 的左，右焦点. 若双曲线上存在点 \(A\)，使 \(\angle F_{1}AF_{2} = 90^{\circ}\)，且 \(|AF_{1}| = 3|AF_{2}|\)，则双曲线离心率为（\quad）
...
\begin{problem}
设 \(F\) 为抛物线 \(y^{2} = 4x\) 的焦点，\(A \text{，} B \text{，} C\) 为该抛物线上三点. 若 \(\overrightarrow{FA} + \overrightarrow{FB} + \overrightarrow{FC} = 0\)，则 \(|FA| + |FB| + |FC| =\)（\quad）
...
\begin{problem}
在某项测量中，测量结果 \(C\) 服从正态分布 \(N(1, \sigma^2)\)（\(\sigma > 0\)）. 若 \(\xi\) 在 \((0, 1)\) 内取值的概率为 0.4，则 \(\xi\) 在 \((0, 2)\) 内取值的概率为\fillinblank{}.
\end{problem}
...
\begin{problem}
已知数列的通项 \(a_{n} = -5n + 2\)，其前 \(n\) 项和为 \(S_{n}\)，则 \(\lim\limits_{n\to \infty}\frac{S_n}{n^2} = \)\fillinblank{}.
\end{problem}
