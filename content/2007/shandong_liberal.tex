...
\begin{solution}
因为 \(\dfrac12=2^{-1}\)，\(4=2^2\)，且指数函数 \(2^x\) 在实数上单调递增，所以
\(2^{-1}<2^{x+1}<2^2\)，\(\Longrightarrow\)，\(-1<x+1<2\).
从而 \(-2<x<1\). 再结合 \(x\in\Z\)，得
\[
N=\{-1,0\}
\]
于是 \(M\cap N=\{-1\}\)，选 B.
...
\begin{solution}
逐个观察图中的三视图：几何体 \(\circled{1}\) 的三个视图相同；几何体 \(\circled{2}\) 的俯视图为圆，主视图和左视图均为等腰三角形，恰有两个视图相同；几何体 \(\circled{3}\) 的三视图中没有恰好两个相同；几何体 \(\circled{4}\) 的主视图和左视图相同，俯视图不同，也恰有两个视图相同. 因此满足条件的是 \(\circled{2}\) 和 \(\circled{4}\)，选 D.
\end{solution}
...
=\sin\left(\frac{\pi}{2}+x-\frac{\pi}{3}\right)
=\sin\left(x+\frac{\pi}{6}\right)
\]
若将 \(y=\sin(x+\frac{\pi}{6})\) 的图象向右平移 \(h\) 个单位，所得函数为
\[
...
由题设
\[
2\bs a-\bs b=2(1,n)-(-1,n)=(3,n)
\]
由于 \(2\bs a-\bs b\) 与 \(\bs b=(-1,n)\) 垂直，有
\[
(3,n)\cdot(-1,n)=-3+n^2=0
\]
所以 \(n^2=3\). 从而
\[
|\bs a|=\sqrt{1^2+n^2}=\sqrt{1+3}=2
\]
因此选 C.
\end{solution}
...
对 \(f(x)=\log_2x\)（定义域为正数），有
\[
f(xy)=\log_2(xy)=\log_2x+\log_2y=f(x)+f(y)
\]
所以它满足第一个等式.
...
\[
\sin(xy)=\sin\frac{\pi^2}{4}\leqslant1\neq2=\sin x+\sin y;
\]
取 \(x=y=\frac{\pi}{4}\)，则
\[
\sin(x+y)=1\neq\frac12=\sin x\sin y
\]
对第三个等式，取 \(x=y=\frac{\pi}{6}\)，此时各项都有定义，但
\[
\sin(x+y)=\sin\frac{\pi}{3}=\frac{\sqrt3}{2}
\neq\frac{\frac12+\frac12}{1-\frac12\cdot\frac12}
=\frac43
\]
...
\begin{solution}
原命题是“对任意 \(x\in\R\)，\(x^3-x^2+1\leqslant0\)”. 全称命题
\(\forall x\in\R\)，\(p(x)\leqslant0\)
的否定是存在一个实数使不等式严格反向，即
\(\exists x\in\R\)，\(p(x)>0\).
因此本题的否定为“存在 \(x\in\R\)，\(x^3-x^2+1>0\)”，选 C.
\end{solution}
...
\begin{solution}
直方图的组距均为 \(1\)，所以每组频率等于“矩形高乘组距”. 由图读得六组的频率依次为
...
\begin{solution}
抛物线 \(y^2=2px\) 的焦点为 \(F(\frac p2,0)\). 设 \(A=(x,y)\). 因为射线 \(FA\) 与 \(x\) 轴正向的夹角为 \(60^\circ\)，有
\[
y=\sqrt3\left(x-\frac p2\right)
\]
并且 \(x-\frac p2\geqslant0\). 记
\[
u=x-\frac p2\geqslant0
\]
则 \(y=\sqrt3u\)，代入抛物线方程得
\[
3u^2=2p\left(u+\frac p2\right)=2pu+p^2
\]
即
\[
(3u+p)(u-p)=0
\]
由于 \(u\geqslant0\) 且 \(p>0\)，\(3u+p>0\)，故 \(u=p\). 因此
\[
A=\left(\frac{3p}{2},\sqrt3p\right)
\]
从而
\[
|\overrightarrow{OA}|=\sqrt{\left(\frac{3p}{2}\right)^2+(\sqrt3p)^2}
=\frac{\sqrt{21}p}{2}
\]
所以选 B.
\end{solution}
...
\begin{solution}
由程序框图，输入 \(n=100\) 后，初始 \(S=T=0\). 每次循环先把当前 \(n\) 加到 \(S\)，再令 \(n\) 减 \(1\)，把新的 \(n\) 加到 \(T\)，再令 \(n\) 减 \(1\)，直到 \(n<2\) 时输出. 因此
\[
S=100+98+\cdots+2=2(1+2+\cdots+50)=50\times51=2550
\]
\[
...
交点的横坐标满足
\[
x^3=\left(\frac12\right)^{x-2}=2^{2-x}
\]
令
\[
h(x)=x^3-2^{2-x}
\]
则
\(h(1)=1-2=-1<0\)，\(h(2)=8-1=7>0\).
函数 \(h\) 连续，所以由零点定理，方程在 \((1,2)\) 内有根. 又 \(x^3\) 在实数上严格递增，而 \(2^{2-x}\) 严格递减，二者至多有一个交点. 因此交点横坐标 \(x_0\) 必在 \((1,2)\) 内，选 B.
\end{solution}
...
\begin{solution}
从 \(A=\{1,2\}\) 和 \(B=\{1,2,3\}\) 中各取一个数，共有 \(2\times3=6\) 个等可能的有序点：
\((1,1)\)，\((1,2)\)，\((1,3)\)，\((2,1)\)，\((2,2)\)，\((2,3)\).
按 \(a+b=n\) 分类，和为 \(2\)，\(3\)，\(4\)，\(5\) 时对应的点数分别为
\(1\)，\(2\)，\(2\)，\(1\).
所以事件 \(C_2\)，\(C_3\)，\(C_4\)，\(C_5\) 的概率依次为
\(\frac16\)，\(\frac26\)，\(\frac26\)，\(\frac16\).
最大概率为 \(\frac26\)，在 \(n=3\) 和 \(n=4\) 时取得，选 D.
\end{solution}
...
\begin{solution}
按复合函数的顺序从内向外计算：
\(f_3(2007)=2007^2\)，\(f_2(f_3(2007))=(2007^2)^{-1}=\frac1{2007^2}\).
由于 \(2007>0\)，
\[
f_1(f_2(f_3(2007)))
=\left(\frac1{2007^2}\right)^{1/2}
=\frac1{2007}
\]
\end{solution}
...
\begin{solution}
当 \(x=1\) 时，\(y=a^{1-x}=a^0=1\)，与 \(a\) 的取值无关，所以图象恒过定点 \(A=(1,1)\). 点 \(A\) 在给定直线上，故
\(m+n-1=0\)，\(m+n=1\).
又 \(mn>0\)，结合 \(m+n=1>0\) 可知 \(m\)，\(n>0\). 因而
\[
\frac1m+\frac1n=\frac{m+n}{mn}=\frac1{mn}
\]
由均值不等式，
\[
mn\leqslant\left(\frac{m+n}{2}\right)^2=\frac14
\]
所以
\[
\frac1m+\frac1n=\frac1{mn}\geqslant4
\]
当且仅当 \(m=n=\frac12\) 时取等号，故最小值为 \(4\).
\end{solution}
...
\lim_{x\to1^+}g(x)=-5
\]
因此对任意 \(x\in(1,2)\)，都有 \(g(x)>-5\). 若 \(m\leqslant-5\)，则 \(m<g(x)\)，原不等式恒成立.

反之，若 \(m>-5\)，由 \(g(x)\to-5\ (x\to1^+)\)，可取充分接近 \(1\) 的 \(x\in(1,2)\) 使 \(g(x)<m\)，此时 \(m<g(x)\) 不成立，原不等式不能恒成立. 因此
\[
m\leqslant-5
\]
\end{solution}
...
(x-6)^2+(y-6)^2=18
\]
故其圆心为 \(C=(6,6)\)，半径为 \(R=3\sqrt2\). 设所求圆圆心为 \(P\)，半径为 \(r>0\)，直线为 \(L:x+y-2=0\). 由于所求圆与 \(L\) 相切，
\[
d(P,L)=r
\]
而
\[
d(C,L)=\frac{|6+6-2|}{\sqrt2}=5\sqrt2
\]

若两圆外切，则 \(PC=r+3\sqrt2\). 由点到直线距离的三角不等式，
\[
PC\geqslant d(C,L)-d(P,L)=5\sqrt2-r
\]
当 \(r<5\sqrt2\) 时，从而
\[
r+3\sqrt2\geqslant5\sqrt2-r
\]
即 \(r\geqslant\sqrt2\). 当 \(r\geqslant5\sqrt2\) 时更显然有 \(r>\sqrt2\)，所以外切情形的半径均不小于 \(\sqrt2\).

两圆内切时，所求圆应包含在已知圆内或已知圆包含所求圆. 若所求圆包含在已知圆内，则它的每一点到直线 \(L\) 的距离至少为
\[
d(C,L)-R=5\sqrt2-3\sqrt2=2\sqrt2>0
\]
不可能与 \(L\) 相切；若已知圆包含在所求圆内，则 \(r\geqslant R>\sqrt2\). 因而最小半径仍只能在外切情形取得.

取 \(P=(2,2)\)，则
\(d(P,L)=\frac{|2+2-2|}{\sqrt2}=\sqrt2\)，\(PC=\sqrt{(6-2)^2+(6-2)^2}=4\sqrt2 =\sqrt2+3\sqrt2\).
所以以 \(P\) 为圆心、半径为 \(\sqrt2\) 的圆同时与直线 \(L\) 和已知圆外切，达到下界. 所求圆的标准方程为
\[
\boxed{(x-2)^2+(y-2)^2=2}
\]
\end{solution}
...
x_{0}=\sqrt{-\frac{b}{2a}}
\]
当 \(a>0\) 时，\(b<0\)，故 \(f'(x)<0\)（\(0<x<x_{0}\)）且 \(f'(x)>0\)（\(x>x_{0}\)），所以 \(x_{0}\) 是极小值点；当 \(a<0\) 时，\(b>0\)，导数符号相反，所以 \(x_{0}\) 是极大值点. 因而 \(x_{0}\) 是唯一的极值点，极值为
\[
