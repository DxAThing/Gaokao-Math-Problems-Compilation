\chapter{2007年福建卷（文）}

\section{单选题}

\begin{problem}
已知全集 \(U = \{1, 2, 3, 4, 5\}\)，且 \(A = \{2, 3, 4\}\)，\(B = \{1, 2\}\)，则 \(A \cap \left( \complement_U B \right)\) 等于（\quad）

\choices
    {\(\{2\}\)}
    {\(\{5\}\)}
    {\(\{3, 4\}\)}
    {\(\{2, 3, 4, 5\}\)}

\end{problem}

\begin{problem}
等比数列 \(\{a_n\}\) 中，\(a_4 = 4\)，则 \(a_2 \cdot a_6\) 等于（\quad）

\choices
    {4}
    {8}
    {16}
    {32}

\end{problem}

\begin{problem}
\(\sin 15^\circ \cos 75^\circ + \cos 15^\circ \sin 105^\circ\) 等于（\quad）

\choices
    {0}
    {\(\frac{1}{2}\)}
    {\(\frac{\sqrt{3}}{2}\)}
    {1}

\end{problem}

\begin{problem}
“\(|x| < 2\)”是“\(x^2 - x - 6 < 0\)”的（\quad）

\choices
    {充分而不必要条件}
    {必要而不充分条件}
    {充要条件}
    {既不充分也不必要条件}

\end{problem}

\begin{problem}
函数 \(f(x) = \sin \left( 2x + \frac{\pi}{3} \right)\) 的图象（\quad）

\choices
    {关于点 \(\left( \frac{\pi}{3}, 0 \right)\) 对称}
    {关于直线 \(x = \frac{\pi}{4}\) 对称}
    {关于点 \(\left( \frac{\pi}{4}, 0 \right)\) 对称}
    {关于直线 \(x = \frac{\pi}{3}\) 对称}

\end{problem}

\begin{problem}
如图，在正方体 \(ABCD - A_1B_1C_1D_1\) 中，\(E, F, G, H\) 分别为 \(AA_1, AB, BB_1, B_1C_1\) 的中点，则异面直线 \(EF\) 与 \(GH\) 所成的角等于（\quad）

\choices
    {\(45^\circ\)}
    {\(60^\circ\)}
    {\(90^\circ\)}
    {\(120^\circ\)}

\end{problem}

\begin{problem}
已知 \(f(x)\) 为 \(\R\) 上的减函数，则满足 \(f\left( \frac{1}{x} \right) > f(1)\) 的实数 \(x\) 的取值范围是（\quad）

\choices
    {\((-\infty, 1)\)}
    {\((1, +\infty)\)}
    {\((-\infty, 0) \cup (0, 1)\)}
    {\((-\infty, 0) \cup (1, +\infty)\)}

\end{problem}

\begin{problem}
对于向量 \(a\)、\(b\)、\(c\) 和实数 \(\lambda\)，下列命题中真命题的是（\quad）

\choices
    {若 \(a \cdot b = 0\)，则 \(a = 0\) 或 \(b = 0\)}
    {若 \(\lambda a = 0\)，则 \(\lambda = 0\) 或 \(a = 0\)}
    {若 \(a^2 = b^2\)，则 \(a = b\) 或 \(a = -b\)}
    {若 \(a \cdot b = a \cdot c\)，则 \(b = c\)}

\end{problem}

\begin{problem}
已知 \(m, n\) 为两条不同的直线，\(\alpha, \beta\) 为两个不同的平面，则下列命题中正确的是（\quad）

\choices
    {\(m \subset \alpha\)，\(n \subset \alpha\)，\(m \nmid \beta\)，\(n \nmid \beta \Rightarrow \alpha \nmid \beta\)}
    {\(\alpha \nmid \beta\)，\(m \subset \alpha\)，\(n \subset \beta \Rightarrow m \nmid n\)}
    {\(m \perp \alpha\)，\(m \perp n \Rightarrow n \nmid \alpha\)}
    {\(n \nmid m\)，\(n \subset \alpha \Rightarrow m \perp \alpha\)}

\end{problem}

\begin{problem}
以双曲线 \(x^2 - y^2 = 2\) 的右焦点为圆心，且与其右准线相切的圆的方程是（\quad）

\choices
    {\(x^2 + y^2 - 4x - 3 = 0\)}
    {\(x^2 + y^2 - 4x + 3 = 0\)}
    {\(x^2 + y^2 + 4x - 5 = 0\)}
    {\(x^2 + y^2 + 4x + 5 = 0\)}

\end{problem}

\begin{problem}
已知对任意实数 \(x\)，有 \(f(-x) = -f(x)\)，\(g(-x) = g(x)\)，且 \(x > 0\) 时，\(f'(x) > 0\)，\(g'(x) > 0\)，则 \(x < 0\) 时（\quad）

\choices
    {\(f'(x) > 0\)，\(g'(x) > 0\)}
    {\(f'(x) > 0\)，\(g'(x) < 0\)}
    {\(f'(x) < 0\)，\(g'(x) > 0\)}
    {\(f'(x) < 0\)，\(g'(x) < 0\)}

\end{problem}

\begin{problem}
某通讯公司推出一组手机号码，卡号的前七位数字固定，从“××××××× 0000”到“×××××××9999”共 10000 个号码. 公司限定: 凡卡号的后四位带有数字“4”或“7”的一律作为“优惠卡”，则这组号码中“优惠卡”的个数为（\quad）

\choices
    {2000}
    {4096}
    {5904}
    {8320}

\end{problem}

\section{填空题}

\begin{problem}
\(\left( x^2 + \frac{1}{x} \right)^6\) 的展开式中常数项是 \fillinblank{}. （用数字作答）

\end{problem}

\begin{problem}
已知实数 \(x, y\) 满足 \(\left\{ \begin{array}{l} x + y \geqslant 2, \\ x - y \leqslant 2, \\ 0 \leqslant y \leqslant 3. \end{array} \right.\)，则 \(2x - y\) 的取值范围是 \fillinblank{}.

\end{problem}

\begin{problem}
已知长方形 \(ABCD\)，\(AB = 4\)，\(BC = 3\)，则以 \(A, B\) 为焦点，且过 \(C, D\) 两点的椭圆的离心率为 \fillinblank{}.

\end{problem}

\begin{problem}
中学数学中存在许多关系，比如“相等关系”“平行关系”等等. 如果集合 \(A\) 中元素之间的一个关系“\(\sim\)”满足以下三个条件: \circled{1} 自反性: 对于任意 \(a \in A\)，都有 \(a \sim a\); \circled{2} 对称性: 对于任意 \(a, b \in A\)，若 \(a \sim b\)，则有 \(b \sim a\); \circled{3} 传递性: 对于任意 \(a, b, c \in A\)，若 \(a \sim b\)，\(b \sim c\)，则有 \(a \sim c\). 则称“\(\sim\)”是集合 \(A\) 的一个等价关系. 例如: “数的相等”是等价关系，而“直线的平行”不是等价关系（自反性不成立）. 请你再列出两个等价关系: \fillinblank{}.

\end{problem}

\section{解答题}

\begin{problem}
甲、乙两名跳高运动员一次试跳 2 米高度成功的概率分别为 0.7、0.6，且每次试跳成功与否相互之间没有影响，求：

（1）甲试跳三次，第三次才成功的概率；

（2）甲、乙两人在第一次试跳中至少有一人成功的概率；

（3）甲、乙各试跳两次，甲比乙的成功次数恰好一次的概率。

\end{problem}

\begin{problem}
如图，正三棱柱 \(ABC - A_1B_1C_1\) 的所有棱长都为 2, \(D\) 为 \(CC_1\) 中点.

（1）求证: \(AB_1 \perp\) 平面 \(A_1BD\)；

（2）求二面角 \(A - AD_1 - B\) 的大小。

\end{problem}

\begin{problem}
设函数 \(f(x) = tx^2 + 2t^2x + t - 1\)（\(x \in \R\)，\(t > 0\)）.

（1）求 \(f(x)\) 的最小值 \(h(t)\)；

（2）若 \(h(t) < -2t + m\) 对 \(t \in (0, 2)\) 恒成立，求实数 \(m\) 的取值范围。

\end{problem}

\begin{problem}
数列 \(\{a_n\}\) 的前 \(n\) 项和为 \(S_n\)，\(a_1 = 1\)，\(a_{n+1} = 2S_n\)（\(n \in \mathbf{N}^*\)）.

（1）求数列 \(\{a_n\}\) 的通项 \(a_n\)；

（2）求数列 \(\{na_n\}\) 的前 \(n\) 项和 \(T_n\)。

\end{problem}

\begin{problem}
如图，已知点 \(F(1, 0)\)，直线 \(l: x = -1\)，\(P\) 为平面上的动点，过 \(P\) 作直线 \(l\) 的垂线，垂足为点 \(Q\)，且 \(\overrightarrow{QP} \cdot \overrightarrow{QF} = \overrightarrow{FP} \cdot \overrightarrow{FQ}\).

（1）求动点 \(P\) 的轨迹 \(C\) 的方程；

（2）过点 \(F\) 的直线交轨迹 \(C\) 于 \(A\)、\(B\) 两点，交直线 \(l\) 于点 \(M\).

\circled{1} 已知 \(\overrightarrow{MA} = \lambda_1 \overrightarrow{AP}\)，\(\overrightarrow{MB} = \lambda_2 \overrightarrow{BP}\)，求 \(\lambda_1 + \lambda_2\) 的值；

\circled{2} 求 \(\left| \overrightarrow{MA} \right| \cdot \left| \overrightarrow{MB} \right|\) 的最小值。
\end{problem}

