...
\[
2x+\frac{\pi}{3}=k\pi,
\quad x=\frac{k\pi}{2}-\frac{\pi}{6}\quad(k\in\Z).
\]
...
\begin{solution}
第三个命题正确：因为 \(m\perp\alpha\)，直线 \(m\) 的方向为平面 \(\alpha\) 的法向方向；又 \(m\perp n\)，所以直线 \(n\) 的方向平行于平面 \(\alpha\)，从而 \(n\parallel\alpha\). 第一个命题不正确，例如取相交平面 \(\alpha\colon y=0\)，\(\beta\colon z=0\)，并在 \(\alpha\) 内取两条互相平行且分别位于 \(z=1\) 与 \(z=2\) 的水平直线，它们都平行于 \(\beta\)，但 \(\alpha\) 与 \(\beta\) 相交. 第二个命题不正确：即使 \(\alpha\parallel\beta\)，分别位于两平面内的两条直线也不一定平行. 第四个命题不正确：若 \(n\) 在 \(\alpha\) 内，取与 \(n\) 平行且不垂直于 \(\alpha\) 的直线 \(m\)，则 \(n\parallel m\)，但 \(m\not\perp\alpha\). 故选第三个命题.
\end{solution}
...
由题设得
\[
x\geqslant2-y,\qquad x\leqslant2+y,\qquad 0\leqslant y\leqslant3.
\]
...
\begin{solution}
图形的全等关系具有自反性、对称性和传递性：任一图形与自身全等；若图形 \(X\) 与图形 \(Y\) 全等，则图形 \(Y\) 与图形 \(X\) 全等；若图形 \(X\) 与图形 \(Y\) 全等且图形 \(Y\) 与图形 \(Z\) 全等，则图形 \(X\) 与图形 \(Z\) 全等. 因而图形的全等是等价关系. 图形的相似关系同样具有这三个性质，所以也是等价关系. 对非零向量，若 \(\bs{a}\sim\bs{b}\) 表示两向量共线，则 \(\bs{a}=\lambda\bs{b}\)，其中 \(\lambda\neq0\)，从而自反性和对称性成立；若 \(\bs{a}=\lambda\bs{b}\)，\(\bs{b}=\mu\bs{c}\)，则 \(\bs{a}=\lambda\mu\bs{c}\)，故传递性也成立. 所以图形的全等、图形的相似、非零向量的共线均为等价关系.
\end{solution}
...
\]
又 \(0<A+B<\pi\)，且 \(A+B\) 为锐角，所以 \(A+B=\frac{\pi}{4}\)，从而
\[
...
\begin{solution}
建立如图所示的空间直角坐标系，取点 \(A(0,0,0)\)，\(B(2,0,0)\)，\(C(1,\sqrt{3},0)\)，\(A_{1}(0,0,2)\)，\(B_{1}(2,0,2)\)，\(C_{1}(1,\sqrt{3},2)\).
...
\item 原题第（2）小题印作二面角 \(A-AD_{1}-B\)，但题干和图中只定义了点 \(D\)，未定义点 \(D_{1}\). 结合图形、前一问的平面 \(A_{1}BD\) 及标准二面角记号，以下将其解释为二面角 \(A-A_{1}D-B\)，即棱为 \(A_{1}D\)，两侧面为 \(AA_{1}D\) 和 \(A_{1}BD\).
...
f'(x)=\e^{x}-\e
\]
因为 \(\e^{x}<\e\) 当且仅当 \(x<1\)，所以 \(f'(x)<0\) 在 \((-\infty,1)\) 上成立，\(f'(x)>0\) 在 \((1,+\infty)\) 上成立. 故 \(f(x)\) 在 \((-\infty,1)\) 上单调递减，在 \((1,+\infty)\) 上单调递增.

\item 令 \(t=|x|\)，则 \(t\geqslant0\)，题设等价于
\[
\e^{t}-kt>0\quad\text{对任意 }t\geqslant0
\]
当 \(t=0\) 时不等式显然成立. 当 \(t>0\) 时，不等式等价于
\[
k<\frac{\e^{t}}{t}
\]
令 \(h(t)=\frac{\e^{t}}{t}\)，则
