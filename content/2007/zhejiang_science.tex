...
\begin{solution}
正方形任意两个不同顶点的距离至少为 \(16\text{ 米}\)，而一个喷水龙头的喷洒直径为 \(12\text{ 米}\)，所以一个喷水龙头不可能同时覆盖两个顶点；要覆盖正方形的四个顶点，至少需要 \(4\) 个喷水龙头.
...
\(2i+j=10\)，\(0\leqslant i\leqslant8\)，\(0\leqslant j\leqslant3\).
...
\(E(\xi^2)=a+c=\frac23\)，\(D(\xi)=E(\xi^2)-E(\xi)^2=\frac23-\frac19=\frac59\).
\end{solution}
\end{problem}
...
\(\bs{e}_1\perp\bs{e}_2\)，\(\bs{e}_1\perp\bs{e}_3\)，\(\bs{e}_2\cdot\bs{e}_3=\cos\theta\).
...
\(A=(3,4)\)，\(B=(3,-3m)\)，\(C=\left(-\frac5{2m+1},\frac{5m}{2m+1}\right)\).
...
\(C=(0,0,0)\)，\(A=(2,0,0)\)，\(B=(0,2,0)\)，\(E=(2,0,1)\)，\(D=(0,2,2)\).
因为 \(M\) 是 \(AB\) 的中点，所以 \(M=(1,1,0)\). 于是
\(\overrightarrow{CM}=(1,1,0)\)，\(\overrightarrow{EM}=(-1,1,-1)\)
从而
\[
...
故 \(CM\perp EM\).
\item 由
\(\overrightarrow{CD}=(0,2,2)\)，\(\overrightarrow{CE}=(2,0,1)\)
可取平面 \(CDE\) 的一个法向量为
\[
...
\]
故 \(\dfrac{\lvert b\rvert}{\sqrt{1+k^2}}=1\)，从而 \(b^2=1+k^2\).
...
\((a_1,a_2)=(2,3)\)，\((a_3,a_4)=(4,6)\)，\((a_5,a_6)=(8,9)\)，\((a_7,a_8)=(12,16)\).
...
\varepsilon_j=(-1)^{f(j)}
\]
由于 \(j\) 为正整数且 \(\pi\) 为无理数，\(\sin j\ne0\)，所以 \(\varepsilon_j\) 有定义且 \(\lvert\varepsilon_j\rvert=1\). 又因为 \(0<2<3<\pi<4<\dfrac{3\pi}{2}\)，所以 \(\sin2>0\)，\(\sin3>0\)，\(\sin4<0\)，从而 \(\varepsilon_2=\varepsilon_3=1\)，\(\varepsilon_4=-1\). 又
\[
a_{2k-1}a_{2k}=3k\cdot2^k
\]
故 \(T_n=\displaystyle\sum_{k=1}^n\frac{\varepsilon_{k+1}}{3k2^k}\).
...
\(h(x)=\frac{x^3}{3}-4x+\frac{16}{3}\)，\(h'(x)=x^2-4=(x-2)(x+2)\).
...
\begin{enumerate}
\item 取 \(x=1\)，\(t=-1\)，则
\(f(1)=\frac13\)，\(g_{-1}(1)=1+\frac23=\frac53\)
所以 \(f(1)<g_{-1}(1)\)，题面所述的“不论 \(t\) 为何实数”命题不成立.
\item 对任意正实数 \(x_0\)，令 \(t=-s\)，则
\(g_{-s}(x_0)=s^{2/3}x_0+\frac23s\longrightarrow+\infty\)，\((s\longrightarrow+\infty)\)
而 \(g_8(x_0)=4x_0-\dfrac{16}{3}\) 是定值. 因此不存在正实数 \(x_0\)，使 \(g_8(x_0)\geqslant g_t(x_0)\) 对任意实数 \(t\) 成立.
\end{enumerate}
