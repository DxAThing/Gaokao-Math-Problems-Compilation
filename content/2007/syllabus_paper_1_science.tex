...
\[
\{1,a+b,a\}=\{1,0,a\},\qquad \left\{0,\frac ba,b\right\}=\{0,-1,-a\}.
\]
...
\[
\left|\overrightarrow{A_{1}B}\cdot\overrightarrow{AD_{1}}\right|=4s^2,
\qquad
|\overrightarrow{A_{1}B}|=|\overrightarrow{AD_{1}}|=\sqrt{5}s.
\]
...
\[
\binom{n}{k}(x^2)^{n-k}\left(-\frac1x\right)^k=(-1)^k\binom{n}{k}x^{2n-3k}.
\]
常数项要求 \(2n-3k=0\)，即 \(k=\frac{2n}{3}\) 为整数. 当 \(n=3\) 时 \(k=2\)，常数项系数为 \(\binom32=3\)；当 \(n=4,5\) 时 \(k\) 不是整数，不存在常数项；当 \(n=6\) 时 \(k=4\)，常数项系数为 \((-1)^4\binom64=15\). 所以 \(n=6\)，故选 D.
\end{solution}
...
\[
B=(0,0,0),\quad C=(2\sqrt2,0,0),\quad A=(\sqrt2,\sqrt2,0),\quad D=(3\sqrt2,\sqrt2,0).
\]
...
\[
|\bs{n}|=2\sqrt2,\qquad |\overrightarrow{SD}|=\sqrt{11},\qquad \left|\overrightarrow{SD}\cdot\bs{n}\right|=4.
\]
...
\[
a_n-\sqrt2=\sqrt2r^n,\qquad a_n=\sqrt2(1+r^n).
\]
...
因为 \(3+2\sqrt2=r^{-2}\)，故
\[
3+2\sqrt2(1+r^k)\geqslant r^{-2},\qquad \varphi(a_k)-\sqrt2\leqslant\sqrt2r^{k+4}=a_{k+4}-\sqrt2.
\]
