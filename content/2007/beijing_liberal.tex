...
\begin{enumerate}
\item 当 \(a=3\) 时，\(P=(-1,3)\).
\item 当 \(Q\subseteq P\) 时，正数 \(a\) 的取值范围为 \((2,+\infty)\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 平面 \(COD\perp\) 平面 \(AOB\).
\item 异面直线 \(AO\) 与 \(CD\) 所成角为 \(\arctan\frac{\sqrt{15}}{3}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 这 \(6\) 位乘客在互不相同的车站下车的概率为 \(\frac{189}{1250}\).
\item 恰有 \(3\) 人在终点站下车的概率为 \(\frac{729}{50000}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(AD\) 边所在直线的方程为 \(3x+y+2=0\).
\item 矩形外接圆的方程为 \(\left(x-2\right)^2+y^2=8\).
\item 动圆圆心的轨迹为 \(\frac{\left(x-2\right)^2}{4}-\frac{y^2}{4}=1\) 的左支，即 \(x\leqslant0\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
\item \(k>2\sqrt{2}\).
\item \(t=\frac{x_1^2-2}{2x_1}=\frac{x_1}{2}-\frac{1}{x_1}\)，定义域为 \((0,\sqrt{2})\)，值域为 \((-\infty,0)\).
\item \(|OM|=|ON|\).
\end{enumerate}
\end{solution}
