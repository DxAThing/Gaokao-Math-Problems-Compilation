...
\section{解答题}
\begin{problem}
记关于 \(x\) 的不等式 \(\frac{x - a}{x + 1} < 0\) 的解集为 \(P\)，不等式 \(|x - 1| \leqslant 1\) 的解集为 \(Q\). (1) 若 \(a = 3\)，求 \(P\); (2) 若 \(Q \subseteq P\)，求正数 \(a\) 的取值范围.
\end{problem}
...
\begin{problem}
如图，在 Rt\(\triangle\)AOB 中，\(\angle\)OAB = \( \frac{\pi}{6} \) ，斜边 AB = 4. Rt\(\triangle\)AOC 可以通过 Rt\(\triangle\)AOB 以直线 AO 为轴旋转得到，且二面角 B - AO - C 是直二面角. D 是 AB 的中点. (1) 求证: 平面 \(COD \perp\) 平面 \(AOB\); (2) 求异面直线 AO 与 CD 所成角的大小.
...
\begin{problem}
如图，矩形 \(ABCD\) 的两条对角线相交于点 \(M(2,0)\)，\(AB\) 边所在直线的方程为 \(x - 3y - 6 = 0\)，点 \(T(-1,1)\) 在 \(AD\) 边所在直线上. (1) 求 AD 边所在直线的方程; (2) 求矩形 ABCD 外接圆的方程; (3) 若动圆 P 过点 \( N(-2,0) \) ，且与矩形 ABCD 的外接圆外切，求动圆 P 的圆心的轨迹方程.
