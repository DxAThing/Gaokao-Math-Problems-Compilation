...
\begin{problem}
若向量 \(\overrightarrow{a},\overrightarrow{b}\) 满足 \(|\overrightarrow{a}|=\sqrt{2}\)，\(|\overrightarrow{b}|=1\)，\(\overrightarrow{a}\cdot(\overrightarrow{a}+\overrightarrow{b})=1\)，则向量 \(\overrightarrow{a},\overrightarrow{b}\) 的夹角的大小为\fillinblank{}.
\end{problem}
...
\bitmapfigure[max width=0.30\linewidth,max totalheight=4.0cm]{img/2007/shanghai_spring/q13_fig1.png}
...
\bitmapfigure[max width=0.24\linewidth,max totalheight=3.0cm]{img/2007/shanghai_spring/q14_fig1.png}
...
\bitmapfigure[max width=0.36\linewidth,max totalheight=4.6cm]{img/2007/shanghai_spring/q16_fig1.png}
\end{problem}

\begin{solution}
解法一：如图建立空间直角坐标系. 由题意可知
\examdisplayaligned{
A'(2,0,2),\ C(0,2,0),\ E(2,1,2),\ F(2,1,0),\\
...
解法二：连结 \(EB\). 因为 \(A'E\parallel BF\)，且 \(A'E=BF\)，所以 \(A'FBE\) 是平行四边形，从而 \(A'F\parallel EB\). 因此异面直线 \(A'F\) 与 \(CE\) 所成的角就是 \(CE\) 与 \(EB\) 所成的角. 又因为 \(CB\perp\) 平面 \(ABB'A'\)，所以 \(CB\perp BE\). 在直角三角形 \(CEB\) 中，\(CB=2\)，\(BE=\sqrt{5}\)，于是
\examdisplaymath{
...
（1）求到直线 \(3x+4y=0\) 的距离为 \(2\) 的点的轨迹方程. 设所求轨迹上任意一点为 \(P(x,y)\)，则 \(\frac{|3x+4y|}{5}=2\)，所求轨迹为 \(3x+4y-10=0\) 或 \(3x+4y+10=0\).

（2）若点 \(P(2,1)\) 到直线 \(l:ax+by=0\) 的距离为 \(2\)，求直线 \(l\) 的方程. 由 \(\frac{|2a+b|}{\sqrt{a^2+b^2}}=2\)，化简得 \(4ab-3b^2=0\)，即 \(b=0\) 或 \(4a=3b\). 所以直线 \(l\) 的方程为 \(x=0\) 或 \(3x+4y=0\).

意义不大的“逆向”问题可能是：

（3）点 \(P(2,1)\) 是不是到直线 \(3x+4y=0\) 的距离为 \(2\) 的一个点？因为 \(\frac{|3\cdot2+4\cdot1|}{\sqrt{3^2+4^2}}=2\)，所以点 \(P(2,1)\) 是到直线 \(3x+4y=0\) 的距离为 \(2\) 的一个点.

（4）点 \(Q(1,1)\) 是不是到直线 \(3x+4y=0\) 的距离为 \(2\) 的一个点？因为 \(\frac{|3\cdot1+4\cdot1|}{\sqrt{3^2+4^2}}=\frac{7}{5}\neq 2\)，所以点 \(Q(1,1)\) 不是到直线 \(3x+4y=0\) 的距离为 \(2\) 的一个点.

（5）点 \(P(2,1)\) 是不是到直线 \(5x+12y=0\) 的距离为 \(2\) 的一个点？因为 \(\frac{|5\cdot2+12\cdot1|}{\sqrt{5^2+12^2}}=\frac{22}{13}\neq 2\)，所以点 \(P(2,1)\) 不是到直线 \(5x+12y=0\) 的距离为 \(2\) 的一个点.
\end{solution}
...
\bitmapfigure[max width=0.38\linewidth,max totalheight=4.8cm]{img/2007/shanghai_spring/q18_fig1.png}

\end{problem}

\begin{solution}
（1）解法一：因为 \(l\) 垂直于 \(x\) 轴，所以 \(F_2\) 的坐标为 \((\sqrt{2},0)\). 由题意可知
\examdisplaymath{
\begin{cases}
...
解法二：由椭圆定义可知 \(|MF_1|+|MF_2|=2a\). 由题意 \(|MF_2|=1\)，因此 \(|MF_1|=2a-1\). 又由直角三角形 \(MF_1F_2\) 可知 \((2a-1)^2=(2\sqrt{2})^2+1\)，且 \(a>0\)，所以 \(a=2\). 再由 \(a^2-b^2=2\) 得 \(b^2=2\). 因此椭圆 \(C\) 的方程为 \(\frac{x^2}{4}+\frac{y^2}{2}=1\).

（2）直线 \(BF_2\) 的方程为 \(y=x-\sqrt{2}\). 由
\examdisplaymath{
\begin{cases}
y=x-\sqrt{2},\\
...
得点 \(N\) 的纵坐标为 \(\frac{\sqrt{2}}{3}\). 又 \(|F_1F_2|=2\sqrt{2}\)，所以
\examdisplaymath{
S_{\triangle F_1BN}
=\frac{1}{2}\times\left(\sqrt{2}+\frac{\sqrt{2}}{3}\right)\times 2\sqrt{2}
=\frac{8}{3}.
}
\end{solution}
...
\begin{center}
\bitmapinclude[max width=0.28\linewidth,max totalheight=4.0cm]{img/2007/shanghai_spring/q19_fig1.png}\hfill
\bitmapinclude[max width=0.46\linewidth,max totalheight=4.0cm]{img/2007/shanghai_spring/q19_fig2.png}
\end{center}
\end{problem}

\begin{solution}
（1）图 2 是由四块图 1 所示地砖绕点 \(C\) 按顺时针旋转 \(90^\circ\) 后得到，\(\triangle CFE\) 为等腰直角三角形，所以四边形 \(EFGH\) 是正方形.

（2）设 \(CE=x\)，则 \(BE=0.4-x\). 每块地砖的费用为 \(W\). 制成 \(\triangle CFE\)，\(\triangle ABE\) 和四边形 \(AEFD\) 三种材料的每平方米价格依次为 \(3a\)，\(2a\)，\(a\)（元），于是
\examdisplaymath{
\begin{aligned}
...
\bitmapfigure[max width=0.28\linewidth,max totalheight=3.5cm]{img/2007/shanghai_spring/q20_fig1.png}

\end{problem}

\begin{solution}
（1）\(\triangle ABC\) 的外接圆半径为 \(2\). 在 \(\triangle ABC\) 中，\(AC=2R\sin B=2\sqrt{2}\)，\(\sin A=\frac{BC}{2R}=\frac{1}{2}\)，所以 \(A=30^\circ\). 因而
\examdisplaymath{
\begin{aligned}
AB^2
&=BC^2+AC^2-2BC\cdot AC\cdot\cos C\\
&=4+8+8\sqrt{2}\cos(A+B)\\
&=4(\sqrt{3}+2)\\
&=2(\sqrt{3}+1)^2.
\end{aligned}
}
...
（2）由 \(\sin A=\frac{a}{2R}\)，\(\sin B=\frac{b}{2R}\). 因为 \(\angle C\) 是钝角，所以 \(\angle A\)，\(\angle B\) 都是锐角，于是
\examdisplaymath{
\begin{aligned}
\cos A&=\frac{1}{2R}\sqrt{4R^2-a^2},\\
\cos B&=\frac{1}{2R}\sqrt{4R^2-b^2},\\
\cos C&=-\cos(A+B)\\
&=\frac{1}{4R^2}\left(ab-\sqrt{4R^2-a^2}\sqrt{4R^2-b^2}\right)<0.
\end{aligned}
}
...
\begin{solution}
（1）\(B_1=q\)，\(B_2=1+q\)，\(B_3=1+(1+q)=2+q\)，\(\cdots\)，\(B_n=(n-1)+q\). 所以
\examdisplaymath{
B_1+B_2+\cdots+B_n=1+2+\cdots+(n-1)+nq=\frac{n(n-1)}{2}+nq.
}
...
（3）若选择 \circled{1}：先设 \(c_1,c_2,c_3\) 成等比数列. 由 \(c_1c_3=c_2^2\)，得 \(3+2q+q^2=(2+q)^2\)，所以 \(q=-\frac{1}{2}\). 此时 \(c_1=1\)，\(c_2=\frac{3}{2}\)，\(c_3=\frac{9}{4}\)，所以 \(c_1,c_2,c_3\) 是一个公比为 \(\frac{3}{2}\) 的等比数列. 如果 \(m\geqslant4\) 时 \(c_1,c_2,\cdots,c_m\) 为等比数列，那么 \(c_1,c_2,c_3\) 一定是等比数列. 由上所述，此时 \(q=-\frac{1}{2}\)，且 \(c_4=\frac{23}{8}\). 因为 \(\frac{c_4}{c_3}\neq\frac{3}{2}\)，所以对于任意 \(m\geqslant4\)，数列 \(c_1,c_2,\cdots,c_m\) 一定不是等比数列. 综上所述，当且仅当 \(m=3\) 且 \(q=-\frac{1}{2}\) 时，数列 \(c_1,c_2,\cdots,c_m\) 是等比数列.

若选择 \circled{2}：设 \(x_1,x_2,x_3\) 和 \(y_1,y_2,y_3\) 分别是第 \(k+1\) 列和第 \(m+1\) 列的前三项，其中 \(1\leqslant k<m\leqslant n-1\). 则
\examdisplaymath{
x_1=1,\ x_2=k+q,\ x_3=\frac{k(k+1)}{2}+kq+q^2.
}
若第 \(k+1\) 列的前三项 \(x_1,x_2,x_3\) 是等比数列，则由 \(x_1x_3=x_2^2\) 得
\examdisplaymath{
\frac{k(k+1)}{2}+kq+q^2=(k+q)^2,
}
