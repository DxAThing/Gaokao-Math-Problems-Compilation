...
\begin{enumerate}
\item \(C=\dfrac{\pi}{3}\).
\item \(\triangle ABC\) 的周长为 \(5+\sqrt{7}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 平面 \(ABEF\perp\) 平面 \(EFDC\).
\item 二面角 \(E-BC-A\) 的余弦值为 \(-\dfrac{2\sqrt{19}}{19}\).
\end{enumerate}
\end{solution}
...
\bitmapfigure[float=inline,width=0.50\linewidth]{img/2016/national_paper_1_science/q19_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item \(P(X=16),P(X=17),\ldots,P(X=22)=(0.04,0.16,0.24,0.24,0.20,0.08,0.04)\).
\item \(n=19\).
\item 选购 \(19\) 个易损零件.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(|EA|+|EB|=4\)，点 \(E\) 的轨迹方程为 \(\dfrac{x^2}{4}+\dfrac{y^2}{3}=1\).
\item 四边形 \(MPNQ\) 面积的取值范围为 \([12,8\sqrt{3})\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(C_1\) 是以 \((0,1)\) 为圆心、\(a\) 为半径的圆，极坐标方程为 \(\rho^2-2\rho\sin\theta+1-a^2=0\).
\item \(a=1\).
\end{enumerate}
\end{solution}
...
\end{cases}
\)
\item 不等式的解集为 \(\left(-\infty,\dfrac{1}{3}\right)\cup(1,3)\cup(5,+\infty)\).
\end{enumerate}
\end{solution}
