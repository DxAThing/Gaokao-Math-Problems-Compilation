...
\begin{solution}
短答案：\(a_n=3n-1\)；\(S_n=\dfrac32-\dfrac{1}{2\cdot3^{n-1}}\).
\end{solution}
...
\begin{solution}
短答案：\(G\) 为 \(AB\) 的中点；在平面 \(PAB\) 内过 \(E\) 作 \(EF\parallel PB\)，交 \(PA\) 于 \(F\)，则 \(F\) 为点 \(E\) 在平面 \(PAC\) 内的正投影；四面体 \(PDEF\) 的体积为 \(\dfrac43\).
\end{solution}
...
\(n\) 的最小值为 \(19\)；同时购买 \(19\) 个和 \(20\) 个零件时，平均费用分别为 \(4000\) 元和 \(4050\) 元，因此应购买 \(19\) 个.
\end{solution}
...
\begin{solution}
短答案：\(\dfrac{|OH|}{|ON|}=2\)；除 \(H\) 以外，直线 \(MH\) 与抛物线 \(C\) 没有其它公共点.
\end{solution}
...
\begin{solution}
短答案：当 \(a\geq0\) 时，\(f(x)\) 在 \((-\infty,1)\) 上单调递减，在 \((1,+\infty)\) 上单调递增；当 \(a=-\dfrac{\e}{2}\) 时，\(f(x)\) 在 \(\R\) 上单调递增；当 \(-\dfrac{\e}{2}<a<0\) 时，\(f(x)\) 在 \((-\infty,\ln(-2a))\) 和 \((1,+\infty)\) 上单调递增，在 \((\ln(-2a),1)\) 上单调递减；当 \(a<-\dfrac{\e}{2}\) 时，\(f(x)\) 在 \((-\infty,1)\) 和 \((\ln(-2a),+\infty)\) 上单调递增，在 \((1,\ln(-2a))\) 上单调递减；\(f(x)\) 有两个零点的充要条件是 \(a>0\).
\end{solution}
...
\begin{solution}
短答案：直线 \(AB\) 与 \(\odot O\) 相切；\(AB\parallel CD\).
\end{solution}
...
\begin{solution}
短答案：\(C_1\) 是圆心为 \((0,1)\)、半径为 \(a\) 的圆，极坐标方程为 \(\rho^2-2\rho\sin\theta+1-a^2=0\)；\(a=1\).
\end{solution}
...
f(x)=\begin{cases}
x-4,&x\leq-1,\\
3x-2,&-1<x\leq\dfrac32,\\
-x+4,&x>\dfrac32.
\end{cases}
\]

图象按上述分段函数作出；\(|f(x)|>1\) 的解集为 \(\left\{x\mid x<\dfrac13\text{ 或 }1<x<3\text{ 或 }x>5\right\}\).
\end{solution}
