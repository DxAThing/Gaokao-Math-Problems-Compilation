\chapter{2016年上海卷（秋文）}

\section{填空题}

\begin{problem}
设 \(x\in\R\)，则不等式 \(|x-3|<1\) 的解集为\fillinblank{}.
\end{problem}

\begin{answer}
\((2,4)\)
\end{answer}

\begin{problem}
设 \(z=\dfrac{3+2\i}{\i}\)，其中 \(\i\) 为虚数单位，则 \(z\) 的虚部等于\fillinblank{}.
\end{problem}

\begin{answer}
\(-3\)
\end{answer}

\begin{problem}
已知平行直线 \(l_1:2x+y-1=0\)，\(l_2:2x+y+1=0\)，则 \(l_1\) 与 \(l_2\) 的距离是\fillinblank{}.
\end{problem}

\begin{answer}
\(\dfrac{2\sqrt{5}}{5}\)
\end{answer}

\begin{problem}
某次体检，\(5\) 位同学的身高（单位：米）分别为 \(1.72\)，\(1.78\)，\(1.80\)，\(1.69\)，\(1.76\)，则这组数据的中位数是\fillinblank{}（米）.
\end{problem}

\begin{answer}
\(1.76\)
\end{answer}

\begin{problem}
若函数 \(f(x)=4\sin x+a\cos x\) 的最大值为 \(5\)，则常数 \(a=\fillinblank{}\).
\end{problem}

\begin{answer}
\(\pm 3\)
\end{answer}

\begin{problem}
已知点 \((3,9)\) 在函数 \(f(x)=1+a^x\) 的图像上，则 \(f(x)\) 的反函数 \(f^{-1}(x)=\fillinblank{}\).
\end{problem}

\begin{answer}
\(\log_2(x-1)\)
\end{answer}

\begin{problem}
若 \(x\)，\(y\) 满足
\(
\begin{cases}
x\ge0,\\
y\ge0,\\
y\ge x+1,
\end{cases}
\)
则 \(x-2y\) 的最大值为\fillinblank{}.
\end{problem}

\begin{answer}
\(-2\)
\end{answer}

\begin{problem}
方程 \(3\sin x=1+\cos2x\) 在区间 \([0,2\pi]\) 上的解为\fillinblank{}.
\end{problem}

\begin{answer}
\(\frac{\pi}{6}\)，\(\frac{5\pi}{6}\)
\end{answer}

\begin{problem}
在 \(\left(\sqrt[3]{x}-\dfrac2x\right)^n\) 的二项展开式中，所有项的二项式系数之和为 \(256\)，则常数项等于\fillinblank{}.
\end{problem}

\begin{answer}
\(112\)
\end{answer}

\begin{problem}
已知 \(\triangle ABC\) 的三边长分别为 \(3\)，\(5\)，\(7\)，则该三角形的外接圆半径等于\fillinblank{}.
\end{problem}

\begin{answer}
\(\dfrac{7\sqrt{3}}{3}\)
\end{answer}

\begin{problem}
某食堂规定，每份午餐可以在四种水果中任选两种，则甲，乙两同学各自所选的两种水果相同的概率为\fillinblank{}.
\end{problem}

\begin{answer}
\(\dfrac{1}{6}\)
\end{answer}

\begin{problem}
如图，已知点 \(O(0,0)\)，\(A(1,0)\)，\(B(0,-1)\)，\(P\) 是曲线 \(y=\sqrt{1-x^2}\) 上一个动点，则 \(\overrightarrow{OP}\cdot\overrightarrow{BA}\) 的取值范围是\fillinblank{}.

\bitmapfigure[max width=6.0cm]{img/2016/shanghai_liberal/q12_fig1.png}
\end{problem}

\begin{answer}
\([-1,\sqrt{2}]\)
\end{answer}

\begin{problem}
设 \(a>0\)，\(b>0\)，若关于 \(x\)，\(y\) 的方程组
\(
\begin{cases}
ax+y=1,\\
x+by=1
\end{cases}
\)
无解，则 \(a+b\) 的取值范围是\fillinblank{}.
\end{problem}

\begin{answer}
\((2,+\infty)\)
\end{answer}

\begin{problem}
无穷数列 \(\{a_n\}\) 由 \(k\) 个不同的数组成，\(S_n\) 为 \(\{a_n\}\) 的前 \(n\) 项和. 若对任意的 \(n\in\N^*\)，\(S_n\in\{2,3\}\)，则 \(k\) 的最大值为\fillinblank{}.
\end{problem}

\begin{answer}
4
\end{answer}

\section{单选题}

\begin{problem}
设 \(a\in\R\)，则“\(a>1\)”是“\(a^2>1\)”的（\quad）

\choices
    {充分非必要条件}
    {必要非充分条件}
    {充要条件}
    {既非充分也非必要条件}
\end{problem}

\begin{answer}
A
\end{answer}

\begin{problem}
如图，在正方体 \(ABCD-A_1B_1C_1D_1\) 中，\(E\)，\(F\) 分别为 \(BC\)，\(BB_1\) 的中点，则下列直线中与直线 \(EF\) 相交的是（\quad）

\bitmapfigure[max width=6.0cm]{img/2016/shanghai_liberal/q16_fig1.png}

\choices
    {直线 \(AA_1\)}
    {直线 \(A_1B_1\)}
    {直线 \(A_1D_1\)}
    {直线 \(B_1C_1\)}
\end{problem}

\begin{answer}
D
\end{answer}

\begin{problem}
设 \(a\in\R\)，\(b\in[0,2\pi)\). 若对任意实数 \(x\) 都有 \(\sin\!\left(3x-\dfrac{\pi}{3}\right)=\sin(ax+b)\)，则满足条件的有序实数对 \((a,b)\) 的对数为（\quad）

\choices
    {\(1\)}
    {\(2\)}
    {\(3\)}
    {\(4\)}
\end{problem}

\begin{answer}
B
\end{answer}

\begin{problem}
设 \(f(x)\)，\(g(x)\)，\(h(x)\) 是定义域为 \(\R\) 的三个函数. 对于命题：
\begin{circlelist}
\item 若 \(f(x)+g(x)\)，\(f(x)+h(x)\)，\(g(x)+h(x)\) 均为增函数，则 \(f(x)\)，\(g(x)\)，\(h(x)\) 中至少有一个为增函数；
\item 若 \(f(x)+g(x)\)，\(f(x)+h(x)\)，\(g(x)+h(x)\) 均是以 \(T\) 为周期的函数，则 \(f(x)\)，\(g(x)\)，\(h(x)\) 均是以 \(T\) 为周期的函数.
\end{circlelist}
则下列判断正确的是（\quad）

\choices
    {\(\circled{1}\) 和 \(\circled{2}\) 均为真命题}
    {\(\circled{1}\) 和 \(\circled{2}\) 均为假命题}
    {\(\circled{1}\) 为真命题，\(\circled{2}\) 为假命题}
    {\(\circled{1}\) 为假命题，\(\circled{2}\) 为真命题}
\end{problem}

\begin{answer}
D
\end{answer}

\section{解答题}

\begin{problem}
将边长为 \(1\) 的正方形 \(AA_1O_1O\)（及其内部）绕 \(OO_1\) 旋转一周形成圆柱，如图，\(\widehat{AC}\) 长为 \(\dfrac{5\pi}{6}\)，\(\widehat{A_1B_1}\) 长为 \(\dfrac{\pi}{3}\)，其中 \(B_1\) 与 \(C\) 在平面 \(AA_1O_1O\) 的同侧.

\begin{enumerate}
\item 求圆柱的体积与侧面积；
\item 求异面直线 \(O_1B_1\) 与 \(OC\) 所成的角的大小.
\end{enumerate}

\bitmapfigure[max width=6.0cm]{img/2016/shanghai_liberal/q19_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 短答案：圆柱的体积为 \(\pi\)，侧面积为 \(2\pi\).
\item 短答案：异面直线 \(O_1B_1\) 与 \(OC\) 所成的角为 \(\dfrac{\pi}{2}\).
\end{enumerate}
\end{solution}

\begin{problem}
有一块正方形菜地 \(EFGH\)，\(EH\) 所在直线是一条小河，收获的蔬菜可送到 \(F\) 点或河边运走. 于是，菜地分为两个区域 \(S_1\) 和 \(S_2\)，其中 \(S_1\) 中的蔬菜运到河边较近，\(S_2\) 中的蔬菜运到 \(F\) 点较近，而菜地内 \(S_1\) 和 \(S_2\) 的分界线 \(C\) 上的点到河边与到 \(F\) 点的距离相等. 现建立平面直角坐标系，其中原点 \(O\) 为 \(EF\) 的中点，点 \(F\) 的坐标为 \((1,0)\)，如图.

\begin{enumerate}
\item 求菜地内的分界线 \(C\) 的方程；
\item 菜农从蔬菜运量估计出 \(S_1\) 面积是 \(S_2\) 面积的两倍，由此得到 \(S_1\) 面积的“经验值”为 \(\dfrac83\). 设 \(M\) 是 \(C\) 上纵坐标为 \(1\) 的点，请计算以 \(EH\) 为一边，另一边过点 \(M\) 的矩形的面积，及五边形 \(EOMGH\) 的面积，并判断哪一个更接近于 \(S_1\) 面积的“经验值”.
\end{enumerate}

\bitmapfigure[max width=6.0cm]{img/2016/shanghai_liberal/q20_fig1.png}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 短答案：菜地内的分界线 \(C\) 的方程为 \(y^2=4x\)（\(0<y<2\)）.
\item 短答案：点 \(M\) 的坐标为 \(\left(\dfrac14,1\right)\)，矩形面积为 \(\dfrac52\)，五边形 \(EOMGH\) 的面积为 \(\dfrac{11}{4}\)，五边形面积更接近于 \(S_1\) 面积的“经验值”.
\end{enumerate}
\end{solution}

\begin{problem}
双曲线 \(x^2-\dfrac{y^2}{b^2}=1\)（\(b>0\)）的左，右焦点分别为 \(F_1\)，\(F_2\)，直线 \(l\) 过 \(F_2\) 且与双曲线交于 \(A\)，\(B\) 两点.

\begin{enumerate}
\item 若 \(l\) 的倾斜角为 \(\dfrac{\pi}{2}\)，\(\triangle F_1AB\) 是等边三角形，求双曲线的渐近线方程；
\item 设 \(b=\sqrt3\)，若 \(l\) 的斜率存在，且 \(|AB|=4\)，求 \(l\) 的斜率.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 短答案：双曲线的渐近线方程为 \(y=\pm\sqrt{2}x\).
\item 短答案：直线 \(l\) 的斜率为 \(\pm\dfrac{\sqrt{15}}{5}\).
\end{enumerate}
\end{solution}

\begin{problem}
对于无穷数列 \(\{a_n\}\) 与 \(\{b_n\}\)，记 \(A=\{x\mid x=a_n,\ n\in\N^*\}\)，\(B=\{x\mid x=b_n,\ n\in\N^*\}\). 若同时满足条件：
\begin{circlelist}
\item \(\{a_n\}\)，\(\{b_n\}\) 均单调递增；
\item \(A\cap B=\varnothing\) 且 \(A\cup B=\N^*\)，则称 \(\{a_n\}\) 与 \(\{b_n\}\) 是无穷互补数列.
\end{circlelist}

\begin{enumerate}
\item 若 \(a_n=2n-1\)，\(b_n=4n-2\)，判断 \(\{a_n\}\) 与 \(\{b_n\}\) 是否为无穷互补数列，并说明理由；
\item 若 \(a_n=2^n\) 且 \(\{a_n\}\) 与 \(\{b_n\}\) 是无穷互补数列，求数列 \(\{b_n\}\) 的前 \(16\) 项的和；
\item 若 \(\{a_n\}\) 与 \(\{b_n\}\) 是无穷互补数列，\(\{a_n\}\) 为等差数列且 \(a_{16}=36\)，求 \(\{a_n\}\) 与 \(\{b_n\}\) 的通项公式.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 短答案：\(\{a_n\}\) 与 \(\{b_n\}\) 不是无穷互补数列.
\item 短答案：数列 \(\{b_n\}\) 的前 \(16\) 项的和为 \(180\).
\item 短答案：\(a_n=2n+4\)，且 \(\displaystyle b_n=
\begin{cases}
n, & n\leqslant 5,\\
2n-5, & n>5
\end{cases}\).
\end{enumerate}
\end{solution}

\begin{problem}
已知 \(a\in\R\)，函数 \(f(x)=\log_2\!\left(\dfrac1x+a\right)\).

\begin{enumerate}
\item 当 \(a=1\) 时，解不等式 \(f(x)>1\)；
\item 若关于 \(x\) 的方程 \(f(x)+\log_2(x^2)=0\) 的解集中恰有一个元素，求 \(a\) 的值；
\item 设 \(a>0\)，若对任意 \(t\in\left[\dfrac12,1\right]\)，函数 \(f(x)\) 在区间 \([t,t+1]\) 上的最大值与最小值的差不超过 \(1\)，求 \(a\) 的取值范围.
\end{enumerate}
\end{problem}

\begin{solution}
\begin{enumerate}
\item 短答案：\(x\in(0,1)\).
\item 短答案：\(a=0\) 或 \(a=-\dfrac14\).
\item 短答案：\(a\in\left[\dfrac23,+\infty\right)\).
\end{enumerate}
\end{solution}
