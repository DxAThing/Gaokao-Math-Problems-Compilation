...
\item 在该样本的一等品中，随机抽取 2 件产品. \circled{1} 用产品编号列出所有可能的结果； \circled{2} 设事件 B 为“在取出的 2 件产品中，每件产品的综合指标 \(S\) 都等于 4”，求事件 B 发生的概率.
\end{enumerate}
\end{problem}
...
\begin{problem}
设 \(a \in [-2,0]\)，已知函数 \(f(x) = \begin{cases} x^2 - (a + 5)x, & x \leqslant 0, \\ x^3 - \frac{a + 3}{2}x^2 + ax, & x > 0. \end{cases}\)
\begin{enumerate}[label={（\arabic*）},itemsep=0.2em]
