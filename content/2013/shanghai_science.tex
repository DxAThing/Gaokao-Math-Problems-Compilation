...
\begin{solution}
展开式的第 \(k+1\) 项为
\(\mathrm C_5^k(x^2)^{5-k}\left(\frac{a}{x}\right)^k =\mathrm C_5^k a^k x^{10-3k}\)，\(k=0\)，\(1\)，\(\ldots\)，\(5\).
含 \(x^7\) 项时 \(10-3k=7\)，所以 \(k=1\)，该项系数为 \(\mathrm C_5^1 a=5a\). 由 \(5a=-10\)，得 \(a=-2\).
\end{solution}
\end{problem}
...
\]
故所需条件等价于
\(9x+\frac{a^2}{x}\geqslant a+8\)，\(x>0\).
因为 \(a<0\)，由基本不等式
\[
...
\begin{solution}
由反函数的定义，题设等式分别等价于
\(x\in[1,2)\Longleftrightarrow f(x)\in[0,1)\)，\(x\in[0,1)\Longleftrightarrow f(x)\in(2,4]\).
设 \(x_0\in[0,3]\) 且 \(f(x_0)=x_0\). 若 \(x_0\in[0,1)\)，则由第二个等价关系 \(f(x_0)\in(2,4]\)，与 \(f(x_0)=x_0<1\) 矛盾. 若 \(x_0\in[1,2)\)，则由第一个等价关系 \(f(x_0)\in[0,1)\)，与 \(f(x_0)=x_0\geqslant1\) 矛盾. 若 \(x_0\in(2,3]\)，则由第二个等价关系的逆向使用，\(f(x_0)=x_0\in(2,4]\) 将推出 \(x_0\in[0,1)\)，仍矛盾. 因此只能有 \(x_0=2\).
\end{solution}
\end{problem}
...
\]
要使 \(A\cup B=\R\)，必须使 \(B\) 的左端点满足 \(a-1\leqslant1\)，即 \(a\leqslant2\)；反之 \(1<a\leqslant2\) 时确实有 \(A\cup B=\R\). 综上 \(a\leqslant2\)，故选 B.
\end{solution}
\end{problem}
...
\begin{solution}
取正六边形外接圆圆心为原点，令
\(A=(1,0)\)，\(\ B=\left(\frac12,\frac{\sqrt3}{2}\right)\)，\(\ C=\left(-\frac12,\frac{\sqrt3}{2}\right)\)，\(\ D=(-1,0)\)
并按顺序取 \(E\)，\(F\). 则从 \(A\) 出发的五个向量的 \(x\) 分量为
\(-\frac12\)，\(-\frac32\)，\(-2\)，\(-\frac32\)，\(-\frac12\)
从 \(D\) 出发的五个向量的 \(x\) 分量为
\(2\)，\(\frac32\)，\(\frac12\)，\(\frac12\)，\(\frac32\).
任取其中三个，前一组向量和的 \(x\) 分量不大于
\[
...
\frac12+\frac12+\frac32=\frac52
\]
两组向量的 \(y\) 分量分别由两个 \(\frac{\sqrt3}{2}\)、两个 \(-\frac{\sqrt3}{2}\) 和一个 \(0\) 中任取三个相加，故各自绝对值不超过 \(\sqrt3\). 设两组和分别为 \((X,Y)\)、\((U,V)\)，则
\(XU\leqslant-\frac{25}{4}\)，\(YV\leqslant|Y||V|\leqslant3\).
所以任意一个题设点积均满足
\[
...
\(P(x)=\frac{900}{x}\cdot100\left(5x+1-\frac{3}{x}\right)=90000\left(5+\frac{1}{x}-\frac{3}{x^2}\right)\)，\(1\leqslant x\leqslant10\).
求导得
\[
...
\(\frac{\omega\pi}{4}\leqslant\frac{\pi}{2}\)，\(\frac{2\omega\pi}{3}\leqslant\frac{\pi}{2}\).
...
\]
因此
\(2x+\frac{\pi}{3}=-\frac{\pi}{6}+2k\pi \quad\text{或}\quad 2x+\frac{\pi}{3}=\frac{7\pi}{6}+2k\pi\)，\(k\in\Z\)
即零点为
\[
