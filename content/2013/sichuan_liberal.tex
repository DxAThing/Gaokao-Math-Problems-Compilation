...
令 \(g(t)=\e^t+t\)，则 \(g(t)\) 在 \(\R\) 上严格递增. 如果 \(u>b\)，左边 \(g(b)-g(u)<0\)，右边 \(u^2-b^2>0\)，矛盾；如果 \(u<b\)，左边为正而右边为负，也矛盾. 因此只能有 \(u=b\). 代回得
\(a=\e^b+b-b^2\).
...
\begin{solution}
函数在 \(x=3\) 处取得极小值，且该点在定义域内部，所以 \(f'(3)=0\). 由 \(f'(x)=4-\frac{a}{x^2}\)，得
\(4-\frac{a}{9}=0\)，解得 \(a=36\). 因此填 \(36\).
\end{solution}
...
\texfigure[max width=0.48\linewidth]{img_repaint/2013/sichuan_liberal/q18_fig1.tex}
\end{problem}
...
按算法得到的理论概率为 \(\left(\frac12,\frac13,\frac16\right)\). 乙的三个频率分别约为 \(0.5005\)，\(0.3314\)，\(0.1681\)，都与理论概率接近；甲的第二、第三个频率分别约为 \(0.1790\)，\(0.3319\)，与理论概率的对应关系明显不符. 因此乙同学所编程序符合算法要求的可能性较大.
\end{enumerate}
\end{solution}
...
又因为 \(D\) 是 \(BC\) 的中点，所以 \(AD\perp BC\). 由 \(l\parallel BC\)，得 \(l\perp AD\). 同时 \(AA_{1}\perp\) 平面 \(ABC\)，而 \(l\) 在平面 \(ABC\) 内，所以 \(l\perp AA_{1}\). 直线 \(AD\) 与 \(AA_{1}\) 是平面 \(ADD_{1}A_{1}\) 内相交的两条直线，故 \(l\perp\) 平面 \(ADD_{1}A_{1}\).
...
\(n^2=k^2m^2=\frac35(m^2+12)\). 由于圆在 \(x\) 轴上方，且线段 \(MN\) 上的点满足 \(n>0\)，所以
\(n=\sqrt{\frac35(m^{2}+12)}\).
由题设分母不为零，知 \(m\neq0\). 再由 \(|k|>\sqrt3\) 及上式中的 \(k^2\) 可得 \(0<m^2<3\)，故定义域为 \(-\sqrt3<m<0\) 或 \(0<m<\sqrt3\). 当 \(k>\sqrt3\) 时，弦端点横坐标同为正；当 \(k<-\sqrt3\) 时，弦端点横坐标同为负，且 \(n=km>0\)，所以上述定义域中的点确实对应于线段 \(MN\) 上的点.
\end{enumerate}
\end{solution}
...
\(g'(t)=\frac{2t^2+2t-1}{2t^3}>0\). 所以 \(g\) 递增，且
\(\lim_{t\to\frac12+}g(t)=-1-\ln2\)，\(\lim_{t\to+\infty}g(t)=+\infty\). 因此 \(a\) 的取值范围为 \((-1-\ln2,+\infty)\).
\end{enumerate}
\end{solution}
