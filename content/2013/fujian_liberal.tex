...
\begin{solution}
短答案：\(a_1=-1\) 或 \(a_1=2\)；\(a_1\) 的取值范围为 \((-5,2)\).
\end{solution}
...
\begin{solution}
短答案：正视图的竖直尺寸为 \(PD=4\sqrt{3}\)，水平尺寸为 \(AB=6\)；\(DM\parallel\) 平面 \(PBC\)；\(V_{D-PBC}=8\sqrt{3}\).
\end{solution}
...
\begin{solution}
短答案：第（1）问概率为 \(\frac{7}{10}\)；第（2）问列联表为
...
\(\chi^2=\frac{25}{14}\approx1.79<2.706\)，没有 \(90\%\) 的把握认为生产能手与工人所在的年龄组有关.
\end{solution}
...
\begin{solution}
短答案：第（1）问 \(|MN|=2\)；第（2）问圆 \(C\) 的半径为 \(\frac{\sqrt{33}}{2}\).
\end{solution}
...
\begin{solution}
短答案：第（1）问 \(PM=1\) 或 \(PM=3\)；第（2）问当 \(\angle POM=30^{\circ}\) 时，\(\triangle OMN\) 的面积最小，最小值为 \(8-4\sqrt{3}\).
\end{solution}
...
\begin{solution}
短答案：第（1）问 \(a=e\)；第（2）问当 \(a\leqslant0\) 时函数无极值，当 \(a>0\) 时函数在 \(x=\ln a\) 处取得极小值，极小值为 \(\ln a\)；第（3）问 \(k\) 的最大值为 \(1\).
\end{solution}
