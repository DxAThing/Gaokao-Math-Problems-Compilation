...
\par\noindent\hfill\bitmapinclude[max width=0.26\linewidth,max totalheight=5cm]{img/2013/shanghai_spring/q25_fig1.png}\hfill\par
...
\par\noindent\hfill\bitmapinclude[max width=0.22\linewidth,max totalheight=3cm]{img/2013/shanghai_spring/q26_fig1.png}\hfill\par
...
\par\noindent\hfill\bitmapinclude[max width=0.55\linewidth,max totalheight=3.5cm]{img/2013/shanghai_spring/q30_fig1.png}\hfill\par
...
健身房占地面积为 \(y\) 平方米.因为 \(\triangle CFP\sim\triangle CBA\)，\(\frac{FP}{BA}=\frac{CF}{CB}\)，即 \(\frac{x}{40}=\frac{50-BF}{50}\)，求得 \(BF=50-\frac{5}{4}x\).
...
\]
且 \(a_1=S_1=0\). 因此 \(b_n=2^{a_n}=2^{-2n+2}=\bigl(\frac14\bigr)^{n-1}\). 于是 \(\displaystyle\lim_{n\to\infty}(b_1+b_2+\cdots+b_n)=\frac{1}{1-\frac14}=\frac43\).
\end{solution}

\begin{solution}
\textbf{28.}（1）设椭圆 \(C\) 的方程为 \(\frac{x^2}{a^2}+\frac{y^2}{b^2}=1\;(a>b>0)\)，则 \(c=1\)，\(a^2=b^2+c^2\). 由 \(\triangle F_1B_1B_2\) 为等边三角形，得 \(F_1B_1^2=1+b^2=(2b)^2\)，所以 \(b^2=\frac13\)，\(a^2=\frac43\). 故 \(\frac{x^2}{4/3}+\frac{y^2}{1/3}=1\).

（2）短轴长为 \(2\)，故 \(b=1\)，\(a^2=2\). 椭圆方程为 \(\frac{x^2}{2}+y^2=1\). 设直线 \(l\) 的方程为 \(y=k(x-1)\). 由 \(P,Q\) 在椭圆上且 \(\overrightarrow{F_1P}\perp \overrightarrow{F_1Q}\)，可得 \(k=\pm\frac{\sqrt7}{7}\). 故直线 \(l\) 的方程为 \(x+\sqrt{7}\,y-1=0\) 或 \(x-\sqrt{7}\,y-1=0\).
\end{solution}

\begin{solution}
\textbf{29.}（1）设 \(P(x,y)\)，\(A(x_A,y_A)\). 由 \(\overrightarrow{AP}=-2\overrightarrow{FA}\) 得 \(x_A=2-x\)，\(y_A=-y\). 又 \(A\) 在抛物线 \(C:y^2=4x\) 上，故 \(y_A^2=4x_A\)，即 \(y^2=8-4x\).

（2）设 \(Q(t,0)\). 其关于 \(y=2x\) 的对称点为 \(Q'\bigl(-\frac{3}{5}t,\frac{4}{5}t\bigr)\). 若 \(Q'\) 在 \(C\) 上，则 \(\bigl(\frac{4}{5}t\bigr)^2=4\bigl(-\frac{3}{5}t\bigr)\)，解得 \(t=0\) 或 \(t=-\frac{15}{4}\). 所以 \(Q\) 的坐标为 \((0,0)\) 和 \(\bigl(-\frac{15}{4},0\bigr)\).
\end{solution}

\begin{solution}
\textbf{30.}（1）设 \(A(0,t)\). 因为 \(P_n(2^{n-1},0)\)，所以 \(\tan\theta_3=\frac{4t}{t^2+32}=\frac13\). 解得 \(t=4\) 或 \(t=8\). 故点 \(A\) 的坐标为 \((0,4)\) 或 \((0,8)\).

（2）此时 \(A(0,8\sqrt{2})\). 设 \(P_n(2^{n-1},0)\)，则 \(\tan\theta_n=\frac{4\sqrt{2}\cdot 2^{n-1}}{2^{2n-2}+64}\le \frac{\sqrt{2}}{4}\)，当且仅当 \(n=4\) 时取等号.故 \(\theta_n\) 的最大值为 \(\arctan\frac{\sqrt{2}}{4}\)，相应 \(n=4\).
\end{solution}

\begin{solution}
\textbf{31.}（1）平移后函数解析式为 \(y=(x+1)^3-3(x+1)^2+2=x^3-3x\). 由于 \(x^3-3x\) 是奇函数，所以函数 \(g(x)\) 的图像对称中心为 \((1,-2)\).
