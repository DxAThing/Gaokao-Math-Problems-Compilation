...
\begin{enumerate}
\item \(B=\frac{\pi}{4}\).
\item \(\triangle ABC\) 面积的最大值为 \(1+\sqrt{2}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(BC_1\parallel\) 平面 \(A_1CD\).
\item 二面角 \(D-A_1C-E\) 的正弦值为 \(\frac{\sqrt{6}}{3}\).
\end{enumerate}
\end{solution}
...
\item 利润 \(T\) 不少于 57000 元的概率估计值为 \(0.7\).
...
\begin{solution}
\begin{enumerate}
\item \(M:\frac{x^2}{6}+\frac{y^2}{3}=1\).
\item 四边形 \(ACBD\) 面积的最大值为 \(\frac{8\sqrt{6}}{3}\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(m=1\). \(f(x)\) 在 \((-1,0)\) 上单调递减，在 \((0,+\infty)\) 上单调递增.
\item 当 \(m\leqslant2\) 时，\(f(x)>0\).
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(CA\) 是 \(\triangle ABC\) 外接圆的直径.
\item 过 \(B\)，\(E\)，\(F\)，\(C\) 四点的圆的面积与 \(\triangle ABC\) 外接圆面积的比值为 \(\frac{1}{2}\).
\end{enumerate}
\end{solution}
...
\item \(d=\sqrt{2+2\cos\alpha}=2\left|\cos\frac{\alpha}{2}\right|\). 当 \(\alpha=\pi\) 时 \(M\) 在坐标原点，故轨迹过坐标原点.
\end{enumerate}
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item \(ab+bc+ca\leqslant\frac13\).
\item \(\frac{a^2}{b}+\frac{b^2}{c}+\frac{c^2}{a}\geqslant1\).
\end{enumerate}
\end{solution}
