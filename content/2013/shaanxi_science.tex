...
\item 设事件 \(A\)，\(B\)，\(C\) 分别表示观众甲、乙、丙选中 \(3\) 号歌手. 则 \(P(A)=\dfrac23\)，\(P(B)=P(C)=\dfrac35\)，且三个事件相互独立. 随机变量 \(X\) 的可能取值为 \(0\)，\(1\)，\(2\)，\(3\). 有 \(P(X=0)=\dfrac13\left(\dfrac25\right)^2=\dfrac4{75}\)，\(P(X=1)=\dfrac23\left(\dfrac25\right)^2+\dfrac13\cdot2\cdot\dfrac35\cdot\dfrac25=\dfrac4{15}\)，\(P(X=2)=\dfrac23\cdot2\cdot\dfrac35\cdot\dfrac25+\dfrac13\left(\dfrac35\right)^2=\dfrac{11}{25}\)，\(P(X=3)=\dfrac23\left(\dfrac35\right)^2=\dfrac6{25}\). 因此分布列为
...
\begin{solution}
\begin{enumerate}
\item 设动圆圆心为 \(O_1(u,v)\)，半径为 \(R\). 设弦 \(MN\) 的中点为 \(H\)，则 \(O_1H\perp MN\)，且 \(H\) 在 \(y\) 轴上. 因为弦长为 \(8\)，所以 \(MH=4\)，从而 \(R^2=O_1H^2+MH^2=u^2+16\). 又圆过点 \(A(4,0)\)，故 \(R^2=(u-4)^2+v^2\). 两式相等得 \((u-4)^2+v^2=u^2+16\)，即 \(v^2=8u\). 当 \(u=0\) 时圆心为原点，半径为 \(4\)，也满足条件，所以轨迹 \(C\) 的方程为 \(y^2=8x\).
...
\item 由 \(\e^x=mx^2\) 得 \(m=\dfrac{\e^x}{x^2}\). 令 \(\varphi(x)=\dfrac{\e^x}{x^2}\)，则 \(\varphi'(x)=\dfrac{\e^x(x-2)}{x^3}\). 所以 \(\varphi\) 在 \((0,2)\) 上递减，在 \((2,+\infty)\) 上递增，且最小值为 \(\varphi(2)=\dfrac{\e^2}{4}\). 因此，当 \(0<m<\dfrac{\e^2}{4}\) 时没有公共点；当 \(m=\dfrac{\e^2}{4}\) 时有一个公共点；当 \(m>\dfrac{\e^2}{4}\) 时有两个公共点.
\item 函数 \(f(x)=\e^x\) 在 \([a,b]\) 上严格凸. 对任意 \(x\in(a,b)\)，严格凸性给出 \(f(x)<\dfrac{b-x}{b-a}f(a)+\dfrac{x-a}{b-a}f(b)\). 两端积分并除以 \(b-a>0\)，得 \(\dfrac1{b-a}\int_a^b f(x)\,\mathrm dx<\dfrac{f(a)+f(b)}2\). 又 \(\int_a^b f(x)\,\mathrm dx=f(b)-f(a)\)，所以 \(\dfrac{f(b)-f(a)}{b-a}<\dfrac{f(a)+f(b)}2\).
\end{enumerate}
\end{solution}
