...
\bitmapfigure[max width=0.88\linewidth,max totalheight=6.8cm]{img/2013/shanghai_liberal/q9_options.png}
\end{problem}

\begin{problem}
已知圆柱 \(\Omega\) 的母线长为 \(l\)，底面半径为 \(r\)，\(O\) 是上底面圆心，\(A, B\) 是下底面圆周上两个不同的点，\(BC\) 是母线，如图. 若直线 \(OA\) 与 \(BC\) 所成角的大小为 \(\frac{\pi}{6}\)，则 \(\frac{l}{r} = \fillinblank{}.\)

\bitmapfigure[max width=0.36\linewidth,max totalheight=4.8cm]{img/2013/shanghai_liberal/q10_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.4cm]{img/2013/shanghai_liberal/q19_fig1.png}
\end{problem}
...
(2) 令 \(\omega = 2\)，将函数 \(y = f(x)\) 的图象向左平移 \(\frac{\pi}{6}\) 个单位，再向上平移 \(1\) 个单位，得到函数 \(y = g(x)\) 的图象. 对任意的 \(a \in \R\)，求 \(y = g(x)\) 在区间 \([a, a + 10\pi]\) 上零点个数的所有可能值.
\end{problem}
...
\bitmapfigure[max width=0.38\linewidth,max totalheight=5.2cm]{img/2013/shanghai_liberal/q23_fig1.png}
...
(2) 设 \(a > -1\)，且当 \(x \in \left[-\frac{a}{2}, \frac{1}{2}\right)\) 时，\(f(x) \leqslant g(x)\)，求 \(a\) 的取值范围.
\end{problem}
