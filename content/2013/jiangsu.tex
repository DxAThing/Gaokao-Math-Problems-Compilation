...
\begin{optionlist}
\textbf{
\item[\textbf{A.}] }如图，\(AB\) 和 \(BC\) 分别与圆 \(O\) 相切于点 \(D,C\)，\(AC\) 经过圆心 \(O\)，且 \(BC=2OC\). 求证：\(AC=2AD\).

\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2013/jiangsu/q21_fig1.png}

\textbf{
\item[\textbf{B.}] }已知矩阵 \(A=\begin{bmatrix}-1&0\\0&2\end{bmatrix}\)，\(B=\begin{bmatrix}1&2\\0&6\end{bmatrix}\)，求矩阵 \(A^{-1}B\).

\textbf{
\item[\textbf{C.}] }在平面直角坐标系 \(xOy\) 中，直线 \(l\) 的参数方程为 \(x=t+1\)，\(y=2t\)（\(t\) 为参数），曲线 \(C\) 的参数方程为 \(x=2\tan^2\theta\)，\(y=2\tan\theta\)（\(\theta\) 为参数）. 试求直线 \(l\) 和曲线 \(C\) 的普通方程，并求出它们的公共点的坐标.

\textbf{
\item[\textbf{D.}] }已知 \(a\geqslant b>0\)，求证：\(2a^3-b^3\geqslant 2ab^2-a^2b\).
\end{optionlist}
\end{problem}
