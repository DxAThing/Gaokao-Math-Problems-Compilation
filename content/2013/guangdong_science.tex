...
\begin{problem}
若复数 \(z\) 满足 \(iz = 2 + 4\i\)，则在复平面内，\(z\) 对应的点的坐标是（\quad）
...
\bitmapfigure[max width=0.62\linewidth,max totalheight=5.0cm]{img/2013/guangdong_science/q5_fig1.png}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=4.8cm]{img/2013/guangdong_science/q11_fig1.png}
\end{problem}
...
\begin{problem}
给定区域 \(D: \left\{ \begin{array}{l} x + 4y \geqslant 4, \\ x + y \leqslant 4, \\ x \geqslant 0, \end{array} \right.\) 令点集 \(T = \{(x_0, y_0) \in D \mid x_0, y_0 \in \Z,\ (x_0, y_0)\text{ 是 } z = x + y \text{ 在 } D \text{ 上取得最大值或最小值的点}\}\)，则 \(T\) 中的点共确定 \fillinblank{} 条不同的直线.
\end{problem}

\begin{problem}
已知曲线 \(C\) 的参数方程为 \(\left\{ \begin{array}{l} x = \sqrt{2} \cos t, \\ y = \sqrt{2} \sin t, \end{array} \right.\) （\(t\) 为参数），\(C\) 在点 \((1,1)\) 处的切线为 \(l\)，以坐标原点为极点，\(x\) 轴的正半轴为极轴建立极坐标系，则 \(l\) 的极坐标方程为 \fillinblank{}.
\end{problem}

\begin{problem}
如图，\(AB\) 是圆 \(O\) 的直径，点 \(C\) 在圆 \(O\) 上，延长 \(BC\) 到 \(D\) 使 \(BC = CD\)，过 \(C\) 作圆 \(O\) 的切线交 \(AD\) 于 \(E\). 若 \(AB = 6\)，\(ED = 2\)，则 \(BC = \fillinblank{}\).
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2013/guangdong_science/q15_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.34\linewidth,max totalheight=3.0cm]{img/2013/guangdong_science/q17_fig1.png}
...
\bitmapfigure[max width=0.66\linewidth,max totalheight=5.0cm]{img/2013/guangdong_science/q18_fig1.png}
