...
若 \(\operatorname{tg} \alpha = -2\)，且 \(\sin \alpha < 0\)，则 \(\cos \alpha\) 的值是 \fillinblank{}.
...
（1）在直角坐标系内，方程 \(x=3\operatorname{ctg}\varphi,\ y=2\csc\varphi\)（\(\varphi\) 是参数）表示什么曲线？画出它的图形.

（2）在极坐标系内，方程 \(\frac{2\sin\theta}{\rho}=1\) 表示什么曲线？画出它的图形.

（1）方程表示的曲线是 \fillinblank{}.

（2）方程表示的曲线是 \fillinblank{}.

\end{problem}

\begin{problem}

（1）求 \(\left(x^{3}-\frac{1}{x^{2}}\right)^{15}\) 的展开式中的常数项.
