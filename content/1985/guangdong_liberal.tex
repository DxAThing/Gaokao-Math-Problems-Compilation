...
\begin{solution}
当 \(n=1\) 时，左边 \(=1=\) 右边，所以原等式成立.

假设当 \(n=k\)（\(k\geqslant 1\)）时等式成立，即
...
\end{gathered}
\]
这就是说，原等式对于 \(n=k+1\) 也成立. 根据上述两步，原等式对于任何自然数 \(n\) 都成立.
\end{solution}
...
\begin{solution}
\begin{enumerate}
\item 【证法一】因为 \(ABC-A_{1}B_{1}C_{1}\) 是直三棱柱，因此 \(AA_{1}\perp\) 底面 \(ABC\). 因为 \(CE\subset\) 平面 \(ABC\)，因此 \(CE\perp AA_{1}\). 又 \(E\) 是等腰三角形 \(ABC\) 底边 \(AB\) 的中点，因此 \(CE\perp AB\)，故 \(CE\perp\) 平面 \(ABB_{1}A_{1}\). 因为 \(AB_{1}\subset\) 平面 \(ABB_{1}A_{1}\)，因此 \(CE\perp AB_{1}\).
