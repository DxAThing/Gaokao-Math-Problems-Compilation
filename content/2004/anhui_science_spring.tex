...
\begin{solution}
设椭圆的半焦距为 \(c\)，则 \(F_1F_2=2c\). 因为 \(F_1F_2\) 在 \(x\) 轴上，而 \(MF_1\perp x\) 轴，所以三角形 \(F_1MF_2\) 在 \(F_1\) 处为直角. 又 \(\angle F_1MF_2=60^\circ\)，设 \(MF_1=u\)，则 \(MF_2=2u\)，且
...
\begin{solution}
二面角为 \(120^\circ\)，故两平面的锐二面角为 \(60^\circ\). 直线 \(m\) 垂直于平面 \(\alpha\)，因此它与平面 \(\beta\) 所成的角为 \(90^\circ-60^\circ=30^\circ\). 在平面 \(\beta\) 内，取其内的直线方向逐渐转动，直线与 \(m\) 所成的角的最小值为 \(30^\circ\)，当该直线垂直于 \(m\) 在平面 \(\beta\) 内的射影时取最大值 \(90^\circ\). 故取值范围为 \([30^\circ,90^\circ]\)，选 D.
\end{solution}
...
\begin{solution}
设动点为 \(P(x,y)\). 点 \(P\) 到两直线的距离分别为 \(d_1=\frac{|kx-y|}{\sqrt{1+k^2}}\)，\(d_2=\frac{|kx+y|}{\sqrt{1+k^2}}\).
\begin{enumerate}
\item 由平方和公式，得
\[
d_1^2+d_2^2
...
\begin{solution}
建立空间直角坐标系，使底面 \(ABC\) 在 \(z=0\) 平面内，取 \(A(0,0,0)\)，\(C(a,0,0)\)，\(B\left(\dfrac a2,\dfrac{\sqrt3}{2}a,0\right)\). 由于底面为正三角形，且侧面 \(A_1ACC_1\) 垂直于底面并以 \(AC\) 为交线，可令 \(A_1=(u,0,v)\)，并取 \(z\) 轴指向 \(A_1\)，故 \(v>0\). 由侧棱长为 \(a\)，有
\[
u^2+v^2=a^2
\]
...
\]
结合 \(u^2+v^2=a^2\)，化简得 \(u=\dfrac a2\)，进而 \(v^2=\dfrac34a^2\). 由于 \(v>0\)，故 \(v=\dfrac{\sqrt3}{2}a\)，从而
...
\]
于是 \(\overrightarrow{AC}\cdot\overrightarrow{BC_1}=a^2\)，
\(|\overrightarrow{AC}|=a\)，
...
\]
其法线分别为 \(y-a=-\frac{x+2}{2\sqrt D}\) 和 \(y-a=\frac{x+2}{2\sqrt D}\).
