...
\begin{problem}
已知向量 \(a\)，\(b\) 满足：\(|a| = 1, |b| = 2, |a - b| = 2\)，则 \(\left|a+b\right| =\)（\quad）
...
\begin{problem}
已知 8 支球队中有 3 支弱队，以抽签方式将这 8 支球队分为 \(A\)，\(B\) 两组，每组 4 支，求：

\begin{enumerate}
\item \(A\)，\(B\) 两组中有一组恰有两支弱队的概率；
\item \(A\) 组中至少有两支弱队的概率.
\end{enumerate}
\end{problem}
...
\item 求面 \(B_{1}BD\) 与 \(CBD\) 所成二面角的大小.
\end{enumerate}

\begin{center}
\bitmapinclude[max width=0.42\linewidth,max totalheight=4.8cm]{img/2004/jilin_liberal/q18_fig1.png}
\end{center}
\end{problem}
