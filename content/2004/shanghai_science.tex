...
\begin{problem}
设等比数列 \(\{a_{n}\}\)（\(n \in \mathbf{N}\)）的公比 \(q = -\frac{1}{2}\)，且 \(\displaystyle\lim_{n \to \infty} (a_{1} + a_{3} + a_{5} + \cdots + a_{2n-1}) = \frac{5}{3}\)，则 \(a_{1} =\fillinblank{}\).
\end{problem}

\begin{problem}
设奇函数 \( f(x) \) 的定义域为 \([-5, 5]\). 若当 \( x \in [0, 5] \) 时，\( f(x) \) 的图象如右图，则不等式 \( f(x) < 0 \) 的解是\fillinblank{}\fillinblank{}.

\bitmapfigure[max width=0.34\linewidth,max totalheight=4.2cm]{img/2004/shanghai_science/q5_fig1.png}
\end{problem}

\begin{problem}
已知点 \(A(1, -2)\)，若向量 \(\overrightarrow{AB}\) 与 \(\bs{a} = (2, 3)\) 同向，\(\left|\overrightarrow{AB}\right| = 2\sqrt{13}\)，则点 \(B\) 的坐标为\fillinblank{}\fillinblank{}.
\end{problem}
...
\begin{circlelist}[start=1,itemsep=0pt,topsep=0pt,parsep=0pt,partopsep=0pt]
\item \(S_{1}\) 与 \(S_{2}\).
\end{circlelist}
&
\begin{circlelist}[start=2,itemsep=0pt,topsep=0pt,parsep=0pt,partopsep=0pt]
\item \(a_{2}\) 与 \(S_{3}\).
\end{circlelist}
\\
\begin{circlelist}[start=3,itemsep=0pt,topsep=0pt,parsep=0pt,partopsep=0pt]
\item \(a_{1}\) 与 \(a_{n}\).
\end{circlelist}
&
\begin{circlelist}[start=4,itemsep=0pt,topsep=0pt,parsep=0pt,partopsep=0pt]
\item \(q\) 与 \(a_{n}\).
\end{circlelist}
\end{tabular}
...
\bitmapfigure[max width=0.35\linewidth,max totalheight=4.8cm]{img/2004/shanghai_science/q18_fig1.png}
\end{problem}
...
\bitmapfigure[max width=0.42\linewidth,max totalheight=4.8cm]{img/2004/shanghai_science/q21_fig1.png}
\end{problem}
