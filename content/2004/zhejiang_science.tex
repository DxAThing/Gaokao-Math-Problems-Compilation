...
\begin{solution}
单位圆的半径为 \(1\)，所以点 \(P\) 运动的弧长等于所对应的圆心角，即
\(\theta=\frac{2\pi/3}{1}=\frac{2\pi}{3}\). 从点 \((1,0)\) 沿逆时针方向转过此角后，
\(Q=\left(\cos\frac{2\pi}{3},\sin\frac{2\pi}{3}\right)=\left(-\frac{1}{2},\frac{\sqrt{3}}{2}\right)\)，故选 A.
\end{solution}
...
\begin{solution}
设等差数列的首项为 \(a_1\). 由公差为 \(2\)，得
\(a_3=a_1+4\)，\(a_4=a_1+6\). 因为 \(a_1\)，\(a_3\)，\(a_4\) 成等比数列，所以
\((a_1+4)^2=a_1(a_1+6)\)，即
\(a_1^2+8a_1+16=a_1^2+6a_1\)，从而 \(a_1=-8\)，故
\(a_2=a_1+2=-6\)，选 B.
\end{solution}
...
\begin{solution}
将原曲线上点 \((x,y)\) 关于直线 \(x=2\) 对称，所得点的横坐标为 \(4-x\)，纵坐标仍为 \(y\). 记对称后的横坐标为 \(X\)，则 \(x=4-X\). 代入原方程 \(y^2=4x\)，得
\(y^2=4(4-X)=16-4X\)，故对称曲线方程为 \(y^2=16-4x\)，选 C.
\end{solution}
...
\begin{solution}
令 \(u=x+y-3\)，\(v=x-2y\)，则 \(u\geqslant0\)，\(v\geqslant0\). 由
\(x+y=3+u\) 与 \(x-2y=v\) 解得
\(y=1+\frac{u-v}{3}\)，\(x=2+\frac{2u+v}{3}\). 因此
\(z=x-y=1+\frac{u+2v}{3}\geqslant1\). 当 \(u=v=0\)，即 \(x=2\)，\(y=1\) 时取等号，所以 \(z\) 的最小值为 \(1\)，选 A.
\end{solution}
...
\begin{solution}
有 \(z_1z_2=(3+4\i)(t+\i)=(3t-4)+(3+4t)\i\).
要使 \(z_1z_2\) 为实数，必须有虚部 \(3+4t=0\)，所以 \(t=-\frac{3}{4}\)，选 D.
\end{solution}
...
\begin{solution}
在展开式中取 \(k\) 个 \(\frac{2}{\sqrt[3]{x}}\)，取 \(n-k\) 个 \(\sqrt{x}\)，所得一般项中 \(x\) 的指数为
\(\frac{n-k}{2}-\frac{k}{3}=\frac{n}{2}-\frac{5k}{6}\). 存在常数项的条件是
\(\frac{n}{2}-\frac{5k}{6}=0\)，即 \(3n=5k\). 因此 \(n\) 必须是 \(5\) 的倍数，给出的选项中只有 \(n=10\) 符合；此时 \(k=6\)，且 \(0\leqslant k\leqslant n\)，故选 C.
\end{solution}
...
\begin{solution}
在三角形中 \(0<A<\pi\)，故
\(\sin A>\frac12\iff\frac{\pi}{6}<A<\frac{5\pi}{6}\). 这说明 \(\sin A>\frac12\) 必能推出 \(A>\frac{\pi}{6}\)，但反过来不成立，例如 \(A=\frac{5\pi}{6}\) 时仍有 \(A>\frac{\pi}{6}\)，而 \(\sin A=\frac12\). 所以“\(A>30^{\circ}\)”是“\(\sin A>\frac12\)”的必要而不充分条件，选 B.
\end{solution}
...
\begin{solution}
设椭圆的半焦距为 \(c\). 抛物线 \(y^2=2bx\) 可写成 \(y^2=4px\)，所以其焦点横坐标为
\(p=\frac b2\). 由于该焦点在 \(F_1F_2\) 的中点右侧，故
\(\frac{c+b/2}{c-b/2}=\frac53\). 于是
\(3\left(c+\frac b2\right)=5\left(c-\frac b2\right)\)，解得 \(c=2b\).

又由椭圆关系 \(a^2=b^2+c^2\)，得 \(a^2=5b^2\)，所以离心率
\(e=\frac ca=\frac{2}{\sqrt5}=\frac{2\sqrt5}{5}\)，选 D.
\end{solution}
...
\begin{solution}
正三棱柱的侧棱垂直于底面. 平面 \(AA_1C_1C\) 包含直线 \(AC\) 和侧棱方向，因此 \(AD\) 垂直于该平面的分量，等于等边三角形 \(ABC\) 中从 \(B\) 到 \(AC\) 的高，即 \(\frac{\sqrt3}{2}\). 又
\(AD=\sqrt{AB^2+BD^2}=\sqrt{1^2+1^2}=\sqrt2\). 设直线与平面所成的角为 \(\alpha\)，则
\(\sin\alpha=\frac{\sqrt3/2}{\sqrt2}=\frac{\sqrt6}{4}\)，故选 D.
\end{solution}
...
\begin{solution}
由图可知，\(f'(x)>0\) 时 \(x<0\) 或 \(x>2\)，\(f'(x)<0\) 时 \(0<x<2\)，且 \(f'(0)=f'(2)=0\). 因此 \(f(x)\) 在 \((-\infty,0)\) 上递增，在 \((0,2)\) 上递减，在 \((2,+\infty)\) 上递增，所以 \(x=0\) 是极大值点，\(x=2\) 是极小值点. 四个图象中只有第三个图象符合，故选 C.
\end{solution}
...
\begin{solution}
设方程 \(x-f(g(x))=0\) 的一个实数解为 \(x_0\)，令 \(y_0=g(x_0)\)，则 \(x_0=f(y_0)\)，从而 \(g(f(y_0))=g(x_0)=y_0\). 因此，函数 \(g\circ f\) 必有实数不动点，即方程 \(g(f(x))=x\) 必有实数解. 逐项检验选项对应的函数：
\(x^2+x-\frac{1}{5}=x\) 化为 \(x^2-\frac{1}{5}=0\)，有实数解；\(x^2+x+\frac{1}{5}=x\) 化为 \(x^2+\frac{1}{5}=0\)，无实数解；\(x^2-\frac{1}{5}=x\) 的判别式为 \(1+\frac{4}{5}>0\)，有实数解；\(x^2+\frac{1}{5}=x\) 的判别式为 \(1-\frac{4}{5}>0\)，也有实数解. 因此不可能的是 B.
\end{solution}
...
\begin{solution}
当 \(x\geqslant-2\) 时，\(f(x+2)=1\)，不等式化为
\(x+(x+2)\leqslant5\)，即 \(x\leqslant\frac32\). 结合 \(x\geqslant-2\)，此时解为
\(\left[-2,\frac32\right]\).

当 \(x<-2\) 时，\(f(x+2)=-1\)，不等式化为
\(x-(x+2)=-2\leqslant5\)，恒成立，所以此时解为 \((-\infty,-2)\). 合并得解集
...
\begin{solution}
设向正方向跳的次数为 \(p\)，向负方向跳的次数为 \(q\). 由
\(p+q=5\) 以及最终横坐标 \(p-q=3\)，解得 \(p=4\)，\(q=1\). 只需确定唯一一次向负方向跳动发生在 \(5\) 次跳动中的哪一次，因此运动方法共有
\(\mathrm C_5^1=5\) 种.
\end{solution}
...
\begin{solution}
设点 \(A\) 在平面 \(\beta\) 内的射影与点 \(B\) 在平面 \(\alpha\) 内的射影均为 \(C\). 若 \(AC\neq0\)，则 \(A,C\in\alpha\) 且 \(AC\perp\beta\)，所以平面 \(\alpha\) 与平面 \(\beta\) 垂直. 若 \(AC=0\)，则 \(A=C\in l\). 又因 \(PA=1\)，\(PB=2\)，所以 \(B\neq C\)；此时 \(B,C\in\beta\) 且 \(BC\perp\alpha\)，仍可得 \(\alpha\perp\beta\). 于是可建立直角坐标系，使 \(l\) 为 \(z\) 轴，\(\alpha\) 为 \(xOz\) 平面，\(\beta\) 为 \(yOz\) 平面. 设 \(P=(x,y,z)\)，则
\(PA=|y|=1\)，\(PB=|x|=2\). 点 \(P\) 到 \(l\) 的距离为
...
\]
因此所求的值为 \(\frac23-\frac79=-\frac19\).
\item 由余弦定理及 \(\cos A=\frac13\)，得
\[
3=a^2=b^2+c^2-2bc\cos A=b^2+c^2-\frac23bc
=(b-c)^2+\frac43bc.
\]
所以 \(bc=\frac34\left(3-(b-c)^2\right)\leqslant\frac94\). 当 \(b=c\) 时等号成立；此时由上式得 \(b=c=\frac32\)，确有 \(a=\sqrt3\). 因此 \(bc\) 的最大值为 \(\frac94\).
\end{enumerate}
\end{solution}
...
\(\frac3{10}\)，\(\frac4{10}\)，\(\frac3{10}\). 由于取球后放回，两次取球相互独立. 随机变量 \(\varepsilon\) 的可能取值为 \(2\)，\(3\)，\(4\)，\(6\)，\(7\)，\(10\). 由各标号组合及独立性，得
\(P(\varepsilon=2)=\frac3{10}\cdot\frac3{10}=\frac9{100}\)，
\(P(\varepsilon=3)=2\cdot\frac3{10}\cdot\frac4{10}=\frac{24}{100}\)，
\(P(\varepsilon=4)=\frac4{10}\cdot\frac4{10}=\frac{16}{100}\)，
\(P(\varepsilon=6)=2\cdot\frac3{10}\cdot\frac3{10}=\frac{18}{100}\)，
\(P(\varepsilon=7)=2\cdot\frac4{10}\cdot\frac3{10}=\frac{24}{100}\)，
\(P(\varepsilon=10)=\frac3{10}\cdot\frac3{10}=\frac9{100}\). 所以分布列为
\begin{center}
...
\end{tabular}
\end{center}
\item 设两次取到球的标号分别为 \(\varepsilon_1\)，\(\varepsilon_2\)，则
\(E(\varepsilon_1)=E(\varepsilon_2)=1\cdot\frac3{10}+2\cdot\frac4{10}+5\cdot\frac3{10}=\frac{13}{5}\). 因此
\(E(\varepsilon)=E(\varepsilon_1+\varepsilon_2)=E(\varepsilon_1)+E(\varepsilon_2)=\frac{26}{5}\).
\end{enumerate}
\end{solution}
...
\begin{enumerate}
\item 取直角坐标系，使
\(A=(-1,0,0)\)，\(C=(1,0,0)\)，正方形 \(ABCD\) 在 \(xOy\) 平面内，矩形 \(ACEF\) 在 \(xOz\) 平面内. 因为 \(AC=2\)，\(AB=\sqrt2\)，所以
\(B=(0,1,0)\)，\(D=(0,-1,0)\). 又 \(AF=1\)，从而
\(E=(1,0,1)\)，\(F=(-1,0,1)\)，\(M=(0,0,1)\).
\(\overrightarrow{AM}=(1,0,1)\)，\(\overrightarrow{BD}=(0,-2,0)\)，\(\overrightarrow{BE}=(1,-1,1)\). 平面 \(BDE\) 的一个法向量为
\(\overrightarrow{BD}\times\overrightarrow{BE}=(-2,0,2)\)，且
\((1,0,1)\cdot(-2,0,2)=0\). 因此 \(AM\parallel\) 平面 \(BDE\).
\item 平面 \(ADF\) 与平面 \(BDF\) 的法向量可分别取为
\[
\bs n_1=\overrightarrow{DA}\times\overrightarrow{DF}=(1,1,0),\qquad
\bs n_2=\overrightarrow{DB}\times\overrightarrow{DF}=(2,0,2),
\]
...
\begin{solution}
\begin{enumerate}
\item 因为 \(y'=-\e^{-x}\)，所以曲线在 \(M(t,\e^{-t})\) 处的切线斜率为 \(-\e^{-t}\). 切线 \(l\) 的方程为
\[
y-\e^{-t}=-\e^{-t}(x-t),
\]
即 \(y=\e^{-t}(t+1-x)\).
\item 切线与 \(x\) 轴交于 \((t+1,0)\)，与 \(y\) 轴交于 \((0,(t+1)\e^{-t})\). 由于 \(t\geqslant0\)，这两个截距均非负，故
\[
...
\]
所以 \(S(t)\) 在 \([0,1]\) 上递增，在 \([1,+\infty)\) 上递减，最大值在 \(t=1\) 时取得，为
\(S(1)=\frac2{\e}\).
\end{enumerate}
\end{solution}
...
\]
\item 设双曲线方程为 \(x^2-\frac{y^2}{b^2}=1\)，其中 \(b>0\). 因为 \(M\) 是 \(\triangle APQ\) 的内心，\(AM\) 是顶点 \(A\) 处的内角平分线. 又 \(AM\) 为 \(x\) 轴，所以 \(P\)，\(Q\) 关于 \(x\) 轴对称. 设 \(P=(x,y)\)，\(Q=(x,-y)\)，其中 \(y>0\). 直线 \(PQ\) 为 \(x=x\)，且内心 \(M\) 到该边的距离为 \(1\). 由于 \(M\) 位于三角形内部，\(x>m\)，故
\(x-m=1\)，即 \(x=2+\sqrt2\).

直线 \(AP\) 的斜率为 \(k=\frac{y}{x-1}\). 利用已知的点到直线距离条件以及 \(m-1=\sqrt2\)，有
\[
\frac{|k|\sqrt2}{\sqrt{1+k^2}}=1,
\]
解得 \(k^2=1\). 取 \(y>0\)，得 \(k=1\)，所以 \(y=x-1=1+\sqrt2\). 代入双曲线方程，得
\[
...
\bitmapfigure[width=0.32\linewidth]{img/2004/zhejiang_science/q22_fig1.png}
\end{problem}
...
\[
a_1=\frac12y_1+y_2+y_3=2,\qquad
a_2=\frac12y_2+y_3+y_4=2,\qquad
a_3=\frac12y_3+y_4+y_5=2.
\]
...
\[
y_{n+4}=1-\frac14y_n,\qquad n\in\N^*.
\]
\item 由上式，
\[
