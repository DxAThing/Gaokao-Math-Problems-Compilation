...
\begin{solution}
设 \(\overrightarrow{PA'}=u\overrightarrow{PA}\)，\(\overrightarrow{PB'}=v\overrightarrow{PB}\)，\(\overrightarrow{PC'}=w\overrightarrow{PC}\). 由四面体体积公式，
\[
V_{P-ABC}=\frac{1}{6}\left|\det\left(\overrightarrow{PA},\overrightarrow{PB},\overrightarrow{PC}\right)\right|
\]
将 \(\overrightarrow{PA'}\)，\(\overrightarrow{PB'}\)，\(\overrightarrow{PC'}\) 分别代入同一体积公式，行列式的三列分别乘以 \(u\)，\(v\)，\(w\)，所以
\[
...
\begin{solution}
令 \(y=\ln(\sqrt{x+1}-1)\)，则 \(\e^y=\sqrt{x+1}-1\). 因而 \(\sqrt{x+1}=\e^y+1\)，从而
\[
x=(\e^y+1)^2-1=\e^{2y}+2\e^y
\]
交换 \(x\)，\(y\)，得到 \(f^{-1}(x)=\e^{2x}+2\e^x\).
\end{solution}
...
\]
因 \(\sin\alpha\ne0\)，\(\sin2\alpha\ne0\)，可约去 \(2\sin\alpha\sin2\alpha\)，得 \(\cos\alpha=\cos2\alpha\). 因而
\[
...
\]
所以 \(y=-x\).
正东观测点比正西观测点晚 \(4\mathrm{~s}\) 听到声音，所以
\[
...
\]
所以双曲线方程为 \(\frac{x^2}{680^2}-\frac{y^2}{578000}=1\).
代入 \(y=-x\)，并利用 \(578000=\dfrac54\cdot680^2\)，得
\[
\frac{x^2}{680^2}-\frac{x^2}{578000}=1
\]
化简得 \(x^2=5\cdot680^2\).
由于 \(PB>PA\)，点 \(P\) 在双曲线左支，故 \(x<0\). 所以
\[
...
\item 设 \(x_1=\e^{-m}-m\)，\(x_0=1-m\)，\(x_2=\e^{2m}-m\).
当整数 \(m>1\) 时，三点均在函数定义域内，且 \(x_1<x_0<x_2\). 函数在 \([x_1,x_2]\) 上连续，并且
\(f(x_1)=\e^{-m}>0\)，且 \(f(x_0)=1-m<0\).
由题给定理，方程 \(f(x)=0\) 在 \((x_1,x_0)\) 内至少有一个实根.

又因为对 \(t>0\) 有 \(\e^t>1+t+\dfrac{t^2}{2}\)，所以
\[
f(x_2)=\e^{2m}-3m>1+2m+2m^2-3m=2m^2-m+1>0
\]
再次由题给定理，方程 \(f(x)=0\) 在 \((x_0,x_2)\) 内至少有一个实根. 这两个区间不相交，故所得两个实根不同；结合第一问得到的单调性，每个区间至多有一个实根，所以方程在 \([\e^{-m}-m,\e^{2m}-m]\) 内恰有两个实根.
\end{enumerate}
\end{solution}
