...
因 \(PA\) 的斜率存在，\(x_1\neq x_0\)，所以
\(k_1=\frac{y_1-y_0}{x_1-x_0}=\frac{2p}{y_1+y_0}\). 同样，由
\(y_2^2=2px_2\) 和 \(y_0^2=2px_0\) 得
\((y_2-y_0)(y_2+y_0)=2p(x_2-x_0)\).
因 \(PB\) 的斜率存在，\(x_2\neq x_0\)，从而
\(k_2=\frac{y_2-y_0}{x_2-x_0}=\frac{2p}{y_2+y_0}\).
...
\begin{enumerate}
\item
由 \(f(x)=2f\left(\dfrac{x}{2}\right)\) 取 \(x=0\)，得 \(f(0)=2f(0)\)，所以 \(f(0)=0\). 取 \(x=1\)，得 \(f(1)=2f\left(\dfrac12\right)\)，从而 \(f\left(\dfrac12\right)=\dfrac12\). 再取 \(x=\dfrac12\)，得 \(f\left(\dfrac14\right)=\dfrac12f\left(\dfrac12\right)=\dfrac14\). 下面验证一般项的归纳步骤：
若已知 \(f\left(\dfrac1{2^i}\right)=\dfrac1{2^i}\)，在函数关系中取
\(x=\dfrac1{2^i}\)，就有
\(f\left(\dfrac1{2^{i+1}}\right)=\dfrac12f\left(\dfrac1{2^i}\right)=\dfrac1{2^{i+1}}\).
结合 \(i=1\) 时的 \(f\left(\dfrac12\right)=\dfrac12\)，归纳得
\(f\left(\dfrac1{2^i}\right)=\dfrac1{2^i}\)，\((i=1,2,\cdots)\).
...
由于 \(f\) 是增函数，且 \(f\left(\dfrac1{2^i}\right)=\dfrac1{2^i}\)，当区间内的 \(x\) 从右侧趋近 \(\dfrac1{2^i}\) 时，函数值不能小于端点处的函数值，因此必须有
\(\dfrac1{2^i}\leqslant\dfrac1{2^{i-1}}-\dfrac{k}{2^i}\). 这给出 \(k\leqslant1\). 又因为每个区间上的图象是增函数的一段直线，其斜率必须为正数，故 \(k>0\). 因此 \(k\) 的定义域为 \(0<k\leqslant1\).
