...
\begin{solution}
原不等式的定义域为 \(x\neq-1\). 移项通分，得 \(x+\frac{2}{x+1}-2=\frac{x(x-1)}{x+1}>0\). 临界点为 \(-1\)、\(0\)、\(1\). 作符号判断可知，商在 \((-1,0)\) 和 \((1,+\infty)\) 上为正，因此解集为 \((-1,0)\cup(1,+\infty)\)，
故选 A.
\end{solution}
...
\begin{solution}
第一个命题中，\(\alpha\parallel\beta\) 且 \(m\subset\alpha\)，所以直线 \(m\) 与平面 \(\beta\) 平行，命题正确.

第二个命题不一定正确. 两条平行直线中，一条直线平行于平面 \(\beta\) 时，另一条平行直线仍可能在平面 \(\beta\) 内，因而不满足与平面 \(\beta\) 平行的定义.

第三个命题不一定正确. 分别位于两个平面的两条直线可能相交，也可能平行，并不一定是异面直线.

第四个命题不一定正确. 直线 \(m\parallel\alpha\) 只说明 \(m\) 的方向与平面 \(\alpha\) 平行，不能推出 \(m\) 垂直于与 \(\alpha\) 垂直的平面 \(\beta\).
...
结合 \(|PF_1|=4|PF_2|\)，得
\[
3|PF_2|=2a,\qquad |PF_2|=\frac{2a}{3}.
\]
右焦点到右支上点的距离在顶点处取得最小值 \(c-a\)，所以
\[
...
\]
当 \(P\) 取右顶点、且 \(c=\frac{5a}{3}\) 时等号成立，所以离心率的最大值为 \(\frac{5}{3}\)，选 B.
\end{solution}
...
\begin{solution}
六个直通孔按方向分成三组，每组两个. 由于每个面的两个边长为 \(1\) 的孔从外表面扣除，正方体外表面的面积为 \(S_{\text{外}}=6\left(5^2-2\cdot1^2\right)=138\). 每个通道若单独计算，四个内壁的面积为 \(4\times5\times1=20\). 按图逐一计数各通道交会处被切去的内壁小正方形：沿第一、第二、第三方向的每条通道，分别有 \(4\)、\(6\)、\(8\) 个内壁单位面处在交会处，因此三组两条通道的内壁面积分别为 \((20-4)\times2=16\times2\)、\((20-6)\times2=14\times2\) 和 \((20-8)\times2=12\times2\). 这里交会处的内壁只按实际边界计算一次. 因此孔内各面的总面积为 \(S_{\text{内}}=16\times2+14\times2+12\times2=84\)，
故有孔正方体的表面积为 \(S=S_{\text{外}}+S_{\text{内}}=138+84=222\)，
故选 C.
\end{solution}
...
\]
即 \(t-9(1-t)=0\)，所以 \(t=\frac9{10}\). 因而 \(E=(0,\frac{9a}{10},\frac{3a}{10})\). 由 \(AM=EF=ta\) 且 \(M\in AB\)，得 \(M=(\frac{9a}{10},0,0)\).
...
\begin{solution}
记抛物线顶点为 \(O\). 因为所取直线经过 \(Q(2p,0)\)，且与抛物线交于相异两点，所以它不是 \(x\) 轴，可设其方程为 \(x=2p+\lambda y\). 设它与抛物线的交点为 \(A=(x_1,y_1)\)、\(B=(x_2,y_2)\)，则 \(y_1,y_2\) 是方程
\[
...
\[
\alpha=\frac{5+\sqrt{37}}{6},\qquad \beta=\frac{5-\sqrt{37}}{6}.
\]
