...
\begin{tikzpicture}[baseline=(current bounding box.center), y=1.13cm,
  ...
  \draw[flowbox] (inputTopL) -- (inputTopR) -- (inputBotR) -- (inputBotL) -- (inputTopL);
  ...
  \draw[flowbox] (outputTopL) -- (outputTopR) -- (outputBotR) -- (outputBotL) -- (outputTopL);
  ...
  % Keep the return shaft at least 2.5 mm above updateb's top edge.
  \coordinate (looplevel) at (-3.90,-1.40);
  \draw[line width=0.45pt] (test.west) -- node[flowtext, below] {否}
    (loopleft) -- (looplevel) -- (-0.45,-1.40) -- (0,-1.40);
  % The return arrow is on its horizontal shaft; the main down-shaft keeps one arrowhead.
  \draw[flowarrow] (looplevel) -- (-0.45,-1.40);
  \draw[line width=0.45pt] (-0.45,-1.40) -- (0,-1.40) -- (updateb.north);
