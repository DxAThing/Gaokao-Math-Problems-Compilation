...
\begin{tikzpicture}[
  ...
  \node[process] (updateb) at (0,6.47) {$b=b+2a$};
  \node[process,minimum width=2.90cm] (updatean) at (0,5.24) {$a=b-a,n=n+1$};
  \node[decision] (check) at (0,3.72) {$\left|\dfrac{b^2}{a^2}-2\right|<0.01$};
  \node[inputoutput,minimum width=1.80cm] (output) at (0,1.88) {输出 $n$};
  \node[startstop] (finish) at (0,0.58) {结束};

  \draw[->] (start.south) -- (input.north);
  \draw[->] (input.south) -- (updateb.north);
  \draw[->] (updateb.south) -- (updatean.north);
  \draw[->] (updatean.south) -- (check.north);
  \draw[->] (check.south) -- node[branchlabel,anchor=east,pos=0.18] {是} (output.north);
  \draw[->] (output.south) -- (finish.north);

  % 否分支左回，箭头在输入后、b 更新前的共享入口，和主干下行箭头独立。
  \coordinate (loopJoin) at (0,6.86);
  \coordinate (loopBottom) at (-3.15,3.72);
  \coordinate (loopTop) at (-3.15,6.86);
  \draw (check.west) -- node[branchlabel,below,pos=0.24] {否} (loopBottom)
    -- (loopTop);
  \draw[->] (loopTop) -- (loopJoin);

  \path[use as bounding box] (-3.45,0.25) rectangle (3.45,9.28);
\end{tikzpicture}
