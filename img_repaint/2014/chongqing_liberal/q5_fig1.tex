...
\begin{tikzpicture}[
    ...
    \draw[->] (test.east) -- node[above=4pt,pos=0,anchor=south east] {是} (sum.west);
    \draw[->] (sum.north) -- (update.south);
    % Return arrow lands on the main stem interior with clearance from init;
    % the stem itself remains the single directed init-to-test segment.
    \draw[->] (update.north) -- (3.20,5.90) -- (0,5.90);
\end{tikzpicture}
