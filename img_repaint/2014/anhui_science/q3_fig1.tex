...
% main stem immediately before z=x+y; 否→输出 z→结束. No other connecting segments are present.
...
\begin{tikzpicture}[
  ...
  % Keep the main stem as one directed segment; the return arrow lands on its
  % interior rather than creating an extra graph junction.
  ...
  % The yes branch returns left from y=z to the stem immediately before z=x+y.
  \coordinate (loop-left) at (-3.55,1.42);
  \coordinate (loop-merge) at (-3.55,7.05);
  \draw[->] (equaly.south) -- (0,1.42) -- (loop-left) -- (loop-merge) -- (0,7.05);
