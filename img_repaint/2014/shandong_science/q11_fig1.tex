...
% Labels: 是 follows the downward true branch; 否 sits beside the right false branch.
...
\begin{tikzpicture}[
  ...
  \node[flowbox,minimum width=1.45cm] (init) at (0,6.25) {$n=0$};
  \coordinate (merge) at (0,5.55);
  ...
  \coordinate (loop-entry) at (-3.55,5.55);
  \draw (inc.south) -- (loop-bottom) -- (loop-left) -- (loop-entry);
  % Keep the return arrow tip clear of the degree-3 merge; the short
  % unarrowed continuation preserves the same loop-entry connection.
  \draw[flowarrow] (loop-entry) -- (-0.60,5.55);
  \draw (-0.60,5.55) -- (merge);
