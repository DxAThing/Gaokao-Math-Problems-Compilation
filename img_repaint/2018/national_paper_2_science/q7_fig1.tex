...
\begin{tikzpicture}[
  ...
  % The loop return joins the central path below the i=1 box; a plain
  % spacer segment keeps the return arrowhead away from the shared trunk.
  \coordinate (loopjoin) at (0,5.10);
  \coordinate (testentry) at (1.05,5.10);
  ...
  \draw[->] (increment.west) -- (-4.85,0.35) -- (-4.85,5.10) -- (-1.00,5.10);
  \draw (-1.00,5.10) -- (loopjoin);
  \draw (loopjoin) -- (testentry);
  \draw[->] (testentry) -- (check.north);
