...
\begin{tikzpicture}[
  ...
  line width=0.45pt,
  node distance=0pt,
  % Every node is locally zero-indent and centered in both axes.
  every node/.style={font=\normalsize, text centered, anchor=center,
    execute at begin node=\setlength{\parindent}{0pt}},
  flowbox/.style={draw, minimum height=0.72cm, inner xsep=4pt, inner ysep=2.5pt,
    text centered, anchor=center},
  rounded/.style={flowbox, rounded corners=3pt},
  decision/.style={draw, diamond, aspect=2.7, minimum width=3.5cm,
    minimum height=1.0cm, text centered, anchor=center},
  io/.style={draw, trapezium, trapezium left angle=74, trapezium right angle=106,
    minimum height=0.72cm, minimum width=2.3cm, text centered, anchor=center}
]
  ...
  \draw (seti.south) -- (0,8.72);
  \draw[->] (0,8.72) -- (test.north);
  \draw[->] (accn.south) -- (acct.north);
  \draw[->] (acct.south) -- (inc.north);
  \draw[->] (test.east) -- (2.0,8.08) -- (sub.north)
    node[pos=0.10, above=1pt] {否};
  \draw[->] (sub.south) -- (out.north);
  \draw[->] (out.south) -- (finish.north);

  % Yes branch and loop-back path.
  \draw[->] (test.west) -- (-2.0,8.08) -- (accn.north)
    node[pos=0.10, above=1pt] {是};
  ...
  \draw[->] (inc.west) -- (-3.65,3.65) -- (-3.65,8.72) -- (-0.35,8.72);
  \draw (-0.35,8.72) -- (0,8.72);
\end{tikzpicture}
