...
\begin{tikzpicture}[
  ...
  \path[use as bounding box] (-2.10,0.45) rectangle (5.10,9.65);

  \node[terminal] (start) at (0,9.30) {开始};
  \node[io, minimum width=2.45cm] (input) at (0,8.00) {输入 $n,m$};
  \node[process, minimum width=2.60cm] (init) at (0,6.72)
    {$k=1,p=1$};
  \node[process, minimum width=3.55cm] (updateP) at (0,5.35)
    {$p=p(n-m+k)$};
  \node[decision] (test) at (0,3.85) {$k<m$};
  \node[process, minimum width=2.25cm] (inc) at (3.35,5.35)
    {$k=k+1$};
  \node[io] (output) at (0,2.45) {输出 $p$};
  \node[terminal] (finish) at (0,1.10) {结束};

  \draw[->] (start.south) -- (input.north);
  \draw[->] (input.south) -- (init.north);
  \coordinate (merge) at (0,6.05);
  \draw (init.south) -- (merge);
  \draw[->] (merge) -- (updateP.north);
  \draw[->] (updateP.south) -- (test.north);

  % Yes branch: right, then upward into the increment node.
  \draw (test.east) -- node[branchlabel, above=2pt] {是} (3.35,3.85);
  \draw[->] (3.35,3.85) -- (3.35,5.01) -- (inc.south);
  \draw[->] (inc.north) -- (3.35,6.05) -- (merge);
