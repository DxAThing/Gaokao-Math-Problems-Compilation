...
\begin{tikzpicture}[
  ...
  % Three millimetres below the formula box keeps the return line clear.
  \coordinate (return-level) at (0,5.95);
  \coordinate (return-arrow-tip) at (0.35,5.95);
  ...
  \draw (minus.north) -- (3.25,5.95);
  % The return arrow stops before the degree-3 merge, then joins without a second tip.
  \draw[flowarrow] (3.25,5.95) -- (return-arrow-tip);
  \draw (return-arrow-tip) -- (return-level);
  \node[branchlabel] at (1.75,4.85) {是};

  % The first-test yes path and second-test no path merge immediately before output.
  \coordinate (outleft-arrow) at (-0.35,2.85);
  \coordinate (outdown-arrow) at (0,3.20);
  \draw[flowarrow] (const.south) -- (outleft) -- (outleft-arrow);
  \draw (outleft-arrow) -- (outjoin);
  \draw[flowarrow] (test2.south) -- (outdown-arrow);
  \draw (outdown-arrow) -- (outjoin);
