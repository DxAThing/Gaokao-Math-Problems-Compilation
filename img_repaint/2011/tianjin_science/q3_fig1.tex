...
\begin{tikzpicture}[
  ...
  % Keep the shared stem clear of the init box; return arrow stops before the junction.
  ...
  \coordinate (loop-tip) at (-0.35,8.35);
  \draw (test.west) -- node[above=2pt] {否} (loop-left);
  \draw (loop-left) -- (loop-entry);
  \draw[flowarrow] (loop-entry) -- (loop-tip);
  \draw (loop-tip) -- (loopjoin);
