...
\begin{tikzpicture}[
  ...
  \node[flow,rectangle,minimum width=2.00cm,minimum height=0.62cm]
    (updatea) at (2.64,5.98) {\(a=2a+1\)};
  \node[flow,rectangle,minimum width=2.00cm,minimum height=0.62cm]
    (updatek) at (2.64,4.73) {\(k=k+1\)};
  \node[flow,diamond,aspect=2.75,minimum width=2.45cm,minimum height=0.72cm]
    (check) at (2.64,3.48) {\(a>10\)};
  \path[flow] (1.90,2.44) -- (3.40,2.44) -- (3.20,1.75) -- (1.65,1.75) -- cycle;
  \node at (2.54,2.09) {输出 \(k\)};
  \node[flow,rounded corners=0.16cm,minimum width=1.05cm,minimum height=0.64cm]
    (finish) at (2.64,0.78) {结束};
  ...
  \draw[->,flow] (check.south) -- node[pos=0.28,right=0.06cm] {是} (2.64,2.44);
  \draw[->,flow] (2.64,1.75) -- (finish.north);

  % The false branch has its own left return path and re-enters updatea.
  \coordinate (loop-left) at (0.45,3.48);
  ...
  \coordinate (loop-top) at (0.45,6.55);
  \coordinate (loop-merge) at (2.64,6.55);
  \draw[flow] (check.west) -- (loop-left) -- (loop-top);
  \draw[->,flow] (loop-top) -- (loop-merge);
  \draw[flow] (loop-merge) -- (updatea.north);
  \node[anchor=east,inner sep=0pt] at (1.33,3.85) {否};
\end{tikzpicture}
