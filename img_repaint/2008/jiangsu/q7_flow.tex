...
\begin{tikzpicture}[
  ...
  \node[flowbox,minimum width=2.70cm] (increment) at (-3.35,5.30)
    {$i\leftarrow i+1$};
  ...
  % 否分支回到输入框；回线不与输出分支共线。
  \draw[flowarrow] (test.west) -- (-3.35,3.72) -- (-3.35,5.30)
    -- (increment.south);
  \draw[flowarrow] (increment.north) -- (-3.35,6.80) -- (input.west);
  \node[anchor=east] at (-1.70,3.84) {N};
  ...
  \path[use as bounding box] (-4.75,0.30) rectangle (2.35,11.30);
\end{tikzpicture}
