...
\begin{tikzpicture}[
    ...
    \draw (first.south) -- node[branchlabel,left=2pt] {否} (0,4.65);
    \draw[->] (0,4.65) -- (second.north);
    \draw[->] (first.east) -- node[branchlabel,above=1pt] {是} (3.30,6.42)
        -- (setb.north);
    \draw[->] (setb.south) -- (3.30,4.65) -- (0,4.65);

    % Second decision branch and the output path.
    \draw (second.south) -- node[branchlabel,left=2pt] {否} (0,1.95);
