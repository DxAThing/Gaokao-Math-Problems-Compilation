...
\begin{tikzpicture}[baseline=(current bounding box.center), x=1cm, y=1.18cm,
  ...
  % The two branches meet at an unarrowed junction; one arrow then enters i=i+1.
  % The odd-branch arrow tip is kept 3 mm from that junction.
  \draw[line width=0.45pt] (seven.south) -- (0,-6.20);
  \draw[flowarrow] (sodd.south) -- (3.10,-6.20) -- (0.30,-6.20);
  \draw[line width=0.45pt] (0.30,-6.20) -- (0,-6.20);
  \draw[flowarrow] (0,-6.20) -- (inc.north);
  \draw[flowarrow] (inc) -- (gefour);
  % The loop returns with a right-pointing arrow into the trunk above the
  % decision; the main trunk retains its separate downward arrowhead.
  \draw[line width=0.45pt] (gefour.west) -- (-3.30,-8.25) -- (-3.30,-1.60);
  \draw[flowarrow] (-3.30,-1.60) -- (-0.30,-1.60);
  \draw[line width=0.45pt] (-0.30,-1.60) -- (0,-1.60);
