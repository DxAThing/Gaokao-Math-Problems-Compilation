...
\begin{tikzpicture}[
  ...
  \draw[->] (increment.west) -- (loop-join);

  % 否 branch terminates independently; the loop return is the sole horizontal back edge.
  \draw[->] (test.south) -- node[flowtext, right=2pt] {否} (output.north);
  \draw[->] (output.south) -- (finish.north);
  \node[flowtext, anchor=south east] at (1.85,8.12) {是};
