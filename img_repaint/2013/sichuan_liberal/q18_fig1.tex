...
\begin{tikzpicture}[
    ...
    \node[process, minimum width=1.85cm] (y3) at (0,3.15) {$y=3$};
    \node[io, minimum width=2.25cm] (output) at (0,1.72) {输出 $y$};
    ...
    % Keep the two inward side arrowheads and the separate downward arrows.
    ...
    \draw[->] (merge-right) -- (merge);
    \draw[->] (merge-left) -- (merge);
    \draw[->] (y3.south) -- (merge);
