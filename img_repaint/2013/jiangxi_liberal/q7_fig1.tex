...
\begin{tikzpicture}[
  ...
  \node[flowbox,minimum width=2.85cm] (even) at (0,4.70) {$S=2*i+2$};
  \coordinate (branchmerge) at (0,3.98);
  \node[decision,minimum width=4.60cm] (test) at (0,3.30) {};
  \node[io,minimum width=2.10cm] (output) at (0,1.82) {输出 $i$};
  ...
  \draw[->] (odd.south) -- (4.00,3.98) -- (branchmerge);
  \draw[->] (even.south) -- (branchmerge);
  ...
  \draw[->] (test.west) -- (loop-left) -- (loop-entry) -- (preinc);
  \draw[->] (test.south) -- (output.north);
  \draw[->] (output.south) -- (finish.north);

  \node[branchlabel,anchor=south] at (2.72,6.50) {否};
  \node[branchlabel,anchor=west] at (0.22,5.35) {是};
  \node[branchlabel,anchor=east] at (-2.85,3.08) {是};
  \node[branchlabel,anchor=west] at (0.22,2.72) {否};
