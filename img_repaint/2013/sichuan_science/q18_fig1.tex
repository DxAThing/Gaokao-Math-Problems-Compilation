...
\begin{tikzpicture}[
  ...
  % Keep the incoming arrows directional while reserving the exact junction
  % for plain connector stubs (avoids stacked arrowheads at one point).
  \draw[->] (yone.south) -- (4.05,3.95) -- (0.35,3.95);
  \draw (0.35,3.95) -- (merge);
  \draw[->] (ytwo.south) -- (-4.05,3.95) -- (-0.35,3.95);
  \draw (-0.35,3.95) -- (merge);
  \draw[->] (ythree.south) -- (0,4.30);
  \draw (0,4.30) -- (merge);
