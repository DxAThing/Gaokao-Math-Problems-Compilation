% Source: source/普通高考/2015/2015陕西理.pdf, page 1, question 8.
% Flowchart topology: 开始 -> 输入 x -> x=x-2 -> x>=0?; 是 loops to the
% entry above x=x-2, 否 -> y=3^{-x}+1 -> 输出 y -> 结束.
\begin{tikzpicture}[
  ...
  \node[process, minimum width=3.05cm] (update) at (0,7.55) {$x=x-2$};
  \node[decision] (test) at (0,5.80) {$x\geqslant 0$};
  \node[process, minimum width=3.75cm] (calc) at (0,4.05) {$y=3^{-x}+1$};
