...
\begin{tikzpicture}[
    ...
    % separate downward arrow into the decision node.  Leave a short
    % unarrowed run into the junction so the arrow tip is not at the shared
    % degree-3 point (the direction remains explicit and the topology is
    % unchanged).
    \draw[->] (loop-top) -- (-0.36,6.55);
    \draw (-0.36,6.55) -- (0,6.55);
