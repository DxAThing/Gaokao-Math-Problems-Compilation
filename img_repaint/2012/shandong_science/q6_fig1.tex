...
\begin{tikzpicture}[
  ...
  \node[process, minimum width=4.35cm] (initialize) at (0,7.05)
    {$P=0,Q=1,n=0$};
  \node[decision] (test) at (0,5.45) {$P\leqslant Q$};
  \node[process, minimum width=3.35cm] (updateP) at (0,3.98)
    {$P=P+a^n$};
  \node[process, minimum width=3.35cm] (updateQ) at (0,2.55)
    {$Q=2Q+1$};
  \node[process, minimum width=3.35cm] (updateN) at (0,1.12)
    {$n=n+1$};
  \node[io] (output) at (4.00,3.98) {输出 $n$};
