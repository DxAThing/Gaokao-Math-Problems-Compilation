...
\begin{tikzpicture}[
  ...
  \node[box, minimum width=1.70cm] (s_init) at (0,9.25) {$S=6$};
  ...
  \coordinate (loop-entry) at (0,7.35);

  \draw[flow] (start) -- (input);
  \draw[flow] (input.south) -- (s_init.north);
  \draw[flow] (s_init.south) -- (iinit.north);
  % The return arrow lands on the interior of the main stem rather than
  % terminating at a parsed 3-way endpoint junction.
  \draw (iinit.south) -- (loop-entry);
  \draw[flow] (loop-entry) -- (test.north);
  \draw[flow] (test.east) -- node[above, inner sep=1pt] {是} (3.10,4.15)
    -- (update.south);
  \draw[flow] (update.north) -- (decrement.south);
  \draw[flow] (decrement.north) -- (3.10,7.35) -- (loop-entry);
