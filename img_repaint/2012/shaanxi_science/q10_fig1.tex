...
\begin{tikzpicture}[
    ...
    \coordinate (loop-entry) at (0,8.65);

    \draw[->] (start.south) -- (init.north);
    \draw (init.south) -- (loop-entry);
    \draw[->] (loop-entry) -- (random.north);
    \draw[->] (random.south) -- (inside.north);

    \draw[->] (inside.south) -- node[branchlabel, right=2pt] {是} (countM.north);
    \draw (inside.east) -- (4.40,6.35) -- (4.40,5.60) -- (3.00,5.60);
    \draw[->] (3.00,5.60) -- (countN.north);
    ...
    \draw[->] (countN.south) -- (3.00,4.10) -- (0.35,4.10) -- (0,4.10);
    ...
    \draw (test.west) -- (-4.10,1.85) -- (-4.10,8.65);
    \draw[->] (-4.10,8.65) -- (loop-entry);
    \node[branchlabel] at (-3.55,2.13) {否};

    \path[use as bounding box] (-4.35,-2.75) rectangle (4.65,11.30);
\end{tikzpicture}
