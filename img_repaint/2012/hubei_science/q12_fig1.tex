...
\begin{tikzpicture}[
  ...
  \node[process,minimum width=1.72cm] (sum) at (0,-2.40) {$s=s+a$};
  ...
  \node[io,minimum width=1.85cm] (output) at (2.80,-5.42) {输出 $s$};
  \node[terminal] (finish) at (2.80,-6.60) {结束};
  ...
  % Keep the loop return 2.8 mm below init's lower edge; place its arrowhead
  % in the middle of the horizontal return so the shared merge has no loop tip.
  \coordinate (merge-level) at (0,-1.74);
  \coordinate (loop-arrow-start) at (-1.80,-1.74);
  \coordinate (loop-arrow-tip) at (-1.05,-1.74);
  \draw (incN.south) -- (loop-left) -- (loop-left |- merge-level) -- (loop-arrow-start);
  \draw[->] (loop-arrow-start) -- (loop-arrow-tip);
  \draw (loop-arrow-tip) -- (merge-level);
