...
\begin{tikzpicture}[
    ...
    \path[use as bounding box] (-4.45,0.10) rectangle (4.90,13.35);

    \node[terminal] (start) at (0,13.00) {开始};
    \node[io, minimum width=5.65cm] (input) at (0,11.70)
        {输入 $N,a_1,a_2,\ldots,a_N$};
    \node[process, minimum width=5.65cm] (init) at (0,10.25)
        {$k=1,A=a_1,B=a_1$};
    ...
    \coordinate (loop-merge) at (0,9.50);
    ...
    \draw (testB.south) -- node[right=2pt, inner sep=0pt] {否}
        (0,4.65);
    \draw (setB.south) -- (-3.00,4.65) -- (0,4.65);
    \draw (setA.south) -- (3.00,4.65) -- (0,4.65);
    \draw[->] (0,4.65) -- (testN.north);
    ...
    \draw[->] (testN.east) -- node[above=2pt, inner sep=0pt] {否}
        (4.30,3.60) -- (4.30,7.58) -- (inc.south);
    \draw[->] (inc.north) -- (4.30,9.50) -- (0.35,9.50) -- (loop-merge);
\end{tikzpicture}
