...
\begin{tikzpicture}[
  ...
  \coordinate (loop-entry) at (0,5.72);

  \draw[flowarrow] (start.south) -- (input.north);
  \draw[flowarrow] (input.south) -- (init.north);
  % Keep the loop arrow tip on the main stem interior, not at a 3-way
  % endpoint junction; the stem itself remains one directed edge.
  \draw (init.south) -- (loop-entry);
  \draw[flowarrow] (loop-entry) -- (test.north);
  ...
  \draw[flowarrow] (loop-left-top) -- (loop-entry);
