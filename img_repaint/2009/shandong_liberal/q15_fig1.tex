% Original PDF: source/普通高考/2009/2009山东文.pdf, question 15.
% Nodes and directed edges are transcribed from the source flowchart:
% 开始 -> initialization -> T>S?; 是 -> 输出T -> 结束; 否 -> S=S+5
% -> n=n+2 -> T=T+n -> loop back to the left side of T>S?.
\begin{tikzpicture}[
  >=Stealth,
  line width=0.55pt,
  line cap=round,
  ...
  % The return arrow enters the decision's left point and is kept separate from
  % the vertical computation path.
