...
\begin{tikzpicture}[
  ...
  % Keep the full update box and recurrence box inside the standalone bounding box.
  \path[use as bounding box] (-2.55,0.20) rectangle (7.10,10.05);
  ...
  \draw[->] (test.east) -- node[pos=0.25,branchlabel,above=5pt] {是} (update.west);
  \draw[->] (update.north) -- (inc.south);
  % The return branch joins the main vertical line below the initialization box,
  % matching the source flowchart and keeping the arrow clear of the node text.
  \draw[->] (inc.west) |- (0,6.15);
\end{tikzpicture}
