...
\begin{tikzpicture}[x=1cm,y=0.90cm,>=stealth,
  ...
  flowbox/.style={draw,anchor=center,minimum height=0.62cm,inner xsep=4pt,inner ysep=2pt,
  ...
  io/.style={draw,shape=trapezium,trapezium left angle=78,trapezium right angle=102,
    trapezium stretches=false,anchor=center,
    minimum height=0.72cm,inner xsep=4pt,inner ysep=2pt,align=center,
    execute at begin node={\setlength{\parindent}{0pt}}},
  flowarrow/.style={->,line width=0.45pt},
]
  % Main vertical sequence.
  \node[flowbox,rounded corners=0.16cm,minimum width=1.10cm] (start) at (0,10.80) {开始};
  \node[io,minimum width=2.85cm] (input) at (0,9.58) {输入 $a,b$};
  \node[flowbox,minimum width=2.30cm] (init) at (0,8.65) {$n=0,s=0$};
  ...
  \node[flowbox,diamond,aspect=2.75,minimum width=2.72cm,minimum height=0.84cm] (test) at (0,2.05) {$s>16$};
  \node[io,minimum width=2.25cm] (output) at (0,0.78) {输出 $n$};
  \node[flowbox,rounded corners=0.16cm,minimum width=1.10cm] (finish) at (0,-0.48) {结束};
  ...
  \coordinate (entry) at (-2.28,7.80);
  \draw (test.west) -- node[above=2pt] {否} (left) -- (entry);
  \coordinate (loop-arrow-tip) at (-0.70,7.80);
  \draw[flowarrow] (entry) -- (loop-arrow-tip);
