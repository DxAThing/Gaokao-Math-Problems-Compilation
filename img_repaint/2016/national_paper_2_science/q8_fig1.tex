...
\begin{tikzpicture}[
    ...
    \coordinate (loop-return) at (3.25,6.96);
    \coordinate (loop-arrow-tip) at (0.70,6.96);
    \draw (test.east) -- node[branchlabel,above] {否} (loop-right) -- (loop-return);
    % The return arrow runs to the actual merge endpoint; the downward edge
    % into 输入 a remains the distinct continuation from that merge.
    \draw[->] (loop-return) -- (loop-arrow-tip) -- (loop-top);
\end{tikzpicture}
