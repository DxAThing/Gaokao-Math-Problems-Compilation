...
\begin{tikzpicture}[
  ...
  \node[process, minimum width=2.75cm, minimum height=1.05cm] (update) at (0,4.15)
    {$\begin{gathered}s=s\cdot x+a\\ k=k+1\end{gathered}$};
  ...
  % Decision labels and branches.
  \node (no-label) at (1.85,2.98) {否};
  \node (yes-label) at (0.62,1.98) {是};
  ...
  \draw[->] (check.east) -- (loopRight) -- (loopTop) -- (loopJoin);
\end{tikzpicture}
