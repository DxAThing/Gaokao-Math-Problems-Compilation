...
\begin{tikzpicture}[
    ...
    \path[use as bounding box] (-4.20,-0.48) rectangle (3.55,11.42);
    ...
    \node[decision, minimum width=3.65cm, minimum height=1.10cm] (testS)
        at (0,7.02) {$S\geq6?$};
    \node[process, minimum width=2.45cm] (double) at (-2.80,5.42) {$S=2S$};
    ...
    % The return edge carries the sole arrowhead; the initial trunk remains a shaft
    % into the same decision, matching the source junction without stacked heads.
    ...
    \coordinate (noTurn) at (-2.80,7.02);
    \draw[->] (testS.west) -- node[above left] {否} (noTurn) -- (double.north);
    ...
    \coordinate (loopTop) at (2.90,7.80);
    \coordinate (loopJoin) at (0.30,7.80);
    \draw (testn.east) -- node[above] {否} (loopRight) -- (loopTop);
    \draw[->] (loopTop) -- (loopJoin);
    \draw[-] (loopJoin) -- (0,7.80) -- (testS.north);
\end{tikzpicture}
