...
\begin{tikzpicture}[
    ...
    \node[process, minimum width=2.65cm] (sub) at (1.25,5.42) {$S=S-6$};
    ...
    \draw[->] (testS.south) -- node[right=2pt] {是} (sub.north);
    \draw[->] (sub.south) -- (inc.north east);
    ...
    \draw (testS.west) -- (noTurn) -- (noDrop);
    \draw[->] (noDrop) -- (double.north);
    \draw[->] (double.south) -- (inc.north west);
    \node[anchor=south east, inner sep=1pt] at (-1.80,7.16) {否};

    % Increment and second decision.
    \draw[->] (inc.south) -- (testn.north);
    \draw[->] (testn.south) -- node[right=2pt] {是} (output.north);
    ...
    \draw (testn.east) -- (loopRight) -- (loopTop);
    \draw[->] (loopTop) -- (loopJoin) -- (testS.north);
    \node[anchor=south west, inner sep=1pt] at (1.62,2.76) {否};
\end{tikzpicture}
