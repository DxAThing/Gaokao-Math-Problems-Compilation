...
\begin{tikzpicture}[
  ...
  % The forward stem and loop return share one arrow entering the decision;
  % no second arrowhead is placed on the loop's horizontal apex segment.
  \draw (input.south) -- (0,7.85);
  \draw[->] (0,7.85) -- (test.north);
  ...
  \draw (stepk.west) -- (-1.80,3.11) -- (-1.80,7.85) -- (0,7.85);
  % False branch bypasses the loop and follows the right-side route into output.east;
  % it then continues output -> finish.  There is no k -> output connector.
  \draw[->] (test.east) -- node[flowtext, above] {否} (2.00,7.40)
    -- (2.00,1.68) -- (output.east);
\end{tikzpicture}
