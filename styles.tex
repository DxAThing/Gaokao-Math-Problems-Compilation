...
\newcount\examfigure@lastbox
\examfigure@lastbox=-1
...
\def\examfigure@queuepending{%
    \expandafter\ifx\csname examfigure@queued@\number\@currbox\endcsname\relax
        ...
        \expandafter\xdef\csname examfigure@reserved@\number\@currbox\endcsname
            {\the\dimexpr\ht\@currbox+\textfloatsep\relax}%
        \global\advance\examfigure@pendingheight by \dimexpr\ht\@currbox+\textfloatsep\relax
        \global\examfigure@tcbreservetrue
    \fi
}
\def\examfigure@queueconsumed{%
    \expandafter\ifx\csname examfigure@queued@\number\@currbox\endcsname\relax\else
        \global\examfigure@tcbreadytrue
    \fi
}
...
\def\@xfloat#1[#2]{%
    ...
    \ifexamfigure@recordsource
        \global\examfigure@lastbox=\@currbox\relax
        \typeout{EXAM-XFLOAT curr=\number\@currbox source=\number\examfigure@sourcepage}%
...
\def\@addtotoporbot{%
    \typeout{EXAM-ADDTOP-BEFORE curr=\number\@currbox page=\the\c@page mapped=\expandafter\ifx\csname examfigure@sourcepage@\number\@currbox\endcsname\relax0\else1\fi insert=\if@insert1\else0\fi}%
    ...
    \else
        \ifnum\csname examfigure@sourcepage@\number\@currbox\endcsname<\c@page
            \examfigure@orig@addtotoporbot
            \typeout{EXAM-ADDTOP-AFTER curr=\number\@currbox page=\the\c@page insert=\if@insert1\else0\fi}%
            \if@insert\examfigure@queueconsumed\fi
        \else
            \@addtobot
...
\def\@addtocurcol{%
    \typeout{EXAM-ADDCUR-BEFORE curr=\number\@currbox page=\the\c@page mapped=\expandafter\ifx\csname examfigure@sourcepage@\number\@currbox\endcsname\relax0\else1\fi insert=\if@insert1\else0\fi}%
    \typeout{EXAM-ADDCUR curr=\number\@currbox last=\the\examfigure@lastbox page=\the\c@page insert=\if@insert1\else0\fi}%
    \examfigure@orig@addtocurcol
    \typeout{EXAM-ADDCUR-AFTER curr=\number\@currbox page=\the\c@page mapped=\expandafter\ifx\csname examfigure@sourcepage@\number\@currbox\endcsname\relax0\else1\fi insert=\if@insert1\else0\fi}%
    \if@insert\else
        \ifnum\@currbox=\examfigure@lastbox\relax
...
\def\examfigure@dispatch#1#2#3{%
    ...
    \else
        ...
        \else\ifx\examfigure@float\examfigure@modetop
            \global\advance\examfigure@pendingheight by \dimexpr\examfigureheight+\textfloatsep\relax
            \examfigure@emittop{#2}%
        \else
            \ifdim\dimexpr\examfigureheight+2\intextsep+\baselineskip\relax>\dimexpr\pagegoal-\pagetotal\relax
                \global\advance\examfigure@pendingheight by \dimexpr\examfigureheight+\textfloatsep\relax
            \fi
...
% Keep the patch narrow: only the non-floating, non-multicol page branch is
% changed, with the package's compression allowance retained.
\makeatletter
...
\patchcmd{\tcb@split@first}
  ...
  {\long\def\tcb@after@first{\par\global\examfigure@tcbreadytrue\pagebreak\tcb@enlbreak@next}}
  {}
  {\PackageError{styles}{Failed to patch tcolorbox first-break reserve hook}
    {The installed tcolorbox breakable implementation is incompatible with the figure layout guard.}}
\patchcmd{\tcb@split@first}
  {\long\def\tcb@after@middle{\par\vfill\eject\tcb@enlbreak@next}}
  {\long\def\tcb@after@middle{\par\vfill\global\examfigure@tcbreadytrue\eject\tcb@enlbreak@next}}
  {}
  {\PackageError{styles}{Failed to patch tcolorbox middle-break reserve hook}
    {The installed tcolorbox breakable implementation is incompatible with the figure layout guard.}}
\makeatother
