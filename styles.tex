...
\newcommand{\bitmapfigure}[2][]{%
    ...
    \begin{center}
    % Keep the image and its caption in one unbreakable vertical box.  A plain
    % center environment is a breakable list, so TeX could ship the image on
    % one page and captionof's paragraph on the next page.
...
\newcommand{\texfigure}[2][]{%
    ...
    \bitmappreparecaption{#2}%
    \begin{center}
    % Keep the figure body together when the surrounding paragraph reaches a
    % page boundary; this mirrors bitmapfigure without changing its options.
    \begin{minipage}{\linewidth}
        \centering
        \texinclude[#1]{#2}%
        \bitmapinsertcaption
...
\newcommand{\answerbegin}{%
    \problembodyend
    \ifshowanswer
        \begin{tcolorbox}[examanswer,breakable=false]
